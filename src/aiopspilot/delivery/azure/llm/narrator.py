...
Implements :class:`~aiopspilot.core.conversation.narrator.Narrator` by
...
- **Translator, never a judge.** The system prompt forbids the model
  ...
  tool call.
- **RBAC pre-filter.** :func:`~aiopspilot.core.conversation.narrator.format_prompt_tool_list`
...
_SYSTEM_PROMPT_TEMPLATE: Final[str] = (
    "You are the AIOpsPilot operator console narrator. Your ONLY job is to "
...
class WorkloadIdentitySync(Protocol):
    ...
    :class:`~aiopspilot.delivery.azure.dev_workload_identity.AzureCliWorkloadIdentity`
    satisfies it.
    """
