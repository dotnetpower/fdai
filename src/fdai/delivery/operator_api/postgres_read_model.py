"""Postgres-backed :class:`ConsoleReadModel` for production Operator API.
...
- **Same driver as the writer.** Uses ``psycopg`` 3 (already in
  ...
  the Operator API's three routes are low-frequency compared with the
  writer path.
...
from fdai.delivery.operator_api.persistence.postgres_projection import (
    aggregate_kpi,
    row_to_audit_item,
...
from fdai.delivery.operator_api.persistence.postgres_sql import (
...
class PostgresConsoleReadModel(ConsoleReadModel):
    ...
    async def list_incidents(
        ...
        *,
        ...
        from fdai.delivery.operator_api.routes.incident_projection import project_incidents
