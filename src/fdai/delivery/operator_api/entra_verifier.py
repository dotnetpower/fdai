"""Generic Entra ID JWT verifier for the console Operator API.

Concrete :data:`fdai.delivery.operator_api.auth.ClaimsVerifier` implementation
...
:class:`~fdai.delivery.operator_api.auth.Authenticator`.
"""
...
class EntraJwtVerifier:
    ...
    Satisfies the :data:`~fdai.delivery.operator_api.auth.ClaimsVerifier`
    structural type (``(token) -> claims``) by duck typing - it does not
    import the alias, only raises :class:`AuthenticationError` on failure,
    which :meth:`~fdai.delivery.operator_api.auth.Authenticator.authenticate`
    maps to HTTP ``401``.
    """
