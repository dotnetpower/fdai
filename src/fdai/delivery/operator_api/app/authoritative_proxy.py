"""Allowlisted local proxy to an authoritative Entra-protected Operator API."""
...
AUTHORITATIVE_OPERATOR_API_ENV = "FDAI_AUTHORITATIVE_OPERATOR_API_BASE_URL"
    ...
    async def __call__(self, scope: Scope, receive: Receive, send: Send) -> None:
        ...
        try:
            ...
            await _json_error(
                send,
                503,
                f"authoritative Operator API is unreachable: {type(exc).__name__}",
            )
...
def authoritative_read_proxy_from_env(
    ...
    base_url = env.get(AUTHORITATIVE_OPERATOR_API_ENV, "").strip()
...
def _validated_base_url(value: str) -> str:
    ...
    if parsed.scheme != "https" or not parsed.hostname:
        raise ValueError(f"{AUTHORITATIVE_OPERATOR_API_ENV} MUST be an absolute HTTPS URL")
    if parsed.username is not None or parsed.password is not None:
        raise ValueError(f"{AUTHORITATIVE_OPERATOR_API_ENV} MUST NOT contain credentials")
    if parsed.query or parsed.fragment or parsed.path not in ("", "/"):
        raise ValueError(f"{AUTHORITATIVE_OPERATOR_API_ENV} MUST contain only an origin")
