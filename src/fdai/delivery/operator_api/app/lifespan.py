"""ASGI lifespan assembly for Operator API background services."""
        ...
        if web_search_resolver is not None:
            ...
            if callable(verify):
                try:
                    ...
                    logger.warning(
                        "operator_api_web_search_readiness_failed",
            ...
            probe_tasks.append(
                asyncio.create_task(
                    _run_correlated_latency_probe(
                        ...
                        correlation_id="operator-api:web-search-latency-probe",
                    )
                )
            )
        try:
            ...
            for callback in config.shutdown_callbacks:
                try:
                    ...
                    logger.warning("operator_api_shutdown_callback_failed", exc_info=True)
