"""Public configuration contract for the console Operator API factory."""
...
class OperatorApiConfig:
    ...
    by never setting ``FDAI_OPERATOR_API_DEV_MODE`` on deployed
    revisions.
    """
    ...
    Requires ``FDAI_OPERATOR_API_LOCAL_AZURE_CLI=1`` and is refused in staging
    ...
    :mod:`fdai.delivery.operator_api.panels`). Empty by default so the
    ...
    When configured, the Operator API exposes a GET projection and an append-only
    ...
    Operator API strictly GET-only. The route consumes a
    ...
    uses this instead of giving the Operator API an executor identity."""
    ...
    the ingest topic. ``None`` (the default) keeps the Operator API strictly
    GET-only. The ingress never executes a change - it only injects an
    event, so no privilege boundary is crossed. See
    :mod:`fdai.delivery.operator_api.routes.webhook`."""
    ...
    :mod:`fdai.delivery.operator_api.dev.local`) opts in. See
    :mod:`fdai.delivery.operator_api.live_stream` for the read-only /
    fan-out contract."""
    ...
    the shared sink via
    :class:`~fdai.delivery.operator_api.streaming.provision_stream.SseProvisionPublisher`.
    Default ``None``. See
    :mod:`fdai.delivery.operator_api.provision_stream`."""
    ...
    :mod:`fdai.delivery.operator_api.streaming.agent_activity_stream`."""
    ...
    See :mod:`fdai.delivery.operator_api.blast_radius`."""
    ...
    never mutates state. See :mod:`fdai.delivery.operator_api.rule_catalog`."""
    ...
    :class:`~fdai.delivery.operator_api.routes.chat.ChatBackend`), registers
    ``POST /chat`` for the console's screen-aware conversational
    surface. The backend is a read-only translator - it never issues a
    privileged call. Wire
    :class:`~fdai.delivery.operator_api.routes.chat.OpenAiCompatibleChatBackend`
    (or a fork adapter) at composition root; leave ``None`` to keep the endpoint
    ...
    cancellation remain outside the Operator API."""
    ...
    exposes Thor's executor identity to the Operator API."""
    ...
    (:class:`~fdai.delivery.operator_api.routes.console_action.ConsoleActionSubmitter`).
    ...
    See :mod:`fdai.delivery.operator_api.pantheon`."""
    ...
    mutation. See :mod:`fdai.delivery.operator_api.routes.stewardship`."""
    ...
    :class:`~fdai.delivery.operator_api.routes.workflow_authoring.WorkflowAuthoringConfig`),
    ...
    :class:`~fdai.delivery.operator_api.routes.reporting.ReportingConfig`),
    ...
    :mod:`fdai.delivery.operator_api.reporting` and
