...
from fdai.delivery.operator_api.routes.skill_runtime import (
    OPERATOR_API_SKILL_TOOL_IDS,
    ...
    *,
    ...
    async def startup() -> None:
        ...
        bundle_catalog = load_skill_bundle_catalog(
            ...
            available_tools=OPERATOR_API_SKILL_TOOL_IDS,
...
def _load_trusted_publishers(env: Mapping[str, str]) -> dict[str, bytes]:
    ...
    if not isinstance(decoded, dict):
        raise ProdOperatorApiConfigError("trusted skill publisher registry MUST be a JSON object")
    publishers: dict[str, bytes] = {}
    for source, public_key in decoded.items():
        if not isinstance(source, str) or not source.strip():
            raise ProdOperatorApiConfigError("trusted skill publisher source MUST be non-empty")
