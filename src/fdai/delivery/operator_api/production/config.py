...
_CORS_ORIGINS_ENV: Final[str] = "FDAI_OPERATOR_API_CORS_ALLOW_ORIGINS"
_STATEMENT_TIMEOUT_ENV: Final[str] = "FDAI_OPERATOR_API_STATEMENT_TIMEOUT_MS"
_CONNECT_TIMEOUT_ENV: Final[str] = "FDAI_OPERATOR_API_CONNECT_TIMEOUT_S"
...
class ProdOperatorApiConfigError(ValueError):
...
def _require_env(environ: Mapping[str, str], key: str) -> str:
    ...
    if not value:
        raise ProdOperatorApiConfigError(
            f"{key} is required to build the production Operator API; set it in "
...
def _check_required_env(environ: Mapping[str, str], keys: Sequence[str]) -> None:
    ...
    if missing:
        raise ProdOperatorApiConfigError(
            ...
            f"Operator API and are missing or empty: {', '.join(missing)}"
        )
...
def _parse_cors_origins(raw: str | None) -> tuple[str, ...]:
    ...
    The shared :func:`~fdai.delivery.operator_api.main.build_app` only refuses
...
def _parse_positive_int(environ: Mapping[str, str], key: str, default: int) -> int:
    ...
    try:
        ...
        raise ProdOperatorApiConfigError(f"{key}={raw!r} MUST be an integer") from exc
    if value < 1:
        raise ProdOperatorApiConfigError(f"{key}={value} MUST be >= 1")
    return value
