...
console Operator API). Framework-neutral by design: neither FastAPI nor
Starlette is imported at module load. The Operator API composition binds this
into an ASGI app; unit tests exercise it with plain callables.

Boundary contract
-----------------

The Operator API is responsible for **JWT signature + audience + issuer + expiry
...
On any failure, raise :class:`AuthenticationError` - the Operator API layer
...
class AuthenticationError(Exception):
    ...
    Operator API layer maps this to HTTP ``401 Unauthorized``. Never leaks the
...
def build_authenticator(*, verifier: ClaimsVerifier, resolver: RoleResolver) -> Authenticator:
    ...
    Convenience factory the Operator API composition root calls once at
...
class UnsafeClaimsExtractor:
    ...
    verifier (:class:`~fdai.delivery.operator_api.entra_verifier.EntraJwtVerifier`:
    JWKS + audience + issuer + expiry) replaces this.

    Kept in-tree so unit tests can exercise the Operator API surface without
