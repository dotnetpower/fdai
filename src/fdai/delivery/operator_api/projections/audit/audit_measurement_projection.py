"""Aggregate audit records into a read-only autonomy payload.

The module owns deterministic projection only. It does not own routes,
authorization, persistence, policy decisions, approval, or execution.
"""
...
def merge_items(*groups: Sequence[AuditItem]) -> tuple[AuditItem, ...]:
    """Deduplicate by sequence and return newest-first audit order."""
...
def values(items: Sequence[AuditItem], key: str, *, baseline: bool = False) -> list[float]:
    """Return only the latest numeric observation per event and metric key."""
...
def guards(items: Sequence[AuditItem]) -> list[Mapping[str, Any]]:
    """Return the first newest-to-oldest non-empty valid guard set."""
    for item in items:
