"""Public compatibility surface for the live Operator API stream.

The three existing GET routes (:mod:`fdai.delivery.operator_api.main`) return
...
- **Read-only.** ``GET`` only; there is no matching mutating verb. The
  ...
  holds identically to the other Operator API routes.
- **Never shares the executor identity.** The stream is a consumer of
  ...
  executor.
- **Opt-in.** :func:`~fdai.delivery.operator_api.main.build_app` registers
...
- same-origin cookie session (recommended: Static Web App + Operator API on
...
from fdai.delivery.operator_api.streaming.contracts import (
    LiveEmitter,
    LiveStageProducer,
    LiveStreamConfig,
)
from fdai.delivery.operator_api.streaming.live_route import make_live_stream_route
from fdai.delivery.operator_api.streaming.sse_protocol import (
    _MAX_SSE_DATA_CHARS,
...
from fdai.delivery.operator_api.streaming.synthetic_emitter import SyntheticLiveEmitter
