...
In production the Operator API pod does not run the pipeline; the real
...
through :func:`~fdai.delivery.operator_api.streaming.agent_activity_projection.project_stage`,
...
tees an in-process ControlLoop (the dev harness), this broadcaster is the cross-replica path: it
needs only the shared event bus and the agent-activity publisher, so the composition root binds it
to the real Kafka bus + SSE sink.
