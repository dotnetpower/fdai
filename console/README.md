...
Apps and adds the allowlisted Manual Studio files under `/manuals`. A production
Console defaults to that same-origin path, while `VITE_MANUAL_STUDIO_URL` can
select a separately hosted Manual Studio. The publisher verifies the deployed
Console entry asset, manual catalog, and manual library against the local files.
Custom domain, CSP headers, and MSAL app-registration values are supplied by the
fork; the upstream repo ships schema and empty defaults only.
...
| `VITE_MANUAL_STUDIO_URL` | Optional HTTPS base URL for a separately hosted manual catalog. Local development defaults to `http://127.0.0.1:5474`; production defaults to the Console's same-origin `/manuals` path. |
