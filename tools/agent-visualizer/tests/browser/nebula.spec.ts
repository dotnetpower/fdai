...
test("nebula is a local static backdrop that preserves layout and graph controls", async ({ page }, testInfo) => {
  const errors: string[] = [];
  ...
  const contrast = await stage.evaluate(async (element) => {
  ...
  await stage.evaluate((element) => { element.style.backgroundImage = "none"; });
  expect(await geometry()).toEqual(decoratedGeometry);
  const before = await stage.screenshot();
  await stage.evaluate((element) => { element.style.removeProperty("background-image"); });
  ...
  await expect(stage).toHaveCSS("background-image", background);
...
test("nebula remains bounded on constrained and mobile screens", async ({ page }) => {
  ...
  for (const viewport of [{ width: 993, height: 641 }, { width: 390, height: 844 }, { width: 320, height: 844 }]) {
    ...
    await expect(page.locator("#stage")).not.toHaveCSS("background-image", "none");
