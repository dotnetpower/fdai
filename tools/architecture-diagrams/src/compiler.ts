...
export async function compileDiagram(spec: DiagramSpec): Promise<DiagramArtifact[]> {
  ...
  const manifest = {
    ...
    ...(spec.canvas.profile === "network-azure-reference" ? { profile: spec.canvas.profile } : {}),
    ...(spec.canvas.networkPreset ? { networkPreset: spec.canvas.networkPreset } : {}),
    ...
    ...(spec.kind === "network" ? { groups: spec.groups.map((group) => ({
      id: group.id,
      kind: group.kind,
      label: group.label,
      ...(group.networkRole ? { networkRole: group.networkRole } : {}),
      ...(group.addressPrefixes ? { addressPrefixes: group.addressPrefixes } : {}),
      ...(group.region ? { region: group.region } : {}),
      ...(group.availabilityZones ? { availabilityZones: group.availabilityZones } : {}),
    })) } : {}),
    nodes: spec.nodes.map((node) => ({
      ...
      ...(node.networkRole ? { networkRole: node.networkRole } : {}),
      ...(node.addresses ? { addresses: node.addresses } : {}),
      ...(node.listener ? { listener: node.listener } : {}),
      ...(node.sku ? { sku: node.sku } : {}),
      ...(node.securityFacts ? { securityFacts: node.securityFacts } : {}),
    })),
    edges: spec.edges.map((edge) => ({
      ...
      ...(edge.connectionKind ? { connectionKind: edge.connectionKind } : {}),
      ...(edge.direction ? { direction: edge.direction } : {}),
      ...(edge.trafficClass ? { trafficClass: edge.trafficClass } : {}),
      ...(edge.policy ? { policy: edge.policy } : {}),
      ...(edge.protocol ? { protocol: edge.protocol } : {}),
      ...(edge.port ? { port: edge.port } : {}),
      ...(edge.nextHop ? { nextHop: edge.nextHop } : {}),
      ...(edge.sourceEvidence ? { sourceEvidence: edge.sourceEvidence } : {}),
    })),
