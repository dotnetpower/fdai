...
test("repository migration inventory validates every task-owned Mermaid block", async () => {
  ...
  assert.equal(plan.totalBlocks, 72);
  assert.equal(plan.deferredBlocks, 2);
  assert.equal(plan.specs.length, 72);
  assert.equal(new Set(plan.specs.map((spec) => spec.id)).size, 72);
});
