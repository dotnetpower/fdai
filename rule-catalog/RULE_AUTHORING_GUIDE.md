...
Every `typical_parents` value must resolve to another entry in the same combined registry, and the
resulting parent graph must remain acyclic.

- Add bounded, non-blank English and Korean `query_terms` when operators need to select the type in an
  ...
  never creates an implicit ambiguity. Collision checks use the runtime's Unicode, slash,
  backslash, underscore, case, and whitespace normalization.
...
- When two semantic types share one Azure ARM type, declare `azure_kind_tokens` on every variant.
  ...
  the shared ARM type, and aren't display labels. Missing or unmatched kind evidence remains
  unmapped instead of selecting a default. This compatibility mapping remains read-only provider
  metadata and is expected to move to the versioned Azure discovery-profile catalog described in
  the discovery design.

### Inventory query language

Natural-language inventory behavior is catalog-as-code in
