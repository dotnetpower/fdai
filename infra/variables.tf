...
variable "inventory_cron_expression" {
  description = "Cron for inventory change drains, due checks, and failed-attempt retries. A tick that is not due exits after one query, so the cadence sets change-detection latency rather than scan cost. Empty disables the job."
  type        = string
  default     = "* * * * *"
}
...
variable "inventory_change_min_interval_seconds" {
  description = "Floor between two change-triggered reconciliations. An observed provider change makes a scan due once the active snapshot is at least this old."
  type        = number
  default     = 120
...
variable "inventory_arg_requests_per_second" {
  description = "Sustained Azure Resource Graph request budget shared by every shard of one scan. Keeps a continuous background scan inside the provider quota instead of bursting into a throttle."
  type        = number
  default     = 3
