variable "name" {
  description = "Deployment-supplied Operator API Container App name."
  type        = string
}
...
variable "operator_api_identity_id" {
  description = "Dedicated Operator API user-assigned MI resource id (ACR pull + Key Vault secret read only)."
  type        = string
}

variable "operator_api_identity_client_id" {
...
variable "iam_directory_provider" {
  description = "Human identity directory adapter. Empty disables search; supported value is entra after User.Read.All admin consent is granted to the Operator API managed identity."
  type        = string
  default     = ""
}
...
variable "min_replicas" {
  description = "Minimum replicas. 1 keeps the Operator API always-on for the console."
  type        = number
  default     = 1
}
