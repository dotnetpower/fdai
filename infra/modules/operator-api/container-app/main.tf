# FDAI operator console Operator API - Azure Container App (+ migration job).
#
# Serves the read-only console API (`fdai.delivery.operator_api.prod:app`) with
...
# RBAC group resolution (see `src/fdai/delivery/operator_api/prod.py`).
#
...
moved {
  from = azurerm_container_app.read_api
  to   = azurerm_container_app.operator_api
}
...
resource "azurerm_container_app" "operator_api" {
  ...
  template {
    ...
    container {
      ...
      command = ["uvicorn"]
      args = [
        "fdai.delivery.operator_api.prod:app",
        "--factory",
        "--host",
        "0.0.0.0",
        "--port",
      ...
      env {
        ...
        value = var.operator_api_identity_client_id
      }
      env {
  ...
  lifecycle {
    precondition {
      ...
      error_message = "Operator API requires at least one real FDAI maintainer binding."
    }
    precondition {
      condition     = var.iam_directory_provider == "entra"
      error_message = "Operator API requires the Entra identity directory provider."
    }
    precondition {
      condition = alltrue([
      ...
      error_message = "Operator API requires stewardship bindings for every non-autonomous pantheon agent."
    }
  }
