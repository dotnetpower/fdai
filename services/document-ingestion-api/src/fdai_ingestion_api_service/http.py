"""Starlette transport for document upload and lifecycle operations."""
...
class IngestionGatewayConfig:
    """Immutable HTTP boundary and lifecycle configuration."""
...
def build_app(
    *,
    ...
    def authorize(request: Request, roles: tuple[Role, ...]) -> Principal:
        ...
        return authenticator.require_roles(
    ...
    async def lifespan(_app: Starlette) -> AsyncIterator[None]:
        for check in resolved.startup_checks:
            await check()
        tasks = [asyncio.create_task(service()) for service in resolved.background_services]
        try:
            yield
        finally:
            for task in tasks:
...
def _create_request(
    ...
    *,
    ...
    return CreateUploadRequest(
        ...
        access_descriptor_ref=managed_access_ref
        if default_reader_groups
        else str(body["access_descriptor_ref"]),
        reader_groups=tuple(
...
def _access_principals(principal: Principal) -> frozenset[str]:
    return frozenset(principal.groups | {f"role:{role.value}" for role in principal.roles})
