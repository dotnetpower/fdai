"""Minimal liveness and readiness server owned by the Executor process."""
...
class RuntimeHealthServer:
    """Serve bounded process-local liveness and readiness responses."""

    port: int
    host: str = "0.0.0.0"  # noqa: S104 - probes connect through the pod IP
    ...
    async def start(self) -> None:
        """Start the health listener once."""

        if self._server is None:
            self._server = await asyncio.start_server(self._handle, self.host, self.port)

    async def close(self) -> None:
        """Close the health listener and release its socket."""
