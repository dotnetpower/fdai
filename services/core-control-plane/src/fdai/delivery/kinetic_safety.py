...
class ExistingProposalKineticSafetyWriter:
    ...
    def __init__(
        ...
        *,
        ...
        prospective_lineage_readiness: ProspectiveLineageReadinessReader | None = None,
    ) -> None:
        self._proposal_store = proposal_store
        self._artifact_store = artifact_store
        self._action_types_by_name = dict(action_types_by_name)
        self._active_release = active_release
        self._prospective_lineage_readiness = prospective_lineage_readiness
    ...
    async def persist(
        ...
        *,
        ...
        if proposal is None:
            return None
        if (
            self._prospective_lineage_readiness is not None
            and not await self._prospective_lineage_readiness.ready(proposal.proposal_id)
        ):
            raise ValueError(
                "kinetic proposal prospective lineage is not materialized and Saga-sealed"
            )
