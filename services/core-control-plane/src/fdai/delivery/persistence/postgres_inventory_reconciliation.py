...
The decision is kept apart from the snapshot store it reads. The store owns
immutable candidates and the active pointer; this module owns only the
scheduling question, so a change to one cannot quietly alter the other.
"""
...
#: First retry delay after a failed attempt. Each further consecutive failure
#: doubles the wait up to the routine interval, so a source that keeps failing
#: costs the provider budget of one routine scan rather than one scan per tick.
...
def has_unreconciled_change(
    ...
    *,
    ...
    """Report whether any recorded change may be missing from the active snapshot.

    The comparison uses the snapshot's start, not its completion: a scan only
    observes state from the moment it began, so a change recorded while it ran
    is not proven to be covered.

    A malformed marker, and a marker set large enough to have been truncated,
    both count as unresolved. Reading a broken or clipped change record as
    "nothing happened" would let the observed graph drift silently, and one
    extra reconciliation is the cheaper failure.
    """
...
def _change_marker_recorded_at(value: Any) -> datetime | None:
    """Return when this control plane recorded the change, not when it happened.

    ``observed_at`` carries the provider's event clock, which cannot be ordered
    against a snapshot timestamp from this database. ``recorded_at`` is written
    by the drain on our own clock; older markers that predate it fall back to
    the provider value rather than being read as absent.
    """
...
def failure_retry_delay_seconds(
    *,
    ...
    """Return the wait a consecutive failure streak has earned.

    The delay doubles per consecutive failure and is capped at the routine
    interval, so a source that fails forever settles at the cost of routine
    reconciliation instead of retrying on every tick.
    """
    ...
    # Cap the exponent before shifting so a long streak cannot build a huge int.
