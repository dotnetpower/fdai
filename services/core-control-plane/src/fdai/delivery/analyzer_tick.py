...
closed without publishing, an active claim owned by another tick fails closed,
and a broker acknowledgement whose receipt could not be persisted stays
uncertain until its lease expires rather than being republished immediately.
"""
    ...
    async def run_once(self, targets: Sequence[AnalyzerTarget]) -> AnalyzerTickReport:
        ...
        for finding in report.findings:
            ...
            try:
                ...
                error = f"publication_claim={type(exc).__name__}:{exc}"
                ...
                _LOGGER.warning(
                    "analyzer_publication_claim_failed",
