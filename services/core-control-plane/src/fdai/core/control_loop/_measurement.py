...
class TerminalMeasurementRecorder:
    ...
    retries them before ingest can discard a redelivery as a duplicate. Errors
    propagate; retries preserve the original classification and capture time.
    ...
    async def record(
        ...
        await self._persist(retained)

    async def retry_pending(self) -> None:
        """Repair failed writes before another event enters the ingest dedupe cache."""
        for fields in tuple(self._pending.values()):
            await self._persist(fields)
