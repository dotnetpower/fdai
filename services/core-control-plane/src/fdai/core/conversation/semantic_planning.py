...
_MAX_GROUNDED_FILTER_VALUES = 16
        ...
        *,
        ...
        try:
            ...
            if any(node.kind.value == "object_set" for node in plan.nodes):
                ...
                plan, grounded = _ground_stated_value_filters(
                    plan,
                    utterance=utterance,
                    descriptors=descriptors,
                )
...
def _stated_value_filters(
    ...
    for descriptor in descriptors:
        ...
        for property_name, declaration in properties.items():
            ...
            if len(selected) != 1 or not 1 <= len(selected[0]) <= _MAX_GROUNDED_FILTER_VALUES:
                continue
...
def _ground_stated_value_filters(
    plan: OntologyQueryPlan,
    *,
    utterance: str,
    descriptors: Sequence[Mapping[str, Any]],
