...
class AssignmentOutcomeConsumer:
    ...
    scoped: ScopedDutyOwnership | None = None
    ...
    async def deliver(self, payload: Mapping[str, Any], *, bus: EventBus) -> None:
        ...
        if result.schema_version == "1.2.0":
            await self._deliver_scoped(decision, bus=bus)
            return
        ...
        if result.state == "approved":
            if self.ownership is None or self.base is None:
                ...
                if case.intent.revocation is not None and case.state is AssignmentState.APPROVED:
                    try:
                        await self.ownership.request_revocation(
                            case_id=case.case_id, expected_revision=case.revision
                        )
                    except ValueError:
                        status = "held"
                    else:
                        status = "iam_review_requested"
                elif case.state.value in {"approved", "ownership_pr_open", "iam_revoked"}:
                    try:
    ...
    async def _deliver_scoped(self, decision: AssignmentAgentDecision, *, bus: EventBus) -> None:
        """Consume only a sealed ownership-only result; never enter personal IAM coordination."""
        result = decision.result
        if result is None:
            return
        status, pr_ref = "not_required", None
        if result.state == "approved":
            if self.scoped is None:
                ...
                current = await self.scoped.cases.get(result.case_id)
                try:
                    opened = await self.scoped.open_proposal(
                        current.case_id, expected_revision=current.revision
                    )
                except (PermissionError, ValueError):
                    status = "held"
                else:
                    status, pr_ref = "pr_open", opened.pr_ref
        receipt = {
            "proposal_id": decision.notice.proposal_id,
            "request_digest": decision.notice.proposal_digest,
            "case_id": result.case_id,
        ...
        await self.store.write_state_with_audit_if_absent(
            f"human_assignment:scoped-artifact-receipt:{decision.notice.proposal_id}:{status}",
            receipt,
            {"actor": "Saga", "action_kind": "human.scoped_duty.artifact_observed", **receipt},
        )
        await bus.publish(
            ASSIGNMENT_PROJECTION_TOPIC,
            decision.notice.case_id,
            {"kind": "scoped_duty_artifact", "schema_version": "1.0.0", **receipt},
        )

    async def reconcile_revocation(self, case_id: str, expected_revision: int) -> None:
        """Retry only a Core-audited removal artifact; no provider effect or approval occurs."""
        if self.ownership is None or self.base is None:
            raise ValueError("revocation artifact delivery is unavailable")
        case = await self.ownership.cases.get_case(case_id)
        if (
            case.revision != expected_revision
            or case.intent.revocation is None
            or case.state is not AssignmentState.IAM_REVOKED
        ):
            raise ValueError("revocation artifact source revision changed")
        for command in case.command_receipts:
            result = await self.store.read_state(
                f"human_assignment:command-result:{command.proposal_id}"
            )
            if (
                result is not None
                and result.get("case_id") == case_id
                and result.get("request_digest") == command.request_digest
                and result.get("state") == "approved"
            ):
                await self.ownership.open_proposal(
                    case_id=case_id,
                ...
                return
        raise ValueError("revocation artifact has no sealed independent-review result")
