...
async def test_rate_limiter_paces_requests_at_the_configured_budget(
    ...
    """Every acquisition is spaced, not only the ones that follow a throttle."""
...
@pytest.mark.asyncio
async def test_rate_limiter_burst_allows_the_documented_head_start(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    now = 0.0
    delays: list[float] = []
...
@pytest.mark.asyncio
async def test_rate_limiter_is_shared_across_concurrent_shards(
    monkeypatch: pytest.MonkeyPatch,
) -> None:
    """Concurrent shards spend one budget instead of one budget each."""
...
def test_rate_limiter_rejects_a_non_positive_budget(requests_per_second: float) -> None:
...
def test_rate_limiter_rejects_a_zero_burst() -> None:
    with pytest.raises(ValueError, match="burst"):
        arg_transport.ArgRateLimiter(burst=0)
...
async def test_throttle_gate_caps_a_long_quota_reset_window(
    ...
    """A multi-hour reset header must not park a scan past every useful deadline."""
...
@pytest.mark.parametrize("max_defer_seconds", [0.0, -1.0, float("inf")])
def test_throttle_gate_rejects_an_invalid_defer_cap(max_defer_seconds: float) -> None:
    with pytest.raises(ValueError, match="max_defer_seconds"):
        arg_transport.ArgThrottleGate(max_defer_seconds=max_defer_seconds)
...
async def test_query_factory_paces_pagination_with_the_configured_budget(
    ...
    """The factory applies its budget to page follow-ups, not just to shard starts."""
