...
@pytest.mark.asyncio
async def test_source_follows_bounded_pull_request_pagination() -> None:
    proposal = _proposal()
    digest = str(proposal["proposal_digest"])
    ...
    def handler(request: httpx.Request) -> httpx.Response:
        if request.url.path.endswith("/pulls"):
            if request.url.params["page"] == "1":
                return httpx.Response(200, json=[{"draft": False}] * 100)
            return httpx.Response(
                200,
                json=[
                    {
...
@pytest.mark.asyncio
async def test_source_rejects_pagination_beyond_bound() -> None:
    def handler(_request: httpx.Request) -> httpx.Response:
        return httpx.Response(200, json=[{"draft": False}] * 100)

    async with httpx.AsyncClient(transport=httpx.MockTransport(handler)) as client:
        source = GitHubModelLifecycleObservationSource(
            config=GitHubModelLifecycleObservationConfig(
                owner="example",
                repo="fdai",
                max_pages=1,
        ...
        with pytest.raises(RuntimeError, match="pagination exceeds"):
            await source.load()
