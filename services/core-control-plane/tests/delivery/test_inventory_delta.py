...
class _FinalBatchInventory:
    def __init__(
        self,
        *,
        emit_after_final: bool = False,
        relationship_reconciliation_after: str | None = None,
    ) -> None:
        self.emit_after_final = emit_after_final
        self.relationship_reconciliation_after = relationship_reconciliation_after
    ...
    async def delta(self, cursor: str):  # type: ignore[no-untyped-def]
        ...
        yield InventoryBatch(
            ...
            cursor="cursor-final",
            final=True,
            relationship_reconciliation_after=self.relationship_reconciliation_after,
        )
...
@pytest.mark.asyncio
async def test_forward_delta_persists_newest_relationship_reconciliation_marker() -> None:
    state = InMemoryStateStore()
    marker_key = "inventory-relationship-reconciliation:subscription-1"
    await state.write_state(marker_key, {"observed_at": "2026-07-15T00:30:00+00:00"})

    await forward_inventory_delta(
        inventory=_FinalBatchInventory(relationship_reconciliation_after="2026-07-15T00:15:00Z"),
        state_store=state,
        event_bus=InMemoryEventBus(),
        topic="events",
        scope="subscription-1",
