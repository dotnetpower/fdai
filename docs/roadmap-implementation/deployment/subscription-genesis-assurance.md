...
|------|-------|----------|-------|
| Foundation state and external control planes | in-progress | Existing bootstrap Terraform, `fdai_deployment_cli.bootstrap_reconcile`, and protected workflows | The pre-runner plan now pins every read to the verified subscription, distinguishes missing from unreadable state, checks state-account name and security posture, and seals intent separately from expiring observations. Approved apply, secure enrollment, state postconditions, and migration receipts remain open. |
| Authority, concurrency, cancellation, and recovery | in-progress | Exact-plan claims; `fdai_deployment_cli.state`; `fdai_deployment_cli.github_actions`; focused dispatch, journal, resume, and simulation tests | Request-bound plan/apply, explicit approval, and verification-only resume are implemented. Subscription-wide leases, cancellation, clock assurance, quorum enforcement, and live rollback rehearsal remain open. |
