...
| Database migrations and catalog projection | in-progress | `bootstrap-service-migrations.sh`; service migrations including `20260905_core_inventory_observation_journal.py`; `DatabaseSemanticReadback`; Operator migration and catalog Jobs | Fresh-database migration and the complete fail-closed readiness evidence contract now succeed. The normalized observation journal is Core-owned and must precede inventory runtime. Pre-runtime marker production and independent runtime-principal evidence remain open. |
