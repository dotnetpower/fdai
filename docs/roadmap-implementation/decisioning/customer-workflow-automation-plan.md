...
|------|-------|--------|----------|-----------|
| 2026-09-17 | implemented | Added presentation-only text, category, and step-kind filters plus selected-node ActionType and control-step inspection to the published catalog review. | `current change`; Workflow Builder source, bilingual catalogs, route-owned CSS, 27 focused unit tests, Console typecheck, and one desktop Playwright scenario passed. | Retain authenticated standard-port evidence after the Operator API is available; reviewed catalog proposal and governed runtime advancement remain open. |
