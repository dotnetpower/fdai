...
| 2026-09-12 | implemented | Made the isolated migration fixture accept CI's `FDAI_DATABASE_URL` when the preferred local `FDAI_VALIDATION_DATABASE_URL` is absent, avoiding an unintended CI skip. | `current change`; `env -u FDAI_VALIDATION_DATABASE_URL pytest -q --no-cov tests/integration/services/test_skill_proposal_evidence_migration.py -rs` (`1 passed`, local PostgreSQL, no skips). | Pushed-SHA CI and deployed duplicate-delivery evidence are not supplied by this local check. |
