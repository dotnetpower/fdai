...
| What-if and configuration-drift integration | in-progress | `services/core-control-plane/src/fdai/core/risk_gate/blast_radius_simulator.py`; `services/core-control-plane/src/fdai/core/detection/configuration_drift.py` | Pure modules and focused tests exist, but Phase 1 does not yet cite an audited ControlLoop path joining them to T0 before action authority. |
...
| Change Safety out-of-band detection | in-progress | `services/core-control-plane/src/fdai/core/verticals/change_safety/detector.py`; `uv run pytest -q --no-cov services/core-control-plane/tests/verticals/test_change_safety_detector.py services/core-control-plane/tests/verticals/test_change_safety_detection_metrics.py` passed 87 cases | Azure Activity Log behavior is implemented and measured locally. The broader Phase 1 signal-set claim requires the scope decision in issue #339. |
