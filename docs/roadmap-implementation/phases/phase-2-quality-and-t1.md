...
| Collector review to activation ownership | implemented | `rule_catalog/pipeline/{snapshot_mirror,review,promotion,orchestrator}.py`; `delivery/rule_catalog_delivery.py`; focused pipeline checks (`111 passed`) | Collection artifacts are content-addressed and inert. Mimir-governed Phase 2 review owns activation; the promotion controller records shadow decisions only, and rollback retains the last-good revision. |
| T2 quality gate core and runtime binding | implemented | `services/core-control-plane/src/fdai/core/quality_gate/deterministic_evidence.py`; `gate.py`; `services/core-control-plane/src/fdai/composition/_helpers.py`; `services/core-control-plane/src/fdai/runtime/control_loop.py`; focused quality-gate, T2, and runtime binding checks (`71 passed`) | The production-composed path requires current, conflict-free, non-synthetic what-if and security records from fixed independent authority classes before model cross-check or risk evaluation. Partial or absent binding fails closed; direct construction remains available for isolated compatibility tests. Live producer and promotion evidence remain separate validation. |
...
| Assurance Twin read-only query slice | in-progress | `services/core-control-plane/src/fdai/core/assurance_twin/{query,semantic_query}.py`; `services/core-control-plane/src/fdai/runtime/assurance_twin_query.py`; focused query/runtime checks (`50 passed`) | Strict compiler and inert discovery seams are implemented with explicit unavailable defaults. A concrete model/discovery binding and governed authenticated receipt remain open. |
...
|------|-------|--------|----------|-----------|
| 2026-08-31 | implemented | Added the strict Assurance Twin compiler and discovery composition boundary. Accepted plans are exact-input-bound, evidence-cited, bounded, and read-only; invalid plans become explicit ambiguity and no-authority discovery gaps. | `current change`; 50 focused query and runtime checks passed. | Bind a governed model compiler/discovery sink and retain the authenticated issue #344 runtime receipt. |
| 2026-08-31 | implemented | Fixed the non-duplicated Phase 1/2 ownership boundary: collection ends at a no-authority review package, while Phase 2 owns separately reviewed activation and rollback. | `current change`; 111 focused collector, parser, mirror, review, and orchestrator checks passed. | Live PR delivery and watcher receipts remain operational evidence, not activation authority. |
| 2026-08-31 | implemented | Completed the deterministic T2 evidence path with versioned what-if and security records bound to the exact candidate digest, fixed independent authorities, freshness, conflicts, provenance, and synthetic status. Runtime binds explicit unavailable providers until both DI implementations exist; unresolved or prohibited candidates short-circuit before model cross-check and risk evaluation. | `current change`; deterministic evidence contract, quality gate and audit projection, Container/runtime binding, and 71 focused checks. | Retain governed live producer receipts before any promotion claim; synthetic evidence remains ineligible. |
