...
| Global provider-schema accounting | implemented | `provider-schema-catalog/index.json`; provider-schema watcher, relationship review, generation, projection, and durable ledgers under `services/core-control-plane/src/fdai/delivery/`; focused schema, fixture, D4, and ledger checks | The bounded content-addressed review records eight reviewed-mapping overlaps and publishes no-authority drift on `object.drift` with `event_type: provider.schema_drift`. Transitive invalidation preserves unrelated components, and reviewed promotion history retains immutable contexts without activation authority. Protected scheduled-run receipts remain separate. |

### Implementation history

| Date | State | Change | Evidence | Remaining |
|------|-------|--------|----------|-----------|
| 2026-08-31 | implemented | Completed local provider-relationship refresh and review mechanics: canonical complete-generation directions, transitive-only invalidation, immutable D4 contexts/comparison, and append-only distinct-reviewer history remain proposal-only. | `current change`; provider relationship generation and ledger, direction snapshot serialization, full infrastructure fixtures, D4 comparison and promotion tests. | Retain protected scheduled-run, Heimdall, Saga, and complete release-bound D4 review evidence; none is claimed here. |
