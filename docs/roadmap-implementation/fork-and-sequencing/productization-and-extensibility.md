...
| Signed deployment bundle construction and verification | implemented | `scripts/deployment/release/airgap-drill.sh`; bundle builder and `fdaictl bundle verify` tests | Construction, external signing, materialization, extraction, and verification are executable and covered independently of publication. |
...
| 2026-09-12 | in-progress | Corrected release-state ownership: signed bundle construction and verification are implemented, while protected publication and full release-workflow enforcement remain open. | `current change`; `scripts/deployment/release/airgap-drill.sh`; `.github/workflows/automatic-version.yml`; English and Korean roadmap owners. | Implement the protected release-batch workflow without conflating it with local bundle construction or automatic version tagging. |
...
- [x] Retain executable signed-bundle construction, external signing, extraction, and verification
  evidence independently of publication.
- [ ] Implement a protected release-batch workflow that enforces the required clean-checkout,
  productization, dependency-audit, disposable-database, and signed-bundle checks before exposing
