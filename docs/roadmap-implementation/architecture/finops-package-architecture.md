...
| Operator PostgreSQL projection compatibility | in-progress | `services/operator-service/src/fdai_operator_service/postgres_cost_governance.py`; `services/operator-service/tests/test_postgres_cost_governance.py`; current change | The reader uses JSON scope membership and normalizes SQLAlchemy-style psycopg DSNs for direct driver use. Focused Operator test results remain to be recorded before claiming implemented status. |
