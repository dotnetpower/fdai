...
| Derived ReviewCase and ReviewCheck projection | implemented | `core/architecture_review/projection.py`; focused projection tests | The projection reads observation lineage, reserves `runs_review` for the workflow runner, and adds `change_instantiates_process` only for an existing compatible Process without blocking the core projection. |
...
| 2026-08-27 | implemented | Bounded the process-local projection-status fallback to 1,024 entries and stopped duplicating status locally when the store provides the durable marker contract. | `current change`; observation-loop checks (`21 passed`); Ruff and strict mypy. | Retain restart evidence from the durable store separately. |
| 2026-08-27 | implemented | Corrected observation lineage so Change lifecycle provenance no longer occupies the workflow runner's `runs_review` relationship. Existing compatible Process objects receive `change_instantiates_process`; missing or reused Process identities leave the core projection intact. | `current change`; schema-validating missing-Process and one-to-one Process-reuse regressions plus focused observation and projection checks. | Retain deployed typed-bus and durable-store evidence separately. |
