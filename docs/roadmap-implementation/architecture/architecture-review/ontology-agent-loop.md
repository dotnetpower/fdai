...
| ARB pantheon integration evidence | in-progress | `core/architecture_review/observation_trace.py`; `agents/_framework/runtime.py`; `tests/core/architecture_review/test_observation_trace.py`; `tests/agents/test_runtime.py` | The immutable observation-only replay projection now binds to owned runtime topics and retains terminal observer degradation evidence. Missing owned context, evidence-bundle, impact-envelope, recommendation, and derived projection records still keep the trace on explicit hold. |

### Implementation history

| Date | State | Change | Evidence | Remaining |
|------|-------|--------|----------|-----------|
| 2026-08-31 | implemented | Re-derived unsafe link conditions at the graph-receipt boundary and completed the Forseti negative matrix for stale, mixed-release, incomplete, conflicting, synthetic, future, and truncated evidence. Every case keeps the planned change on human review without a direct agent call. | `current change`; `core/impact_analysis/change_assessment.py`, `tests/core/impact_analysis/test_change_assessment.py`, and `tests/agents/test_change_management_chain.py`; focused impact, agent-chain, layout, Ruff, and strict mypy checks. | Compose the wider evidence bundle, scenario branch, and DecisionCase path without direct agent calls. |
| 2026-08-29 | in-progress | Bound the ARB observation trace to owned runtime topics through a dedicated non-agent observer group and retained terminal consumer degradation evidence without granting mutation or execution authority. Focused tests now prove runtime wiring, replay across duplicate/reordered/restarted delivery, and held traces for late or degraded stages. | `current change`; `services/core-control-plane/src/fdai/core/architecture_review/{__init__,observation_trace}.py`, `services/core-control-plane/src/fdai/agents/_framework/runtime.py`, `services/core-control-plane/tests/core/architecture_review/test_observation_trace.py`, `services/core-control-plane/tests/agents/{test_runtime,test_njord_cost_governance}.py`; focused pytest, Ruff, and strict mypy checks plus ARB/readiness and documentation guard scripts. | Extend owned records for context, evidence bundle, impact envelope, recommendation, and the derived `ReviewCase` projection so the first ARB-2 vertical slice can close without synthesizing missing stages. |
...
- [ ] Derive `ReviewCase` and `ReviewCheck` from authoritative lineage and reconcile expired or
  removed evidence.
- [x] Bind `replay_architecture_review_trace` to owned runtime topics through a dedicated
  non-agent observer group and retain terminal consumer degradation evidence without granting
  mutation or execution authority.
- [ ] Extend owned records for context, evidence bundle, impact envelope, recommendation, and the
  derived `ReviewCase` projection; the focused replay test now covers duplicate, reordered,
  restarted, missing, conflicting, late, incorrectly owned, degraded, and no-mutation behavior.
