...
| Evidence attestation and decision authority | in-progress | [Focused owner](../../roadmap/architecture/architecture-review/evidence-and-authority.md); [delivery ledger](architecture-review/evidence-and-authority.md); `core/architecture_review/readiness.py`; focused readiness tests | The reusable checker rejects incomplete accepted blocker contracts and metadata-only production evidence. An injected provider must return the exact bounded body, scope, revision, freshness, and approver authority. Immutable decision binding remains open. |
