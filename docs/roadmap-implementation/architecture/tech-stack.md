...
| 2026-09-12 | in-progress | Corrected the preceding #356 completion claim after independent review found no matching closed-or-merged proposal branch and no enforced shadow-replay gate. The governed draft-only run remains validated evidence for the implemented subset. | `current change`; workflow and receipt inspection; independent review of the 64-test evidence boundary. | Implement closed-or-merged retry handling and represent shadow replay as a required gate before closing #356. |
...
- [ ] Under [issue #356](https://github.com/dotnetpower/fdai/issues/356), retain the validated
  expired-unmerged hold and governed draft receipt while adding explicit matching
  closed-or-merged proposal handling and an enforced shadow-replay gate with focused tests.
