...
|------|-------|----------|-------|
| C6 local Kubernetes manifest schema | validated | `test_rendered_resources_pass_strict_kubernetes_schemas` ran with `kubernetes-validate==1.36.0`: 3 passed, none skipped. | Six rendered resources plus Pod template pass strict 1.30/1.34/1.36 schemas; unknown-field controls reject. This is structural validation, not a selected-cluster admission or rollout receipt. |
...
|------|-------|--------|----------|-----------|
| 2026-09-20 | implemented | Recovered invalid, expired or revoked preflight readers into audited no-fact inspection proposals on fresh discovery while preserving known owner pins. Old fact-bearing reads remain blocked and storage-service failures propagate. | `current change`; 23 proposal cases passed, including actual signature revocation/expiry and an explicit storage-failure control. | Complete other sources, scheduled collection, protected installation and C2-C6 final review; recovery is not source readiness. |
| 2026-09-20 | validated | Directly checked renderer output and actual Pod template against pinned local strict Kubernetes schemas, then retained repeatable optional tests without modifying runtime dependencies. | `current change`; 3 versioned schema tests passed with 21 positive resource validations and 3 rejecting negative controls. | Retain exact-target admission, installed effect, rollback, revocation and removal evidence; other C2-C6 implementation and full review campaign remain open. |
...
- [x] Validate generated Kubernetes resource structure against pinned 1.30/1.34/1.36 schemas and a rejecting strict-field control.
- [ ] Validate server admission under a reviewed exact target; connect approved apply, activation, upgrade, rollback and removal with independent readback.
