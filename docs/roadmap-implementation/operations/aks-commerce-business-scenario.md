...
|------|-------|--------|----------|-----------|
| 2026-09-17 | implemented | Serialized scenario-lab Terraform apply after two exact recovery applies deterministically failed three NSG-to-subnet associations while the fourth association succeeded. | Failed protected runs `35169623286` and `35170392577`; successful recovery plan `35170229866` retained 26 resources and showed exactly 3 creates, 0 updates, and 0 deletes/replacements. The apply now uses `-parallelism=1` so Azure Network RP updates to one VNet cannot contend with each other. | Pass focused workflow contracts and protected CI, merge the repair, then build and approve a fresh exact recovery plan before another apply. |
