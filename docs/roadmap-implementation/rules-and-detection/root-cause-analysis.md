...
| Knowledge evidence and provider binding | implemented | `core/rca/{knowledge_evidence,governed_knowledge_evidence,governed_document_evidence}.py`; `shared/providers/knowledge.py`; `delivery/pgvector/knowledge.py`; `delivery/azure/llm/rca_model.py`; focused provider, adapter, and runtime tests | Unscoped documents retain the existing pgvector source. Governed uploads use a separate principal-, collection-, purpose-, scope-, cutoff-, access-, and revision-bound OperationalEvidenceBundle path. A missing or rejected governed context holds and never falls back to unscoped retrieval. |
...
|------|-------|--------|----------|-----------|
| 2026-09-04 | implemented | Connected the optional T1 `IncidentMemberSource` and reviewed resource-dependency graph from the immutable composition Container into the default ControlLoop assembly. The unbound default remains unavailable instead of permitting unscoped causal claims. | `current change`; focused runtime composition and T1 control-loop checks, Ruff, and strict mypy. | Bind the deployment-owned incident member source and retain the governed operational cohort. |
| 2026-09-04 | implemented | Hardening round 5 made an empty custom governed gatherer result fail closed. When governed context is requested, zero citations without an explicit hold now becomes `document_evidence_missing` instead of allowing unrelated evidence to continue. | `current change`; focused custom-gatherer regression, RCA tests, Ruff, and strict mypy. | Bind a deployment-owned governed context provider and retain the operational cohort. |
| 2026-09-04 | implemented | Hardening round 4 required exact set equality between governed document excerpts and document-lane citation manifest entries. Extra, duplicate, or missing manifest entries now hold before RCA receives a citation. | `current change`; focused forged-manifest regression, governed knowledge tests, Ruff, and strict mypy. | Bind a deployment-owned governed context provider and retain the operational cohort. |
| 2026-09-04 | implemented | Added a separate principal-scoped governed-document evidence path for RCA. It rechecks current document metadata and authorization after search, binds collection, access descriptors, purpose, scope, cutoff, exact revision, redaction, and citation evidence into a document-only OperationalEvidenceBundle, and emits opaque citations only. Deleted, revoked, stale, conflicting, redacted, mismatched, or unavailable evidence holds without unscoped fallback. | `current change`; focused governed-document, RCA coordinator, operational evidence bundle, Ruff, and strict mypy checks. | Bind a deployment-owned governed context provider before claiming automatic incident use; retain the exact-revision operational cohort. |
