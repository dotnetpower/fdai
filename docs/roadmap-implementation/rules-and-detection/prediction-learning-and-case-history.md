...
| Durable `Pattern` publication | in-progress | `agents/{muninn,norns}.py`; `core/operational_learning/cohort_retention.py`; `core/ontology_platform/pattern_queries.py`; `test_operating_pattern_learning_e2e.py` | Inert publication, current-case recompiled retention, independent principal-to-case-scope query admission, late runtime binding, new-layout purge, and legacy base-cohort fences are locally tested. Historical suffixed rows, downstream retention, and actual broker recovery remain open. |
...
The external-change journal witness is locally implemented with incomplete coverage. Earlier
...
independent proofs, UI completion, approved T1 enrollment, downstream deletion, and deployed
...
- [ ] Qualify historical suffixed snapshot/Pattern/emission cleanup and downstream broker/candidate
  retention; the base-cohort fence does not claim those separate copies or live redrive.
