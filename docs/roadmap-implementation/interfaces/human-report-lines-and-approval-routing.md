...
| Requester consent and HIL routing | implemented | `current change`; HIL, runtime, Operator, and Playwright tests; `core_hil_park_queue_index_20260921` | Consent is action-, path-, and scope-bound; delivery, escalation, and resolution revalidate current authority. Active scans filter lifecycle status without deleting resolved approval evidence. |
...
|------|-------|--------|----------|-----------|
| 2026-09-21 | implemented | Pushed pending and contact-consent status filters into active HIL queue reads and added a concurrent partial index, while retaining every resolved park and audit record. | `current change`; local PostgreSQL measured 1,667 resolved and 289 pending parks; 22 focused HIL tests and 73 migration inventory tests passed. | Physical deletion remains out of scope until a reviewed approval-retention and legal-hold policy exists. |
| 2026-09-20 | implemented | Assigned the incident-attention poller and observer-deployment projection regressions exactly once to the Operator service suite. | `current change`; service-suite JSON validation and 20 focused distribution tests passed. | No report-line runtime or authority changed; retain protected CI evidence. |
