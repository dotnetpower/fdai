...
| Console one-time delivery diagnostic | implemented | Operator `teams_workflow_diagnostics.py`; Console `settings-teams-workflow.*`; focused Operator and Console tests | An Owner can test one transient public-cloud URL with fixed synthetic content. Only a URL digest and prepared/completed metadata persist; the production binding remains deployment-managed. |
