...
|------|-------|----------|-------|
| Contracts, lifecycle, bounds, and format extraction | implemented | `packages/service-contracts/src/fdai_service_contracts/document_formats.py`; `services/document-processing-worker/src/fdai_document_worker_service/adapters/{ooxml,pdf_isolation,processing}.py`; focused format, parser, isolation, and ingestion tests | One intake allowlist covers bounded text, PDF, DOCX, PPTX, XLSX, PNG, JPEG, and TIFF. Tests cover signature mismatch, legacy Office rejection, malformed and encrypted packages, PDF page images, OOXML embedded-image OCR, formula text, OCR unavailability, and empty extraction. |
...
- [ ] Retain a deployed mixed-format OCR receipt covering standalone PNG/JPEG/TIFF, scanned and mixed PDF, and embedded Office images with no content in transport or audit.
