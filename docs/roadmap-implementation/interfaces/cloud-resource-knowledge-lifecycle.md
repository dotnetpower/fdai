...
records the delivered v3 software, bilingual query contracts, sixteen critique rounds and Issue
#1061's retained-source preparation. It retains explicit source-quality holds and does not close
this ledger's operating prerequisites.
...
| Policy-backed requester application | not-started | [Requester target](../../roadmap/interfaces/cloud-resource-knowledge-lifecycle.md#requester-initiated-application-target) and [current operation](../../runbooks/cloud-resource-knowledge.md#requester-application-target-and-current-operation) | No new approving person per eligible document inside independently established policy is the target, not current runtime. Actual requester binding, exact authorization, revocation fencing, safeguards, Var/Saga/Muninn and independent readback remain required. |
...
| K5: Source dates and inspect-confirm-import UI | implemented | [Knowledge panel](../../../console/src/routes/cloud-knowledge.tsx), [source views](../../../console/src/routes/cloud-knowledge.views.tsx), [package intake](../../../console/src/routes/cloud-knowledge-package.tsx), [model tests](../../../console/src/routes/cloud-knowledge.model.test.ts), and [browser checks](../../../console/tests/e2e/cloud-knowledge.spec.ts) | Bilingual source ranges, weakest freshness, outcomes, setup prerequisites, accessible source/package views, local-only file selection, exact inspected-byte confirmation, and review-only requests are explicit. Read-only inspection/export failures remain retryable; an uncertain mutating request requires authoritative reload. Import acknowledgement never grants activation or execution authority. |
...
|------|-------|--------|----------|-----------|
| 2026-09-17 | implemented | Completed the Knowledge overview interaction for cloud-reference status and offline package intake. The Console now separates source and package work, explains deployment-owned setup, permits local file preparation without upload, and keeps read-only inspection failures retryable without weakening uncertain-write reload fencing. | `current change`; task-owned Console model, views, styles, bilingual catalogs, and browser regression; 43 focused unit checks, strict TypeScript, and 3 desktop/mobile Playwright scenarios passed. | Production source rights, signing trust, reviewer identities, service policy mounts, and the authorized connected/restricted-network evidence in Issue #995 remain separate prerequisites. |
| 2026-09-15 | not-started | Recorded the requester-initiated application target under prior independent content policy, rather than requiring a new approving person for every eligible reference update. Distinguished it from the still-enforced manual lane and deferred uploader identity until actual authenticated intake. | `current change`; bilingual lifecycle, structured retrieval, ingestion ownership and runbook guidance. Design critique covered self-granting authority, A3-E confusion, policy races, premature success and hidden holds; no runtime behavior or approval was changed. | Implement a versioned bounded authorization contract, server-owned requester entitlement, Var/Saga verification and worker/CAS revocation fences. Prove denial, duplicate/restart, complete source dates, readback and independent promotion before enabling requester application. |
| 2026-09-15 | implemented | Extended durable retained-source upgrades with exact origin, reviewed metadata and normalizer identity. Same-body checks cannot reuse stale title/rights; processing-only work never renews source checks. | `current change`; [Issue #1061](https://github.com/dotnetpower/fdai/issues/1061), [collection regressions](../../../services/document-ingestion-api/tests/test_cloud_knowledge_collection.py), and the [392-case follow-up evidence](cloud-resource-knowledge-structured-rag.md#retained-source-follow-up-evidence). | Protected follow-up delivery and Issue #995 operational requirements remain separate. Local OS-isolated preparation is not production import or answer qualification. |
...
- [ ] **Requester application:** Implement the [policy-backed target](../../roadmap/interfaces/cloud-resource-knowledge-lifecycle.md#requester-initiated-application-target) for eligible public reference generations without per-document manual approval. Retain exact requester/policy/content/collection binding, denial for self-granted/missing/expired/revoked policy, request-time and final-visibility checks, fixed-agent ownership, current source/security gates, seven safeguards and independent readback. Runtime remains manual until focused tests and independent promotion evidence exist.
...
- [ ] **Deferred expansion:** Keep non-Azure adapters, deltas, mirror-specific upstream proof,
  ...
  The bounded requester target above now has a design and separate open implementation work;
  broader unattended activation remains deferred.
