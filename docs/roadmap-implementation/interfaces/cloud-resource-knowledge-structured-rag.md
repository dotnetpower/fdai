...
| Bilingual query and evaluation boundaries | in-progress | Query/evaluation contracts and focused semantic dispatch/evaluation tests; PR #1047 attempt 1 | CI exposed a recovery prompt-budget regression after the fourteen-round checkpoint. Actual model quality and independent labels remain separate. |
| Ten-round critique and hardening | in-progress | Fourteen original rounds and post-publication rounds below | New CI evidence reopens final review; no prior pass is treated as universal absence of defects. |
...
- [ ] Complete post-publication critique rounds and re-review until no confirmed in-scope Medium-or-higher defect remains. Unsupported-source qualification and operating prerequisites are not downgraded to satisfy this condition.
