...
| Reviewed icon asset loading | implemented | `architecture-network-icon-urls.ts`; `architecture-network-icons.ts`; focused icon tests; Console typecheck and production build | Interactive graphs resolve static icon URLs without eager Vite URL modules. Raw SVG content remains available only to the self-contained export path. |
