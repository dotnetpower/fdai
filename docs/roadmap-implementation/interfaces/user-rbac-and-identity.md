...
| Slack signed actor handoff | implemented | `families/iam/slack_handoff.py`; `slack_adapter.py`; `test_slack_hil_interactivity.py`; focused Operator/Console and synthetic desktop checks | Slack buttons carry only approval identity. The signed click mints a short-lived nonce digest tied to original action, workspace, sender, mapping revision, and expiry. A decision requires fresh signed Entra API-token `auth_time`; missing claims fail closed. The authenticated provider journey and actual Slack posting remain unverified. |
...
|------|-------|--------|----------|-----------|
| 2026-09-26 | implemented | Joined outbound Slack A1 Block Kit actions to the existing signed one-use browser handoff without granting authority to the card, click, or browser. | `current change`; `slack_adapter.py`, `test_slack_hil_interactivity.py`, focused Core/Operator/Console tests. | Retain live Slack and Entra evidence, a real PostgreSQL nonce race, and #943 author confirmation. |
