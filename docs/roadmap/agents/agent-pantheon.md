...
A repeated-event episode counts each non-empty Event `idempotency_key` at most once and uses the validated source event time when present, so at-least-once or delayed replay cannot advance Heimdall's threshold from duplicate or historically distant evidence. One accepted episode emits no same-severity candidate until a quiet repeat window resets it; the next episode receives a distinct opaque identity so a closed Incident does not absorb a recurrence.
