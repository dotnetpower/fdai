...
A repeated-event episode counts each non-empty Event `idempotency_key` at most once, so at-least-once delivery cannot advance Heimdall's threshold or create an Incident candidate from duplicate evidence.
