...
A repeated-event episode counts each non-empty Event `idempotency_key` at most once, so at-least-once delivery cannot advance Heimdall's threshold or create an Incident candidate from duplicate evidence. One accepted episode emits no same-severity candidate until a quiet repeat window resets it; the next episode receives a distinct opaque identity so a closed Incident does not absorb a recurrence.
