...
lineage, including the positive attempt number, through Huginn, Forseti, and Thor. Thor also
preserves an action identifier when the verdict supplies one; it never invents one from the
correlation id. A delivery-owned producer stores an optional argument-bound kinetic proposal for one complete operational plan; Forseti resolves it through an injected source and keeps it on the same Verdict-to-ActionRun path after strict validation. Both are attribution and evidence only;
