...
| 2026-08-27 | implemented | Replaced Forseti's fixed planned-change freshness boolean with an injected active-inventory receipt source. The change preserves AgentSpec, topics, ownership, approval, judgment, and execution boundaries; it only allows a complete fresh exact-release read to preserve eligibility, while every unavailable or invalid receipt requires review. | `current change`; focused impact, persistence-decoder, change-chain, lineage, and framework-layout checks (`64 passed`); Ruff and strict mypy. | Retain one deployed exact-release receipt without changing the remaining KPI and promotion gates. |
| 2026-08-27 | implemented | Hardened Forseti's planned-change input after independent review. Persisted ontology and operating-model manifests, global overlay absence, and a stable post-traversal re-read now constrain the receipt; configured PostgreSQL failures become an explicit failed assessment and human review. AgentSpec, topics, roles, and action authority remain unchanged. | `current change`; focused freshness, change-chain, and framework checks (`49 passed`); Ruff and strict mypy. | Retain deployed evidence without changing the remaining KPI and promotion gates. |
...
| Forseti | Judge | 2 | Verdict, RCA, SecurityEvent, ArbitrationRequest | produces verdicts and observation-only ARB reviews for planned intent; optional context can only lower autonomy; no executor role | yes (T2 abstain only) |
...
Missing, stale, failed, or review-required assessment forces human approval. Observed changes remain context only. Planned-change assessment now reads a content-addressed exact-release receipt from the
active PostgreSQL inventory generation under the promotion lock; target, source, relationship, identity, overlay, successor, truncation, and time gaps all keep the change in review. Operational-context freshness entries require an explicit string source, timestamp, and integer maximum age; malformed values, including boolean ages, fail closed and lower the verdict
...
Azure SDK or writes the inventory database directly. The scheduled Inventory sync job remains the periodic reconciliation backstop that repairs missed signals with a complete ARG/ARM snapshot. Stale or degraded inventory remains unavailable;
