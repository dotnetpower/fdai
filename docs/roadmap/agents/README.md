...
| Bounded task workers | in-progress | [Bounded Task Workers implementation status](../../roadmap-implementation/agents/bounded-task-workers.md#implementation-status) | The worker core, durable store and opt-in production composition are implemented. Parent admission, store-backed projections, Console presentation, detached delivery and governed runtime evidence remain incomplete. |
