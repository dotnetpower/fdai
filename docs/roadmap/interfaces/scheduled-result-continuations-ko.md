...
translation_source_sha: 934ed241f2f3619c6ebdd7fd72f9fc7d7ec053d1
...
| PostgreSQL 앵커 영속성 | in-progress | `services/core-control-plane/src/fdai/delivery/persistence/postgres_scheduled_continuation.py`; `services/core-control-plane/tests/persistence/test_scheduled_continuation.py` | 행 codec과 compare-and-set 경로가 있습니다. 건너뛰는 사례가 없는 실제 데이터베이스 및 재시작 실행이 남아 있습니다. |
...
- [ ] 지원되는 로컬 데이터베이스에서 PostgreSQL 앵커 사례를 건너뛰지 않고 실행하고 재시작, 동시 만료, winner-only 감사 및 멱등적 재시도 근거를 보존합니다.
