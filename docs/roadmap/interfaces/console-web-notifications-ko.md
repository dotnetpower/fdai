...
translation_source_sha: 8d74be3154c67aabc8627c66598053da18a3259f
...
알림에는 현지화된 일반 텍스트, 범위가 제한된 불투명 이벤트 태그, claim 토큰 및 서버가 제공한
상관관계 식별자로 필터링한 동일 출처 감사 로그 링크를 포함합니다. 실시간 프레임만으로 정식
인시던트가 존재한다고 판단하지 않습니다. 승인 및 실패 이벤트는 인시던트 없이 감사 이력만
남길 수 있습니다. 탭에서 Sample을 선택했더라도 감사 로그는 실제 근거만 조회합니다.
알림에는 원본 오류, 리소스 식별자, 승인 컨트롤 또는 실행 링크를 포함하지 않습니다.
...
탐색은 등록 범위 안의 감사 로그 경로와 기존 인시던트 경로만 허용합니다. 이미 표시된 알림은
원래 인시던트 목적지를 유지합니다. 선택한 실제 인시던트를 조회할 수 없으면 **관련 감사 이력 보기**로
같은 상관관계의 감사 로그를 열 수 있습니다. 인시던트를 만들어 내거나 필터를 변경하거나 운영
성공으로 간주하지 않습니다. Sample 선택에는 이 실제 근거 조회 링크를 표시하지 않습니다.
