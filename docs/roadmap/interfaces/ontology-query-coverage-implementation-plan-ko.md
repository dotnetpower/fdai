...
| Kubernetes 워크로드 대상 선택 및 평가 | implemented | `resource-types.yaml`, `metric-semantics.yaml`, `semantic_kubernetes_rollout_planning.py`, `semantic_kubernetes_pod_recovery_planning.py`, exact-release rollout 및 Pod 복구 FunctionType, 집중 검사, 인증된 대상 없는 Console 증적 | 검토된 Deployment 및 Pod 용어가 대상 없는 S12와 S1 요청을 Resource 타입 값 도메인에 연결합니다. Core는 프로바이더 I/O 전에 정확한 후보를 반환합니다. 정확한 Deployment는 4-node 소유권 조회로, 정확한 Pod는 발급된 증적, 검토된 30분 동일 UID 재시작 횟수 변화량, 명시적인 ReplicaSet 및 Deployment 소유자 증적을 결합하는 5-node 계획으로 컴파일합니다. 대상 없는 0건 결과는 `source_incomplete`를 보존하고, 정확한 범위가 불완전하거나 모호하면 메트릭 읽기 전에 중단합니다. 복구에는 범위가 제한된 양수 변화량과 ready 및 available 상태인 소유자 replica 전체가 필요합니다. 두 경로 모두 근본 원인과 실행 권한을 false로 유지합니다. |
