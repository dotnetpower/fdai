...
translation_source_sha: e70ede7d3106a9a39542006cdd9cc5d4338a4f63
...
관리되는 클라우드 참조 조회는 출처 날짜를 보존하며, 타입이 지정된 판단에서 원문 구간에
연결된 공급자, 세대, SKU 및 기타 적용 조건을 근거 객체가 아닌 단일 값 선택자로 받습니다.
특정 확인 시점의 안내에는 최신이며 조건이 맞는 근거가 필요하고, 실시간 최신 요청은 답변 중
수집 없이 종료합니다. [수명 주기 설계](cloud-resource-knowledge-lifecycle-ko.md)는 이 기준을
기록합니다. 개발 중인 [구조화 검색 확장](cloud-resource-knowledge-structured-rag-ko.md)은 의미
판단 `1.2.0`의 `document_query`를 수락된 모델 판단에서만 받으며, 프레임과 계획을 만들기 전에
다이제스트로 정확한 원래 질문에 연결합니다. 검색어는 적용 조건을 대체하거나 접근 범위를
넓힐 수 없습니다. v1.2 검색어가 없거나 결속이 맞지 않으면 문서 경로를 보류하며, 검색어 필드가
없는 이전 버전 동작과 구분합니다. 커버리지 목록 재생성은 출처 다이제스트만 갱신하며 측정된
커버리지를 바꾸지 않습니다.
