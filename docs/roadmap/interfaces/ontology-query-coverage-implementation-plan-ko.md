...
translation_source_sha: ba62309e763ccdda950edb2a5e283e7ebc66fe2f
...
| Kubernetes 워크로드 대상 선택 및 평가 | implemented | Kubernetes rollout, Pod 복구, Resource 이벤트 이력 플래너 및 FunctionType, 검토된 메트릭 의미 규칙, 프로바이더 읽기 경로, 집중 검사, 인증된 대상 없는 Console 증적 | 검토된 용어는 S12 및 S1 대상을 연결합니다. 정확한 Deployment 및 Pod 계획은 소유권과 동일 UID 재시작 변화량을 검증하고 `source_incomplete`를 보존하며 원인 및 실행 권한을 false로 유지합니다. Descriptor가 광고한 `resource_event.kubernetes` frame은 typed judgment duration이 범위가 제한된 frame lookback과 같을 때만 기존 2-node 계획으로 컴파일됩니다. Function은 정확한 현재 child 하나에 증적에 결속된 UID selector를 사용할 수 있고 identity가 없거나 일치하지 않으면 provider I/O 전에 incomplete로 유지합니다. 명시적인 cluster scope만 삭제된 객체 이력을 조회할 수 있습니다. Kubernetes Event API 결과는 `source_retention_unverified`를 보존하므로 행 0개는 과거 부재를 증명하지 않습니다. 영속 보존과 인증된 정확한 대상 근거는 열린 상태입니다. |
