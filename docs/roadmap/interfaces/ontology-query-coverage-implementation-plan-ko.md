...
> 구성합니다. 측정된 실행은 이 연결 이전의 결과입니다. Operator 서비스가 의미 턴을
> publish하고 근거에 묶인 변환 결과를 consume하며 실제 운영 서비스 간 증적을 만들 때까지
> 운영 완료는 계속 차단됩니다.
...
> 준비 상태에 exact missing-provider 사유를 보고합니다. Operator 발신함 게시와
> receipt-backed 통합 근거가 전환 게이트로 남아 있습니다.
>
...
|------|------------------|---------------------|
| 대화 라우팅 | 기본값 호환성 조정기는 exact 정본 명령만 수락합니다. 구성된 Core 의미 토픽은 이제 Azure 계획 수립 어댑터를 사용하고 현재 온톨로지 저장소에서 검증된 ordinary-language DAG를 실행한 다음 최종 그래프/근거 변환 결과를 저장/publish합니다. | Operator 게시, 변환 결과 재생/스트리밍, 실제 운영 서비스 간 증적 및 complete-manifest 한계를 넘는 매니페스트를 위한 서술자 인덱스가 남아 있습니다. `legacy`는 명시적 temporary 호환성 모드로만 존재합니다. |
| 서비스 간 의미 wire | 버전 1.2 요청/변환 결과 묶음은 실행 권한 없이 범위가 제한된 의미 입력과 근거에 묶인 최종 출력을 전달합니다. | 런타임 발행기, 소비자, 영속 재생 변환 결과 및 운영 준비 상태가 남아 있으며 의미 기록은 N-1로 downgrade하지 않습니다. |
| Console 의도 그래프 | Core는 검증된 계획에서 범위가 제한된 그래프/증적 근거를 만들며 shared SDK 변환 결과는 Console v2/v1 파서와 정확히 일치합니다. | 운영 turn-completion 스트림은 아직 생성된 그래프/근거를 첨부하지 않습니다. |
| 의미 interpretation | Azure OpenAI 어댑터는 bearer-token authentication과 resolved-candidate 대체 경로를 통해 `SemanticProblemFrame` 및 타입이 지정된 DAG를 strict 범위가 제한된 JSON 객체 두 개로 제안합니다. Core는 신원을 부여하고 Pydantic 스키마와 principal 매니페스트를 검증하며 exact 요청 역할로 실행합니다. | 서술자 한계를 넘으면 complete-manifest 선택자가 보류합니다. Operator 서비스는 근거에 묶인 변환 결과를 visible streamed 답변으로 연결해야 합니다. |
...
| 현재 토폴로지 | Azure 변환 결과는 resource-group/VNet containment, 첨부 및 범위가 제한된 의존성 허용 목록을 발행합니다. | Azure 어댑터가 `peered_with` 또는 `routes_to`를 발행하지 않으며 비공개 엔드포인트, 워크로드 및 서비스 의존성 커버리지가 불완전한합니다. |
