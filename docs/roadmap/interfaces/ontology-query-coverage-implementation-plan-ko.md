...
translation_source_sha: 28bffcbbb1968602602cc4ede84c5f2c3e363090
...
| 온톨로지 선언 개수 | 구현됨 | `semantic_manifest_planning.py`, `semantic_planning_plan_dispatch.py`, 선언 및 운영 개수 집중 회귀 검사 | 대상이 정확한 선언 종류인 검증된 집계 frame은 서버 소유 계획으로 `query.manifest`와 `count`를 컴파일합니다. 운영 개수, 중복 선언 종류, 사용할 수 없는 매니페스트 Function 및 다른 측정값은 기존의 안전한 실패 경로를 유지합니다. |
