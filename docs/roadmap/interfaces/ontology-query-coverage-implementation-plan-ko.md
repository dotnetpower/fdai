...
증분 온톨로지 빌드는 정본 문서 전체가 같을 때만 재사용합니다. 내용 다이제스트가 같다는 사실만으로 임베딩 공간의 출처를 확인할 수 없으므로 이전 벡터와 준비 세대 신원은 버리고, 구성된 어댑터가 새 벡터를 생성해야 합니다.
완전한 정본 선언 목록은 개수·상세 복구를 생략합니다. [계획 설계](hierarchical-conversation-planning-ko.md)에 정확한 타입 형태와 제한된 모델 비교를 기록합니다. principal 범위를 넓히거나 전체 조회 커버리지를 인증하지 않습니다.
[검색 적격성 설계](../rules-and-detection/rule-semantic-retrieval-ko.md)는 설정된 집단이 빠지면 평가기 자체가 보류하도록 요구합니다. 정책 `1.1.0`은 기존 `1.0.0` 증적 신원을 보존하면서 명시적 지표별 최소 표본 수를 결속하고 검토 시 다시 검사합니다. 모든 집단이 있다는 사실이나 이 정책 구현만으로 충분한 실제 임베딩 평가 데이터와 관련성이 입증되지는 않습니다.
의미 판단을 수행한 경우 선언 목록·상세 프레임은 수락됐고 모호하지 않은 스키마 기능군 의도를 요구합니다. 거절됐거나 다른 기능군인 판단은 이후 모델 프레임으로 선언 범위를 넓힐 수 없으며, 조회 실행 전에 기존의 타입이 지정된 사용 불가 결과를 반환합니다.

### 구현 범위

영속 인스턴스 검색은 Rule의 `active` 또는 `discovery` 포인터를 재사용하면 안 됩니다. 저장 신원은 principal 범위, 정확한 release, 권위 있는 원본 세대 및 임베딩 공간을 결속해야 합니다. 완전하고 인가된 변환 결과만 후보를 구성하며 모델 출력은 관측 인스턴스를 만들 수 없습니다. 담당 에이전트가 크기가 제한된 불변 청크를 준비하고 독립 검증기가 신원을 재구성한 뒤, 이벤트 기반 비교·교체로 활성화합니다. 실패하면 이전 세대를 보존합니다. 삭제, 원본 변경, ACL 변경 및 release 변경을 무효화 조건에 포함하며 읽기는 이 결속을 재확인하고 권위 있는 그래프에서 결과 ID를 다시 인가한 뒤 사실을 표시합니다. 이는 수정된 구현 경계이며 영속 기록기나 런타임 소비자가 이미 있다는 주장은 아닙니다.
...
| 정확한 의미 프롬프트 프로필 및 복구 예산 | 구현됨 | `core/prompts/profiles.py`, `semantic_query_azure_composition.py`, `semantic_planning_judgment.py`, `semantic_operational_summary_planning.py`, Azure 의미 계획, 보증, 온톨로지, 제안자, RCA 및 검토 어댑터, 결합 검사 804개 통과 | 정확한 활성 아티팩트, 최종 전송 요청, 생성된 스키마, 복구 지시 및 거부된 보증 요청은 범위가 제한된 재실행 근거를 전달합니다. 수락된 바인딩 `query.incident_evidence` intent는 추가 모델 호출 없이 읽기 전용 frame과 plan을 결정론적으로 구성합니다. 인시던트 frame 모델링이 계속 필요한 경우에도 전체 principal 매니페스트 대신 `Incident`와 `query.incident_evidence` 서술자만 전달하며, 최종 plan 검증은 항상 전체 매니페스트를 사용합니다. 일반 T2 frame 복구는 33,000자 legacy frame에 내용을 덧붙이지 않고 별도의 compact 역할을 사용합니다. Compact 판단, frame 및 plan 처리군은 shadow로 유지합니다. |
| 온톨로지 매니페스트 목록 및 선언 개수 | 구현됨 | `semantic_manifest_planning.py`, `semantic_planning_frame_checks.py`, `semantic_planning_plan_dispatch.py`, `semantic_turn_processor.py`, 집중 판단, 계획 및 이중 언어 표현 회귀 검사 | 특정 대상이 없는 검증된 스키마 목록 요청은 프레임이나 계획 모델을 다시 호출하지 않고 정확한 선언 종류 하나를 서버 소유 `query.manifest` 목록으로 컴파일합니다. 가시성과 현재 범위를 명시하는 기존 형식도 계속 지원합니다. 복수 선언 종류와 `available`, `queryable`, `readable`을 분리하거나 결합한 간결한 facet은 별도의 범위 facet이 없어도 같은 현재 principal 매니페스트를 사용합니다. 이 기계 필드들은 접근 범위를 넓히거나 발화 원문에서 의도를 추론하지 않습니다. 선언 종류 혼합, 모호성, 보조 의도 및 실행 권한이 있으면 이 간결한 경로로 진입하지 않습니다. 선언 개수 요청에서는 정본 `*Type` 대상 하나가 충돌하는 프레임 대상보다 우선하고 선언이 아닌 영역 대상은 제외합니다. 완전한 출력은 타입이 지정된 행 또는 개수와 읽기 전용 출처를 표시하고, 표시 종류를 모델이 작성한 노드 ID가 아니라 검증된 프레임에 결속합니다. 사용할 수 없는 매니페스트 함수, 불완전한 출력 및 다른 측정값은 기존의 안전한 실패 동작을 유지합니다. |
...
| 정확한 논리 서비스 런타임 상태 | 구현됨 | `semantic_logical_service_planning.py`, `semantic_logical_service_answer.py`, 운영 모델 프로바이더 및 런타임 토픽 결선, 집중 frame, verifier, 전체 DAG, 프로바이더, 런타임 변환, 배포, 로컬 동일성 및 이중 언어 표현 검사 | 정확한 `BusinessService` 또는 `Workload` id, 이름, 승인 alias 하나를 `implemented_by`와 `workload_runs_on`을 통해 App Service, Container Apps, Kubernetes Deployment, Service, Pod Resource에 연결합니다. 서버 소유 계획은 검증된 현재 Resource 상태만 읽습니다. 신원이 중복되면 그래프 I/O 전에 보류하며 매핑이 없거나 불완전하면 프로바이더 이름, 태그, 레이블 또는 fuzzy 일치로 넓히지 않습니다. 표현은 구성요소 관측을 보여 주되 전체 건강도나 원인을 추론하지 않고 `execution_authority=false`를 유지합니다. |
| 컬렉션 범위 Resource 상태 및 근거 기능군 격리 | implemented | `semantic_resource_state_planning.py`, `resource_state_queries.py`, `semantic_planning_value_filters.py`, `inventory-query-language.yaml`, 집중 플래너, FunctionType, 조립, prompt 및 표현 검사, Python 전체 회귀 shard 2에서 7,294개 통과 | 서버 소유 FunctionType이 보안이 적용된 Resource 컬렉션을 완전한 observed `StateFactMetadata`로 필터링합니다. 현재 속성별 메타데이터 묶음과 유지되는 이전 평면 형식을 모두 허용합니다. ObjectSet은 범위가 넓은 읽기를 매니페스트에 선언되고 검토된 운영 상태 경로와 상태 메타데이터가 있는 Resource 유형으로 먼저 제한하므로 관련 없는 인벤토리 양이 잘못된 빈 상태 결과가 되지 않습니다. 타입이 지정된 양성 필터와 선언된 상태 하나의 제외 필터는 서버 소유 닫힌 조건식으로 컴파일하며, 검증된 행은 관측된 공급자 중립 `resource_group` 및 `region` 값만 변환하고 누락된 범위는 사용할 수 없음으로 유지합니다. 원본 범위가 불완전하면 검증된 양성 일치 행은 불완전 상태로 표시하되 0행으로 전체 부재를 판단하지 않습니다. 스키마로 검증한 언어 레지스트리는 현재 인벤토리 상태와 구독 상태 평가를 분리하며, 바인딩되지 않은 상태 평가, 메트릭, 이력, 맥락 및 커버리지 기능군은 관련 없는 Resource 행 대신 타입이 지정된 보류를 반환합니다. 현재 상태 컬렉션이 상태 변경 이력도 요구하면 검증된 join 기능을 연결할 때까지 provider I/O 전에 보류합니다. 인증된 로컬 Console 관측은 구현 주장을 뒷받침하지만 통제된 릴리스 근거는 아닙니다. |
...
| 날짜 | 상태 | 변경 | 근거 | 남은 작업 |
|------|------|------|------|-----------|
| 2026-09-20 | implemented | 온톨로지 의미 세대 v2를 명시적 principal 범위 다이제스트에 결속하고 문서 매니페스트를 독립적으로 검사하며 검증 시 정확한 선언 본문을 재구성합니다. 다른 principal 재사용과 재해시 변조는 안전하게 차단됩니다. | `current change`, 수정 전 회귀 2개 실패, 수정 후 세대·선택기 테스트 23개 통과, 8,500행 증분 범위와 완전히 재해시한 선언 변조 포함, Ruff 및 strict mypy 통과 | 격리된 영속 인스턴스 기록기, 담당 에이전트 이벤트 수명 주기, 원본 세대 무효화 및 운영 후보 소비자를 구현해야 합니다. 이 검사는 Rule 인덱스 게시를 허가하지 않습니다. |
| 2026-09-20 | implemented | 거절되거나 다른 기능군인 의미 판단이 선언 목록·상세 프레임으로 다시 진입하지 못하게 했습니다. 라우팅 수정 후 실제 실행은 조회에 도달했지만, 객체 유형 판단이 거절된 뒤 모든 선언 종류를 선택했습니다. 이 부분 답변은 검증 성공으로 인정하지 않았습니다. | `current change`, 수정 전 음성 사례 6개 실패, 수정 후 계획 및 매니페스트 테스트 269개 통과, Ruff 및 strict mypy 통과 | 정확한 EN/KO 스키마 해석과 완전한 조회 증적은 아직 검증되지 않았습니다. 이 안전한 실패 수정은 범위 확대를 막을 뿐 모델 해석을 만들어 내거나 영속 인덱스·관련성 작업을 완료하지 않습니다. |
...
- [ ] 격리된 영속 인스턴스 세대와 담당 에이전트 이벤트 활성화를 구현하고 런타임 검색 연결 전에 동시 principal 격리, 완전한 원본 결속, 재시작·재생, 무효화 및 롤백을 입증합니다.
- [ ] 커밋된 간결한 facet 수정본을 반영한 뒤, 읽을 수 있는 객체 유형 질문의 EN/KO 증적이 모두 `object`만 선택하고 완전한 근거와 실행 권한 없음을 보존하는지 확인합니다.
