...
[대시보드 시안](../../../mocks/ui/dashboard.html)은 합성 데이터를 사용하는 화면 설계이며,
...
실제 콘솔에 적용하려면 인증과 서버 소유 범위 검사가 적용된 조회 결과, 동일 스냅샷 집계,
고정 정렬 순서, 스냅샷에 연결된 페이지 커서, 최신성 및 완전성 정보가 필요합니다.
검색은 현재 페이지만이 아니라 권한이 있는 전체 결과를 대상으로 해야 합니다. 스냅샷 만료와
응답 순서 역전은 세대를 섞지 않고 명시적으로 복구해야 합니다. 브라우저는 필터에서 범위 권한,
정상 여부 또는 접근 권한을 추론하지 않습니다. 로컬 생성기와 메모리 내 집계를 실제 조회 경로로
교체하고, 합성 예제는 대화형 Console에 가져오지 않습니다. 로컬 브라우저 검사는 화면 동작만
입증하며 서버 처리 용량이나 실제 운영 준비 상태를 입증하지 않습니다.
