...
translation_source_sha: a9fc8e0d6fe3e3753c42179b79a4911a812360a7
translation_revised: 2026-09-21
---

# 콘솔 운영

이 문서는 기존 FDAI Console이 운영 업무를 표시하고 범위가 제한된 운영 요청을 접수하는 방법을 정의합니다.
별도 애플리케이션, 범용 작업 항목 모델 또는 두 번째 실행 권한을 추가하지 않습니다.

> **제품 경계:** 제품명은 `FDAI Console`로 유지합니다. `Operations` / `운영`은 제품 안에 이미 존재하는 탐색 그룹입니다.
> 콘솔은 Thor의 실행기 신원을 받거나 관리 리소스를 직접 변경하지 않습니다.
>
> **지식 경계:** `Knowledge` / `지식`은 거버넌스가 적용된 문서 업로드와 리포지토리 원본 설정을 위한 동급 탐색 그룹입니다.
...
불확실성은 정확한 요청 키의 최종 결과로만 해소하며 승인과 효과 상태는 정본 Process가 소유합니다. 격리된 테스트 데이터로 인증된 화면의 접근성을 입증할 수는 없습니다. 구성 기준선, Skills, 대화 전달, 예측 학습의 근거 새로 고침은 기존 GET 조회만 사용합니다. 현재 보기를 유지하고 즉시 로딩 골격을 표시하며, 로딩 중 중복 실행을 막습니다. 소스가 복구되면 다시 읽을 수 있지만 생성자를 활성화하거나 자동 폴링 또는 변경 요청을 수행하지는 않습니다. 예측 학습은 결과 표와 함께 기록된 전체, 열린, 종료, 판단 보류 에피소드 수, 삭제 진행 건수, 기한이 도래한 가장 오래된 게시 시각을 표시합니다. 종료 결과가 없어도 진행 중인 에피소드를 숨기지 않으며, 이 개수로 예측 품질을 입증하거나 아직 없는 평가 집단의 측정값을 채우지 않습니다. 스케줄러 실행에서 현재 작업과 상태를 다시 조회하면 같은 URL로 이동만 하지 않고 기존 요청 세대 검증을 사용해 첫 근거 페이지를 다시 읽습니다. 이전 페이지 누적을 초기화하고 작업/상태 범위를 유지하며, 예약 작업을 재시도하거나 취소 또는 실행하지 않습니다.
...
별도 `/resource-dashboard` 경로는 기존 `/overview`와 기본 대시보드를 유지합니다. 이전 `/dashboard-v2` 경로는 쿼리 문자열을 보존하고 브라우저 이력에서 정식 경로로 교체되는 호환 별칭으로 유지합니다.
