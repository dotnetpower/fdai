...
translation_source_sha: 4ba5a14dfcfd25bc28b8b6b69307289692b8373d
...
불확실성은 정확한 요청 키의 최종 결과로만 해소하며 승인과 효과 상태는 정본 Process가 소유합니다. 격리된 테스트 데이터로 인증된 화면의 접근성을 입증할 수는 없습니다. 스케줄러 실행에서 현재 작업과 상태를 다시 조회하면 같은 URL로 이동만 하지 않고 기존 요청 세대 검증을 사용해 첫 근거 페이지를 다시 읽습니다. 이전 페이지 누적을 초기화하고 작업/상태 범위를 유지하며, 예약 작업을 재시도하거나 취소 또는 실행하지 않습니다.
