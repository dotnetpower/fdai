...
translation_source_sha: c3cebf4258302c2fd8d9a1c90519d578f9788155
...
이 배치는 선택한 Resource의 순서는 올바르게 잡지만 한 단계를 넘어서는 소유 관계는 여전히
표현하지 못합니다. 그래서 포함 관계는 중첩으로 옮깁니다. Resource가 포함한 대상을 그 안에
그립니다. 다른 상자 안에 놓인 상자는 방향을 잘못 가리킬 수 없고, 흡수한 포함 관계 선은 모두
캔버스에서 사라집니다. 소유자마다 자식을 정사각형에 가까운 격자로 채우고 한도가 제외한 자식
수를 밝히므로, 접힌 소유자를 빈 소유자로 오해하지 않습니다. 실측한 클러스터에서 중첩 배치는
현재 캔버스가 보여 주는 소유자 7개를 비슷한 면적에 담고, 한도를 적용하지 않으면 포함된
Resource 102개 전부를 4628 x 1452에 담습니다. 모델을 먼저 도입하며 렌더링은 바꾸지 않습니다.
중첩 그림이 대체하기 전까지 캔버스는 포함 관계를 계속 선으로 그립니다.
