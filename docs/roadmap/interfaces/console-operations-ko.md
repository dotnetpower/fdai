...
translation_source_sha: 48430e6a4e8afeddeba30f4e747b00ef0c3df774
...
Operator API는 에이전트 직접 호출 대신 타입 지정 이벤트를 중계합니다. 기능별 조립 모듈은 요청 상태와 감독되는 작업자를 연결할 뿐 실행기나 작업 흐름 권한을 갖지 않습니다.
출처 필터는 합계를 보존하며 알림 표 머리글은 읽기 쉬운 텍스트 토큰을 사용합니다. [알림 과다 수신 요청](../operations/alert-noise-governance-ko.md)은 원래 접수와 서명된 결과를 표시합니다.
불확실성은 정확한 요청 키의 최종 결과로만 해소하며 승인과 효과 상태는 정본 Process가 소유합니다. 격리된 테스트 데이터로 인증된 화면의 접근성을 입증할 수는 없습니다. 구성 기준선, Skills, 대화 전달, 예측 학습의 근거 새로 고침은 기존 GET 조회만 사용합니다. 현재 보기를 유지하고 즉시 로딩 골격을 표시하며, 로딩 중 중복 실행을 막습니다. 소스가 복구되면 다시 읽을 수 있지만 생성자를 활성화하거나 자동 폴링 또는 변경 요청을 수행하지는 않습니다.
...
온톨로지 인스턴스 작업 영역은 기본 `/ontology` 읽기 전용 운영 화면입니다. 선언 그래프를
요청하지 않고 마운트되며, 의미 모델, 선언, ActionType 및 카탈로그 토폴로지 참조는 기존 상세
링크를 유지한 채 이름이 명확한 하나의 HTML 기본 펼침 메뉴 안에 둡니다. 참조 보기를 불러오지
...
v2는 현재 인벤토리가 커밋된 온톨로지 매니페스트와 일치할 때만 기록 상태를 읽습니다.
반환 범위와 상세 화면으로 연결되는 개수를 하나의 스냅샷 표면에 표시하고, Resource 현황을 기본
작업 영역으로 유지하며, 근거 범위를 지도 아래에 배치합니다. 보조 열에는 우선 확인 기록 또는
선택한 Resource Inspector만 표시합니다. 이 시각 계층은 근거, 필터 또는 권한을 바꾸지 않습니다.
서로 다른 세대의 페이지를 차단하고 알 수 없음의 원인을 구분해 유지하며, 무효화 알림을 받으면
범위가 제한된 스냅샷을 교체합니다. 검색 중에는 목록 상한 밖의 Resource도 계속 찾을 수 있습니다.
제안을 직접 선택하지 않았다면 `Enter`는 입력한 검색어를 그대로 실행합니다.
...
Incidents 목록은 서버 소유 `title_source`를 렌더링합니다. `recorded_subject`는 기록된 대상과
사유를 사용하고, `identifier_fallback`은 계속 사용 불가로 표시합니다. 기계에서 파생한 제목에는
같은 근거에서 만든 길이가 제한된 `title_presentation` 메타데이터가 포함됩니다. Console은 원시
문구를 해석하지 않고 폐쇄형 템플릿을 현지화하며, 정리된 기술 맥락을 유지하고 사람이 읽을 수 있는
제목을 상세 화면 제목으로 사용합니다. 기록된 제목과 요약은 유지하며 목록은 월별 `incident_number`를 우선하고 이전 레코드는 `correlation_id`로 대체합니다. Audit, Trace, RCA, dossier 링크는 계속 정규 correlation
신원을 해석합니다.
...
사람 승인 결정은 서로 분리된 두 인증 전송 경로로 들어옵니다. Teams와 Slack은 서명된 공급자 콜백과 신원 매핑 검사를 유지합니다. FDAI Console은 `POST /hil/{approval_id}/operator-decision`을 사용하며, 서버에서 Entra 사용자와 `approve-runtime-hil` 기능을 확인하고 결정과 범위가 제한된 사유만 받습니다. 어느 전송 경로도 행위자, 역할, 작업 해시 또는 실행 권한을 직접 제공할 수 없습니다.

두 전송 경로 모두 정확한 승인 ID와 원래 보류 신원을 다시 읽습니다. 서버는 승인 잠금을 유지한 채 대기 상태, 만료 시각, 제출자, 결정 경로와 필수 역할을 확인한 다음 결정 증적과 지속성 outbox를 원자적으로 기록합니다.
역할 누락, 알 수 없는 경로, 오래된 맥락과 자체 승인은 권한을 부여하지 않습니다. 요청 접수나 브로커 수락은 Thor가 작업을 실행했다는 뜻이 아닙니다. 메타데이터가 없는 작업 보류 레코드는 Core 레지스트리가 정의한 정식 `action` 경로로만 정규화합니다. 명시적으로 비어 있거나 알 수 없거나 잘못된 경로는 계속 사용할 수 없습니다.
...
|-----------|---------------------------|
| 콘솔 구성 일치 | 개요, 운영, 에이전트, 거버넌스 시안은 Console 화면 등록 정보와 최신 중립적 시각 스타일을 따르며 운영 성과 대시보드와 리소스 대시보드를 구분합니다. 운영 화면은 탭과 미리 보기 선택기를 지표 위에 배치하고 공간이 충분하면 상세 내용을 목록 옆에 유지합니다. [운영 검사](../../../console/tests/e2e/operator-workspaces.spec.ts)와 [거버넌스 검사](../../../console/tests/e2e/governance-current-mock.spec.ts)는 경로, 조작, 반응형 화면을 확인합니다. 관찰 선호도(Observation affinity)는 Azure Resource Graph나 공급자를 직접 조회하지 않고, 검증된 합성 온톨로지 인스턴스 스냅샷에서 정확한 리소스 그룹과 리소스 식별자를 선택합니다. 원본이 불완전하면 선택을 차단합니다. 이 선호 설정은 기본 모니터링 범위를 줄이거나 권한을 부여하지 않습니다. 합성 예제는 운영 요청을 전송하지 않으며, 대기열 접수 예제로 수명 주기를 변경하거나 런타임 감사 근거를 만들어 내지 않습니다. |
...
현재 `Operations` 탐색 그룹을 하나의 제품 표면으로 유지합니다. 별도 셸을 만들지 않고 다음 화면을 추가하거나 개선합니다.
...
- **Operational 상세:** 출처 타임라인, 근거, 소유자 에이전트, 최신성, 사용할 수 있는 도메인 운영 기능을 보여줍니다.
- **온보딩 준비 상태:** 최종 배치의 네 개 요약 항목과 두 개 근거 섹션 형태를 유지하는 화면 전용 스켈레톤을 사용합니다.
  로드가 끝나면 준비 상태, 관찰 수, 요청 완료 시각, 담당 화면 링크, 이름이 붙은 리소스, 의미가 드러나는 역할 표, 출처 설명을 같은 계층에 둡니다.
  구성된 결과만 차이를 `누락`으로 표시하고, 프로브가 없거나 실패하면 저장소 기준을 `필수`로 표시하며 관찰 수는 사용할 수 없음으로 유지합니다.
  `Sample`은 합성 데이터임을 명시하고 정규 토큰을 유지하며, 테넌트 준비 상태를 주장하지 않고 `다시 불러오기`를 제공합니다.

서버 상태가 사용할 수 있는 운영 기능을 결정합니다. 브라우저는 사용성을 위해 컨트롤을 숨길 수 있지만 모든 제출은 권한 확인과 개정 번호 검사를 반복합니다. SSE는 영향받은 출처 참조를 무효화하고 클라이언트가 권위 있는 상태를 다시 읽게 할 수 있습니다.
