...
translation_source_sha: 6b8d6c930aedfd12a7172a4fef7ba6d8d61fdd37
...
사람 승인 결정은 서로 분리된 두 인증 전송 경로로 들어옵니다. Teams와 Slack은 서명된 공급자
콜백과 신원 매핑 검사를 유지합니다. FDAI Console은
`POST /hil/{approval_id}/operator-decision`을 사용하며, 서버에서 Entra 사용자와
`approve-runtime-hil` 기능을 확인하고 결정과 범위가 제한된 사유만 받습니다. 어느 전송
경로도 행위자, 역할, 작업 해시 또는 실행 권한을 직접 제공할 수 없습니다.

두 전송 경로 모두 정확한 승인 ID와 원래 보류 신원을 다시 읽습니다. 서버는 승인 잠금을
유지한 채 대기 상태, 만료 시각, 제출자, 결정 경로와 필수 역할을 확인한 다음 결정 증적과
지속성 outbox를 원자적으로 기록합니다. 역할 누락, 알 수 없는 경로, 오래된 맥락과 자체
승인은 권한을 부여하지 않습니다. 요청 접수나 브로커 수락은 Thor가 작업을 실행했다는
뜻이 아닙니다. 메타데이터가 없는 작업 보류 레코드는 Core 레지스트리가 정의한 정식
`action` 경로로만 정규화합니다. 명시적으로 비어 있거나 알 수 없거나 잘못된 경로는 계속
사용할 수 없습니다.
