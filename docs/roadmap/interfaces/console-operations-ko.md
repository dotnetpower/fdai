...
translation_source_sha: c76324028b8da68ce1f02fcad3ef98889ca11d91
...
사용하며 선택한 축을 표시합니다. 적용 가능한 운영 값의 누락은 계속 드러납니다. 이는 화면 표현
우선순위일 뿐이며 축을 합치거나 가용성 또는 프로비저닝에서 운영 성공이나 정상 상태를 추론하지
않습니다.
...
|-----------|---------------------------|
| 콘솔 구성 일치 | 개요, 운영, 에이전트 시안은 현재 Console 화면 등록 정보를 따르며 운영 성과 대시보드와 리소스 대시보드를 구분합니다. [운영 시안 검사](../../../console/tests/e2e/operator-workspaces.spec.ts)는 경로 목록, 로컬 조작, 반응형 화면을 확인합니다. 합성 예제는 운영 요청을 전송하지 않으며, 대기열 접수 예제로 수명 주기를 변경하거나 감사 근거를 만들어 내지 않습니다. |
...
Console 설정과 정적 시안은 Calm Slate 컨트롤 토큰과 표현 프리미티브를 공유합니다.
데스크톱 폼은 34 px 표준 컨트롤과 28 px 간소 작업을 사용하며 터치 대상은 44 px입니다.
브라우저 로컬 환경 설정, 계정 환경 설정, 배포 정책, 근거, 권한은 시각적으로 구분하되
영속성이나 권한 부여 계약은 변경하지 않습니다.
