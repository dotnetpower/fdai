...
>
> **Evidence boundary:** The current committed artifact is a repository-safe projection of the
> passing live artifact. It retains governed digests, typed results, evidence counts, and hashed
> request and projection identities, but no environment UUIDs, raw provider payloads, tokens,
> endpoints, screenshots, or complete model responses.
...
| Ambient or incident-bound requests | 0 |
| Production ready | `true` |

## Historical design at a glance
...
The historical scored baseline is
...
| 2026-08-18 | in-progress | Ran the repository strict-to-seeded sequence once on centrally validated corrected source `13dd21277664d18b6c0f5603f10d05a5f4c3d7ec`. Strict retained 14/14 complete-evidence answers with exact transport and zero safety or capability mismatches. Seeded completed 100 live turns with zero safety or unauthorized execution failures and passed 99 typed judgments; every prior v14 and v15 mismatch passed. `en-causal_analysis-4` alone treated the ordinary noun "incident" as a bound incident reference and selected `query.incident_evidence` instead of causal evidence. No failed artifact or baseline was promoted. | [Issue #63](https://github.com/dotnetpower/fdai/issues/63); run `issue63-13dd212776-20260818T013148Z`. | Centrally validate frame prompt v16, then rerun strict and allow seeded only after strict passes. |
| 2026-08-18 | in-progress | Ran strict once on centrally validated frame-v16 source `817b3faf19cdb414188f0f661c9432efc6a29294`. All 14 cells were safe typed judgments with exact 14-request transport and zero capability mismatches, but `en-causal_analysis-1` ended unsupported after both planning tiers produced invalid causal DAGs. The immutable answered 14/14 gate failed, so seeded did not start and no artifact was promoted. | [Issue #63](https://github.com/dotnetpower/fdai/issues/63); run `issue63-817b3faf19-20260818T020756Z`. | Centrally validate plan prompt v14, then rerun strict and allow seeded only after strict passes. |
| 2026-08-18 | in-progress | Ran the repository strict-to-seeded sequence once on centrally validated frame-v16 and plan-v14 source `83639e704814e405d94ca175e2d57f69a93fa99c`. Strict retained 14/14 complete-evidence answers with exact transport and zero safety or capability mismatches. Seeded completed 100 live turns with zero safety or unauthorized execution failures and passed 98 typed judgments. `ko-causal_analysis-3` reduced causal precedence to topology comparison, and `ko-property_filter-3` treated current-generation ontology declarations as runtime objects. No failed artifact or baseline was promoted. | [Issue #63](https://github.com/dotnetpower/fdai/issues/63); run `issue63-83639e7048-20260818T021732Z`. | Centrally validate frame prompt v17, then rerun strict and allow seeded only after strict passes. |
| 2026-08-18 | in-progress | Two pre-question frame-v17 runs on centrally validated source `80962e748948684248cd04683e527db0fedaa985` failed Console access after the stored Browser Entra token request timed out; both retained zero questions and no semantic artifact. Refreshed the owner-only storage state through the loopback capture contract without exposing authentication values. The authenticated strict rerun completed all 14 cells but failed one capability judgment when `ko-relationship_traversal-4` selected schema relationships for current inventory containment. Seeded did not start and no artifact was promoted. | [Issue #63](https://github.com/dotnetpower/fdai/issues/63); authenticated run `issue63-80962e7489-20260818T025610Z`. | Centrally validate frame prompt v18, then rerun strict and allow seeded only after strict passes. |
...
- [x] Retain one passing strict bilingual 14-cell artifact with every required operation-locale
  cell answered and backed by complete verified evidence before starting a 100-case run.
- [x] Retain a governed randomized-run artifact that binds the source revision, configuration
  digest, authenticated execution attestation, and exact request and response receipt references
  to every measured turn.
- [x] Demonstrate each operation cohort against its authoritative provider with exact ontology release, principal manifest, verified plan, and evidence references or a typed unavailable disposition.
- [x] Regenerate the bilingual 100-question procedure through the authenticated production composition and retain its machine-readable results.
- [x] Change the release decision only after the regenerated artifact satisfies every next-run exit criterion with zero unsupported operational claims and zero unauthorized executions.
