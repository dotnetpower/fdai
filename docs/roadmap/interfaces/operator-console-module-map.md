...
The service-owned IAM manifest pins methods, paths, names, and envelopes. Pure handover transitions, restricted PostgreSQL readers, and the document-route checklist keep current people, source/ACL evidence, revisions, independent review, and durable session budgets server-owned. Current Core goal/reviewer/admission/retrieval bindings are connected. Owner-only `GET /handover/readiness` now reports empty `source_gaps`, but retains external blockers, ten-minute expiry, `shadow`, and `operationally_ready=false`. Operator records original HIL decisions; fixed owners consume them for isolated execution without giving the browser, Operator, or Core a mutation identity. The [final record](../../internals/handover-lifecycle-hardening-20260914.md#final-integrated-critique-after-remaining-source-implementation) completes 12 distinct integrated source-review rounds after remaining implementation, with no unresolved confirmed Medium/High source finding. [Human-agent assignment](human-agent-assignment-implementation-plan.md#current-change-evidence-and-remaining-scope) keeps translation refresh, canonical generation, hooks, publication/CI, and full UI/assistive/live evidence open separately.
