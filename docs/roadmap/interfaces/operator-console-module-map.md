...
| Deterministic subscription terminal boundary | implemented | `adapters/subscription_scope.py`; `families/conversation/subscription_scope.py`; `postgres_semantic_turn_store.py`; focused Operator and Console checks | A catalog-matched subscription identity request is stored as local work, cannot be claimed by the semantic outbox, and replays one typed verified terminal receipt through the existing principal-scoped stream. Core wire ownership remains unchanged. |
...
| 2026-08-17 | implemented | Added an Operator-local deterministic terminal path for current-subscription identity reads without widening the Core projection contract or semantic outbox. | `current change`; focused durable bridge, provider, parser, and type checks. | Retain an authenticated browser receipt for both locales. |
