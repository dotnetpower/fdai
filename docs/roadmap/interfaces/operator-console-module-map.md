...
| Continuous question-space assurance oracle | implemented | [Continuous Question Space](continuous-question-space.md); `console/tests/live-e2e/ontology-query-assurance.ts`; focused Console tests | The Console harness owns the fixed bilingual 100-case taxonomy and strict v2 22-cell oracle. It verifies exact plan capabilities, transport identity, complete evidence for answered turns, and zero unauthorized execution without becoming runtime infrastructure. |
