...
The service-owned IAM manifest pins methods, paths, names, and envelopes. Console kind membership uses `Set` lookups with unchanged exact-effect and no-authority checks, without regex or baseline exceptions. Pure handover transitions, restricted PostgreSQL readers, and the document-route checklist keep current people, source/ACL evidence, revisions, independent review, and durable session budgets server-owned. Current Core goal/reviewer/admission/retrieval bindings are connected. Owner-only `GET /handover/readiness` reports empty `source_gaps`, but retains external blockers, ten-minute expiry, `shadow`, and `operationally_ready=false`. Operator records original HIL decisions; fixed owners consume them for isolated execution without giving the browser, Operator, or Core a mutation identity. The [final record](../../internals/handover-lifecycle-hardening-20260914.md#final-integrated-critique-after-remaining-source-implementation) records 12 distinct integrated source-review rounds with no unresolved confirmed Medium/High source finding at that checkpoint. The second local main merge succeeded and was published as `c8edd2769` in [PR #1014](https://github.com/dotnetpower/fdai/pull/1014), but [CI run 34921323157, attempt 1](https://github.com/dotnetpower/fdai/actions/runs/34921323157/attempts/1) failed. Focused source repairs are implemented locally; `c8edd2769` remains the latest published head. The [ledger](../../roadmap-implementation/interfaces/operator-console-module-map.md) keeps reviewed translation refresh, generation, repair hooks/PR update, and passing exact-head protected CI/merge under [#946](https://github.com/dotnetpower/fdai/issues/946) separate from full UI/assistive/live/deployment/promotion evidence.
The actual `/agent-oversight/mapping-reviews` route now hosts `ScopedDutyWorkspace` with separate model, API, localized copy, and style modules. Its six existing ownership-only routes expose exact catalog/scope/case reads and create/submit/review proposals: user/group/schedule duties, UTC windows, static fallback, and supersession never imply personal IAM or future coverage. HTTP 202 stays `awaiting_core` until an explicit GET; uncertain retries preserve request identity, account changes reset state and fence late responses, and no case-list API, background polling, browser persistence, or provider writer is added. The [ledger](../../roadmap-implementation/interfaces/operator-console-module-map.md) records focused unit and actual-route desktop/mobile/account-reset/contrast evidence, not complete WCAG/rubric or live certification.
