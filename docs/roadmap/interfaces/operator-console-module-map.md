...
| Shared semantic row projection | implemented | `families/conversation/presentation_rows.py`; v1 and v2 presentation modules; focused Operator presentation checks (`94 passed`) | One bounded pure projection keeps direct scalars, lifts only `name`, `type`, `status`, and `location` through at most two nested verified property bags, and leads with readable fields before opaque identity. Exact untouched rows remain in technical details. |
