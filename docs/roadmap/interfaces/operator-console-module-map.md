...
The service-owned IAM manifest pins methods, paths, names, and envelopes. Console `Set` membership preserves exact-effect and no-authority checks without lexical exceptions. Current people, source/ACL evidence, revisions, independent review and session budgets remain server-owned. Core goal/reviewer/admission/retrieval bindings are connected; Owner-only readiness has empty `source_gaps` but retains external blockers, ten-minute expiry, `shadow` and `operationally_ready=false`. Operator records original human decisions without giving the browser, Operator or Core a mutation identity. [PR #1014](https://github.com/dotnetpower/fdai/pull/1014) delivered reviewed head `8c1d9977c` as `953a17de4`; exact-head CI `34925881557` and post-merge CI `34926168342` succeeded, completing #946. Earlier failures remain in the [append-only ledger](../../roadmap-implementation/interfaces/operator-console-module-map.md), not current delivery blockers.
The actual `/agent-oversight/mapping-reviews` route hosts `ScopedDutyWorkspace` with separate model, API, localized copy and styles. Its six existing ownership-only operations preserve exact subjects/scopes, UTC windows, fallback, supersession and manual GET after HTTP 202. Uncertain retries retain identity; account changes fence late replies. The document checklist renders each server-advertised operation independently and clears client/goal-private state on changes. Both regions recover lost request focus without stealing deliberate navigation, expose field corrections, and retain shared loading/error primitives. The [#1017 local review](../../internals/handover-ui-evidence-20260915.md) records 28 distinct synthetic browser scenarios, 127 unit checks, ten focused and ten final critiques, and all 50 rubric IDs. Real screen-reader evidence remains `needs-human`; no final score, WCAG/live certification, new API, polling, persistence or provider writer is claimed.
Repository catalogs are materialized into revisioned Operator projections, while `runtime_projection_reader.py` and `conversation_assurance_reader.py` read only the durable tables granted to the Operator role. Conversation assurance detail joins a stored assessment to semantic request and result records only when principal, conversation, and turn identities all match. Browser replay retains that authoritative assessment identity, hydrates an older cached answer once when the identity is missing, and uses the same detector for initial restoration and explicit retry. List refresh accepts only the latest in-flight response. A valid content-addressed link can read principal-scoped detail beyond the bounded list, while a missing detail stays unavailable and refreshes both list and detail generations. Multiplexed non-request Kafka publications add their logical-topic marker before one bounded encode; schema-validated semantic requests retain their producer-codec boundary. A Sample outcome exposes its Live-evidence separation as an accessible note and keeps that note within desktop and mobile content bounds. These readers return measured records or explicit empty evidence states and never infer execution eligibility.
The WARA read model preserves exact evaluator identity and structured manual-evidence requirements. Console explains the required producer, scope, freshness ceiling, and owner without exposing an evidence writer or remediation control.
The Ontology Instances projection keeps observed `runtime_calls` separate from generic direct relationships in the Inspector, dense legend, and current-screen evidence context. It preserves the stored source-to-target direction and cannot infer a call when the runtime source is unavailable. Grounded source controls include primary and secondary evidence-posture labels in accessible names. The Live ARG count reads the active immutable inventory snapshot without waiting for ontology generation alignment; per-resource state pages retain that strict alignment. Bounded transcript storage finds the longest retained suffix with logarithmic full-array encodes. The Console panel registry keeps every route module behind a lazy import. Named route exports use one typed adapter, routes that share a module reuse one loader, and Incident queued/applied status reuses existing timeline catalog entries. The main catalog also defines dedicated intervention-applied copy without changing the audit timeline's authority. The production entry-bundle check verifies the required lazy boundaries and enforces both raw and gzip limits.
...
`console/src/routes/dashboard-v2.tsx` and `dashboard-v2.css` own the Resource dashboard's compact returned-scope presentation. Provenance and snapshot time remain visible, every count remains a filtered native link, and the low-height desktop strip reflows at narrow widths without changing recorded data, authorization, or execution authority.
