...
> 세션 가용성 event, 제한된 invitation, snooze, decline, 근거 연결, 독립 review가
...
거버넌스 화면은 Agent oversight 작업 공간을 제공합니다. 사람 중심 IAM 편집기나 중첩 카드
...
| 지식 인수인계 | 에이전트 소유 목표 템플릿, 근거 가중치, ACL과 출처 span, 최신성, 피로도 예산, 로그인 초대입니다. |
...
동일한 소스 범위를 인용합니다.
조각 기록에는 타입이 지정된 출처 span, ACL 참조, chunk-policy 버전, 내용 다이제스트,
선택적 목표 참조가 포함됩니다. 후보 event는 내용이 없는이며 항상 review가 필요합니다.
자동 goal-to-upload 연결과 agent 후보 전달은 롤아웃 작업으로 남습니다.
...
1. **할당 프로젝션:** 통합 읽기 모델, 커버리지 검증기, IAM ID 프로젝션, 거버넌스 Agent
