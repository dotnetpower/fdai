...
ID, FDAI App Role, 접근 요청을 담당합니다. `Governance > Agent oversight`는 사람 의존성 맵,
...
### Agent oversight 작업 공간

Governance 화면은 Agent oversight 작업 공간을 제공합니다. 사람 중심 IAM 편집기나 중첩 카드
...
| 사람 의존성 | 고정 Pantheon 역할, 에이전트와 범위의 담당 체계, 정확한 주체, 그룹 또는 일정, 유효 기간, fail-closed 검증입니다. |
...
1. **할당 프로젝션:** 통합 읽기 모델, 커버리지 검증기, IAM ID 프로젝션, Governance Agent
