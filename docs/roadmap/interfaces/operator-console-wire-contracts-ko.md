...
translation_source_sha: aa03866ef49e2d8805a91ac746a9e770e8bfa3ea
...
- **인시던트 초안**: 검증된 `incident_create` 판정은 범위가 제한된 심각도와 대상,
  ...
  초안을 반환합니다. 서버는 원본 다이제스트와 10분 만료 시각을 보존합니다. 초안을
...
- **타입이 지정된 인시던트 확인**: `POST /chat/action/confirm`은
  ...
  변경된 인자, 지원되지 않는 작업 유형은 차단됩니다.
- **전용 인시던트 전송**: 보낼 편지함은 `IncidentCreationRequest`를
  `operator.incident-creation.requests`에 게시합니다. Core는 기존
  `IncidentLifecycleWorkflow`가 인시던트를 생성하거나 재사용하기 전에 다이제스트,
  역할 하한, 원본 신원, 파티션 키, 권한 없음 필드를 검증합니다. 이 레코드 작업은
  Thor를 사용하지 않으며 ActionType의 shadow 또는 적용 모드에 의존하지 않습니다.
- **호환 엔드포인트**: 독립 Operator Service는 이전 raw-prompt
  `POST /chat/action` 경로를 등록하지 않습니다. 브라우저와 API 클라이언트는 의미 기반
  초안과 타입이 지정된 확인 경로를 사용하므로 자연어가 쓰기 엔드포인트로 직접
  전달되지 않습니다.
...
- **응답**(수락됨): `202 {"submitted": true, "created": false,
  "action_type": "incident.create", "request_id": ..., "dispatch_status":
  "pending", ...}`입니다. 이 응답은 영속 수락만 입증하며 인시던트 생성 완료를
  입증하지 않습니다. Core가 `incident.open`을 기록한 뒤 권위 있는 `/incidents`
  변환 결과에서 완료 여부를 확인합니다.
...
`logical_service_current_state` 답변 형식은 정확한 BusinessService, Workload, 런타임 Resource
매핑과 최신 검증 구성요소 상태를 표시합니다. 비어 있거나 불완전한 매핑은 명시적으로 유지하며
전체 서비스 건강도나 원인을 파생하지 않고 항상 `execution_authority=false`를 유지합니다.
...
선언 워크벤치는 요약 payload를 늘리지 않고 다음과 같은 범위가 제한된 읽기 경로를
추가합니다.

- `GET /ontology/declarations/{kind}/{name}`은 활성 release에서 역할과 목적에 따라
  필터링한 ObjectType, LinkType 또는 ActionType 상세 하나를 반환합니다.
- `GET /ontology/declarations/{kind}/{name}/dependents`는 결정론적 카탈로그 토폴로지
  참조만 반환하며 결과 상한과 잘림 상태를 명시합니다.
- `GET /ontology/object-types/{name}/evidence-health`는 정제된 원본, 세대, 기준 시점,
  최신성, 완전성, 충돌, 합성 여부 및 집계 수를 반환합니다. 런타임 원본이 연결되지 않은
  유형은 사용할 수 없음으로 표시하며 0을 만들어 내지 않습니다.
...
없는 기존 ActionType은 완전성을 낮추며 추론하지 않습니다. InterfaceType과 FunctionType은 의미 있는
...
| 대화 assurance 최종 묶음 | implemented | Core 의미 턴 처리기, Operator 의미 턴 런타임 및 presentation, 집중 assurance 전달 테스트 | 유효한 `completed`, `deferred`, `held` 평가는 범위가 제한된 사유와 정확한 답변 생성 모드를 보존합니다. Assurance 변환 결과 실패는 `semantic_runtime_failed`를 사용하고 `result_store_*` 단계 실패만 `semantic_result_store_unavailable`을 사용합니다. |
...
- [ ] 카탈로그 데이터를 런타임 근거로 표현하지 않으면서 catalog 다이제스트, ObjectType, LinkType, ActionType, 워크벤치 상세 및 생성된 map을 묶는 관리되는 읽기 전용 온톨로지 증적 하나를 보존합니다.
