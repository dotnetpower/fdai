...
translation_source_sha: bffefd2f04be6f3165665f71636afaed1498f88b
...
- **배포 주기:** FDAI 설계와 구현 지식은 운영 컨트롤 플레인을 다시 배포하지 않고 갱신할 수
  ...
  바뀌지 않은 병합 시점 갱신은 원본 커밋과 전체 다이제스트만 변경합니다. 인용한 소유 문서를
...
같은 레코드를 다른 시각에 다시 빌드하면 별개의 패키지 산출물이 생성됩니다. 인용한 소스가
바뀌면 같은 변경 집합에서 패키징 전에 카탈로그를 다시 빌드해야 blob 고정값과 다이제스트가
release 트리와 일치합니다. 서식만 압축하거나 경로 맥락을 재배치하거나 Markdown 구조를 수정하면
...
다시 빌드 규칙을 적용합니다.
Loki의 검증된 `object.resilience-score` 경로를 포함한 Pantheon producer 수정은 인용과
다이제스트만 갱신합니다. 카탈로그는 정규화 Event를 소비하거나 runtime 후보를 만들지 않습니다.
...
콘솔 복원력과 문서 버전 표현 원본 개정에도 같은 규칙을 적용합니다. 재시도, 로딩, 책임 분리
변경은 원본 blob 고정값과 카탈로그 다이제스트만 갱신하며 카탈로그 내용, 검색, 메시지 전달
또는 권한 범위를 넓히지 않습니다.
A3-E 원장 표 복구도 원본 고정값만 갱신합니다. 승인 검증기를 연결하거나 복귀를 호출하거나
운영 근거를 확립하지 않습니다.
임계값 동등성 원본 개정에도 같은 규칙을 적용합니다. 카탈로그는 검토된 절대 범위와 구성 분리를 기록하지만 활성 임계값을 선택하거나 정책을 승격하거나 런타임 권한을 부여하지 않습니다.
질문은행 원본 통합과 소유 문서 압축에도 같은 규칙을 적용합니다. 생성 다이제스트 갱신은 질문
400개를 유지하지만 의미 커버리지나 실제 운영 근거를 인증하지 않습니다.
격리된 AKS 지원에 대한 Action Ontology 원본 고정값 갱신도 조건부 런타임 지원만 기록하며 승인,
배포 또는 실행 권한을 부여하지 않습니다. 어댑터 테스트를 격리된 실행기 서비스 모음에 할당해도
수집 메타데이터만 바뀝니다.
적응형 Azure Monitor 조사 원본에도 같은 규칙을 적용합니다. 카탈로그는 원시 KQL을 노출하거나
조회 권한을 부여하지 않고 검토된 recipe 선택, 고정 KQL 실행 및 no-data 제한을 설명합니다.
