...
연결하는 제한된 파이프라인을 정의합니다. 일반 개선 트리거는 이 경계에서 새롭고 근거 준비가 완료된 읽기 사례 하나를 선택하고 해당 질문을 재시도하지 않는 browser 소유 측정 1회만 허용합니다. 이 파이프라인은 읽기 전용이며 `shadow` 모드로
...
현재 생성기 갱신도 누적 Console 카탈로그와 정규 명확화 문구 변경에 이 규칙을 적용합니다. 원본 및 인벤토리 다이제스트만 바뀌고 400개 질문 코호트, 온톨로지 범위 수치, 근거 제한 및 권한은 변경되지 않습니다.
...
| Strict v2 릴리스 게이트 | implemented | `console/tests/live-e2e/ontology-query-assurance.ts`; `scripts/automation/run_ontology_assurance.py`; `scripts/automation/ontology_assurance_{evidence,execution,preparation,supervisor}.py`; 집중 Console 및 감독기 검사 | 호환성 실행기는 불변 근거 게이트, 프로세스 실행, 격리 작업공간 준비를 책임이 분리된 모듈에 위임합니다. 고정 100개 사례는 영어와 한국어 각각 50개를 유지합니다. Strict v2는 기존 14개와 선언, 릴리스/근거, 인벤토리 영향, Rule 상태를 두 로캘로 추가한 22개를 선택합니다. 릴리스 근거는 정확한 전송 22/22를 요구합니다. |
...
| 날짜 | 상태 | 변경 | 근거 | 남은 작업 |
|------|------|------|------|-----------|
| 2026-09-27 | implemented | 테스트 맥락 Console 레이블로 연결된 영어와 한국어 카탈로그 다이제스트가 바뀌어 질문 400개의 질문은행과 이에 의존하는 의미 의도 범위 산출물을 다시 생성했습니다. 모든 질문 신원, 준비 상태, 지표 정의 93개, 분모, 범위 수치, 권한은 그대로입니다. | `current change`; 공식 `build_question_bank.py` 실행 후 `build_semantic_intent_coverage.py` 실행, 질문은행 및 의미 범위 산출물의 정확한 일치 검사 통과. | 사례별 의미 검토와 런타임 검증을 계속합니다. 이 다이제스트 갱신은 답변 가능 상태나 운영 준비 근거를 추가하지 않습니다. |
...
| 2026-09-16 | implemented | upstream Console 카탈로그를 통합하면서 기록된 영어 및 한국어 소스 다이제스트만 바뀐 뒤 통합 질문은행 인벤토리를 다시 생성했습니다. 질문 400개의 신원, 문구, 준비 상태 및 권한 필드는 바뀌지 않았습니다. | `current change`; 공식 질문은행 생성기, 생성 산출물 집중 동등성 검사 통과. | 의미, 런타임 연결, 근거 또는 권한 동작은 바뀌지 않았습니다. |
| 2026-09-16 | implemented | 보호된 main 통합으로 후보 원본 하나가 바뀐 뒤 질문 400개 인벤토리를 유지하면서 통합 질문은행 산출물을 다시 생성했습니다. | `current change`; 공식 질문은행 생성기, 산출물 동등성 집중 검사 8개 통과. | 의미, 런타임 연결, 근거 또는 권한 동작은 바뀌지 않았습니다. |
...
| 2026-09-14 | implemented | 검토된 역할 및 소유권 레이블 변경으로 Console 카탈로그 원본 다이제스트가 바뀐 뒤 통합 질문은행을 다시 생성했습니다. 질문 400개의 신원, 검토 상태, 지표 및 분모는 그대로 유지했습니다. | `current change`; 공식 질문은행 생성기 및 집중 생성 산출물 동등성 테스트. | 의미, 런타임 연결, 근거 또는 권한 동작은 바뀌지 않았습니다. |
...
- [ ] `sha256:7c2c5f31de2bc23c98dcccdfe5e411b7c030b54e7c72a63a0678c4abb93a4a0a`로 식별한 60개 사례 표본의 모든 준비도 축을 충족합니다. 각 사례에는 검토된 문구, 의미 계약, 도달 가능한 런타임 연결, 권위 있는 근거 출처, 계약 검증 및 질문별 표현 oracle이 필요합니다.
