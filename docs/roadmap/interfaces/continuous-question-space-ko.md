...
| 구조화된 조사 보증 | implemented | `semantic_{investigation,investigation_planning,planning_cascade}.py`, `semantic_turn_presentation.py`, Azure activity 및 metric 신원 adapter, 조사, shipped catalog, query-node 및 이중 언어 표현 테스트 | 대상 결속 diagnostic 인과 사례는 정확한 entity 해석, 정렬된 근거 window, 온톨로지로 순위를 정한 관계 경로, 증상 방향, Activity Log 근거, 경쟁하는 지지 및 반증을 요구합니다. T1이 정확한 대상과 causal frame을 유지했지만 중첩 조사만 누락하면 Core는 T2를 호출하지 않고 검증된 span, 매니페스트 선언, 검토된 metric concept로 이를 완성합니다. 최종 artifact와 상세 활동 13개는 대상, 증상, window, 측정된 변화, 각 query 결과, 가설 근거, limitation, evidence reference, confidence basis, `execution_authority=false`를 보존합니다. Container Apps service latency는 exact-resource `ResponseTime`을 사용합니다. Dependency metric node는 같은 exact affected-target ObjectSet을 사용하고 relationship traversal은 별도 topology 단계로 유지됩니다. Request 및 dependency span은 표준 `cloud.resource_id` 속성이 있어야 하며, 없으면 명시적인 provider gap으로 유지됩니다. Console UI 기본 설정이 영어여도 한국어 prompt는 해당 turn의 한국어 응답을 선택합니다. |
...
| Cloud-operations golden dataset | in-progress | `eval/golden-dataset/`, `scripts/automation/build_golden_dataset.py`, `tests/integration/evaluation/test_golden_dataset.py` (`9 passed`) | 논리적 expectation 35개가 표현 방식 8개에 걸쳐 영어와 한국어 280쌍을 생성합니다. 생성된 모든 사례는 개념적 anchor 종류와 구현된 runtime context를 구분합니다. 장애 결속과 서버 범위는 실행 가능한 입력으로 유지하고 객체별 첫 요청은 정확한 대상 명확화를 요구합니다. 정적 검증은 공식 Azure 및 Kubernetes 리소스 어휘, 검토된 일반 약어, catalog 및 FunctionType 참조, 실제 식별자 비포함 경계를 결속합니다. 의미 캠페인 실행과 답변 인증은 남아 있습니다. |
