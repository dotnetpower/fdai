...
translation_source_sha: 19628fe534cbc4c322cda71d410f8beb89403cc7
...
서비스 소유 IAM 매니페스트는 메서드, 경로, 이름, 응답 묶음을 고정합니다. Console의 종류 포함 여부는 `Set`으로 확인하며 정확한 효과 및 권한 없음 검사는 바꾸지 않고 정규식이나 기준선 예외도 추가하지 않습니다. 순수 인수인계 전환, 제한된 PostgreSQL 조회기, 문서 화면 체크리스트는 현재 사람, 원본/ACL 근거, 리비전, 독립 검토, 영속 세션 예산을 서버에 유지합니다. 현재 Core 목표/검토자/원본 허용/검색은 연결되어 있습니다. Owner 전용 `GET /handover/readiness`의 `source_gaps`는 비어 있지만 외부 차단 요인, 10분 만료, `shadow`, `operationally_ready=false`는 유지합니다. Operator가 원래 HIL 결정을 기록하고 고정 담당자가 격리 실행에 사용하며 브라우저, Operator, Core에는 변경 신원을 주지 않습니다. [최종 기록](../../internals/handover-lifecycle-hardening-20260914.md#final-integrated-critique-after-remaining-source-implementation)은 서로 다른 통합 소스 검토 12회와 해당 체크포인트에서 미해결로 확인된 Medium/High 소스 문제가 없음을 기록합니다. 두 번째 로컬 main 병합은 성공했고 [PR #1014](https://github.com/dotnetpower/fdai/pull/1014)에 `c8edd2769`로 게시되었지만 [CI 실행 34921323157의 1차 시도](https://github.com/dotnetpower/fdai/actions/runs/34921323157/attempts/1)는 실패했습니다. 집중 소스 수정은 로컬에서 구현되었으며 최신 게시 헤드는 여전히 `c8edd2769`입니다. [구현 원장](../../roadmap-implementation/interfaces/operator-console-module-map.md)은 검토 후 번역 갱신, 정본 생성, 수정본 훅/PR 갱신, 정확한 헤드의 보호된 CI 통과/병합을 [#946](https://github.com/dotnetpower/fdai/issues/946)에서 추적하고 전체 UI/보조 기술/실제 운영/배포/승격 근거와 구분합니다.
실제 `/agent-oversight/mapping-reviews` 경로에 이제 `ScopedDutyWorkspace`와 별도의 모델, API, 현지화 문구, 스타일 모듈이 연결되어 있습니다. 기존 담당 체계 전용 경로 6개는 정확한 카탈로그/범위/사례 조회와 생성/제출/검토 제안을 제공합니다. 사용자/그룹/일정 임무, UTC 구간, 고정 대체 담당자, 사례 대체는 개인 IAM이나 미래 담당 권한이 아닙니다. HTTP 202는 명시적 GET 전까지 `awaiting_core`이며 결과가 불확실한 요청의 재시도는 같은 신원을 유지합니다. 계정 변경은 상태를 초기화하고 늦은 응답을 차단합니다. 사례 목록 API, 백그라운드 폴링, 브라우저 저장, 공급자 쓰기는 추가하지 않았습니다. [구현 원장](../../roadmap-implementation/interfaces/operator-console-module-map.md)은 집중 단위 검사와 실제 경로의 데스크톱/모바일/계정 초기화/대비 근거를 기록하며 전체 WCAG/평가 기준이나 실제 운영 인증을 주장하지 않습니다.
...
원장과 근거 공백 때문에 보류된 행에 서로 다른 문구를 사용합니다. 좁은 화면에서는 불투명한 출처 정보와 보류된 식별자 값이 기본 콘텐츠 너비 안에서 줄바꿈되고 테이블 넘침은 자체 스크롤 컨테이너 안에
