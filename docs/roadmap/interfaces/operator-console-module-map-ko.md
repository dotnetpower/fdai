...
translation_source_sha: dbb0c080ef6d712c3a595993cabe61a0970b6120
...
`console/src/components/browser-notification-control.tsx`는 명시적인 클라이언트 로컬 `console-web` 선택을 소유합니다. Principal 범위의 제한된 원장은 표시와 클릭 확인을 분리하고 실제 관찰된 같은 출처 인시던트 대상만 허용하며 Web Locks로 수신자 하나를 선출합니다. 각 claim은 예측 불가능한 확인 token을 사용하며 tag와 token이 모두 일치해야 표시, 실패 해제 또는 확인의 단조 상태를 바꿀 수 있습니다. 재생, 합성, 출처 미확인, principal 불일치, 이전 세대 및 token 없는 확인 시도는 fail-closed 처리합니다. 등록, 저장소, entropy 및 leader 오류는 범위가 제한된 사용 불가 또는 재시도 상태가 됩니다. 로컬 증적은 Core 전달을 갱신하거나 근거 읽기를 입증하거나 승인 또는 실행 권한을 부여하지 않습니다.
화면에 보이는 제목 컴포넌트는 렌더링되는 제목 내용에만 `title`을 사용합니다. 기본 HTML `title` 말풍선은 접근성과 호환되는 요소로 제한하며, 그 밖의 맥락 도움말은 공통 Tooltip 컴포넌트를 사용합니다. 정적 컴포넌트 갤러리는 `mocks/ui/assets/component-registry.json`의 계약을 읽고 각 시안을 소유자, 원본, 상태, 사용 지침, 반응형 동작, 접근성 계약 및 제품 참조보다 먼저 표시합니다. 레지스트리가 없거나 잘못되면 정규 상태를 추론하지 않고 문서화를 차단합니다. 갤러리는 합성 표현 근거이며 Console, Operator API 또는 실행기 권한을 부여하지 않습니다.
