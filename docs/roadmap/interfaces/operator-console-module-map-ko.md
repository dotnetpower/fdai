...
package 책임, 경로 계열 분류, 후보 대상 및 가져오기 표면 상태를 기록합니다. 이 인벤토리는
file-count 목표가 아닌 설명 기준이지만, executable 완전성 gate는 현재 모든 모듈 디렉터리와
경로 모듈을 분류된 상태로 유지하도록 요구합니다.
후보 대상은 package 힌트입니다. 새 프로세스, 신원, 전송 계층 또는 데이터 소유자의 gate는 [서비스 승격과 데이터 소유권](../architecture/service-graduation-and-ownership-ko.md)입니다.
[`test_operator_api_layout.py`](../../../services/operator-service/tests/)는 현재 모든
package와 경로 모듈이 분류된 상태인지 확인하고, exact 기본 메서드, 경로, route-name 집합 및 대표 HTTP
...
Gate는 운영 factory, 개발 factory 및 런타임 초기화의 unique 내부 가져오기도
...
줄어들 수 있지만 증가는 CI를 실패시킵니다. 좁은 package 또는 touched-file 검사에는 하나 이상의
...
않습니다. Report-only 발견 사항은 이행 인벤토리이며 owning package가 정리된 후에만 enforce 대상으로
전환합니다.

`check-boundary-docstrings.py`는 exact 검토된 package 모듈에서 비어 있지 않은 Responsibility,
경계, 권한 and 상태, 의존성, 배포 section을 검사합니다. 범위는 보고
모드로 시작하고 검토 후에만 enforce로 이동합니다. Justified exclusion은 누락된, 범위 밖
또는 불필요한 상태가 되면 실패합니다. 이 structural AST 검사는 semantic truth를 증명하지 않습니다.
...
chat, 작업 흐름 또는 조사보다 behavioral 표면이 작습니다.

구현 소유자는 `fdai.delivery.operator_api.projections.audit`로 이동하며 파일 이름과 공개
symbol은 변경하지 않습니다. App-side 감사 조회 use와 운영 패널 조립은 새 package facade를
가져옵니다. 개발 조립은 shared 운영 패널 빌더를 통해 같은 facade에 도달합니다.
기존의 모든 `routes.audit_*` 모듈은 명시적 per-module compatibility shim으로 유지합니다. 메서드,
경로, 경로 이름, 권한 확인, 응답 페이로드, 출처 이력 및 데이터베이스 소유권은 변경하지 않습니다.

Rollback에서도 두 가져오기 표면을 안정적으로 유지합니다. 구현 파일을 `routes/` 아래에
복원하고 새 `projections.audit` 모듈 각각을 복원된 경로 모듈의 forwarding shim으로 변경하며
조립 가져오기는 package facade에 유지합니다. 이 절차는 API 또는 wire 롤백 없이 physical
소유권을 되돌리고 broad 와일드카드 facade를 만들지 않습니다.

### 대화 턴 애플리케이션 경계

Issue 71은 JSON 및 SSE chat 경로가 공유하는 process-local application-service 경계로
...
Core는 principal-manifest-verified 조회 계획에서 이 기록을 만듭니다. Operator는 영속 semantic
변환 결과를 기존 `done` 프레임으로 변환하고 운영 SSE는 Console 파서를 변경하지 않은 채 그래프,
근거, 검증 counter 및 결정론적 답변을 전달합니다.

독립 Operator/Core 브리지에는 이제 semantic 턴과 최종 변환 결과를 위한 가산 버전 1.2 wire
...
증적 생성을 계속 담당합니다. 이 계약은 서비스 구현의 direct 가져오기 또는 semantic 페이로드의
older peer downgrade를 허용하지 않습니다.

Core semantic 런타임은 이제 계획 수립, dependency-wave 실행 및 이 변환 결과를 하나의 비동기 서버
결과로 구성합니다. 모든 accepted 턴은 답변, 명확화, 보류, 지원하지 않는, 액션 초안 또는
취소로 종료됩니다. Synchronous compatibility 조정기는 exact 정본 명령을 기본값으로
사용하며 명시적 temporary 호출자가 `legacy` 모드를 선택하지 않으면 natural-language 정규식, 키워드
서술 또는 canonical-string 읽기 계획 수립을 적용하지 않습니다.

독립 Operator package는 `adapters/` 아래에 구체적인 Event Hubs Kafka 어댑터를 소유합니다. 운영
조립은 초기화, 요청 토픽 및 변환 결과 토픽이 all-or-none 검증을 통과한 경우에만 두
포트에 하나의 어댑터를 구성합니다. 생산자는 멱등적이고 소비자는 수동 커밋을 사용합니다.
Valid 대응은 semantic 브리지가 처리한 뒤에만 커밋하고 malformed 또는 oversized JSON은 형제
DLQ에 쓴 다음 커밋합니다. 어댑터는 managed-identity 자격 증명을 소유하고 애플리케이션 수명 주기에서
이를 닫습니다. 명시적으로 주입된 발행기/출처 쌍은 테스트 및 downstream 재정의 경계로 유지됩니다.
Semantic Kafka와 dev-only 로컬 서술기는 상호 배타적입니다.

### 대화 단정 애플리케이션 경계

SD-01 단정 slice는 `fdai.delivery.operator_api.application.conversation.claims` 아래에서
결정론적 answer-claim 검증을 소유합니다. 추출, 근거 collection, matching,
...
소유된 최종 검증기는 명시적 단정 package facade를 가져옵니다. 기존
`routes.chat_claim*` 모듈의 repository-wide 소비자는 내부 구현 또는 테스트 가져오기였고
같은 slice에서 이동했으므로 점유 compatibility shim은 남기지 않습니다. Rollback은 구현
모듈과 facade를 `routes/` 아래에 복원한 다음 단정 package facade가 복원된 소유자를 가리키게 합니다.
이 과정에서 JSON 또는 SSE wire 계약은 변경하지 않습니다.

### 대화 검증 애플리케이션 경계

SD-01 검증 slice는 `fdai.delivery.operator_api.application.conversation.verification`
아래에서 최종 답변 검증을 소유합니다. 이 package는 정본 결과, text-integrity
...
내부 경로와 테스트 소비자는 명시적 package facade를 가져옵니다. 기능 카탈로그는 해당
owned package를 직접 사용하므로 `routes/` 아래에 검증 compatibility 모듈이 남지 않습니다.
Rollback은 이동한 모듈을 `routes/` 아래에 복원하고 package facade가 복원된 소유자를 가리키게 합니다.
...
선택과 검증된 근거 산출물 compilation을 소유합니다. 이 package에는 표현 계약,
형태 프로파일, 범위가 제한된 플래너, 결정론적 인벤토리 및 subscription-health 산출물 컴파일러가
포함됩니다. 읽기 전용이며 request-local입니다.

JSON 및 SSE 경로는 명시적 표현 facade를 가져오기하며 authentication, HTTP 상태 대응,
JSON 묶음, SSE 순서와 개정 번호, 취소, 최종 assembly 및 대화 이력을
계속 소유합니다. 기존 `routes.chat_presentation*` 모듈은 내부 가져오기 경로였으므로 compatibility
shim을 남기지 않습니다. 이 이동은 정본 텍스트 대체 경로, 산출물 스키마, localized 라벨,
근거 참조, 바이트 한계 및 플래너 성능 저하를 정확히 보존합니다. Rollback은 경로
구현 모듈을 복원하고 wire 계약을 변경하지 않은 채 표현 facade가 복원된
소유자를 가리키게 합니다.

### 대화 인벤토리 애플리케이션 및 변환 결과 경계

SD-01 인벤토리 slice는 타입이 지정된 조회, 결정론적 compilation, 후속 조치 범위,
catalog-backed 언어/리소스 semantic, 온톨로지 함수, semantic 수집 및 provider-read
...
경로와 최종 검증은 책임에 따라 명시적 애플리케이션 또는 변환 결과 facade를 가져옵니다.
기존 `routes.chat_inventory*` 소비자는 모두 내부 구현 또는 테스트 코드였으므로
compatibility shim을 남기지 않습니다. JSON, SSE 순서/개정 번호, 권한 확인, 프로바이더 범위 및
대화 이력 행동은 변경되지 않습니다.

Rollback은 인벤토리 구현 모듈을 `routes/` 아래에 복원하고 두 인벤토리 package
facade가 복원된 소유자를 가리키게 합니다. Wire 계약과 권위 있는 인벤토리 프로바이더는
변경하지 않습니다.

### 대화 백엔드 애플리케이션 및 어댑터 경계

SD-01 백엔드 slice는 `fdai.delivery.operator_api.application.conversation.backend` 아래에서
프로바이더 중립적인 계약과 request-local 지연 시간 라우팅을 소유합니다. 애플리케이션 package는 injected
...
테스트 가져오기였으므로 compatibility shim을 남기지 않습니다.

Rollback은 다섯 백엔드 모듈을 `routes/` 아래에 복원한 다음 애플리케이션 및 어댑터 facade가 복원된
...
모두 내부 출처 또는 테스트 가져오기였고 이 slice에서 이동했으므로 compatibility shim은 남기지
않습니다. Rollback은 다섯 경로 구현을 복원하고 JSON, SSE, authentication, 근거
권한 또는 이력을 변경하지 않은 채 근거 facade가 복원된 소유자를 가리키게 합니다.

### 대화 진행 상황 메트릭 변환 결과 경계

SD-01 스트리밍 메트릭 slice는
...
기존 `routes.chat_stream_metrics` 모듈에는 외부 compatibility 소비자가 없었으므로 shim을
남기지 않습니다. Rollback은 집약기를 `routes/` 아래에 복원하고 메트릭 이름, SSE 프레임 또는
취소 행동을 변경하지 않은 채 스트림 경로 가져오기를 되돌립니다.

### 대화 최종 변환 결과 경계

SD-01 최종 slice는 `fdai.delivery.operator_api.projections.conversation.terminal` 아래에서 pure
verification-frame assembly, 최종 페이로드 compilation, 측정된 LLM 사용량 렌더링, 영속 인벤토리
결과 맥락 및 source-failure 재생 맥락을 소유합니다. 이 package는 최종 응답에 사용되는
...
소비자는 명시적 최종 facade로 이동했고 package는 `fdai.delivery.operator_api.routes` 모듈을 가져오기하지
않으므로 compatibility shim이 남지 않습니다. Rollback은 경로 구현 4개를 복원하고 최종 facade를
...
완료를 소유합니다. 답변 세대 이후 이 package는 기존 순서대로 범위가 제한된 quality 검토,
...
package는 `fdai.delivery.operator_api.routes` 모듈을 가져오기하지 않습니다. 기존
`routes.chat_stream_post_generation` 경로는 내부이었으므로 compatibility shim을 남기지 않습니다.
...
target-agent 파생을 소유합니다. 이 package는 server-authenticated, byte-bounded JSON 객체 하나를
...
JSON chat은 기존 전송 계층 순서를 유지하면서 같은 preparation 계약과 보조 로직을 가져옵니다.
...
`BusyInputCoordinator`는 active-turn registration과 arbitration을 담당하는 코어 권한으로 유지됩니다.
...
이전 경로 모듈 소비자는 모두 내부 출처 또는 테스트 코드였으므로 compatibility shim은 남기지
...
SD-01 최종 support slice는 범위가 제한된 trajectory-detail 재생, 결정론적 current-screen T0
답변, 명시적 선택 민감정보가 제거된 model-call trace, 검증된 resource-follow-up 응답 맥락을
...
어댑터는 이미 수행된 모델 요청과 응답을 tracing 변환 결과에 기록하고 프로바이더 호출은 계속
어댑터가 소유합니다. JSON 및 SSE 경로는 authentication, 본문 파싱, 상태 대응, 프레임 순서와
개정 번호, 취소, 최종 전달, 대화 이력을 유지합니다. 이전 경로 소비자는 모두
내부이므로 compatibility shim은 남기지 않습니다. Rollback은 네 경로 구현을 복원하고
내부 소비자를 redirect하며 wire 계약은 변경하지 않습니다.

### 대화 영속성 및 문서 근거 경계

SD-01 영속성 slice는 principal 범위 transcript 쓰기, 내용이 없는 정책 증적, 재생
메타데이터 및 conversation-image 수명 주기를
`fdai.delivery.operator_api.persistence.conversation` 아래에서 소유합니다. 명시적 facade는 고정된
...
검증된 이미지는 기존 pending 생성, exact-attempt compensation 및 영속 finalization 순서를
유지합니다. Turn 메타데이터에는 이미지 id, display 이름 및 검증된 media 타입만 포함됩니다. 이미지 바이트는
...
테스트 코드였으므로 compatibility shim은 남기지 않습니다. Rollback은 세 구현을 `routes/` 아래에
복원하고 내부 가져오기를 되돌리며 transcript 신원, 이미지 만료, 문서 참조, JSON 또는 SSE 행동은
변경하지 않습니다.

### 대화 기능 애플리케이션 경계

SD-01 기능 slice는 범위가 제한된 Pantheon 위임, runtime-skill disclosure,
configuration-baseline 읽기, 공개 웹 근거 해석, request-time 기능 가시성 및 strict
topology 의도를 `fdai.delivery.operator_api.application.conversation` 아래에서 소유합니다. 에이전트
...
프로바이더 중립적인 웹 검색 해석기는 결정론적 및 semantic 의도 precedence, sanitization, 범위가 제한된
...
domain, 엔드포인트, 배포 또는 자격 증명을 제공하지 않습니다. 구성 drift는 정확한 server-pinned
문서 경로를 action-context 문구보다 먼저 유지하고, topology 의도는 계속 exact 서버가 소유한
선택자를 요구합니다.

JSON 및 SSE 경로는 authentication, 요청 파싱, HTTP 상태 대응, 프레임 순서와 개정 번호,
취소, 최종 전달 및 대화 이력을 유지합니다. 이전 경로 모듈 6개의 소비자는
모두 내부 출처 또는 테스트 가져오기였으므로 compatibility shim을 남기지 않습니다. Rollback은 해당
...
애플리케이션, 변환 결과 및 영속성 package는 경로 모듈을 가져오기하지 않습니다.
...
계획 수립, 근거, 세대와 스트림 collection, busy 입력, 검증, 응답 완료,
...
`seq`를 추가하며 개정 번호를 변경하지 않고 보존합니다. `chat_registration.py`는 registration,
...
Issue 72는 `OperatorApiConfig(**kwargs)`를 범위가 제한된 compatibility 생성자로 유지하고 경로를 등록하기
...
대화, governed-route 및 fixed-HTTP 기록으로 그룹화합니다. 각 registration 함수는 이전 방식
...
경로 메서드, 경로, 이름, registration 순서, 권한 확인, CORS, 응답 페이로드 및 가용성 기본값은
변경하지 않습니다. Rollback은 변경할 수 없는 기록 정의와 검증을 `app/config.py`로 다시 옮기고
`app/composition.py`를 제거하며 이전 방식 생성자, `split()` 대응, 공개 `main` facade 및 registration
서명을 그대로 유지합니다. 이 절차는 wire 또는 호출자 이행 없이 physical 소유권을 되돌립니다.

| Package | 현재 책임 | 이행 규칙 |
|---------|-----------|----------------|
| 루트 | 공개 facade 및 foundational 계약 | 분류된 replacement가 준비될 때까지 유지합니다. |
| `adapters/` | HTTP 경로 밖의 구체적인 Operator API 프로바이더 구현 | 프로바이더 I/O를 애플리케이션 계약 뒤에 유지합니다. |
| `adapters/conversation/` | Azure 및 OpenAI-compatible 서술기 전송 계층과 웹 검색 시작 construction | 명시적 모듈로 가져오고 자격 증명, 엔드포인트, 배포 선택 및 전송 계층은 애플리케이션과 경로 밖에 유지합니다. |
| `app/` | Shared ASGI assembly, middleware, registration 및 lifespan | HTTP 조립 경계로 유지합니다. |
...
| `application/conversation/turn_execution/` | 타입이 지정된, Starlette-free JSON 및 streamed 턴 요청 preparation, 계획 수립, 근거, 세대, 검증, 영속성, metering 및 완료 coordination | 명시적 facade로 가져오고 authentication, 본문 파싱, 상태 대응, JSON/SSE 인코딩, 하트비트, 순서, 개정 번호 및 응답 전달은 경로에 유지합니다. |
| `application/conversation/capabilities/` | Domain별 타입이 지정된 process-local 에이전트 위임, runtime-skill, configuration-drift, 웹 검색 및 read-model 기능 | Non-authoritative 기능 소유자로 유지하고 injected 읽기 전용 런타임과 프로바이더 계약을 사용합니다. |
| `application/conversation/capabilities/inventory/` | 타입이 지정된 인벤토리 조회, 결정론적 compilation, semantic grounding 및 provider-read coordination | 명시적 package facade로 가져오고 JSON, SSE, authentication 및 이력은 경로에 유지합니다. |
| `application/conversation/backend/` | 프로바이더 중립적인 백엔드 계약 및 request-local 지연 시간 라우팅 | 명시적 facade로 가져오고 프로바이더 구현은 어댑터에 유지합니다. |
| `application/conversation/claims/` | 결정론적 answer-claim 추출 및 범위가 제한된 근거 검증 | 명시적 package facade로 가져오고 JSON, SSE 및 authentication은 경로에 유지합니다. |
| `application/conversation/verification/` | 결정론적 최종 답변 검증 및 범위가 제한된 근거 렌더링 | 명시적 package facade로 가져오고 wire 행동과 authentication은 경로에 유지합니다. |
| `application/conversation/evidence/` | Operational 근거 해석, 출처 이력, 가지 수명 주기 및 authority-preserving 병합 | 명시적 package facade로 가져오고 JSON, SSE, authentication, 취소 및 이력은 경로에 유지합니다. |
| `application/conversation/post_generation/` | Quality 검토, 검증, 이력 영속성 coordination, 최종 페이로드 검증 및 post-turn 검토 | 명시적 package facade로 가져오고 권한 확인, 요청 파싱, 하트비트 framing, 순서, 취소 및 SSE 전달은 경로에 유지합니다. |
| `application/conversation/request_preparation/` | 내용 정책과 재생, 선호 설정, 문서 참조, 이력, 이전 맥락, 리소스와 최신성 맥락, 후속 조치 범위, 답변 계획 및 target-agent 파생 | 명시적 package facade로 가져오고 권한 확인, 범위가 제한된 본문 파싱, HTTP 대응, SSE 순서 및 전송 계층 전달은 경로에 유지합니다. |
...
| `persistence/conversation/` | principal 범위 transcript, 정책 증적, 재생 메타데이터 및 conversation-image 수명 주기 영속성 | 명시적 facade로 가져오고 HTTP, SSE, authentication, 상태 대응 및 전송 계층은 경로에 유지합니다. |
...
| `projections/change_lineage/` | 범위가 제한된 정본 Change-lineage 요약 및 상세 화면 | 명시적 facade로 가져오고 HTTP, 프로바이더 I/O, 영속성, 실행 및 승격은 package 밖에 유지합니다. |
| `projections/conversation/` | 화면 데이터, exact 문서 근거, 모델 trace, trajectory 상세, 리소스 응답 맥락 및 큐에 수락된 진행 상황 메트릭 reduction을 포함하는 request-local 대화 읽기 변환 결과 | Service-graduation 근거가 준비될 때까지 프로세스 안에 유지합니다. |
| `projections/conversation/presentation/` | Value-free 배치 선택 및 검증된 근거 산출물 compilation | 명시적 facade로 가져오고 JSON 및 SSE 행동은 경로에 유지합니다. |
| `projections/conversation/inventory/` | 인벤토리 근거 sanitization, 결과 변환 결과 및 결정론적 렌더링 | 명시적 facade로 가져오고 조회 compilation과 프로바이더 coordination은 애플리케이션 package에 유지합니다. |
| `projections/conversation/terminal/` | 최종 페이로드, LLM 사용량, resource-result 및 source-failure 변환 결과 | 명시적 facade로 가져오고 JSON, SSE, authentication, 취소 및 이력은 경로에 유지합니다. |
| `production/` | 운영 프로바이더 construction 및 연결 | Wire 행동을 변경하지 않고 fanout을 점진적으로 줄입니다. |
| `routes/` | HTTP/SSE 전송 계층, 경로 registration, domain 요청 어댑터 및 분류된 compatibility facade | 전송 계층 및 검토된 facade 경계로 유지하고 대화 수명 주기 orchestration은 타입이 지정된 애플리케이션 facade 뒤에 둡니다. |
| `streaming/` | 읽기 전용 SSE 전송 계층, 민감정보 제거, fanout 및 런타임 변환 결과 | Versioned 중계 및 재생 계약이 준비될 때까지 유지합니다. |

`fdai.delivery.operator_api.main`은 공개 앱 facade입니다. `read_model`은 검토된 replacement가 준비될
때까지 공개 전달 계약으로 유지합니다. `fdai.delivery.auth`는 framework-neutral bearer 및 Entra
검증을 소유하고 `operator_api.auth`와 `operator_api.entra_verifier`는 compatibility facade로만
유지됩니다.
`main` facade의 `busy_input_runtime` re-export는 새 런타임 소유권 점유가 아닌 transitional 공개
경계입니다.
...
분류된 compatibility 필요가 있을 때만 모듈별 forwarding shim을 사용합니다.
...
`streaming.provision_stream` compatibility는 별도로 분류합니다. Issue 71은 기준선에 기록된 chat wire
debt를 해소합니다. 버전 1 semantic 프레임에는 서버가 소유한 요청 id와 integer 순서가 필요하고,
...
두 번째 스키마 소유자를 만들지 않습니다. Service-owned 스키마 및 이행 lane에는 별도 검토된 경계가
필요합니다.
...
- [`services/core-control-plane/src/fdai/core/conversation/`](../../../services/core-control-plane/src/fdai/core/conversation)
 ...
 transcript는 principal 범위로 한정된 `ConversationHistoryStore`가 소유합니다.
- [`cli/`](../../../cli)
...
- [`services/core-control-plane/src/fdai/core/conversation/channel_gateway.py`](../../../services/core-control-plane/src/fdai/core/conversation/channel_gateway.py)는
 sender를 인증하고 메시지 멱등성 키를 점유하며 조정기를 호출합니다. 영속 전달이
...
- [`current_time.py`](../../../services/operator-service/src/fdai_operator_service/)는 injected
 aware 시계와 principal IANA timezone에서 current-time 질문을 해석합니다.
...
- `application/conversation/capabilities/`는 액션, prior-context, current-time, 출처,
 준비 상태, knowledge, metering, 로그, 네트워크, subscription-health, T2 복구, 행동 및
 read-model 보조 로직을 소유합니다. `application/conversation/`은 턴/intent-graph 계획 수립, 프롬프트
 assembly, 공개 웹 의도 및 vision 검증을 소유합니다. 두 package 모두 `routes`를 가져오기하지
 않습니다.
...
- `projections/conversation/`은 incident-dossier와 RCA 렌더링, 범위가 제한된 execution-output
 ...
 민감정보가 제거된 모델 trace, trajectory-detail 재생 및 resource-follow-up 응답 변환 결과를 소유합니다.
 이동된 내부 보조 로직에는 compatibility shim이 없습니다.
...
- 다섯 파일의 `chat*.py` structural 인벤토리에는 `chat.py`, `chat_registration.py`,
 ...
 JSON HTTP 전송 계층과 compatibility 연결만 소유합니다. `chat_stream.py`는 SSE 전송 계층만 소유하고
 애플리케이션 개정 번호를 전송 계층 순서가 있는 프레임으로 매핑합니다. 나머지 세 파일은 각각
 registration, SSE 프로토콜 및 request-transport 소유자로 유지됩니다.
...
- `routes/`는 JSON/SSE 묶음, authentication, HTTP/SSE 상태 대응, 프레임 순서,
 연결 취소, 경로 registration과 그래프, data-source, 준비 상태 변환 결과의 HTTP
 핸들러를 유지합니다.
- `read_investigation_responder.py`는 등록된 Heimdall 읽기 의도를 타입이 지정된 근거에서 렌더링합니다.
 ...
 ID, 소유권 또는 계획 연결 drift 시 시작을 차단합니다.
- `routes/rule_catalog.py`는 읽기 전용 활성/발견 Rule 참조 변환 결과를 제공합니다.
 카탈로그와 일치하는 활성 세대에서만 semantic 순위를 사용하고, 그 외에는 명시적인
...
영어 및 한국어 표현 literal은 NFC UTF-8을 사용합니다. Escaped Hangul은 정확한 근거 설명이 있는
...
제출, 재시도, 취소, 정리, execute 또는 승인 control이 없습니다. 자격 증명 및 Thor 신원을
