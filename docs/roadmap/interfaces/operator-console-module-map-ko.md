...
translation_source_sha: 1e433380a8e995914535717eb5bd03a6b105e924
...
| Workflow Builder 대기 및 승인 저작 | 구현됨 | `console/src/routes/workflow-builder.{model,editor,session,viz}.ts`, `console/tests/e2e/workflow-builder-control-steps.spec.ts`, 집중 Console 검사 | 유형별 저작, 미리 보기 및 탭 복구를 별도 모듈로 유지합니다. 필수 제한 시간, 승인 역할, 정족수 및 자기 승인 방지 필드는 복제, 순서 변경, 검증 및 복원 과정에서 유지됩니다. 검증은 서버가 계속 담당하며 저장된 비공개 초안은 shadow 상태로 실행할 수 없습니다. |
...
Workflow Builder는 편집 가능한 단계 모델, 순수 초안 변환, 방어적인 세션 디코더, 유형별 시각화
및 Preact 편집기를 인접한 `workflow-builder.*` 모듈에 유지합니다. 작업 선택지는 검토된
ActionType 팔레트에서 가져옵니다. 대기 및 승인 필드는 서버가 소유한 Workflow 계약을 따르며,
기존 검증 경로가 확인한 뒤에만 수락됩니다. 브라우저는 저장된 비공개 초안을 게시하거나
실행하지 않습니다.
