...
translation_source_sha: 34807036b321707abbba5ea5a0c91911222a284b
...
준비된 행은 권한 없는 사용자에게 콘텐츠를 노출하지 않으면서 원본, 형식, 크기, 컬렉션,
용도, 분류, 보호, 버전, 경고, 보존, 가능한 작업을 함께 표시합니다. 교체하면 변경할 수
없는 버전을 만들고 `ready` 이후에만 활성 포인터를 이동합니다.
...
**수정된 설계.** 서버 소유 컬렉션을 이동하거나 이름을 바꿀 수 없는 최상위 폴더로 표시합니다. 검색 및 인덱스 필터를 적용한 뒤 같은 이름의 업로드를 펼칠 수 있는 그룹으로 묶습니다. 행은 수명 주기와 검색 인덱스 상태를 구분합니다. `ready`와 `ready_with_warnings`는 **인덱싱됨**, 이전 상태는 **대기 중** 또는 **인덱싱 중**, 최종 보류와 실패는 **인덱싱되지 않음**으로 표시합니다.

각 콘텐츠 작업은 현재 메타데이터와 권한을 다시 확인합니다.

- **미리 보기:** 컬렉션 및 위임된 보호 권한 확인 후 범위가 제한된 추출 텍스트를 반환합니다.
- **다운로드:** 콘텐츠 없는 감사 요청 후 보호되지 않은 인덱싱 완료 원본을 반환하며 권한 관리 원본은 아직 지원하지 않습니다.
- **삭제:** 두 단계 확인, 서버 권한 재검사, 보존 및 법적 보존 확인 후 워커 소유 삭제 Saga를 시작합니다.
