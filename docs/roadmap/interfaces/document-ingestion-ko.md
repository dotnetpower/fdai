...
translation_source_sha: 5033e01818afb2a298a68cdf52d8cf6ed36e92a1
...
대상 컬렉션은 브라우저에서 변경하는 폴더가 아니라 거버넌스가 적용된 논리적 이름
공간입니다. 경로를 새로 고치면 선택한 컬렉션의 최신 문서 버전 목록을 영속
메타데이터에서 범위가 제한된 개수만큼 다시 불러옵니다. 컬렉션에 업로드해도 물리적
...
형식 허용 여부, 파서 선택, OCR 요구 사항 및 타입이 지정된 실패는
[통제된 문서 형식 정책](document-ingestion-format-policy-ko.md)이 소유합니다. 클라이언트 힌트는
사용 가능한 파서만 선택하며 서버의 시그니처 및 콘텐츠 검증이 계속 최종 기준입니다.
