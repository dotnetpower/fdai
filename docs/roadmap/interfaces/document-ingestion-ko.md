...
translation_source_sha: 723532a55b48d4ca93ed22389dedf1963541ba91
...
브라우저 미디어 형식은 사용할 수 있는 파서를 선택할 뿐이며, 워커는 추출 전에 실제 바이트
시그니처를 다시 검증합니다.
...
| DOC, PPT, XLS 및 기타 OLE 바이너리 | 허용하지 않음 | 원본 애플리케이션에서 최신 OOXML 또는 PDF 버전으로 먼저 저장합니다. |
| ZIP 및 기타 일반 압축 파일 | 허용하지 않음 | 압축 파일 확장은 업로드 형식으로 사용하지 않습니다. |

포함된 이미지는 범위가 제한된 패키지 구성원 추출과 구성된 OCR 공급자를 사용합니다. 최신
Office 문서에 사용할 수 있는 기본 텍스트가 있지만 포함된 이미지를 OCR로 처리할 수 없으면
문서는 `ready_with_warnings`로 완료됩니다. 이미지만 있는 문서, 스캔 PDF 또는 이미지만 있는
Office 패키지는 위치가 확인된 OCR 텍스트 없이는 준비 상태가 될 수 없습니다. 추출 결과가
비어 있으면 검색 가능한 빈 문서로 만들지 않고 타입이 지정된 실패로 처리합니다.

잘못된 패키지, 확장자와 시그니처 불일치, 암호화된 파일, 지원하지 않는 이미지 인코딩 및
파서 예산 위반은 서로 다른 정제된 실패 코드를 생성합니다. FDAI는 구형 형식을 최선 노력으로
