...
translation_source_sha: b1572955da779c594ba6e6268c0975cd75942f90
...
Console은 지식 > 문서의 `/documents`에서 거버넌스가 적용된 인제스트를 제공합니다.
컬렉션은 변경 가능한 브라우저 폴더가 아니라 서버 소유 논리적 이름 공간입니다. 새로
고치면 영속 목록을 복원하고 컬렉션을 바꾸면 다음 권한 적용 결과 전에 기존 행을 지웁니다.

업로드 전에 다음 정보를 표시합니다.

- **대상 컬렉션:** 문서를 소유할 작업 영역입니다.
- **볼 수 있는 사람:** 메타데이터와 콘텐츠를 읽을 역할 또는 그룹입니다. Console 접근만으로 문서 권한을 부여하지 않습니다.
- **사용 목적:** Knowledge Base 근거, 수동 정제 또는 둘 다입니다.
- **보존:** 승인된 원본 및 파생 산출물 보존 정책입니다.
- **지원 형식과 제한:** 서버에서 확인한 형식, 파일 크기, 배치, 보관 정책입니다.
- **보호 콘텐츠 처리:** 승인된 암호 해독 권한이 없을 때 보류하거나 수락하지 않습니다.

확인 문구는 명확하고 수집별로 구체적이어야 합니다.

> 이 업로드는 업로더 개인에게만 공개되는 파일이 아닙니다. FDAI에서 `<collection>`에
> 접근할 수 있는 사용자는 메타데이터와 콘텐츠를 볼 수 있습니다. 이 대상이 접근하면 안 되는 콘텐츠는 업로드하지 마세요.

컬렉션, 대상, 용도 또는 보존이 바뀌면 다시 확인합니다. 동의 기록에는 문서 텍스트 없이
정책 버전, 컬렉션, 행위자, 시각을 저장합니다.

### 업로드와 처리 중

업로드 진행률과 처리 진행률을 구분합니다.

1. **업로드 중:** 해시 계산과 범위가 제한된 PUT을 수행하며 바이트 진행 및 재개는 공급자 지원이 필요합니다.
2. **수신됨:** 원본 해시와 바이트 수가 수락되었습니다.
3. **안전성 검사:** 맬웨어, 보관, 시크릿, 개인 데이터, 보호 상태를 검사합니다.
4. **추출 중:** 추출기가 제공하는 페이지, 슬라이드, 시트, 이미지, 첨부 진행률을 표시합니다.
5. **인덱싱:** 조각과 임베딩을 커밋하고 있습니다.
6. **준비됨, 보류, 실패:** 민감한 미리 보기 없이 실행 가능한 최종 결과를 표시합니다.

`complete` 이후 브라우저를 닫아도 서버 처리는 취소되지 않습니다. 취소하면 새 작업을
중단하고 부분 산출물 삭제를 예약합니다. [형식 정책](document-ingestion-format-policy-ko.md)이
허용 형식, 파서, OCR, 타입이 지정된 실패를 소유하며 서버 검증이 최종 기준입니다.

### 처리 후

준비된 행은 권한 없는 사용자에게 콘텐츠를 노출하지 않으면서 원본, 형식, 크기, 컬렉션, 용도, 분류, 보호, 버전, 경고, 보존, 가능한 작업을 함께 표시합니다.
일반 거버넌스 적용 Console 업로드에서는 한 컬렉션 안의 정확한 원본 이름으로 교체 후보를 식별합니다. 서버는 해시 계산 후 생성 및 삭제 권한을 다시 확인합니다. 바이트와 거버넌스 입력이 모두 같으면 원본을 다시 쓰거나 수명 주기를 다시 실행하지 않고 기존 준비 완료 버전을 재사용합니다. 바이트 또는 거버넌스 입력이 달라지면 같은 `document_id`에 변경할 수 없는 새 버전을 만들고, `ready` 이후에만 활성 포인터를 이동합니다.

### 문서 보관함과 컬렉션 탐색

**초기 설계.** 변경 가능한 브라우저 폴더 트리와 직접 콘텐츠 작업을 사용합니다.

**비판.** 이 방식은 컬렉션 정책, 보존, 버전 계보, 원본 보호 밖에 별도 계층을 만듭니다.
하나의 배지로는 업로드, 안전성 검사, 추출, 인덱스 준비 상태도 구분하기 어렵습니다.

**수정된 설계.** 서버 소유 컬렉션을 이동하거나 이름을 바꿀 수 없는 최상위 폴더로 표시합니다. 검색 및 인덱스 필터를 적용한 뒤 정확히 같은 이름의 기록을 파일 행 하나로 묶습니다. 버전 이력을 열면 권한이 있고 삭제되지 않은 모든 불변 버전을 불러오며, 서버가 교체를 관리하기 전에 생성된 같은 이름의 기록도 포함하되 업로더와 액세스 구성원 세부 정보는 제외합니다. 행은 수명 주기와 검색 인덱스 상태를 구분합니다. `ready`와 `ready_with_warnings`는 **인덱싱됨**, 이전 상태는 **대기 중** 또는 **인덱싱 중**, 최종 보류와 실패는 **인덱싱되지 않음**으로 표시합니다.

각 콘텐츠 작업은 현재 메타데이터와 권한을 다시 확인합니다.

- **미리 보기:** 컬렉션 및 위임된 보호 권한 확인 후 선택한 사용 가능 버전의 범위가 제한된 추출 텍스트를 반환합니다.
- **다운로드:** 콘텐츠 없는 감사 요청 후 선택한 변경 불가, 사용 가능, 보호되지 않은 원본을 반환하며 권한 관리 원본은 아직 지원하지 않습니다.
- **삭제:** 선택한 버전에 대한 두 단계 확인, 서버 권한 재검사, 보존 및 법적 보존 확인 후 워커 소유 삭제 Saga를 시작합니다. 활성 버전을 삭제해도 이전 버전을 자동으로 다시 활성화하지 않습니다.
...
공개 콘솔은 인증된 게이트웨이를 통해 비공개 ADLS로 바이트를 스트림하고 SHA-256과 크기
메타데이터를 봉인한 뒤 `fdai.pantheon.objects`에 논리 `object.event`를 게시합니다. 영속 워커는
커밋되지 않은 실패를 재시도하고 같은 경계에 검사 수명 주기 이벤트를 게시하며,
`fdai.pipeline.stages`는 운영 활동에만 사용합니다. ClamAV를 통과한 문서만 추출, pgvector
인덱싱 및 격리 구역에서 통제된 영역으로의 원자적 이름 변경에 도달합니다.
...
| 대체된 버전 정리 실패 | 원본 리비전마다 멱등적인 취소 행 하나를 유지합니다. 리비전 키에는 512자 원본 제한을 그대로 적용합니다. 각 커넥터 요청은 범위가 제한된 페이지를 처리하고, 새 원본 이벤트가 없어도 API 백그라운드 조정기가 범위가 제한된 원본 항목 페이지를 재시도합니다. |
