...
translation_source_sha: 34d0f090077ce4948bb5ee288daebaa2c7fe446e
...
> append-only audit, read-only console projection을 포함합니다. Runtime, deployment, recovery,
> verification 상세 내용은 [에이전트 운영 책임 수명 주기](agent-stewardship-operations-ko.md)를
> 참조하세요.
...
Console은 "Propose a change" 안내와 파일 경로를 보여주지만 mutation button이나 GitHub
credential을 제공하지 않습니다. Owner는 file을 직접 편집하거나 `handover_bootstrap` document를
제출할 수 있습니다. Grounded result는 ingestion service가 draft PR로 전달합니다. Loader는
maintainer 1명 미만을 거부하고 console은 2명 미만에 권장 banner를 표시합니다.
...
webhook이 merge audit를 작성하고 affected owner에게 알립니다. Console은 read-only를 유지하며
어떤 map도 autonomous apply하지 않습니다. 전체 state, failure, deployment contract는
