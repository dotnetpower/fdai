...
> Terraform 신원 injection, scheduled stale-OID transition 감사, 근거에 기반한 인계 인제스트,
> 멱등적 GitHub draft-PR 전달, signed 병합 관측, stakeholder notification,
> 추가 전용 감사, 읽기 전용 콘솔 변환 결과, 인제스트를 통해 draft-PR proposal을 제출하는
> guided registration form을 포함합니다. 런타임, 배포, 복구, 검증 상세 내용은
> [에이전트 운영 책임 수명 주기](agent-stewardship-operations-ko.md)를 참조하세요.
> 스키마 v2 임무 parsing, v1 compatibility 발견 사항, 비인플레이스 이행 렌더러도 구현되었습니다.
> Tracked upstream 지도는 실제 배포가 백업을 지정할 때까지 v1을 유지합니다.
...
5. **콘솔은 지도를 직접 변경하지 않는다.** 스튜어드십 변환 결과는 읽기 전용을 유지합니다.
 Guided registration form은 구조화된 `handover_bootstrap` 문서를 인제스트 경계에
...
6. **모든 변경은 통보되고 감사되어야 한다.** Core는 recipient와 감사 페이로드를 결정론적으로
 계산합니다. 실제 운영 PR/병합 통합은 이 기본 요소를 notification/감사 어댑터에 배선해야 합니다.
...
`config/agent-stewardship.yaml`에서 새 배포 지도는 버전 2를 사용합니다. Tracked upstream
compatibility 지도는 배포가 실제로 서로 다른 백업을 공급할 때까지 버전 1을 유지합니다.
...
Loader는 버전 1과 2를 허용합니다. v1에서는 첫 accountable 대상을 기본, 이후 대상을
...
drift할 수 없다. `core/`는 `agents/`를 가져오기하지 않으며(module-boundary 규칙), 동등성 테스트가
테스트 시점에 둘을 연결한다.
...
3. 그다음 domain 에스컬레이션 담당자,
4. 그다음 informed 담당자,
5. 그다음 관리자 집합.

계획은 각 person tier의 `hop_timeout_seconds` 힌트를 포함합니다. 담당 체계는 recipient 정렬을
...
notifications matrix는 **channel-id**로 라우팅하지만 담당자는 **사람**이다. 브릿지는 순서대로
해석한다:

1. `agent-stewardship.yaml`의 명시적 `channels[<oid>]` 바인딩,
2. 없으면 `notifications-matrix.yaml`의 에이전트 카테고리 경로(그 사람은 도메인 채널로 도달됨).

`kind: group` 담당자는 항상 matrix 카테고리 경로로 해석된다(그룹은 단일 개인 채널이 없다).
...
만들고 그 사람은 실제 운영 에스컬레이션에서 제거된다(다음 tier / 관리자로 폴백). 이는 hot 경로
...
- 자리 표시자 정책: tracked upstream 구성은 all-zero 값을 사용하고 deployed 환경은
...
거버넌스 PR로 변환합니다. Signed GitHub webhook은 changed 파일과 merged YAML을 다시 읽고 코어
...
3. **감사.** Saga 추가 전용 `AuditEntry`가 actor 신원, 아티팩트, before -> after 요약, 상관관계
  id, 타임스탬프, 승인 결정을 기록한다. 감사 항목은 L0 영어이며 절대 억제되지 않는다.
...
거버넌스 > 에이전트 oversight 경로는 읽기 전용 변환 결과와 통제된 proposal form을 함께
제공합니다.

- **현재 담당 체계** - `GET /stewardship`에서 15개 에이전트, accountable 소유자, notification contact,
...
- **검토 결과** - 처리된 초안은 대응, 해결되지 않은 신원, unmapped 에이전트 개수, 생성된 YAML,
 거버넌스 전달이 활성화된일 때 멱등적 초안 PR link를 표시합니다.

SPA는 Git 자격 증명을 보유하지 않고 `config/agent-stewardship.yaml`을 직접 쓰지 않습니다.
인제스트 서비스가 신원을 해석하고 코어 해석기로 가산 후보를 validate한 다음 Owner가
검토할 초안 PR을 생성합니다. Loader는 관리자 1명 미만을 거부하고 콘솔은 2명 미만에 권장
배너를 표시합니다.

## 10. 보안과 안전

- 스튜어드십은 실행기 Managed 신원을 보유하거나 부여하지 않는다.
- Console 버튼은 proposal을 제출할 수 있지만 지도 변경에는 감사되는 거버넌스 PR
...
1. **결정론적 추출** (`extractor.py` + `agent_domains.py`). 각 문서 라인을 에이전트별
 ...
 form을 사용합니다. 이 필드들은 권위 있는하므로 신원 텍스트가 다른 에이전트를 추가하거나
 responsibility를 바꿀 수 없고 malformed 또는 unknown 구조화된 배정은 무시됩니다. 이것이
 결정론 우선 단계입니다.
...
`DocumentEnvelope`를 해당 용도에 주입된 `DocumentReadyConsumer`로 전달합니다. Upstream
로컬과 운영 조립은 `HandoverBootstrapConsumer`를 연결하고 인증된
`GET /ingestion/uploads/{upload_id}/handover-draft`로 제공합니다. Console은 processing 상태를
...
Graph application 권한인 `User.Read.All`과 `Group.Read.All`만 할당하고 정기적으로
...
webhook이 병합 감사를 작성하고 affected 소유자에게 알립니다. Console은 출처 proposal을
제출할 수 있지만 resulting 지도를 적용할 수 없으며 어떤 지도도 자율 apply하지 않습니다. 전체
상태, 실패, 배포 계약은
...
`HandoverInterpreter`입니다. Upstream 운영은 mixed-model 연결을 명시적으로 공급하지
