...
The Sample autonomy fixture supplies an eight-point series for each of the five Operating Outcomes metrics. Cost Governance selects separate cost, `optimization_case`, and `outcome` records for its routes. Every record retains the `synthetic-preview` source, while unsupported settlement and unit-economics fields remain unavailable.
