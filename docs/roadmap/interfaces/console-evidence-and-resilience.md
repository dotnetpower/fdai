...
Optional Overview branches suppress only typed absence states. A decoder-origin `502` remains a visible route error instead of silently dropping one dataset from an otherwise healthy-looking page.
The initial Operator API facade keeps route-owned Audit, agent-activity, Insights, and Reporting decoders out of startup. Each decoder loads only after its owning read begins, and the production entry-bundle gate retains raw and gzip limits.
