...
This document owns the operator console contracts for evidence provenance, localization, stream recovery, durable replay, and Architecture-map resilience. The conversational tool and RBAC contract remains in [operator-console.md](operator-console.md). The Knowledge overview exposes [cloud-reference lifecycle](cloud-resource-knowledge-lifecycle.md) dates, weakest source freshness, exact-byte package review, and confirmed rollback requests. It separates source status from offline package intake, explains deployment-owned setup without accepting trust material, and keeps rejected read-only inspection retryable while uncertain writes require authoritative reload. Import and pending index verification never appear as searchable activation; source URLs remain inert and expired policy disables writes without hiding history. Its case-insensitive JSON filename check validates file format only, never conversational intent.

On the narrow Console route, Assurance Twin keeps withheld posture and change-review gaps in separate status regions with visible identities and reasons. Focused responsive and accessibility checks verify that these read-only gaps are neither action controls nor successful reports.
## Navigation context
Selecting an Activity Bar domain opens its Explorer and navigates to the first visible panel under the operator's local order and visibility preferences. Before an unpinned Explorer becomes hidden or inert, it restores focus to its owning Activity Bar domain button. This navigation remains active when the Command Deck is closed or floating; a full-workspace Deck closes before the route changes. Browser evidence that opens the current-screen Deck begins from an ordinary panel such as Overview. Settings owns a modal overlay and is not a valid hidden launcher context.
The Activity Bar conversation control opens or resumes general chat without changing the route or Explorer. Its pressed state tracks general chat only. The bottom launcher and `Ctrl+K` or `/` select a separate current-screen conversation. General chat defaults to the workspace; screen chat defaults to the right dock. The rail tooltip opens to the right and the bottom tooltip above, on hover or keyboard focus, and each names its context and actual open/close action. The [conversation lifecycle](operator-console-progressive-conversations.md#command-deck-workspace-lifecycle) owns draft preservation and explicit screen attachments. Every Command Deck date and time label explicitly uses the active Console language. The browser or operating-system locale cannot introduce Korean day-period text into an English conversation, or English day-period text into a Korean conversation.
...
Direct Settings URLs keep a static background shell until the overlay closes, never loading a hidden Dashboard projection.
The Settings shell and other top-level lazy surfaces show a loading state and a reload action if their module fails.
Models, Runtime policies, Identity and access, and document capability reads also let the operator retry a failed initial request without reloading the
whole console. Before the initial `/iam/self` check, Console binds the signed-in authentication context to every dependent client. A failed check keeps data closed, while explicit retry reuses those bindings before reopening the requested route. This contract is identical for AKS gateway and direct Container Apps API bindings; missing or invalid identity still triggers the global 401 boundary. An external Browser Entra route sweep restores the approved session-storage bootstrap before navigation and treats the access-token error surface as a failed route; a rendered shell, heading, or loading completion cannot count as authenticated route evidence when that boundary is visible. Isolated recovery tests prove client behavior, not live deployment health. Baselines, Skills, Conversation Delivery, and Forecast Learning also expose a shared bilingual Refresh evidence button. It re-reads only the existing projection, renders a skeleton and suppresses duplicate activation while loading, and preserves the active view without installing skills, invoking providers, or raising authority.
Dashboard uses [bounded automatic recovery](recorded-resource-state.md#bounded-automatic-recovery) to show initial synchronization and preserve same-client verified evidence; release mismatch and budget exhaustion stop retries without weakening access checks. It checks Cost Governance availability before requesting the optional overview projection, so an unavailable or disabled package remains an explicit unavailable metric without a failing projection request. The lightweight current-screen launcher remains available without loading the full Command Deck.
The console loads the Deck on the first pointer, `Ctrl+K`, `/`, general-conversation, or incident-conversation request and replays that bounded request when the listener is ready.
...
Command Deck renders transport tokens only in the explicit draft state and renders each receipt-bound revision or confirmed segment as soon as it arrives. A later malformed, interrupted, sequence-gap, or missing-terminal stream retracts the draft and renders an unavailable outcome instead of a success-shaped partial answer. The final server-owned conversation binding updates both the durable index and the active in-memory metadata before the next turn. A schema-valid held terminal receipt remains attached to the reply when its request id matches, so the Console preserves typed unavailability without turning it into success or discarding its provenance. Verified answers also preserve the semantic receipt's evidence posture: stale, incomplete, conflicting, and unavailable evidence render as attention states even when claim checks passed. Empty citation sets use the neutral grounded-source label, and malformed conversation preflight output uses the planner-recovery guidance.
The shared page title renders the domain and panel labels when they differ, including `Overview / Dashboard`. A domain root whose panel title repeats the domain label and a standalone utility keep a single title. Optional Overview branches suppress only typed absence states. A decoder-origin `502` remains a visible route error instead of silently dropping one dataset from an otherwise healthy-looking page. The initial Operator API facade keeps route-owned Audit, agent-activity, Insights, and Reporting decoders out of startup. Each decoder loads only after its owning read begins, and the production entry-bundle gate retains raw and gzip limits.
...
A GET-only Sample client serves ordinary projections without authentication or network access. Sample Live uses twelve representative cards, a 180-loop metric warmup, and replayable tier-specific timing defined by [Live and Audit presentation](live-audit-presentation.md). Live, Provision, and Scheduled Continuations use bounded local fixtures; Sample removes Incident intervention, rejects every POST, never activates from API failure, and never mixes with live values. Unsupported routes hide the control and remain Live. Runtime-notification audit destinations pin `data=live` so a tab's Sample preference cannot redirect authoritative audit evidence. Contract tests retain required inventory-progress completeness fields, omit production-only panels from static-mock inventories, and tolerate only the exact lazy embedded-ontology `net::ERR_ABORTED` after the embedded body loads; every other page or request failure remains fatal.
The Sample autonomy fixture supplies an eight-point series for each of the five Operating Outcomes metrics. Cost Governance selects separate cost, `optimization_case`, and `outcome` records for its routes. Every record retains the `synthetic-preview` source. Only for that source, the Console sums `effect_verified` outcome amounts and compares them with candidate recommendation savings for presentation. Intermediate settlement stages, SLO regressions, and unit-economics fields remain unavailable. When Live has retained cost observations but no case, recommendation, or outcome records, Optimization Cases and Outcomes show the disclosed cost basis and observation count above the fold without reclassifying them as later-stage evidence.
...
Activity presents the newest evidence first while preserving source labels and independent bounds: 700 runtime rows, 200 durable audit parents, and 200 recorded conversation rows. These browser limits do not delete durable evidence. A structured handler start and terminal transition share one identity and render as one resource-first row. The server projects a display name from the exact logical Resource reference while retaining that reference for inspection. The row also shows resource and event types, terminal phase, measured duration, topic, and correlation when recorded; it never derives identity from topic text or provider payload. Legacy state frames remain separate.
Recorded and live agent-to-agent turns remain individual `from -> to` rows with their full bounded text. There is no persistent live-tail control. The top of the list receives new rows automatically; scrolling into history or focusing a row holds one bounded reading snapshot. A new-events action counts matching unseen rows in the current retained buffer, not all historical arrivals, and returns to the latest evidence without losing keyboard focus. Changing a filter also returns to the latest matching rows. Time, route, type, detail, and correlation columns are configurable and visible by default. Fullscreen changes presentation only and preserves room for its controls.
Time shows `HH:mm:ss.SSS` in the browser's IANA timezone (`KST` for `Asia/Seoul`), with the complete recorded timestamp available through the shared tooltip and machine row. Display precision does not increase source-clock accuracy or establish causal order; equal-time rows retain deterministic source-sequence ordering. Waterfall remains the durable audit master-detail surface for lifecycle, inputs, outputs, recorded conversations, and hashes, with its existing time order.
...
rendered deterministically from typed evidence. Skills diagnostics expose the stored last successful source refresh, reject malformed timestamps, and distinguish enabled-but-unmeasured sources from ready sources without clearing existing errors or granting load authority. Ontology browse requires a target and browse verb,
...
The Agent Activity route schedules accepted stream frames in 250 ms batches, flushing early at 1024 pending messages. It drops exact replay duplicates without hiding later observations and retains at most 100 conversation turns per Incident. The log virtualizes measured variable-height rows with six rows of overscan on each side, retains the keyboard-focused row and its neighbors, and exposes full retained row counts and absolute row indices through table semantics. Evicted rows also leave the measurement cache. Cached locale/timezone formatters avoid constructing one formatter per rendered timestamp. Source bounds, full-row text, filters, and durable history remain unchanged; a held reading snapshot cannot grow with new events.
...
A manual refresh keeps the last complete view, reports progress, and blocks duplicate requests; a generation transition adds a delay notice. The bulk Dashboard and Ontology Instances directory use committed invalidations plus a five-minute fallback. The directory keeps its SSE connection before selection and preserves the current server search during revalidation. Unrelated failures remain visible errors. Recorded-state summaries and filters count Not provided and Not applicable separately
from genuine Unknown values, while tooltips expose each fact's source authority and observation time. Stale Serving evidence keeps its value with a warning tone and is excluded from the current Serving count. Ontology Instances labels the additive Azure model-serving metric source separately and preserves its partial or unavailable reason without hiding baseline sources. If a refreshed projection no longer contains the optional Serving axis, the selected lens returns to Operational instead of leaving an unlabeled, inapplicable view.
