...
Dashboard v2 restarts its complete bounded page traversal only for typed inventory or ontology
generation transitions. It discards partial pages, shares the original deadline across two delayed
retries, and keeps persistent mismatch visible instead of mixing generations or returning an empty
inventory.
