...
Current Ontology Instances responses must include explicit `runtime_call_graph` and
`postgres_role_evidence` source states. The browser rejects either omission instead of converting
it to availability, an empty source, or a measured zero.
