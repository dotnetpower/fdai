...
| Activity Bar conversation entry | implemented | `console/src/components/navigation-shell.tsx`; `console/src/deck/open-deck.ts`; focused Console tests (`23 passed`); Console typecheck; authenticated three-viewport Browser inspection | The bottom utility rail toggles the current-screen Command Deck without navigation, mirrors open state through an accessible pressed control, and preserves the existing launcher and keyboard shortcuts. The presentation inspection found no horizontal overflow; no governed runtime receipt is claimed. |
...
The icon-only conversation control in the Activity Bar's bottom utility area toggles the current-screen Command Deck without changing the route or Explorer state. Its pressed state mirrors whether the Deck is open, while the existing bottom launcher and `Ctrl+K` or `/` shortcuts remain available.
