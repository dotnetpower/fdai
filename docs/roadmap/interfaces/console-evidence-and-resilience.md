...
The `llm-model-deployment` child uses the runtime layer, the existing Azure application-service
color family, and the stable `model` abbreviation, while its parent remains `llm-endpoint`.
