...
| Web presentation module registry | implemented | `console/src/deck/presentation-modules/`; `presentation-artifact.ts`; `structured-reply.tsx`; focused Console checks (`115 passed`), typecheck, and production build | The thin shell delegates every accepted block to one closed registry entry. V2 charts require descriptions and exact tables; keyboard focus and reduced-motion rules are pinned. Three-viewport Browser evidence remains open. |
