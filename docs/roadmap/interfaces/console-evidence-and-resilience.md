...
| Operational ontology instance explorer | implemented | `instance_explorer.py`; `postgres_family_store.py`; `ontology-instances*.{tsx,ts,css}`; focused Operator and Console checks; strict mypy, Ruff, task-scoped editor diagnostics, production Vite build, and authenticated three-viewport inspection | The `Instances` view binds an active-generation Resource directory and one exact selected Resource to a bounded bidirectional graph, allowlisted properties, newest-first exact-resource FDAI audit facts, independent source availability, release identity, generation, cutoff, and truncation. A Resource Group or Subscription root shows at most 36 type-diverse direct relationships and leaves child-to-child relationships in the full count and Inspector. Other roots retain their bounded branch context. An exact AKS node-Resource-Group mapping orders the cluster's managed Resource Group, contained VMSS and Load Balancer, and their recorded identity and outbound-IP dependencies without inventing an AKS-to-VMSS edge. The graph uses wheel zoom and native full screen, and its Inspector collapses without losing the selected detail view. Reviewed Azure icons cover 55 of 58 active Resource types; three types remain explicit generic fallbacks. Azure Resource Health and Activity Log stay unavailable until their Operator projections are bound. |
