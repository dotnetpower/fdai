...
An exact configuration-baseline filename or S13 baseline request selects the read-only baseline tool before action-context classification. A negative instruction such as "do not call mitigation tools" cannot turn the document read into an action-lifecycle question. The deterministic answer cites the pinned DOCX in every section and reports unavailable relationships as unknown instead of inferring them from prose.
