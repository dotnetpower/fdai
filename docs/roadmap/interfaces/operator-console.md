...
Push-direction notifications (system → human) live in [channels-and-notifications.md](channels-and-notifications.md); operational views and requests are defined in [console-operations.md](console-operations.md), and the SPA lives under
...
Inventory scan, ontology projection, and current-state read correlations remain visible identifiers
without an audit-trace link. A manual lookup with no matching audit steps renders a neutral
unavailable state instead of an operational failure.
...
| Deterministic subscription identity and semantic isolation | validated | `adapters/subscription_scope.py`; `families/conversation/subscription_scope.py`; `semantic_turn_runtime.py`; `semantic_turn_consumer.py`; focused Operator, Core, contract, and Console checks; authenticated standard-port Browser evidence | Catalog-matched subscription identity reads use the server-owned Azure Reader scope and persist one verified deterministic receipt without invoking semantic model transport. Conversation T2 is disabled in production composition. Semantic requests use `session_id` as the Kafka key and two same-group Core consumers use broker partitions so one slow session does not block another. Local startup reconciles the physical topic to at least two partitions, and deployed infrastructure rejects a lower partition count. English and Korean local turns rendered the ARM name, state, masked id, observation time, model trace off, and evidence and verification `1/1`. |
...
| 2026-08-17 | validated | Restored the deleted current-subscription deterministic read in the independent Operator Service, disabled automatic conversation T2 binding, and replaced global semantic-turn serialization with session-keyed partition workers. | `current change`; focused provider, bridge, contract, Core worker, local partition, Terraform, and Console checks; authenticated English and Korean Browser turns showed model trace off and evidence and verification `1/1`. While another session's semantic response remained pending, the deterministic turn completed in 879 ms. Kafka high watermarks did not advance for deterministic turns. No Azure authentication or subscription-RBAC defect caused the reported failure. | Retain deployed-revision evidence separately under the Azure read-investigation owner. |
