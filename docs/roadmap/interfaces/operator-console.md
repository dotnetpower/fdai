...
The Overview treats an unavailable Cost Governance projection as optional, omits unsupported autonomy summaries, renders evidence timestamps in the operator locale, and reflows through container queries when the Explorer changes the available width. Command Deck planning feedback remains decorative, respects reduced-motion preferences, and does not imply that an action has been approved or executed.
...
| Overview and Command Deck presentation resilience | in-progress | `console/src/routes/dashboard.executive.tsx`; `console/src/routes/dashboard.loading.ts`; `console/src/deck/structured-reply.css`; `console/src/styles.css`; current change | Optional Cost Governance authorization gaps render as unavailable, absent autonomy metrics no longer produce a synthetic summary, timestamps use locale-aware formatting, narrow Explorer layouts reflow through container queries, and planning motion respects reduced-motion settings. Focused Console checks remain to be recorded before this row can claim implemented status. |
