...
The production layout uses visible headings for source notices, follows the provisioning mock's run summary, and separates Stages, Readiness, and Resources into keyboard-accessible views without copying mock-only lifecycle data. Console catalog changes refresh the source-bound question bank before semantic-intent coverage and CQAS provenance so all generated artifacts retain the same authoritative source digest. A digest-only refresh changes no runtime authority.
