...
The managed full-stack task defers authoritative inventory refresh to its continuous reconciliation process so the Console, Core, and Operator processes can start while collection runs. Complete readiness still requires active-scope inventory coverage and a clean analyzer tick. Standalone preparation retains synchronous inventory refresh.
