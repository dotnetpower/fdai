...
Command Deck busy-turn follow-ups, mode inspection, and conversational cancellation follow [Busy Conversation Input Modes](busy-input-modes.md). Controls require a strictly decoded, size-bounded, same-session Operator projection; without one they remain unavailable, and the local Stop command remains separate. These controls never cancel actions or approvals.
The Deck displays source-bound test-context candidates without treating them as authorized scope or policy choices. A separate, authenticated command-receipt lookup exposes only the requesting principal's delivery and historical audited application; current authorization remains unevaluated. Until reviewed principal-to-case mappings and independent proofs exist, the browser offers no proposal, independent review, or revocation controls. See [the case-history owner](../rules-and-detection/prediction-learning-and-case-history.md) and [progressive conversation owner](operator-console-progressive-conversations.md).
...
The dashboard preserves bounded service-health answers across loading, partial, and unavailable states. It displays exact resource identity, evidence timing, and provider limitations from the semantic projection rather than substituting generic inventory data. Service Health event category, active status, severity, and source completeness use locale-specific display labels, while the provider advisory title remains unchanged evidence text. Inventory invalidation epoch changes require a successful authenticated snapshot reread before acknowledging a new stream cursor; failed, timed-out or identity-cancelled reads cannot resume it. This read-only reset never changes incident, approval, attachment or assessment replay authority.
Conversation-assurance readiness for Resource state and Resource Health uses the state-bearing,
metadata-bearing Resource subset rather than every inventory object. This limits challenge
selection only; it does not hide unavailable evidence or widen what the Console may read.
...
This doc covers the **pull direction** - the operator asks, simulates, approves - across every channel the notification doc already ships adapters for. Push and pull share the same channel credentials and the same audit contract, but they are distinct integration surfaces. For local preparation, the private `console/.env.local` may carry browser bindings plus the server-owned `FDAI_LOCAL_RESOURCE_GROUP` selector; explicit process values take precedence, duplicate keys fail closed, and the selector grants no execution authority. Local preparation never reads Terraform state. Reusing the local-state preparation cache also requires the live legacy and all five service migration heads, so recreating the database inside the same Docker volume reruns schema preparation.
The Rules workspace separates catalog origin from deployment-local activation membership. A
selected Rule shows its current PostgreSQL generation and actor history. A Contributor can submit
an inert membership request, and a distinct Approver or Owner can approve the exact proposal
...
The three existing Slack handoff method/path entries belong to the IAM route family exactly once. The aggregate route check pins their ownership and registered count; it grants no new approval or execution authority.
...
The Console accepts only Core-projected version 2 intent evidence, displays exact revision citations and incomplete-coverage limits, and never treats document text as instructions or current operational state. The committed semantic-intent coverage artifact is regenerated from authoritative sources, binds the exact federated question-bank digest, and grants no runtime authority; format-only owner reflow may refresh its catalog citation pin without changing Console behavior. The same source-only refresh neither creates nor changes a human report line. A busy-input catalog label refresh likewise regenerates the 400-question inventory and dependent semantic-intent coverage digest without changing question identity, readiness, evidence authority, attachment behavior, or report-line behavior. Registering an isolated AKS execution route changes neither Console approval authority nor the requirement for independent live effect evidence. Assigning its tests to the isolated Executor service suite changes test collection metadata only.
The test-context candidate labels also require a digest-only refresh of the joined English and Korean question-bank sources and dependent semantic-intent coverage. The 400 question identities, readiness states, coverage denominators, evidence authority, and action authority remain unchanged; generated-artifact equality is not semantic or operational readiness evidence.
