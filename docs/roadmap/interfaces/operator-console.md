...
Knowledge > Documents restores a bounded collection-scoped document list after refresh. Changing
the collection clears prior rows before the next authorized read, and loading, unavailable, error,
and empty states remain distinct without granting document or collection authority.
