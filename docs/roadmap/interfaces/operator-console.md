...
Vertical and Operating Outcomes keep domain rows visible when attribution is missing, preserve the canonical observed-event denominator, and separate unavailable from measured zero through the schema-versioned wire contract. They present zero savings only when at least one cost event was observed in the measurement window.
