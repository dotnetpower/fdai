...
| Exact contextual Resource selection | implemented | `context.tsx`; `backend-context.ts`; `ontology-instances.tsx`; `instance_explorer.py`; focused Console context, Operator envelope, and Core contextual FunctionType checks | A complete screen publishes an exact resource-id set only with a server-issued digest bound to principal scope, release, generation, and completeness. Operator carries it as typed `conversation_context`, and Core intersects explicit utterance filters before the contextual read. Duplicate, incomplete, or widened membership returns a typed limitation and never broadens to the principal-visible Resource set. Authenticated Browser receipt remains open. |
