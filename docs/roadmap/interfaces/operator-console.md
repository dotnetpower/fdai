...
One Operator-owned Event Hubs Kafka adapter publishes proposals and consumes projections with all-or-none topics, command identity, idempotent bounded JSON, post-persistence commits, and sibling-DLQ handling; `GET /chat/health` reads that bridge's process-owned worker readiness directly instead of requiring a durable conversation projection row.
