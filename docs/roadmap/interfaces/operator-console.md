...
Local startup reuse fingerprints derive tracked source state from Git index identities and current
worktree differences, include non-ignored additions and exact explicitly named private files, and
exclude ignored generated directory artifacts. Preparation and service launch emit content-free
`duration_ms` events for each input digest and preparation stage. Reuse still requires the
expected outputs, authentication mode, managed ownership, and current readiness; a changed
fingerprint never bypasses the managed whole-stack restart boundary. The active supervisor holds
one retry-stable local analyzer run identity: a duplicate start reuses that identity, while a new
supervisor generation creates a fresh identity so readiness still requires its first clean tick.
After the shared readiness check passes, the duplicate start exits successfully and the generation
owner alone continues supervising the long-lived children.
