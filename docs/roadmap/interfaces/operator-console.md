...
| CLI | n/a | stdin/stdout UI calling the shared Operator API `/chat`; the explicit local Azure CLI profile bootstraps only through loopback `GET /local-auth/me`, retains its opaque bearer in process memory for snapshot, conversation, and SSE reads, accepts no token through arguments, URLs, or environment variables, and preserves missing bootstrap, `401`, and `403` as non-authorizing outcomes | local auth/Operator API config; does not replace or weaken Browser Entra |
