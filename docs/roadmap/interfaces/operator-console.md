...
| Deterministic subscription identity and semantic isolation | implemented | `adapters/subscription_scope.py`; `families/conversation/subscription_scope.py`; `semantic_turn_runtime.py`; `semantic_turn_consumer.py`; focused Operator, Core, contract, and Console checks | Catalog-matched subscription identity reads use the server-owned Azure Reader scope and persist one verified deterministic receipt without invoking semantic model transport. Conversation T2 is disabled in production composition. Semantic requests use `session_id` as the Kafka key and two same-group Core consumers use broker partitions so one slow session does not block another. Local startup reconciles the physical topic to at least two partitions, and deployed infrastructure rejects a lower partition count. Authenticated browser evidence remains open. |
...
| 2026-08-17 | implemented | Restored the deleted current-subscription deterministic read in the independent Operator Service, disabled automatic conversation T2 binding, and replaced global semantic-turn serialization with session-keyed partition workers. | `current change`; focused provider, bridge, contract, Core worker, local partition, Terraform, and Console parser checks. No Azure authentication or subscription-RBAC defect caused the reported turn. | Retain an authenticated bilingual browser receipt and a synthetic throttled-session isolation receipt before reporting runtime validation. |
