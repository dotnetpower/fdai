...
| `query_inventory(resource_type, filter)` | Server-owned Azure inventory-view count, list, type, location, resource-group, name, status, and relationship queries. Returns bounded allowlisted fields plus active view and snapshot source/freshness. Local VM state comes from `az vm list --show-details`; local PostgreSQL Flexible Server state comes from `az postgres flexible-server list`; deployed Resource Graph rows normalize the observed service `properties.state`. Colloquial database status filters such as `stopped db` and `중지된 디비` stay on this deterministic local tool and don't start agent or public-web branches unless the operator explicitly requests web search. By-type wording groups only records that satisfy every resource and status predicate. Stopped and paused observations remain distinct, while colloquial Korean `멈춰 있는` can select both. After the server read completes, the stream exposes a canonical `query_inventory` operation with server scope and query values redacted, plus a bounded status and count summary. Provider failure renders unavailable. AKS results stay cluster-only unless a server-owned workload provider is explicitly bound to a kubeconfig, context, and inventory-matching cluster name; a valid binding adds bounded Deployment and Pod readiness with observation time for that cluster, while other matched clusters remain an explicit coverage gap. | Reader | `InventoryGraphProvider`, `KubernetesWorkloadProvider` |
...
| `query_subscription_health()` | Inspect the server-configured Azure reader scope for explicit subscription checks and general service-outage questions. Query Resource Graph inventory and Resource Health, fall back to current Resource Health status by allowed resource group when ARG is empty, then run bounded representative metric checks. Return findings, cause classification, coverage gaps, freshness, and truncation without allowing caller-supplied scope. | Reader | `SubscriptionHealthProvider` |
