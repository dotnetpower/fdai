...
| Command Deck workspace visual hierarchy | implemented | `console/src/styles.css`; `structured-reply.css`; `command-deck-workspace-visual.test.ts`; focused Console checks (`29 passed`) and typecheck | Workspace answers and structured evidence share one 760 px reading measure, long values wrap within their cells, long-table headers remain visible on desktop, and the existing labeled-row reflow remains active at narrow widths. A ten-question authenticated browser canary found no horizontal body, transcript, or answer overflow at 993 px or 390 px. No governed browser artifact was retained. |
...
- [x] Keep Command Deck answers, structured evidence, and the composer on one bounded reading hierarchy; focused tests, typecheck, and the ten-question desktop/mobile browser canary pass with no horizontal overflow.
