...
The Document ingestion route derives picker filters and readable format labels from server capabilities. It rejects unsupported and legacy Office extensions before upload and keeps signature mismatch, malformed package, unavailable OCR, encrypted content, and no-content outcomes explicit. Client preflight is guidance only; the API and worker remain authoritative.
