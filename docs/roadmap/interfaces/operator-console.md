...
Tag-remediation canaries remain outside the Console mutation surface. The Console can present the
ActionType mode, human approval, dispatch, rollback, and independently verified effect records, but
it cannot select a target, supply a tag value, promote the ActionType, or call the development
operations gateway. Those transitions continue through typed control-loop and deployment paths.
