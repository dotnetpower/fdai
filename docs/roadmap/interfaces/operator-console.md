...
| Exact contextual Resource selection | implemented | `context.tsx`; `backend-context.ts`; `ontology-instances.tsx`; focused Console context, Operator envelope, and Core contextual FunctionType checks | The selected screen publishes an exact resource-id set. Operator carries it as typed `conversation_context`, and Core uses a server-owned id predicate before the contextual read. Duplicate, incomplete, or widened membership returns a typed limitation and never broadens to the principal-visible Resource set. Authenticated Browser receipt remains open. |
