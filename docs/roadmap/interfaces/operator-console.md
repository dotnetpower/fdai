...
Settings > Runtime policies exposes aggressive T2 answer recovery to Owners. Development shows the control enabled by default, while staging and production remain disabled until promotion evidence exists. Saving the audited revision applies to later interactive read turns without restarting Core. The control cannot enable T2 for Golden campaigns, action drafts, scope or authorization denials, or execution paths, and it does not relax ontology or evidence verification. For optional Console projections, typed `404`, `501`, and source-gate `503` responses render as unavailable. Authentication failures, unexpected transport or `500` responses, and decoder failures remain visible errors.
