...
Vertical and Operating Outcomes keep domain rows visible when attribution is missing, preserve the canonical observed-event denominator, report missing values as unavailable, and present zero savings only when at least one cost event was observed in the measurement window.
