...
| `query_inventory(resource_type, filter)` | Server-owned Azure inventory-view count, list, type, location, resource-group, name, status, and relationship queries. Returns bounded allowlisted fields plus active view and snapshot source/freshness. Local VM state comes from `az vm list --show-details`; local PostgreSQL Flexible Server state comes from `az postgres flexible-server list`; deployed Resource Graph rows normalize the observed service `properties.state`. Colloquial database status filters such as `stopped db` stay on this deterministic local tool and don't start agent or public-web branches unless the operator explicitly requests web search. Provider failure renders unavailable. AKS results stay cluster-only unless a server-owned workload provider is explicitly bound to a kubeconfig, context, and inventory-matching cluster name; a valid binding adds bounded Deployment and Pod readiness with observation time for that cluster, while other matched clusters remain an explicit coverage gap. | Reader | `InventoryGraphProvider`, `KubernetesWorkloadProvider` |
