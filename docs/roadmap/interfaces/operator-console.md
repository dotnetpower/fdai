...
Knowledge > Documents restores a bounded collection-scoped document list after refresh. Changing the collection clears prior rows before the next authorized read, and loading, unavailable, error, and empty states remain distinct without granting document or collection authority.
...
The header places a localized Guides command immediately before the account identity. It opens a modal right drawer populated from a strictly validated `catalog.json` at the independent Manual Studio URL configured through `VITE_MANUAL_STUDIO_URL`. The drawer presents the five-stage FDAI Value-to-Scale Journey as a compact dark Cover Flow with one pixel-stable focused manual, adjacent context, and a decorative fading reflection. Each 4:5 book cover includes its deck type, `L100` through `L400` depth, stage, title, duration, and slide count instead of repeating those facts below the carousel. A missing depth defaults to `L100`. Mouse or touch dragging captures immediately, interpolates each cover independently, and settles across one or several manuals according to gesture distance. Covers leave toward the corresponding outer edge while fading instead of collapsing behind the focused cover. Arrows and the keyboard preserve single-step navigation. The independent library expands the same journey under a compact masthead without changing its information model. Cover assets must remain within that configured base path, and selecting a manual opens its HTML slide presentation in a new tab. The Manual Studio server permits catalog reads only from `MANUAL_STUDIO_ALLOWED_ORIGINS`. An unavailable or invalid catalog remains explicit and grants no runtime authority.
