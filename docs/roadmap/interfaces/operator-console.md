...
| Command Deck workspace visual hierarchy | implemented | `console/src/styles.css`; `investigation-timeline.tsx`; `retrieval-trace.tsx`; `structured-reply.css`; focused Command Deck checks (`30 passed`) | Workspace answers and structured evidence share one 760 px reading measure. Settled observed work becomes one collapsed native disclosure before the answer, while its read-only execution details remain inspectable. On desktop, `Preparing answer` keeps the header and each stage on one line, truncates bounded detail with an ellipsis, and preserves the open three-source evidence window. Verified tables prioritize readable names and keep body, transcript, and table horizontal overflow at zero. No governed browser artifact was retained. |
