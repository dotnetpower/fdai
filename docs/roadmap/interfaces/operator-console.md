...
The Console accepts only Core-projected version 2 intent evidence, displays exact revision citations and incomplete-coverage limits, and never treats document text as instructions or current operational state. The committed semantic-intent coverage artifact is regenerated from authoritative sources, binds the exact federated question-bank digest, and grants no runtime authority; format-only owner reflow may refresh its catalog citation pin without changing Console behavior.
