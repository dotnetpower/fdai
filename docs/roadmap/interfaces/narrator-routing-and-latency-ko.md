...
translation_source_sha: d4607ebe47a461f54a18be975c33a76703ae54ef
...
[알림 품질 API](../operations/alert-noise-governance-ko.md)는 대화 라우팅의 우회 경로가 아니라
...
함께 활성화하지 않습니다. 어댑터는 주입된 HTTP 스트림을 명시적 `POST` 메서드로 호출합니다.
답변 텍스트가 없는 Azure 프롬프트 필터, 통과한 콘텐츠 필터, 종료 및 사용량 메타데이터 프레임은
건너뛰고, 잘못되었거나 크기 제한을 넘는 데이터는 계속 안전하게 차단합니다. 프로바이더 상태 및
잘못된 프레임 경고는 범위가 제한되며 프롬프트, 엔드포인트, 토큰 또는 응답 본문을 포함하지
않습니다. 범위가 제한된 시스템 프롬프트는 FDAI의 제품 범위를 설명하고 FDAI를 임의로 확장할
약어가 아닌 제품 이름으로 취급합니다. 불투명한 대화 이미지 식별자를 검증된 크기 제한 바이트로
변환하는 서버 소유 해석기가 없으면 이미지 턴은 계속 사용 불가입니다. 클라이언트 이미지 필드는
그 권한을 대신할 수 없습니다.
...
| 사용자별 라우팅 선호 설정 및 런타임 지연 시간 변환 결과 | in-progress | `services/operator-service/src/fdai_operator_service/{postgres_iam_configuration,model_lifecycle_startup}.py`; `services/operator-service/tests/test_narrator_preference_persistence.py`; `tests/integration/services/test_narrator_preference_postgres.py`; `scripts/lib/design-routes.json` | 인증된 설정 경로는 시작 시 다이제스트가 고정된 해석 모델의 서술기 후보로 선택을 검증하고, principal별 상태와 실행 권한이 없는 제안을 하나의 개정 번호 보호 PostgreSQL 트랜잭션으로 기록합니다. 어댑터를 다시 만들어도 저장된 선택을 읽으며 배포가 제거되면 선택을 지우지 않고 실효 선택을 `Auto`로 되돌립니다. 경로 및 결정적인 원자적 상태 검사는 통과했습니다. 승인된 로컬 전용 DSN은 이 세션에서 가려져 있어 실제 PostgreSQL 재시작 및 동시 쓰기 근거는 아직 없습니다. T2 연결은 시스템이 관리하며 배포 및 지연 시간 근거는 남아 있습니다. |
...
- [x] principal별 `Auto` 또는 허용된 서술기 선호 설정 검증과 정제된 변환 결과가 `services/operator-service/src/fdai_operator_service/adapters/narrator_preferences.py`에 있으며 집중 선호 설정 검사가 이를 뒷받침합니다. 변환 결과는 `personalizes_t2_bindings: false`를 선언하고 엔드포인트나 자격 증명을 담지 않습니다.
- [ ] 지원되는 로컬 전용 `FDAI_ASSIGNMENT_TEST_DSN`으로 `uv run pytest -q --no-cov tests/integration/services/test_narrator_preference_postgres.py`를 실행하고, 정확한 시작 시 고정 출처에 대해 실제 PostgreSQL 재시작 조회, 원자적 감사/상태 기록, 동시 개정 번호 비교 및 교환의 통과 근거를 남긴 뒤 principal별 영속 선호 설정 작업을 완료합니다.
