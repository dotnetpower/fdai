...
translation_source_sha: b80715dad5b45749fcdfb6b9eccaaeb81dc8455c
...
지식 > 문서는 새로 고친 뒤 컬렉션 범위가 적용된 문서 목록을 제한된 개수로 복원합니다. 컬렉션을 바꾸면 다음 권한 적용 읽기 전에 이전 행을 지우며, 문서나 컬렉션 권한을 부여하지 않고 로딩, 사용 불가, 오류, 빈 상태를 서로 구분합니다. 사용할 수 없는 문서 작업은 키보드로 초점을 이동하여 그 사유를 확인할 수 있지만 활성화 처리기가 없으므로 작업을 실행할 수 없습니다. 인증된 변환 결과 판독기는 드라이버 자리 표시자와 식 그룹화를 명시적으로 유지하여 저장된 데이터가 SQL 구문 분석 오류로 가려지지 않게 합니다. 실시간 상태 요약과 문서 식별 행은 1440, 993, 390 픽셀 너비에서 주 콘텐츠 안으로 재배치됩니다.
