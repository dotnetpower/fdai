...
| 정확한 contextual Resource 선택 | 구현됨 | `context.tsx`, `backend-context.ts`, `ontology-instances.tsx`, `context_selection.py`, `instance_explorer.py`, 집중 Console context, Operator envelope 및 Core contextual FunctionType 검사 | 완전한 화면은 인증된 principal, 일반 소문자 role 범위, purpose, 정확한 release, source generation, completeness 및 Resource id에 결속된 불투명한 서버 발급 selection token을 게시합니다. token은 전용 `context_capability` redaction allowlist를 통해 전달되고, Operator는 `conversation_context`를 만들기 전에 token을 조회합니다. 클라이언트 id 위조/recompute, 재시작 후 사라진 token, 다른 principal/role/purpose의 token은 거부됩니다. Core의 FunctionType 자체도 capability를 요구하고 연결되지 않은 specialized node를 거부한 뒤 object-only contextual read 전에 명시적 발화 조건과 교집합합니다. 불완전한 결과는 전체 Resource 집합으로 확장되지 않습니다. 인증된 Browser receipt는 아직 열려 있습니다. |
...
| 2026-08-20 | implemented | Extension 계열 때문에 기존의 operation당 10개 gate가 산술적으로 불가능해진 뒤 seeded operation coverage를 정본 생성 cohort에 결속했습니다. Exact histogram 검사는 generator 분포를 release oracle에 복제하지 않고 누락된 결과 또는 같은 크기의 operation 대체를 거부합니다. | [Issue #233](https://github.com/dotnetpower/fdai/issues/233), `current change`, 실행 `question-space-final-b4604e07b-20260820-r11`은 모든 hard-zero 카운터 0으로 seeded judgment 100/100을 완료했고 집중 보증 테스트 101개가 통과했습니다. | 중앙 검증 뒤 통과한 정확한 소스 seeded artifact를 보존합니다. |
...
Operator 소유 Kafka 어댑터는 의미 제안을 게시하고 의미 변환 결과를 소비하며 검증된 Core 단계 및 Pantheon 런타임 상태 프레임을 별도의 범위 제한 `/live/stream`과 `/agents/stream` SSE 허브로 중계합니다. 인증된 `GET /agents/activity`는 Console이 새 스트림 프레임을 적용하기 전에 영속 원본에서 범위가 제한된 인벤토리 검사, 온톨로지 변환 및 현재 상태 읽기 이력을 제공하며 현재 상태 재생과 실제 운영 프레임은 같은 hash-correlation activity id를 사용합니다. 이 관찰 경로는 스냅샷 읽기와 같은 bearer 게이트를 사용하고 Kafka가 없으면 연결 유지 신호를 보내며, 권위 있는 프레임이 도착할 때까지 `Awaiting source`를 표시합니다. `GET /chat/health`는 영속 대화 변환 결과 행을 요구하지 않고 의미 브리지의 프로세스 소유 워커 준비 상태를 직접 읽습니다.
