...
translation_source_sha: e964634cfd0528444c2ee85b183b101c348eab1f
...
| 정확한 contextual Resource 선택 | 구현됨 | `context.tsx`, `backend-context.ts`, `ontology-instances.tsx`, `context_selection.py`, `instance_explorer.py`, 집중 Console context, Operator envelope 및 Core contextual FunctionType 검사 | 완전한 화면은 인증된 principal, 일반 소문자 role 범위, purpose, 정확한 release, source generation, completeness 및 Resource id에 결속된 불투명한 서버 발급 selection token을 게시합니다. Operator는 `conversation_context`를 만들기 전에 token을 조회합니다. 클라이언트가 보낸 id나 다시 계산한 id, 재시작 후 사라진 token, 다른 principal/role/purpose의 token은 거부됩니다. Core는 object-only contextual read 전에 명시적 발화 조건과 교집합하며, 불완전한 결과는 held 또는 unavailable로 남고 principal 전체 Resource 집합으로 확장되지 않습니다. 인증된 Browser receipt는 아직 열려 있습니다. |
