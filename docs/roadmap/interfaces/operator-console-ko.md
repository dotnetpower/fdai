...
translation_source_sha: 52ad71b1f06760fd24d3a7addd70ed1d2a1f3518
...
`GET /audit/traces?limit=25`는 최신 감사 기록 500개에서 서버가 소유하는 탐색 인덱스를
제공합니다. limit는 1부터 50까지로 제한합니다. 응답은 표본 제한과 더 오래된 감사 기록의
존재 여부를 표시하므로 Console은 이를 최근 인덱스로 표시하고 전체 이력이라고 제시하지 않습니다.

각 탐색 행은 상관관계 ID, 최신 순번과 시각, 최신 행위자와 작업 종류, 추적 종류, 최근 기록된
판단과 모드, 고유 대상 수와 모호하지 않은 정확한 대상, 표본 감사 기록에서 인시던트 또는 근본
원인 근거가 발견됐는지를 보존합니다. 이 근거 플래그는 기록된 행만 설명하며 목적지 변환 결과를
사용할 수 있음을 보장하지 않습니다.

선택한 추적 응답은 계속 권위 있는 상세입니다. 순서가 있는 단계 외에 추적 종류, 출처 권한,
완전성, 최초 및 최종 기록 시각, 최신 활동, 최종 명명 단계, 고유 대상 범위, 액션 시도 수,
독립 효과 관측 수를 제공합니다. 서버가 추적이 500개 제한을 넘지 않음을 확인한 경우에만
완전성을 true로 표시합니다.
