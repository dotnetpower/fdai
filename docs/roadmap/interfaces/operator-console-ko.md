...
translation_source_sha: 632f409e19d7b2e31779e2148593fa594a4c0d6a
...
헤더는 계정 ID 바로 앞에 현지화된 설명서 명령을 배치합니다. 이 명령은 `VITE_MANUAL_STUDIO_URL`로 구성한 독립 Manual Studio URL의 엄격하게 검증된 `catalog.json`으로 채운 모달 우측 패널을 엽니다. 패널은 5단계 FDAI Value-to-Scale Journey를 픽셀 크기를 유지한 하나의 설명서에 초점을 맞추고 인접 맥락과 아래로 흐려지는 장식용 반사를 함께 보여 주는 간결한 어두운 Cover Flow로 표시합니다. 각 4:5 책 표지는 carousel 아래에 같은 내용을 반복하지 않고 설명서 종류, `L100`부터 `L400`까지의 깊이, 단계, 제목, 예상 시간, 슬라이드 수를 표지 안에 표시합니다. 깊이 값이 없으면 `L100`을 기본값으로 사용합니다. 마우스나 터치를 누르는 즉시 포인터를 포착하고 각 표지의 위치를 독립적으로 보간하며, 이동 거리에 따라 설명서 한 개 또는 여러 개를 건너뛰어 정렬합니다. 화면에서 빠지는 표지는 중심 표지 뒤로 모이지 않고 해당 바깥 방향으로 계속 이동하면서 흐려집니다. 화살표와 키보드는 한 단계 이동을 유지합니다. 독립 라이브러리는 같은 정보 모델을 유지하면서 간결한 상단 영역 아래에 여정을 더 넓게 표시합니다. 표지 자산은 구성된 기준 경로 안에 있어야 하며, 설명서를 선택하면 HTML 슬라이드 프레젠테이션이 새 탭에서 열립니다. Manual Studio 서버는 `MANUAL_STUDIO_ALLOWED_ORIGINS`에 명시된 출처의 카탈로그 읽기만 허용합니다. 카탈로그를 사용할 수 없거나 검증할 수 없으면 그 상태를 명시적으로 표시하며 런타임 권한을 부여하지 않습니다.
