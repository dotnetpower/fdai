...
translation_source_sha: 74460e1daaa1541ed8d3b7550999cd01d1ae79ff
...
에이전트 활동은 시간 순서가 있는 작업 및 인계 근거를 소유합니다. URL로 복원할 수 있는 역할 및 소유권 대화 상자에는 고정 보고 트리, 소유 객체 유형, 권한 경계 및 선택한 에이전트 요약만 표시합니다. 대화 상자는 활동 필터를 유지하고 Back 또는 Escape로 닫히며 선택한 역할을 필터가 적용된 Waterfall로 연결합니다. `/pantheon`은 공유할 수 있는 역할 전용 fallback입니다. 두 조직 화면 모두 인시던트 타임라인, 워크플로 또는 대화 기록을 렌더링하지 않습니다. 실시간 활동, 영속 감사 상위 기록 및 펼친 감사 대화에는 독립된 한도를 적용하므로 대화를 펼쳐도 명시적으로 가져온 운영 활동 500건이 제거되지 않습니다. 실시간 커버리지 카드는 로딩 및 사용 불가 상태에서도 링크로 유지되고 권한을 부여하지 않는 인벤토리 묶음에서만 ARG 합계를 해석하며, 선택적 `rule.findings-summary` 변환 결과가 없으면 평가되지 않음으로 표시합니다. 공유 워크플로 어댑터는 이 요약을 문서 첨부나 근거 작성기로 라우팅하지 않습니다.
