...
translation_source_sha: 8f4c09d2244c90bce4c31d7998e54fe4328218e0
...
지식 > 문서는 새로 고친 뒤 컬렉션 범위가 적용된 문서 목록을 제한된 개수로 복원합니다. 컬렉션을 바꾸면 다음 권한 적용 읽기 전에 이전 행을 지우며, 문서나 컬렉션 권한을 부여하지 않고 로딩, 사용 불가, 오류, 빈 상태를 서로 구분합니다. 사용할 수 없는 문서 작업은 키보드로 초점을 이동하여 그 사유를 확인할 수 있지만 활성화 처리기가 없으므로 작업을 실행할 수 없습니다. 인증된 변환 결과 판독기는 드라이버 자리 표시자와 식 그룹화를 명시적으로 유지하여 저장된 데이터가 SQL 구문 분석 오류로 가려지지 않게 합니다. 실시간 상태 요약과 문서 식별 행은 1440, 993, 390 픽셀 너비에서 주 콘텐츠 안으로 재배치됩니다. 필수 리포트 변수는 오퍼레이터가 변수 집합의 일부를 입력하기 전까지 오류가 아닌 초기 입력 안내로 유지됩니다.
...
운영 화면은 소스 안내에 보이는 제목을 사용하고 프로비저닝 목업의 실행 요약을 따르며, 단계, 준비 상태, 리소스를 키보드로 조작할 수 있는 보기로 구분하되 목업 전용 수명 주기 데이터는 복사하지 않습니다. Console 카탈로그가 바뀌면 소스에 결속된 질문은행과 CQAS 출처 이력도 갱신합니다.
...
체크섬으로 고정한 저장소 생성기는 백엔드 서비스 5개가 사용하는 것과 같은 N/N-1 JSON Schema에서
이를 만듭니다. 생성 인터페이스는 컴파일 시점의 정렬만 개선합니다. 배포된 AKS에서는 배포 조정기가
수렴 뒤 정확한 Static Web Apps redirect를 등록하고 런타임 API 연결을 게시합니다. Operator 서비스는
계속 기준 스키마로 와이어 페이로드를 검증하며 Console에는 배포, 승인, 변경 또는 실행 권한이 없습니다.
감사 작업 영역은 추가적인 `summary=true` 묶음을 요청합니다. 현재 Operator는 보존 중인 조회 범위
수치, 기록 맥락 및 무결성 관측을 반환하고 이전 Operator는 Console이 요약 근거를 사용할 수 없는
상태로 처리하는 페이지 전용 묶음을 유지합니다. Incident, Agent Activity 및 Trace 읽기는 이 활성화
설정을 보내지 않고 범위가 제한된 페이지 비용을 유지합니다.
