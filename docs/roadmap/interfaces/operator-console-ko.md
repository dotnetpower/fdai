...
translation_source_sha: 162a73ac1e0cd24d05fa1ae202e73277fb4a5ee2
...
연결된 Console 카탈로그 원본이 바뀌면 전체 질문 은행과 의미 의도 범위를 차례로 다시 생성합니다. 산출물 동등성 검사는 오래된 원본 다이제스트를 차단하며 출처 이력만 갱신하는 작업은 질문 신원, 준비 상태 또는 권한을 바꿀 수 없습니다. 대화 전달도 기록된 차단기 모드별 개수만 표시합니다. System Knowledge 원본의 검증값을 다시 생성해도 브라우저에 일시 중지, 재개, 재시도, 승인 또는 에스컬레이션 권한이 생기지는 않습니다.
