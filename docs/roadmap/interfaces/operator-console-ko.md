...
translation_source_sha: 65a9f151a6195239d2b3b6bd20006f49969fc896
...
| `query_inventory(resource_type, filter)` | Server-owned Azure inventory count, list, type, location, resource-group, name, status, relationship query입니다. Schema-validated `inventory-query-language.yaml` catalog가 자연어 term, 상태와 operation 의미, evidence authority, grouping, projection, scope default, freshness requirement를 소유하고, Python은 질문별 alias 없이 generic token matching과 typed query assembly만 수행합니다. Resource type은 별도의 canonical resource-type catalog에서 가져옵니다. Typed query가 evidence retrieval 전에 scope, grouping, projection, workload intent, state-history intent, freshness를 소유하므로 renderer는 prompt를 다시 해석하지 않습니다. 한정되지 않은 cross-screen inventory read는 server-owned subscription root를 사용하고, current-view wording을 명시하면 active architecture view를 유지합니다. Current-state 질문은 provider refresh barrier를 기다리고 stale evidence로 확정 답변을 하는 대신 unavailable을 반환합니다. 일반적인 degraded 또는 unavailable resource collection은 catalog authority에 따라 `query_subscription_health`로 route되고, 구체적인 resource-family filter는 inventory read에 유지됩니다. 결과는 제한된 allowlist field, 정확한 scope, snapshot source/freshness 및 모든 predicate에 일치하는 record만 노출합니다. 명시적 semantic state group은 서로 분리되고 근거 있는 zero-result group도 표시할 수 있습니다. 상태 필터 답변은 normalized current operational status가 deployment 또는 Activity Log 실패의 부재를 증명하지 않는다는 한계도 표시합니다. Stream은 redaction된 server scope와 bounded result metadata가 포함된 verifier-accepted canonical `query_inventory` operation을 노출합니다. Provider failure는 unavailable로 표시합니다. Server-owned workload provider가 inventory와 일치하는 cluster에 명시적으로 binding되지 않으면 AKS result는 cluster resource만 포함합니다. 유효한 binding은 bounded Deployment와 Pod readiness를 추가하고, 일치하는 다른 cluster는 명시적인 coverage gap으로 유지합니다. Current snapshot은 workload가 해당 상태로 전이된 시각을 증명하지 않으며, Kubernetes event 또는 다른 history authority가 연결될 때까지 답변은 그 transition time을 미확정으로 유지합니다. | Reader | `InventoryGraphProvider`, `KubernetesWorkloadProvider` |
