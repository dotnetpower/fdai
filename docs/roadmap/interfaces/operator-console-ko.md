...
translation_source_sha: 6d230f6b148dc017a9b46c8b03126c077b8602d8
...
규칙 화면은 승인된 카탈로그 작업공간 계층을 따릅니다. 권위 있는 요약 지표와 필터 다음에
220 CSS 픽셀 너비의 카탈로그 보기 레일과 범위가 제한된 상세 표를 배치합니다. 레일은 서버가
제공한 전체, 활성, 수집 및 컨트롤 목적지만 사용하며, 시안의 합성 예외나 shadow 정확도 값을
복사하지 않습니다. 규칙을 선택하면 카탈로그를 다시 마운트하지 않고 URL로 복원할 수 있는
대화 상자를 열며, Escape 또는 브라우저 뒤로 가기로 닫으면 원래 규칙 상세 버튼으로 초점을
되돌립니다. 규칙별 결과 프로바이더가 연결되지 않은 경우, 카탈로그에 있는 규칙은 운영 `503`
대신 빈 결과 목록과 `evaluated: false`를 반환합니다. 알 수 없는 규칙은 계속 `404`를 반환합니다.
