...
Semantic transport가 구성되면 production composition은 proposal publish와 projection consume에 하나의
...
| `query_llm_usage(group_by, lookback_days, usage_scope)` | 1-90일의 bounded window에서 일, 모델, workload scope 또는 mode별로 측정된 LLM token 사용량을 읽습니다. Independent Operator Service는 SELECT-only runtime role로 `llm_invocation`에서 동일한 token-only projection을 제공하며 record 및 conversation ledger는 500개로 제한하지만 aggregate count는 정확하게 유지합니다. Operator-chat record로 좁힐 수 있고, 측정된 가격 근거 없이 금액을 추정하지 않으며, deterministic prose, table 또는 chart output을 반환합니다. `LlmCostPanel`은 `conversation_tool`로 이 tool을 선언하며 chat-enabled composition에 선언된 capability가 없으면 startup이 실패합니다. | Reader | `MeteringReader` |
...
deterministic hold가 authoritative 상태를 유지합니다. Resolver는 catalog vector를 build하거나 query embedder를 호출하기 전에 empty prompt, control character 및 4,096자를 초과하는 text를 거부합니다.
