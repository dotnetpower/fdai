...
- **대화 원장**: inbound와 terminal assistant turn은 stable request idempotency
  ...
  reference만 남긴다.
- **완료 요청 재실행**: 같은 principal과 conversation에서 동일 요청을 재시도하면 evidence
  retrieval, narrator, post-turn review를 다시 실행하지 않고 저장된 terminal assistant payload를
  반환합니다. 변경된 content 또는 다른 conversation에서 같은 key를 재사용하면 conflict이며,
  JSON과 SSE는 같은 durable terminal payload를 사용합니다.
...
- **세션 복구** - principal-scoped `ConversationHistoryStore`에서 session id로 이전 turn을
  ...
  raw transcript가 아니라 hash와 reference만 남습니다. JSON, SSE, cross-transport retry는
  두 번째 backend 호출 없이 하나의 저장된 terminal payload를 재실행합니다.
...
각 manifest route에는 정확히 하나의 owner만 있습니다. SPA는 요청 path에서 query와 fragment를
제거하고 path segment 경계에서 exact path 또는 descendant를 match한 뒤 가장 긴 owner를
선택합니다. 비슷한 prefix를 가진 sibling path는 ownership을 상속하지 않습니다.
...
Agent stream의 `401`은 전체 화면 login recovery를 기다리고, `403`은 bounded reconnect를 사용하므로
새로 할당된 App Role을 page reload 없이 적용할 수 있습니다.
Canvas
