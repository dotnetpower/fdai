...
translation_source_sha: 0449d4ab24b9f5cb47d42a5beee2d3c1c3f7fafe
...
초기화하며, 취소 또는 실패한 실행은 명시적인 최종 결과로 유지합니다. 최초 인벤토리 중 `/provision/stream`은 Operator 역할의 PostgreSQL `SELECT` 전용 권한을 통해 Core 소유 개수 전용 원장을 재생합니다. 허용 목록 페이로드에는 구독, 리소스 또는 프로바이더 식별자가 없습니다. Console은 인벤토리 실행과 순서를 전체 초기 구성 실행과 별도로 추적하므로 새 스캔이 단계 준비 상태나 전체 진행률을 초기화할 수 없습니다. 리소스, 페이지, 프로바이더 유형, 관계, 미매핑 개체, 커버리지 공백의 상세 개수는 권위 있는 상태가 아니며 독립적으로 종결된 최종 레코드만 인벤토리 준비 상태를 나타냅니다.
