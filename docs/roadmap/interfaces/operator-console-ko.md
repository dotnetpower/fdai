...
translation_source_sha: 0b14993fb7c25aa73de04093df9daa85fc76b2d0
...
인벤토리 스캔, 온톨로지 변환 결과 및 현재 상태 읽기 상관관계는 감사 추적 링크 없이 식별자로 표시합니다. 일치하는 감사 단계가 없는 수동 조회는 운영 실패가 아니라 중립적인 사용 불가 상태로 표시합니다. 에이전트 활동은 시간 순서가 있는 작업 및 인계 근거를 소유합니다. URL로 복원할 수 있는 역할 및 소유권 대화 상자에는 고정 보고 트리, 소유 객체 유형, 권한 경계 및 선택한 에이전트 요약만 표시합니다. 대화 상자는 활동 필터를 유지하고 Back 또는 Escape로 닫히며 선택한 역할을 필터가 적용된 Waterfall로 연결합니다. `/pantheon`은 공유할 수 있는 역할 전용 fallback입니다. 두 조직 화면 모두 인시던트 타임라인, 워크플로 또는 대화 기록을 렌더링하지 않습니다.
추적 화면은 요약, 상관관계 조회, 순서가 있는 단계 목록, 선택한 근거 상세, 액션 시도별 수명
주기 및 전체 감사 타임라인을 하나의 범위가 제한된 작업 공간에 유지합니다. 대기, 로딩, 준비,
빈 결과, 사용 불가 및 오류 상태에서도 같은 구조를 사용합니다. 단계를 선택하면 표시만 바뀝니다.
현지화된 단계, 작업, 상태 및 시각 레이블은 근거 검토를 위해 정규 원문 값과 정확한 타임스탬프를 유지합니다.
...
- 각 단계는 항목 해시와 이전 항목 해시를 감사 원문 출처 이력으로 보존합니다. 추적 화면은
...
Console은 `GET /audit?limit=500`을 최근 탐색 표본으로 재사용합니다. 이미 권위 있는 행을
표시 목적으로만 그룹화하고 정렬하며, 더 오래된 감사 기록이 있음을 나타내는 명시적 신호로
`next_cursor`를 유지합니다. 이 목록은 최근 기록으로 표시하며 전체 이력이라고 제시하지
않습니다. 표시용 그룹화는 새 권한을 부여하거나 별도 읽기 모델 계약을 만들지 않습니다.

각 탐색 행은 상관관계 ID, 최신 순번과 시각, 최신 행위자와 작업 종류, 최근 기록된 판단과 모드,
모호하지 않은 정확한 대상, 표본 감사 기록에서 인시던트 또는 근본 원인 근거가 발견됐는지를
보존합니다. 이 근거 플래그는 표본 행만 설명하며 목적지 변환 결과를 사용할 수 있음을 보장하지
않습니다.

선택한 추적 응답은 계속 권위 있는 상세입니다. 서버는 순서가 있는 단계 외에 추적 종류, 출처
권한, 완전성, 최초 및 최종 기록 시각, 최신 활동, 최종 명명 단계, 고유 대상 범위, 액션 시도 수,
독립 효과 관측 수를 제공합니다. 서버가 추적이 500개 제한을 넘지 않음을 확인한 경우에만 완전성을
true로 표시합니다.

상관관계를 선택하지 않으면 Console은 최근 탐색 목록을 열고, 선택한 뒤에도 간결한 공개
컨트롤로 유지합니다. 첫 요약은 판단, 운영 효과, 완전성 및 최종 단계와 최신 상태의 차이를
보여 줍니다. 해당 없음, 기록 없음, 대기, 실패 및 독립 관측 상태를 구분합니다. 단계가 없는
활동은 현지화된 작업 제목을 사용하고, 행위자는 현지화해 표시하되 상세에는 정규 행위자를
유지합니다. 판단, 결과 및 모드는 각각 레이블을 표시합니다. 복사 작업은 감사 기록을 변경하지
않고 범위가 제한된 완료 피드백을 제공합니다.

액션 식별 정보가 없는 추적은 액션 수명 주기를 한 줄의 액션 없음 상태로 축약합니다. 전체
타임라인 공개 컨트롤은 기록 수를 표시합니다. 좁은 화면에서는 요약을 간결한 행으로 바꾸고,
44 px 컨트롤, 텍스트 간격 및 정확한 식별자를 보존하면서 두 번째 화면 안에서 단계 목록을
시작합니다. 관련 목적지는 추가 오버레이 대신 범위가 제한된 2열 그리드로 재배치합니다.
넓은 화면에서는 상관관계 입력, 복사, 관련 근거 및 새로 고침을 도구 모음 한 행에 유지합니다.
좁은 화면에서는 판단 요약 4개를 2x2 그리드로 표시하고 상세 맥락을 접힌 상태로 시작합니다.
...
작업대가 넓어지지 않게 합니다.
같은 너비에서 복사와 새로 고침 컨트롤은 각각 읽을 수 있는 전체 행을 유지하고 관련 근거 링크는
축약된 2열 그리드로 유지합니다.
읽기 전용 경계의 짧은 설명은 320픽셀에서 제목 아래에 배치해 문장을 읽기 좋은 너비로
유지합니다.
