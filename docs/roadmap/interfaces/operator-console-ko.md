...
translation_source_sha: 7588c70f4164c4776ecbe20488d5bedcd720b05e
...
인증된 `/provisioning` 경로는 하나의 영속 구독 초기 구성 실행을 보여주는 읽기 전용 변환 결과입니다.
완료된 단계, 리소스 검색, 최종 검증을 재생하지만 배포를 시작, 재시도, 승인 또는 변경하지 않습니다.
...
Console은 Core가 제공한 버전 2 의도 근거만 수락하며 정확한 개정 인용과 불완전한 범위의 한계를 표시합니다. 문서 텍스트를 지시나 현재 운영 상태로 취급하지 않습니다. 커밋된 의미 의도 범위 artifact는 권위 있는 원본에서 다시 생성하며 런타임 권한을 부여하지 않습니다. 소유 문서의 서식만 재배치하면 Console 동작을 바꾸지 않고 카탈로그 인용 고정값만 갱신할 수 있습니다.
