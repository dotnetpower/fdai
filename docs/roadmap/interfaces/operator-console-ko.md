...
translation_source_sha: 209771d239e355bef2ee6a1968132aedaba61a29
...
Operator 소유 Kafka 어댑터는 의미 제안을 게시하고 의미 변환 결과를 소비하며 검증된 Core 단계 및 Pantheon 런타임 상태 프레임을 별도의 범위 제한 `/live/stream`과 `/agents/stream` SSE 허브로 중계합니다. 인증된 `GET /agents/activity`는 Console이 새 스트림 프레임을 적용하기 전에 영속 원본에서 범위가 제한된 인벤토리 검사, 온톨로지 변환 및 현재 상태 읽기 이력을 제공합니다. 이 관찰 경로는 스냅샷 읽기와 같은 bearer 게이트를 사용하고 Kafka가 없으면 연결 유지 신호를 보내며, 권위 있는 프레임이 도착할 때까지 `Awaiting source`를 표시합니다. `GET /chat/health`는 영속 대화 변환 결과 행을 요구하지 않고 의미 브리지의 프로세스 소유 워커 준비 상태를 직접 읽습니다.
