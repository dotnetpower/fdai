...
| 2026-08-16 | implemented | Added the runtime registry document loader so `load_execution_backend_registry_file` has a real caller. An unset path yields no profiles, and a configured document that is missing, non-regular, oversized, malformed, non-object, or self-enabling fails startup instead of degrading silently. | `current change`; `services/core-control-plane/src/fdai/runtime/execution_backends.py`; `pytest services/core-control-plane/tests/runtime/test_execution_backends.py` (11 passed). | Bind both adapters through deployment composition and retain governed shadow receipts. |
