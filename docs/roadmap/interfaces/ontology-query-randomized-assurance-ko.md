...
`chat.stream` reader로 구성합니다. 이 어댑터는 화면 컨텍스트와 함께 모델을 호출하고
...
2. 프로덕션 의미 모델, principal 범위로 한정된 서술자 인덱스, 정확한 온톨로지 릴리스, 쿼리
  핸들러, 과거 토폴로지 reader, 메트릭 프로바이더 및 규칙과 담당 체계 변환 결과를 Core에
  바인딩합니다.
...
| 13 | 리소스 상태 | 진행 중: 보안 ObjectSet 읽기를 바인딩하고 없는 속성은 unknown으로 유지합니다. |
| 14 | VNet 라우팅 | 진행 중: 토폴로지 및 정확한 리소스 경로 근거를 바인딩합니다. |
| 15 | 프라이빗 엔드포인트 | 진행 중: 정확한 첨부, DNS 및 연결 상태 관측을 바인딩합니다. |
| 16 | 과거 토폴로지 | 진행 중: 신뢰할 수 있는 기준 시점과 bitemporal reader를 구성합니다. |
...
| 28 | 키워드 hardening 제외 | 완료: 구문별 라우팅을 수정 조치에서 제외했습니다. |
...
- 보류, 명확화, 지원하지 않는, 액션 초안 또는 취소된 결과는 prose에서 추론하지
