...
| 2026-08-16 | in-progress | Restored the in-memory behavior index and the tracked-source freshness validator under the current service topology. | `pytest services/core-control-plane/tests/knowledge/test_behavior_index.py` passed 13 focused tests covering idempotent upsert, match-class and authority ordering, stale and untracked citations, Korean paraphrase retrieval, comparison withholding, the retrieval floor, and citation-only exposure. | Restore the 13 reference seeds, bind the server-owned Operator answer path, add persistence, and record governed runtime evidence. |
