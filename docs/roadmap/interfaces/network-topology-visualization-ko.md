...
개요는 표시 기록을 네트워크 경계와 보고된 네트워크 역할로 제한합니다. 순위가 높은 VNet 경계
최대 2개, 관련 Subnet 경계 4개, 관련 네트워크 역할 4개 및 필요한 상위 범위를 유지합니다. 찾은
경로는 반환된 전체 근거 그래프에서 제외됐던 네트워크 평면과 상위 범위를 복원할 수 있습니다. API 응답의 배치 좌표는
