...
translation_source_sha: a07978093c7860df6eeae0c3fed936898c812ff9
...
| 담당 체계 | 담당 체계 v2, 불변 Operator 증적 수신, 감사된 에이전트 검토, 초안 PR 생성, 정확한 서명 병합 상관관계 | 관리되는 GitHub App, 배포된 신원, 현재 수명 주기 근거 |
| 승인 | `HilResumeCoordinator`, 온콜 기본/보조 영수증, 다시 알림, 부하 제어, 주기적 shadow 무응답 관찰 | 운영 승격, 실제 운영 rung-role 검증, urgency 압축 |
| 대화 | 인증된 세션, 영속 턴, 현지화된 초대, 현재 담당자를 다시 확인하는 명령 | 완전한 세션 예산과 수락 근거 |
| 문서 | 에이전트 소유 승인, 원본 구간, 결정적 조각, 형식화된 목표 읽기 관찰, 비활성 후보 | ACL 및 삭제 복구, 담당 에이전트의 후보 소비, 관리되는 전달 근거 |
...
목표 관찰은 내용을 담지 않는 별도의 읽기 전용 계약을 사용합니다. Core는 Operator 목표를
변경할 수 없습니다. 생산자는 한 번에 읽는 행 수를 제한하고 페이지와 출처 우선순위를 순회하며,
실패한 페이지를 재시도합니다. 관찰된 목표 상태는 문서를 허용하거나 지식 후보를 승격하지
않습니다. 공유 테이블 방어와 불변 증적 스키마는 Core가 소유하고 Operator는 이에 의존하는
추가·읽기 권한만 소유합니다. 증적이 남아 있으면 스키마 롤백으로 삭제할 수 없습니다.
...
로컬 수명 주기 강화는 계획을 정확한 사례 리비전, 불변 검토 정족수, 신원 공급자, 정규화된
허용 그룹 하나에 결속합니다. 활성화 전에 공급자 증적을 전체 대상 계획의 다이제스트와
비교합니다. 롤백은 역방향 대상을 독립적으로 다시 읽어 확인한 뒤에만 성공입니다. 권한이나
승격 게이트를 사용할 수 없으면 계속 차단합니다. 회수 계획을 제안하기 전에 대체 사례의
정확한 활성 리비전과 검증된 담당 체계·IAM 결과로 담당 범위를 확인합니다. 계획은 임무나
접근 권한을 제거하지 않으며, 대체 담당자를 승인으로 취급하거나 적용 모드를 활성화하지 않습니다.
기존 회수 ActionType은 shadow 계획에 제한된 `replacement_revisions`를 받습니다.
런타임은 전용 사용자 접근 어댑터 뒤에 계획을 연결하며 담당 범위가 부족하면 보류합니다.
독립적으로 검토한 승격과 실제 롤백 근거가 없으면 적용 모드는 계속 거부합니다.
...
포함합니다. 목표와 업로드 연결 및 비활성 후보 생성이 구현되어 있습니다. Operator 목표는
형식화된 읽기 전용 어댑터로 관찰하며 실제 PostgreSQL에서 쓰기 역할 분리를 검증했습니다.
담당 에이전트의 후보 소비, 문서 ACL 및 삭제 수렴, 실제 공급자 근거는 별도의 잔여 작업입니다.
