...
| Current live certification | in-progress | [Issue #233](https://github.com/dotnetpower/fdai/issues/233); exact-source run `question-space-final-b4604e07b-20260820-r11`; `current change`; focused Console assurance tests | Run r11 passed strict v2 with 22/22 transport, 16 complete-evidence answers, and every hard-zero counter at zero. Seeded assurance then judged all 100 cases, preserved 81/81 complete-evidence answers and every hard-zero counter at zero, but the release oracle required ten cases for all 14 operations, an impossible 140-case distribution. The oracle now compares results with the authoritative generated 100-case cohort histogram. Fresh exact-source strict and seeded evidence is still required for validation. |
