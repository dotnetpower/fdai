...
| Current live certification | in-progress | [Issue #233](https://github.com/dotnetpower/fdai/issues/233); exact-source run `question-space-final-709603208-20260820-r9`; `current change`; 62 focused semantic planning and prompt registry tests | Run r9 transported and judged all 22 cells with every hard-zero counter at zero, but answered only 15 cells and failed the immutable minimum of 16 before seeded assurance. Core now rebinds canonical manifest aggregate kinds and missing exact property predicates from the verified frame, while prompt versions v25 and v18 preserve the two closed axes. A fresh strict and seeded run is still required for validation. |
