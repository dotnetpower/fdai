...
| Current live certification | in-progress | [Issue #233](https://github.com/dotnetpower/fdai/issues/233); exact-source run `question-space-final-f7fff0f9e-20260820-r13`; `current change`; focused semantic planning and prompt tests | Run r13 passed strict v2 with 22/22 transport, 16/16 complete-evidence answers, and every hard-zero counter at zero. Seeded assurance completed all 100 turns and isolated one Korean listing request answered through `query.manifest` plus `aggregate`. Explicit listing without a count or grouping operator now rejects an aggregation frame, while explicit aggregation keeps precedence; frame prompt v27 preserves the same contract. Fresh exact-source strict and seeded evidence is still required for validation. |
