...
| Current live certification | in-progress | [Issue #233](https://github.com/dotnetpower/fdai/issues/233); exact-source run `question-space-final-a50812c49-20260820-r8`; `current change`; 61 focused semantic planning and prompt registry tests | Run r8 answered all 16 answer-required cells, then found four optional extension cells answered through generic `topology_at` or `object_set` plans while their exact functions were unavailable. Dedicated release/evidence-health and inventory-impact output families now reject generic substitutes. A fresh strict and seeded run is still required for validation. |
