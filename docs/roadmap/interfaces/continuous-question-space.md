...
| Structured investigation assurance | implemented | `semantic_investigation.py`; `semantic_investigation_planning.py`; investigation query-node and presentation tests; semantic frame prompt v28 | Target-bound diagnostic causal cases add exact entity resolution, aligned evidence windows, ordered relationship paths, symptom direction, competing support and refutation, and deterministic presentation without multiplying unrelated declarations. Generic principal-scope causal cases retain the existing bounded plan. |
