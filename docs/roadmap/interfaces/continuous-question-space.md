...
| Current live certification | in-progress | [Issue #233](https://github.com/dotnetpower/fdai/issues/233); exact-source run `question-space-final-b997de285-20260820-r12`; `current change`; focused shared-contract, semantic planning, and prompt tests | Run r12 passed strict v2 and completed all 100 seeded turns, but one Korean health-grouping question was answered through a filtered ObjectSet and failed exact capability matching. The shared semantic contract now carries an additive `aggregate` operation, frame validation requires it to match `aggregation_table`, and an explicit count or grouping request rejects any nonaggregation frame before plan work. Frame prompt v26 preserves the same contract. Fresh exact-source strict and seeded evidence is still required for validation. |
