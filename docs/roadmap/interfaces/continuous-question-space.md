...
| Current live certification | in-progress | [Issue #233](https://github.com/dotnetpower/fdai/issues/233); `current change`; 164 focused semantic planning, composition, declaration query, processor, and round-trip tests | An authenticated exact-source strict v2 run executed all 22 cells and found four declaration cells planned as `query.manifest` or `object_set`. The new exact `ontology_declaration` output family and intent verifier resolve the source defect. A fresh strict and seeded run is still required for validation. |
