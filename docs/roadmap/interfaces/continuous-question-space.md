...
| Structured investigation assurance | implemented | `semantic_investigation.py`; `semantic_investigation_planning.py`; `semantic_turn_presentation.py`; investigation query-node and bilingual presentation tests; semantic frame prompt v30 | Target-bound diagnostic causal cases require a non-null structured intent, exact entity resolution, aligned evidence windows, ordered relationship paths, symptom direction, competing support and refutation, and deterministic presentation without multiplying unrelated declarations. The terminal artifact preserves target, symptom, windows, measured change, hypothesis evidence, and limitations. Generic principal-scope causal cases retain the existing bounded plan. A throttled fallback candidate is attempted once and returns typed frame unavailability without same-candidate delay. |
...
- [ ] Retain authenticated resource-filter and structured slowdown answers from the exact integrated
  source, including verified query evidence, explicit limitations, and `execution_authority=false`.
