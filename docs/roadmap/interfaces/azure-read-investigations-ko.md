...
> **범위:** 이 설계는 리소스 조회, 활동 로그 귀속, Resource Health, 게스트 로그 대체 경로,
> 구성된 NSG 룰, VNet 피어링 topology, 실행시간 예측, 진행 상황 전달 및 detached 조사
...
| Saga | 질문이 FDAI 액션에 관한 경우 FDAI 감사 체인에서 답합니다. | 상관관계 없이 Azure 활동 로그를 FDAI 감사 근거로 취급하지 않습니다. |
| Thor | 기존 `ActionRun` 상태를 보고하고 승인된 타입이 지정된 액션을 실행합니다. | 인벤토리, 활동 로그, Resource Health 또는 guest-log 읽기를 실행하지 않습니다. |
...
| 플래너 의도 커버리지 | 구현됨 | 하나의 변경할 수 없는 런타임 의도 spec이 계획 ID, 기본값 및 interactive 도구, 조회 구간을 소유합니다. Enum 공백은 가져오기 및 exhaustive 테스트에서 실패하고 카탈로그 plan-ID drift는 시작에서 차단됩니다. |
| Resource-state 온톨로지 그림자 비교 | Foundation만 구현됨, 연결되지 않음 | Side-effect-free 비교기는 exact 검토된 `QueryProfile`과 `VerifiedSemanticPlan`을 요구하고 정본 요청 및 증적 계보를 다시 검증합니다. 하나의 trusted 기준 시점과 의도의 300초 최신성 상한을 양쪽 관측에 적용하고 범위가 제한된 변경할 수 없는 비교 근거를 기록합니다. 기존 읽기 결과만 최종 답변 권한으로 유지됩니다. 경로, 조립 또는 런타임 연결은 없으며 read-investigation 서비스 추출 전에는 연결하지 않습니다. |
| 대화형 리소스 연속성 | 구현됨 | Command Deck은 서버가 선택한 인벤토리 리소스 하나를 최종 턴 사이에 유지합니다. Resource Health 이력은 리소스 그룹, 시각 및 상태로 구성된 완전한 anomalous-event anchor 하나도 유지할 수 있습니다. 생략된 이력 및 장애 직전 후속 질문은 semantic 및 공개 웹 계획 수립을 우회하고, Heimdall이 범위가 제한된 맥락을 다시 검증한 뒤 일치하는 읽기 근거를 직접 반환합니다. |
| 구독 범위 신원 | 구현됨 | 현재 구독 신원 질문은 서버에 구성된된 구독 이름과 상태를 Azure Resource Manager에서 읽고, masked 구독 ID만 렌더링하며, 서술기 모델을 호출하지 않습니다. |
| 구독 상태 일괄 점검 | 구현됨 | 명시적인 구독 점검, 일반적인 service-outage 질문 및 일반적인 degraded 또는 사용 불가 resource-state 질문이 구성된 읽기 담당 범위를 사용합니다. 인벤토리 언어 카탈로그가 가용성 의미에 대해 Resource Health 권한을 선택합니다. 프로바이더는 구성된 resource-group 허용 목록을 기본으로 사용합니다. 명시적인 서버가 소유한 구독 모드는 interactive 로컬 상태 범위를 구독 인벤토리와 맞춥니다. Platform-impact 읽기는 활성 서비스 Health 이벤트와 impacted 리소스를 조회하고 장애를 maintenance 및 참고용과 분리한 다음 Resource Health cause와 correlate합니다. 다른 diagnosis 읽기는 최대 16개 supported 리소스의 대표 메트릭을 동시성 4 이하로 확인할 수 있습니다. |
| Azure 근거 어댑터 | 구현됨 | REST는 상태, 활동 로그, Resource Health, 게스트 로그, 구성된 NSG 룰 및 VNet 피어링 속성을 지원합니다. Interactive 로컬은 실행기 신원을 받지 않고 등록된 개발 operations 게이트웨이를 통해 NSG 및 피어링 읽기를 전달할 수 있습니다. 타입이 지정된 CLI 대체 경로는 등록된 계획으로 리소스, VM 상태, 활동 로그를 지원합니다. |
| 선택적 Azure MCP 읽기 | 구현됨 | 공식 MCP Python SDK가 고정된 Azure MCP 서버를 stdio로 시작하고 트래픽 전에 이름 공간 허용 목록을 탐색합니다. VM 상태, 활동 로그, Resource Health에 사용하며 사용 불가 상태이거나 circuit 차단기에서 차단되면 타입이 지정된 REST로 즉시 대체 경로합니다. |
| Read-tool attenuation | 구현됨 | `background.read-only`는 읽기 담당 도구 7개만 포함하고 변경, 승인, shell, arbitrary-query, nested-worker 기능을 차단합니다. |
...
- **`network_security`**: 구성된 NSG 룰과 subnet 또는 NIC association을 반환합니다.
...
`read-only`, `customer-initiated`, `platform-initiated`와 같은 근거 및 cause 한정어는
리소스 선택자가 아닙니다. 이러한 용어가 포함된 collection 질문은 서로 다른 identifier-like
리소스 이름 하나도 함께 포함하지 않는 한 collection 읽기를 유지합니다.
...
범위와 권한 cause 중 하나를 선택하도록 하지 않습니다.

Resource Health 이력이 degraded, 사용 불가 또는 알 수 없음 가용성 이벤트가 있는 리소스
하나를 선택하면 최종 맥락에 해당 이벤트의 리소스 그룹, 시각 및 상태도 포함할 수
있습니다. 세 필드는 모두 있거나 모두 없는 범위가 제한된 인시던트 anchor로만 수락됩니다. 장애 직전 후속
질문은 server-configured 범위에서 최대 24시간과 활동 로그 이벤트 200개를 읽고, anchor 이전의 같은
...
반환합니다. 완전한 anomalous-event anchor가 없는 후속 질문은 활동 로그나 서술기를 호출하지 않고
최종 검증되지 않은 근거 공백을 반환합니다. 잘림은 명시하며 답변에는 최대 20개 matching
이벤트만 포함합니다. Anchor가 있는 모든 답변은 analysis-window 시작, 범위가 제한된 간격 및 인시던트
anchor를 명시적인 타임라인 기준점으로 표시합니다.

Collection 질문은 별도의 타입이 지정된 활동 조회를 사용합니다. 서버는 Azure 구독 및
resource-group 허용 목록을 고정하고 조회 구간을 최대 30일, 반환 이벤트를 최대 200개로 제한하며 이벤트
시간, 정규화된 연산/상태, 리소스 이름, 리소스 타입 및 리소스 그룹만 변환 결과합니다.
호출자 신원과 raw 리소스 ID는 collection 답변에 들어가지 않습니다. 프로바이더는 neutral 타입을
...
수락된 모든 현재 또는 활동 collection은 출처, 결과 종류, 최대 8개 조건식 및 선택적
...
않은 modifier는 전체 리소스로 확장하지 않고 abstain합니다. Semantic 플래너는 결정론적 abstain
...
이름, 유형, 상태, 근거 또는 대표 리소스 하나만 요청하더라도 semantic 계획 수립 전에 fresh
...
임베딩 해석기는 exact 용어로 조회를 완성할 수 없을 때 해당 semantic 표면을 검색합니다.
...
verb는 공개 웹 근거를 선택하지 않습니다. Operator가 해당 medium이나 다른 명시적인 web
...
Resource-state 그림자 비교는 프로바이더 관측 하나를 exact 온톨로지 객체, release, 출처 개정 번호, 기준 시점에 바인딩한 뒤 읽기 전용 일치, mismatch 또는 사용 불가 증적을 기록합니다.
...
상태 탐색은 application-to-database 근거가 아닙니다. 브라우저와 모델은 URL, host, 구독,
...
service-outage 질문 또는 카탈로그 의미가 Resource Health를 요구하는 일반적인 리소스 collection
...
7. 서비스 Health 이벤트, Resource Health cause와 이력, 실패한 프로비저닝 및 메트릭 후보를
...
Broad CPU spike 질문도 semantic 또는 화면 interpretation 전에 이 서버가 소유한 메트릭 경로를
...
잘림 한계를 유지합니다.
Before/after 메트릭 비교에는 검증된 인시던트 anchor 하나와 별도로 범위가 제한된된 구간 두 개가
필요합니다. Anchor가 없으면 결정론적 도구는 point-in-time 메트릭 일괄 점검을 실행하거나 저장소,
...
대체하지 않습니다.
명시적인 상태 collection의 최종 답변은 근거 있는 빈 그룹을 포함하여 요청된 모든 카탈로그
상태를 요청 순서로 렌더링하고, 정규화된 상태가 해당 그룹에 속하는 발견 사항만 나열합니다.
구체적인 계열 조회는 카탈로그의 프로바이더 타입, Azure 종류 토큰 및 requested 가용성 상태로
`Resources`와 `HealthResources`를 prefilter합니다. 종류 토큰은 Web App과 함수 App처럼 하나의
ARM 타입을 공유하는 semantic 타입을 분리합니다. 질문에 CPU, 기억 또는 처리량 같은 diagnosis
...
카탈로그 상태에 속할 때만 발견 사항이 되므로 not-running collection은 모든 관찰된 상태를
...
검증은 결정론적하게 유지합니다. 선택적 presentation-only mini 모델은 근거 collection
...
1. **현재 상태:** Resource Graph 또는 인벤토리가 VM을 해석하고 instance 화면이 `running`,
  `stopped` 또는 `deallocated`를 확인합니다.
2. **Control-plane 행위자:** 활동 로그는 기록이 있는 경우 성공한 Stop, Power Off 또는 Deallocate
 연산과 호출자를 식별합니다. Conversational 귀속 경로는 exact 해석과 활동
 로그만 기본으로 사용하며, 게스트 종료 및 platform-cause 근거는 별도 의도 또는 명시적
 deep 조사에서 추가합니다.
3. **최신 control-plane 변경:** 활동 로그는 종류와 관계없이 가장 최신 successful 연산을
 선택하고 연산, 시간, 행위자 종류 및 opaque 행위자 참조를 반환합니다. 더 최신 시작 또는
 갱신이 있으면 이전 stop-only 귀속을 재사용하지 않습니다.
4. **장애 직전 control-plane 변경:** 완전한 Resource Health 인시던트 anchor는 해당 이벤트 이전의
...
5. **게스트 종료:** Control-plane 연산이 없는 `stopped` VM은 Windows Event 로그 또는 Linux
  ...
  반환합니다.
6. **Platform 이벤트:** Resource Health는 host, maintenance 또는 platform 가용성 맥락을
 제공합니다. ARG 이력이 비어 있으면 current-status 대체 경로의 관측 시각이 요청한
 조회 구간 안에 있을 때만 근거로 사용합니다. 사용자가 이벤트를 시작했다는 사실을 증명하지는
 않습니다.

활동 로그 miss는 누구도 VM을 중지하지 않았음을 증명하지 않습니다. 보존, 인제스트 delay,
...
| `streamed` | 4초 초과 15초 이하 | Chat 스트림을 열어 두고 범위가 제한된 semantic 진행 상황을 보냅니다. |
...
화면 상태, 변경 가능한 기억, shell, 실행기 신원 또는 변경 도구를 받지 않습니다.
...
반환합니다. 초기 streamed 상한은 20초이므로 cold exact 활동 로그 귀속 추정치가 열림 chat
...
개수, 잘림, 캐시 상태, 기록된 시간 및 trace 참조가 있는 `ToolCallReceipt`를 내보냅니다.
...
연산을 `input_kind=query`로 표시하고 안전한 상태/개수 요약을 제공합니다. Shell exit
...
완료될 때 `activity` 이벤트를 보내고, 리소스 해석과 근거 collection이 운영자 경험을
...
> 현재 VM 상태와 최근 Azure 활동 로그를 확인하겠습니다. 측정된 프로바이더 지연 시간을 기준으로 보통
> 10-20초 정도 걸립니다.
...
TLS와 프로세스 경로 필드, telemetry 선호 설정만 받습니다. 데이터베이스 URL, webhook 및 다른 애플리케이션
...
고정된 Azure MCP 패키지에는 glibc-linked .NET executable이 포함되며 musl wheel 또는 출처
분포는 제공되지 않습니다. 따라서 런타임 이미지는 digest-pinned Python Debian slim을
사용하고 ICU를 설치하며, .NET bundle 추출과 user 캐시를 위한 writable nonroot 위치를
...
- **프로바이더 throttling:** ARG 요청은 할당량이 0이면 `x-ms-user-quota-resets-after`만큼 기다리는
 shared gate를 사용합니다. Numeric `Retry-After` 또는 범위가 제한된 지터는 시간 초과와 범위 안에 있습니다.
- **보존 부족:** 요청한 조회 구간이 source-specific 구성된 보존을 넘으면 cloud I/O 전에
 `unavailable`을 반환합니다. 활동 로그는 기본 90일, 게스트 로그는 기본 30일이며 배포는 실제
...
- **취소:** Pending 프로바이더 작업을 중지하고 `cancelled`를 커밋하며 이미 작성된 completed
 근거 참조를 유지합니다.
- **근거의 프롬프트 injection:** 프로바이더 문자열을 데이터로 취급하고 도구, 범위, 권한 확인 또는
 실행 모드를 변경하려는 출력을 차단합니다.

## 구현 순서 및 release gate

1. 프로바이더 중립적인 계약, 타입이 지정된 도구, 정규화된 근거 및 bilingual 라우팅이 구현되었습니다.
2. Direct, streamed, detached 실행, 영속 증적 및 지연 시간 프로파일, 할당량, semantic 진행 상황,
...
- Property 테스트가 모든 조사 도구가 읽기 전용이고 attenuation이 변경, 승인, shell,
...
- 실제 운영 Azure 검사가 리소스 변경 없이 활동 로그 호출자 귀속, Resource Health 대체 경로,
...
| 알아볼 내용 | 문서 |
|-------------|------|
| Operator 도구 및 chat tier | [Operator Console](operator-console-ko.md) |
