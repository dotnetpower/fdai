...
| Package 7: handover goal core and commands | implemented | `goals.py`; `handover_runtime.py`; `handover.py`; focused Core, Operator, and Console checks | Durable invitations include live ownership revalidation, weekly fatigue fencing, server-bound agent routing, snooze, decline, and fail-closed suppression while incident or approval work is active. Agent-authored gap production remains open. |
| Package 8: knowledge evidence delivery | in-progress | document contracts; `console/src/routes/document-ingestion.tsx`; Operator handover goal evidence state; focused Console and Operator checks | Governed web upload associates an admitted document receipt with the goal. Later access revalidates the receipt and marks the goal stale after deletion, revocation, or replacement. Candidate delivery and ACL-filtered agent retrieval remain open. |
...
- [ ] Complete agent-owned handover gap production, candidate delivery, and ACL-filtered agent retrieval; conflict fencing and staleness/deletion propagation now have focused passing evidence.
