...
|------|-------|----------|-------|
| Minimum Azure inventory model | in-progress | `infra/`; [Current Terraform inventory reconciliation](#current-terraform-inventory-reconciliation) | Terraform declares the deployable inventory, but the historical price rows are not generated from the current plan. |
...
| Current SKU and quantity reconciliation | not-started | [Per-resource estimate](#per-resource-estimate) | Rows still say `recalculate from current plan`; no checked-in plan-to-cost reconciliation artifact exists. |
| Price confirmation and deployment baseline | not-started | [Open decisions](#open-decisions) | No governed `pricing.confirmed_at`, Retail Prices or Calculator receipt, measured billing baseline, or variance alert evidence is retained. |
