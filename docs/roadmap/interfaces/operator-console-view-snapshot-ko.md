...
translation_source_sha: b17b0cbf41ffbba78167e30485db7cf1719de94f
...
| 답변 계획 및 qualification | implemented | `services/core-control-plane/src/fdai/core/conversation/answer_plan.py`; `answer_planning.py`; `answer_planning_qualification.py`; `scripts/evaluation/answer-planning-qualification.py`; 집중 conversation 및 CLI 테스트 | 범위가 제한된 계획, shadow 레코드, 엄격한 측정 배치 수집 및 readiness-only 증적이 activation 권한 없이 존재합니다. CLI는 측정값을 생성하거나 planning을 활성화할 수 없습니다. |
| Shadow contributor 수집 | in-progress | Answer-planning source 및 최종 메타데이터 경로; [`test_answer_planning_conflicts.py`](../../../services/core-control-plane/tests/core/conversation/test_answer_planning_conflicts.py) | Phase C shadow 레코드가 있고, 보존된 Phase E conflict 사례가 상충하는 contributor가 두 근거 집합을 보존하고 승자를 고르지 않으며 기본 agent를 바꾸지 못하고 권한 필드를 갖지 않음을 입증합니다. Phase D 선택적 activation은 승격되지 않았습니다. |
...
| 2026-08-28 | implemented | 권위 있는 발급자에서 선택 토큰 경계를 바로잡았습니다. 이제 Operator 변환 결과는 선택 다이제스트를 계산하고 불투명 토큰을 발급하기 전에 공유된 선택 불가 Resource 유형 집합을 사용하므로, 토큰을 해소해도 브라우저가 로컬에서 필터링한 숨겨진 `authorization.role-assignment` ID를 복원할 수 없습니다. | `current change`; 집중 Operator 인벤토리 인스턴스 변환 결과 검사(`8 passed`); Ruff 및 strict mypy | 승격 전에 거버넌스가 적용된 화면 간 근거를 보존합니다. 실제 서비스는 조회하지 않았습니다. |

### 남은 작업

- [ ] 표시된 snapshot 다이제스트, 서버 근거, branch 수명 주기, 최종 검증, stale 전이 및 경로 이동을 묶는 인증된 화면 간 증적 하나를 보존합니다.
- [ ] 실제 sealed 100-case bilingual answer-planning batch를 제공하고
  `scripts/evaluation/answer-planning-qualification.py --require-ready`로 실행한 뒤, 지원하지
  않는 단정 및 권한 escape가 0건인 변경할 수 없는 증적과 별도의 선택적 activation 검토
  결과를 보존합니다.
- [x] 상충하는 contributor가 두 근거 집합을 보존하고 기본 검증 답변을 바꾸거나 권한을 부여할 수 없음을 입증하는 Phase E conflict 사례를 추가하고 보존합니다.
