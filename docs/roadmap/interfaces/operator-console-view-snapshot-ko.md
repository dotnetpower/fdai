...
> [operator-console-ko.md](operator-console-ko.md) section 13.4에서 분리한 focused 소유자 문서입니다.
...
- `constraints`: 한도, 선행 조건, 안전성 경계 및 연산을 사용할 수 없는
 이유.
- Domain 기록 collection: 조회와 causal explanation에 필요한 실제 visible 행.

경로는 이 계약을 `*.view.ts`에 위임할 수 있습니다. 선택적 `explanations` 묶음은
selection, relationships, 수명 주기 기준, deduplication, 출처 이력을 표준화하며 메타데이터가
...
`Issue` 또는 문제라는 domain 명사만 있으면 current-screen 참조로 유지합니다. 최근성, 인시던트, outage,
실패 또는 cause 표현이 명시되면 서버가 소유한 `ConsoleReadModel`의
...
근거에 기반한이고 cause와 인용이 모두 있는 RCA 가설만 포함. Bragi는
abstained 또는 인용 없는 가설에서 인시던트 cause를 단정하면 안 됨.
...
2. Azure 리소스, KPI, pending 승인, 감사, 인시던트 목록 질문에는 서버가 소유한
 ...
 사용하지만 모델 synthesis 전에 결정론적 `read-model-health` 경로를
 사용합니다. 답변은 관측된 event 샘플, 승인 적체, execution-mode mix,
 근거 시간을 보고하며 모든 component가 healthy라고 추론하지 않습니다.
3. agent-owned domain에는 `PantheonChatDelegate`를 사용합니다. Bragi는 기본
...
`_tool_evidence`, `_agent_evidence`를 제거합니다. 브라우저는 chat 상태, JSON,
...
텍스트가 준비되고 최소 420 ms가 지날 때까지 수집 trace를 유지한 다음, 같은
...
버퍼를 한 번에 표시하지 않습니다. Terminal-only 답변은 토큰을 최대 30개 조각으로 묶어 visual 노출을 약 300 ms로 제한하며 서버 완료 시각이 실행 timing의 기준입니다. 답변이 처음 표시될 때와 최종 개정 번호가
렌더링될 때 transcript는 preparation 중 운영자가 위로 scroll했더라도 최신
내용으로 이동합니다. 완료된 회신은 매니페스트 항목을 독립 출처가 아니라
근거 참조로 표시합니다. 지원하지 않는 문장을 제거하고 재검증을 통과한
범위가 제한된 correction은 검증된 visual treatment를 사용합니다.

회신 렌더러는 ATX heading, emphasis, strong 텍스트, strikethrough,
unordered/ordered 목록, 읽기 전용 작업 목록, blockquote, thematic break, 안전한
`http` / `https` / relative link, 표, fenced 코드 및 chart 블록을 지원합니다.
...
control은 같은 대화를 유지하면서 이동 가능한 floating 패널 또는 full
workspace로 전환합니다. Floating 헤더 title을 drag해 패널을 이동합니다. 왼쪽과
상단에는 12 px guard를 유지하고 오른쪽과 하단은 viewport 밖으로 이동할 수
있습니다. Sidebar의 왼쪽 구분자를 pointer 또는 arrow 키로 조작해 340-720 px
범위에서 resize할 수 있습니다. Right-sidebar 모드는 shell 본문 폭을 현재 sidebar
...
형태를 복원합니다. 간결한 mobile viewport는 저장된 선호 설정을 바꾸지 않고
...
`primary_agent`는 위임 및 trace 메타데이터에 보존됩니다. 관측 소유권은
보이지만 specialist를 human 대화의 소유자로 표시하지 않습니다.

모든 event는 단조 증가 `seq`를 가지며 답변을 바꾸는 event는 단조 증가
`revision`도 가짐. 클라이언트는 stale 개정 번호와 최종 event 이후 event를 무시.
...
범위, RCA cause 또는 absence 점유를 바꿀 수 없음. `none`, `ambiguous`,
...
정본 검증된 cause로 교체 MAY. Screen-only 답변은 `consistent`로 종료.
...
Fact는 localized synonym을 범위가 제한된 `aliases`로 publish할 수 있습니다. 중복 값은 가장 가까운 `label` 또는 별칭에 연결하며 일치하지 않으면 모호한으로 유지합니다. 이 correction은 rewrite 전후에 supported 점유가 하나 이상 있어야 합니다. `0/N` 결과, 잘린 스냅샷 또는 추출 초과분은 계속 abstention으로 종료됩니다.

지연 시간 대상은 요청 admission 후 첫 진행 상황 event 100 ms 이내, 일반 모델
TTFT p95 2.5초 이내, 근거 조회 완료 후 fast-path 최종 답변 p95 500 ms
이내, provisional 완료 후 첫 검증 event 100 ms 이내,
provisional-to-terminal 검증 p95 1초 이내. 진행 상황은 실제 완료 검사를
보고하며 가짜 비율을 사용하지 않음.
Incremental SSE delta는 클라이언트 측 delay 없이 렌더링됩니다. 큰 single 프레임 또는
같은 tick의 큐 burst만 paint-sized 조각과 짧은 cosmetic cadence로 묶습니다.
...
근거를 먼저 다루고 스냅샷에 있는 경우에만 control, 제약 또는 안전성
...
bounded-scope 점유를 인식하며, 각 점유는 출처 span, 정규화된 값, support
상태, 정확한 스냅샷 근거 참조와 matching에 사용된 fact 별칭을 hashed 근거 항목에 기록합니다. `evidence_manifest`는 경로,
...
모델과 토크나이저 footprint는 `124498008` 바이트. Unknown 결과는 benefit으로 계산하지
...
권위가 유지되고 변경되지 않음. 향후 제안이 material contradiction benefit을 측정해
...
계획은 의도별 section, 범위가 제한된 word 대상, format, 근거 요구사항을 제공합니다. 서버가
소유한 스냅샷 메타데이터로 주입되고 JSON과 SSE 최종 응답에 모두 반환되며 transcript에
...
Phase B는 기존 `UserPreferenceStore` 경계를 통해 명시적이고 principal 범위인 응답 선호 설정
...
conversational 포트를 병렬 호출합니다. 기여자는 근거에 기반한 fact, 보증된 근거 참조, 추천
section, caveat, 확신도가 포함된 타입이 지정된 `AnswerContribution` 기록을 반환합니다. 운영
pantheon 어댑터는 routine collection에서 Bragi, Norns, Odin을 제외합니다. Saga는 감사, 이력,
issue 또는 인계 질문에만 참여합니다. 액션 형태의 요청은 기존 typed-pipeline guard를 통해
abstain합니다.
...
degraded 메타데이터가 됩니다. 지원 가능한 답변을 차단하거나 변경하지 않습니다. Phase C에서는
기여자 fact가 서술기 스냅샷에 들어가지 않으므로 primary-only 답변이 최종 답변으로
유지됩니다.
...
- **관측 출처는 기록하며 추론하지 않습니다.** 실제 운영과 에이전트 활동 프레임은
 ...
 `runtime-observed`는 생산자 경로를 설명할 뿐 Azure 상태 또는 실행 attestation이
 아닙니다.
