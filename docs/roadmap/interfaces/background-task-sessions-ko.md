...
translation_source_sha: e3ca27b5a3cbd53d3f2392a1cbc1dd760a8db0df
...
| 완료 싱크 및 영속 Web 대화 인계 | implemented | Core EventBus 완료 싱크, `read-investigation-completion` `1.0.0`, Operator 완료 저장소, consumer, 대화 writer, 보존 worker 및 migration grant, 집중 완료 및 권한 검사(`24 passed`) | Core는 기존 임차 발신함에서 변경할 수 없는 최종 결과 하나를 publish합니다. Operator 운영 조립은 exact 제안에 결속된 inbox 레코드를 원자적으로 수락하고 멱등적인 Web assistant turn 하나를 쓰며 기한이 지난 inbox 행을 정리합니다. 채널 outbound enqueue 및 통제된 재시작 근거는 열린 상태입니다. |
...
| 감사, 원격 분석 및 운영 근거 | in-progress | `services/core-control-plane/src/fdai/core/background_task/service.py`; `services/core-control-plane/src/fdai/delivery/persistence/background_task_lifecycle_audit.py`; `services/core-control-plane/src/fdai/delivery/persistence/background_task_completion_audit.py` | 운영 생성, 취소 및 완료는 멱등적인 lifecycle 표식을 사용하며 완료 싱크도 조립되어 있습니다. 런타임 원격 분석과 통제된 retry, 재시작 및 채널 전달 증적은 열린 상태입니다. |
...
| 2026-09-12 | implemented | 현재 운영 binding에 맞춰 조립된 versioned 완료 handoff, 멱등적 Web turn 및 deadline 순서 inbox 보존을 정합화했습니다. | `current change`; 완료 transport, Operator runtime/store 및 writer-grant test(`24 passed`, skip 없음). | 검증된 채널 outbound enqueue를 추가하고 통제된 retry, 재시작 및 telemetry 근거를 보존합니다. |
...
- [x] 범위가 제한된 Operator 소유 worker와 inbox 전용 delete grant를 통해 계약 기한에
  보존된 완료 inbox 행을 정리합니다(`24 passed`, skip 없음).
- [ ] 채널 binding 해석과 영속 outbound enqueue를 추가한 뒤 통제된 재시도 및 재시작 근거를
  보존합니다.
