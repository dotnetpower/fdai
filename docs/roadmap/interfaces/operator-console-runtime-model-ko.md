...
> [operator-console-ko.md](operator-console-ko.md) 섹션 1.2 및 4-6에서 분리한 focused 소유자 문서입니다.
...
개정 번호, 마지막 행위자와 갱신 시간, 검증 충돌을 표시합니다. Stale 개정 번호는 충돌을
반환하며 운영자가 재시도 전에 최신 상태를 검토하도록 합니다.

## 4. Narrator - LLM 계층 모델
...
특정 계정 이름에 고정되지 않고 `resolved-models.json`과 환경 조립에서 선택됩니다.

### 4.1 세 계층 (trust 라우터를 반영)
...
공개 웹 의도도 같은 계층 형태를 사용합니다. T0는 high-confidence explicit-search 및 local-scope
pattern을 유지합니다. 대상 턴이 `none`으로 남으면 Azure Responses 후보가 전용 system 프롬프트와
strict JSON 스키마를 사용해 경로, 분류 확신도, 사유 코드 및 범위가 제한된 English 검색
조회를 반환합니다. 대안 발견은 목표, 비교 대상 및 2-8개 기능도 반환하며,
조정기가 해당 기능에서 실제 조회를 다시 구성합니다. 현재 화면 스냅샷 또는 이력은
받지 않습니다. 대안 수집은 direct product 페이지만 수락하며 medium 검색 맥락으로 filtering 전
서로 다른 product를 최소 3개 요청합니다. Self 참조, 범용 homepage, conceptual 지침, editorial 또는
blog 페이지, documentation 인덱스 및 중복 product 신원은 근거가 Bragi에 도달하기 전에 제거합니다.
잘못된, low-confidence 또는 사용 불가 출력은 `none`을 유지하며
로컬 또는 sensitive-data denial을 재정의할 수 없습니다. 대안이 아닌 목표에서는 라우팅에
...
- **RBAC 게이트를 우회.** 조정기는 서술기를 호출하기 **전에** 하한을
...
- **tool-인자 내용을 명령으로 취급.** 오퍼레이터-공급 인자 값 (하나의
  ...
  가 권위. 민감정보 제거 (action-ontology §5.2)은 시크릿을 strip; 주입 방어
  가 아니다 - 검증기 재확인이 방어.

### 4.4 비용과 비율 한도

D12에 따라: mini (t1.판정자)는 항상 켜져 있고 오퍼레이터 예산 가정은
이것이 normal-cost 표면 라는 것. 업스트림 기본은 **넘치지만-유한한**
턴 당 토큰 예산과 세션 당 홉 상한 (구성 키
`console.max_completion_tokens_per_turn`, 기본 4096, 그리고
`console.max_tool_hops_per_turn`, 기본 8)을 ship - 비용 거버넌스 버티컬
이 지출을 단속하는 제품이 자신의 콘솔을 무계 LLM 표면으로 ship 할 수
없음. 기본에 사용자당 *비율* 한도는 없음; 포크는 구성으로 추가 MAY.
측정된 각 LLM 호출은 계층, 모델 배포 id, 워크로드 범위,
...
게이트에서 설정된 가격표를 계속 사용할 수 있습니다. 헤드리스 코어와 Operator API는
...
- Core 서술기는 RBAC로 보이는 도구 스키마만 받아 정본 동사 줄 또는 abstention을
...
- `InboundTurn`은 조정기가 보기 전에 범위가 제한된 채널, 메시지, 발신자, 스레드, 텍스트 필드를
  검증합니다. `ConversationChannelGateway`는 해결되지 않은 발신자를 차단하고 도구 실행 전에 중복
  메시지 id를 제거합니다.
- Push-방향 어댑터
...
서버는 프롬프트 표현이 아니라 실제 작업을 기준으로 필요한 최소 표현을 선택합니다.
...
Web은 현재 활동 그룹과 completed 활동 그룹의 셸을 계속 표시합니다. 이정표 또는 최종 프레임은 앞선 그룹을 settled 상태로
...
- `Turn` = `{turn_id, 역할, 내용, tool_calls?, tool_results?, 계층,
  audit_entry_id}`.
...
- **변환 결과 일관성 경계**: 선호 설정, 기억, 정책 및 briefing 구독
  ...
  5회 실패한 작업은 무기한 재시도하지 않고 운영자 진단용 dead-letter로
  이동합니다. 온톨로지 변환 결과는 출처 기록에서 재구성할 수 있습니다.
- **선제적 동작**: 허용 목록된 `ConversationPolicy` 기록만 고정 서술기 프롬프트
  조각으로 compile한다. Opening briefing과 scheduled briefing은 결정적
  `BriefingSpec`을 공유하며, 영속 구독은 IANA 표준 시간대를 사용하고
  근거에 기반한 `BriefingRun`을 소유 principal별로 저장한다.
- **Web 대화 탐색**: Console SPA는 대화 목록과 **새 대화** 컨트롤을
  표시. 목록은 분리된 대화 기록 캐시를 가리키는 principal 범위로 한정된
  `localStorage` 인덱스이므로 스레드 전환, 탭 reload 또는 브라우저 재실행 시 완료된
  턴을 복원하면서 agent-scoped 대화와 일반 대화를 섞지 않음. Persistent 브라우저
  저장소가 차단된 환경에서는 `sessionStorage`로 대체 경로합니다.
  Operator는 로드된 대화 기록을 검색하고 일치하는 턴 사이를 이동할
  수 있음. 기본 대화는 비식별 user 해시와 정규화된 URL 경로 이름별로
  분리. query-only 필터 변경은 같은 경로 이름 세션을 재사용하고, 다른
  메뉴 또는 분석 상세 URL은 자체 대화 기록을 시작하거나 복원. 기본
  ...
  대화를 만들고 첫 턴 전에 해당 에이전트를 대상으로 저장합니다. 이전 에이전트 대화 기록을
  ...
  메타데이터를 최대 1,000건까지 브라우저 인덱스에 병합하며 대화 기록 본문은 선택할 때만 로드합니다. 이전 random id를 사용하는 이전 방식 대화도
  ...
  unpaired surrogate 코드 지점, 허용되지 않은 C0/C1 컨트롤 및 bidirectional 재정의 또는
  isolate 컨트롤을 차단합니다. 경로는 malformed 텍스트를 저장하지 않고 localized 검증되지 않은
  답변을 반환합니다. Newline, 탭 및 script-shaping zero-width 결합기는 계속 허용합니다.
  ...
  자리 표시자로 마스킹합니다. 검토자는 초안을 통과하거나 narrator-authored 산문을 rewrite하거나
  ...
  변경하지 않음. **다른 화면**의 스레드를 선택하면 대화 기록을 복원하기 전에
...
- Narrator의 raw 세대 추적, per-token 로그, 또는 오퍼레이터 프롬프트
...
세션 대화 기록은 **기억 of 기록**다. 모든 턴은 보존 정책이
...
JSON 및 SSE 채팅 경로는 후속 조치 계획 수립 전에 인증된 `(principal_id, conversation_id)`
쌍으로 영속 저장소에서 이력을 해석합니다. 저장소가 구성된 경우 클라이언트가 제공한 이력은
사용하지 않습니다. 해석기는 턴 수 제한 없이 전체 대화 기록을 읽고, 이력이 기본
...
history-compaction 조각은 범위가 제한된 깊이 및 탐색 예산 안에서 분할하여 원인 턴만 모델
맥락에서 제외하며 영속 대화 기록 행은 변경하지 않습니다. 프롬프트에는 내용이 없는 omission
...
- **타입이 지정된 사실** - 타입이 지정된 파이프라인에서 변환 결과 된 결정론적 no-LLM
  ...
  서술기 기억이 아니라 감사가능하고 범위 태깅된 trusted 레이어로 (섹션 1).
...
- **Hierarchical 요약** - 나머지 전부를 rolling 요약으로 접음(수준 1
  이 턴을, 수준 2가 level-1 요약을 접음)므로 요약 계층은 세션 길이 `L`에
  대해 `O(log L)`로 성장. 순수
...
상위 우선순위 계층의 미사용 예산은 다음 계층으로 spill 되므로, 짧은 세션은
...
dropped id, 계층별 토큰)를 기록하므로 어떤 프롬프트든 기억 of 기록에서
재구성 가능하다.
...
화면에는 수명 주기 컨트롤이 없습니다. [컨텍스트 선택 정책](../decisioning/context-selection-policy-ko.md)을 참고하세요.

**에이전트도 동일 메커니즘.** 에이전트 conversational 포트 (agent-to-agent
introspection)는 correlation-scoped 대화 기록 위에서 같은 작성기를
...
- [x] Principal 범위 PostgreSQL conversation history, 이미지, 최신 context 및 검색 사례를 건너뛰지 않고 실행하고 complete-history 조립의 재시작 근거를 보존합니다.
