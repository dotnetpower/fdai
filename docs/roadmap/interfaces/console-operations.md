...
| Authenticated semantic receipt evidence harness | implemented | `console/tests/live-e2e/browser-entra-state.ts`; authenticated governed-receipt and bilingual assurance Playwright specs; focused Console typecheck | The harness restores an existing Browser Entra MSAL session into `sessionStorage` before first navigation, consumes the bootstrap once, and leaves principal and App Role validation with the Operator API. Passing retained evidence remains required before readiness. |
