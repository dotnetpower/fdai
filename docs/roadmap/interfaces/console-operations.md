...
Provisioning success and a successful provider request never become operational success. Configuration baselines likewise read only completed Core-owned checks, preserving pinned metadata, verdicts, and measured latency. Unconfigured or blocked Knowledge remains unavailable, not a measured citation count; viewing evidence never activates a baseline or review campaign. Forecast Learning preserves the server's due-cohort closure ratio and scored miss origin; a cohort with no due episodes shows Not measured rather than a blank value or zero.
...
Reference loading or failure never removes the path back to instances. Direction backgrounds fill the complete graph viewport independently from shorter SVG geometry. Directory bounds and refresh state remain compact in the toolbar, while coverage and legend details start collapsed. Its graph-first layout, selected resource and Inspector-owned collapse state are presentation controls only;
...
Human approval decisions enter through two distinct authenticated transports. Teams and Slack keep their signed provider callback and identity-mapping checks. FDAI Console uses `POST /hil/{approval_id}/operator-decision`, derives the Entra human and `approve-runtime-hil` capability on the server, and accepts only the decision plus a bounded justification. Neither transport can supply its own actor, role, action hash, or execution authority.

Both transports reload the exact approval id and original parked identity. The server checks the pending state, expiry, submitter, decision route, and required role while holding the approval fence, then commits the decision receipt and durable outbox atomically.
Missing roles, unknown routes, stale context, and self-approval grant no authority. Request or broker acceptance never means that Thor executed the action. Action parks without metadata normalize to the canonical `action` route defined by the Core registry, while an explicit empty, unknown, or malformed route remains unavailable.
...
|---------|-------------------------------|
| Console alignment | Overview, Operations, Agents, and Governance previews follow the Console registry and current neutral presentation, including separate executive and resource Dashboards. Operations workspaces place tabs and preview controls above metrics and retain adjacent detail where space permits. The [operator checks](../../../console/tests/e2e/operator-workspaces.spec.ts) and [Governance checks](../../../console/tests/e2e/governance-current-mock.spec.ts) cover routes, interactions, and responsive presentation. Observation affinity selects exact resource-group and resource identities from a validated synthetic ontology-instance snapshot, not Azure Resource Graph or provider queries; incomplete sources block selection. These soft preferences do not narrow baseline monitoring or grant authority. Fixtures never submit operational requests; queued examples never apply lifecycle changes or fabricate runtime audit evidence. |
...
use the same schema, authorization, and source binding; fixture principals remain pytest-only. The static localization source inventory follows the current component set and canonical panel labels when a route is replaced, and its visual-boundary checks reject decorative edge emphasis without changing runtime behavior or authority.
