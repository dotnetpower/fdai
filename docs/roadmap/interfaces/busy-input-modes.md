...
An agent-targeted active turn keeps its selected agent while the Operator API waits on the bounded
...
A validated current-turn image attachment bypasses prompt-only semantic tool planning and omitted-subject LLM usage refinement so queued and steered image turns retain vision narration. Terminal verification preserves that interpretation as unverified with a current `conversation-image` ref. An explicit measured LLM usage request remains a deterministic tool request.
...
| FDAI Console and client controls | implemented | `console/src/deck/busy-input-client.ts`; `console/src/deck/busy-input-controls.tsx`; `console/src/deck/busy-input-client.test.ts`; `console/tests/e2e/busy-input-controls.spec.ts`; focused client, typecheck, and mocked Console browser checks | The Deck uses its exact active chat session ID and existing authentication headers for all four routes. Only a confirmed matching GET enables controls or reports a pending disposition. Unavailable and unconfirmed responses retain the draft. The browser check uses synthetic responses, not a production Operator consumer or live conversational cancellation. |
...
| 2026-09-26 | implemented | Added bounded authenticated Console follow-up, inspect, mode, and conversational-cancel controls without changing local Stop, action authority, or the backend consumers. | `current change`; `console/src/deck/busy-input-client.ts`, `busy-input-controls.tsx`, `busy-input-client.test.ts`, `console/tests/e2e/busy-input-controls.spec.ts`; 17 focused client tests, Console typecheck, and 3 synthetic desktop browser scenarios passed. | Connect Core JSON/SSE turns, materialize Operator busy operations, and record governed telemetry and runtime evidence. |
...
- [x] Add FDAI Console client controls for submit, inspect, mode change, and conversational cancellation, with focused interaction and accessibility checks (`busy-input-client.test.ts`; `busy-input-controls.spec.ts` desktop synthetic route).
