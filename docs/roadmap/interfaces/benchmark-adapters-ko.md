...
이 설계는 FDAI 런타임에 특정 벤치마크 package를 추가하지 않고 외부 평가 실행 장치를 FDAI에
연결하는 방법을 정의합니다. 독립 SDK가 neutral 계약과 범위가 제한된 실행기를 소유하고, FDAI는
공개 host와 그 뒤의 통제된 실행을 소유합니다.

> **범위:** 벤치마크 어댑터는 실행 장치 수명 주기와 data를 변환합니다. FDAI 액션을 판단,
> 승인, 승격 또는 실행하지 않습니다.
>
> **구현 상태:** 독립 package SDK, 공개 host 및 세션, 기능 attenuation, 산출물
> 보관, workspace 정책 브로커, SREGym 이행, CyberGym acceptance driver, compatibility
> facade, installed-adapter 발견, 범위가 제한된 Kubernetes 근거, 실행기 준비 상태 검사 및
> 의존성 gate가 구현되었습니다.

## 설계 요약

FDAI wheel에는 SREGym, CyberGym 또는 다른 실행 장치 프로토콜이 포함되지 않습니다. 외부
driver는 `fdai-evaluation-sdk`에 의존하고 공개 `EvaluationHost`를 받은 뒤 범위가 제한된 세션을
시작합니다. Host는 neutral 작업을 타입이 지정된 유입으로 변환하고 결정, risk, 승인, 실행 및
감사를 FDAI 내부에 유지합니다.
...
| 계층 | 위치 | 책임 |
|-------|------|------|
| Evaluation SDK | `evaluation-sdk/` | 변경할 수 없는 요청, 작업, 결과, 대상, 기능, workspace, 산출물, 증적, 어댑터, host 및 실행기 계약입니다. |
| FDAI host | `services/core-control-plane/src/fdai/evaluation/` | 타입이 지정된 유입, 기능 attenuation, workspace 및 산출물 정책, 결과 대응, 정리 및 감사입니다. |
| 실행 장치 driver | `benchmarks/<name>/` | 실행 장치 수명 주기, neutral 작업 대응, 외부 검증, package 의존성 및 테스트입니다. |
| Compatibility facade | `services/core-control-plane/src/fdai/benchmarking/` | 이행 기간의 이전 방식 텍스트 작업/제출, 플러그인, 연결 및 실행기 API입니다. |
...
요구사항을 포함하는 전체 세션 묶음을 선언합니다. `EvaluationTask`는 열림 phase,
objective, 타입이 지정된 대상, 입력 산출물 참조, declared 출력 명세, 기능,
기한, 리소스 한도 및 변경할 수 없는 메타데이터를 전달합니다.

`EvaluationResult`는 세션, 작업 및 phase 신원을 보존합니다. `completed`, `held` 또는
...
`EvaluationRunner`는 작업을 읽기 전에 host 세션을 열고 중복 또는 cross-session
...
기능을 요청합니다. FDAI는 요청, host 허용 목록, 세션 범위, RBAC, 승격 레지스트리,
risk 결정 및 승인 결정의 교집합으로 effective 기능을 계산합니다. Host 카탈로그가
...
Host는 서버가 소유한 정책을 통해 각 neutral 대상 종류를 라우팅 리소스 타입으로 매핑합니다.
SREGym에서는 evaluation 작업이 관련 없는 cluster-governance rule을 재사용하지 않도록
...
구체적인 host는 조립을 통해 타입이 지정된 collaborator를 받고 공개 세션 프로토콜만 반환합니다.
`EvaluationRunner`는 세션을 열기 전에 API 버전이 SDK의 exact 버전과 다른 host를
차단합니다.

산출물 publication은 범위가 제한된 바이트 스트림을 소비하고 내용 기반 주소를 가진 변경할 수 없는 `ArtifactRef`를
...
하며, host는 각 참조를 브로커를 통해 다시 읽어 범위, 만료, 크기 및 다이제스트를 검증합니다.
누락된, 중복, altered 또는 undeclared 출력은 driver 제출 전에 실패 시 차단됩니다.

Workspace 접근은 host 경로 또는 raw 명령 문자열을 노출하지 않습니다. 프로바이더는 task-root
...
- **에이전트 직접 호출 없음:** 공개 host는 타입이 지정된 유입을 통해 publish합니다. Driver는 Pantheon
 에이전트를 직접 가져오기하거나 호출하지 않습니다.
- **숨은 판단 없음:** 어댑터는 단계와 페이로드만 변환합니다. Tier를 선택하거나 결정 또는
 승인을 만들 수 없습니다.
- **권한 증가 없음:** 플러그인 구성은 승격, risk, 역할, 승인 또는 실행 모드를
 변경할 수 없습니다.
- **범위가 제한된 근거:** 외부 메트릭, 로그, trace, 인벤토리, 파일 및 검증 증적은 범위가 제한된
 신뢰할 수 없는 근거로 유지됩니다.
...
| `GET /get_app` | Objective 메타데이터 및 범위가 제한된 Kubernetes 이름 공간 대상입니다. |
...
정규화합니다. 구성 URL의 자격 증명, 조회 문자열 및 fragment는 차단됩니다. 명시적 포트는
...
어댑터는 가장 최근 `next_task()` 호출이 반환한 정확한 세션, 작업 및 phase 신원에 대한
결과만 허용합니다. Conductor가 제출을 수락한 후에만 이 신원을 clear하므로,
전송 계층 실패는 같은 결과를 재시도할 수 있지만 발급되지 않았거나 phase가 다른 제출은
허용되지 않습니다.
이 신원이 outstanding 상태인 동안 다른 `next_task()` 호출은 conductor를 polling하기 전에
실패합니다.

Package는 `fdai_evaluation_sdk`만 가져옵니다. Neutral Kubernetes 및 메트릭 관측 기능을
...
런타임은 벤치마크 package를 가져오기하지 않고 선택된 `EvaluationAdapter` 계약을 load합니다.
SREGym package는 이 그룹에 `sregym`을 등록합니다.
...
Kubernetes 전달 package가 소유하므로
evaluation, 런타임 근거, 용량 및 할당량 진단은 동일한 exact base-unit 의미를 사용합니다.
Pod 인벤토리는 이미지 또는 명령 literal을 보존하지 않고 변경할 수 없는 UID와 집계 CPU/기억
...
보존합니다. 로그 및 trace 기능은 범위가 제한된 프로바이더를 설치할 때까지 advertise하지 않습니다.

공유 Kubernetes package에는 hold-only 엔드포인트 의존성 집약기도 있습니다. 완전한
...
실패한 Kubernetes admission 이벤트는 범위가 제한된 webhook TLS, 시간 초과, 사용 불가 또는 Pod Security
거절 코드로 분류됩니다. 분류에는 실패한 이벤트 사유가 필요합니다. Informational 텍스트,
malformed webhook 신원 및 인식되지 않은 메시지는 분류하지 않습니다. 인식된 admission
...
후보를 생성하고 raw 메시지는 보존하지 않습니다. FDAI는 campaign의 fixed numeric ranking
가중치를 복사하지 않습니다. Downstream ranking은 상관관계를 proven causation으로 취급하지 않고
...
출처 campaign의 host-port conflict reason-specific RCA priority는 포트하지 않습니다. Absorbed
발견 사항은 `candidate_only` 및 `hold`로 유지되므로 사유 문자열이 권위 있는 structural cause로
...
이 semantic 집약기만으로 방식이 operationalized되지는 않습니다.
...
출처 campaign의 CronJob 하위 deletion, sidecar patching, finalizer/RBAC 변경, deny-all
복원 및 reason-specific RCA precedence는 포트하지 않습니다. 신원 정규화,
세대 검사 및 semantic churn 허용 오차는 7개 액션 safeguard나 causal 권한을 독립적으로
증명하지 않습니다.
...
출처 campaign의 arbitrary custom 소유자 spec field-basename 검증은 포트하지 않습니다. 일치하는
CRD OpenAPI 스키마와 exact 스키마 경로가 없으면 `runAsUser`, `effect` 또는 `updateStrategy`라는 필드
이름만으로 Kubernetes security-context, toleration 또는 워크로드 strategy semantic을 증명할 수
...
이는 direct 소유권 relationship을 증명하지만 소유자 구성이 성능 저하를 일으켰다고
증명하지 않습니다. 출처 campaign의 `configuration_precedes` 주장과 임의 custom spec 변환 결과는
구성 변경 시각 또는 interventional 근거가 없으므로 의도적으로 거부합니다.

인벤토리 근거는 Pod 이미지 pull 실패와 owning 배포, StatefulSet 또는 DaemonSet
템플릿의 drift를 correlate할 수 있습니다. 상관관계에는 완전한 컨테이너 변환 결과, 각 홉의
exact controller 소유자 하나, 체인 내 모든 리소스의 변경할 수 없는 UID 일치, 인식된 waiting 사유 및
같은 컨테이너 이름의 서로 다른 SHA-256 image-reference fingerprint가 필요합니다. Raw 이미지
참조는 변환 결과하지 않습니다. Recreated, 모호한, malformed 또는 잘린 근거는
발견 사항을 생성하지 않으며 결과는 템플릿 drift가 pull 실패를 일으켰다는 주장이 아닌 hold-only
후보로 유지됩니다.
출처 campaign의 automatic operator-namespace 탐색은 포트하지 않습니다. 이 구현은 custom
...
verb/리소스, 완전한 역할/연결 변환 결과 및 명시적 범위가 제한된 범위를 사용해야 합니다.

Campaign의 범용 custom-resource patch 허용 목록은 포트하지 않습니다. Exact API-version/종류
허용 목록과 세대 검사는 새 변경 기본 요소에 필요하지만 충분하지 않습니다. 출처
변경은 arbitrary custom 리소스에 대한 영속 대상 lock, persistent 중복 suppression,
효과 이전 감사 의도, 범위가 제한된 롤백 drill 및 observer-independent 효과 검증을
...
변환 결과 전체에서 webhook 이름이 유일하고 affected 리소스가 대상 이름 공간에 있을 때만
구성 후보를 식별할 수 있습니다. TLS, 시간 초과 및 백엔드 실패는 검토된 출처
경로와 범위가 제한된 실패 정책/서비스 신원을 보존합니다. Webhook URL과 CA bundle은 계속
제외합니다. 발견 사항은 후보 전용이며 webhook 이름 일치는 구성이 외부 실패를
일으켰다는 증명이 아닙니다.
누락된 webhook 백엔드 semantic은 이름 공간 인벤토리보다 강한 absence 경계를 사용합니다.
후보에는 완전한 webhook 변환 결과 하나와 successful 읽기가 absence를 확인한 exact targeted
서비스 증적 하나가 필요합니다. Present, 실패한, 모호한, malformed 또는 잘린 증적은
발견 사항을 생성하지 않습니다. 후보는 구성 신원, webhook 이름, 실패 정책,
...
근거를 불완전한으로 만듭니다.
FDAI는 webhook 서비스 참조를 사용해 백엔드 이름 공간의 full 인벤토리를 수집하지 않습니다.
해당 참조는 이름 공간 내 모든 리소스에 대한 의존성나 근거 표면 확장 권한을
증명하지 않습니다. Exact targeted 서비스 증적이 백엔드 absence 근거에서 출처 campaign의
broad cross-namespace 탐색을 대체합니다.
FDAI는 webhook 백엔드 Pod를 선택하는 deny-all NetworkPolicy도 API-server 트래픽 차단의 증거로
...
출처 campaign의 automatic `failurePolicy: Fail` to `Ignore` 복구 시드는 포트하지 않습니다.
이 변경은 admission security 의도를 fail-open으로 바꾸고 누락된 백엔드를 복구하지 않으며,
resulting admission과 롤백이 intended control을 보존한다는 독립적인 증명도 없습니다. Approval,
...
우회하며 trust 체인이나 intended admission control을 복구하지 않습니다.
Webhook 이름 공간 선택자가 표현식 없이 exact
...
Cumulative 시간 초과 근거는 별도 후보 전용 방식입니다. 서로 다른 webhook 이름의
...
malformed 또는 잘린 이벤트는 발견 사항을 생성하지 않습니다. 출처 campaign 구현과 달리 direct
policy-path 및 temporal 근거 없이 NetworkPolicy나 degraded 워크로드를 원인으로 추론하지 않습니다.
출처 campaign의 cumulative-timeout NetworkPolicy 복구 patch는 포트하지 않습니다. Common
...
공유 Kubernetes package에는 hold-only admission resource-drift 집약기가 있습니다. Exact 코어/v1
Pod 생성 rule을 가진 완전한 selector-free, namespace-unscoped
MutatingWebhookConfiguration 하나와 정규화된 요청 또는 한도 drift 사이의 후보 전용
상관관계를 보고합니다. 완전한 워크로드 선택자도 완전한 Pod 라벨과 일치해야 합니다.
집약기는 webhook이 drift를 일으켰다고 주장하지 않습니다. 여러 mutator, conditional mutator,
...
이름 공간 인벤토리와 범위가 제한된 cluster-scoped webhook 변환 결과를 결합합니다. Webhook URL, CA bundle
및 검토되지 않은 필드는 변환 결과하지 않습니다. 이는 이름 공간 범위 또는 rule applicability를
증명하지 않고 mutator 하나를 causal로 취급하던 출처 campaign 동작을 강화합니다.
...
배포의 desired 복제본이 ready 복제본보다 많을 때만 correlate합니다. 발견 사항은 closed 검토된
violation vocabulary, 프로파일/버전, 변경할 수 없는 신원을 포함하고 raw 메시지는 제외합니다. Unknown,
...
fingerprint 하나가 필요합니다. 탐색 명령, HTTP 경로, 헤더 및 주소는 변환 결과하지 않고
방식, 범위가 제한된 timing 및 SHA-256 정의 fingerprint만 보존합니다. Drift, 모호함, stale/future
Event 및 잘린 근거는 abstain합니다. FDAI는 출처 campaign의 fixed sleep 및 second Event
...
액션 권한을 만들지 않습니다. 체인의 시작 gate가 불일치하면 abstain합니다.
준비 상태 실패는 동일한 recent Event, exact UID-chain, degraded-owner 및 identical privacy-safe
fingerprint kernel을 재사용합니다. 분류에는 kubelet 보고기와 검토된 readiness-failure
문구도 필요합니다. Raw 메시지나 fixed-delay Event refresh 없이 별도 hold-only 후보를
생성하며 drift, stale 신원, 모호함 및 잘림은 abstain합니다.
프로바이더 중립적인 저장소 semantic은 degraded multi-replica 워크로드에 완전한 필수 hostname
...
volume, mount 및 anti-affinity 인벤토리 변환 결과는 별도 프로바이더 작업입니다.
프로바이더 중립적인 init 의존성 semantic은 running init 컨테이너 하나, exact 변경할 수 없는
Pod-to-ReplicaSet-to-Deployment 체인, 세 spec에서 동일한 범위가 제한된 명령 fingerprint 및 서비스
...
프로바이더 중립적인 ConfigMap mount semantic은 degraded 워크로드, 완전한 volume/container-mount
변환 결과, mounted ConfigMap volume 하나 및 same-namespace ConfigMap absence를 확인한 exact targeted
...
프로바이더 중립적인 롤아웃 semantic은 완전한 strategy 근거에서 degraded 배포의 available
...
프로바이더 중립적인 CoreDNS semantic은 범위가 제한된 `template` 블록 하나의 헤더가 `svc.cluster.local`을
...
프로바이더 작업입니다.
출처 campaign의 automatic Corefile 템플릿 removal 및 CoreDNS 재시작은 포트하지 않습니다.
...
않습니다.
프로바이더 중립적인 서비스 semantic은 엔드포인트 근거가 완전한하고 빈이며 서비스 선택자가
완전한하고, same-namespace 워크로드 하나의 완전한 템플릿 라벨이 exact 일치하며 desired
복제본이 0일 때만 hold-only scaled-to-zero 후보를 생성합니다. Ready, nonzero, 모호한,
selector-mismatched, 불완전한 또는 잘린 근거는 abstain합니다. 구체적인 엔드포인트 및 라벨
변환 결과는 별도 프로바이더 작업입니다.
프로바이더 중립적인 autoscaling semantic은 완전한 HPA 하나가 CPU 사용률을 사용하고 closed
...
출처 campaign의 결정론적 SecurityContext patch는 포트하지 않습니다. Syntactically 근거에 기반한
템플릿 변경도 프로세스 신원, 기능 및 워크로드 행동을 바꿀 수 있으며 admission
성공만으로 롤아웃 상태, application 정확성 또는 롤백 복원을 증명할 수 없습니다.
해당 효과와 7개 액션 safeguard를 독립적으로 검증할 때까지 실제 운영 실행기는
`remediate.kubernetes-patch`를 미등록 상태로 유지하고 기반 호출을 수행하지 않습니다.

결정론적 판단 보류 시 기존 근거에 기반한 RCA 경로가 작업 objective와 범위가 제한된 근거를 받습니다.
...
허용합니다. Mismatched 종류 또는 unknown 참조는 계속 가설을 차단합니다.
...
전송하므로 stale 또는 누락된 모델 배포는 ready로 표시되지 않습니다. 모든 검사를 통과해도
host 권한은 관찰 모드로 유지됩니다.

구독에 capability-specific 배포를 추가할 할당량이 없으면 엔드포인트 발견이
`t2.rca`를 같은 account의 기존 검증된 배포에 연결할 수 있습니다. 생성된 연결은 URL 대신
abstract `azure-openai:<account>` 참조를 저장합니다. 런타임 조립은
`FDAI_LLM_ENDPOINT`와 일치하는 참조만 해석하며 다른 account 참조는 시작을
차단합니다.

플러그인 이미지는 digest-pinned SREGym 에이전트 base 위에 FDAI 분포, rule 및 정책 카탈로그,
SREGym 플러그인을 포함합니다. 고정된 FDAI/SREGym workspace package와 diagnostic 원장을 설치하고,
검토된 OPA binary를 포함하며 UID 65532로 실행합니다. 루트 Docker 빌드 맥락은 로컬 런타임
상태, resolved 모델 파일, 로그, temporary 산출물 및 시크릿을 제외합니다.

## CyberGym driver

독립 `benchmarks/cybergym/` package는 FDAI 코어 변경 없이 두 모드를 증명합니다.
...
`ExternalValidationReceipt`는 항상 실행에 대해 신뢰할 수 없는으로 표시됩니다. Host는 참조 작업
세션이 닫힌 뒤에만 이를 수락하고 unexpired same-task 산출물 참조를 검증하며, exact
재시도는 deduplicate하고 conflict는 차단합니다.
...
출처 data 및 검증기 준비 상태를 확인합니다. `patch-only` 모드가 성공하려면 project-test 단계
...
실패한 상태입니다. 작업 repository, 변경할 수 없는 및 pre-patch 경로는 relative 경로여야 하며 상위
탐색 component를 포함할 수 없습니다. 유효하지 않은 작업 경로는 컨테이너 시작 전에
실패합니다. 실행기는 구성된 출력 루트 아래에 범위가 제한된 에이전트 로그, `fix.patch`, `result.json`
및 시도한 검증 단계별 JSON 증적을 보존합니다. Host 명령의 stdout과 stderr는 독립된
바이트 상한을 적용하여 스트리밍하며, 두 스트림 중 하나라도 한도를 초과하면 하위 프로세스를 즉시
종료합니다. 검증 전에 실행기는 patch가 modify, rename 또는 copy하는 경로를 작업의 변경할 수 없는
...
literal 및 검토된 workspace 프로바이더를 우회하는 명령 실행을 차단합니다. 별도 CI 작업은
고정된 multi-package workspace를 설치하고 모든 evaluation 모음을 실행하며 SDK, SREGym 및
CyberGym wheel을 독립적으로 빌드합니다. 각 package는 90% line-and-branch 커버리지 하한, strict
...
workspace-only 의존성으로 연결합니다. FDAI 런타임 의존성에는 포함되지 않으며 각 wheel은 계속
...
멱등성, 시간 초과, 취소, 정리, 외부 검증, package 경계 및 두 벤치마크
수명 주기를 검증합니다.
...
| 알아볼 내용 | 문서 |
|-------------|------|
| Repository 및 의존성 경계 | [Project Structure](../architecture/project-structure-ko.md) |
| 프로바이더 injection 계약 | [CSP Neutrality](../architecture/csp-neutrality-ko.md) |
