...
translation_source_sha: e83acf97b27f507601e2567c83d4de4bd10b5f61
...
활동 화면은 출처 라벨을 유지하면서 영속 감사 행과 브라우저 세션의 런타임 행을 시간순으로 최대 200개 표시합니다. 구조화된 처리기 시작과 종료 전이는 하나의 활동 식별자를 공유하며 리소스를 먼저 보여 주는 하나의 작업 행으로 표시합니다. 서버는 정확한 논리 Resource 참조에서 표시 이름을 만들고 검사할 수 있도록 완전한 참조를 함께 유지합니다.
...
기록된 에이전트 간 턴과 실시간 턴은 길이 제한 내의 전체 메시지를 포함한 개별 `from -> to` 행으로 유지합니다. 최신 기록 자동 추적은 처음에 활성화되며 위로 스크롤하면 일시 중지합니다. 에이전트와 키워드로 필터링할 수 있고 시간, 경로, 유형, 상세, 상관관계 열을 선택할 수 있으며 모든 열을 기본으로 표시합니다. 전체 화면은 표시만 변경합니다.
시간은 브라우저의 IANA 시간대에 따른 시각을 표시하고 `Asia/Seoul`에는 `KST`를 사용합니다. 기계가 읽는 행에는 전체 타임스탬프를 유지합니다. Waterfall은 수명 주기, 입력, 출력, 기록된 대화, 해시를 살펴보는 영속 감사 목록·상세 화면으로 유지합니다.
...
Agent Activity 경로는 허용된 스트림 프레임을 최대 100 ms 또는 대기 메시지 1024건까지 모은 뒤 상태 축약기에 한 번 전달하고, 이후 관측을 숨기지 않으면서 완전히 동일한 재생 중복을 버리며, 인시던트별 대화 턴은 최신 100건까지만 유지합니다. 화면 밖 로그 행은 필요할 때까지 레이아웃과 그리기를 생략하되 44 CSS 픽셀 고유 형상을 유지합니다. DOM 행, 필터 및 접근 가능한 표 의미는 바뀌지 않습니다.
