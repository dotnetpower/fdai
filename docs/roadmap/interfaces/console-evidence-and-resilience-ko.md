...
translation_source_sha: e7b00dab6668291807a1fb9c40722e2ada13bab8
...
GET 전용 Sample 클라이언트가 인증이나 네트워크 접근 없이 일반 변환 결과를 제공합니다. Sample Live는 [Live와 감사 로그 화면](live-audit-presentation-ko.md)에 정의된 대표 카드 12개, 루프 180개의 지표 초기화, 다시 재생할 수 있는 티어별 시점을 사용합니다. Live, 프로비저닝, 예약 연속 작업은 범위가 제한된 로컬 예시를 사용합니다. Sample은 인시던트 개입을 제거하고 모든 POST를 차단하며 API 실패로 자동 활성화되거나 실제 값과 혼합되지 않습니다. 지원하지 않는 경로는 컨트롤을 숨기고 Live를 유지합니다.
...
각 Dashboard 라우팅 분포의 화면 정수 백분율은 정확한 점유율을 한 묶음으로 배분하여 합계가
100%가 되게 합니다. 구간 너비는 계속 정확한 점유율을 사용하므로 합계 보정이 원래 비율을
왜곡하지 않습니다.
