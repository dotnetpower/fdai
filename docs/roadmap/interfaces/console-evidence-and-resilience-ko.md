...
translation_source_sha: 839849f746d709f4cf9417191f0cc91fd3901231
...
Sample 자율성 예시는 운영 성과 지표 5개마다 8개 구간의 추세를 제공합니다. 비용 거버넌스는 경로에 따라 비용, `optimization_case`, `outcome` 레코드를 구분해 선택합니다. 모든 레코드는 `synthetic-preview` 출처를 유지합니다. Console은 이 출처에 한해서만 화면 표현을 위해 `effect_verified` `outcome` 금액을 합산하고 후보 추천 절감액과 비교합니다. 중간 정산 단계, SLO 회귀 및 단위 경제 필드는 사용 불가 상태로 남깁니다.
