...
Dashboard는 [제한된 자동 복구](recorded-resource-state-ko.md#제한된-자동-복구)로 최초 동기화 상태를 표시하고 같은 클라이언트의 검증된 근거를 유지합니다. 릴리스 불일치나 예산 소진 시 접근 검사를 완화하지 않고 재시도를 멈춥니다. 가벼운 현재 화면 실행 버튼은 전체 Command Deck을 불러오지 않아도 사용할 수 있습니다.
