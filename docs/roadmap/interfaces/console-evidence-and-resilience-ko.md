...
translation_source_sha: 632836a25ea6e463b539e1622a26958c3fc5c10d
...
있습니다. Console은 최초 `/iam/self` 확인 전에 로그인한 인증 컨텍스트를 모든 하위 클라이언트에 연결합니다. 확인에 실패하면 데이터 접근을 차단하고, 명시적 재시도에서는 이 연결을 재사용한 뒤 요청한 화면을 엽니다. 이 계약은 AKS 게이트웨이와 직접 연결하는 Container Apps API에 동일하게 적용됩니다. 인증 정보가 없거나 유효하지 않으면 전역 401 처리로 계속 차단합니다. 격리된 복구 테스트는 클라이언트 동작을 검증하며 실제 배포 상태를 증명하지 않습니다.
