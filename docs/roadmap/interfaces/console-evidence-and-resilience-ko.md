...
translation_source_sha: 12fe18098f8fed0119062e08faa81113607b6fe3
...
선택적 전체 현황 분기는 형식이 지정된 부재 상태만 숨깁니다. 디코더에서 발생한 `502`는 정상처럼 보이는 페이지에서 데이터 하나를 조용히 제거하지 않고 화면 오류로 표시합니다.
초기 Operator API 퍼사드는 경로가 소유한 감사, 에이전트 활동, 분석 및 보고 디코더를 시작 청크에 포함하지 않습니다. 각 디코더는 소유한 읽기가 시작된 뒤에만 로드되며 프로덕션 진입 번들 검사는 원시 크기와 gzip 한도를 유지합니다.
