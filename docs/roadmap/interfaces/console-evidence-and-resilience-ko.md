...
translation_source_sha: 4aabc47c4b135acecc039eca4642be3d2e168448
...
| 운영 온톨로지 인스턴스 탐색기 | implemented | `instance_explorer.py`, `postgres_family_store.py`, `ontology-instances*.{tsx,ts,css}`, 포커스 Operator 및 Console 검사, strict mypy, Ruff, 작업 범위 편집기 진단, 운영 Vite 빌드 및 인증된 세 화면 크기 검사 | `인스턴스` 보기는 활성 세대 Resource 디렉터리와 정확한 선택 Resource 하나를 범위가 제한된 양방향 그래프, 허용 목록 속성, 최신순 정확한 Resource FDAI 감사 사실, 독립적인 출처 가용성, 릴리스 신원, 세대, 기준 시점 및 잘림 상태에 연결합니다. Resource Group 또는 Subscription 루트는 유형 다양성을 보존한 직접 관계를 최대 36개까지 표시하고 자식 사이 관계는 전체 개수와 상세 패널에 유지합니다. 다른 루트는 범위가 제한된 분기 맥락을 유지합니다. 정확한 AKS node Resource Group mapping은 AKS에서 VMSS로 향하는 edge를 만들지 않고 cluster의 managed Resource Group, 포함된 VMSS와 Load Balancer, 기록된 identity와 outbound IP dependency를 순서대로 배치합니다. 그래프는 마우스 휠 확대 및 축소와 기본 전체 화면을 사용하고 상세 패널은 선택한 상세 보기를 잃지 않고 접을 수 있습니다. 검토된 Azure 아이콘은 활성 Resource 유형 58개 중 55개를 지원하며 3개 유형은 명시적인 일반 대체 아이콘을 유지합니다. Azure Resource Health와 Activity Log는 Operator 변환 결과가 연결될 때까지 사용할 수 없는 상태로 유지합니다. |
...
| Activity Bar 대화 진입점 | implemented | `console/src/components/navigation-shell.tsx`, `console/src/deck/open-deck.ts`, 집중 Console 테스트 23개, Console 타입 검사 및 인증된 세 화면 크기 Browser 검사 | 하단 유틸리티 표시줄은 경로를 바꾸지 않고 현재 화면의 Command Deck을 열고 닫으며, 접근 가능한 눌림 상태로 열림 여부를 표시하고, 기존 실행 표시줄과 키보드 단축키를 유지합니다. 표현 검사에서 가로 overflow는 없었으며 통제된 런타임 증적을 주장하지 않습니다. |
### 구현 이력
| 날짜 | 상태 | 변경 | 근거 | 잔여 작업 |
|------|------|------|------|-----------|
...
| 2026-08-25 | implemented | Activity Bar 하단 유틸리티 위에 아이콘 전용 대화 컨트롤을 추가했습니다. 지연 로드되는 Deck 수신기가 준비되기 전의 토글 하나를 보존하고, 기존 Deck 수명 주기를 통해 새 일반 대화를 열며, 표시줄이 노출된 화면에서는 두 번째 활성화로 닫고, 경로나 실행 권한은 바꾸지 않습니다. | `current change`, `open-deck.test.ts`와 `navigation-shell.test.ts`의 집중 테스트 23개, Console 소스와 테스트 타입 검사 및 운영 빌드 통과. 인증된 1440 x 900, 993 x 641, 390 x 844 Browser 검사에서 44 px 컨트롤, 바뀌지 않은 경로, 실행 표시줄 겹침 없음 및 가로 overflow 0을 측정했습니다. | 범위가 제한된 구현 작업은 남아 있지 않습니다. Browser 관측은 로컬 표현 근거이며 통제된 런타임 증적이 아닙니다. |
| 2026-08-22 | implemented | 인벤토리 또는 실행 권한을 바꾸지 않고 아키텍처에 별도의 2D Network 모드를 추가했습니다. 관측된 VNet 포커스 하나를 선택하고, 표현 필터 6개와 가장 짧은 타입 지정 경로 추적을 제공하며, 불완전한 부재는 unknown으로 유지하고, 키보드와 동등한 SVG 컨트롤 및 식별자가 없는 관측 토폴로지 보고서의 SVG 또는 PNG 내보내기를 제공합니다. | `current change`, 집중 네트워크 모델, 배치, 렌더러, 경로 및 localization 검사, 소스 TypeScript 검사, 합성 Playwright가 `1440x900`, `993x641`, `390x844`에서 순차 통과해 가로 overflow 0, 2단계 경로 근거, 정제된 다운로드 및 44 px 모바일 내보내기 컨트롤을 확인했고 인증된 모바일에서 경계 8개, 노드 4개, 링크 4개 및 관측된 홉 1개를 확인했습니다. | 이 영역을 `validated`로 바꾸기 전에 정확한 소스에 연결된 관리되는 데스크톱 및 모바일 Browser 근거를 보존합니다. |
| 2026-08-22 | implemented | 밀집된 dual-VNet 토폴로지에서 Network 모드와 다운로드 보고서를 강화했습니다. Peer focus는 peer VNet, 직접 subnet 및 범위가 제한된 attached resource를 유지하고, 하나의 obstacle-aware router를 live SVG와 내보내기가 공유하며, VNet peering은 직접 경계 corridor를 사용합니다. Self-contained 내보내기는 runtime icon fetch 없이 공식 icon, 타입 지정 방향, 근거 상태 및 정제를 유지합니다. | `current change`, 집중 network 및 icon 검사 74개, Console 타입 검사와 운영 빌드 통과. `1440x900`, `993x641`, `390x844` 순차 Playwright에서 직접 검토한 capture 5개를 보존하고 공식 icon, 타입 지정 link 9개, 직접 양방향 peering, 2단계 highlight, 정제된 SVG 내용, nonblank PNG, 44 px 모바일 node와 control, panning 및 document overflow 0을 검증했습니다. | 이 영역을 `validated`로 바꾸기 전에 정확한 소스에 연결된 관리되는 데스크톱 및 모바일 Browser 근거를 보존합니다. |
| 2026-08-22 | implemented | 하나의 활성 Resource 세대를 사용하는 별도의 온톨로지 `인스턴스` 워크벤치를 추가했습니다. 운영자는 Resource 하나를 검색하고 선택하며, 직접 들어오고 나가는 인접 항목으로 이동하고, 허용 목록의 식별 및 스냅샷 사실을 검사하며, 정확한 Resource에 연결된 FDAI 감사 활동을 검토하고, Azure 이벤트 출처를 추론하지 않고 사용 불가 상태로 확인할 수 있습니다. | `current change`, focused Operator 테스트 39개, Console decoder, 온톨로지 및 localization 테스트 22개, strict mypy, Ruff, 작업 범위 editor diagnostics, 운영 Vite 빌드 및 entry bundle 검사 통과. 인증된 데스크톱, 제한된 데스크톱 및 모바일 검사에서 문서와 작업 영역 넘침이 없었고 모든 모바일 컨트롤이 44 px였으며 canvas에 불투명 표본 20,735개가 있었습니다. | 대표 Container App을 포함하는 완전한 인벤토리 세대를 승격한 뒤 정확한 대상 선택, 인접 항목 이동 및 이벤트 근거를 보존합니다. 해당 이벤트 계열을 표시하기 전에 Resource Health와 Activity Log를 검토된 Operator 변환 결과로 연결합니다. |
| 2026-08-23 | implemented | 온톨로지 인스턴스의 검토된 아이콘 집합을 확장하고 그래프 제어를 단순화했습니다. 선택한 그래프는 기본 전체 화면으로 전환되고 일반 마우스 휠 입력으로 확대 및 축소하며, 상세 패널은 활성 보기를 유지한 채 접고 다시 펼칠 수 있습니다. 현재 활성 스냅샷 감사에서 Resource 유형 58개 중 55개는 검토된 제품 아이콘을 사용합니다. Role assignment, data collection endpoint 및 의도적으로 분류되지 않은 Resource는 일반 대체 아이콘을 유지합니다. | `current change`; Azure asset 무결성 검사 107개와 아이콘, 보기 제어 및 지역화 포커스 테스트 통과. 인증된 데스크톱, 제한된 데스크톱 및 모바일 검사에서 정확한 전체 화면 경계, 상세 패널 공간 회수, 44 px 모바일 컨트롤, 마우스 휠 확대 및 축소, 문서 overflow 0을 측정했습니다. | 이 범위의 상태를 `validated`로 바꾸기 전에 정확한 소스에 연결된 관리되는 Browser 근거를 보존합니다. |
| 2026-08-23 | implemented | 관계가 많은 Resource Group 및 Subscription 루트를 유형 다양성을 보존한 직접 소속 보기로 제한했습니다. 범위 루트는 더 이상 모든 자식 분기를 확장하거나 두 자식이 모두 보인다는 이유만으로 자식 사이 관계를 추가하지 않습니다. 전체 응답 개수, 완전성, 잘림 상태 및 상세 패널 근거는 그대로 유지하고 자식을 선택하면 해당 Resource의 범위가 제한된 분기 맥락을 다시 표시합니다. | `current change`; 포커스 그래프 모델 테스트 25개 통과. 인증된 `rg-fdai-dev-krc` 응답은 `resource_limit`과 함께 Resource 200개, 관계 478개인 불완전 상태를 유지했고 화면 표현은 노드 197개와 간선 450개에서 Resource 유형 28개를 포함한 고유 노드 37개와 직접 간선 36개로 줄었습니다. 데스크톱, 제한된 데스크톱 및 모바일 검사에서 중복 항목, 좌표 충돌, 왼쪽에서 오른쪽으로 향하지 않는 포함 또는 의존 관계, 문서 overflow가 모두 0개였습니다. | 이 범위의 상태를 `validated`로 바꾸기 전에 정확한 소스에 연결된 관리되는 Browser 증적을 보존합니다. |
| 2026-08-23 | implemented | 근거로 제한된 AKS infrastructure hierarchy를 추가했습니다. Console은 정확한 `azure.aks-attached-to-node-resource-group` mapping에서만 이를 활성화하고, managed Resource Group을 저장된 포함 infrastructure보다 앞에 배치하며, 기록된 identity와 outbound IP target을 해당 VMSS 또는 Load Balancer source 뒤에 배치합니다. 저장된 source와 target 값은 바꾸지 않습니다. | `current change`; 포커스 Console 검사 47개와 typecheck, production build가 통과했습니다. 새로 고친 인증된 AKS 보기는 main Resource Group `-2`, AKS `0`, managed Resource Group `1`, VMSS와 Load Balancer `2`, identity와 outbound IP `3`을 표시했습니다. 세 화면 크기 모두 node 7개와 저장된 edge 10개를 유지했고 중복 occurrence, collision, LR이 아닌 edge, document overflow는 모두 0개였습니다. | 이 범위의 상태를 `validated`로 바꾸기 전에 정확한 소스에 연결된 관리되는 Browser 증적을 보존합니다. |
...
| 2026-08-22 | implemented | `https://www.tremor.so/charts`를 직접 검토한 뒤 완전성 기준을 정정했습니다. 내보내기 component 이름 13개와 composition family 7개만으로는 페이지에 공개된 example 33개를 다루지 못했습니다. `TREMOR_CHART_CATALOG`는 이제 공식 순서의 heading 33개를 모두 고정합니다. 공유 renderer는 stacked 및 percent 영역과 막대 geometry, 축 제목, 시작/끝 tick, `layout="vertical"`을 통한 가로 막대, grouped 막대, 조건부 색상, 사용자 정의 tooltip content, 설정 가능한 styling 및 radius, gradient 막대, 도넛 및 영역 callback, radius와 child metric을 지원하는 ProgressCircle variant 4개, CategoryBar marker를 구현합니다. | `current change`, `chart-colors.ts`, `charts-series.tsx`, `charts-compact.tsx`, `charts.tsx`, 운영 panel 56개 fixture 및 정적 catalog card 33개, focused 차트, tooltip 및 Process 검사 31개 통과, Console typecheck, production build 및 entry bundle 통과, 정적 갤러리 계약 8개 통과, 세 viewport Playwright가 7.2초에 정확한 catalog 순서와 variant 동작을 실행했습니다. Browser 검사에서 stacked area path, 100% domain, 축 제목, 288 x 6 px 가로 막대, 번갈아 적용된 조건부 색상, gradient 및 marker geometry와 document, fixture, panel, grid, card, iframe, outer overflow 0을 확인했습니다. 정적 catalog는 desktop, 제한된 화면, mobile에서 각각 3 x 11, 2 x 17, 1 x 33으로 렌더링됐습니다. | 통제된 인증 런타임 근거는 별도입니다. Catalog fixture는 synthetic이며 표현 mechanics만 입증하고 측정 또는 작업 권한을 부여하지 않습니다. |
| 2026-08-22 | implemented | 두 차례 독립 검토를 통해 모든 차트 함수와 catalog example 33개를 다시 감사한 뒤 확인된 모든 Medium 이상 finding을 수정했습니다. 선 및 영역 차트는 유한하지 않은 값의 gap을 보존하고, percent 차트는 음수 입력을 숨기지 않고 거부하며, 영역 gradient는 실제 SVG gradient를 사용합니다. CategoryBar는 Tremor 색상 9개를 모두 사용하고 total이 0이면 marker를 표시하지 않으며 marker animation은 reduced motion을 따릅니다. Donut은 hole을 인식하는 pointer 선택과 최초 arrow-key 탐색을 지원하고 BarList는 음수 값을 알리면서 너비 0 막대를 그리지 않습니다. Donut 및 Tracker list 안에서 native button role을 보존하며 series 차트는 유지되는 안내선과 `aria-pressed`를 가진 keyboard 선택 가능 `onValueChange`를 구현합니다. | `current change`, 차트 source 및 운영 panel 62개 adversarial fixture, focused 차트, tooltip 및 Process 검사 33개 통과, Console typecheck 및 production build 통과, 정적 계약 8개 통과, 세 viewport Playwright 검사 2개가 9.7초에 통과했습니다. Browser 검사에서 gap을 보존하는 영역 segment 2개와 isolated point 1개, 잘못된 percent data 거부, CategoryBar 고유 색상 9개, zero-total marker 부재, Donut keyboard 및 hole callback, gradient와 heatmap 구분, ProgressCircle variant 색상, marker reduced motion, signed 및 single-point 경계, native list/button semantics, 이름이 있는 focus target 33개와 production 및 static overflow 0을 확인했습니다. BarList 수정 뒤 두 번째 독립 감사에서 남은 Medium 이상 chart 결함은 없었습니다. | 통제된 인증 런타임 근거는 별도입니다. Adversarial fixture는 synthetic 표현 근거이며 측정 또는 작업 권한을 부여하지 않습니다. |
### 잔여 작업
- [ ] 대표 Container App을 포함하는 완전한 활성 인벤토리 세대를 승격한 뒤 선택, 1단계
    인접 항목 이동, 영속 FDAI 활동, 독립적인 출처 공백, 키보드 상호 작용 및 세 화면 크기
    넘침에 대한 인증된 정확한 대상 근거를 보존합니다.
- [ ] 프로바이더 접근 또는 브라우저 조회 권한을 추가하지 않고 출처별 시각, 완전성 및 근거
    참조를 보존하는 범위가 제한된 Resource Health 및 Azure Activity Log 변환 결과를 인스턴스
    탐색기에 연결합니다.
...
- [x] Web 블록 레지스트리와 접근 가능한 v2 차트 모듈을 구현한 뒤 1440 x 900,
