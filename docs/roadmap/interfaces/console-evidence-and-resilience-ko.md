...
좁은 콘솔 화면에서 Assurance Twin의 보류된 자세와 변경 검토 근거 공백은 식별자와 이유가 보이는 별도 상태 영역으로 유지합니다. 집중 반응형 및 접근성 검사는 이 읽기 전용 공백이 작업 제어나 성공한 보고서로 표시되지 않는지 확인합니다.
## 탐색 컨텍스트
활동 Bar 영역을 선택하면 Explorer가 열리고 운영자의 로컬 순서 및 표시 설정에 따라 첫 번째 visible 패널로 이동합니다. 고정하지 않은 Explorer를 숨김 또는 비활성 상태로 바꾸기 전에 소유 활동 Bar 영역 버튼으로 초점을 되돌립니다. Command Deck이 닫혀 있거나 floating 상태여도 이 탐색은 동작하며, full-workspace Deck은 경로가 변경되기 전에 닫힙니다. 현재 화면 Command Deck을 여는 브라우저 근거는 `Overview`와 같은 일반 패널에서 시작합니다. `Settings`는 모달 오버레이를 소유하므로 뒤쪽의 실행 버튼을 누르는 컨텍스트로 사용하지 않습니다.
좌측 메뉴의 대화 버튼은 경로나 탐색 패널을 바꾸지 않고 일반 대화를 열거나 이어갑니다. `aria-pressed` 상태는 일반 대화만 나타냅니다. 하단 버튼과 `Ctrl+K` 또는 `/`는 현재 화면의 별도 대화를 엽니다. 일반 대화는 전체 작업 영역, 화면 대화는 우측 패널이 기본값입니다. 마우스를 올리거나 키보드 포커스를 주면 좌측 툴팁은 오른쪽에, 하단 툴팁은 위쪽에 표시하며 참고 맥락과 실제 열기 또는 닫기 동작을 안내합니다. 초안 보존과 명시적 화면 추가는 [대화 수명 주기](operator-console-progressive-conversations-ko.md#command-deck-작업-영역-수명-주기)에서 정의합니다. 모든 Command Deck 날짜 및 시간 레이블은 활성 Console 언어를 명시적으로 사용합니다. 브라우저나 운영 체제의 로케일 때문에 영어 대화에 한국어 오전 또는 오후 표기가 나타나거나 한국어 대화에 영어 오전 또는 오후 표기가 나타나지 않습니다.
...
직접 Settings URL은 오버레이를 닫을 때까지 숨겨진 Dashboard 변환 결과를 불러오지 않고 정적 배경 셸을 유지합니다.
Settings 셸과 다른 최상위 지연 로딩 화면은 로딩 상태를 표시하며, 모듈 로딩 실패 시 새로고침 작업을 제공합니다.
모델, 런타임 정책, ID 및 액세스, 문서 기능 조회는 최초 요청 실패 시 전체 콘솔을 다시 불러오지 않고 재시도할 수 있습니다.
Console은 최초 `/iam/self` 확인 전에 로그인한 인증 컨텍스트를 모든 하위 클라이언트에 연결합니다. 확인에 실패하면 데이터 접근을 차단하고, 명시적 재시도에서는 이 연결을 재사용한 뒤 요청한 화면을 엽니다. 이 계약은 AKS 게이트웨이와 직접 연결하는 Container Apps API에 동일하게 적용됩니다. 인증 정보가 없거나 유효하지 않으면 전역 401 처리로 계속 차단합니다. 외부 Browser Entra 경로 점검은 탐색 전에 승인된 세션 스토리지 부트스트랩을 복원하고 액세스 토큰 오류 화면을 경로 실패로 처리하며, 이 경계가 표시되면 셸, 제목 또는 로딩 완료가 렌더링되더라도 인증된 경로 근거로 간주하지 않습니다. 격리된 복구 테스트는 클라이언트 동작을 검증하며 실제 배포 상태를 증명하지 않습니다. 구성 기준선, Skills, 대화 전달, 예측 학습에도 공통 근거 새로 고침 버튼을 제공합니다. 기존 변환 결과만 다시 읽고, 로딩 골격과 중복 실행 방지를 적용하며, 현재 보기를 유지합니다. 스킬 설치, 프로바이더 호출 또는 권한 상승은 수행하지 않습니다.
Dashboard는 [제한된 자동 복구](recorded-resource-state-ko.md#제한된-자동-복구)로 최초 동기화 상태를 표시하고 같은 클라이언트의 검증된 근거를 유지합니다. 릴리스 불일치나 예산 소진 시 접근 검사를 완화하지 않고 재시도를 멈춥니다. 선택적 Overview 변환 결과를 요청하기 전에 Cost Governance 가용성을 확인하므로, 패키지를 사용할 수 없거나 비활성화한 경우 명시적인 사용 불가 지표를 유지하고 실패하는 변환 결과 요청을 보내지 않습니다. 가벼운 현재 화면 실행 버튼은 전체 Command Deck을 불러오지 않아도 사용할 수 있습니다.
콘솔은 첫 포인터, `Ctrl+K`, `/`, 일반 대화 또는 인시던트 대화 요청 후 Deck을 불러오며, 수신기가 준비되면 범위가 제한된 해당 요청을 다시 전달합니다.
...
배치에서는 대화 기록 폭을 줄이지 않고 그 위에 overlay로 엽니다. Workspace에서는 포인터 또는 keyboard 구분자로 대화 이력 폭을 180-360 px 범위에서 조절하고 마지막 폭을 로컬에 저장합니다. 좁은 배치는 구분자를 숨깁니다. 이력 헤더는 검색과 icon-only 새 대화를 간결한한 한 줄에 배치하고 lightweight 필터 tab을 사용하며, 컨트롤 대신 목록만 scroll합니다. 현재 화면 다이제스트는 workspace 컨트롤로 유지됩니다. Deck은 열린 표면마다 조립 계층이 소유한 데이터 소스 매니페스트를 한 번 읽고 대화 기록 위에 인벤토리, Incidents, 감사, Knowledge 및 자동화 준비 상태 링크를 간결한하게 표시합니다. 클라이언트는 동시에 발생한 읽기를 하나로 합치고 성공한 매니페스트를 최대 15초 동안만 재사용합니다. 따라서 Operator Service가 다시 시작되거나 구성이 복구된 뒤 다음 읽기에서 매니페스트를 재검증하여 이전의 사용할 수 없음 상태에 고정되지 않습니다. 누락되거나 정본이 아닌 출처는 `unknown`으로 유지합니다. 브라우저는 상태를 추론하거나 원시 프로바이더 상세를 노출하거나 경로 존재로 매니페스트를 대체하지 않습니다. 로딩은 고정된 골격을 사용하고 매니페스트 실패는 대화 이력을 차단하지 않으면서 진단으로 연결합니다.
...
Command Deck은 전송 토큰을 명시적인 초안 상태로만 표시하고 증적에 결속된 수정본 또는 확정 구간은 도착 즉시 렌더링합니다. 이후 스트림이 잘못되거나 중단되거나 순서 공백이 있거나 최종 프레임이 누락되면 초안을 철회하고 성공처럼 보이는 부분 답변 대신 사용 불가 결과를 표시합니다. 서버가 소유한 최종 대화 연결 정보는 다음 턴 전에 영속 인덱스와 활성 메모리 내 메타데이터에 모두 반영됩니다. 스키마에 맞는 보류 최종 증적은 요청 ID가 일치하면 응답에 계속 연결되므로, Console은 형식화된 사용 불가 상태를 성공으로 바꾸거나 출처 이력을 버리지 않고 보존합니다. 검증된 답변은 의미 증적의 근거 상태도 유지합니다. 주장 검사를 통과했더라도 오래되거나 불완전하거나 충돌하거나 사용할 수 없는 근거는 주의 상태로 표시합니다. 인용이 비어 있으면 중립적인 근거 출처 레이블을 사용하고 잘못된 대화 사전 검사 출력에는 플래너 복구 안내를 표시합니다.
공통 페이지 제목은 영역과 패널 레이블이 다를 때 `전체 현황 / Dashboard`를 포함해 둘을 함께 렌더링합니다. 패널 제목이 영역 레이블을 반복하는 영역 루트와 독립 utility는 단일 제목을 유지합니다. 선택적 전체 현황 분기는 형식이 지정된 부재 상태만 숨깁니다. 디코더에서 발생한 `502`는 정상처럼 보이는 페이지에서 데이터 하나를 조용히 제거하지 않고 화면 오류로 표시합니다. 초기 Operator API 퍼사드는 경로가 소유한 감사, 에이전트 활동, 분석 및 보고 디코더를 시작 청크에 포함하지 않습니다. 각 디코더는 소유한 읽기가 시작된 뒤에만 로드되며 프로덕션 진입 번들 검사는 원시 크기와 gzip 한도를 유지합니다.
...
GET 전용 Sample 클라이언트가 인증이나 네트워크 접근 없이 일반 변환 결과를 제공합니다. Sample Live는 [Live와 감사 로그 화면](live-audit-presentation-ko.md)에 정의된 대표 카드 12개, 루프 180개의 지표 초기화, 다시 재생할 수 있는 티어별 시점을 사용합니다. Live, 프로비저닝, 예약 연속 작업은 범위가 제한된 로컬 예시를 사용합니다. Sample은 인시던트 개입을 제거하고 모든 POST를 차단하며 API 실패로 자동 활성화되거나 실제 값과 혼합되지 않습니다. 지원하지 않는 경로는 컨트롤을 숨기고 Live를 유지합니다. 런타임 알림의 감사 목적지는 `data=live`를 고정하여 탭의 Sample 기본 설정이 정본 감사 근거를 다른 곳으로 보내지 못하게 합니다. 계약 테스트는 필수 인벤토리 진행 완전성 필드를 유지하고 정적 시안 목록에서 프로덕션 전용 패널을 제외하며, 포함된 온톨로지 본문이 로드된 뒤 lazy iframe에서 발생한 정확한 `net::ERR_ABORTED`만 허용합니다. 그 밖의 페이지 또는 요청 실패는 모두 차단합니다.
Sample 자율성 예시는 운영 성과 지표 5개마다 8개 구간의 추세를 제공합니다. 비용 거버넌스는 경로에 따라 비용, `optimization_case`, `outcome` 레코드를 구분해 선택합니다. 모든 레코드는 `synthetic-preview` 출처를 유지합니다. Console은 이 출처에 한해서만 화면 표현을 위해 `effect_verified` `outcome` 금액을 합산하고 후보 추천 절감액과 비교합니다. 중간 정산 단계, SLO 회귀 및 단위 경제 필드는 사용 불가 상태로 남깁니다. Live에 보존된 비용 관찰은 있지만 사례, 추천 또는 성과 레코드가 없으면 최적화 사례와 성과 화면은 공개 가능한 비용 기준과 관찰 건수를 상단에 표시하되 이후 단계 근거로 재분류하지 않습니다.
...
격자와 목록은 카드 내용을 공유하며 현재 제어 루프와 소스 항목을 최대 15개까지 이벤트 시작
순서로 유지합니다. 데스크톱 격자는 네 열을 사용합니다. 상세 패널은 기록된 위험, 영향, SLA와
제어 상태를 유지합니다. 티어, 자율성, 모드 배지는 전체화면에서도 포인터와 키보드 툴팁을
공유합니다. 없는 사실은 `관찰되지 않음`으로 표시하고, 보존된 최종 결과는 해당 이력 경로에서 확인합니다.
...
활동 화면은 출처 라벨을 유지하면서 최신 근거부터 표시하며, 런타임 행 700개, 영속 감사 원본 행 200개, 기록된 대화 행 200개를 각각 독립적으로 제한합니다. 이 브라우저 제한은 영속 근거를 삭제하지 않습니다. 구조화된 처리기 시작과 종료 전이는 하나의 활동 식별자를 공유하며 리소스를 먼저 보여 주는 하나의 작업 행으로 표시합니다. 서버는 정확한 논리 Resource 참조에서 표시 이름을 만들고 검사할 수 있도록 완전한 참조를 함께 유지합니다.
범위가 제한된 해당 사실이 기록되었으면 리소스 및 이벤트 유형, 종료 단계, 측정된 소요 시간, 토픽, 유지된 상관관계도 표시합니다. 토픽 문자열이나 프로바이더 페이로드에서 리소스 식별자를 추론하지 않으며 이전 방식의 상태 프레임은 별도 행으로 유지합니다.
기록된 에이전트 간 턴과 실시간 턴은 길이 제한 내의 전체 메시지를 포함한 개별 `from -> to` 행으로 유지합니다. 항상 표시되는 따라가기 버튼은 없습니다. 목록 맨 위에서는 새 행을 자동으로 반영하며, 과거 기록으로 스크롤하거나 행에 포커스를 두면 크기가 제한된 읽기 스냅샷 하나를 고정합니다. 새 이벤트 버튼은 현재 보존 버퍼에서 필터에 맞는 미확인 행만 세며 전체 도착 이력을 뜻하지 않습니다. 버튼을 누르면 키보드 포커스를 잃지 않고 최신 근거로 이동하며, 필터를 바꿔도 최신 일치 행으로 돌아갑니다. 시간, 경로, 유형, 상세, 상관관계 열을 선택할 수 있으며 모든 열을 기본으로 표시합니다. 전체 화면은 표시만 변경하고 컨트롤이 차지할 공간을 유지합니다.
시간은 브라우저의 IANA 시간대에 따른 `HH:mm:ss.SSS` 형식으로 표시하고 `Asia/Seoul`에는 `KST`를 사용합니다. 공통 툴팁과 기계가 읽는 행에는 기록된 전체 타임스탬프를 유지합니다. 표시 자릿수가 원본 시계의 정확도를 높이거나 인과 순서를 증명하지는 않으며, 시각이 같은 행은 출처별 순번에 따른 결정론적 순서를 유지합니다. Waterfall은 기존 시간 순서를 유지하며 수명 주기, 입력, 출력, 기록된 대화, 해시를 살펴보는 영속 감사 목록·상세 화면으로 남습니다.

감사 로그는 승인된 기록 검토 시안에 맞춰 조회 컨트롤, 최신순 기록 목록, 선택한 증적, 원본 JSON, 출처를 표시합니다. 기록 선택은 화면 안에서 이루어지며, 개별 기록 링크는 서버 측 순번 범위와 현재 필터를 유지합니다.
텍스트는 불러온 기록에서만 검색합니다. 결정, 기간, 상관관계 필터는 기존 Operator API에 조회합니다. 다음 페이지를 불러와도 선택은 유지되며, 실패하면 기존 증적을 버리지 않고 오류를 표시합니다.
서버 프로젝션이 없으면 원장 전체 집계는 확인 불가로 유지합니다. 선택한 기록에 명시된 단계만 증적 경로에 표시하며, 전달이나 관측만으로 효과를 입증하지 않습니다. 저장된 해시를 브라우저의 무결성 검증 결과로 간주하지 않습니다.
원장 보장 항목은 미검증으로 유지하고 지원하지 않는 재현·내보내기 작업은 생략합니다. 영어·한국어 화면은 식별자가 잘리지 않도록 재배치되며, 현재 화면 대화 맥락에는 사유 필드와 정확한 선택 기록을 유지합니다.
주기적인 idle 및 watching 상태 스냅샷은 변경되지 않은 영속 감사 페이지를 다시 로드하지 않고 현재 에이전트 상태와 관측 시간만 갱신합니다. 활성 작업, 완료된 핸들러 transition, 인시던트 및 인계는 계속 감사 근거를 새로 고칩니다.
활동 헤더는 반복되는 passive 스냅샷을 작업 행으로 추가하지 않고 마지막으로 관찰된 하트비트 시각을 표시합니다.
...
제목과 카드는 같은 형식과 비율 정밀도를 유지합니다. 각 Dashboard 라우팅 분포는 정확한 점유율에서 화면 정수 백분율을 한 묶음으로 배분해 합계를 100%로 맞추고, 구간 너비에는 정확한 점유율을 유지합니다.
대시보드 라우팅은 일반 감사 활동과 구분되는 정본 표본을 유지합니다. Trust Routing은 백분율과 동일한 자율성 측정 기준 시점, 표본 수, 정확한 비율을 사용해 각 티어에 표시할 이벤트 수를 계산합니다. 정식 티어 키가 없으면 해당 티어에서 측정된 0으로 처리하며, 티어가 기록되지 않은 이벤트는 표본에 남기고 T2로 다시 분류하지 않습니다.
...
근거에서 결정론적으로 렌더링됩니다. Skills 진단은 저장된 마지막 출처 갱신 성공 시각을 표시하고 잘못된 시각을 거부합니다. 활성화되었지만 측정되지 않은 출처와 준비된 출처를 구분하며 기존 오류를 지우거나 로드 권한을 부여하지 않습니다. 온톨로지 browse는 대상과 browse verb를 요구하고,
...
경로는 주요 가설이 없거나 판단을 보류하면 범용 응답을 숨깁니다. 대응 사실은 뒤에 기록된 결정
또는 롤백만 연결하고, 기록된 작업 유형과 유효 모드를 표시하거나 사용 불가로 유지하며, 두 순번을
정확한 감사 링크로 제공합니다. 이전 결정과 수명 주기 행은 대응이나 원인이 되지 않습니다. 검토
순서는 가설, 신뢰도, 체인, 인용, 대응입니다. 제한된 체인은 스크롤하고 모바일은 44 px 근거 링크를
갖춘 세로 구조로 바뀝니다. 결과 상태는 서로 구분하며 다른 상관관계가 로딩 요청을 교체할 수
있지만 중복 요청은 차단합니다. 제출 후
...
Agent Activity 경로는 허용된 스트림 프레임을 250 ms 단위로 모아 전달하되 대기 메시지가 1024건에 도달하면 먼저 처리합니다. 이후 관측을 숨기지 않으면서 완전히 동일한 재생 중복을 버리고 인시던트별 대화 턴은 최신 100건까지만 유지합니다. 로그는 실제 높이를 측정한 행을 가상화하고 화면 위아래로 각각 여섯 행을 추가로 렌더링합니다. 키보드 포커스가 있는 행과 앞뒤 행은 유지하며, 표 의미를 통해 보존된 전체 행 수와 절대 행 순번을 제공합니다. 보존 범위를 벗어난 행은 높이 측정 캐시에서도 제거합니다. 언어와 시간대별 포맷터를 재사용하므로 시각을 표시할 때마다 새 포맷터를 만들지 않습니다. 출처별 상한, 행의 전체 텍스트, 필터 및 영속 이력은 바뀌지 않으며 고정된 읽기 스냅샷은 새 이벤트가 들어와도 커지지 않습니다.
...
별도 [Console 아키텍처 작업 영역](console-architecture-workbench-ko.md) 문서가 Landscape 집계,
Resource 포커스, 생성 형상, Network 경로, 영향 표현, 반응형 상호 작용 및 형상이 없는 라이브 응답
회귀 게이트를 정의합니다.

이 복원력 문서는 다른 Console 경로와 공유하는 인벤토리 출처 가용성, 스냅샷 최신성, 범위가 제한된
새로 고침, 지역화 대체 경로 및 사용 불가 상태 동작을 계속 정의합니다. 수동 새로 고침은 마지막으로
완료된 화면과 진행 상태를 유지하고 중복 요청을 차단하며, 세대 전환은 지연 안내를 추가합니다. 관련
없는 실패는 계속 오류로 표시합니다. 대량 Dashboard와 온톨로지 인스턴스 디렉터리는 커밋된
무효화와 5분 fallback을 사용합니다. 디렉터리는 선택 전에도 SSE 연결을 유지하고 재검증 중에 현재
서버 검색어를 보존합니다.
...
| 알아볼 내용 | 읽을 문서 |
|-------------|-----------|
| 아키텍처 Landscape, 포커스 및 생성 형상 | [Console 아키텍처 작업 영역](console-architecture-workbench-ko.md) |
