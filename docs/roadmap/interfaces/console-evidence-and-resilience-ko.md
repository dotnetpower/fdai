...
translation_source_sha: da77e248bbaa897c4038f497b6ad787d961c9abb
...
활동 화면은 출처 라벨을 유지하면서 영속 감사 행과 브라우저 세션의 런타임 행을 시간순으로 최대 200개 표시합니다. 기록된 에이전트 간 턴과 실시간 턴은 길이 제한 내의 전체 메시지를 포함한 개별 `from -> to` 행으로 유지합니다.
최신 기록 자동 추적은 처음에 활성화되며 위로 스크롤하면 일시 중지합니다. 에이전트와 키워드로 필터링할 수 있고 시간, 경로, 유형, 상세, 상관관계 열을 선택할 수 있습니다. 유형 열은 처음에 숨겨지며 전체 화면은 표시만 변경합니다.
