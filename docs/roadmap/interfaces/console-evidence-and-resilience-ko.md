...
서술기는 표시된 사실만 받으며, 없는 값을 만들거나 권위 있는 출처를 대체하지 않습니다.
제목과 카드는 같은 형식을 사용하여 비율의 의미와 화면에 표시된 정밀도를 유지합니다.
대시보드 라우팅은 일반 감사 활동과 구분되는 정본 표본을 유지합니다.
모드, 티어, 조치, 결과, 구간과 순번 필터는 서버가 소유한 감사 조회까지 전달됩니다.
승인된 실측군 비교는 현재 Live 값과 분리되고 유효 기간이 있으며 어떤 권한도 부여하지 않습니다. 캡처한 화면 컨텍스트도 현재 기준선을 대체하지 않고 두 군과 유효 기간을 보존합니다. 만료되면 화면 근거를 다시 발행하고, 질문을 제출할 때마다 고정된 스냅샷에서도 만료된 실측군 묶음을 제거합니다.
감사 기반 변환 결과는 UTC 구간과 추가 전용 감사의 최종 순번을 고정한 뒤 그 아래의
정본 분류, 독립 결과, 지표 관측값과 사람 입력을 읽습니다. 완전한 조회가 예산을
초과하면 일부만 읽은 비율을 보고하지 않고 차단합니다. 정규화 이벤트 식별자가
분모를 정하고, 인시던트 및 변경 식별자가 시간 지표의 표본 수를 정합니다.
기준 순번 이후의 동시 추가는 스냅샷에 들어오지 않습니다.
명시적인 `measurement.action_outcome.v1` 기록이 enforce, 검증된, auto, non-rollback
...
아키텍처 경로는 지도 오른쪽 위에 떠 있는 간결한 패널에 범위 선택만 배치합니다. 인벤토리 개수, 설명문 및 계층 필터는 표시하지 않습니다. 변환 결과에 선택 가능한 범위가 없으면 이 패널도 표시하지 않습니다. 리소스를 선택하기 전의 리소스 지도 모드는 리소스, 관계, 지도 컨트롤, 범례 또는 관계 인덱스를 렌더링하지 않으며 리소스 선택기, 사용할 수 있는 범위 선택기, 표현 모드 컨트롤 및 가운데 선택 안내는 계속 사용할 수 있습니다. 리소스를 선택하면 범위가 제한된 지도와 점검 컨트롤을 표시합니다. 잘린 그래프는 짧은 상태 배지 하나로 알립니다. 리소스 색상 범례는 떠 있는 패널이나 아래쪽 패널이 아니라 구독 경계 옆 2D 캔버스에 직접 그립니다. 지도 맞춤은 범례가 들어갈 가로 공간을 예약합니다. 고정된 범례 상자, 제목 또는 색상 견본 없이 리소스 타입 이름을 표시합니다. 타입 이름은 이동과 함께 움직이고 읽을 수 있는 범위 안에서 지도 확대 및 축소에 비례해 조정됩니다. 좁은 캔버스에서는 이 보조 색상 범례를 생략하고 리소스 지도에 전체 너비를 제공합니다. 약어, 라벨, 선택 상세 정보 및 관계 인덱스는 동일한 식별 정보를 유지합니다.
...
관계 범례는 간결한 캔버스 컨트롤로 유지합니다. 리소스 지도는 관계 표시가 켜진 하나의 고정 직교 2D 변환 결과를 사용합니다. 포함 관계는 흐린 점선 링크로, 첨부 및 의존성은 각각의 방향 표시 방식으로 나타내며 각 리소스를 색상과 약어를 함께 사용하는 평면 도형으로 렌더링합니다. 원근 크기 조정, 반사 표면, 카메라 회전 또는 대체 카메라 보기는 제공하지 않습니다. 단순 변환 결과는 선택된 단일 범위를 포함한 모든 리소스 그룹 패널의 크기를 관측된 하위 리소스 수에 따라 정하고 균형 잡힌 작업 영역에 패널을 배치합니다. 포커스 서비스 및 리소스 그룹 화면은 전체 구독 프레임이 아니라 다시 배치한 내용에 맞춰 표시합니다. 리소스 노드는 표준 Event Grid 토픽 블록보다 작게 렌더링되지 않습니다. 인벤토리에 맞춰 작업 영역과 캔버스가 커지며 작성된 중첩 배치는 제공된 형상을 유지합니다. 지도는 작업 공간 전체 너비를 사용하고 점검 상세는 아래에 배치합니다. 좁은 화면에서는 상자를 읽을 수 없게 줄이는 대신 읽을 수 있는 최소 배율을 유지하고 지도 이동을 사용합니다. 선택은 인벤토리를 다시 불러오지 않고 정본 딥 링크를 갱신하며 기술 식별자보다 방향 관계를 먼저 표시합니다. 선택 중에는 모든 공통 리소스 좌표를 유지하면서 보조 이웃만 표시합니다. 관련 없는 리소스는 흐리게 처리하지 않으며 선택된 외곽선과 점검 상세만 사용해 선택을 나타냅니다. 가상 머신을 포함한 모든 리소스 선택은 현재 지도 배율과 위치를 유지합니다. 확대 및 축소, 맞춤, 이동은 운영자가 명시적으로 조작할 때만 변경됩니다.

별도의 `Network` 모드는 완전한 인벤토리 응답을 권위 있는 상태로 유지하고 자체적으로 범위가 제한된 2D 포커스를 사용합니다. 리소스를 선택한 경우 해당 VNet을 선택하고, 선택한 리소스가 없으면 관측된 서브넷 포함 관계가 가장 많은 VNet을 선택합니다. VNet 및 서브넷 사각형은 타입이 지정된 `contains` 링크로 다시 만들며 리소스 배치는 아래의 제한된 고유 서브넷 첨부 규칙만 사용합니다. 범주 필터 6개, 선택 및 경로 강조는 표현만 바꿉니다.
...
사실 개수와 점검 인덱스는 계속 완전한 권위 있는 인벤토리를 사용합니다. 2D
개요는 네트워크 인터페이스와 managed disk를 표시하고 진단, certificate 및 프로바이더 보조 로직
리소스를 접는 표현 전용 변환 결과를 적용합니다. 표시된 각 소유자는 접힌 이웃
...
부풀리지 않습니다. Virtual Network와 서브넷은 중첩된 평면으로 렌더링하므로 컴퓨팅, 데이터 및 게이트웨이 노드를 네트워크
경계 안에서 읽을 수 있습니다. Azure 인벤토리는 VNet 페이로드 안에서 관찰된 서브넷만
...
리소스 지도 렌더러는 VNet을 바깥 평면으로, 서브넷을 표시된 구성원 수에 따라 크기가 정해지는 안쪽
평면으로 그립니다. 근거에서 파생한 구성원 선과 직접 `attached_to` 링크는 같은 2D 평면에
유지하고 `depends_on` 화살표는 구분되는 방향 표시 방식을 유지합니다. 평면 이름은 떠 있는 라벨 카드
없이 작업 영역 축을 따릅니다. 평면을 선택하면 동일한 리소스 상세 패널을 사용하며 가장 작은 포함
평면이 포인터 대상으로 유지됩니다. 포커스 서비스 또는 리소스 그룹 화면은 공간이 허용될 때
네트워크 평면 3개를 한 행에 배치하는 넓은 배치 대상을 사용합니다. 완전한 인벤토리 화면보다
작은 데스크톱 범례 예약 너비와 캔버스 높이를 사용합니다. 좁은 화면에서는 읽을 수 있는 최소 노드
배율을 유지하고 캔버스를 520 px로 제한하며 더 넓어진 평면을 이동으로 탐색합니다.

서브넷 안에서 표시되는 경로 참여자는 관찰된 `attached_to` 구성 요소로 묶여 네트워크 경계부터 공용 IP 및 보안 리소스, 네트워크 인터페이스, 워크로드, 디스크와 데이터 리소스 순서로 연속 배치되며 이는 추론한 트래픽 방향이 아닙니다. 각 구성 요소는 순서가 있는 레인을 사용하고 겹치는 서브넷 내부 간선은 하나의 평면 중심선을 공유하며 평면을 가로지르는 연결만 직접 경로를 유지합니다. 워크로드는 보조 리소스보다 크게 표시됩니다. 밀집된 개요에서는 글리프가 보조 라벨을 대체하고 선택 및 포커스 화면은 이름, 유형 및 서브넷 라벨을 복원합니다. 직교 변환 결과는 같은 작업 영역 거리를 같은 화면 배율로 유지하고 리소스 높이를 무시하므로 가까운 리소스와 먼 리소스를 같은 기준으로 비교할 수 있습니다. 확대 및 축소는 512배까지 상세 탐색을 지원하고 포인터를 중심으로 확대하며, 내용에 따라 정해지는 작업 영역은 고정 캔버스 높이 상한 없이 확장됩니다. 맞춤은 간결한 2D 작업 영역을 가운데에 배치합니다. 내용에 따라 정해지는 데스크톱 캔버스가 변환된 작업 영역보다 훨씬 높으면 작업 영역을 접기 아래에 가운데 정렬하지 않고 첫 화면에 위쪽 경계를 고정합니다. 맞춤은 완전한 프레임을 복원하는 명시적 컨트롤로 유지됩니다. 왼쪽 버튼과 가운데 버튼 끌기는 모두 2D 작업 영역을 이동합니다. 오른쪽 버튼은 브라우저 동작을 유지합니다. 끌기 입력은 같은 애니메이션 프레임 병합을 사용하고 포인터가 움직이는 동안에만 라벨을 미룹니다.
...
캔버스는 포함 관계를 흐린 점선 중심 간 간선으로 렌더링합니다. 의미 관계는
같은 2D 평면에서 방향이 있는 노드 간 화살표를 사용하며 리소스 그룹 영역을 운영
