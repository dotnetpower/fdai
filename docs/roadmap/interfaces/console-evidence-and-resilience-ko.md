...
직접 맥락만으로 예산을 넘으면 표현 범위에 지도에서 제외된 직접 관계 수를 표시하고, 상세 패널은
...
경로 추적은 범위가 제한된 Network 개요가 아니라 반환된 완전한 근거 그래프를 항상 사용합니다.
경로를 찾으면 정확한 Resource, 상위 경계 및 저장된 관계를 표현에 추가합니다.
범주 필터는 이 복원보다 먼저 주변 맥락에 적용되므로, 상세 패널이 경로를 찾았다고 표시하는 동안
현재 경로의 hop을 지도나 정제된 내보내기에서 제거할 수 없습니다.
