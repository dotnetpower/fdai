...
| Ownership proposal and matching-merge coordination | in-progress | `ownership_coordination.py`; `test_ownership_coordination.py`; signed merge intake in `services/document-ingestion-api/src/fdai_ingestion_api_service/adapters/stewardship.py` | The coordinator opens one idempotent shadow draft, binds the case, PR ref, and candidate digest, rejects mismatched merge content, records the ownership effect, and publishes one typed shadow IAM request. Production composition and retained delivery receipts remain open. |
...
- [ ] Bind the implemented digest-bound ownership coordinator to the production GitOps publisher and signed merge record consumer, then retain a restart-safe receipt proving only the matching merge advances the case.
