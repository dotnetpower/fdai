...
1. **Structured extraction** (`extractor.py`). The deterministic path accepts only the explicit
  registration form `Agent: <name>; responsibility: <value>; subject: <kind>; identity: <display
  name>`. It validates the canonical agent, responsibility, subject kind, and source span without
  interpreting prose. Identity text cannot add another agent or change responsibility, and
  malformed or unknown assignments are ignored.
...
of free-form structure. Upstream production keeps
the abstaining implementation unless a mixed-model binding is explicitly supplied;
structured extraction and Graph resolution still run. All seams are async and
