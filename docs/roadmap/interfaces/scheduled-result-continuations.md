...
| PostgreSQL anchor persistence | implemented | `services/core-control-plane/src/fdai/delivery/persistence/postgres_scheduled_continuation.py`; `services/core-control-plane/tests/persistence/test_scheduled_continuation.py` | Row codecs, restart reads, idempotent create and audit retry, concurrent compare-and-set expiry, and winner-only expiry audit pass two focused cases with zero skips against a disposable supported PostgreSQL database. |
