...
| Durable principal-scoped history and complete-history assembly | implemented | Conversation history providers; `services/core-control-plane/tests/persistence/test_postgres_user_context_latest.py`; `services/core-control-plane/tests/persistence/test_postgres_conversation_images.py`; `services/core-control-plane/tests/persistence/test_conversation_search.py`; Operator conversation application | Restart reads, principal isolation, complete turn history, latest context, first questions, images, and search pass focused PostgreSQL tests. A governed long-session receipt remains open. |
