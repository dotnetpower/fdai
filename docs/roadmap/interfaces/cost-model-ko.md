...
> **과거 계획 예시 - 배포 견적 아님.** 아래 가격 band는 초기 최소 세트의 planning 스냅샷이며
...
|---|--------|----------|---------------------|----------|------|
| 1 | Container Apps 환경 | 환경 fee = $0; vCPU-초 + GB-초 소비 | **현재 계획으로 재산정** | variable | Core 복제본 floor와 명시적 선택 앱 수에 따라 달라짐 |
...
- Azure Bot Free tier는 downstream에서 Teams 채널을 선택할 때 별도 제공하며 upstream
 Terraform 기본 배포에는 포함되지 않습니다.
- Static Web Apps Free tier (읽기 전용 콘솔 호스팅).
- App registration + 워크로드 신원 federation.
- Diagnostic Settings 포워더 자체 (비용은 Event Hubs 행에 있음).

## 월간 묶음 (초기 planning 스냅샷, 모델 비용 제외)
...
| **Variable** | 초기 scale-to-zero 가정의 Container Apps/Jobs + Key Vault + 로그 Analytics | **≈ $6 - $28** |
| **합계 (초기 최소 세트 예시)** | 현재 Terraform topology의 견적이 아님 | **≈ $53 - $80 / 월 (historical)** |
...
Azure OpenAI, 문서 인제스트, Operator API/콘솔, email 채널은 각각 별도 줄 item입니다.

### 현재 Terraform 인벤토리 조정

| 범위 | 현재 리소스 | 견적 처리 |
|------|------------|-----------|
| 기본 | Container Apps 환경, 코어 복제본 1개, scheduled jobs, Event Hubs, Event Grid 인벤토리 구독, PostgreSQL, Key Vault, identities, 로그 Analytics/Application Insights, ACR, canary | 모든 활성 SKU와 복제본/리소스 사용량을 계획에서 다시 계산 |
| 운영 delta | zone-redundant PostgreSQL HA, 35일 geo 백업, 비공개 networking/DNS 및 비공개 실행기 경로 | dev B1ms band에 포함하지 않고 별도 계산 |
| `enable_llm` | Azure OpenAI/Foundry account와 기능 배포 | 토큰/PTU 및 임베딩 사용량을 모델 예산에 합산 |
...
| 채널/콘솔 명시적 선택 | Operator API/채널 앱, Static Web Apps, ACS Email/SMS 등 활성 어댑터 | 실제 활성화와 전송량 기준으로 별도 계산 |
...
그 모델 선택과 예산 gate를 관장합니다. 별도 보고 이유:
...
설정된 가격은 내부 예산 gate에 계속 사용할 수 있지만, 리전 및 협상 요율이 다르므로 콘솔은
...
|----------|-----------------|-------------------|-----------|
| **베이스라인 (≤10 k 이벤트/월)** | 현재 계획 + 측정 사용량 | 코어 복제본 floor, standing 서비스 | 활성 최소 세트와 예산 검증 |
| **10 k - 100 k 이벤트/월** | 계획 + telemetry로 재산정 | 로그 Analytics 인제스트, Container Apps 컴퓨트 | 티어 유지; 로그 Analytics **daily 상한** 설정; 인제스트 예산 알림 감시 |
| **100 k - 1 M 이벤트/월** | 계획 + telemetry로 재산정 | 로그 Analytics 인제스트, Container Apps 컴퓨트, PostgreSQL 저장소 | 감사 스트림에 **Basic Logs** 고려, PostgreSQL 저장소 티어 업, 코어 복제본/리소스 sizing 검토 |
| **≥ 1 M 이벤트/월** | 재모델링 | 대부분 행 | 인벤토리 리뷰 재실행; Event Hubs 추가 TU 또는 Dedicated, PostgreSQL General 용도, 전용 vector 저장소 평가 |

승격 트리거(코어 복제본/리소스 sizing, PostgreSQL 티어 업, 로그 Analytics 분리)는
...
| 옵션 | 절감 | Trade-off |
|------|------|-----------|
| PostgreSQL **Reserved Instance / 절감 계획** (1년 또는 3년) | 컴퓨트 30-55% 감소 | 티어 약정; 다운그레이드는 조기 해지 필요 |
| **로그 Analytics daily 상한** | 폭주하는 인제스트 월 방지 | 상한 초과 로그는 워크스페이스 정책에 따라 드롭 또는 스로틀 |
...
| **복제본 floor를 워크로드별로 설정** | Scheduled 작업은 실행 사이 0; 코어는 기본 1 | Core를 0으로 내리려면 Event Hubs lag scaler와 wake-up 검증 필요 |
...
Scale-to-zero는 조건을 충족한 작업/lane의 목표이지 현재 코어 기본값이 아닙니다. 일괄 min-replicas = 0 은 긴급 복구 의 MTTR 에
...
인시던트, 활성 event storm(cold 시작 로 serialize 될 교정 burst), 그리고
off-hours(콘솔에 이미 warm 한 사람이 없어 autonomous 복구 가 유일한 fast 경로)
- 에만 **warm** lane (min-replicas > 0)을 권고하고, scaler와 wake-up 경로가 검증된 나머지 lane은 scale-to-zero 에
남긴다. 임계값 는 fork-tunable 구성 이고, 정책은 순수 권고다: 배포 layer
가 계획 시간 에 `min_replicas` floor 를 읽고 런타임 이 액션 등급 별
...
- **DR / secondary-region 리소스** - 현재 최소 인벤토리 밖이며 별도 배포 topology와
 계획으로 산정합니다.
...
- [ ] 최소 세트 내 구체적인 티어 값(PostgreSQL 저장소, 로그 Analytics daily 상한, ACR 보존
