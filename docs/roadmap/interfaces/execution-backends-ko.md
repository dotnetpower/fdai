...
모든 요청에는 고정된 멱등성 키, 변경할 수 없는 산출물 다이제스트, Thor 소유자 trace, stop 조건,
...
list에서 활성화된 프로파일 id를 선택합니다. Unknown 필드, unknown 활성화된 id, 중복 값, malformed
...
워크로드 id, 네트워크, 자격 증명 ref, 지역, 범위에서 검증된 권한의 subset이어야 합니다.
Workspace 순위와 모든 numeric 상한은 같거나 낮아야 합니다.

요청, 생성된 작업, installed skill, 프로파일 file, downstream 분포는 명령, 자격 증명,
...
멱등성 키로 식별하며 변경할 수 없는 요청 근거, 프로바이더 ref, 상태, 취소 의도, 정리
...
- **취소 race**: 프로바이더가 관측한 최종 success 또는 실패를 취소된으로 바꾸지 않습니다.
...
제공할 수 없습니다. 어댑터는 빈 시작 본문을 보내고 서버가 소유한 템플릿 map에서 작업 리소스를
해석합니다.
...
증적 호출은 ARM host와 작업 실행 경로를 검증합니다.
...
- **실패 자세**: 모호한 제출 또는 상태는 최종이며 운영자 review가 필요합니다.
 Circuit-open health는 healthy-by-default가 아니라 사용 불가입니다.
- **정리 자세**: 로컬 증적을 release하고, VM 실행 Command 리소스는 기존 취소 경로로
...
| 로컬 및 deployed parity | [런타임 Parity](../deployment/dev-and-deploy-parity-ko.md) |
