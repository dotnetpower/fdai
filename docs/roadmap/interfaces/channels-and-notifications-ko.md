...
translation_source_sha: 4c8b4617e34c197d00ee6cf2fc0dbb78ab923b46
...
| Teams, Slack 및 아웃바운드 알림 어댑터 | in-progress | [`teams_adapter.py`](../../../services/core-control-plane/src/fdai/delivery/chatops/teams_adapter.py), [`slack.py`](../../../services/core-control-plane/src/fdai/delivery/notifications/slack.py), [`slack_transport.py`](../../../services/core-control-plane/src/fdai/delivery/channels/slack_transport.py), [`test_teams_adapter.py`](../../../services/core-control-plane/tests/chatops/test_teams_adapter.py), [`test_adapters.py`](../../../services/core-control-plane/tests/notifications/test_adapters.py), [`test_slack_transport.py`](../../../services/core-control-plane/tests/delivery/channels/test_slack_transport.py) | Teams는 `HilChannel`을 구현하고 Slack은 아직 조립되지 않은 운영 A3 전송을 구현했으며 A2/A4 알림 adapter는 집중 테스트를 통과했습니다. Slack `HilChannel`, A1 callback, Entra 재인증 흐름, Teams A3 전송 및 운영 A3 런타임은 없습니다. |
...
| 순수 채널 표현 렌더링 | 구현됨 | `shared/providers/channel_presentation.py`, `delivery/channels/`, `test_presentation_renderers.py`, 집중 채널 검사 40개 통과 | 정규화된 묶음 하나가 정본 텍스트, 사실, 제한, 근거, 권한 및 사용 불가 상태를 보존합니다. 순수 Teams 및 Slack 페이로드 builder와 가짜 사용자 지정 렌더러가 전송 또는 확인 응답 없이 기능 상한을 강제하고 별도 Slack 전송이 이를 변경 없이 사용합니다. |
...
- [ ] 인증된 Slack 및 Teams A3 유입과 범위가 제한된 rich-response 발행기를 추가하고
  signature, service identity, principal 해석 및 확인 응답 집중 테스트를 통과합니다.
- [ ] `ProductionChannelRuntime`, 운영 ASGI factory, 서비스 entry point 및 Terraform workload를
...
`ProductionChannelRuntime`은 standalone 채널 게이트웨이 프로세스를 위한 계획된 library
런타임입니다. 읽기 전용 콘솔 API에 mount되지 않고 실행기 신원을 받지 않습니다.
저장소는 아직 이 런타임, 운영 ASGI factory 또는 Terraform workload를 제공하지 않습니다.
