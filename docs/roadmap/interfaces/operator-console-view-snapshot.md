...
| 2026-08-28 | implemented | Closed two Console-side gaps against the section 13.4 `ViewSnapshot` boundary. `backend-context.ts` spread the whole `ViewSnapshot`, including `contextIdentity`'s principal, scope digest, and release digest fields, into the narrator `view_context`; it now forwards only the opaque `selectionToken` and other non-identity view fields through a `narratorViewFields` helper. Separately, `ontology-instances.tsx`'s exact-screen selection identity built `resourceIds` from every fetched resource, including hidden `authorization.role-assignment` resources the same screen already excludes from rendering; the identity builder moved to `ontology-instances.model.ts` as `ontologyInstanceContextIdentity` and now filters through the existing `isOntologyInstancePresentationResource` guard before mapping ids. | `current change`; `console/src/deck/backend-context.test.ts` (`22 passed`) and `console/src/routes/ontology-instances.model.test.ts` (`30 passed`); `npm run typecheck` clean. | Retain governed cross-screen and shadow-qualification evidence before promotion; no authority or contract-shape change. |
| 2026-08-28 | implemented | Corrected the selection-token boundary in the authoritative issuer. The Operator projection now uses the shared unselectable-resource type set before computing the selection digest and issuing the opaque token, so resolving the token cannot restore a hidden `authorization.role-assignment` id that the browser filtered locally. | `current change`; focused Operator inventory-instance projection checks (`8 passed`); Ruff and strict mypy. | Retain governed cross-screen evidence before promotion; no live service was queried. |
