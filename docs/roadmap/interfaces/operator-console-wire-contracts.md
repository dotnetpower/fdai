...
| Slack browser handoff | `POST /hil/slack/interaction`; `GET/POST /hil/slack/handoff/{nonce}` | The outbound `fdai_hil_approve` and `fdai_hil_reject` buttons carry only the original approval id; dispatch identity is separate and digest-bound. Verify Slack v0 over exact form bytes and a five-minute timestamp window before extracting `team.id` and `user.id`. One click gets one nonce digest bound to original approval, action, workspace, user map revision, and expiry. The signed receiver returns a server-origin Console URL. The browser preview supplies no authority; POST accepts only justification, requires a verified API access token with `auth_time` after the click and a current exact mapping, atomically consumes the nonce, and calls the same decision service. Missing claim, authority, context, or mapping holds the decision. Live Slack delivery and Entra optional-claim configuration remain external. |
