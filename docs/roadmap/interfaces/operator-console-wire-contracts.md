...
ordered local projection and authenticated GET route; its one-minute lease grants no approval or
Incident creation authority. The shared Kafka allowlist adds only this subscription and its
sanitized DLQ, never a proposal, approval or mutation publisher. See the
[observer delivery contract](../architecture/aks-outbound-connector.md#operator-delivery-contract).
...
The same terminal requires `pantheon_prompt_profiles`. `answer_participants` carries at most three
agent names, prompt versions, situation keys, and SYSTEM SHA-256 values. `evaluator_profiles`
carries at most three profile identities, versions, digests, SYSTEM SHA-256 values, and bounded
...
| Slack browser handoff | `POST /hil/slack/interaction`; `GET/POST /hil/slack/handoff/{nonce}` | Verify Slack v0 over exact form bytes and a five-minute timestamp window before extracting `team.id` and `user.id`. One click gets one nonce digest bound to original approval, action, workspace, user map revision, and expiry. The authenticated browser preview supplies no authority; POST accepts only justification, requires a verified API access token with `auth_time` after the click and a current exact mapping, atomically consumes the nonce, and calls the same decision service. Missing claim, authority, context, or mapping holds the decision. This route does not claim live Slack delivery or Entra optional-claim configuration. |
...
The `logical_service_current_state` answer shape presents the exact BusinessService, Workload, and
runtime Resource mapping plus fresh verified component states. Empty or incomplete mappings remain
explicit. The answer never derives aggregate service health or cause and always keeps
...
| Runtime impact | Schema `1.1.0` adds `relationship_evidence_complete`, `relationship_source_coverage`, and each edge's bounded `evidence` object to the `1.0.0` envelope. Both versions retain `schema_version`, exact ontology release, `source_generation`, `source_cutoff`, exact target, traversal depth and ordered LinkTypes, reached nodes, traversed edges, affected count, query completeness, depth/edge truncation reasons, and both authority flags fixed to `false`. Every edge carries visible `verification_status`; an optional map must match the same snapshot generation or cutoff and contain every Resource and edge in the exact simulation result. Missing or incomplete relationship-source coverage keeps configuration evidence unavailable for verification while preserving its bounded provenance fields and an exact recovery reason. |
...
| Conversation assurance terminal envelope | implemented | Core semantic turn processor; Operator semantic turn runtime and presentation; focused assurance transport tests | Valid assessments preserve bounded reasons and exact answer-generation mode. Only `completed` emits `answered`; `deferred`, `held`, and legacy `unavailable` emit `held`. Opt-in model calls may add a bounded prompt profile, SYSTEM digest, ordered layer, and budget manifest without raw prompt text or authority. Pantheon assurance terminals also carry bounded content-free participant and evaluator prompt profiles for the expanded Run Record. Assurance projection failures use `semantic_runtime_failed`; only failures in a `result_store_*` stage use `semantic_result_store_unavailable`. |
...
| Slack browser handoff | in-progress | `families/iam/slack_handoff.py`; `test_hil_slack_handoff.py`; `console/tests/e2e/approvals-sample.spec.ts` | Signed interaction and one-use persisted actor/action context have focused synthetic coverage. Decision fails closed without a signed fresh `auth_time`; authenticated Entra, actual Slack posting and a real PostgreSQL race receipt remain unverified. |
...
| 2026-09-19 | implemented | Added required content-free Pantheon participant and evaluator prompt-profile evidence to the assurance terminal and expanded Run Record. | `current change`; focused Core projection, Operator validation, Console parser, persistence, presentation, browser-worker, Ruff, strict mypy, and typecheck checks. | Retain one fresh authenticated Run Record on a later clean committed revision. No raw prompt, live score, or qualification evidence is claimed. |
| 2026-09-19 | implemented | Added an optional content-free prompt replay manifest to each captured semantic model call so the Console can verify dynamic SYSTEM profile, ordered layer, digest, and budget evidence without receiving raw hidden prompt text. | `current change`; focused Core semantic projection and Console model-trace parser and presentation tests; Ruff, strict mypy, and typecheck. | Retain a fresh authenticated browser Run Record on a later clean committed revision; no live prompt-quality or qualification claim is part of this change. |
