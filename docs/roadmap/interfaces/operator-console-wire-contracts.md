...
| Conversation assurance terminal envelope | implemented | Core semantic turn processor; Operator semantic turn runtime and presentation; focused assurance transport tests | Valid `completed`, `deferred`, and `held` assessments preserve bounded reasons and exact answer-generation mode. Assurance projection failures use `semantic_runtime_failed`; only failures in a `result_store_*` stage use `semantic_result_store_unavailable`. |
