...
The store writes the terminal attempt before the completion sink runs. The sink appends one
...
Task and progress rows expire under the task retention policy; progress rows cascade with attempts.
...
coalescing, bounded concurrency, timeout, shutdown, process-loss reconciliation, live PostgreSQL
migration and restart, immediate HTTP response, RBAC, cross-owner hiding, SSE completion, isolated
backend inputs, and idempotent conversation handoff.
