...
> Direct and streamed read investigations use a separate Core-owned run ledger and append-only
> progress store. They share the planner and read provider but do not reuse this detached attempt
> state machine.
...
The cross-process read-investigation cancellation command addresses the canonical task identity.
Core routes it to either the interactive run ledger or this detached task store and preserves one
writer for the selected state machine. Closing an Operator SSE response is not a cancellation
command. It detaches only the delivery subscriber, while an active durable run or task continues.
...
| Completion sink and durable Web conversation handoff | implemented | Core EventBus completion sink; `read-investigation-completion` `1.0.0`; Operator completion repository, consumer, conversation writer, retention worker, and migration grants; focused completion and privilege checks (`24 passed`) | Core publishes one immutable terminal result from its existing leased outbox. Operator production composition atomically accepts the exact proposal-bound inbox record, writes one idempotent Web assistant turn, and purges deadline-expired inbox rows. Channel outbound enqueue and governed restart evidence remain open. |
...
| 2026-09-12 | implemented | Reconciled the composed versioned completion handoff, idempotent Web turn, and deadline-ordered inbox retention against the current production bindings. | `current change`; completion transport, Operator runtime/store, and writer-grant tests (`24 passed`, zero skips). | Add verified channel outbound enqueue and retain governed retry, restart, and telemetry evidence. |
...
- [x] Purge retained completion inbox rows at their contract deadline through the bounded
  Operator-owned worker and inbox-only delete grant (`24 passed`, zero skips).
- [ ] Add channel binding resolution and durable outbound enqueue, then retain governed retry and
  restart evidence.
