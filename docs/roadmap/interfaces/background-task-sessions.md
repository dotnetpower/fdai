...
| Completion sink and durable conversation handoff | in-progress | `services/core-control-plane/src/fdai/core/background_task/completion_sink.py`; `services/core-control-plane/tests/core/background_task/test_completion_sink.py` | `ConversationCompletionSink` appends the deterministic, provenance-labeled, untrusted turn and submits the immutable reply through the durable delivery ledger without rerunning the investigation. Replay reuses the same turn and delivery record. Completion audit markers and production composition are not implemented. |
...
- [ ] Write the `background-task.completed` and `background-task.delivery-enqueued` audit events through durable single-write markers and bind the sink into production composition.
