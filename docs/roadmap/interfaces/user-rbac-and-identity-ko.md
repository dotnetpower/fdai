...
- [ ] 전체 WCAG/UI 점수를 주장하기 전에 명시한 브라우저/스크린 리더 조합으로 실제 영문/한국어 안내를 보존합니다. 평가 결과는 `needs-human`이며 [운영 기준](human-agent-assignment-implementation-plan-ko.md#완료-정의)은 열린 상태입니다.
...
> 검증합니다. Slack 단추는 승인 ID만 전하고 서명된 클릭은 일회용 브라우저
> 인계로 이어집니다. 현재 매핑과 클릭 이후의 서명된 Entra API 토큰 `auth_time`이
> 없으면 결정하지 않으며 기존 토큰의 조용한 갱신을 재인증으로 보지 않습니다.
...
Console은 Owner 전용 명단과 요청 조회가 끝나기 전에 검증된 `GET /iam` 변환 결과를 표시합니다. 해당 조회에 의존하는 탭은 skeleton을 유지합니다. 인증된 API 클라이언트는 중복 명단 조회를 하나로 합치고 성공한 결과만 메모리에 30초 동안 보관합니다. 공유 HTTP나 브라우저 저장소에는 보관하지 않으며 역할 또는 기능 결정은 캐시하지 않습니다.
