...
Semantic request contract는 ordinary role을 최대 4개까지 수락하고 arbitrary Kafka topic을 거부하며,
두 Operator bridge worker가 모두 active인 동안에만 ready를 보고합니다.
...
[`RoleResolver`](../../../services/core-control-plane/src/fdai/core/rbac/resolver.py)가 구현. 이 verifier는
...
subject를 다시 검증하고 변경 불가능한 역할, duty, 목표 및 사유 의도를 기록합니다. 리비전 기반
