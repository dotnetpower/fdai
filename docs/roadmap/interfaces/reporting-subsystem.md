...
| 2026-09-21 | implemented | Moved PostgreSQL chaos-reader construction behind the reporting projection factory so the current Operator composition root stays within its reviewed import ceiling. | `current change`; reporting projection factory, current composition binding, focused projection and composition tests, and the Operator boundary gate. | No additional composition work remains for this binding. |
