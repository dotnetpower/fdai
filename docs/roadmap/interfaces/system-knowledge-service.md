...
- **Deployment cadence:** FDAI design and implementation knowledge can refresh without redeploying
  ...
  replacement. Any cited owner-document edit must regenerate the packaged catalog through the
  canonical compiler before delivery; hand-edited digest pins are invalid.
...
Pantheon producer corrections, including Loki's validated `object.resilience-score` path, refresh
citations and digests only. The catalog neither consumes the normalized Event nor creates a runtime
candidate.
