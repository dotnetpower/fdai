...
| T1 clarification evaluation | implemented | `semantic_planning_models.py`; `semantic_planning_cascade.py`; focused tier-routing and prompt checks | Frame proposals classify missing user context with bounded `clarification_requirements`. A legitimate T1 clarification terminates without T2. Requests for server-bound context, ambiguous or mixed targets, invalid schemas, and deterministic frame-plan mismatches also fail closed without T2; only typed T1 unavailability may use the interactive runtime's bounded fallback. |
