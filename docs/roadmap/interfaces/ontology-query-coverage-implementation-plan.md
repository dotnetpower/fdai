...
|------|---------------------------------|----------------------------|
| Conversation routing | `ConversationCoordinator` uses `_VERB_PATTERNS`; `ReadPlanNarrator` may propose two or three canonical command strings, which `execute_read_plan` runs serially. | No server-side semantic problem frame, intent graph producer, dependency-wave executor, or typed plan bridge. |
