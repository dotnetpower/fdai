...
| Operator persistence and Rule projection | implemented | `semantic_turn.py`, `semantic_turn_runtime.py`, `postgres_semantic_turn_store.py`, `test_semantic_turn_bridge.py`; focused semantic tests passed 88 cases and a rollback-only PostgreSQL transaction probe passed | A valid caller-supplied request UUID is preserved through the semantic envelope and correlation identity while the idempotency key remains separate. An omitted request UUID uses the retry-stable deterministic fallback. Outbox and result leases are recoverable, malformed ownership fails closed, replay ordering is timestamp-aware, and exact Rule reads are isolated by principal and query digest. |
...
| Incident semantic evidence and answer | implemented | `core/incident/ontology_projection.py`; `core/ontology_platform/incident_queries.py`; `semantic_turn_processor.py`; `semantic_turn_presentation.py`; focused Core (`36 passed`), Operator (`46 passed`), cross-service (`1 passed`), and Console (`74 passed`) checks | Canonical Incident state is queryable as an ObjectSet and `query.incident_evidence` preserves distinct canonical incident and audit correlation identities. The answer exposes verified records, explicit causal and evidence limitations, and a read-only evidence-collection next step while exact output remains collapsed and `execution_authority=false`. An authenticated Browser observation passed but wasn't retained as governed evidence, so this row doesn't claim `validated`. |
...
- [x] Constrain the incident answer to profile, correlated evidence, explicit gaps, and a read-only evidence-collection next step while preserving `execution_authority=false` and exact technical output.
