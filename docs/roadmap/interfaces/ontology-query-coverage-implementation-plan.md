...
- [ ] Expose incident competency to the semantic query manifest: project `Incident` instances for `ObjectSet` selection and register a read-only incident-evidence `FunctionType`, so a bound incident investigation can reach a verified plan instead of one clarification.
