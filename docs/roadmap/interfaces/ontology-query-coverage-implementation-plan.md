...
| Incident semantic evidence | in-progress | `core/incident/ontology_projection.py`; `core/ontology_platform/incident_queries.py`; focused Incident and semantic-composition checks (`62 passed`) | Canonical Incident state is queryable as an ObjectSet and `query.incident_evidence` returns an audit-backed profile, correlated records, explicit gaps, and no cause authority. The A3 answer contract and authenticated Console evidence remain open. |
