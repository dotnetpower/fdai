...
| Current instance relationship query | implemented | `instance_relationships.py`, `semantic_instance_relationship_projection.py`, `test_instance_relationships.py`, `test_wire_ontology_relationships.py`, `test_semantic_turn_processor.py`; focused relationship checks pass | A schema-constrained plan selects exact LinkTypes, materializes one bounded secured ObjectSet, and invokes a receipt-authenticated exact-release function. Empty results are closed only for a complete snapshot, and the localized answer grants no execution authority. |
...
| 2026-08-14 | implemented | Added the generic current-instance relationship path. The planner selects exact release LinkTypes, the executor reads stored directed links only through an issued secured ObjectSet receipt, and Core rejects release, receipt, bound, or authority drift before localized rendering. | `current change`; focused instance evaluator, semantic composition, prompt contract, and Core processor checks pass. | Restart an exact-source stack and retain an authenticated relationship answer before running the full bilingual cohort. |
...
| Relationships | Every directed LinkType projects deterministic `<name>.outgoing` and `<name>.incoming` machine query ids with endpoint, cardinality, causal, transitive, and temporal metadata. Current instance questions can select exact LinkTypes and filter stored directed rows through a receipt-authenticated complete snapshot. | Typed multi-hop endpoint-side traversal and governed authenticated evidence remain. |
