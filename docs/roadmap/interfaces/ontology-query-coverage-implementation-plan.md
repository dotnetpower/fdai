...
| Incident semantic evidence | implemented | `core/incident/ontology_projection.py`; `core/ontology_platform/incident_queries.py`; focused Core, Operator, and Console checks (`138 passed`) | Canonical Incident state is queryable as an ObjectSet, T0 records one bounded finding-severity impact row without inventing measurements, and `query.incident_evidence` preserves distinct canonical incident and audit correlation identities while returning an audit-backed profile, correlated records, a citation-grounded recorded root cause, impact evidence, citations, and explicit gaps. The path remains read-only and grants no execution authority. Authenticated Console evidence remains open. |
...
| 2026-08-17 | implemented | Upgraded semantic frame v8 and plan v10 with exact strict-operation fields. Relationship declaration counts use `LinkType`; declared resource-type filters use `Resource.type` with the exact `exists` predicate; generic causal and evidence-validation questions keep their `Resource` capability families. No phrase or question-id route was added. | `current change`; focused prompt registry and strict contract checks passed 7 cases; task-scoped Ruff passed. | Centrally validate the prompt generation and retain a fresh strict artifact before starting the seeded cohort. |
