...
This plan closes the implementation gap between FDAI's bounded conversation and ontology foundations
and the target non-keyword path for operator questions. It records the verified current baseline, service and agent ownership, dependency-ordered work packages, cutover gates, and rollback units for 100% structural query coverage.

> **Coverage boundary:** 100% means every readable declaration in one active ontology release has a principal-scoped
> query descriptor or a typed unavailable reason. It does not promise a complete or correct answer when identity, provider data, history, or evidence is missing.
>
> **Authority boundary:** Natural-language and embedding output remains candidate-only. Read plans have no execution authority. An explicit change request can create only a typed draft that re-enters
...
| Kubernetes workload target selection and assessment | implemented | `resource-types.yaml`; `metric-semantics.yaml`; `semantic_kubernetes_rollout_planning.py`; `semantic_kubernetes_pod_recovery_planning.py`; exact-release rollout and Pod recovery FunctionTypes; focused checks; authenticated targetless Console receipts | Reviewed Deployment and Pod terms ground targetless S12 and S1 requests in the Resource type value domain. Core returns exact candidates before provider I/O. An exact Deployment compiles the four-node ownership read, while an exact Pod compiles a five-node plan that joins its issued receipt, a reviewed 30-minute same-UID restart delta, and explicit ReplicaSet and Deployment owner receipts. Targetless zero results retain `source_incomplete`; incomplete or ambiguous exact scope stops before the metric read. Recovery requires a positive bounded delta plus fully ready and available owner replicas. Both paths keep root cause and execution authority false. |
