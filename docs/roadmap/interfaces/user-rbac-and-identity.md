...
> Customer-agnostic: all group names, app registration names, and GUIDs below are **placeholders**; a fork supplies the real values via config
...
| Human approval callback identity | implemented | `families/iam/hil_callback.py`; `hil_callback_authority.py`; `hil_decision_outbox.py`; `postgres_iam.py`; focused callback, persistence, Kafka, workflow, and canary tests | Teams requires an API-audience OBO token issued to the configured bot, exact provider-to-Entra mapping, and the separately configured group-connected team and channel. Slack uses a separate browser handoff. Callback decisions recover proposal-first persistence and publish through the durable Operator outbox. BreakGlass never contributes Human approval authority. |
| Slack signed actor handoff | implemented | `families/iam/slack_handoff.py`; `slack_adapter.py`; `test_slack_hil_interactivity.py`; focused Operator/Console and synthetic desktop checks | Action-bound Slack buttons carry only approval identity. A signed click mints a five-minute digest-only, one-use handoff. Decision requires a signed Entra API-token `auth_time` after the click, current mapping and park, current role, and no-self-approval. Missing claims fail closed. Live Slack/Entra and real PostgreSQL nonce-race evidence remain open. |
| Tenant-local Entra bootstrap declarations | implemented | `packages/deployment-cli/src/fdai_deployment_cli/entra_bootstrap.py`; `scripts/deployment/azure/genesis_entra.py`; focused bootstrap and repository-configuration tests (`50 passed`) | The protected bootstrap creates and reads back the API, SPA, and approval-bot registrations, their service principals, five role groups, exact API roles, and client scope bindings without retaining tenant values in source. Conditional Access, Access Reviews, provider consent, Teams installation, and live token/denial evidence remain deployment-owned. |
| Local Browser Entra session resilience | implemented | `console/src/auth-session.ts`; `console/src/auth.ts`; `console/src/console-data-mode.ts`; focused Console auth and data-mode tests and typecheck | MSAL Browser v4 uses encrypted `localStorage` only on loopback origins and keeps deployed origins on `sessionStorage`. One coalesced refresh runs at startup, every 30 minutes, and after focus, visibility, or network recovery. Route and data-mode restoration preserve an MSAL callback hash until `handleRedirectPromise` consumes it. Entra can still require interactive authentication. |
...
| IAM administration diagnostics and request projection | implemented | `entra_directory.py`; `families/iam/iam_routes.py`; `postgres_iam.py`; `console/src/routes/settings-iam*`; `console/tests/e2e/settings-{iam,overlay,production-controls}.spec.ts`; focused Operator, Console, and Browser tests | The Console distinguishes FDAI Owner from tenant administration, renders the primary verified projection before Owner-only Graph reads finish, and projects durable request and review proposals without claiming that approval changed membership. A successful roster read is coalesced and cached for 30 seconds only within the authenticated API client; roles, capabilities, failures, and shared HTTP storage are not cached. |
...
| 2026-09-17 | implemented | Preserved the MSAL callback hash while restoring a persisted Sample preference, preventing route hydration from erasing the authorization code before token exchange. | `current change`; `console/src/app.tsx`; `console/src/console-data-mode.ts`; focused Vitest passed 3 cases; an actual loopback Browser login retained Sample mode and populated the account and token cache. | Retain a governed Browser receipt across a webview recreation or overnight suspension before claiming operational validation. |
| 2026-09-17 | implemented | Decoupled the primary Identity and access render from slower Owner-only roster and request reads, and added a 30-second authenticated-client memory cache that coalesces successful roster reads and evicts failures. | `current change`; `console/src/routes/settings-iam.tsx`; `console/src/api-iam-client.ts`; focused cache and delayed-response checks. | Live latency measurement with a shared authenticated Browser session remains separate; authorization and role decisions remain uncached. |
| 2026-09-15 | implemented | Aligned IAM tabs, directory search, role requests, and review controls with shared Console typography, primary and secondary roles, 44 px mobile targets, and visible keyboard focus without changing identity, capability, review, or executor contracts. | `current change`; issue #1085; `console/src/routes/settings-iam*`; `console/src/styles.css`; focused unit and browser checks included in the 96-unit and 35-Playwright Settings results. | No implementation work remains for the bounded IAM presentation change; deployed identity and effect evidence remain separate. |
| 2026-09-15 | implemented | Reconciled #946's completed protected delivery and hardened the existing handover UI without changing identity, role, review or executor contracts. Checklist operations now match each server-advertised capability; private client/goal state and late replies are fenced. | [PR #1014](https://github.com/dotnetpower/fdai/pull/1014): head `8c1d9977c`, squash `953a17de4`, exact-head CI `34925881557` and post-merge CI `34926168342` passed. `current change`; [#1017 UI record](../../internals/handover-ui-evidence-20260915.md): 28 distinct synthetic browser scenarios, 127 unit checks and TypeScript pass, ten focused plus ten final critiques. | Real screen-reader evidence remains `needs-human`; no final UI score. #458 retains current identity, source, provider, recovery, deployment and independent promotion evidence. |
...
> configured bot client, mapped Entra actor, team-and-channel audience, and current App Roles.
> Slack buttons carry only approval id; a signed click returns a server-origin browser handoff.
> Current mapping and signed post-click Entra API-token `auth_time` gate its one-use decision.
...
The Console renders the verified `GET /iam` projection before Owner-only roster and request reads finish. Tabs that depend on those reads retain a skeleton. The authenticated API client coalesces overlapping roster reads and keeps only a successful result in memory for 30 seconds; it never stores that result in shared HTTP or browser storage and never caches role or capability decisions.
