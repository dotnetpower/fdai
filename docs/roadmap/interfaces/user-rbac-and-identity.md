...
| Human approval callback identity | implemented | `families/iam/hil_callback.py`; `hil_callback_authority.py`; `hil_decision_outbox.py`; `postgres_iam.py`; focused callback, persistence, Kafka, workflow, and canary tests | Teams requires an API-audience OBO token issued to the configured bot, exact provider-to-Entra mapping, and the separately configured group-connected team and channel. The separate Slack browser handoff is in progress below. Callback decisions recover proposal-first persistence and publish through the durable Operator outbox. BreakGlass never contributes Human approval authority. |
| Slack signed actor handoff | in-progress | `families/iam/slack_handoff.py`; `console/src/routes/hil-queue.tsx`; focused Operator/Console and synthetic desktop checks | A signed click mints a five-minute digest-only, one-use handoff. Decision requires a signed Entra API-token `auth_time` later than the click, the current mapping and park, current role, and no-self-approval. A missing claim fails closed. Provider optional-claim configuration, interactive authenticated evidence, and real Slack button delivery are not verified. |
...
> Slack enters a signed receiver and one-use browser handoff. A decision needs the current mapping
> and a signed Entra API-token `auth_time` after the click; missing claims hold the handoff.
