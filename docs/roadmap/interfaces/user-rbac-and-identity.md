...
| Per-user Cost Governance access | implemented | `CostAccessGrant`; `CostDisclosureCeiling`; Cost Governance Operator routes and focused tests | A revisioned, expiring principal grant intersects with the deployment disclosure ceiling for each cost read. The server applies `hidden`, `aggregate`, `masked`, or `detailed` disclosure before serialization; the grant cannot enable the package or promote an action. |
