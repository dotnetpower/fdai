...
| Subscription scope identity | implemented | `fdai_operator_service.adapters.subscription_scope`; `fdai_operator_service.families.conversation.subscription_scope`; focused provider and bridge checks | Current-subscription identity questions are matched from the inventory language catalog before semantic transport, read the server-configured subscription name and state from Azure Resource Manager, render only a masked subscription ID, and persist one verified deterministic receipt. Provider failure returns unavailable and never calls a narrator or T2 model. |
...
| 2026-08-17 | implemented | Reintroduced subscription scope identity as an Operator-owned deterministic read after service decomposition removed the prior route and provider. The route uses catalog terms, server-owned scope, masked identity, a content-addressed receipt, and no model fallback. | `current change`; focused subscription provider, responder, durable bridge, least-privilege Terraform, and Console receipt checks. | Retain authenticated local and deployed evidence. The observed failure that triggered this repair was not an Azure authentication or subscription-access denial. |
