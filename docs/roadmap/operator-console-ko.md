...
translation_source_sha: 14af3d116d4cf7d9f80cd1ba8b04b4f81efb817a
...
오퍼레이터 콘솔은 **자율 SRE agent가 아니다**. AIOpsPilot의 판단 authority
...
콘솔은 AIOpsPilot의 `deterministic-first` 원칙과 rule-catalog collection
계약에 fit 하는 것.
...
`oai-aiopspilot-dev-krc` account에 Azure OpenAI를 바인딩.
...
MCP server는 호출 agent를 인증 (mTLS client cert 또는 `aiopspilot-api`
...
- **OD-C4** - CLI REPL history 파일 위치 및 retention. 기본 제안:
  `~/.aiopspilot/console-history.jsonl`, 10 MiB로 cap, write 전 redact.
  Day 1 구현 blocking.
