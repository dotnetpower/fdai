...
translation_source_sha: 1e434776b2753f774d299815ebb67802d4e37ca3
...
- **교차 테넌트 SharePoint**: `sharepoint_connector_enabled=true`는 FDAI-native 커넥터를 활성화합니다. Azure 인제스트 UAMI가 Microsoft 365 테넌트에 등록된 애플리케이션에 federated assertion을 제공하고, 관리자 동의 후 이 애플리케이션이 Microsoft Graph 토큰을 발급합니다. Terraform은 대상 테넌트, 애플리케이션, 사이트, 드라이브, redirect 허용 목록, 서버 소유 문서 정책만 주입합니다. 보호된 서비스 workflow는 명시적인 `sharepoint_connector_transition`을 요구하고 정확한 활성화 또는 비활성화 환경 차이를 plan 번들에 봉인합니다. Power Platform은 배포하거나 호출하지 않습니다.
