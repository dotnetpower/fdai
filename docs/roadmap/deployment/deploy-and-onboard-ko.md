...
translation_source_sha: 2556ece21373ba60309629ec1b86ab1b49020d6e
...
- 보호된 `fdaictl` 전송 계층은 `dev`와 `staging`을 지원합니다. 요청 식별자는 승인된 테넌트,
  ...
  요청은 `document_ocr_action=preserve`를 고정합니다. 작업 흐름은 이 바인딩을 독립적으로 다시
...
보호된 `history-` 요청 모드는 workflow 입력을 추가하지 않고 초기 범위 제한 운영 이력 계획과
적용을 활성화합니다. Workflow는 sibling job environment 값 대신 이 요청 prefix에서 활성화
flag와 범위 제한 Terraform target을 직접 파생합니다. 이후 일반 배포 계획이 명시적 선택 리소스의
제거를 제안하지 않고 보존하도록 해당 계획 전에 `ENABLE_OPERATIONAL_HISTORY` repository 변수를
`true`로 설정합니다.
...
- **교차 테넌트 SharePoint**: `power_platform_connector_enabled=true`는 커넥터 정책과 허용 목록 값만 인제스트 API에 주입합니다. Microsoft 365 테넌트가 SharePoint와 Power Platform 연결을 소유하고 Azure 테넌트가 다중 테넌트 FDAI API audience를 소유합니다. API는 정확한 외부 테넌트, 승인된 OAuth 클라이언트, `DocumentConnector.Ingest` 권한을 요구합니다. 비밀은 공급자 호스팅 연결 또는 Key Vault에 유지합니다.
