...
translation_source_sha: b27ff70270c131bc31f31f5b8edcd36252684774
...
target으로 지정합니다. 기존 environment, subnet, DNS zone의 안정된 resource ID와 versionless
Key Vault secret URI를 사용하여 관련 없는 compute module drift가 범위 제한 계획에 들어오지
않게 합니다. 이후 일반 배포 계획이 이 명시적 선택 리소스를 보존하도록 초기 계획 전에
`ENABLE_OPERATIONAL_HISTORY` repository 변수를 `true`로 설정합니다.
