...
검증합니다. Service rollback은 immutable snapshot에 없는 post-apply secret name만 제거한 뒤 exact Key Vault reference를 복원합니다. 새 plan 저장 전 runner는 24시간이 지난 allowlisted plan, metadata, source,
