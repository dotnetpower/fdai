...
translation_source_sha: 28bea17c268280d1b8697855735df966fe8e9dd2
...
- **교차 테넌트 SharePoint**: `sharepoint_connector_enabled=true`는 FDAI-native 커넥터를 활성화합니다. Azure 인제스트 UAMI가 Microsoft 365 테넌트에 등록된 애플리케이션에 federated assertion을 제공하고, 관리자 동의 후 이 애플리케이션이 Microsoft Graph 토큰을 발급합니다. 루트 조립은 대상 테넌트, 애플리케이션, 사이트, 드라이브, redirect 허용 목록, 서버 소유 문서 정책만 문서 인제스트 모듈로 전달하며 커넥터 상태를 직접 구성하지 않습니다. Power Platform은 배포하거나 호출하지 않습니다.
