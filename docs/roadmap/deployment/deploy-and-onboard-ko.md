...
translation_source_sha: 8b1230c7008f754667cdf37c11d5f1b14b9627d1
...
| `FDAI_INVENTORY_OPERATOR_REQUESTED` | env | 배포 / one-shot | 명시적으로 요청한 one-shot Job 실행에서만 `1`로 설정해 정상이고 유휴 상태인 adaptive scheduler가 즉시 수집하도록 합니다. 이 요청은 활성 수집, provider backoff, throttling, 열린 circuit 또는 인벤토리 근거 gate를 우회하지 않습니다. 영속 Job template에서는 이 값을 설정하지 않습니다. |
| Core 인벤토리 link constraint | 서비스 마이그레이션 | 배포 | Core 마이그레이션 head는 snapshot 및 실시간 인벤토리 table에서 검토된 `runtime_calls` link 형식을 허용합니다. Downgrade는 다시 만들 수 있는 런타임 호출 link만 제거한 뒤 이전 허용 목록을 복원합니다. |
