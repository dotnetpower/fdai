...
translation_source_sha: eae824efc66aa18b38584b7ffb5d2cedc4040d0a
...
Terraform은 provider-schema Job을 루트 주소
`azurerm_container_app_job.provider_schema[0]`에서 소유합니다. 읽기 전용 `data` 블록은
결정적 이름으로 기존 Container Apps 환경과 인벤토리 신원을 확인하고, `moved` 블록은 이전
compute module 주소의 상태를 보존합니다. 따라서 provider 전용 대상은 compute module의
광범위한 플랫폼 의존성 그래프를 상속하지 않습니다.
