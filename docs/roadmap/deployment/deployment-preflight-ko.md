...
>
> **구현 상태.** Production `deploy-dev` workflow는 `fdaictl deploy preflight`를 실행하고
> sanitized Azure/runner evidence를 private Blob에 저장한 뒤 두 digest를 exact plan에
> binding합니다. Control-loop remediation-PR 경로와 실제 GitHub Check publisher는 아직
> 배선되지 않았습니다.

## 루프에서의 위치

설계는 하나의 analyzer를 공유하는 두 진입점을 정의합니다. 현재는 사람 배포 경로가
배포됐고 control-plane 경로는 seam만 있습니다:

- **컨트롤 플레인(계획됨)**: [executor](../architecture/project-structure-ko.md)가 remediation PR을 emit하기
  전에, analyzer는 그 변경이 실제로 대상 scope에 착지할 수 있는지 확인합니다. blocking
  finding은 정책을 실패시킬 PR을 여는 대신 액션을 `hil`로 격하시킵니다.
- **사람 배포(배포됨)**: private runner workflow가 plan 전에 리포트를 생성하고 exact-plan
  metadata에 evidence digest를 binding합니다. PR comment/GitHub Check 전송은 후속입니다.
...
배포됨: 프로브 seam, 제네릭 결정론적 프로브, analyzer + 리포트, Azure CLI 기반
`fdaictl` 진입점, deploy workflow의 protected-plan evidence binding, 테스트.

1. **Azure probe와 protected-plan evidence(배포됨)**: `delivery/azure/preflight/`의 공유 읽기 전용 ARM
  ...
  2. **Capability-mode toggle scaffold(배포됨)**: `infra/modules/preflight-toggles/`와 disk
    reference consumer가 계약을 검증합니다. Root app graph의 실제 consumer wiring은 계획됨.
  3. **Check publishing primitive(배포됨)**: core function, provider Protocol, in-memory
    publisher가 있습니다. GitHub Check adapter와 infra PR workflow wiring은 계획됨.
  4. **Deployment Environment Profile primitive(배포됨)**: bounded in-memory cache, TTL,
    Inventory-delta invalidation helper가 있습니다. Composition refresh task와 durable cache
    wiring은 계획됨.
  5. **Control-loop pre-PR gate(계획됨)**: executor가 remediation PR을 만들기 전에 같은
    analyzer를 호출하고 blocking finding을 `hil`로 낮추는 live path.
