...
shipped pure loop는 **terraform 플랜을 능동적으로 재렌더**할 override를 계산하고 재검증할 수
있습니다. 지원되는 대체 형태는 애초에 거부되는 연산을 emit하지 않습니다. 기존
[executor](../architecture/project-structure-ko.md)를 통한 remediation PR 전달은 live composition
...
>
> **구현 상태.** Bounded convergence loop, toggle proposal builder, ActionType, data-only toggle
> modules와 reference consumer는 배포됐습니다. 실제 policy finding trigger, plan renderer,
> `ProposalSink` -> Huginn 연결, PR publisher 및 audit write는 아직 배선되지 않았습니다.
...
`autofix: false` 토글은 proposal이나 diff를 제출하지 않습니다. Report의 manual guidance로
남고 전체 pass는 escalation되며, 오퍼레이터가 변수를 검토해 적용합니다.
...
- **Blast-radius limit** - 재조립은 선언된 infra 변수만 건드립니다. 캡 초과는 `hil`로
  abstain합니다.
- **Audit-log entry** - pure loop는 audit-grade terminal reason과 toggle provenance를
  반환합니다. Live composition이 이를 executor에 제출할 때 해시 체인 감사 레코드를 쓰며,
  현재 unwired core primitive 자체는 audit store를 호출하지 않습니다.
...
`core/`는 `FeasibilityProbe` Protocol과 caller-supplied `ProposalSink` callable만 봅니다.
