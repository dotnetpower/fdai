...
파생하거나 이름을 바꾸거나 다른 채널로 대체하지 않습니다.
...
| `fdai:vertical` | `shared` / `resilience` / `change-safety` / `cost-governance` | `var.cost_vertical` (기본값 `shared`) | 리소스 비용을 귀속할 AIOps 버티컬입니다. 여러 버티컬이 공유하는 control-plane 인프라는 `shared`를 유지하고, 세 실행기 MI 같은 버티컬별 리소스가 이 키를 재정의합니다. |
