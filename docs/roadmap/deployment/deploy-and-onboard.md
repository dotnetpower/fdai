...
- **Executor shadow deployment**: `enable_isolated_executor=true` provisions the internal app and a dedicated UAMI with ACR pull, command receive, receipt/DLQ send, and state-secret read only. The default is `false`; no action-specific effect role is assigned in SD-07. The private-runner workflow exposes this as `deploy_isolated_executor`, remains plan-only by default, and includes the app's latest revision in post-apply health verification. An Executor plan resolves `runtime_image_revision` (the checkout commit by default), verifies the GHCR attestation, and binds the identical digest already present in ACR. `promote_runtime_image=true` explicitly imports that verified digest without rebuilding it. Exact apply rejects image promotion and consumes only the protected plan artifact.
- **Executor authority cutover**: `enable_isolated_executor_authority_cutover=true` additionally requires the isolated app and development operations gateway. It removes gateway and vertical effect access from Core, authorizes the isolated identity, and keeps Core transport/read identity attached. `verify_executor_effect=true` runs one reversible NSG rule probe, checks the effect through Azure Resource Manager, rejects duplicate writes, records command offsets and terminal receipts, cleans up the rule, and fails when the transition exceeds 900 seconds.
