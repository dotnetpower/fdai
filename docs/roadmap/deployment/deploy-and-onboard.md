...
- The protected `fdaictl` transport supports `dev` and `staging`. Its request identity binds the
  ...
  independently recomputes those bindings, verifies that the target GitHub Environment requires one
...
endpoint, and independently declared root Job. Stable existing environment, subnet, DNS-zone
resource IDs, and the versionless Key Vault secret URI prevent unrelated compute-module drift from
entering that bounded plan. Set the `ENABLE_OPERATIONAL_HISTORY` repository variable to `true` before the
initial plan so subsequent general deployment plans preserve these opt-in resources instead of
...
- **Cross-tenant SharePoint**: `power_platform_connector_enabled=true` injects only connector policy and allowlist values into the ingestion API. The Microsoft 365 tenant owns the SharePoint and Power Platform connection, while the Azure tenant owns the multi-tenant FDAI API audience. The API requires the exact external tenant, approved OAuth client, and `DocumentConnector.Ingest` permission; secrets remain in provider-hosted connections or Key Vault.
