...
| `FDAI_STEWARDSHIP_AUDIT_INTERVAL_SECONDS` / `FDAI_HANDOVER_KNOWLEDGE_INTERVAL_SECONDS` | env | upstream / deployment | Core service cadences for readiness-gated Entra liveness observations and content-free handover gap, candidate, and withdrawal events. Terraform defaults them to 3600 and 60 seconds. They grant no merge, IAM, approval, or execution authority. |
