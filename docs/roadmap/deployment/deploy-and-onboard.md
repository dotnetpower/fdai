...
- **Runner inputs:** Supply an SSH public key, quota headroom, and the Log Analytics destination. Mutation-enabled Genesis runs `prepare-genesis-access-tools.sh` before provider or policy mutation, pinning stable Bastion and Microsoft Entra SSH CLI extensions without creating Azure resources. Run it directly only to prewarm or repair the local CLI; inspection mode remains read-only. Connected bootstrap uses an exact Marketplace Ubuntu version. Offline bootstrap can supply a separately verified prebuilt image.
...
For a new platform state, `foundation_resource_group_context_digest` selects reference-only ownership and verifies the foundation tag and region. A verified failed public-development state can enter the empty Foundation-managed application backend only through the advanced no-delete adoption path, which removes the two resource-group owner addresses, retains the original state, writes a claim before one non-forced push, verifies readback, and blocks every delete or replacement plan before approval.
...
The full plan projection and value-free summary use a current-UID mode-0700 temporary directory and are removed after the metadata builder returns.
Exact apply validates the summary schema, digest, action and resource-type totals, non-destructive flag, and required post-apply observation list. Only `plan-runtime-*` and `plan-observability-*` may carry one `terraform_data` state replacement with zero create, update, or delete actions; every other destructive summary is rejected. Legacy v1 metadata without both fields remains valid until its bounded expiry, while partial summary evidence is rejected.
...
- [`azd-up.sh`](../../../scripts/deployment/azure/azd-up.sh) remains the direct interactive public
  `dev` path. It verifies provider locks, supplies the verified principal, and saves owner-only plans. Missing Linux x64 or ARM64 `azd` is installed from the SHA-256-pinned official 1.34.0 archive into `~/.local/bin` without `sudo` or replacement.
  A failed apply stays fenced until refresh-only readback under a clean descendant records a separate closure. The old plan is never reapplied. An existing policy-private Key Vault routes the deployment to `fdai-up.sh` and its managed host.
...
- **Azure Bots** - the approval bot remains deployment-owned. The independent System Knowledge Service root always creates its UAMI, private Blob claim container, and HTTPS endpoint through the protected workflow. Bot Framework adds an F0 Bot and Teams channel.
  Outgoing Webhook adds no Bot or Graph resource and binds its Teams-issued HMAC key from Key Vault only after endpoint bootstrap.
  The platform `document_blob` diagnostic setting continues to cover the reused claim storage account.
...
| `FDAI_CASE_HISTORY_CONTAINER_URL` / `FDAI_CASE_HISTORY_MI_CLIENT_ID` / `FDAI_CASE_HISTORY_RETENTION_DAYS` / `FDAI_CASE_HISTORY_DELETION_DAYS` / `FDAI_CASE_HISTORY_RETENTION_TICK_SECONDS` | env | upstream / deployment | Private Blob container URL, dedicated attached UAMI client id, active-retention/deletion-due offsets, and the bounded Muninn retention cadence for immutable case revisions. Azure defaults active retention, deletion due, superseded Blob versions, and change feed to 30 days. Terraform derives the storage and identity bindings, validates deletion is not earlier than retention, and startup fails if the dedicated identity id is missing or matches the executor identity; public/key-auth fallback is not used. The retention tick defaults to `86400`; deployment overrides remain bound to the approved data schedule. |
...
| `FDAI_INVENTORY_RECONCILIATION_INTERVAL_SECONDS` | env | upstream | Healthy full-scan interval for the Inventory Job. The default Job cron wakes every 10 minutes, but PostgreSQL attempt state skips scans until this interval is due and retries a newer failed or abandoned attempt on the next tick. A degraded pending projection retains its failure evidence but does not block the next authoritative full scan; an incomplete current projection still fails. A promoted full snapshot that skipped that degraded base is rebound to the actual manifest generation, while a missing manifest generation remains blocked. A fresh full snapshot can derive its prior state from retained complete topology history when its declared base is the exact skipped incomplete generation. Recovery can reuse an identical content-addressed state-transition coverage record across a recalculated batch without duplicating or weakening it. Global and active-scope projection checkpoints never advance past the journal high watermark observed by that snapshot append, even when later writes commit concurrently. Repository Variable `ENABLE_RUNTIME_CALL_EVIDENCE=true` independently enables the deployed runtime-call source after Operator state migration. The independent Operator API and channel edge receive distinct fixed semantic outbox namespaces so one runtime cannot claim the other's pending request. The independent-service `runtime_call_evidence_transition` guard permits only those exact namespace additions or removals for the matching runtime. Within the configured freshness window, runtime-call completeness selects the newest observation per exact endpoint pair before ARM replica verification; a newer unverified observation fails closed, and a newer unpaired observation becomes incomplete after the trailing guard. Protected `plan-runtime-*` and `apply-runtime-*` requests target sibling flag, workspace, and exact-image transition resources whose verified updaters and rollback affect only the existing Job. The workspace transition exists separately so a previously created flag resource never hides a newly required binding. A changed attested digest permits only the image updater's state-only replacement. Runtime-only convergence checks all three transition states, and independent readback verifies the flag, workspace digest, and selected image digest. Environment-integrated Container Apps log rows can carry an empty `_ResourceId`; the source then requires the platform container name and ID to match the exact ARM replica response instead of accepting an unbound name. The scope guard rejects every other address. |
| `FDAI_INVENTORY_OPERATOR_REQUESTED` | env | deployment / one-shot | Set to `1` only on an explicitly requested one-shot Job execution to make a healthy, idle adaptive scheduler collect immediately. The request does not bypass an active collection, provider backoff, throttling, an open circuit, or any inventory evidence gate. The persisted Job template leaves this value unset. The Core migration head admits the reviewed `runtime_calls` link type in snapshot and real-time inventory tables; downgrade removes only rebuildable runtime-call links before restoring the earlier allowlist. |
...
| `FDAI_DIRECT_API_FAKE` | env | test-only | `1` swaps the executor direct-API path for the in-memory shadow fake in automated tests. Interactive local preparation never sets it. Without exactly one scoped development operations gateway, managed-resource execution stays unavailable. Mutually exclusive with `FDAI_DEV_OPERATIONS_GATEWAY_URL`. |
