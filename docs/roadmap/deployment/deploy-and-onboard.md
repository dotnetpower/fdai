...
Its installed `fdai-inventory-sync` wrapper accepts only positional `once` or `loop`, so a protected
exact-image rehearsal can avoid option-shaped Container Apps overrides without changing authority.
...
protected-operation workflow so the maintainer remains a distinct Environment approver. Stable
existing resource IDs and the versionless Key Vault secret URI exclude unrelated compute drift.
Set `ENABLE_OPERATIONAL_HISTORY=true` before the initial plan so later general deployment plans
preserve these opt-in resources.
...
| `FDAI_STEWARDSHIP_REQUIRE_BINDINGS` / `FDAI_STEWARDSHIP_AUDIT_INTERVAL_SECONDS` / `FDAI_HANDOVER_KNOWLEDGE_INTERVAL_SECONDS` | env | deployment / upstream | Set `FDAI_STEWARDSHIP_REQUIRE_BINDINGS=1` so placeholder identities fail startup. The protected workflow maps `ENABLE_STEWARDSHIP_GOVERNANCE`, `GITOPS_OWNER`, and `GITOPS_REPO` from repository Variables and maps `GITOPS_TOKEN` and `GITHUB_WEBHOOK_SECRET` from repository Secrets. Terraform also supplies `FDAI_MAINTAINERS`, `FDAI_STEWARD_<AGENT>`, optional Key Vault-backed GitOps/webhook inputs, and the readiness-gated Entra health and content-free handover lifecycle cadences defined in [Agent operational ownership lifecycle](../interfaces/agent-stewardship-operations.md). The cadence defaults are 3600 and 60 seconds and grant no merge, IAM, approval, or execution authority. |
