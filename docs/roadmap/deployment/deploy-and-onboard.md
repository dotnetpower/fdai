...
endpoint, and independently declared root Job. Stable existing environment, subnet, DNS-zone, and
secret resource IDs prevent unrelated compute-module inputs or infrastructure drift from entering
that bounded plan. Set the `ENABLE_OPERATIONAL_HISTORY` repository variable to `true` before the
