...
|------|-------|----------|-------|
| Protected platform plan and exact apply | implemented | `.github/workflows/deploy-dev.yml`, `materialize-model-binding-proposal.sh`, `model_binding_proposal.py`, and focused deployment workflow checks | Private-runner planning includes an exact read-only Operator proposal handoff, immutable apply claims, post-apply checks, and the optional non-executor channel-edge identity plus versionless secret-scope inputs; no governed platform apply receipt is retained in the repository. |
