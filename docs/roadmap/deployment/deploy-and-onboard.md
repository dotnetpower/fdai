...
Exact apply validates the summary schema, digest, action and resource-type totals, non-destructive flag, and required post-apply observation list. Legacy v1 metadata without both fields remains valid until its bounded expiry, while partial summary evidence is rejected.
