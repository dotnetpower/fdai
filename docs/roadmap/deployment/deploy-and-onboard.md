...
discovers [compatible image and host VM SKUs](subscription-genesis-assurance.md#foundation-and-external-control-planes) from the complete regional catalog before planning, retaining the managed-image and ephemeral-host disk distinction. A later signed Foundation run may adopt an independently verified runner-image receipt without repeating image apply. Its inputs keep the current Foundation source and run while separately binding the original image source, signed verifier source, image run, and exact receipt digest.
