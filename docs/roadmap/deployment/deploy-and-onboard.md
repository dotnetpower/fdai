...
| `FDAI_INVENTORY_RECONCILIATION_INTERVAL_SECONDS` | env | upstream | Healthy full-scan interval for the Inventory Job, and the freshness ceiling the projection then declares on every observed state fact. The Job cron wakes every minute, but PostgreSQL attempt state skips scans until this interval is due and retries a newer failed or abandoned attempt on the next tick. Default `21600`. |
| `FDAI_INVENTORY_CHANGE_MIN_INTERVAL_SECONDS` | env | upstream | Floor between two change-triggered reconciliations. An observed provider change makes a full scan due once the active snapshot is at least this old, so the graph tracks change without a change storm becoming a scan storm. Default `120`. |
| `FDAI_INVENTORY_PROGRESS_DEADLINE_SECONDS` | env | upstream | Longest an inventory source may go without producing anything. Every batch re-arms it, so a stalled provider fails its own attempt as `partial` while a slow one that keeps producing is allowed to finish. Default `900`. |
| `FDAI_INVENTORY_ATTEMPT_DEADLINE_SECONDS` | env | upstream | Absolute ceiling for one attempt, bounding a source that produces just often enough to keep re-arming the progress deadline. Must be at least the progress deadline and at most `1740`, so an attempt always resolves before the 30-minute abandonment window lets a second attempt start. Default `1500`. |
| `FDAI_INVENTORY_ARG_REQUESTS_PER_SECOND` | env | upstream | Sustained Azure Resource Graph request budget shared by every shard of one scan. ARG bills a per-user quota, so pacing at the budget keeps a continuous background scan inside quota instead of bursting into a provider throttle and paying a much longer reset window. Default `3`. |
| `FDAI_INVENTORY_LOOP_SECONDS` | env | dev-only | Sleep between ticks when the CLI runs with `--loop` for local development. The deployed Job uses the cron instead. Default `60`. |
