...
| `FDAI_STEWARDSHIP_REQUIRE_BINDINGS` / `FDAI_STEWARDSHIP_AUDIT_INTERVAL_SECONDS` / `FDAI_HANDOVER_KNOWLEDGE_INTERVAL_SECONDS` | env | deployment / upstream | Set `FDAI_STEWARDSHIP_REQUIRE_BINDINGS=1` so placeholder identities fail startup. Terraform also supplies `FDAI_MAINTAINERS`, `FDAI_STEWARD_<AGENT>`, optional Key Vault-backed GitOps/webhook inputs, and the readiness-gated Entra health and content-free handover lifecycle cadences defined in [Agent operational ownership lifecycle](../interfaces/agent-stewardship-operations.md). The cadence defaults are 3600 and 60 seconds and grant no merge, IAM, approval, or execution authority. |
