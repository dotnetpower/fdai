...
- **Cross-tenant SharePoint**: `sharepoint_connector_enabled=true` activates the FDAI-native connector. The Azure ingestion UAMI supplies a federated assertion to the application registered in the Microsoft 365 tenant, which issues the Microsoft Graph token after administrator consent. Terraform injects only the target tenant, application, site, drive, redirect allowlist, and server-owned document policy. The protected service workflow requires an explicit `sharepoint_connector_transition` and seals the exact enable or disable environment delta into the plan bundle. Power Platform isn't deployed or invoked.
