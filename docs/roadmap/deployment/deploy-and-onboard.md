...
- **Document-service identity selection**: the ingestion API and processing worker receive one exact user-assigned identity client id through `FDAI_MI_CLIENT_ID`. Every Azure storage, Event Hubs, embedding, and optional OCR credential selects that client id explicitly; an ambient or system-assigned fallback is not accepted.
