...
| `FDAI_STEWARDSHIP_REQUIRE_BINDINGS` / `FDAI_STEWARDSHIP_AUDIT_INTERVAL_SECONDS` / `FDAI_HANDOVER_KNOWLEDGE_INTERVAL_SECONDS` | env | deployment / upstream | Set `FDAI_STEWARDSHIP_REQUIRE_BINDINGS=1` so placeholder identities fail startup. The protected workflow maps `ENABLE_STEWARDSHIP_GOVERNANCE`, `GITOPS_OWNER`, `GITOPS_REPO`, `GITHUB_APP_CLIENT_ID`, and `GITHUB_APP_INSTALLATION_ID` from repository Variables. It maps `GITHUB_APP_PRIVATE_KEY` and `GITHUB_WEBHOOK_SECRET` from repository Secrets; `GITOPS_TOKEN` remains a mutually exclusive compatibility secret. Terraform also supplies `FDAI_MAINTAINERS`, `FDAI_STEWARD_<AGENT>`, Key Vault references, and the readiness-gated Entra health and content-free handover lifecycle cadences defined in [Agent operational ownership lifecycle](../interfaces/agent-stewardship-operations.md). The cadence defaults are 3600 and 60 seconds and grant no merge, IAM, approval, or execution authority. |
