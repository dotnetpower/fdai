...
| `FDAI_INVENTORY_ATTEMPT_DEADLINE_SECONDS` | env | upstream | Wall-clock ceiling for one inventory source attempt. A stalled provider fails its own attempt as `partial` instead of holding the single writer of the observed subgraph and suppressing every later scan. Default `900`. |
