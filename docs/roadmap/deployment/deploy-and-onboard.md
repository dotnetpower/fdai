...
Terraform owns the provider-schema Job at the root address `azurerm_container_app_job.provider_schema[0]`. Read-only `data` blocks resolve the existing Container Apps environment and inventory identity by deterministic name, while a `moved` block preserves state from the former compute-module address. The provider-only target therefore does not inherit the compute module's broad platform dependency graph.
