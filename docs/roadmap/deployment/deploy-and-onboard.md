...
for that `for_each` move so Terraform can evaluate both keyed instances together. It also targets
the AI account with its role collection so network and authorization settings converge in one apply.
