...
| 4 | **Event Hubs namespace shards** | 2 x Standard (1 TU, auto-inflate off) | Kafka-wire event bus (endpoints on `:9093`) | primary owns governed ingress, DLQs, HIL, and stages; the synthetic startup round-trip shares Core's provisioned event topic with a dedicated probe consumer instead of adding an eleventh entity. Operational owns canary + DLQ, raw inventory, Executor command + DLQ, and Executor receipt entities. |
