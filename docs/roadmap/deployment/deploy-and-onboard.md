...
| `FDAI_DIRECT_API_FAKE` | env | test-only / dev-local | `1` swaps the executor direct-API path for the in-memory shadow fake. Automated tests set it explicitly; `prepare-local-runtime-env.sh` auto-injects it for interactive local dev when no operations gateway is provisioned, so the `execution_path: direct_api` dispatch stays live without a live backend. Mutually exclusive with `FDAI_DEV_OPERATIONS_GATEWAY_URL`. |
