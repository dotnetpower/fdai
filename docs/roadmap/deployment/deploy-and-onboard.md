...
| 4 | **Event Hubs namespace shards** | 2 x Standard (1 TU, auto-inflate off) | Kafka-wire event bus (endpoints on `:9093`) | Primary owns governed ingress, DLQs, HIL, and stages. Operational owns canary + DLQ, the dedicated synthetic startup round-trip, the readiness transition entity, raw inventory, Executor command + DLQ, and Executor receipt entities. Core receives the operational bootstrap endpoint and startup topic through deployment configuration. Every logical runtime topic the operational bus publishes needs its own entity here because that bus is not multiplexed onto the primary object topic. |
