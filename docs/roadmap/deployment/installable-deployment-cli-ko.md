...
translation_source_sha: 0d6c12bae51a904230f12d560e2328008a09b0c9
...
복구 증적을 일반 Foundation 증적으로 바꾸면 원래 실행과 복구의 출처 및 상태
소유권이 사라집니다. 대신 등록 어댑터는 원래 실행 잠금 안에서 수정되지 않은 복구 증적, 두 실행
전 기록, 검토, 원래 스냅샷, 상태 해시·계보와 비공개 인계를 검증합니다. 공통 상관관계 필드만
메모리 안에서 변환하며, 이 변환 결과는 새 증적이 아니고 증적으로 저장하지 않습니다. 현재 등록 승인,
소스 CI와 독립 호스트 증명은 별도로 필요합니다. 이 수용 경계는 상태를 이행하거나 애플리케이션을
활성화하지 않습니다.
...
참조와 등록 소스를 기록합니다. 검증 전용 재개는 다시 등록하거나 소스를 암묵적으로 바꾸지
않습니다. 일반 Foundation 등록 동작은 그대로 유지합니다.
실행 전 기록과 최종 증적에는 검증된 등록 승인자의 신원 해시를 유지합니다.

### 복구한 상태 이행

상태 전송 자료 생성기는 각각 검증된 복구 구성과 원래 상태를 임시 이행 단계 안에서만
합칩니다. 구성 안의 두 번째 상태 파일을 거부하고 원래 상태의 증적에 연결된 해시를 검증한 뒤,
전송 자료를 발행하기 전에 원래 바이트를 다시 확인합니다. 기존 관리형 호스트의 검증 형식을
유지하며 어느 입력도 새 로컬 상태 소유자가 되지 않습니다.
...
이행 실행 전 기록과 증적에는 복구 증적 스키마, 현재 이행 소스와 검증된 승인자를 보존합니다.
실행 전 기록이 있으면 검증만 허용하며 전송이나 이행을 반복하지 않습니다. 기록된 원격 상태
권한에 따라 로컬 상태가 삭제된 뒤에는 재개 시 해당 권한과 최종 증적을 검증하고 비공개 원격
저장소를 독립적으로 관측합니다. 로컬 상태가 없다는 사실만으로 권한을 얻지는 않습니다.
공개 소스 조정기 연결과 실제 복구 검증은 별도 작업으로 남아 있습니다.
