...
재개를 멈추게 할 수 없습니다.
생성되는 자식 파일은 상위 디렉터리 서술자를 유지한 배타적 심볼릭 링크 차단 writer를
사용합니다. 재개 시 교체는 해당 서술자를 통해 최종 항목만 제거한 뒤 `O_EXCL`로 다시
생성하므로 기존 심볼릭 링크, 이름 있는 파이프, 하드 링크가 다른 파일로 쓰기를 돌리거나 자를
수 없습니다. 서명된 오프라인 키트 메타데이터와 스트리밍 번들 아카이브 게시도 같은 경계를
사용합니다.
...
다시 검증하므로, 경합이 온보딩을 멈추게 하지 않고 추가 작업을 중단시킵니다. 저널 디렉터리
탐색은 유지 중인 상위 서술자를 기준으로 모든 구성 요소를 열고 심볼릭 링크가 있는 상위 경로를
차단합니다. 프로비저닝 이벤트 v2는 READY 검증에 사용한 매니페스트 버전을 기록하며, v1
디코더는 이 필드가 생기기 전에 만든 저널의 재생을 유지합니다. 리소스와 페이지 카운터는 검색
시작 뒤에 나타나고 증가할 수 있지만 이후 진행률 스냅샷에서 사라지거나 감소하거나 예상 합계가
줄어들 수 없습니다.
