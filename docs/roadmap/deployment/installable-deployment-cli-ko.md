...
translation_source_sha: f0ede7c552c426f5a97d8558c12304e8b7fc1203
translation_revised: 2026-09-13
---
# 설치형 배포 CLI

운영자는 격리된 `fdaictl`을 설치하고 읽기 전용 사전 검사를 실행한 뒤 승인된 Terraform
계획을 배포 실행기에 제출합니다. 배포 비밀은 승인된 실행 호스트에 유지합니다.

> **실행 경계:** Terraform은 인프라 실행 엔진이자 정본으로 유지됩니다. 계획된 `fdaictl`
> 배포판은 검증, 계획 분석, 작업 흐름 제출, 배포 후 검사를 위한 얇은 조정 계층입니다.
>
...
재개를 멈추게 할 수 없습니다.
전체 빌드는 3시간 한도와 단계별 실행 시간·무진행 한도를 공유합니다. 내부 감시기는 취소 신호를 전달하고 상위 감시기보다 짧은 종료 유예를 사용하므로 고아 빌드 그룹과 이후 서명·성공 보고를 막습니다.
전체 빌드는 고정된 커밋의 별도 비공개 작업트리를 만들고, 호출자의 로컬 무시 파일과 서명 키를 가져오지 않습니다. 조립 중과 서명 직전에 추적 파일의 실제 바이트, 모드, 변경 메타데이터 지문을 대조합니다. 상태 플래그나 잠금파일이 그대로라는 사실만으로 소스 일치를 보증하지 않습니다.
...
Low보다 높은 발견 문제가 없을 때만 종료합니다. 소스 검토에는 패키지 진입점, 상태 보존 검증 재개, 중첩 취소를 명시적으로 포함하며 완전한 아티팩트 수락 검증을 대신하지 않습니다.
