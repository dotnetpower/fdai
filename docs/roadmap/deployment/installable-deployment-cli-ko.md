...
translation_source_sha: 49aa4ba08d30991c481ac01eace16dfd7e710afe
...
기록합니다. 초기 범위를 다시 확인해 바꾸거나 적용을 반복할 수 없습니다. 성공 증적이 없으면
검토 결과를 반환합니다. 별도 정확한 승인으로 등록·이행을 진행하며 실행 전 기록이 있으면 검증만
선택합니다. 이행 완료 후에는 로컬 상태 삭제 뒤 다시 등록하지 않고 원격 저장소를 관측합니다.
원래 잠금 안에서 검증된 복구 문맥이 기존 전송기에 원래 스냅샷을 전달합니다. 별도 불변 진행
기록은 원래 상태 보고와 준비되지 않았다는 결과를 유지합니다.
