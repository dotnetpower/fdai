...
translation_source_sha: b5190036646c8f6294356ce5780561f08ca642ee
...
이후 기존 빌드 실행 기록과 원래 스냅샷으로 서비스 OCI 이미지 5개를 빌드하거나 다시 검증합니다.
다른 리비전, 불완전한 목록과 잘못된 준비 완료 표식은 거부하며 로컬 도구가 없으면 검토 결과를
반환합니다. 의존 이미지, 레지스트리 import와 활성화는 아직 연결되지 않았습니다.
