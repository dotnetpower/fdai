...
translation_source_sha: a1f30c02e6130bdd154a8d8af7e31899662c8937
...
이후 해당 검증기를 `verify` 모드로 실행합니다. 이후의 검증된 조정기는 복구 소스를 다시 검증하고 검증기 다이제스트가 바뀌지 않았음을 입증한 경우에만 완료된 복구를 재사용할 수 있습니다. 상태 비교는 정확한 `lineage`, 리소스 신원, 비일시적 내용을 유지하면서 백엔드 이행으로 생기는 `serial` 증가 1회와 `check_results` 순서 정규화만 허용합니다. 새로 고침 전용 드리프트(`resource_drift`)는 리소스 ID가 같고 관찰된 `after` 값이 같은 주소의 변경 없음 목표 값과 정확히 일치할 때만 허용하며, 지연되거나 닫히지 않은 드리프트는 차단합니다. 기본 아카이브에 일부 파일만 있거나, 기존 파일이 다르거나, 다시 읽은 결과가 바뀌면 복구가 차단됩니다.
