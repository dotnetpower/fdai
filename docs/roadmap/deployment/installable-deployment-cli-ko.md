...
translation_source_sha: f81047a1676edf1c1a745ac7fe5ab71583bff555
...
이후 해당 검증기를 `verify` 모드로 실행합니다. 상태 비교는 정확한 `lineage`, 리소스 신원, 비일시적 내용을 유지하면서 백엔드 이행으로 생기는 `serial` 증가 1회와 `check_results` 순서 정규화만 허용합니다. 기본 아카이브에 일부 파일만 있거나, 기존 파일이 다르거나, 다시 읽은 결과가 바뀌면 복구가 차단됩니다.
