...
translation_source_sha: 6ba60944667d4dbbb1987422dd3ddd222c3d2895
...
정확한 승인을 받은 Foundation 실행과 비공개 상태 이전, 등록된 호스트로의 검증된 소스 전송까지
지원합니다. 공개 소스 명령은 공유
...
보고는 그대로 유지합니다.
관리 호스트 이미지의 Terraform에는 확인된 CLI의 절대 경로를 실행하는 비공개 mode-`0700`
일반 파일 `az` 실행기만 제공합니다. 이 방식은 CLI가 설치 경로를 기준으로 내부 실행 파일을
찾는 동작을 보존하면서 `PATH` 대체를 허용하지 않습니다. 재개할 때 실행기가 바뀌면
거부하며, 사람 신원 재조회도 같은 신뢰된 CLI를 사용합니다.
제작 VM 종료 대기도 `/usr/bin/az`가 아니라 이 고정된 프로세스 경로에서 `az`를 찾습니다.
전원 상태가 종료 또는 할당 해제일 때만 통과하며 CLI 오류는 그대로 실패로 처리합니다.
실행 전 기록 이후 실패하면 조정기에 성공 증적이 없어도 과금되는 리소스가 남을 수 있습니다.
원래 계획, 실행 전 기록과 Terraform 상태를 보존해야 합니다. 읽기 전용 관측은 계획 재적용을
허용하지 않으며 종료된 VM이 할당 해제됐음을 입증하지도 않습니다.

로컬 종료 대기 실패는 실행 전 기록이 있는 바이너리 계획을 다시 적용하지 않고, 새로운 읽기 전용
잔여 계획부터 준비합니다. 원래 변수와 권위 있는 상태를 유지하고 기존 검토 및 실행 전 기록에
연결하며, 새 Terraform 구성에는 검토된 CLI 경로 수정만 허용합니다. 권위 있는 상태를 다른 실행
위치로 복사하거나 원래 계획을 변경하지 않습니다. 완료된 Azure 리소스와 프로비저닝 해제 명령은
변경 없음이어야 합니다. 실패한 로컬 대기 표시만 교체할 수 있으며 남은 이미지 캡처, 검증 VM,
VM 할당 해제 및 일반화는 원래 확인된 의도를 유지해야 합니다. 미검증 상태 차이, 지연된 작업, 소유권 변경,
추가 리소스 또는 실패한 검사는 결과를 차단합니다. 이 점검은 적용 권한을 부여하지 않습니다.
잔여 계획을 실행하려면 별도의 실행 계약과 현재 승인이 필요합니다.
재조회 차이는 리소스 ID가 그대로이고 변경 없음으로 보존된 리소스에서만 허용합니다.
원래 미확정이었던 계산값, 의미가 같은 null·빈 값, Firewall 애플리케이션 규칙 집합의 순서 차이만
대상입니다. 확인된 설정의 변경이나 모호한 재조회 기록은 계속 차단합니다.
계획 도중 원래 상태, 실행 전 기록, 소스나 구성이 바뀌면 검토 기록을 발행하지 않습니다.
플래너는 미완료 출력을 보존하고 이전 복구 디렉터리를 덮어쓰지 않습니다.
미러 전용 초기화가 provider 링크를 만들면 원래 해시 검증된 provider 트리의 같은 상대 경로를
가리키는 링크만 비공개 파일 복사본으로 바꿉니다. 원래 provider 바이트는 변하지 않아야 하며
복사 후 실행 트리에는 링크가 없어야 합니다. 외부 링크는 계속 거부하고 권위 있는 상태는
이 복사 대상에 포함하지 않습니다.

별도 `genesis_runner_image_recovery_apply.py` 명령은 명시적으로 전달한 새 Genesis 승인만
받습니다. 단계는 `runner-image`, 실행 바인딩은 잔여 검토 digest, 소스는 복구 소스여야 하며
근거는 잔여 검토와 계획의 digest에 정확히 일치해야 합니다. 원래 승인 기록은 이 바인딩을
충족하지 않습니다. 실행 전 기록을 만들기 전에 현재 사람 신원, 정확한 소스의 CI, 구성·provider·계획
해시, 상태 계보와 변경 없는 원래 상태 바이트를 확인합니다. 새 불변 기록에는 사람·실행기 신원
digest와 안정적인 멱등 키를 남깁니다. 저장된 계획은 원래 로컬 상태 경로에 적용하며 두 번째 상태
소유자나 암묵적인 인수를 만들지 않습니다.

잔여 실행 전 기록이 있으면 `--verify-only`만 허용하며 다시 적용하지 않습니다. 성공하려면 기존의
독립 이미지·VM·확장·정책 관측과 새로운 변경 없음 계획이 필요합니다. 별도 잔여 작업 증적을
기록하며 원래 적용 증적이나 배포 준비 상태를 만들어내지 않습니다. 공개 전체 설치 조정기는 아직
이 잔여 작업 증적을 자동으로 소비하지 않습니다.

대화형 체크포인트 프롬프트도 같은 신뢰된 CLI를 찾은 뒤 현재 사람 승인자의 신원을 조회합니다.
신뢰된 실행 파일이 없거나 서비스 주체 계정이면 승인을 만들 수 없습니다.
기존 입력과 함께 사용할 수 없는 `--residual-review <path>` 옵션은 새 검토 기록의 무결성,
유효 기간, 실행 바인딩과 저장된 계획을 확인한 뒤 승인을 요청합니다. 승인은 원래 상태 기록이
아니라 잔여 검토와 복구 소스에 연결됩니다. 기존의 사람 전용 터미널 확인은 계속 필수이며
이 입력을 선택하는 것만으로 승인하거나 리소스를 적용하지 않습니다.
...
`genesis_foundation_recovery_plan.py`는 새로운 비공개 디렉터리에서 이 읽기 전용 비교를
준비합니다. 원래 실행 잠금 안에서 기존 검토·실행 전 기록·스냅샷·실행 복사본을 검증하고,
복사한 구성에는 애플리케이션 이름과 위 제한된 IP 정책 입력만 허용합니다. 원래 상태 경로를 Terraform에 직접
전달하며, 크기가 제한된 비공개 명령 진단과 현재 복구 소스·provider 바이트를 기록합니다.
새 그룹이 없음을 확인하고 동시 상태 변경은 거부합니다. 별도 만료 검토는 적용 권한을 부여하지
않으며 일반 Foundation 적용 경로에서는 사용할 수 없습니다. 공개 조정기로의 증적 인계는
별도 단계이며 이 검토만으로 완료되지 않습니다.

전용 실행기 `genesis_foundation_recovery_apply.py`는 실행 바인딩이 복구 검토 digest이고
소스가 복구 코드인 새로운 `foundation-apply` 승인만 받습니다. 공식 프롬프트에는
`--foundation-recovery-review`를 전달하며 원래 상태 기록의 승인으로 새 계획을 실행할 수
없습니다. 새 실행 전 기록을 만들기 전에 현재 사람 신원, 정확한 소스의 CI, 원래 실행 전 기록과
스냅샷·상태 계보, 구성·provider·계획 해시, 새 그룹의 지속적인 부재를 검증합니다. 현재 VM의
SKU·할당량 근거를 재확인한 뒤 실행 전 기록 직전에 상태·계획·구성·도구 해시와 검토 만료도
다시 확인합니다.

새 불변 실행 전 기록을 남긴 뒤 저장된 계획을 원래 로컬 상태 경로에 한 번 적용합니다.
실행 전 기록이 있으면 `--verify-only`만 허용하며 신원과 근거가 정확히 일치해야 합니다.
기존 Foundation 관측기는 신뢰된 시간 제한 실행 경로로 새 인계를 검증하고, 변경 없음 계획까지
통과해야 별도 복구 증적을 발행합니다. 검증마다 비공개 진단을 따로 보존하며 원래 증적을
만들어내거나 덮어쓰지 않습니다. 복구가 성공해도 호스트 증명, 비공개 상태 이행과 애플리케이션
배포는 아직 미완료입니다.
...
검증된 복구 구성과 원래 상태는 임시 전송 자료에서만 합칩니다. 두 번째 상태 소유자를 거부하고
증적에 연결된 상태 해시를 검증하며 발행 전에 바이트를 다시 확인합니다. 기존 관리형 호스트의
형식과 두 원래 입력은 그대로 유지합니다.
...
소스 실행을 재개합니다. 원래 스냅샷·런타임 설정·리전·예산은 불변이며 현재 실행 소스는 별도로
기록합니다. 이 경로에서는 초기 범위를 다시 확인해 변경할 수 없습니다. 복구 성공 증적이 없으면
검토 결과를 반환하고 적용을 반복하지 않습니다. 별도 정확한 승인으로 호스트 등록과 상태 이행을
진행하며 실행 전 기록이 있으면 검증만 선택합니다. 이행 완료 후에는 로컬 상태 삭제 뒤 등록을
반복하지 않고 원격 저장소 관측으로 재개합니다. 이후 원래 잠금 안에서 검증된 복구 문맥이 기존
소스 전송기에 원래 애플리케이션 스냅샷을 전달합니다. 별도 불변 진행 기록을 남기며 원래 상태
보고와 준비되지 않았다는 결과를 유지합니다. 애플리케이션 활성화는 아직 연결되지 않았으므로
이 경로만으로 배포 완료를 보고하지 않습니다.
