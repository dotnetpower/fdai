...
translation_source_sha: 066e8560df16e610c3a89272e0da0564d113952e
...
소스 실행은 검증된 인프라를 상태가 보존되는 비공개 디렉터리에 복사하고, 고정된 Terraform 바이너리를 검증한 뒤 잠금 파일이 선택한 공급자만 로컬 미러로 가져옵니다.
...
이후 해당 검증기를 `verify` 모드로 실행합니다. 이후의 검증된 조정기는 복구 소스를 다시 검증하고 검증기 다이제스트가 바뀌지 않았음을 입증한 경우에만 완료된 복구를 재사용할 수 있습니다. 상태 비교는 정확한 `lineage`, 리소스 신원, 비일시적 내용을 유지하면서 백엔드 이행으로 생기는 `serial` 증가 1회와 `check_results` 순서 정규화만 허용합니다. 새로 고침 전용 드리프트(`resource_drift`)는 리소스 ID가 같고 관찰된 `after` 값이 같은 주소의 변경 없음 목표 값과 정확히 일치할 때만 허용하며, 지연되거나 닫히지 않은 드리프트는 차단합니다. 정리 후 재관찰은 현재 검증된 관찰기를 소유자 전용 Bastion 경로로 전송하고, 다이제스트를 다시 읽어 확인한 뒤 실행 및 제거하며, 이전 작업 트리를 복원하지 않은 채 원격 상태 바이트를 권한 증거에 결속된 다이제스트와 비교합니다. 기본 아카이브에 일부 파일만 있거나, 기존 파일이 다르거나, 다시 읽은 결과가 바뀌면 복구가 차단됩니다.
...
아직 남아 있습니다. 로컬 검증과 테스트용 전송 근거만으로 Azure 배포 성공을 입증하지 않습니다. 감사되는 Azure Run Command로만 WSL 환경에 도달할 수 있는 적격 기존 Windows 호스트는 이미 피어링된 비공개 경로를 통해 정확한 Linux 배포 호스트에서 별도 실행 묶음을 받을 수 있습니다. 조정기는 이 Linux 호스트에서 실행하며 전체 대상 서술자, 작업 ID, 실행 묶음 증적 digest, 고정 수신기 digest에 연결된 정확한 `run_command` 프로비저닝 프로파일과 현재 사람 승인을 요구합니다. 시작 기록이나 중계 효과 전에 활성 Azure 계정에서 테넌트와 구독 binding을 계산하고, 현재 사람 행위자를 검증하며, 정확한 VM 리소스 ID, 비공개 주소, 배포 UAMI를 다시 읽습니다. 그 뒤 일회성 TLS 중계를 열거나 대상 호스트를 호출하기 전에 변경 불가능한 전송 시작 기록을 남깁니다. 중계는 검증된 서술자로 정확한 묶음과 의존성 없는 고정 수신기를 유지하고, 대상 호스트의 비공개 출발지 주소만 허용하며, 임시 인증서와 키를 메모리 기반 서술자에 보관합니다. 고정 Run Command는 정확한 VM, 중계, 작업, 크기, 개수, 모드, digest 좌표만 받습니다. WSL은 내려받은 인증서를 독립적으로 전달된 digest와 대조한 뒤 CA로 사용하고 두 페이로드를 검증한 다음 고정 수신기를 실행합니다. SAS 값, 계정 키, bearer token, 임의 스크립트 텍스트, 원시 근거는 인자나 로그에 넣지 않으며 클라우드 staging 산출물을 만들지 않습니다. 결과가 불명확한 보존 호출은 별도로 시작 기록을 남긴 `--verify-existing` 호출을 최대 한 번만 허용하고 오래된 다운로드 임시 파일만 제거하며 추출이나 새 전송을 반복하지 않습니다. 성공하려면 형식이 지정된 호스트 근거, 고정 호스트 정리 표식, 출발지 주소로 제한된 중계 완료, 중계 종료가 필요합니다. 추출된 실행 트리는 보존하고 내려받은 임시 파일은 제거합니다. 증적은 현재 staging 승인에 연결되지만 적용 권한을 부여하지 않으며 이후의 정확한 계획과 승인을 대신하지 않습니다. adapter는 명시적으로 호출하며 `fdaictl` 접근 프로파일 자동 연결과 실제 Azure 검증은 별도 작업으로 남습니다.
