...
- **Bounded database host binding**: after initial cutover, the explicit `database_host_binding`
  ...
  non-secret value, compares environment bindings independent of Terraform list order, and rejects
  every accompanying command, identity, or environment change.
