...
| Standalone tenant deployment | implemented | `fdaictl provision azure`; `fdai-up.sh`; standalone managed-host modules; focused package and Genesis tests | Connected and artifact-offline modes share one local coordinator, exact terminal approvals, and a distinct managed-host identity without GitHub workflow dispatch. Governed Azure receipts remain open. |
| Verified public-state adoption | implemented | Application-state staging, standalone coordinator and managed-host checkpoints, focused package and Genesis tests | The advanced recovery path retains the original local state, separates only resource-group ownership, requires an empty remote application backend, pushes once behind an immutable claim, and verifies authoritative readback. A signed-release Azure adoption and convergence receipt remains open. |
...
| Explicit image candidates and targeted PR packaging | implemented | `container-supply-chain.yml`, image selector, workflow and Genesis regressions; 182 focused tests passed | Publication requires protected-main dispatch and explicit images. Genesis requests only its required images and cannot reuse a successful PR or partial candidate; digest verification remains independent. |
...
| Independent-service protected deployment | validated | `config/independent-service-live-evidence-manifest.json` and `config/independent-service-remote-evidence.json` | Protected plans bind source, backend, target, identities, and images; peer isolation and rollback evidence are retained. A standard isolated-Executor update may adopt only the previously absent `FDAI_ISOLATED_EXECUTOR_LEGACY_UNBOUND_TRANSITION=0` binding; enabling or replaying that transition remains blocked. |
| Core package distribution profile | implemented | `service-matrix.json`; `service_contract.py`; `drift_contract.py`; focused service image and drift tests | The independent Core root accepts only its standard image or the Cost Governance distribution image. Both remain exact-source, digest, attestation, plan, health, and rollback bound; deploying the package image grants no activation or action authority. |
...
| 2026-09-12 | implemented | Made the local standalone coordinator and manual managed host the tenant deployment transport, removed public workflow-dispatch commands, and added an artifact-offline deployment appliance path. | `current change`; deployment CLI, appliance scripts, focused package and integration tests | Retain connected and appliance-based Azure deployment receipts; repository workflows remain CI and release automation only. |
| 2026-09-12 | implemented | Added the Cost Governance distribution to the closed Core service image contract after package deployment isolation proved that the platform root owns only the two package Jobs. Service planning, exact apply, pre-refresh drift, health, and rollback continue to bind the complete digest-pinned image reference. | `current change`; service matrix and contract; focused standard, package-profile, wrong-service, and drift regression tests. | Publish one exact Cost Governance candidate, retain a zero-destroy Core plan and exact apply, and verify that Core and both package Jobs use one digest before lifecycle installation. |
| 2026-09-12 | implemented | Added a dedicated protected provider-schema plan/apply mode after the broad operations-gateway target admitted an unrelated Operator API role replacement. The new mode admits only the provider-schema Job, binds one exact attested Core image, runs the Job after apply, and verifies its durable source/generation plus any Heimdall, Forseti, and Saga review chain. | Failed protected plan `34677766334`; `current change`; focused provider runtime, deployment workflow, CLI, Terraform, and evidence checks. | Publish the exact revision, repeat the final Core baseline on that candidate, then retain a zero-destroy provider-schema plan, exact apply, and deployed agent-chain receipt for Issue #290. |
| 2026-09-12 | implemented | Restricted model-only Terraform planning to the Azure OpenAI capability deployment collection so existing account and role-assignment resources cannot enter through whole-module targeting. | `current change`; failed protected plan `34677766334`; `.github/workflows/deploy-dev.yml`; 87 focused model lifecycle and deployment workflow tests; CI contract check. | Publish the corrected workflow, retain a model-binding plan with no unrelated changes, and complete the exact Core image binding and runtime evidence for Issue #90. |
...
- [x] Candidate-only publication, targeted PR packaging, and Genesis caller compatibility pass
  the focused selector, workflow, application, supervisor, and image tests.
...
- [ ] Retain a zero-destroy Core service plan and exact apply for the Cost Governance distribution,
  then verify one shared image digest across Core and both package Jobs before lifecycle install.
