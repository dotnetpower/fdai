...
> **Kubernetes lifecycle parity:** The deployed analyzer Job receives the reviewed
> `FDAI_KUBERNETES_*` bindings and runs the same bounded one-shot lifecycle collection pass as the
> local analyzer loop. Missing bindings leave the optional lifecycle source unavailable.
