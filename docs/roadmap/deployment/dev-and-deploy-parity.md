...
Local preparation requires OPA before dependency or provider work; VS Code task terminals include `~/.local/bin` for user-installed tools. When a model artifact is selected, preparation writes its exact `LLM_RESOLVED_MODELS_SHA256` into the Operator environment; startup rejects a missing pin or changed artifact. With `FDAI_LOCAL_NO_AZURE_DEPLOYMENT=1`, preparation skips Terraform discovery only after verifying the explicit `FDAI_LOCAL_RESOURCE_GROUP` in the active subscription. The mode and scope both participate in the environment cache digest. PostgreSQL and Redpanda stay local, missing sources remain unavailable, and no fake executor or execution gateway is selected. HTTP liveness and `pantheon_ready` alone do not prove consumer readiness or successful conversation inference.
...
Developer guidance points to those launch surfaces, live E2E clears both CLI flags, and the fixed `Contributor` mode cannot validate `Approver` or `Owner` approval access.
