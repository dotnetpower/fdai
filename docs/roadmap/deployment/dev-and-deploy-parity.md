...
Production replicas share the server consumer group so one replica answers each request. The singleton local core uses a process-scoped server group so a restart begins at the current physical-topic offset instead of replaying unrelated Pantheon traffic from a previous process. The generated local Operator environment also derives a semantic outbox namespace from the checkout path. Replicas in one checkout share a queue, while linked worktrees that share PostgreSQL cannot claim or publish each other's pending semantic turns. Deployed replicas keep the configured production namespace.
