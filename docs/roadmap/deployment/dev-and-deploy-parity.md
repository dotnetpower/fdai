...
| Subscription conversation routing parity | implemented | `scripts/deployment/local/prepare-operator-service-env.sh`; `scripts/deployment/local/ensure-semantic-topic-partitions.sh`; `infra/main.tf`; Operator and Core focused checks | Both venues use the same catalog-matched deterministic route, server-owned subscription scope, masked ARM receipt, session partition key, and two-worker consumer contract. Local uses the current Azure CLI credential and reconciles Redpanda to at least two partitions. Deployment uses the Reader managed identity and rejects Event Hubs partition counts below two. |
