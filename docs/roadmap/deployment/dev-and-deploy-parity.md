...
- **Deploy truth**: `terraform apply` provisions the Azure-side realizations of the CSP-neutral contracts. The **LLM subset is deployer-scoped**: the bootstrap resolver queries the deployer's identity against the target region's catalog, provisions **only what the deployer has permission to create**, and records the resolved `{capability → deployment}` mapping plus resolver input provenance in the artifact. Independently deployed services retain two inactive revisions so a protected apply can preserve both the pre-apply current revision and an exact last-ready rollback baseline. Core safeguard evidence binds the exact deployed source revision, isolated execution admits legacy unbound commands only through an explicit authority-cutover transition that defaults off, and recovery effect intake accepts only the configured Heimdall observer identity after the typed agent relay. The isolated public `dev` path may bind the verified Azure CLI human as its Terraform management principal, while protected paths retain the stable deploy UAMI and runtime execution stays on separate identities. VS Code terminals prepend the user-owned `~/.local/bin` so the checksum-pinned `azd` bootstrap is discoverable without changing system paths or granting authority.
Disposable scenario cleanup preserves the same authority split. `destroy-plan` runs in the
`plan-only` environment, renders the exact delete addresses with
`terraform plan -destroy -refresh=false`, and grants no temporary deployment or network authority.
Only a separate reviewed `destroy` request with the exact confirmation can acquire bounded
authority and apply a destroy plan. A plan-only result never authorizes mutation.
All profiles share **one control path**: only composition-root adapters and credentials differ ([project-structure.md § Customization via Dependency Injection](../architecture/project-structure.md#customization-via-dependency-injection)). Local and deployed profiles also load the same pinned semantic hard bounds for LLM routing and incident detection; only the active versioned configuration value may differ within those bounds, and neither profile can derive authority from the declaration. Its reviewed docstring records the existing boundary and does not create a runtime, change state ownership, or allow fixtures. The shared authenticator carries a bounded verified username into the IAM display projection for both browser Entra and local Azure CLI identities; authorization continues to use only the stable `oid` and verified App Roles. Adding a real Azure client is a fork-side injection; it MUST NOT edit `core/`. Teams Workflows follows the same parity rule: the local Operator Service encrypts the URL with domain-separated key material and stores only ciphertext, while deployment writes a versioned Key Vault secret through a dedicated managed identity scoped to that secret. Both modes verify the stored version before testing and return only secret-free metadata. Saving remains separate from explicit local or deployed A2/A4 activation. The standard `console: prepare full stack` task makes the local profile's activation decision explicitly through `FDAI_LOCAL_TEAMS_NOTIFICATION_ACTIVATION=1`; direct script users remain inactive by default. The runtime-environment cache binds this value and the optional Kubernetes lifecycle flag into its digest, so changing either input always regenerates the environment.
...
The shared Operator data-source manifest also assigns the three Assurance Twin read routes to the same service-local projection in both profiles. This ownership reports an explicit unavailable reason when PostgreSQL isn't configured and doesn't change WARA, cost-governance, or other route authority. Both profiles use the focused outbox lifecycle facade to start the retry-safe Incident intervention worker only when PostgreSQL and the event bus are configured; the same readiness check blocks service readiness if that worker stops, without granting the Console or Operator API execution authority. The trusted recovery observer identity is an unconditional Core service binding and never depends on optional Teams approval configuration.
The WAF and CAF assessment consumer follows the same parity rule. Local and deployed Operator profiles consume the same logical topic, validate the same immutable snapshot, and write the same PostgreSQL projection. The protected live-validation workflow is deliberately audit-only because the deploy runner has no assessment sender role; its artifact never substitutes for an Operator projection. Both profiles retain the existing adapters facade for credential construction, without changing venue selection.
...
Cost Governance disclosure delivery follows the same parity rule. Both profiles persist one content-free authorized receipt before returning cost data, block the response when audit persistence fails, and keep the immutable receipt behind the same 400-day retention, 30-day purge grace, legal-hold, and tombstone contract. Cost analytics also uses one scheduled contract: local execution uses only the operator's existing Azure CLI read identity after explicit source enablement, deployment uses the dedicated read-only managed identity, and both persist the same bounded run receipt and analytics snapshot. A current partial tick or failed refresh with a current retained snapshot satisfies degraded process readiness while missing facets and refresh failure remain explicit; a missing or stale snapshot remains unready and never selects Sample data. The scheduled job never creates a `DecisionCase`, approval, or execution request. Local fixtures never become live W7 evidence.
## Audit - What Works Local, What Needs Azure
Test-context commands reuse the same durable-outbox facade and readiness dependency in local and deployed profiles. Import grouping grants neither fixture activation nor execution authority.
Local preparation requires OPA before dependency or provider work; VS Code task terminals include `~/.local/bin` for user-installed tools. When a model artifact is selected, preparation writes its exact `LLM_RESOLVED_MODELS_SHA256` into the Operator environment; startup rejects a missing pin or changed artifact. Local Console preparation never initializes or reads Terraform state. It verifies the explicit `FDAI_LOCAL_RESOURCE_GROUP` in the active Azure CLI subscription and may read that one server-owned setting from the gitignored `console/.env.local`; duplicate keys fail closed, and the scope participates in the environment cache digest. After a successful run, preparation may reuse the prior owner-only generated scope. PostgreSQL and Redpanda stay local. A development operations gateway is bound only when exactly one scoped Function App resolves, and its audience is extracted from the validated private `VITE_MSAL_API_SCOPE` shared with the local Operator Service. The local Console receives no executor identity, and durable promotion evidence, applicable human approval, safeguards, and independent effect verification remain mandatory for Azure changes. HTTP liveness and `pantheon_ready` alone do not prove consumer readiness or successful conversation inference.
Snapshot as of 2026-07-21. "Automated test" means pytest or a committed mock invoked by the test runner. "Full-stack local" means the VS Code compound launch using browser Entra for the operator and the current Azure CLI context for server-side Azure adapters. Test fixtures are never enabled by that launch profile. [Deployed AKS credentials](runtime-deployment-profiles.md#identity-and-secrets) use explicit federation; Container Apps retains attached MI and may bind Thor's isolated Executor only through the default-off exact Kubernetes API and public-CA contract owned by [Runtime Deployment Profiles](runtime-deployment-profiles.md#attached-identity-kubernetes-authentication). This binding cannot grant roles or change local authority. Local venue policy, event/role/approval boundaries remain unchanged, and mocked token tests are not live federation or permission evidence.
...
The process launcher sets `FDAI_EXECUTION_VENUE=local` independently from `RUNTIME_ENV`. Local state uses Docker PostgreSQL on `127.0.0.1:5432` with the owning service role, and local event transport uses Redpanda on `127.0.0.1:19092`. Console preparation bounds disposable development history at 1 GiB by default. When the loopback `fdai` database exceeds `FDAI_LOCAL_DATABASE_RECREATE_MAX_BYTES`, restart first stops the verified same-checkout supervisor, preparation rejects cache reuse, and the entire database plus dedicated local broker generation is recreated before applying the canonical legacy and service migrations. A retained reset marker keeps startup blocked until all empty local consumer groups and topics are removed together. Redpanda defaults every local topic to 24-hour and 256 MiB per-partition retention; inactive consumer-group offsets use the same 24-hour horizon with a one-minute cleanup check, while the startup probe topic remains stricter. Preparation refuses remote, alternate-port, alternate-database, active-connection, active-consumer, foreign-supervisor, and nonlocal Docker targets. Local readiness also rejects a measured primary Core lag above 1,000 records. This development-only generation reset never truncates one audit or observation family independently; deployed evidence remains subject to verified archive, restore, retention, hold, and purge policy.
...
The second `db-integration` shard runs the provider-contract Docker matrix against only the
isolated validation PostgreSQL port `5433` and Redpanda host port `19092`; Compose clients use
`redpanda:29092`. The same assertion file runs the fake and real adapters, but real mode is an
explicit CI opt-in. It creates one exact temporary database and UUID-scoped topics and consumer
groups, then deletes only those resources. A loopback-only socket guard blocks cloud calls, and
missing Docker services fail the real shard instead of becoming a successful provider result.
...
The local analyzer launcher explicitly forwards the service-owned `FDAI_STATE_STORE_DSN` alongside
its inventory DSN. This binds the same read-only decision-evidence admission provider used by the
target selector; omitting it holds state-bearing candidates as `unverified_state_fact`. The binding
...
The helper preserves redirects, permits loopback HTTP only, and stops when the active tenant or registration permission is wrong. Local transport always uses loopback Redpanda. Deployed Event Hubs credentials and refresh behavior remain owned by the deployed runtime profile.
...
before Azure provider access instead of allowing the core runtime to fail after launch.
...
The local runtime environment generator supplies the active Azure CLI subscription and explicitly selected resource group to the bounded Azure read-investigation adapter. It discovers development operations gateway candidates only inside that scope and binds the URL only when exactly one candidate resolves. It derives the audience from the validated private `VITE_MSAL_API_SCOPE`, which is the same Operator API audience used by deployment. No candidate disables the wrapper, multiple candidates stop preparation, and a configured gateway failure reports unavailable without a direct-ARM fallback. The gateway uses separate reader and executor managed identities and does not give the local read
API an execution identity. Upstream deployment Terraform enables the development-only mutation operations for
...
The local runtime environment generator writes the canonical logical and physical topic names for loopback Redpanda without reading deployment state. It verifies the selected resource group in the active Azure CLI subscription before creating the environment. Both profiles execute the same explicitly typed semantic JSONB claim and projection statements. It also
...
| OHL independent effect observation | Deployment binds VM Scale Set and VM-start collectors to a Key Vault-backed Ed25519 seed, the FinOps and Resilience executor credential lineages respectively, and distinct observer, source, and verifier lineages; the Core workload receives the inventory-reader Managed Identity only as the independent observation source, and disabling the binding removes that exact extra identity | Local interactive stays unbound; fixture tests may inject deterministic keys, and governed deployed mailbox evidence remains open |
