...
|------|-------|--------|----------|-----------|
| 2026-08-25 | implemented | Strengthened the shared independent-service rollback contract without changing local/deployed composition, service ownership, identity, or transport. Protected apply now rejects an unhealthy rollback baseline, and every deployed service retains one inactive Container Apps revision while the plan guard permits only the one-time `0 -> 1` retention transition. | Failed Core applies `32839129965` and `32842018230`; `current change`; focused rollback-baseline, plan-guard, five-root Terraform, and independent-service checks. | Restore a healthy Core baseline and retain a successful exact apply plus verified automatic rollback receipt before claiming deployed validation. |
