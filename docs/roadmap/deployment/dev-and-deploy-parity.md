...
AKS deployment selects the explicit projected service-account token through Azure Identity SDK;
Container Apps retains its attached Managed Identity. Local venue selection remains authoritative
and does not activate either deployed credential from a test fixture. Both deployed transports
keep the same events, readiness, roles and approval boundaries. Local mocked token checks prove
selection and failure behavior only, not deployed federation or downstream permissions.
