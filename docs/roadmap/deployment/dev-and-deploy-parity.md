...
> **Kubernetes lifecycle parity:** The deployed analyzer Job and local loop run the same bounded
> pass. The `FDAI_KUBERNETES_*` cluster binding activates it; an inventory DSN alone does not.
...
|------|-------|--------|----------|-----------|
| 2026-08-27 | implemented | Made the optional Kubernetes cluster binding, rather than the shared inventory DSN, activate lifecycle collection in both analyzer venues. Inventory-only local and deployed ticks no longer fail for a Kubernetes source they did not configure. | `current change`; analyzer lifecycle and continuous-graph audit checks (`17 passed`); Ruff and strict mypy. | Retain configured local and deployed lifecycle receipts separately; no live source was invoked. |
