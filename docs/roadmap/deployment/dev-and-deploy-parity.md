...
| Reusable service Terraform module compatibility | implemented | `infra/services/**/modules/**/versions.tf`; Terraform validation and TFLint | Every reusable service module declares Terraform `>= 1.9`; provider ownership remains with the service roots. |
...
volumes cannot inherit stale file markers. A missing or changed Console dependency stage runs
lockfile-backed `npm ci` instead of asking the operator to repair `node_modules` manually. Each
external command runs through `run-bounded-command.py`, which streams output and terminates the
complete child process group after either its total or no-progress deadline. `--force` invalidates
every stage.

The supervisor launches each allowlisted `run-console-service.sh` in parallel with its own lock,
fingerprint, log, and lifecycle. It emits `started` after all launchers are spawned, but the VS Code
task remains active until the supervisor emits exactly one terminal `ready` or `failed` event. The
complete readiness gate defaults to 60 seconds and has a 65-second outer process-group deadline.
Every child exit, readiness failure, signal, or managed-lock failure before readiness emits
`failed` and returns nonzero. Duplicate explicit starts run concurrently through the managed lock
and fingerprint reuse path instead of being silently ignored. `console: wait full stack ready` is
a separate ten-second diagnostic after a successful start, not a second startup phase. The
Core-only recovery task also waits for a fresh Pantheon heartbeat instead of treating process spawn
as readiness. Changed or foreign ownership replaces only the managed task or fails.
