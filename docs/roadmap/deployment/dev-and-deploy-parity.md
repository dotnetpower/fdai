...
Local preparation requires OPA before dependency or provider work; VS Code task terminals include `~/.local/bin` for user-installed tools. When a model artifact is selected, preparation writes its exact `LLM_RESOLVED_MODELS_SHA256` into the Operator environment; startup rejects a missing pin or changed artifact. With `FDAI_LOCAL_NO_AZURE_DEPLOYMENT=1`, preparation skips Terraform discovery only after verifying the explicit `FDAI_LOCAL_RESOURCE_GROUP` in the active subscription. When the process environment does not define them, preparation may read only those two server-owned settings from the gitignored `console/.env.local`; duplicate keys fail closed, and the mode and scope both participate in the environment cache digest. PostgreSQL and Redpanda stay local, missing sources remain unavailable, and no fake executor or execution gateway is selected. HTTP liveness and `pantheon_ready` alone do not prove consumer readiness or successful conversation inference.
...
Local preparation reads the workspace customer GUID from the applied Terraform `log_workspace_customer_id` output. If an older or targeted state does not expose that output, or the operator selected an explicit no-deployment read scope, it lists workspaces only inside the selected resource group and accepts the fallback only when exactly one workspace exists. Zero workspaces leave the provider unavailable, and multiple workspaces stop preparation instead of choosing one implicitly. Regeneration removes any stale local workspace id.
