...
Teams Workflows endpoint configuration follows the same parity rule. The local Operator Service
encrypts the saved URL with domain-separated key material and stores only ciphertext in its
loopback database. A deployment writes a versioned Key Vault secret through a dedicated managed
identity scoped to that single secret. Both modes verify the stored version before testing and use
the same role-gated reveal response described in
[User RBAC and Entra Identity](../interfaces/user-rbac-and-identity.md#implementation-status).
