...
Cost Governance disclosure delivery follows the same parity rule. Both profiles persist one content-free authorized receipt before returning cost data, block the response when audit persistence fails, and keep the immutable receipt behind the same 400-day retention, 30-day purge grace, legal-hold, and tombstone contract. Cost analytics also uses one scheduled contract: local execution uses only the operator's existing Azure CLI read identity after explicit source enablement, deployment uses the dedicated read-only managed identity, and both persist the same bounded run receipt and analytics snapshot, participate in full-stack readiness when enabled, and report stale or failed collection without selecting Sample data. The scheduled job never creates a `DecisionCase`, approval, or execution request. Local fixtures never become live W7 evidence.
## Audit - What Works Local, What Needs Azure
The optional Container Apps Kubernetes binding changes only the deployed Thor runtime's exact
API, public CA, audience, and namespace inputs. Mounted CA and inline public CA use the same
verified TLS transport. The default remains unbound, and the setting cannot enable authority
cutover, select a human credential, or grant roles. Local shadow execution, service-owned storage,
Core and Operator identities, and channel approval transport remain unchanged. See
[Runtime Deployment Profiles](runtime-deployment-profiles.md#attached-identity-kubernetes-authentication).
...
Snapshot as of 2026-07-21. "Automated test" means pytest or a committed mock invoked by the test runner. "Full-stack local" means the VS Code compound launch using browser Entra for the operator and the current Azure CLI context for server-side Azure adapters. Test fixtures are never enabled by that launch profile. [Deployed AKS credentials](runtime-deployment-profiles.md#identity-and-secrets) use explicit federation; Container Apps retains attached MI. Local venue policy, event/role/approval boundaries remain unchanged, and mocked token tests are not live federation or permission evidence.
