...
| 2026-08-17 | implemented | Resolved repository-shipped assets from the installed image layout instead of a fixed parent depth. The analyzer, inventory, and observation ticks now locate `rule-catalog` and `config` in both a checkout and the container image. | `current change`; `repo_assets.py` and the three delivery ticks; focused resolver tests passed 3 cases and Ruff passed. Every deployed analyzer execution had been failing with `probe catalog root not a directory: /app/.venv/rule-catalog/probes`. | Confirm a successful deployed analyzer execution after the next image rollout. |
### Remaining work
