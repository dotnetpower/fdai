...
| Local and deployed composition parity | implemented | `.vscode/tasks.json`, `.vscode/launch.json`, `scripts/deployment/local/`, `infra/`, `fdai_operator_service/composition.py`, `postgres_read_investigation_replay.py`, `incident_intervention_runtime.py`, service integration tests, and focused Operator checks | Composition roots select credentials and adapters without changing evidence authority. Local and deployed Operator composition register the same Reader-scoped `GET /browser-evidence` route, durable read-investigation completion bridge, and authenticated Incident intervention route over versioned topics, PostgreSQL stores, and replay-safe outboxes. Incident intervention rechecks the authoritative target and lifecycle before recording a no-authority proposal; it never calls an executor. The dedicated local restart task changes supervision only. Missing PostgreSQL remains unavailable rather than synthetic. |
...
| 2026-08-29 | implemented | Added the same optional Cost Governance package, activation, collection, projection, and Console source contracts to local and deployed composition. | `current change`; package, Operator, Console, migration, image, and Terraform checks. | Retain paired live-authoritative local and deployed campaign receipts before claiming runtime validation. |
