...
| 2026-09-14 | implemented | Updated the supported `tools.console` launcher, browser recovery error, CLI guide, and executable workflow example for paired API confirmation. A focused test now passes the launcher's environment through the real Operator parser. | `current change`; `tools/console.py`; focused launcher and Operator environment tests; documentation checks. | Repeat the independent integrated critique. |
| 2026-09-14 | implemented | Updated the exact VS Code workspace contract for the 27-task topology, explicit Browser Entra commands, immutable API expectation, and separate Azure CLI debug chain. The required integration gate now exercises the hardened launch shape. | `current change`; `test_vscode_workspace_performance.py`; focused workspace contract suite. | Repeat the independent integrated critique. |
| 2026-09-14 | implemented | Corrected cache-output validation so every marker and generated flag assertion returns failure immediately. A partially corrupted Operator environment can no longer pass because the final confirmation line happens to match. | `current change`; `prepare-console-full-stack.sh`; focused whole-pair and single-flag corruption tests. | Reconcile the exact VS Code task topology contract. |
| 2026-09-14 | implemented | Pinned the VS Code Operator debugger itself to local Browser Entra and disabled both API CLI-principal flags in its process environment. A concurrent CLI-debug preparation cannot switch the debugger-owned API after its browser-mode prelaunch. | `current change`; `.vscode/launch.json`; focused workspace launch contract test. | Repeat the independent integrated critique. |
| 2026-09-14 | implemented | Removed the mutable marker reread from auth-sensitive child launchers. The supervisor now accepts one expected mode, verifies the prepared marker once, and passes that immutable value to every child; standard tasks and the watchdog pass Browser Entra while the separate debug task passes Azure CLI. | `current change`; local launch scripts and VS Code tasks; focused supervisor, standalone launcher, and workspace task tests. | Run the independent integrated critique again. |
| 2026-09-14 | implemented | Hardened both preparation caches against generated-output tampering or stale manual rewrites. Cache reuse now requires the mode marker and both generated Operator flags to match the requested mode exactly once. | `current change`; `prepare-console-full-stack.sh`; focused mutated-output cache regression test. | Carry one immutable mode value across concurrent preparation and startup. |
| 2026-09-14 | implemented | Replaced persistent CLI-auth flag instructions with the explicit launcher and task commands, and documented the fixed `Contributor` ceiling beside the approval limitation. | `current change`; `DEVELOPING.md`; `console/README.md`; `user-rbac-and-identity.md`; bilingual documentation checks. | Complete the independent integrated critique and authenticated Browser Entra validation. |
| 2026-09-14 | implemented | Added an HTTP authority-boundary regression proving Reader receives `count_only` while verified Approver and Owner principals receive `full` approval detail. The test also verifies the detail query flag passed to the authoritative read model. | `current change`; `test_operator_service_composition.py::test_hil_queue_detail_level_follows_verified_operator_role`; three role cases passed. | Complete operator-facing recovery guidance and final integrated review. |
| 2026-09-14 | implemented | Added the requested operator auth mode to the active service-environment stage cache identity. A mode switch now regenerates the Operator environment and its mode marker even when every file input is unchanged. | `current change`; `prepare-console-full-stack.sh`; focused stage-cache tests for both modes. | Add an approval-route authority regression test and complete operator guidance. |
| 2026-09-14 | implemented | Added the requested operator auth mode to the legacy full-stack preparation cache identity. Switching between Browser Entra and Azure CLI debug can no longer reuse a complete preparation result from the other mode. | `current change`; `prepare-console-full-stack.sh`; focused legacy-cache tests for both modes. | Bind the active staged service-environment cache to the same mode. |
| 2026-09-14 | implemented | Added the bounded `--auth-mode` selector to the standalone full-stack web launcher and forwarded it to preparation. No argument selects Browser Entra; Azure CLI debugging is deliberate and names its mode at invocation time. | `current change`; `start-console-web.sh`; focused launcher argument tests. | Bind both preparation caches to the selected mode. |
| 2026-09-14 | implemented | Made the standard VS Code preparation command state `browser-entra` explicitly and added a separately named Azure CLI debug task that states its `Contributor` ceiling. The preparation entry point validates and forwards the same bounded mode, so operators no longer need to persist a hidden Vite override. | `current change`; `.vscode/tasks.json`; `prepare-console-full-stack.sh`; focused workspace task and preparation-argument tests. | Extend the standalone web launcher with the same explicit mode selector. |
| 2026-09-14 | implemented | Pinned the standard VS Code frontend debug configuration to Browser Entra with both CLI-principal values disabled. The compound launch no longer inherits a stale private Vite identity override. | `current change`; `.vscode/launch.json`; focused workspace launch contract test. | Add an explicitly named CLI-debug preparation task rather than overloading the standard task. |
| 2026-09-14 | implemented | Added a stack-wide prepared-auth-mode preflight before any local service fan-out. A missing or invalid mode now leaves every service stopped instead of producing a partial stack. | `current change`; `start-console-services.sh`; focused supervisor tests. | Pin the direct VS Code launch path to the same standard Browser Entra contract. |
| 2026-09-14 | implemented | Added an Operator process preflight that compares the generated API enablement pair with the prepared Console auth mode before Uvicorn starts. A mixed Browser Entra and CLI-principal stack now stops at the launcher boundary. | `current change`; `run-console-service.sh`; focused Operator restart tests. | Add a stack-wide preflight before any sibling service starts. |
| 2026-09-14 | implemented | Bound the managed frontend process to the prepared Console auth-mode marker and set both Vite enablement values explicitly. Vite no longer chooses a different principal from a stale `.env.local` value. | `current change`; `run-console-service.sh`; focused local Console launcher tests. | Add the corresponding Operator process and supervisor preflight checks. |
| 2026-09-14 | implemented | Required paired Vite enablement and confirmation before the browser can bypass MSAL for the local Azure CLI principal. A single stale browser environment value now fails closed instead of changing identity. | `current change`; `console/src/config.ts`; `console/src/config.test.ts`; focused Console configuration tests. | Bind managed launchers to the prepared mode so both confirmation values are set coherently. |
