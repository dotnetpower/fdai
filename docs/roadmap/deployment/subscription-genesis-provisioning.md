...
|------|------------------|------------------------|
| Operator entry point | `fdai-up.sh` supervises the private Foundation, tenant-local Entra configuration, managed-host application plan/apply, explicit initial inventory, and second plan in one process. The Entra bootstrap now converges the API, SPA, and approval-bot registrations plus their exact scope and role-group bindings without granting provider consent. Count-only inventory records replay from PostgreSQL through the Operator API. | Provider consent and Teams installation remain separately approved tenant steps. Exact-revision live Azure evidence for the complete ready receipt remains open. |
