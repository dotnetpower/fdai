...
| 2026-09-12 | implemented | Restored verification-only resume after a live runner-image apply claim exposed that complete-kit acquisition tried to recreate retained online and artifact-offline paths. Resume now reauthenticates the retained signed source and recreates only current-UID private derived snapshots without another public download. | `current change`; deployment-kit acquisition source; online and artifact-offline resume regressions; 811 deployment CLI and productization tests; strict typing and package build | Resume the claimed runner-image effect through verification only, then retain online and artifact-offline Azure convergence receipts. |
