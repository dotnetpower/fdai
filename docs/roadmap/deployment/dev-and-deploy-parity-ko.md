...
translation_source_sha: 2284ebcdbfa032ace8f2f4f2ccc4aebe928a5e0a
...
| Azure Monitor push 경로 조립 동등성 | implemented | `fdai_service_contracts/azure_monitor.py`; Operator 웹훅 경로 및 영속 outbox; Core 진단 Kafka 브리지; 독립 서비스 Terraform 입력; 집중 서비스 간 테스트 | 로컬 및 배포 조립은 동일한 정규화된 shadow Event 계약을 사용합니다. Kafka, PostgreSQL, 서명 또는 진단 구성이 없으면 원시 프로바이더 입력을 수락하지 않고 경로를 사용할 수 없음으로 유지합니다. |
