...
translation_source_sha: 01b398f5864a7281ab71221f354ee0b345411e93
...
AKS 배포는 Azure Identity SDK를 통해 명시된 투영 서비스 계정 토큰을 선택하고, Container Apps는
기존에 연결된 Managed Identity를 유지합니다. 로컬 실행 위치 선택은 계속 우선하며 테스트
픽스처로 배포 자격 증명을 활성화하지 않습니다. 두 배포 전송 방식은 같은 이벤트, 준비 상태,
역할과 승인 경계를 유지합니다. 로컬 모의 토큰 검사는 선택과 실패 동작만 검증하며, 실제
배포의 연합 인증이나 대상 서비스의 권한을 입증하지 않습니다.
