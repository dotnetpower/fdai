...
translation_source_sha: 1a3f19d9a324ca3dbf259ca308770e7e828fc0a6
...
| 로컬 및 배포 composition 동등성 | implemented | `.vscode/tasks.json`, `.vscode/launch.json`, `scripts/deployment/local/`, `infra/`, `fdai_operator_service/composition.py`, `postgres_read_investigation_replay.py`, 서비스 통합 테스트 및 focused Operator 검사 | Composition root는 근거 권한을 바꾸지 않고 자격 증명과 어댑터를 선택합니다. 로컬 및 배포 Operator composition은 같은 Reader 범위 `GET /browser-evidence` 경로와 versioned topic, PostgreSQL store 및 replay 계약을 사용하는 영속 read-investigation completion bridge를 등록합니다. 전용 로컬 재시작 태스크는 supervision만 바꿉니다. PostgreSQL이 없으면 합성 데이터 대신 사용 불가를 반환합니다. |
