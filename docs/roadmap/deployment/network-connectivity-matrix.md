...
| Direct operator-VM access | implemented | `infra/main.tf`, `infra/variables.tf`, and `tests/integration/infra/test_network_hardening.py` | Explicit same-subscription operator VNets receive direct workload peering and links to only the selected authoritative Private DNS zones. Selected Managed Identities receive only subscription `Reader`; data-plane roles remain separate. Runtime evidence remains separate. |
...
|------|-------|--------|----------|-----------|
| 2026-09-16 | implemented | Added optional direct peering and explicit Private DNS zone links for an operator host already running in Azure. | `current change`; `tests/integration/infra/test_network_hardening.py` (`3 passed`). | Retain an approved deployment plan, private DNS/TLS checks, effective-route readback, and least-privilege identity evidence from the selected VM. |
...
An operator host that already runs in an Azure VNet does not need Point-to-Site VPN or gateway
transit for its own traffic. Peer that VNet directly with the workload VNet and link only the
authoritative Private DNS zones selected in `operator_private_dns_zones`. Azure DNS Private
Resolver is needed only when DNS originates outside linked VNets or an existing custom DNS design
requires conditional forwarding. Peering remains non-transitive, and network access does not grant
Azure RBAC or service data-plane permissions. Use `operator_inventory_principal_ids` to grant
subscription `Reader` only to explicitly selected Managed Identities. Add data-plane roles
separately for a concrete service operation.
