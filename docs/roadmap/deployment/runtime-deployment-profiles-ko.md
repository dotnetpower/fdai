...
초기 수집과 반복 수집은 모두 원본 정책의 페이지 및 동시성 제한을 적용합니다. 스냅샷 정책은
현재 변환 용량과 일치하도록 Resource 50,000개 또는 관계 200,000개를 넘을 수 없습니다.
ARM 페이지 수집은 정책의 레코드 및 누적 응답 바이트 제한도 적용합니다.

초기 인벤토리 실행과 반복 실행에는 같은 변환 한도를 적용합니다. 한도를 넘은 후보는 이전 활성 세대를 유지하며, 불완전한 관측으로는 리소스 삭제를 확정할 수 없습니다. 두 경로 모두 영속 청크 쓰기를 전달하고 기존 실행 잠금 아래에서 정확한 문맥의 미완료 후보를 복구합니다. 복구는 연속 토큰을 신뢰하지 않고 공급자 전체 읽기를 반복하며, 원래 관측 시각을 보존하고 봉인과 승격 전에 새 관계 및 최종 커버리지 검증을 요구합니다. 보존된 식별자가 바뀌거나 누락되면 해당 후보를 차단합니다. 원본 누적과 보강 결과는 각각 정규 레코드 16 MiB 상한을 유지합니다. 전용 Linux 진입점은 수집, 보강 및 게시 전체에 OS 주소 공간 2048 MiB 한도를 적용합니다. `FDAI_INVENTORY_MEMORY_LIMIT_MIB`는 256-4096 MiB를 허용하지만 더 낮은 기존 한도를 높일 수 없으며, 시작 시 여유 공간이 부족하면 수집 전에 차단합니다. DB 메모리와 임베드된 호출은 별도 소유 범위이며 읽기, 배포 또는 실행 권한은 바뀌지 않습니다.
전체 기한에는 보강, 승격, 알림도 포함합니다. 취소 정리에도 기한을 두어 후보가 무기한 수집 상태로 남지 않도록 합니다.
...
runtime-settings, ontology-status, collection-health, change-accelerator 및 private-cluster
proposal database pool을 닫습니다.

관리 호스트는 선택한 Deployment 이름, 이미지 참조, 복제본 수 범위를 기록합니다. 상태 재조회는
점검 지점 조정기는 서술자에 안전한 비공개 상태, 잠금, 다이제스트 및 원자적 JSON 교체를 하나의
상태 모듈에 위임하며, 별도의 순수 모듈이 계획 요약과 Foundation 파생 값을 검증합니다. 이 분리는
대상, 계획, 승인, 신원, 적용 또는 독립 재조회 계약을 바꾸지 않습니다. JSON 게시 실패 시 임의로
생성한 비공개 임시 파일을 제거합니다.
