...
| Offline kit and disconnected planning | in-progress | `fdai_deployment_cli.offline_kit`; release scripts; focused signature, exact-file-set, compatibility, and tamper tests | Packaging derives the host platform and stages hashed binary wheels. The drill installs only those wheels with `--no-index` before invoking `fdaictl`; a governed network-free run remains open. |
