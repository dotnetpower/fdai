...
| Offline kit and disconnected planning | in-progress | `fdai_deployment_cli.offline_kit`; release scripts; focused signature, exact-file-set, compatibility, and tamper tests | Packaging derives the staging host platform, exports hashed production dependencies, and stages the CLI plus every transitive binary wheel; explicit cross-platform labeling is blocked. The network-free Terraform air-gap drill remains to be run. |
