...
`fdaictl` repeats verification and passes its installed Python to fixed packaged Genesis launchers; plan generation and reverification use that same CLI interpreter. Every Terraform binary and provider path uses the private
...
`genesis-up.sh` keeps the lower-level Foundation boundary. `fdai-up.sh` supervises exact TTY approvals, tenant-local configuration, exact-main images, protected application apply, and the second zero-change plan while keeping final subscription readiness false until its remaining evidence families close. The supervisor overlaps only independent local artifact, read-only discovery, provider-request, and probe-resource siblings, joins them before the next dependency, and keeps approvals, claims, Terraform state, cleanup, handoffs, repository writes, and protected application transitions serial. When Storage Shared Key is disabled, the image stage uses private builder and verifier VMs with FQDN-allowlisted Firewall Basic egress, no public VM IP or Storage resource, and verification-only restart after its immutable claim instead of Azure VM Image Builder. [Subscription Genesis Provisioning](subscription-genesis-provisioning.md) defines the full lifecycle.
