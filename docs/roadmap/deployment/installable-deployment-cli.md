...
| Offline kit and disconnected planning | in-progress | `fdai_deployment_cli.offline_kit`; release scripts; focused signature, exact-file-set, compatibility, and tamper tests | Packaging exports hashed production dependencies and stages the CLI plus every transitive binary wheel for the staging host platform; cross-platform labeling is blocked. The network-free Terraform air-gap drill remains to be run. |
