...
translation_source_sha: 92bbec4fea4a489b85ecde2560cf35015d802063
...
| 2026-09-05 | implemented | Cluster-wide legacy 마이그레이션 downgrade 검사와 적용된 service 스키마 검사를 분리하고 스키마를 변경하는 카탈로그 수명 주기 회귀를 직렬화했으며 forward Core 마이그레이션으로 root 소유 T2 lookup index를 복원했습니다. 로컬 composite action은 저장소가 소유하고 원격 action은 SHA에 고정합니다. 조정되지 않은 만료가 진행 중인 receipt 또는 보존 근거를 무효화하므로 만료되지 않는 서명 secret 두 개에는 resource-local Checkov 예외를 적용합니다. | `current change`, 집중 마이그레이션, workflow, action pin 및 Terraform 보안 검사 | 배포 전에 exact green required CI 및 supply-chain 증적을 보존합니다. |
| 2026-09-05 | implemented | 누락된 endpoint를 수명 주기 결속 전에 차단하는 복원된 journal 이전 endpoint 검증에 dangling relationship 회귀를 맞췄습니다. | `current change`, PostgreSQL 통합 assertion이 결정적 fail-fast 경계와 일치합니다. | exact green required CI 증적을 보존합니다. |
