...
| 범위가 제한된 Cost Governance 패키지 배포 | implemented | `deploy-dev.yml`, `enforce_plan_scope.py`, 집중 범위 및 workflow 테스트 | 요청에 결속된 `plan-cost-*` 또는 `apply-cost-*` 모드는 Cost Management Reader 역할 배정과 collector 및 analyzer Job만 대상으로 합니다. 독립 변경 주소 검증기는 destructive plan 검토 또는 아티팩트 보존 전에 그 밖의 모든 리소스를 거부합니다. |
