...
translation_source_sha: df7cef6ceb199f0e690e5c6c4c7c9df9ba5f0e73
...
| Scenario-lab AKS Store Demo 워크로드 | implemented | `render_aks_store_demo.py`, `prepare-runner.sh`, `infra/scenario-lab/outputs.tf`, 집중 scenario-lab 테스트 | 승인된 apply는 커밋과 SHA-256으로 고정된 upstream manifest 하나를 내려받고, 모든 워크로드 이미지를 digest로 고정하고, 두 frontend를 비공개로 유지하며, `order-service` replica 3개를 기존 장애 대상에 사용합니다. 보호된 apply 및 워크로드 준비 증적은 아직 필요합니다. |
...
- [ ] 별도로 승인된 장애 시나리오 모음을 실행하기 전에 모든 AKS Store Demo 워크로드가
  Ready이고, 실행 image가 검토된 digest에 결속되며, frontend Service 2개가 모두 비공개임을
  보여 주는 exact protected scenario-lab apply를 보존합니다.
