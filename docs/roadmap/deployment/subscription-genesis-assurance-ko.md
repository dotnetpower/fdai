...
translation_source_sha: b18fd6029cf5efd874e742a51f516de5f2c4ab9e
...
새 준비 작업은 선호 SKU 이름뿐 아니라 해당 지역의 전체 VM 카탈로그를 조회합니다. 서명된
[하드웨어 정책](../../../infra/genesis-runner-image/vm-sku-policy.json)은 x64/Gen2를 요구합니다.
빌더는 지속형 2 vCPU와 8 GiB, 검증기는 2 vCPU와
4-8 GiB, 기반 호스트는 지속형 4 vCPU와 8-16 GiB가 필요합니다. 이미지 VM은 64-GiB 관리형
OS 디스크를 사용합니다. 호스트만 임시 Local ResourceDisk 지원과 생성한 이미지를 담을 공간이
필요합니다. ARM, GPU, 기밀 컴퓨팅, 제한된 코어, 호환되지 않는 저장소 변형은 대체하지 않습니다.
선호 목록은 적격 하드웨어의 순서를 정하며, 목록 밖의 동등한 호환 SKU도 선택할 수 있습니다.

카탈로그 조회는 정확한 구독과 지역 하나에서 4페이지, 4096개 행, 8 MiB, 90초로 제한하며,
개별 요청은 최대 30초입니다. 별도의 할당량 조회도 30초로 제한합니다. 완전한 메타데이터,
적용되는 지역 및 영역 제한, VM 세 대의 지역 및 제품군별 합산 할당량을 확인한 뒤에만 네트워크
생성을 준비합니다. 불확실한 근거는 부재로 간주하지 않고 차단합니다. 전체 사용 제한, 하드웨어
불일치, 불완전한 근거, 할당량 부족을 구분하며, 하드웨어 정보가 불완전하면 후보 개수는 미확인으로
남깁니다. 비공개 스냅샷으로 선택을 재현할 수 있습니다.

준비 단계는 호스트 크기를 봉인하고, 새 이미지 계획은 선택한 두 VM과 호스트의 여유 할당량을
버전이 있는 검토 기록에 봉인합니다. 적용은 봉인된 선택과 할당량, 만료, 승인만 재확인합니다.
기반 계층은 계획 전과 새 적용 전에 호스트와 실제 관리형 이미지 또는 정확한 갤러리 버전의
OS 디스크 크기를 검사합니다. 갤러리 이미지는 일반화된 x64/Gen2 Linux 정의와 선택한 지역의
복제 완료 상태도 필요합니다. 호스트 및 사람 신원 조회가 끝나면 새 적용 시작 기록을 쓰기 전에
정확한 기반 계층 계획과 만료를 다시 검사합니다. 기존 적용 시작 기록이 있으면 효과만 검증하며
SKU 재선택, 크기 변경, 지역 전환, 적용 반복은 하지 않습니다. 기존 서명 정책은 원래의 제한된
선택 계약을 유지합니다. 사전 점검은 용량을 예약하거나 실시간 가격을 제시하지 않으며, 부분 상태
복구와 독립적으로 검증된 배포는 별도 작업입니다. 체크아웃 변경을 실제 운영에 사용하려면 새 서명
키트가 필요합니다.
...
Terraform은 Storage 정책과 호환되는 직접 이미지 그래프를 소유합니다. 비공개 빌더 및 검증기
VM, 확장, 할당 해제 및 일반화, 관리형 이미지 캡처와 전체 네트워크를 포함하며 FQDN 허용
목록이 있는 Firewall Basic 뒤에 VM과 분리된 공개 송신 IP 두 개를 둡니다. 숨겨진 스테이징이
Shared Key를 요구할 수 있으므로 Azure VM Image Builder, Storage, 이미지 템플릿은 금지합니다.
수락에는 이미지 출처, 확장 성공, 두 VM의 할당 해제가 필요하며 점유된 빌드는 재시도하지
않습니다. 정책이 추가한 `FirstPartyUsage=/Unprivileged`만 외부 소유로 처리합니다. 두 정확한
Firewall IP에서 그 값이 같거나 모두 없어야 하고 알 수 없는 태그가 없어야 하며 새 변경 없음
계획도 필요합니다. 정책 효과는 교체, Firewall 다시 연결, 모호한 적용 점유 재사용을 승인하지 않습니다.
...
별도 출력이 있고 인과 관계, 승인, 상태 또는 정리 의존성이 없는 동등 작업에만 범위가 제한된
병렬 실행을 허용합니다. 작업자는 이식 가능한 상태 레코드를 직접 쓰지 않습니다. 소유 상위
프로세스가 모든 작업을 기다리고 결과를 결정적인 순서로 정리한 뒤 상태 전이 하나를 내보냅니다.
실패가 발생하면 범위가 제한된 모든 동등 작업을 기다리거나 종료한 뒤 실패를 기록합니다. Azure
공급자 등록은 보존되는 변경으로 유지하며, 정책 프로브 리소스는 소유권을 확인하는 하나의 직렬
정리 경로를 계속 사용합니다. 병렬 분기는 승인 또는 성공을 게시하거나 Terraform 상태를
진행하거나 후속 단계에 권한을 부여할 수 없습니다.
...
독립 실행형 배포는 복제본이 아니라 검증된 번들을 사용하므로 수정에는 다시 빌드한 서명 키트가
