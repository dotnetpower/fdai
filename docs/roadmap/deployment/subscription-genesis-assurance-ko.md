...
translation_source_sha: c4509b8a12a590e2755b66650b28e80d1767a26f
...
| 역할 | CPU와 메모리 | OS 디스크 |
|------|--------------|-----------|
| 빌더 | 지속형 2 vCPU, 8 GiB | 64-GiB 관리형 |
| 검증기 | 2 vCPU, 4-8 GiB | 64-GiB 관리형 |
| 기반 호스트 | 지속형 4 vCPU, 8-16 GiB | 이미지를 담을 수 있는 임시 Local ResourceDisk |
...
VM 세 대의 지역 및 제품군별 합산 할당량을 확인한 뒤에만 준비합니다. 제품군 사용량이 없으면 미확인이며 0으로 가정해 대체하지 않습니다. 비공개 재현 자료는 사용
...
준비 단계는 호스트를 고정하고, 버전이 있는 이미지 검토 기록은 두 VM과 호스트의 여유 할당량을
봉인합니다. 계획 전과 새 적용 시작 기록 전에는 실제 관리형 이미지 또는 정확한 갤러리 버전의
디스크 크기로 호스트를 검사합니다. 갤러리 정의는 일반화된 x64/Gen2 Linux와 대상 지역의 유일한 복제 완료
상태가 필요합니다. 대소문자·공백 차이로 중복 근거를 허용하지 않습니다. 적용은 봉인된 선택, 할당량, 만료, 승인을 재확인합니다. 호스트와 사람 신원
...
로컬 라우터는 서명 키트의 이미지 및 기반 계층 계획·적용, Bastion 등록, 증명, 상태 인계에
...
Vault의 부재를 확인합니다. 명령 성공만으로 정리를 증명하지 않습니다.
...
기록에 넣지 않습니다. 더 새로운 서명 release는 기존 runner-image 시작 기록의 적용 출처를
...
다이제스트를 비공개 인계에 각각 연결합니다. 현재 실행 바인딩은 이미지 생성 축을 제외하며 승인
전에 이미지를 생성하지 않는 오케스트레이션 상태와 일치해야 합니다.
