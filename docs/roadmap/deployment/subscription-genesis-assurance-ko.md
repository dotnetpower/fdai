...
translation_source_sha: eadedd9062442bc077b016f6946f90d6d344234e
...
유한한 매니페스트마다 구독·환경별 잠금 하나, 봉인된 계획, 독립적인 사후 조건을 사용합니다.
필수 조건을 증명하지 못하면 `ready`가 아니라 `blocked`, `failed`, `cancelled`, `incomplete`로 종료합니다.

## 기반 및 외부 컨트롤 플레인

기반 계층은 첫 Azure 실행 호스트가 생기기 전에 범위가 제한된 로컬 컨트롤 플레인 예외를 사용합니다.

- **로컬 경계:** 로그인한 사람은 그룹, 비공개 상태, 네트워크, 신원, 호스트를 만들며,
  Key Vault, PostgreSQL, 모델, 애플리케이션 데이터 플레인에는 쓰지 않습니다.
- **상태 캐시:** `umask 077`, 모드 `0700` 디렉터리, 모드 `0600` 일반 파일, 제한된 수명을
  사용하고 링크와 비밀 입력을 허용하지 않습니다. 다이제스트만 기록하며 상태를 근거로 업로드하지 않습니다.
- **안정적인 이름:** 전역 고유 이름은 한 번 파생하고 다이제스트만 게시하며 재개 때 재사용합니다.
  부분 기반 시도 후 다른 상태 계정을 선택하지 않습니다.
- **원격 권위:** Terraform 상태의 계보, 일련 번호, 주소, ID, 관리 개수를 맞춰 이전하며 추론하지
  않습니다. 캐시 삭제 전에 원격 변경 없음 계획과 버전 관리 재확인이 필요합니다.
- **데이터 보호:** Blob 버전 관리, 일시 삭제, TLS, 키 인증·공개 접근 비활성화, 비공개
  엔드포인트·DNS, 임대를 사후 조건으로 요구합니다. 최선형 활성화로 기반 단계를 종결하지 않습니다.
...
| 소스 호스트 | 저장소, 변경 불가능한 커밋, 서명된 번들, 의존성 잠금, Managed Host 초기 구성 입력이 실행 매니페스트와 일치합니다. |
...
연결된 준비 작업은 서명된 [하드웨어 정책](../../../infra/genesis-runner-image/vm-sku-policy.json)으로
지역 전체 카탈로그를 읽습니다. 선호 목록은 호환되는 x64/Gen2 Managed Host 하드웨어의
순서만 정하며, 목록 밖의 호환 SKU도 선택할 수 있습니다. 또한 정확한 Canonical Ubuntu 24.04
Marketplace 버전 하나를 확인합니다.
정책은 크기가 제한된 링크 비추적 파일 서술자로 읽고, 안전하지 않은 소유권·링크·쓰기 권한과 읽는 중 변경을 차단합니다. CLI 설정을 사용할 수 없으면 개인 경로를 표시하지 않고 공급자 조회 전에 중단합니다.

| 역할 | CPU와 메모리 | OS 디스크 |
|------|--------------|-----------|
| 기반 호스트 | 지속형 4 vCPU, 8-16 GiB | 정확한 Marketplace 이미지를 담을 수 있는 임시 Local ResourceDisk |

ARM, GPU, 기밀 컴퓨팅, 제한된 코어, 호환되지 않는 저장소 변형은 대체하지 않습니다.
카탈로그 조회는 정확한 구독과 지역 하나에서 4페이지, 4096개 행, 8 MiB, 90초로 제한합니다. 다음 페이지가 누적 한도를 초기화하거나 같은 페이지를 반복할 수 없습니다.
개별 요청과 별도 할당량 조회는 각각 최대 30초입니다. 완전한 메타데이터, 지역 및 영역 제한,
호스트의 지역 및 제품군별 할당량을 확인한 뒤에만 준비합니다. 제품군 사용량이 없으면 미확인이며 0으로 가정해 대체하지 않습니다. 비공개 재현 자료는 사용
제한, 하드웨어 불일치, 불완전한 근거, 할당량 부족을 구분합니다. 하드웨어 정보가 없으면 후보
개수는 미확인으로 남기며 0으로 간주하지 않습니다. 조회 실패 시 완료된 스냅샷과 실패 단계를 보존하며 관측하지 못한 다이제스트는 미확인으로 남깁니다.

준비 단계는 호스트, 정확한 Marketplace 버전, 체크섬으로 고정된 초기 구성 도구 체인을
봉인합니다. 적용은 봉인된 호스트, Marketplace 이미지 메타데이터, 할당량, 만료, 승인을
재확인합니다. 호스트와 사람 신원
조회 후 기반 계층은 적용 시작 기록 전에 정확한 계획과 만료를 다시 검사합니다. 증적 없이 부분 효과만 남아 있어도 기존 기록은
효과만 검증하며 재선택, 크기 변경, 지역 전환, 적용 반복을 허용하지 않습니다. 기존 서명 정책은
원래 계약을 유지합니다. 완전한 독립 실행 키트 빌드는 새 출력 디렉터리만 사용하며 이전 릴리스를 지우지 않습니다. 용량이나 가격은 보장하지 않으며 복구와 배포 검증은 별도입니다.
체크아웃 변경을 실제 운영에 사용하려면 새 서명 키트가 필요합니다. 서명이 중단되면 재사용 가능한 서명을 남기지 않으며, 불완전한 메타데이터는 독립 검증에서 차단됩니다.

아티팩트 오프라인 배포에서 초기 구성 산출물을 내려받을 수 없으면 별도로 검증된 사전 준비 호스트
이미지를 선택할 수 있습니다. 이 선택 경로에서만 빌더와 검증기 용량, 관리형 이미지 출처,
이미지 전용 승인과 복구를 평가합니다.

### 호스트 및 기반 계층 실행

등록에는 공급자 호스팅 인증과 보호된 입력이 필요하며, 불가능하면 승인된 기존 호스트를
기다립니다. 등록·제거·GitHub 토큰과 데이터베이스 암호는 Terraform 입력·상태, 프로세스 인자,
Azure Run Command 페이로드, 로그, 채팅에 넣지 않습니다.

로컬 라우터는 기반 계층 계획·적용, Bastion 등록, 증명, 상태 인계에
완전한 절대 아티팩트·신뢰·프로필·변수·접근 경로를 요구합니다. 새 효과마다 실행, 소스, 단계,
정확한 근거 다이제스트, 최대 1시간의 UTC 구간에 연결된 모드 `0600` 승인이 필요합니다.
단일 사용자 파일은 `dev` 전용이며 staging과 production은 보호된 정족수를 유지합니다.
다른 단계, 변경된 다이제스트, 만료된 기록, 응답 없음은 권한을 부여하지 않습니다.

Run Command 실행 묶음 staging도 전송 전에 같은 경계를 적용합니다. 비공개 승인은 정확한
프로비저닝 프로파일과 전체 대상 서술자에 연결됩니다. 조정기는 활성 Azure 계정에서 테넌트와
구독 binding을 다시 계산하고, 현재 사람 행위자를 검증하며, 실행 전 기록을 쓰기 전에 선택한 VM
리소스 ID, 비공개 주소, 배포 UAMI를 다시 읽습니다. staging 승인은 이후 Terraform 또는 워크로드
적용을 승인하지 않습니다.

연결된 Foundation은 정확한 Marketplace 버전으로 Managed Host를 부팅하고 체크섬으로 고정된
초기 구성 스크립트를 검토된 VM 사용자 데이터로 제공합니다. 스크립트는 정확한 도구 버전을
설치하고 증명 매니페스트를 기록하며 자격 증명 흔적을 제거합니다. 이후 단계에 필요한 증명 및
상태 이전 도구만 노출합니다. Foundation 재확인은 증적을 발행하기 전에 호스트, 신원, 도구
체인, 네트워크, 변경 없음 계획을 검증합니다.

모든 효과는 변경 불가능한 시작 기록을 먼저 씁니다. 시작 기록이나 증적이 있으면 재시작 시
검증만 허용하며 적용, 등록, 전송, 이전을 반복하지 않습니다. 등록 증적은 정확한 사람의 시작
기록에 연결하고 Bastion 신원, 도구 체인, 서비스, GitHub 실행기를 새로 읽어 확인합니다.
등록 자료는 정확한 Bastion 터널의 SSH 표준 입력만 사용하며 Terraform, 인자, Run Command,
기록에 넣지 않습니다. 선택적 아티팩트 오프라인 이미지 경로는 기존 검증 전용 복구 계약을 유지합니다. 새로운 연결된
Foundation 실행은 이미지 빌드 증적을 요구하거나 채택하거나 기다리지 않습니다.
이 등록 token 금지 규칙은 이후 Run Command를 묶음 staging에 선택해도 그대로 유지합니다. 이
전송은 고정된 비밀 아닌 대상, 비공개 중계, 크기, 개수, 모드, digest 좌표만 받고 저장소 소유
부트스트랩과 digest로 고정된 수신기만 실행합니다. 변경 불가능한 시작 기록은 중계 수신과 호출보다
먼저 기록합니다. 중계는 대상 호스트의 비공개 출발지 주소를 고정하고, WSL은 페이로드 전송 전에
독립적으로 전달된 임시 인증서 digest를 고정합니다. staging 증적을 종결하려면 검증 전용 복구
시작 기록 한 번, 형식이 지정된 호스트 결과, 고정 호스트 정리, 중계 종료가 필요합니다. 클라우드
staging 산출물은 만들지 않으며 staging은 승인이나 Terraform 실행 권한을 전달하지 않습니다.
...
정확한 의도 기록, 아카이브·원시 작업 트리 삭제, 부재 확인 후 증적을 씁니다. 잔여물이 있으면
완료를 차단합니다.
...
완전한 준비 상태는 별도의 근거 게이트로 유지합니다. 기반 계층의 직접 실행과 정리 실패도 개인 인자와 파일 경로를 출력하지 않습니다. 정리는 성공 보고 전에 수행하며 원래 실패를 보존합니다. 주 스레드의 중첩 Genesis 실행은 종료 신호를 전달하고 상위 종료 유예 안에 자식 정리를 마칩니다. Foundation은 처음에 Terraform 로컬 backend를 사용하며 다이제스트로 고정한 이전 아카이브에서만 서명된 AzureRM backend 예제를 활성화해 검증된 호스트에 전달합니다. 검증 재개 시 서명된 빈 자리표시자를 포함한 모든 불변 실행 파일을 새로 인증한 번들과 대조합니다. 정확한 경로의 비어 있지 않은 비공개 상태 및 백업만 복구 자료로 허용하며 추가 소스나 적용 권한으로 취급하지 않습니다.

별도 출력이 있는 독립 작업만 병렬 실행하며 작업자는 이식 가능한 상태를 쓰지 않습니다.
상위 프로세스는 결과를 모아 결정적인 순서로 전이 하나를 기록하며, 모든 작업을 기다리거나
종료한 뒤 실패를 기록합니다. 공급자 등록은 보존되는 변경이고 정책 프로브 정리는 소유권을
확인해 직렬로 수행합니다. 병렬 분기는 승인, 성공 선언, Terraform 상태 진행, 후속 단계의
권한 부여를 할 수 없습니다.

매니페스트에서 컴파일한 저장소 설정은 멱등하게 적용하며 누락된 변수·비밀 참조의 이름만
보고하고 값은 표시하지 않습니다. 데이터베이스 자격 증명은 비공개 호스트에서 생성해 승인된
비밀 공급자에 저장하고 참조로 사용하며, 재개 시 묵시적으로 회전하지 않습니다.
...
계획은 차단합니다.
연결된 배포에는 유지되는 빌더, 검증기, Firewall, 공개 IP, 디스크, 이미지 빌드 그래프가
없습니다. 선택적 사전 준비 이미지 경로를 명시한 아티팩트 오프라인 배포만 해당 경로의 유지
리소스를 자체 비용 검토에 포함합니다.

## 네트워크 및 실행 호스트 보증

정확한 대상 구독에서 운영/앱 VNet과 피어링 주소 공간, VPN과 게이트웨이 범위, 허브/경로
테이블 접두사, 로컬 호스트 경로, Private DNS Resolver 범위, 엔드포인트 서브넷을 검사합니다.
관찰할 수 없는 외부 범위는 안전하다고 가정하지 않고 운영자 제약 조건을 요구합니다. 리소스
그룹 범위의 CLI 형식은 제한된 구독 전체 ID를 조회한 뒤 각 정확한 ID를 병렬로 읽습니다.

기본 전달 경로(`default` 또는 경로에 사용된 `0.0.0.0/0`)는 주소 공간을 예약하지 않으므로
경로 검사에서만 제외합니다. 선언된 VNet, 피어, 게이트웨이 범위와 기본 경로가 아닌 접두사는
계속 제약 조건입니다. 근거가 누락되거나 형식이 잘못되면 선택을 차단합니다. 후보가 두 개보다
적으면 사용 가능한 개수와 필요한 개수만 알리고 검토를 요청합니다. 선택 과정은 경로나
방화벽을 수정하지 않으며 연결 가능성을 입증하지 않습니다.

독립 실행형 배포는 복제본이 아니라 검증된 번들을 사용합니다. 정확한 정책·모듈 파일 집합은 변경·누락·추가 바이트를 거부합니다. 수정에는 다시 빌드한 서명 키트가
필요합니다. 추출된 소스를 직접 수정하거나 검증을 끄는 방식은 지원하지 않습니다.

프로필은 VPN을 통한 운영자 경로와 실행기에서 서비스로 가는 경로를 검증합니다. VPN이 배포
권한을 부여하지는 않으며 다음을 증명해야 합니다.

- 실제 호스트에서 DNS, TCP, TLS, 신원, 관리 플레인, GitHub 또는 수동 전송, 아티팩트, 상태,
  Key Vault, PostgreSQL, Event Hubs, 레지스트리, 모델, 모니터링 경로
- 대상 클라우드의 유효 경로, 방화벽, NSG, 서비스 태그, 프록시 신뢰, 비공개 DNS
- 검증된 실행에서 확인되지 않은 `latest` 다운로드를 사용하지 않는 체크섬 고정 도구와 이미지
- 인증된 번들 소스는 고정된 신뢰할 수 있는 인터프리터로만 실행하며 실행 비트나 PATH에 의존하지 않음
...
소버린 클라우드의 접미사, 대상, 서비스 태그, 서비스가 일치해야 하며 공용 대체 경로는 없습니다.
