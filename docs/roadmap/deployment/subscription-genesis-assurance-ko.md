...
새 이미지 계획은 선택한 지역에서 서명된 소형 VM 정책에 따라 호환되는 빌더와 검증기 SKU를
고릅니다. 두 VM은 x64, Gen2, 검토된 메모리와 64-GiB 관리형 OS 디스크 요건을 충족해야 합니다.
범위가 제한된 SKU 및 합산 할당량 스냅샷을 비공개로 보존해 선택을 재현하고, 다이제스트와
크기를 봉인해 승인 전에 표시합니다. 적용은 저장된 크기와 할당량, 만료, 승인만 재확인합니다.
불확실한 근거는 차단하며 재선택, 지역 변경, 용량 예약은 하지 않습니다. 기존 점유는 효과
검증만 재개합니다. 기반 호스트 선택과 복구는 별도이며 비용 검토는 실시간 가격 보장이 아닙니다.
