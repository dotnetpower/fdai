...
잘린 속성은 완전한 세대를 증명할 수 없습니다. 스냅샷 준비 단계에는 최종 검증된 관계 집합만
기록합니다. 거부된 후보는 관계 커버리지를 불완전하게 유지하되 검증된 객체의 갱신까지 막지
않습니다. 그래프 커밋과 Resource 이벤트 전달은 별도의 영속 완료 표식을 사용하므로 초기
준비 상태에서 한쪽 완료를 다른 쪽 완료로 대신할 수 없습니다.
...
현재 조정기는 객체를 PostgreSQL에 준비하면서 관계 검증을 위해 한도가 있는 세대 하나를
