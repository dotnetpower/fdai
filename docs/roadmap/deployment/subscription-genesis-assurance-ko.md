...
translation_source_sha: 933b9394eb2391c0d17a352ad15d14e2a282cb81
...
지역 가용성은 정확한 구독에 결합된 ARM 위치 엔드포인트로 확인합니다. 활성 Azure CLI 구독에
의존하거나 편의 명령에 지원되지 않는 선택 플래그를 전달하지 않습니다.
...
실행기 이미지 생성은 실제 Storage 정책과도 호환돼야 합니다. 구현된 비공개 경로는 숨겨진
스테이징 계정에서 Shared Key를 요구할 수 있는 Azure VM Image Builder를 사용하지 않습니다.
대신 Terraform이 전체 빌더 네트워크, FQDN 허용 목록이 있는 Firewall Basic과 VM에 연결되지
않은 공개 egress IP 두 개, 비공개 빌더 및 검증기 VM, 확장, 할당 해제 및 일반화 작업,
관리형 이미지를 소유합니다. 정확한 계획은 Storage 및 이미지 템플릿 리소스를
...
기본 전달 경로(로컬 호스트의 `default`, Azure 경로 테이블이나 로컬 경로 테이블의
`0.0.0.0/0`)는 주소 공간을 예약하지 않으므로 경로 기반 중복 검사에서만 제외합니다. VNet과
피어링 주소 공간, 로컬 네트워크 게이트웨이 범위, 기본 경로가 아닌 모든 접두사는 계속 충돌
근거로 사용합니다. 근거가 누락되거나 형식이 잘못되면 선택을 차단합니다. 검토된 후보가
두 개보다 적으면 배포 접두사를 노출하지 않고 사용 가능한 개수와 필요한 개수를 알려 주며
네트워크 검토를 요청합니다. 이 선택은 경로를 수정하거나 방화벽 정책을 제거하지 않으며
네트워크 연결 가능성을 입증하지도 않습니다.

독립 실행형 배포는 복제한 저장소가 아니라 검증된 릴리스 번들에서 이 선택기를 가져옵니다.
공개 명령이 소스 수정본을 사용하려면 키트를 다시 빌드하고 서명해야 합니다. 추출된 키트를
직접 수정하거나 검증을 끄는 복구 경로는 지원하지 않습니다.
...
- 모드가 제한된 인증 번들 소스를 고정된 신뢰할 수 있는 인터프리터로만 시작하고 실행
  비트를 광범위하게 복원하거나 주변 경로에서 인터프리터를 선택하지 않음
