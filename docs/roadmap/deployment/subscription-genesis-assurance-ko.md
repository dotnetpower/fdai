...
translation_source_sha: 57f740480a3cdc0a094d4ea5ed10e75dc5ace38a
...
Resource 준비 단계는 한도가 있는 불변 청크와 맥락에 결속된 재개 지점을 원자적으로 보존합니다. 실제 조합 경로는 유효 정책, 관리 대상과 인증 대상, 요청 속도, vocabulary 매핑 및 서명된 대체 입력도 맥락에 고정합니다.
ARG는 생성된 쿼리, 검토된 관계와 전송 설정도 고정하며, ARM 보강 수집을 포함한 설치된 FDAI 생산자 소스의 내용도 고정합니다. 수집과 보강이 끝난 후보는 봉인하며, 동일한 맥락에서 원래 관측 시각과 전체 근거를 보존하고 활성 기준 세대를 재검증한 경우에만 공급자 조회 없이 재개합니다.
Resource 전용 재생은 완료를 증명하지 않습니다. 미완료 공급자 조회는 연속 조회의 유효성, 관계 근거 및 최종 범위 커버리지를 별도로 검증해야 합니다. 봉인 후보 복구는 분할 ontology 게시나 초기 구성 준비 완료 증적이 아닙니다. 취소나 전체 제한 시간 만료 뒤에도 원본 종료 정리에 별도 한도를 적용합니다. 최대 5초와 설정된 진행 제한 시간 중 짧은 값을 사용하며, 종료 실패가 취소를 가리거나 최종 fence 뒤의 승격을 허용해서는 안 됩니다.
