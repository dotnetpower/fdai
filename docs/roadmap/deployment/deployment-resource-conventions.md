...
| Protected model resolution and endpoint artifacts | implemented | `.github/workflows/deploy-dev.yml`; model binding sealer; `llm_model_endpoints` Terraform output; protected service materializer; focused checks | Protected plans seal deterministic bindings and model digests. Terraform exports exact account-qualified origins, and independent Core materialization rejects missing or mismatched Foundry coverage. |
