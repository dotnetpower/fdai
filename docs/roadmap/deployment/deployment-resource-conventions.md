...
| Private Foundry partner module | implemented | `infra/modules/llm/foundry-partner/`; isolated Terraform test (`1 passed`) | The inactive reusable module follows `aif-`/`proj-` naming, disables public access for private deployments, pins partner format/version/capacity, and grants only the project user role. |
