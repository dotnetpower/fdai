...
| Event Bus consumer lag alert | implemented | `event_bus.py`; `infra/modules/observability/monitoring/`; `.github/workflows/deploy-dev.yml`; focused consumer, infrastructure, and workflow checks; strict mypy and `terraform validate` | Bounded commits export sanitized partition progress and lag. A broker-backed heartbeat also reports assigned partitions while downstream processing is stalled, so zero commit progress cannot make the consumer look idle. The explicit `deploy_monitoring` protected input makes the opt-in alert deployable in every environment; protected apply evidence remains open. |
