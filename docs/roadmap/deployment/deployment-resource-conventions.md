...
|------|-------|----------|-------|
| 2026-08-18 | implemented | Bound the core startup probe at the module call instead of the variable default. The deployment supplies a complete `health` object through protected tfvars, so `health = var.health` discarded the default that carried `startup_path` and the larger `startup_failure_count`; the deployed revision showed only Liveness and Readiness probes and `startup_failure_count = 30`. The call now merges the startup fields over whatever the caller supplies. | `current change`; `terraform fmt` and `terraform validate` pass on the service root; independent-service and drift contracts `34 passed`; service Terraform root suite passed. Measured cause: `az containerapp revision show` reported no Startup probe on the revision created by apply run `32107930967`. | Confirm a new revision reaches `Healthy`, then identify which startup phase consumes its deadline; tracked on #181. |
| 2026-08-18 | implemented | Sized the core startup probe budget to the runtime's real boot bound. Startup readiness runs four phases, each bounded separately by `phase_timeout_seconds` (75 s), so a boot that exhausts every phase needs about 300 s before the health port opens. The default 30 attempts at a 5 s interval gave only 150 s, so the first startup probe still could not cover a slow boot. | `current change`; `terraform fmt` and `terraform validate` pass on the service root; independent-service and drift contracts `34 passed`; service Terraform root suite passed. Measured cause: the failing replica's complete log was three application lines ending at `notification_route_unavailable`, with no `startup_readiness_evaluated`, so the boot was still inside phase evaluation when the platform killed it. | Confirm a new revision reaches `Healthy`, then identify which phase consumes its deadline; tracked on #181. |
