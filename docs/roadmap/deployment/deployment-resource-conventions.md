...
sealed mode accepts only the service-specific reviewed topic environment values, immutable image,
and fresh revision suffix. It skips schema migration for this configuration-only transition while
preserving rollback capture, health verification, peer-state isolation, and live observations.
