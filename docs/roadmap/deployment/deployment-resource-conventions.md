...
| Protected model resolution and endpoint artifacts | implemented | `.github/workflows/deploy-dev.yml`; model binding sealer; exact capability gate; `llm_model_endpoints` Terraform output; protected service materializer; focused checks | Protected plans seal deterministic bindings and model digests. ChatOps validation accepts only the reviewed Mistral profile with its exact deployment-owned Foundry reference before Terraform. |
