...
| Protected model resolution artifacts | implemented | `.github/workflows/deploy-dev.yml`; model binding sealer; focused lifecycle and workflow checks | Protected plans add deterministic partner endpoint bindings before sealing full/deployment manifests and SHA-256 digests. Exact apply restores those bytes; runtime receives the same inline JSON and digest. |
| Private Foundry partner module | implemented | `infra/modules/llm/foundry-partner/`; root and module Terraform checks | The reusable module follows `aif-`/`proj-` naming, disables public access for private deployments, pins partner format/version/capacity, grants project-user roles, and is conditionally composed for partner capabilities. |
