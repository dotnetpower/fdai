...
| Scenario-lab AKS Store Demo workload | implemented | `render_aks_store_demo.py`; `verify_store_front_domain.py`; `prepare-runner.sh`; `infra/scenario-lab/outputs.tf`; focused scenario-lab tests | An approved apply downloads one commit- and SHA-256-pinned upstream manifest, pins every workload image by digest, publishes only `store-front` through a deterministic Azure domain, keeps `store-admin` private, and uses three `order-service` replicas as the existing fault target. A protected apply and domain-readiness receipt remain open. |
