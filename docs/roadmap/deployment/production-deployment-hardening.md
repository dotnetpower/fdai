...
| 2026-09-05 | implemented | Separated cluster-wide legacy migration downgrade tests from adopted service-schema tests, serialized the schema-mutating catalog lifecycle regression, and restored the root-owned T2 lookup index through a forward Core migration. Local composite actions remain repository-owned while remote actions stay SHA-pinned. Two non-expiring signing secrets carry resource-local Checkov exceptions because uncoordinated expiry would invalidate in-flight receipts or retained evidence. | `current change`; focused migration, workflow, action-pin, and Terraform security checks. | Retain one exact green required CI and supply-chain receipt before deployment. |
| 2026-09-05 | implemented | Aligned the dangling-relationship regression with the restored pre-journal endpoint validation, which rejects the missing endpoint before lifecycle binding. | `current change`; the PostgreSQL integration assertion now matches the deterministic fail-fast boundary. | Retain the exact green required CI receipt. |
