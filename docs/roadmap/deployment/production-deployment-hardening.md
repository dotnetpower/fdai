...
| Bounded Cost Governance package deployment | implemented | `deploy-dev.yml`; `enforce_plan_scope.py`; `verify_deploy_convergence.sh`; focused scope, convergence, receipt, and workflow tests | A request-bound `plan-cost-*` or `apply-cost-*` mode targets only the Cost Management Reader assignment and the collector and analyzer Jobs. An independent changed-address verifier rejects every other resource before destructive-plan review or artifact retention. Post-apply verification replans the same target set, independently reads both Job images, and binds the sanitized readback digest into the apply receipt. Cost-only apply does not run Core-owned migration, health, legacy inventory, or canary checks. |
...
| Scenario-lab subscription-compatible VM sizes | implemented | `sre-demo-lab.yml`; scenario-lab Terraform variables; focused scenario-lab contract tests | Protected repository variables may override the AKS node and stress VM sizes when a subscription restricts the defaults. Missing overrides preserve `Standard_D2s_v5` and `Standard_B2s`; the exact Terraform plan remains the approval boundary. |
| Scenario-lab AKS Store Demo workload | implemented | `render_aks_store_demo.py`; `prepare-runner.sh`; `infra/scenario-lab/outputs.tf`; focused scenario-lab tests | An approved apply downloads one commit- and SHA-256-pinned upstream manifest, pins every workload image by digest, keeps both front ends private, and uses three `order-service` replicas as the existing fault target. A protected apply and workload-readiness receipt remain open. |
...
| 2026-09-12 | implemented | Isolated Cost Governance platform plans after a protected plan exposed unrelated root-module drift. The workflow now targets exactly the reader assignment and two package Jobs, while a separate allowlist rejects every other changed address. | `current change`; `.github/workflows/deploy-dev.yml`; `scripts/deployment/azure/enforce_plan_scope.py`; 146 focused deployment, request, and dispatch tests; failed protected plan `34683751394` produced no apply or retained plan artifact. | Retain one zero-destroy protected Cost Governance plan and exact apply, then collect the separate W7 lifecycle and campaign evidence. |
...
- [ ] Retain an exact protected scenario-lab apply that shows all AKS Store Demo workloads ready,
    all running images bound to the reviewed digests, and both front-end services private before
    any separately approved fault sweep.
