...
| Bounded Cost Governance package deployment | implemented | `deploy-dev.yml`; `enforce_plan_scope.py`; `verify_deploy_convergence.sh`; focused scope, convergence, and workflow tests | A request-bound `plan-cost-*` or `apply-cost-*` mode targets only the Cost Management Reader assignment and the collector and analyzer Jobs. An independent changed-address verifier rejects every other resource before destructive-plan review or artifact retention. Post-apply verification replans the same target set and independently reads both Job images. Cost-only apply retains Core revision health and canary checks without starting the unrelated legacy inventory Job. |
