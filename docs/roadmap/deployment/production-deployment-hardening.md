...
|------|-------|--------|----------|-----------|
| 2026-09-11 | implemented | Bound status-overriding artifact actions in platform, service, and scenario workflows to the successful protected-source verifier outcome. A failed or skipped verifier can no longer fall through to an `always()` action with the job's permissions. | `current change`; `.github/workflows/{deploy-dev,service-deploy,sre-demo-lab}.yml`; focused CI security contract tests; `check-ci-contracts.py` | Retain governed protected runs that exercise artifact publication after verifier success and prove verifier failure publishes nothing. |
