...
| Bounded Cost Governance package deployment | implemented | `deploy-dev.yml`; `enforce_plan_scope.py`; focused scope and workflow tests | A request-bound `plan-cost-*` or `apply-cost-*` mode targets only the Cost Management Reader assignment and the collector and analyzer Jobs. An independent changed-address verifier rejects every other resource before destructive-plan review or artifact retention. |
