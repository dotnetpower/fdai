...
|------|-------|--------|----------|-----------|
| 2026-09-05 | implemented | Restored the in-workflow GitHub Environment policy check after deployment-step consolidation so a direct workflow dispatch cannot bypass the CLI approval fence. | `current change`; focused deployment workflow, Environment policy, migration, and deployment CLI checks passed 153 cases. | Retain one independently approved exact apply receipt with admin bypass and self-review disabled. |
| 2026-09-05 | implemented | Corrected the preceding transition by restoring the apply job's GitHub Environment binding and folding policy revalidation into the consolidated request-validation step. The original separate check did not bind the job to the Environment and exceeded the current step budget. | `current change`; focused deployment workflow, Environment policy, and deployment CLI checks passed 86 cases. | Retain one independently approved exact apply receipt with admin bypass and self-review disabled. |
