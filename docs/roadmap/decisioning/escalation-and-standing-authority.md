...
| Escalation ladder and urgency catalogs | in-progress | [`escalation_ladder.py`](../../../services/core-control-plane/src/fdai/rule_catalog/schema/escalation_ladder.py); [`forecast_urgency.py`](../../../services/core-control-plane/src/fdai/core/hil_resume/forecast_urgency.py); focused catalog, source, SQL, and approval tests | The supervisor loads reviewed catalogs and verifies the exact retained forecast before compressing response windows. Source, confidence, expiry, cancellation, and replay checks pass; measured urgency-compression cohort evidence remains open. |
| A3-E standing human authorization | in-progress | [`standing-authorization.json`](../../../services/core-control-plane/src/fdai/shared/contracts/authority/standing-authorization.json), [`lifecycle.py`](../../../services/core-control-plane/src/fdai/core/standing_authority/lifecycle.py), [`fence.py`](../../../services/core-control-plane/src/fdai/core/standing_authority/fence.py), [`lease.py`](../../../services/core-control-plane/src/fdai/core/standing_authority/lease.py), [`provider_eligibility.py`](../../../services/core-control-plane/src/fdai/core/standing_authority/provider_eligibility.py), [`promotion_candidate.py`](../../../services/core-control-plane/src/fdai/core/standing_authority/promotion_candidate.py), [`shadow_cohort_runner.py`](../../../services/core-control-plane/src/fdai/core/standing_authority/shadow_cohort_runner.py), [`shadow_cohort_cli.py`](../../../services/core-control-plane/src/fdai/core/standing_authority/shadow_cohort_cli.py), [`shadow_reversion_command.py`](../../../services/core-control-plane/src/fdai/core/standing_authority/shadow_reversion_command.py), [`postgres_standing_authority.py`](../../../services/core-control-plane/src/fdai/delivery/persistence/postgres_standing_authority.py), focused standing-authority and persistence tests | The typed evaluator, immutable revision and proof binding, hash-chained lifecycle, atomic PostgreSQL store, rebuildable snapshot, exact read-time fence contract, inert effect-spanning lease, derived provider-commit-fence eligibility, inert promotion-candidate lifecycle, and deterministic local shadow cohort runner exist. Zero provider adapters are registered as commit-fence capable, so every shipped ActionType derives `INELIGIBLE_CAPABILITY` and no candidate can reach approval. The runner produces a canonical content-addressed receipt bound to source revision, manifest/corpus/configuration digests, candidate/lease contract versions, ordered case outcomes, denial reasons, counts, zero policy escapes, timeout configuration (total 1800s, no-progress 120s, per-case 30s), and registry before/after digests. Receipt is `venue=local`, `evidence_class=synthetic_development`, all authority flags False; it cannot satisfy runtime promotion. A strictly local-only CLI now drives that runner from a closed, bounded corpus schema and refuses any document or receipt that is not `venue=local` and `evidence_class=synthetic_development`. Its schema has no key for declaring provider eligibility, so with zero registered adapters a corpus of real ActionTypes derives `INELIGIBLE_CAPABILITY` for every case and cannot produce a complete cohort. An inert `ShadowReversionCommand` contract, writer Protocol, and fail-closed orchestration seam exist so a later authorized adapter can consume `plan_effect_shadow_reversion`; no adapter implements the Protocol and the mutation seam is not exported from the package namespace. FDAI-CONST-008 remains non-implemented; #632 live/governed promotion remains separately authorized. None of these are wired by design. |
| A3-E shadow-reversion persistence | implemented | `state_store_shadow_reversion.py`; focused command and persistence tests | The unbound adapter accepts one ActionType per command, requires an injected current-human-approval verifier, records intent before mutation, rechecks the lifecycle fence, persists only a shadow reduction, recovers an applying command after restart, and records terminal audit. It performs no provider effect and grants no promotion or execution authority. |

> **Current correction:** The implemented shadow-reversion persistence row supersedes the aggregate
> A3-E row's earlier no-adapter sentence. Runtime binding and authorization remain absent.
...
| 2026-09-13 | in-progress | Added the two remaining code-only A3-E prerequisites. `shadow_reversion_command.py` defines a content-addressed `ShadowReversionCommand` that binds the exact source revision, reverted ActionTypes, failed or unknown observation evidence, reason code, stop/rollback/kill-switch bindings, expected current standing-authorization fence and state, distinct human proposer and reviewer, a stable idempotency key, and two-phase intent and terminal audit records, plus a `ShadowReversionWriter` Protocol and a fail-closed `apply_shadow_reversion` orchestration seam. Matched or pending evidence, a `NONE` transition, self-review, and agent or executor identities are all rejected. `shadow_cohort_cli.py` and `shadow_cohort_corpus.py` add a strictly local-only harness over the existing `run_cohort` with a closed corpus schema, bounded document size, case, review, and elapsed limits, a monotonic CLI deadline, symlink-rejecting output containment, and a receipt venue and evidence-class assertion before any write. Neither package is wired: no adapter implements the writer Protocol, the writer and orchestrator are deliberately absent from the package namespace, and static tests prove no agent, risk gate, executor, HIL resume, workflow, control loop, composition, runtime, or delivery path imports either module. | `current change`; [`shadow_reversion_command.py`](../../../services/core-control-plane/src/fdai/core/standing_authority/shadow_reversion_command.py), [`shadow_cohort_cli.py`](../../../services/core-control-plane/src/fdai/core/standing_authority/shadow_cohort_cli.py), [`shadow_cohort_corpus.py`](../../../services/core-control-plane/src/fdai/core/standing_authority/shadow_cohort_corpus.py); `tests/core/standing_authority` passed 323 cases (99 new); Ruff, format, and strict mypy passed; the file-LOC gate reported 0 failures; core-import, boundary-docstring, subsystem-fanout, and fork-independence gates passed; an independent critique found that a nested record error exited 1 instead of the documented usage code and that the inertness guards scanned an incomplete hardcoded root set, and both were fixed with a whole-tree import scanner plus regression cases. | These are contracts and a development harness, not authority. No writer adapter exists, the CLI emits only synthetic local evidence, and the durable provider boundary, governed runtime cohort, independent review, explicit current human approval, and retained independent effect observations tracked by #632 all remain open. |
| 2026-09-13 | in-progress | Removed the caller-declared provider-eligibility claim surface. `provider_eligibility.py` adds `ProviderFenceCapability`, the empty `A3E_COMMIT_FENCE_ADAPTERS` registry, `a3e_fence_capability`, `partition_provider_eligibility`, and `derive_ineligible_provider_action_types`. `build_candidate_record` no longer accepts `ineligible_provider_action_types`, and `PromotionCandidateRecord.__post_init__` rejects any hand-declared partition, so neither the builder nor the direct dataclass path can assert eligibility that no adapter enforces. Because the adapter registry is empty, every shipped ActionType classifies as `INELIGIBLE_CAPABILITY` and every real candidate denies with `PROVIDER_INELIGIBLE` at creation. Lifecycle state-machine coverage uses an explicit test-only hypothetical registry confined to `tests/`. The module is pure and unwired. | `current change`; [`provider_eligibility.py`](../../../services/core-control-plane/src/fdai/core/standing_authority/provider_eligibility.py), [`test_provider_eligibility.py`](../../../services/core-control-plane/tests/core/standing_authority/test_provider_eligibility.py); `tests/core/standing_authority` passed 242 cases (18 new), including a catalog-wide check over all 49 shipped ActionTypes; Ruff, format, and strict mypy passed; the file-LOC gate reported 0 failures; the core-import gate passed; an independent critique found one false-negative in the new import guard (a package re-export could be imported without being flagged) and one pre-existing vacuous scan root in `test_lifecycle.py`, both fixed and reverified. | This constrains the eligibility claim; it does not create an eligible provider. A durable provider boundary that atomically validates lease and fencing generation at effect commit, the governed runtime cohort, independent review, and explicit current human approval (#632) all remain open. |
...
- [x] Bind the supervisor to reviewed catalogs and an exact current forecast source; focused
  source, SQL, producer, and approval replay tests pass.
- [ ] Retain measured urgency-compression evidence from a governed running cohort.
...
- [x] Derive provider-commit-fence eligibility instead of accepting it from a candidate author, and
  prove that every shipped ActionType is `INELIGIBLE_CAPABILITY` while zero adapters are registered.
- [ ] Implement a durable provider boundary that atomically validates the current lease and fencing
  generation at effect commit, so at least one ActionType can become A3-E eligible.
...
- [x] Define the inert `ShadowReversionCommand`, writer Protocol, and fail-closed orchestration seam
  that a later authorized adapter can use to consume `plan_effect_shadow_reversion`.
- [x] Provide a strictly local-only CLI over the shadow cohort runner that refuses any non-local venue
  or evidence class and writes only inside an explicit, symlink-checked output directory.
- [x] Implement an unbound durable `ShadowReversionWriter` adapter with one-ActionType atomic scope,
  current-approval verification, fence recheck, idempotent restart recovery, and two-phase audit.
- [ ] Bind an independently authenticated approval verifier and separately authorize one exact
  invocation before a failed effect verification can change the production registry.
