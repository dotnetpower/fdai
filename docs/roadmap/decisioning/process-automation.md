...
independent effect evidence. After all receipts verify, the coordinator hands the held target to the durable recovery path; only an admitted
release can clear the hold, and the recovery path owns the terminal `status=compensated` transition. A hold that is still in force after the
recovery attempt closes the Process as `recovery_incomplete`, including when the hold belongs to another Process. Released holds can be
reissued for a later Process, and an older Process cannot release the newer hold.
