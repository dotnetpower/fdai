...
must independently validate each action and compensation receipt before a forward step completes or the Process becomes `compensated`. Missing, rejected, or malformed evidence remains waiting or closes as `recovery_incomplete`; it never becomes success. Verification keeps `not_attempted` distinct from `failed`, leaves pending evidence non-terminal, and preserves the same action identifier, event identity, and positive workflow attempt through retry, compensation, HIL resume, and terminal evidence; pre-executor no-effect exits pass through this recorder without provider invocation, and the step executor maps results without owning verification authority.
...
The recovery-admission boundary separately binds the complete `WorkflowApprovalSnapshot` quorum, exact hold revision, approval step and attempt, target digest, compensation receipt digests, distinct requester, approver, and executor identities, and source revision to one current `DecisionEvidenceAdmission`. It returns typed eligibility only and cannot release a hold or grant authority. The hold ledger exposes an atomic release operation that guards the current durable approval policy and admission window, consumes that admission, increments the fencing generation, and stores a content-addressed no-authority release receipt with terminal audit. The production compensation coordinator now consumes that guarded operation exclusively; the legacy verified-recovery release is removed from the hold ledger, so no path can close a hold without a current admission (#630). The production safeguard lifecycle coordinator performs the final dispatch recheck inside the executor's own logical-target lock, immediately before provider invocation, for every Core PR, direct-API, tool-call, and safeguard-bound isolated path (#640). Its expected lineage comes from the durable dispatch authorization bound to that exact Process step, never from whatever hold record is current at dispatch time. An approved recovery step receives a hold-scoped authorization naming the one active hold revision it may act under, and the guarded release replaces it with the released lineage that later steps must match. A hold reissued, released, or re-released after an authorization therefore denies dispatch instead of matching it, and an authorization that exists but cannot be reconstructed denies rather than reading as no authorization. The RiskGate reads the same proof. Its hold exception is granted only when durable state shows a hold-scoped dispatch authorization for that exact target, Process, canonical `recover_<attempt>` step, and the hold revision still active; a step name is never authority, so a generic `recover_` or `compensate_` string, a cross-Process lineage, and a superseded hold revision all stay denied, and an unreadable authorization fails closed to deny instead of reading as no hold.
...
The independent observation ingress accepts the Heimdall-owned typed event only when its external observer identity matches the composition-supplied trusted set; topic ownership alone never authenticates the external authority.

`WorkflowRecoveryCoordinator` is the production composition of those parts. It derives one distinct attempt identity from the failed compensation proposal digest, hold revision, recovery action type, recovery payload digest, target digest, source revision, and attempt number, and rejects an attempt whose payload reuses the failed proposal. It requires a separate immutable human approval bound to that exact attempt, and no provider dispatch happens until the complete recovery admission proves current approval, quorum, absence of a rejection, a live expiry, no self-approval, executor separation, and a current decision-evidence admission. When no approval exists, the path asks for one through the existing approval journal and stays rejected; asking is never granting. It then takes one exclusive compare-and-set in-flight claim under the target fence and re-reads the hold revision after claiming, so a hold reissued between the claim and dispatch is stale and a concurrent recovery invokes the provider exactly once while the losing caller stays in doubt without reissuing a hold. The executor finalizes the safeguard bundle inside its own logical-target lock, so the workflow binds the finalized bundle that the production dispatch retained. Retention requires an execution that named its own provider invocation and returned a lifecycle-bound receipt, so a denied, duplicate, or unknown execution never supplies that bundle, and without it the attempt never claims an effect and the hold stays in force. A dispatch whose provider outcome is unknown stays in doubt, keeps the hold in force, and is resolved only by later provider reconciliation, never by a fresh dispatch. An independent authoritative observation outside executor and provider identity produces a content-addressed completion claim; a new generation supersedes the prior claim atomically. Only the exact current successful claim is consumed by the approval-guarded hold release, whose receipt is recorded in an immutable pre-release-keyed lookup. Terminalization stages a completion outbox for the Process event and the Saga audit, commits the Process transition under revision compare-and-set with the exact completion digest, and drains the outbox. A crash between hold release, Process compare-and-set, and Saga delivery is healed from the durable lookup and the committed completion digest without a second dispatch or a second release. Healing validates that the persisted hold still proves the same release receipt, released revision, fencing generation, Process, and consumed admission, and terminalizes at the instant that release consumed the claim, so a late repair still closes while a mismatched, reissued, or revoked lineage fails closed. The production compensation resume caller runs that healing before its own hold check, because a released hold is invisible to the held-recovery branch: a Process whose recovery released the hold but never committed its transition is repaired on the next resume after an arbitrary delay, and the Saga delivery is drained with it, instead of the Process closing outside the recovery completion it was released under. A missing observer or observation-intake binding is reported as a named `observer_unavailable` readiness outcome rather than a silent unverified effect, so an unfinished deployment is visible instead of looking like missing evidence.

`recovery_attempt.py` provides attempt identity, exclusive pre-dispatch claim, separate approval and safeguard evidence, stable idempotency, and content-address verification. `recovery_effect_claim.py` provides authoritative external verification, identity separation, monotonic claims, and supersession. `recovery_terminalization.py` provides immutable receipt lookup, independent Process/Saga outbox delivery, exact terminal guards, and reconstructed-entry verification.
`recovery_effect_ingress.py` is the versioned typed production ingress that independent post-effect observations arrive through. It belongs to the observer path: Huginn normalizes the raw external signal onto `object.event`, Heimdall proves Huginn produced it and relays the bounded declared fields onto the `object.recovery-effect-observation` topic Heimdall owns, and a dedicated `recovery-effect-observer` consumer group reads only that topic. Heimdall is therefore the sole authorized producing principal, and the sole privileged executor owns no topic on that path, so no producer reaches the intake by publishing a fully-formed payload onto the shared ingress topic. The ingress proves the supported `observation_schema_version`, an authorized producing principal that the bus authenticated on publish and that is never the sole privileged executor, an `authoritative_external` authority class, an observer identity distinct from both executor and provider, the attempt binding resolved from persisted Process and recovery-step lineage, the target digest, the provider receipt, the effect and envelope digests, a positive and non-future evidence window inside its freshness policy, finality on every watermark, and explicit containment fields before `StateStoreRecoveryEffectObservationJournal.record_independent_observation` persists it. The first authoritative record for an attempt and provider receipt wins, an exact replay is a duplicate, and a later disagreeing event is a conflict, so duplicate and reordered delivery are safe. Persisting is still not verifying: the recovery path re-reads the durable observation and re-applies identity separation, finality, and admission before any completion claim.
`hold_dispatch_fence.py` fences forward dispatch against active, malformed, or unreadable hold state inside the logical-target lock, with an isolated-Executor equivalent that performs the same check without importing Core implementation.

The upstream headless runtime and production Operator API bind `StateStoreWorkflowOutcomeLedger` to the shared durable state store. Only an enforce Action whose identity matches its `ResponseOutcome` can record a receipt, success also requires independent effect evidence, and resolution uses exact proposal, Process, and step lineage rather than caller status. Shadow, unknown, missing, mismatched, or unscorable outcomes cannot advance the Process.

`StateStoreAutomationHoldLedger` writes a target-digested hold before a recovery-incomplete Process closes. Ordinary mutations deny on an active, failed, or malformed hold read; only exact authorized recovery lineage re-enters the safety pipeline at human approval, and every compensation still requires independent effect evidence. Only an admitted release clears the hold and permits the recovery-owned terminal `compensated` transition; remaining or newer holds close as `recovery_incomplete` and cannot be released by an older Process.
