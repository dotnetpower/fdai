...
보기에는 두 가지 모드가 있습니다. 기본 모드는 **읽기 전용 게시 정의 작업 영역**입니다.
권위 있는 출처와 리비전 다음에 선택한 워크플로 사실 4개, 소유권 및 필터 컨트롤,
카탈로그 레일, 순서가 지정된 계약 경로, 안전 검사기, 원본 카탈로그 YAML 펼침 영역을
배치합니다. 선택적인 사용자 정의 및 Python 작업 기능을 사용할 수 없어도 기본 제공
카탈로그는 숨기지 않습니다. 단일 **"새 워크플로 설계"** 진입점에서 대화형 디자이너를 엽니다.
