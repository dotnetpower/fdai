...
translation_source_sha: f18c31e75a94b7b8aae97fe5e30099aa71cf6a57
...
보상 증적을 독립적으로 검증해야 forward 단계가 완료되거나 프로세스가 `compensated`가 됩니다. 근거가 없거나 거부되거나 malformed이면 waiting 상태를 유지하거나 `recovery_incomplete`로 끝나며 성공이 되지 않습니다. 검증은 `not_attempted`와 `failed`를 구분하고 대기 중인 근거를 최종 상태로 만들지 않으며, 동일한 액션 식별자와 양의 작업 흐름 시도 번호를 재시도, 보상, HIL 재개, 최종 근거까지 보존합니다.
...
실행기 선택 또는 대상 잠금 획득 전에 각 Workflow action 시도는 전체 Action digest, 선택한 실행 경로, 실행 fingerprint, 소스 revision을 결속한 변경 불가 pre-bundle commitment 하나를 기록합니다. 선택한 실행기는 이 commitment와 일치해야 하며 action, retry, compensation, 최종 Process 근거까지 최종 묶음 digest를 전달합니다.
복구 승인 경계는 완전한 `WorkflowApprovalSnapshot` 정족수, 정확한 보류 개정, 승인 단계와 시도 번호, 대상 다이제스트, 보상 증적 다이제스트, 서로 다른 요청자, 승인자, 실행기 신원, 소스 개정을 하나의 최신 `DecisionEvidenceAdmission`에 별도로 결속합니다. 이 경계는 타입이 지정된 적격성만 반환하며 보류를 해제하거나 권한을 부여할 수 없습니다. 이슈 #630이 승인의 원자적 사용과 보류 해제를 담당합니다. 이제 보류 원장은 최신 영구 승인 정책과 승인 유효 구간을 같은 트랜잭션에서 확인하고, 승인을 사용하며, fence 세대를 증가시키고, 내용 주소 기반 무권한 해제 증적과 최종 감사를 저장하는 원자적 연산을 제공합니다. 이제 운영 보상 조정기는 이 보호된 연산만 사용하며, 기존의 검증된 복구 해제는 보류 원장에서 제거했습니다. 따라서 어떤 경로도 최신 승인 사용 없이 보류를 해제할 수 없습니다(#630). 운영 안전장치 수명주기 조정기는 Core의 PR, 직접 API, 도구 호출, 안전장치 결속 격리 실행 경로 모두에서 실행기 자신의 논리 대상 잠금 안, 공급자 호출 직전에 최종 디스패치 재검사를 수행합니다(#640). 기대 계보는 디스패치 시점의 보류 기록이 아니라 해당 Process 단계에 결속된 영구 디스패치 권한 부여에서 가져옵니다. 승인된 복구 단계는 자신이 작업할 수 있는 활성 보류 개정 하나를 명시한 보류 범위 권한 부여를 받고, 보호된 해제는 이를 이후 단계가 일치시켜야 할 해제 계보로 교체합니다. 따라서 권한 부여 이후 다시 발행되거나 해제되거나 다시 해제된 보류는 일치로 판정되지 않고 디스패치를 거부하며, 존재하지만 재구성할 수 없는 권한 부여는 권한 부여 없음으로 읽히지 않고 거부됩니다. RiskGate도 같은 증거를 읽습니다. 보류 예외는 정확한 대상, Process, 표준 `recover_<시도>` 단계, 여전히 활성인 보류 개정에 대한 보류 범위 디스패치 권한 부여가 영구 상태에 있을 때만 부여됩니다. 단계 이름은 권한이 아니므로 일반적인 `recover_` 또는 `compensate_` 문자열, 다른 Process의 계보, 대체된 보류 개정은 모두 거부되며, 읽을 수 없는 권한 부여는 보류 없음으로 읽히지 않고 실패 시 차단되어 거부됩니다.

운영 통합은 별도의 승인 후 디스패치 복구 시도를 담당하는 #652, 권위 있는 사후 효과 완료 주장을 담당하는 #656, 주장 fence가 적용된 보류 해제와 비정상 종료에 안전한 Process 및 Saga 최종 처리를 담당하는 #658로 나뉩니다. 변경할 수 없는 실패 보상 제안을 이후 성공한 복구로 다시 표시하지 않습니다.
독립 관측 ingress는 외부 observer 신원이 composition이 제공한 신뢰 집합과 일치할 때만 Heimdall 소유 typed event를 허용합니다. 토픽 소유권만으로 외부 권위를 인증하지 않습니다.
