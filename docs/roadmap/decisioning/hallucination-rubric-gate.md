...
| 2026-08-17 | in-progress | Corrected a safety-relevant module description and covered the case it described. The `promotion.py` docstring claimed that a policy-violation escape demotes unconditionally, while the implementation holds the current posture when the scenario set is unmeasured or undersized. Reading the docstring as a specification would have invited an implementer to demote on untrustworthy evidence. | `current change`; `promotion.py` docstring, `quality_gate/README.md`, `test_rubric_promotion.py` adds `test_escape_on_an_untrustworthy_set_holds_enforce` and `test_escape_demotes_enforce_on_a_trustworthy_set`; `uv run pytest -q --no-cov services/core-control-plane/tests/core/quality_gate/test_rubric_promotion.py` (`31 passed`). | Unchanged by this correction. |
