...
| Enforce 워크플로 명령 | Owner 및 허용 목록으로 제한됨 | 작업 스텝이 형식화된 `operator_request` 이벤트를 게시합니다. 직접 변경 권한은 아닙니다. |
...
|-------------|-------------|----------------|
| 프로세스 발견 | 우선순위 프로세스 목록, 책임자, 빈도, 실패 비용, anti-scope | Downstream onboarding 아티팩트 및 수동 정제 |
| 정의 및 저작 | 버전 지정 작업 흐름, 매개 변수 스키마, 트리거, 가드, 복구 그래프 | 작업 흐름 카탈로그, 정의 저장소, 빌더 |
| 실행 어댑터 | 승격 허용 목록이 있는 형식화된 도구 또는 direct API 어댑터 | Downstream 조립 루트 및 전달 어댑터 |
...
- 출처 이력과 변경할 수 없는 버전이 있는 downstream 카탈로그 항목.
...
- 둘 이상의 실행기 복제본을 실행하는 배포를 위한 지속성 있는 분산 lock.
- Postcondition 탐색, 중지 조건 monitoring 및 보상 또는 상태 전진 복구.
...
- 탭 범위 초안 복구는 구현되었습니다. Deep 링크, 변경할 수 없는 검토 diff 및 완전한 GitHub
 카탈로그 제안 흐름은 남아 있습니다.
...
- Cell-aware 예약, 분산 lock, backpressure 및 테넌트별 동시성 제한.
...
| 순서 | 변경 | 예상 검증 |
|-----:|------|-----------|
| 1 | Downstream 프로세스 목록 및 고정 시나리오 추가 | 원본 해시, 시크릿 검사, 책임자 검토 |
...
| 어댑터 | 그림자 no-op, enforce 허용 목록, 멱등성, 민감정보 제거, 복구 | 증적, 감사 또는 제한된 대상 집합 없이 변경됨 |
| 시뮬레이션 | 대상 집합, 예상 delta, 오래된 상태, staging과 동등성 | 시뮬레이션과 staging이 서로 다른 리소스에 영향을 줌 |
| Operations | Alert, demotion, 취소, 복원, 자격 증명 교대 | 운영자가 실패한 실행을 중지하거나 재구성할 수 없음 |
...
| 확장으로 동시 변경이 발생함 | 분산 리소스 lock 및 테넌트별 동시성 묶음 |
...
| Downstream 구현 경계 | [Downstream 포크 가이드](../fork-and-sequencing/downstream-fork-guide-ko.md) |
