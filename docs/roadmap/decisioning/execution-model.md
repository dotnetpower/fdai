...
- **Upstream Azure gateway binding** - when the development operations gateway URL and Easy Auth
  ...
  PR, direct-API, tool, remote, workflow, and HIL paths retain original correlation plus supplied Action id and attempt; Trace separates each action attempt, potentially effective HIL outcomes enter independent reconciliation before closure, and pre-executor no-effect exits still write the workflow's durable `not_attempted` result without invoking a provider. For `remediate.tag-add@1.1.0`, the gateway accepts one logical Resource id in an allowlisted dev group, locks it, snapshots and merges one tag, verifies with its reader identity, and restores the snapshot on mismatch. A separate reader call receives the Action and expected effect rather than the executor receipt. The existing 7-day, 50-sample, 99% accuracy, zero-escape gate and governance approval still control promotion.
...
| To learn about | Read |
|----------------|------|
| Delivery status and remaining work | [Implementation ledger](../../roadmap-implementation/decisioning/execution-model.md) |
