...
|------|-------|--------|----------|-----------|
| 2026-09-22 | implemented | Added deployment-local Rule membership activation without changing execution authorization. A selected Rule remains subject to its assignment effect, promotion state, effective-access evidence, risk decision, and separate execution approval before any provider action. | `current change`; activation contracts, control-loop generation replacement, and focused T0 and activation tests. | Retain deployed activation evidence separately; no capability, provider permission, or executor identity is inferred from membership. |
| 2026-09-17 | implemented | Expanded the `AccessGrantRequest` ObjectType from six to eleven review-relevant semantic fields as version `2.0.0`. The declaration now includes capability, grant mode, request time, quorum, and revision while preserving the shipped normalized `id` key and existing role-filtered scope. | `current change`; `AccessGrantRequest.yaml`; `test_ontology_declaration_projection.py`; focused declaration-detail regression passed. | The correction changes declaration metadata only. Grant, approval, application, verification, and revocation authority remain unchanged. |
...
`AccessGrantRequest@2.0.0` keeps the normalized ObjectType key `id` and adds `capability_id`,
`grant_mode`, `requested_at`, `quorum`, and `revision` to the previously declared status, scope,
expiry, decision, and plan facets. The runtime store retains its domain-specific `request_id` and
the additional private lifecycle fields. Declaration visibility follows server-owned role
filtering; the catalog shape itself does not grant access to request values.
