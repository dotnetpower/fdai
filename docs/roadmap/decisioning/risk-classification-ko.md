...
은 모든 후보 액션의 baseline을 `auto`, `hil`, `deny` 중 하나로 분류합니다. 통합 RiskGate는
6-axis ceiling을 적용해 최종 결과를 `shadow`까지 낮출 수 있습니다. 이 문서는 **baseline
...
>
> **구현 상태.** `RiskTable` loader/evaluator, feature extractor, 통합 execution-authority
> resolver, 환경 classifier, control-loop audit builder가 배포됐습니다. 현재 audit payload는
> 매칭 rule id, 최종 결정, quorum, resolved ceiling을 기록합니다. Feature-vector snapshot과
> catalog version 필드는 아직 audit payload에 배선되지 않았습니다.

## 테이블이 사는 곳

- **런타임 경로**: `rule-catalog/risk-classification.yaml` - catalog-as-code, 규칙/할당/예외/
  오버라이드처럼 PR로 리뷰. Repository CODEOWNERS는 GitHub `fdai-owners` team을 지정합니다.
  `aw-approvers` 2인 quorum은 배포의 branch protection/CI가 적용하는 governance 계약입니다
...
- 최종 결정 (`auto` / `hil` / `shadow` / `deny`)과 quorum.
- 최종 결정에 기여한 모든 axis를 담은 `resolved_ceiling`.

결정 시점의 feature-vector snapshot과 `risk-classification.yaml` catalog version은
`ExecutionAuthorityDecision`/`RiskTable`에 존재하지만 아직 감사 payload에 직렬화되지 않습니다.
Replay가 catalog 변경 뒤에도 완전히 자기완결적이 되려면 이 두 필드를 추가해야 합니다.
