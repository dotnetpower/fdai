...
> 어댑터를 통해 배포별로 opt-in합니다. 고객별 오버라이드는 fork 전용이며 코어는
> 기본 비활성 fake를 배포합니다
...
> 4.5 delta-2b, 5 alpha와 Azure Responses provider slice가 랜딩되었습니다 - evolving-system-prompt
...
> slice 3** (fork-first second-approval 채널), Wave 5 **T2 통합**
> (코어 T2 tool manifest로 스니펫 threading)은 문서화되어 있지만 아직
...
**역할**은 어떤 base / pack / tool 집합이 적용될지 결정합니다. Catalog는 reviewer,
Proposer, Critic, Judge, Rubric base prompt를 모두 배포합니다.
...
2-model reviewer가 기본 T2 경로입니다. Proposer / Critic / Judge debate는 설정된
disagreement에서만 router를 통해 실행됩니다.
...
코어는 Protocol 뒤에 남고, Azure 어댑터가 seam당 한 구현을 제공합니다. 이 설계의
현재 및 계획된 seam은 다음과 같습니다:
...
| `ProgrammaticPipelineRunner` | async | bounded pipeline | 검토된 tool loop를 isolated venue에서 실행 |
| `OperatorMemoryStore` | async | 3 | scope-bounded 노트 읽기 / append |
| `WebSearchProvider` | async | 5 | allowlist 뒤 outbound HTTP |
| `EvidenceStore` | async | 5 (계획됨) | hash-addressed 웹 스냅샷 저장 |
| `AgentTranscriptStore` | async | 4.5 (계획됨) | append-only debate 행 |
...
- **Capability와 budget**: `llm-registry`가 짧은 Proposer/Critic allowlist를 선택하고, 각 tool의
  `cost_budget_usd_per_call`이 이벤트별 상한에 반영됩니다.
- **Untrusted output**: `<tool_result trusted="false" ...>`는 verifier와 policy re-check용 data로
  남습니다. Judge는 tool을 받지 않으므로 두 번째 Proposer로 붕괴하지 않습니다.
- **Programmatic loop**: 검토된 read/filter/aggregate Python은 digest, sandbox, run capability,
  byte/call limit, receipt 검사를 거쳐 generated client로 bounded subset을 호출합니다. Provider
  credential, recursive authority, mutation authority는 받지 않습니다.
...
- **Eligibility와 replay:** 매 load는 enabled state, tool availability, agent allowlist, stored bytes, publisher signature, reference digest를 다시 확인합니다. Prompt replay는 operation,
  skill name/version, body/raw digest, reference digest, selected/rejected status, reason을 기록합니다.
- **Progressive prompt:** Index는 선택 body 및 reference보다 먼저 들어갑니다. Body는 verification 후에만 trusted reviewed instruction이며 reference는 untrusted data로 남습니다. 기존 reference-free
  single-file skill은 같은 parser와 effective content를 변경 없이 사용합니다. Explicit multi-skill composition은 [Governed Skill Bundles](governed-skill-bundles-ko.md)가 소유합니다.
- **측정 benchmark:** 고정 16-skill catalog의 network incident, cost spike, deployment failure scenario에서 full projection 8194 estimated token이 complete body 하나 선택 시 1544-1546으로 줄어 81.1-81.2% 감소했습니다.
...
- **Audited proposal workshop:** `SkillWorkshop`은 agent draft를 validate하고 inert
  ...
  digest 및 publisher trust verification을 다시 실행한 뒤 active prompt 변경 없이 approved
  artifact를 disabled 상태로 install합니다.
- **승인된 source refresh:** 등록된 GitHub source는 ETag state로 immutable commit을 resolve하고
  declared file만 가져와 exact byte를 quarantine에 저장합니다. 통과한 content는 disabled
  candidate가 됩니다. Approver installation은 disabled-first를 유지하고 Owner revocation은
  provenance를 삭제하지 않고 source와 durable artifact를 disable합니다.
...
Web search는 최후의 수단 툴입니다. 배포별 opt-in이며 절대 grounding source가
아닙니다.
...
> Working-context selection은 [컨텍스트 선택 정책](context-selection-policy-ko.md)이 별도로 소유합니다:
> 불변 `deterministic-tiered-v1@1.0.0`, 필수 validation, shadow evidence, replay 및 rollback.
...
- `core/`-safe 유지: 모듈은 `fdai.core.operator_memory`와
  ...
  import하므로 `scripts/quality/architecture/check-core-imports.sh`가 계속 통과합니다.
  `delivery.*` import 없음.
...
- `runtime.configuration._finalize_llm_bindings`가 이제 upstream composer를
