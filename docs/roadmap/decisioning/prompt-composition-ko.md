...
translation_source_sha: fc2b4d57bca1e24491d31bd1b6d96fefd7c562a2
...
검토기를 위한 `conversation.adaptive.*` 키 5개는 프롬프트 전용입니다. 계획과 답변은 구성된 첫 번째 T1
서술 모델, 검토와 재검증은 독립된 T1 서술 모델, 선택적 보강만 `t2.reasoner.primary`를 사용하며 모델 배포를
...
줄입니다. 순수 지식 계획은 답변 초안을 포함할 수 있지만 운영 설명은 여전히 운영 조회 뒤에 작성합니다.
독립적으로 지지된 부분만 표시하며 선택적 보강은 독립 재검증에 필요한 시간과 호출 수를 남깁니다.
짧은 검토 단계는 명시적으로 지원되는 낮은 추론 설정을 사용할 수 있고 T2 추론 설정은 유지합니다.

본문 없는 단계 로그에는 경과 시간, 남은 시간, 상태 및 실제 호출 수를 기록합니다. 스키마 캐시는
사용자 입력을 보관하지 않으며 기존 자격 증명 캐시를 재사용합니다. 오프라인 호출 수와 시계
