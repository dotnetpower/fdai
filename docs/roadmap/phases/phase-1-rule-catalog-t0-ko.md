...
| `version` | semver | 변경이 추적·역방향 가능; 룰 집합이 규칙 버전 고정 |
...
Azure WAF / AKS 기준선 / MCSB / Azure Policy / Advisor, CIS Benchmarks, OPA/Gatekeeper
...
수 있음; 벤더 특이는 규칙이 아니라 프로바이더 어댑터 뒤에 유지.
...
엔진은 각 정규화·중복제거된 이벤트(게시 `event-ingest`) 를 평가하고 판정 + 인용 규칙 id 산출.
세 결정론 검사:
...
| 알아볼 내용 | 읽을 문서 |
|-------------|-----------|
| 구현 상태 및 남은 작업 | [구현 원장](../../roadmap-implementation/phases/phase-1-rule-catalog-t0.md) |
