...
translation_source_sha: 6db687f14d39bfb8165fb7d640c430ea7f0fbc0b
...
> **구현 상태**: Authored rule/Rego/remediation seeds, ActionType catalog, T0 engine, OPA
> evaluator, control-loop orchestration, GitOps draft-PR adapter, Azure inventory snapshot/delta
> primitives 및 frozen-scenario replay가 구현되어 있습니다. 이 문서의 "shadow only"는 P1이
> 처음 landing할 때의 phase boundary입니다. 현재 repository에는 이후 phase의 promotion,
> risk/HIL 및 enforce-capable adapter도 존재하므로 전체 runtime의 현재 mode를 뜻하지 않습니다.
> Production inventory와 GitOps delivery는 배포별 provider/credential binding이 필요합니다.
...
- **픽스처** 는 정규화 규칙 스키마와 `event-ingest` 이벤트 스키마 따름; 리포 범위 규칙에 따라
  시크릿과 고객 값이 없습니다. Stable key/identifier/path는 ASCII/English를 유지하고
...
- 어떤 T0 판정도 발사되기 전에 **인벤토리 그래프가 채워짐**: 병렬 full-scan 이 원자적으로
  ...
  upsert 로 no-op. Scan 사이에는 Azure resource change가 Huginn을 통해 들어오고 ordered durable
  overlay가 resource, link, tombstone delta를 적용합니다. Graph-dependent action은 RiskGate에서
  snapshot과 overlay freshness를 읽고 freshness 또는 coverage가 unknown, degraded, stale이면 HIL로
  route합니다.
## 의존성
