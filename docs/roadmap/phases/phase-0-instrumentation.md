...
| Task | Title | Deps | Deliverable | Acceptance | Size |
|------|-------|------|-------------|------------|------|
| **W2.1** | Scenario schema | W1.2 | `services/core-control-plane/tests/scenarios/schema.json` - event input, expected verdict, domain, tags | Schema validates in CI; unknown domain / verdict values are rejected | S |
| **W2.2** | Author balanced scenarios | W2.1 | `services/core-control-plane/tests/scenarios/v2026.07/` - Change / DR / FinOps balanced synthetic events (target ≥ N per domain, `N` decided at authoring time) | Balance check in CI: no domain deviates > 10% from the mean count | M |
| **W2.3** | Freeze + version | W2.2 | Directory `services/core-control-plane/tests/scenarios/v2026.07/` is **immutable** via branch protection; a new set is a new version directory | CI rejects any modification to an existing versioned directory | S |
...
| **W3.2** | Baseline runner CLI | W3.1, W1.5 | `python -m tools.baseline_run --scenarios services/core-control-plane/tests/scenarios/v2026.07` - records success metrics + guard metrics + sample size + confidence interval | CLI exit code non-zero on any missing metric | S |
...
| Task | Title | Deps | Deliverable | Acceptance | Size |
|------|-------|------|-------------|------------|------|
| **W5.1** | Exemption artifact schema | W1.2 | `services/core-control-plane/src/fdai/rule_catalog/schema/exemption.schema.json`; PR template in `.github/PULL_REQUEST_TEMPLATE/exemption.md` | Missing `justification` / `expires_at` fails CI | S |
