...
각 산출물은 아래 섹션에 매핑. 모듈 참조는 [`services/core-control-plane/src/fdai/`](../architecture/project-structure-ko.md)에서
해당 산출물을 담고 있는 주요 Python 패키지를 가리킴.

- 복원력, 변경 안전성, 비용 거버넌스에 걸친 **통합 컨트롤 루프** - 하나의 `trust-router` →
 `risk-gate` → `executor` → `audit` 경로, 리소스별 순서/락과 크로스-버티컬 충돌 처리
 ([통합 컨트롤 루프](#통합-컨트롤-루프)).
 모듈: [core/control_loop.py](../../../services/core-control-plane/src/fdai/core/control_loop/orchestrator.py),
 [core/risk_gate/precedence.py](../../../services/core-control-plane/src/fdai/core/risk_gate/precedence.py).
- 윈도우-기반 테스트 장애 조치 / game 일, 딥 DB-DR 처리, 측정된 RPO/RTO 보고 있는 **DR/Chaos
 스케줄러** ([#dr--chaos--스케줄된-주기-테스트](#dr--chaos--스케줄된-주기-테스트)).
 모듈: [core/verticals/resilience.py](../../../services/core-control-plane/src/fdai/core/verticals/resilience/orchestrator.py).
- 교정 PR로 딜리버리되는 리스크-게이팅 자율성 있는 **FinOps auto-actions**
 ([FinOps](#finops)).
 모듈: [core/verticals/finops.py](../../../services/core-control-plane/src/fdai/core/verticals/cost_governance/finops.py).
- 저위험 auto-merge/조정, 고위험 HIL 로의 **통합 변경 안전성**
 ([변경 안전성](#change-safety-integrated)).
 모듈:
...
- **단일 경로**: 모든 도메인 이벤트는 `event-ingest` 에서 정규화되고 공유 `trust-router` 로
 ...
 다름, 루프 구조가 아님.
- **버티컬별 아이덴티티**: 복원력, 변경 안전성, 비용 거버넌스는 각각 자체 액션 화이트리스트에
 범위된 **별개 user-assigned Managed Identity** 하에 실행, 영향 범위가 버티컬로 한계되고 어떤
...
- **크로스-버티컬 충돌 처리**: 헌법 hard 제약이 먼저 부적격 선택지를 제거합니다. 남은
...
- **윈도우-기반 스케줄러**: DR 장애 조치와 Chaos 실험을 승인된 유지 윈도우(테스트 장애 조치 /
 game 일) 안에서만 실행. 스케줄러는 **freeze/quiet 기간** 과 리소스별 **opt-out 태그** 존중,
 **동시 실험** 상한(blast-radius 한도), 각 실행 전후 운영자 알림.
...
- **시간 box**: 모든 실험은 finite 긍정 `max_duration_seconds`를 가집니다. 경과 시간이
 ...
 reservation을 유지하며 복구가 검증될 때까지 replacement 실험을 차단합니다.
- **RPO/RTO 보고**: 각 실행에 대해 명시된 목표 대비 **측정된 RPO** (장애 조치 데이터 손실) 와
...
- **Replication/백업**: point-in-time 복원 (PITR / continuous 복원), geo-replication
...
- **라우팅과 자율성**: 후보 액션(idle 종료, rightsizing, spot/autoscale) 은 공유
 `trust-router` 로 라우팅되고 `risk-gate` 게이팅 - **non-prod, 저위험 액션은 auto-execute;
 프로덕션-영향 액션은 HIL**.
- **딜리버리**: 액션은 **교정 PR** (GitOps) 로 딜리버리, 감사·리뷰·롤백이 git에서 옴 -
...
- 저위험 변경은 **auto-merge/조정**; 고위험은 **HIL** 로 이동, 사람이 승인·거부·요청 시간 초과
 (거부와 시간 초과는 여전히 감사하는 no-op). 승인과 실행은 별개 principal 유지
 ([security-and-identity-ko.md](../architecture/security-and-identity-ko.md)).
- **변경 lead 시간** 은 같은 시나리오 세트에서 P0 참조 에이전트 대비 **측정** 감소로 보고
...
- DR 스케줄, FinOps 후보, 규칙 엔트리 픽스처는 시크릿과 고객 값이 없고 고정된
 키/식별자/경로는 ASCII/English를 유지합니다. Natural-language 값은 한국어와 영어를 모두 허용하며 정규화 규칙 스키마를 따름
...
## 열림 Questions (각각 소유자 필요)

- 안전 장애 조치 윈도우와 large-DB 복원 RTO 목표 - 소유자: DR/Chaos 리드.
