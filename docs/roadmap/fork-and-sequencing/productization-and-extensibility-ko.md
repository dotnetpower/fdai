...
> cloud-operations 컨트롤 plane으로 유지됩니다. 새 인터페이스는 실행기 신원을 받지 않으며
...
> `부분 구현`은 안전한 foundation이 있지만 운영 전송 계층, 영속 어댑터, release
...
| 우선순위 | 의미 | 승격 규칙 |
|----------|------|-----------|
| P0 | 필수 platform foundation | 통합 또는 user experience를 넓히기 전에 완료 |
...
| ID | 기능 | 상태 | Exit 게이트 |
|----|------------|------|-----------|
| P0-01 | 설치형 `fdaictl` 항목 지점 | 구현됨 | 출처 및 wheel 항목 지점이 해석되고 결정론적 `version` 텍스트 및 JSON 통과 |
...
| P0-15 | 운영 채널 발행기 및 경로 | 구현됨 | Standalone ASGI 런타임이 시크릿 참조를 해석하고 signed Slack 및 구체적인 Teams auth/발행기를 wiring하며 게이트웨이 소비자 시작, 실패 시 차단 시작, 종료 경로/작업/채널/owned HTTP 정리 통과 |
...
| P0-20 | 영속 스케줄러 전달 원장 | 구현됨 | Atomic 점유, publish/fail, stale-to-lost 조정, 재시도, 이행, 운영 wiring 통과 |
...
| P1-12 | 모델 상태, cooldown, 복구 상태 | P0-21 | 구현됨: role-agnostic 민감정보가 제거된 실패/복구/선택 전이를 PostgreSQL에 저장, 범위가 제한된 cooldown 및 장애 조치는 telemetry 실패 시에도 유지 |
...
| P1-20 | OpenTelemetry 내보내기 도구 및 라우팅 전이 | 기존 telemetry | 구현됨: secure 선택적 OTLP/gRPC 내보내기 및 범위가 제한된 고정된 구간/메트릭을 채널, 확장, 모델, 스케줄러, security 전이에 기본값 제공 |
| P1-21 | 공개 확장 authoring kit | P0-17부터 P0-19 | 구현됨: strict 템플릿/스키마, `fdaictl extension validate`, 보관 다이제스트, 호스트 호환성, disabled-first, mandatory security checklist 동시 제공 |
...
| P1-26 | 선택적 code-assurance 패키지 | P0-17부터 P1-10 | 구현됨: 독립 wheel이 범위가 제한된 읽기 전용 GitHub pull-request 코드/security 도구, exact-SHA 근거, 통제된 스킬, package-carried 도구 메타데이터, disabled-first 확장 activation을 제공하며 GitHub 쓰기 표면은 없음 |

공개 확장 kit는 `examples/extension-kit/extension-kit.template.json`에 있고 머신 스키마는
...
선택적 `extensions/code-assurance/` workspace 패키지는 FDAI wheel 밖에 유지됩니다. `Pull
...
런타임 trust는 별도의 `fdai.extension-signature.v1` 및 `fdai.skill-signature.v1` 페이로드 domain을
...
- **Wake-word, continuous 어조, camera, 위치, SMS-device, screen-control 노드:** Cloud-operations
 컨트롤과 관계없는 device-trust domain을 만듭니다.
...
- **Arbitrary dynamic 코드/플러그인 로딩:** 확장은 검토된 타입이 지정된 번들을 등록합니다.
 컨트롤 plane 안에서 검토되지 않은 패키지를 download하고 실행하지 않습니다.
...
1. P1 확장 중에도 모든 P0 배포 및 release 게이트를 enforce 상태로 유지합니다.
2. release/백업/onboarding, 채널 richness, 확장 및 스킬 UX, 모델 상태, 기억,
  예약, 웹훅, observability, authoring kit 순으로 P1을 구현합니다.
...
release 배치는 clean 체크아웃에서 `scripts/verify.sh --all`도 실행하고 wheel 및 배포
번들을 빌드하고 isolated 환경에 wheel을 설치하고 서명을 검증하고 disposable
...
subsystem을 검사하고 Alembic 헤드가 하나인지 확인하고 wheel을 빌드하고 isolated `uvx`
