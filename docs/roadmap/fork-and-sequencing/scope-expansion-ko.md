...
Resolved 및 closed 인시던트는 경보하지 않습니다. `project_incident_metrics`는
...
사후 분석 을 demand. FDAI 는 raw 자료 (감사 로그, 발견 사항,
...
- **Knowledge 추출 (재사용 lesson)**:
  ...
  **실패 시 차단**: 감사 이력 에 기록된 루트 원인 *와* 최소 하나의
...
| SRE 의무 | 상태 | 위치 |
|----------|------|------|
| 인시던트 감지 / 분류 / 수명 주기 | Covered | `core/incident/` (§ 3.1), `core/event_ingest/` |
...
| 모니터링 / alerting (외부 신호 인제스트) | Covered | 메트릭, 범위가 제한된 KQL, App Insights 추적, Activity Log, 진단 스트림, anomaly, 예측, RCA 텔레메트리 grounding을 제공합니다. |
