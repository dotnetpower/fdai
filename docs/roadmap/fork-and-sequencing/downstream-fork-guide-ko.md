...
Downstream FDAI 분포를 만들고 동기화 상태를 유지하며 지원되는 경계를 통해 기능을
제한하거나 확장하는 방법입니다. **포크 유지관리자**를 위한 단일 진입점입니다. 포크는
customization 프로파일을 패키지하며 배포, 테넌트, 환경, 운영 상태가 아닙니다.

Upstream 저장소는 의도적으로 범용하고 customer-agnostic 합니다
([generic-scope.instructions.md](../../../.github/instructions/generic-scope.instructions.md)).
포크에는 범용으로 유지할 수 없는 downstream 어댑터 구현, 카탈로그 또는 표현
overlay가 들어갑니다. 배포 값, 테넌트 신원, 시크릿, 환경, 승격 상태는
배포 구성 또는 시크릿 저장소에 남습니다. 아래 규칙은 포크가 충돌 없이
upstream과 sync하고 upstream 이력에 customer 값이 들어가지 않게 합니다.
...
- **Upstream** = 이 저장소. 범용한 컨트롤 플레인 (코어 엔진, DI
 경계, 기본 비활성 가짜, 카탈로그 스키마) 배포.
- **포크** = 선택적인 downstream 분포. 지원되는 경계로 적용하는 구체적인 어댑터와
 기능, 카탈로그, 정책, 표현 overlay를 포함합니다.
- **배포** = upstream 또는 포크의 running instance입니다. 테넌트 신원, 시크릿 참조,
 리소스 범위, 환경, 승격 상태를 출처 컨트롤 밖에서 제공합니다.
- **기여 방향**: upstream은 포크에서 절대 pull하지 않음. 포크가
 개선을 위해 upstream `main`에서 pull. 포크가 모든 고객에게 유용한
...
| 축 | 예 | 포크가 선택하나요? |
|----|----|---------------------|
| 분포 | upstream, downstream 포크 | 예, 출처/패키지 경계만 선택 |
| 배포 환경 | dev, staging, 운영 | 아니요 |
| 근거 프로파일 | 권위 있는, 고정본 | 아니요 |
| 자율성 | 기능별 그림자, enforce | 아니요 |
...
1. **Fresh clone에서 기준선이 green인지 확인**: `uv sync` 후
 ...
 리포지토리 검사입니다. 손대지 않은 체크아웃에서 upstream 테스트 스위트가
...
4. **포크의 CI에서 시크릿 검사 활성화** - upstream의
...
1. 구성이나 가짜에 속하는 값을 주입하려 함. 이미 존재하는
  경계를 찾으세요.
2. Upstream 설계에 진짜 공백을 발견함. Upstream issue를 열거나 fork-
  로컬 래퍼로 `core/`를 patch하지 않고 감싸는 변경을 배포하세요.
  그 후 래퍼를 scrub해서 upstream에 기여.
...
- Upstream의 `scripts/integrity/check-protected-paths.sh`가 변경된 파일을
 ...
 (upstream)하거나 **하드 차단(포크)** 합니다. 포크는 `FDAI_FORK=1`
...
- **서명된 무결성 매니페스토**로 framework 표면 변조를 OFFLINE에서
 ...
 매니페스토 - 항상 오류입니다. 포크는 upstream 개인키 없이는 유효한
 매니페스토를 만들 수 없기 때문입니다)와 **콘텐츠(내용)**
 불일치(편집/추가/삭제된 표면 파일 - 포크 모드에서는 하드 실패,
 upstream에서는 권고). 표면 목록의 단일 소스는
...
**Copy-ready shipped 예제**: upstream에 더 작은 종단 간 참조도
...
포크는 upstream `main`을 스케줄로 pull하여 건강을 유지 (매주가 좋은
기본값). 포크가 `core/`를 절대 편집하지 않고 고객 값을 절대 커밋하지
않으므로, 병합은 일반적으로 clean.

### 6.1 버전 pinning 전략

"upstream `main`을 매주 track"은 aspirational; 실무에서 포크는
**known-good upstream 참조**에 pin하고 pin을 의도적으로 advance 해야
SHOULD. 수용 가능한 두 전략:

1. **Tag에 pin** (권장). Upstream은 이정표 경계에서 semver-adjacent
  태그를 컷. 포크의 `pyproject.toml`이 upstream 패키지에 의존한다면
  (또는 포크의 `git subtree` / submodule 포인터가) 그 태그를 참조.
  Pin advance는 리뷰된 PR: upstream CHANGELOG 읽기, 포크 테스트 스위트
  실행, 그 다음 advance.
...
Upstream 정책은 한 release 동안 새 프로토콜을 old와 함께 배포한 뒤
...
- [coding-conventions.instructions.md](../../../.github/instructions/coding-conventions.instructions.md) -
 안전성 invariant, shadow-mode 기본값, async-Protocol 계약, 포크가
...
- [`docs/runbooks/`](../../runbooks) - 포크의 on-call이 실행하는 운영
 ...
 Fork-specific 런북은 `fork/runbooks/` 아래 두고 upstream 템플릿을
 참조.
