...
release-batch 검사이며 두 로컬 결과만으로 입증되지 않습니다. 이 검사들은 double-build 비교를
통한 재현 가능한 signed 번들 구성도 입증하지 않습니다. 현재 automatic-version 작업 흐름은 버전
메타데이터와 태그만 게시합니다. 포크에서 이를 보호되는 release 배치, signed 번들 게시 또는 설치
게이트로 간주해서는 안 됩니다.
