...
**Live `resolved-models.json`은 deployment artifact입니다.** Bootstrap
`llm_resolver_cli`가 생성하며 `LLM_RESOLVED_MODELS_PATH`는 filesystem path 또는 inline JSON을
받습니다. Live 결과에는 deployer/subscription/deployment/region provenance가 있으므로 fork에
커밋하지 않습니다. Upstream의 tracked `resolved-models*.json`은 all-zero identity를 사용하는
synthetic generated baseline이며 hand-edit하지 않습니다. Direct Key Vault loader는 reconciler와
함께 연기됐고, 현재 secretRef env 또는 mounted file이 day-zero 전달 방식입니다.
...
**중복 `id`는 hard error**. `load_rule_catalog`는 한 root 안의 중복을 거부하고,
두 root를 합친 뒤 `RuleIndex.build`가 cross-root 중복을 거부합니다. Ontology dispatch는
...
`RuleIndex.build`가 두 root 사이의 duplicate id를 최종 fail-close합니다. Index를 만들지 않는
...
Schema와 `fdai.shared.contracts.models`의 pydantic 모델로 검증된 immutable tuple을 반환합니다.
각 loader는 root 내부 중복을 거부합니다. Upstream + fork tuple을 합칠 때는 composition root가
...
| `CriticModel` / `JudgeModel` | HTTP 에러, quota | Raise; `DebateOrchestrator`가 `DebateVerdict.ABORT`와 `error_class`를 반환 -> HIL. |
| `WebSearchProvider` | HTTP 에러, timeout | Raise하거나 empty result를 반환할 수 있습니다. Caller가 exception을 sanitized `provider_error` evidence로 변환하며 action authority를 높이지 않습니다. |
...
| `OperatorMemoryStore` | 쓰기 시 DB down | Raise; entry가 저장되지 않으며 caller가 승인 workflow를 fail closed합니다. |
| `OperatorMemoryStore` | 읽기 시 DB down | Raise; composer가 stale/empty memory로 조용히 진행하지 않고 current request를 fail closed합니다. |
...
- `default_mode` - upstream과 fork의 모든 새 catalog entry는 반드시 `shadow`입니다.
  Loader가 `enforce`를 거부하며 promotion state는 authoritative registry가 별도로 소유합니다.
...
- `trigger_kind` (선택) - `{kind: rule_violation}`, `{kind: operator_request}`, 또는
  `{kind: both}` object. `operator_request` 또는 `both`일 때는 콘솔이 coordinator
...
`RemediationPr`은 완전히 렌더된 payload (title, body, patch, patch_path, labels,
action/idempotency id)를 carry하고, `PublishReceipt`는 audit log가 나중에
인용할 수 있는 stable `pr_ref`를 반드시 포함. Upstream executor는
...
**Compatibility helper로 재사용** (upstream에서 import, 재정의 금지):

- `_resolve_catalog_root` / `_resolve_policies_root` -
  환경 / 파일시스템 discovery.
- `_finalize_llm_bindings` - env-driven upstream entry를 위한 compatibility wrapper.
  Programmatic fork composition은 public `wire_azure_container` + `AzureWireOverrides`를
  직접 사용하는 것이 기본입니다.
...
- 전체 `__main__.py`를 copy-paste 해서 in-place 편집. Upstream sync
  ...
  것.
- Azure mode에서 env-driven `_finalize_llm_bindings`와 programmatic
  `wire_azure_container`를 섞어 두 번 binding. 둘 중 하나를 선택하고 public
  `AzureWireOverrides` validation을 우회하지 마세요.
