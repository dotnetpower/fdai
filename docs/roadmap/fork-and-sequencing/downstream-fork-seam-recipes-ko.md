...
Downstream FDAI 포크를 위한 per-seam 조리서. 각 항목은 동일한 형태를
따릅니다: **언제 재정의**, **경계**, **바인딩 방법**, **테스트
방법**. 모든 스니펫은 Python 3.12+와 upstream 패키지가 `fdai`로 가져오기
가능하다고 가정합니다.

이 파일은 [downstream-fork-guide-ko.md](downstream-fork-guide-ko.md)
의 동반 문서 - 허브 파일이 포크 모델, Day-1 체크리스트, one-hard-rule,
repo 레이아웃, upstream sync, anti-pattern을 소유합니다. 그 허브를 먼저
...
커밋하지 않습니다. Upstream의 tracked `resolved-models*.json`은 all-zero 신원을 사용하는
...
Upstream이 전체 Azure wire-up을 위한 **공개 조립 API**를
...
**하위 호환성**: upstream의 `runtime.configuration._finalize_llm_bindings`는
env var (`FDAI_LLM_ENDPOINT`, `FDAI_CATALOG_ROOT`,
`FDAI_OPERATOR_MEMORY_DSN`)를 읽고 `wire_azure_container`에
위임하는 얇은 래퍼. 기존 테스트와 upstream 진입점은 변경 없이 계속
작동. Env-driven wiring을 선호하는 포크는 래퍼를 호출; 프로그램적
...
**테스트 방법**: 단위 테스트에는 upstream in-memory 가짜
...
upstream이 배포하는 순수 도메인 모듈; 이를 트리거하는 "second 승인"
...
두 허용 목록의 **교집합**에 있는 domain을 가진 스니펫만 반환 - 조회가
...
Upstream 테스트는 sanitizer + `NoOpWebSearchProvider`를 커버; 포크는
`httpx.MockTransport`로 자체 어댑터 레벨 테스트 추가.

### 5.5 HilChannel (Teams / Slack / custom)

**언제 재정의**: 어떤 HIL 흐름든 활성화. Upstream은 in-memory 가짜를
...
instance 전달, HIL 승인이 전달되는 컨트롤 루프에 wire.
...
**언제 재정의**: 실제 이벤트에 대해 운영자 기억 활성화. Upstream이
...
두 기능: `t2.critic` (upstream이 이미 선언) + `t1.judge`
...
지역별 카탈로그 고정본에 대해 LLM 해석기 CLI 실행. Upstream CLI는
...
라우터 킬스위치)는 이미 upstream에 커버됨; 포크의 테스트는 실제 운영 어댑터에
집중.
...
- Upstream 룰 비활성화: 동일-id 재정의를 배포하지 말 것.
...
- Upstream 룰의 동작 변경: fork-patch 하지 말고 upstream issue를
 열 것. Upstream 룰 카탈로그는 customer-agnostic; 그 동작에 대한
 customer-specific 변경은 upstream에 구성 knob이 필요하다는 신호.
...
별도 소비자도 결합 tuple의 id uniqueness를 같은 방식으로 검사해야 합니다.
...
스키마와 `fdai.shared.contracts.models`의 pydantic 모델로 검증된 변경할 수 없는 tuple을 반환합니다.
각 로더는 루트 내부 중복을 거부합니다. Upstream + 포크 tuple을 합칠 때는 조립 루트가
...
2. LinkType당 YAML 하나를 `fork/vocabulary/link-types/`에 배치 (예:
  ...
  ObjectType 레지스트리 (upstream + 포크)에서 해석되어야 함;
...
3. 조립 루트에서 두 루트를 로드하고 `dataclasses.replace`로 주입:
  ...
  objects = upstream_objects + fork_objects
   ...
   if len(object_names) != len(집합(object_names)):
    raise ValueError("중복 ObjectType 이름 across upstream and 포크 roots")
  ...
  fork_links = load_link_type_catalog(
   ...
   if len(link_names) != len(집합(link_names)):
    raise ValueError("중복 LinkType 이름 across upstream and 포크 roots")
  컨테이너 = replace(
    컨테이너,
...
테스트는 joint 부하 (upstream + 포크 루트)와, 새 ObjectType이 필요한
소비자(assurance twin, 운영자 콘솔, custom 전달 어댑터)
에서 dispatchable한지 assert 하나에 집중.

**작동 참조**: upstream이 `ChangeSummary` (
...
- 배포된 `rule-catalog/vocabulary/object-types/*.yaml` 편집 -
 built-in ObjectType 변경은 포크가 아닌 upstream으로.
...
overlay는 자율성을 낮추기만 가능하고 절대 올릴 수 없음, per
...
모듈은 아직 overlay 파일을 로드하지 않습니다.** 오늘 두 개의 권위 있는
...
**Overlay wire가 landing할 때까지 포크 지침**: 의도된 tighter 상한을
...
**추적**: Overlay wire는 Wave 4.5 delta-2b의 후속 조치로 계획됨;
...
|------|------------------|-----------|
| `EmbeddingModel` / `CrossCheckModel` | HTTP 에러, 시간 초과 | Raise; upstream이 catch하고 quality 후보를 abstain (HIL). 합성 빈 응답을 절대 반환하지 말 것. |
...
| `HilChannel.send` | 배달 실패 | Raise; upstream이 로그하고 감사 trail이 승인을 `dispatch_failed`로 표시. 액션은 pending 유지; auto-execute 없음. |
...
공통 invariant: **실제 운영 어댑터 에러에서 성공을 조작하지 말 것**.
...
프로토콜을 준수함을 증명, (b) composition-root 변경 후에도 upstream
...
ActionType을 조정할 때는 형제 디렉터리에 same-name overlay를 배치
...
- `operation` - `fdai.shared.contracts.models`의 `Operation` enum에
 ...
 동사가 필요하면 upstream issue 열 것 - enum은 감사 어휘라서
 포크되지 않아야 함.
...
- `default_mode` - upstream과 포크의 모든 새 카탈로그 항목은 반드시 `shadow`입니다.
...
- `preconditions[]` / `stop_conditions[]` - T0 검증기가 risk-gate
 전에 평가하는 결정론적 검사. 빈 리스트는 실행기가 독립 invariant를
...
추가하려는 포크는 overlay 이름 아래에 전체 precondition 리스트를 배포.
...
- 배포된 `rule-catalog/action-types/*.yaml` 편집 - ObjectType 편집과
 동일 규칙: 배포된 ActionType은 upstream으로, 포크는 새로 배포하거나
 overlay.
...
인용할 수 있는 고정된 `pr_ref`를 반드시 포함. Upstream 실행기는
...
**Composition-root wiring** (기본 발행기 대체):
...
- [`panels.py`](../../../services/operator-service/src/fdai_operator_service/) 아래
 upstream `ExampleFinOpsPanel`은 참조 구현이며 기본으로
...
**Composition-root wiring** (포크의 `entry.py`에 등록):
...
MUST. 그 콘솔 편집은 포크의 repo `console/` 아래에서만 살고 upstream
`console/`은 범용 유지.
...
"upstream `__main__` 대신 이 모듈에서 가져오기 하도록 프로세스
진입점을 이름 변경"이라고 명시; 이 recipe는 작동하는 `fork/entry.py`가
어떻게 생겼는지 보여줍니다.

**경계**: upstream의 [`services/core-control-plane/src/fdai/__main__.py`](../../../services/core-control-plane/src/fdai/__main__.py)는
...
**호환성 보조 로직으로 재사용** (upstream에서 가져오기, 재정의 금지):

- `_resolve_catalog_root` / `_resolve_policies_root` -
 환경 / 파일시스템 발견.
- `_finalize_llm_bindings` - env-driven upstream 항목을 위한 호환성 래퍼.
...
- `_build_control_loop` - 카탈로그, ActionType, 온톨로지 (5.8a),
 룰의 조립. 포크는 보통 upstream 헬퍼를 호출하고 반환값을
...
Upstream은 동일한 `fdai` 스크립트를 `fdai.__main__:main`을 가리키게
...
1. `container.ontology_object_types`가 upstream과 포크 이름을 모두
  포함.
...
**경계** (셋 다 upstream에서 abstain하므로, 미배선 포크는 규칙을
...
최신성 diff, 커버리지)는 upstream이며 포크 작업이 필요 없다. 빌드
타임 오케스트레이터
...
--snapshot <file>`로 한 번의 pass를 실행한다. 계획은 inert하다 - 증류된
후보는 enforce 전에 여전히 grounding / 그림자 / 회귀 / 승격
게이트를 거친다.
...
`Distiller` 출력이 `source_ref` 출처 이력을 인용하고 커버리지 diff를
통과하는지 assert한다.
...
1. Upstream과 포크의 도구, ActionType, 작업 흐름 카탈로그를 함께 로드합니다.
