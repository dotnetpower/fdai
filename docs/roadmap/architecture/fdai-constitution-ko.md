...
FDAI는 증거로 관리되는 자율 클라우드 운영 컨트롤 플레인입니다. 독립적으로 실행 가능한 15개
...
및 ChatOps 화면을 갖춘 헤드리스 컨트롤 플레인이며 챗봇, 대시보드, 클라우드 운영 밖의 범용
...
실행 장소(execution venue)는 컨트롤 플레인 동작을 바꾸지 않습니다. 모든 장소는 동일한
권위 있는 소스, 유입 경로, 계약, 컨트롤 루프 단계 및 화면을 바인딩하며 자격 증명, 엔드포인트,
규모 및 공급자 범위만 다를 수 있습니다. 바인딩하지 않은 장소별 경로는 허용되는 상태가 아니라
결함입니다. 사용 불가 또는 빈 화면은 그 권위 있는 소스가 실제로 없거나 도달 불가하거나
권한이 없는 동안에만 옳으며, 장소가 구성하지 못한 바인딩을 대신하는 표시로는 결코 옳지
않습니다. 합성, 픽스처, 시드 또는 데모 대체는 모든 장소에서 계속 금지하므로, 장소 간 패리티
격차를 닫는다는 것은 그 출력을 만들어내는 것이 아니라 실제 소스를 바인딩한다는 뜻입니다.
...
- Saga와 Vidar는 변경 작업의 필수 의존성입니다. 사용할 수 없으면 기능을 shadow 또는
  무작업으로 낮추며 실패 시 권한을 열지 않습니다.

의존성 loss는 필요한 계약 전체가 독립적으로 계속 사용 가능한 경로만 보존합니다. 읽기,
거부, 큐 및 shadow evaluation은 계속할 수 있습니다. Judge, 실행기, auditor, 복구 경로,
...
**FDAI-CONST-005 - 하나의 컨트롤 플레인과 여러 운영 도메인.** 도메인 기능은 도메인별
...
유지합니다. 성공을 보고하기 전에 독립적인 효과 검증이 필요합니다. 새 기능은 shadow 모드에서
...
순서, 실패 간선 또는 보상을 바꾸면 새 변경할 수 없는 작업 흐름 버전이 되어 shadow로 돌아갑니다.
...
| 007 | Security and 신원, 액션 온톨로지 | shadow, 예행 실행, 잠금, 멱등성, 롤백, 감사 테스트 |
