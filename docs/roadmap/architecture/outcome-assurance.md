...
| Cost Governance effect settlement source | implemented | `core/measurement/cost_effect_settlement.py`; `core/measurement/cost_retention.py`; focused Cost Governance settlement tests | Cost, capacity, service, and recovery effects remain separate and close only from independent observations. This source does not complete the broader `OutcomeAssuranceProjection`. |
