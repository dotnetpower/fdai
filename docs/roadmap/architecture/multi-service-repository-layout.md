...
| Core service entry points | The Core manifest exposes service-owned bounded maintenance entry points, including the shadow operational-history lifecycle Job and the positional protected certification runner used by Container Apps. Each entry point composes delivery adapters inside Core's package boundary and does not grant executor authority. |
