...
| Service communication | Services exchange versioned contracts over PostgreSQL-owned projections and the event bus. Rule activation follows this boundary: Operator owns authenticated proposals and its outbox, Core owns immutable receipts and generation application, and the shared package owns only content-free wire contracts. One service never imports another service's implementation package. |
