...
| Operational state decision-evidence admission | implemented | [`evidence_bundle.py`](../../../services/core-control-plane/src/fdai/core/operational_context/evidence_bundle.py), [`test_evidence_bundle.py`](../../../services/core-control-plane/tests/core/operational_context/test_evidence_bundle.py) | Each decision-bound state item retains the shared receipt and verification-bundle admission. The bundle recomputes the exact state-item and scope digests and lowers to `SHADOW_ONLY` with an explicit hold when admission is missing, mismatched, or expired. Direct state consumers outside the operational bundle remain separate migration work. |
