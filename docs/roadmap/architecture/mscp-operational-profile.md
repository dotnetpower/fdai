...
|------|-------|----------|-------|
| HIL effect-reconciliation composition boundary | implemented | `runtime/control_loop.py`; `core/hil_resume/coordinator.py`; focused HIL reconciliation checks | Runtime supplies the ordinary reconciliation sink to HIL resume independently from the optional MSCP observer pair. Accepted or pending dispatch remains unverified until authoritative observation closes it. |
...
|------|-------|--------|----------|-----------|
| 2026-09-22 | implemented | Made active Rule membership replaceable between complete decisions while preserving the MSCP profile boundary. Generation changes neither activate effect observation nor alter `ResponseOutcome`, authority ceilings, approval, or execution. | `current change`; control-loop generation replacement and focused T0 and activation tests. | The existing measured-readiness and governed-gating work remains unchanged. |
| 2026-09-14 | implemented | Wired the existing effect-reconciliation request sink into HIL resume without activating MSCP or changing its authority ceiling. | `current change`; Issue #962; focused HIL reconciliation and runtime wiring checks. | Retain deployed independent-observation evidence for one HIL-approved action. |
...
The composition also supplies the normal effect-reconciliation request sink to HIL resume. That
sink is independent of the optional MSCP pair: it does not activate this profile, raise autonomy, or
convert an accepted or pending dispatch into verified effect evidence.
