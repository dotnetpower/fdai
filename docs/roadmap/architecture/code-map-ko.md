...
translation_source_sha: 07fb5398d0ce8058b1cd3d9346f85c84f421b88d
...
> 같은 검토된 Kubernetes binding을 받습니다. 또한 실행 환경별 broker 보안을 선택하고 공통
> 300초 tick 상한을 강제해 local/deployed parity를 유지합니다.
> LIST pagination은 observation 2048개로 제한된 각 pass 뒤에 비공개 이어받기 상태를 보존하고
> snapshot을 모두 처리한 뒤에만 watch cursor를 설정합니다. Pod inventory는 creation timestamp와
> 명시적으로 controller 표시된 owner identity를 보존합니다.
...
|------|----------------|--------|------|
| Kubernetes Resource 이벤트 이력 | 출처에 근거한 정확한 대상 계획, 실패 시 차단하는 정확한 identity 개수 검사, 증적에 결속된 정확한 child UID 필터 또는 명시적인 정확한 클러스터 범위의 제한된 Kubernetes Event 읽기, 영속적이며 범위가 제한된 LIST 이어받기, 연속된 조회 기간 coverage 검사, 정규화한 이벤트 시각, 내용 기반 근거, 제한 사항을 보여주는 이중 언어 답변, 독립적인 Azure/Kubernetes 기능군 라우팅, 명시적인 불완전 결과, 원시 메시지와 원인, 변경 또는 실행 권한 없음 | [의미 플래너](../../../services/core-control-plane/src/fdai/core/conversation/semantic_resource_event_planning.py), [FunctionType](../../../services/core-control-plane/src/fdai/core/ontology_platform/resource_event_queries.py), [lifecycle 출처](../../../services/core-control-plane/src/fdai/delivery/kubernetes_lifecycle_source.py), [영속 reader](../../../services/core-control-plane/src/fdai/delivery/durable_kubernetes_resource_event_history.py), [PostgreSQL store](../../../services/core-control-plane/src/fdai/delivery/persistence/postgres_kubernetes_lifecycle.py), [기능군 라우터](../../../services/core-control-plane/src/fdai/delivery/resource_event_history.py), [런타임 연결](../../../services/core-control-plane/src/fdai/runtime/resource_event_providers.py), [답변 변환 결과](../../../services/core-control-plane/src/fdai_core_service/semantic_turn_processor.py) | [의미 계획 테스트](../../../services/core-control-plane/tests/conversation/test_semantic_planning.py), [lifecycle 출처 테스트](../../../services/core-control-plane/tests/delivery/test_kubernetes_lifecycle_source.py), [영속 reader 테스트](../../../services/core-control-plane/tests/delivery/test_durable_kubernetes_resource_event_history.py), [라우터 테스트](../../../services/core-control-plane/tests/delivery/test_resource_event_history.py), [FunctionType 테스트](../../../services/core-control-plane/tests/core/ontology_platform/test_resource_event_queries.py), [답변 테스트](../../../services/core-control-plane/tests/test_semantic_turn_processor.py), [런타임 테스트](../../../services/core-control-plane/tests/runtime/test_resource_health_provider.py) |
| Kubernetes Pod lifecycle cohort | 완전한 inventory snapshot의 변경할 수 없는 Pod identity를 저장하고 정확한 과거 UID로 이벤트를 결합하며 요청한 조회 기간의 연속 수집 coverage를 요구하고 현재 후보를 증적에 결속하는 별도의 컨트롤러 기반 교체 근거입니다. 형제 이벤트를 다시 표시하거나 Event `related.uid`를 소유권으로 신뢰하지 않습니다. | [cohort FunctionType](../../../services/core-control-plane/src/fdai/core/ontology_platform/kubernetes_pod_lifecycle_cohort_queries.py), [identity 변환 결과](../../../services/core-control-plane/src/fdai/delivery/kubernetes_pod_lifecycle_identity.py), [영속 reader](../../../services/core-control-plane/src/fdai/delivery/durable_kubernetes_pod_lifecycle_cohort.py), [PostgreSQL store](../../../services/core-control-plane/src/fdai/delivery/persistence/postgres_kubernetes_lifecycle.py) | [cohort 테스트](../../../services/core-control-plane/tests/delivery/test_durable_kubernetes_pod_lifecycle_cohort.py), [identity 테스트](../../../services/core-control-plane/tests/delivery/test_kubernetes_pod_lifecycle_identity.py), [축약기 및 조회 테스트](../../../services/core-control-plane/tests/core/ontology_platform/test_kubernetes_pod_recovery_queries.py), [조립 테스트](../../../services/core-control-plane/tests/composition/test_wire_kubernetes_pod_recovery.py) |
| Pod 재시작 및 교체 분리 | 현재 Pod 재시작 상태를 컨트롤러 전체 교체 불확실성과 분리하고 교체 미비점은 별도 경로를 사용합니다. 긍정적인 교체 검증에는 형제 Pod 추정이 아니라 독립적으로 근거가 확인된 정확한 이전 Pod가 필요합니다. | [복구 조회](../../../services/core-control-plane/src/fdai/core/ontology_platform/kubernetes_pod_recovery_queries.py) 및 [복구 근거](../../../services/core-control-plane/src/fdai/core/ontology_platform/kubernetes_pod_recovery_evidence.py) | [복구 테스트](../../../services/core-control-plane/tests/core/ontology_platform/test_kubernetes_pod_recovery_queries.py) |
| 정확한 Pod 진단 | 보안이 적용된 정확한 Pod UID, 범위가 제한된 종료 조회 기간, 최신이며 완전하고 충돌 없는 공급자 상태 메타데이터, UID로 필터링한 수명 주기 사유, 본문 없는 로그 근거 및 원인 또는 실행 권한 없음 | [진단 FunctionType](../../../services/core-control-plane/src/fdai/core/ontology_platform/kubernetes_pod_diagnosis_queries.py) 및 [진단 축약기](../../../services/core-control-plane/src/fdai/core/ontology_platform/kubernetes_pod_diagnosis_evidence.py) | [진단 조회 테스트](../../../services/core-control-plane/tests/core/ontology_platform/test_kubernetes_pod_diagnosis_queries.py) 및 [진단 축약기 테스트](../../../services/core-control-plane/tests/core/ontology_platform/test_kubernetes_pod_diagnosis_evidence.py) |
...
| 계획된 변경의 그래프 최신성 | 일반적인 Resource-Workload-BusinessService-Objective 연결에 도달하는 범위가 제한된 기본 3단계 탐색을 사용합니다. 내용 기반 주소와 변경 권한이 없는 증적을 통해 정확한 대상의 영향 분석을 안정된 인벤토리 매니페스트, 일치하는 변환된 운영 모델 개정, 활성 세대, 온톨로지 릴리스에 결합합니다. 순회 후 재검증은 각 감사 읽기 시각을 보존하지만 이를 그래프 변경으로 잘못 판단하지 않고 안정된 그래프 상태를 비교합니다. | [영향 평가](../../../services/core-control-plane/src/fdai/core/impact_analysis/change_assessment.py), [인벤토리 증적 출처](../../../services/core-control-plane/src/fdai/delivery/persistence/postgres_graph_freshness.py), [판테온 연결](../../../services/core-control-plane/src/fdai/runtime/bootstrap_pantheon.py) | [영향 테스트](../../../services/core-control-plane/tests/core/impact_analysis/), [증적 출처 테스트](../../../services/core-control-plane/tests/persistence/test_postgres_graph_freshness.py), [변경 체인 테스트](../../../services/core-control-plane/tests/agents/test_change_management_chain.py) |
| 관찰 모드 ARB 온톨로지 루프 | 계획된 의도만 허용하는 타입 지정 Huginn Change 유입, 정확한 릴리스에 결속된 컨텍스트와 근거 공급자 경계, 범위가 제한된 copy-on-write 시나리오, Forseti 소유 관찰 판정, Saga 감사, 키별 멱등 재현, `runs_review`를 워크플로 실행에만 사용하는 검증된 `Change -> change_instantiates_process -> Process` 출처 계보 기반 ReviewCase 변환 결과 | [관찰 루프](../../../services/core-control-plane/src/fdai/core/architecture_review/observation_loop.py), [Forseti 연결](../../../services/core-control-plane/src/fdai/agents/forseti.py), [ARB 변환 결과](../../../services/core-control-plane/src/fdai/core/architecture_review/projection.py) | [관찰 추적 테스트](../../../services/core-control-plane/tests/core/architecture_review/test_observation_loop.py), [변환 결과 테스트](../../../services/core-control-plane/tests/core/architecture_review/test_projection.py), [준비 상태 테스트](../../../services/core-control-plane/tests/core/architecture_review/test_readiness.py) |
...
| ActionType 승격 전달 | 리스 차단을 적용한 정확한 거버넌스 증적 예약, 실패 결과 중복 제거를 피하는 재시도 가능 실패 의미, 재시작 뒤 소진 상태 증적 재생, 검증된 영속 권한 새로 읽기 및 여러 복제본 사이의 개정 비교 후 설정 영속화 | [승격 전달](../../../services/core-control-plane/src/fdai/delivery/promotion.py), [승격 영속화](../../../services/core-control-plane/src/fdai/delivery/persistence/state_store_action_promotion.py), [직접 API 실행](../../../services/core-control-plane/src/fdai/core/executor/direct_api.py) | [거버넌스 전달 테스트](../../../services/core-control-plane/tests/delivery/test_governance_dispatch.py), [승격 실행기 테스트](../../../services/core-control-plane/tests/delivery/test_promotion_executor.py), [영속화 테스트](../../../services/core-control-plane/tests/persistence/test_state_store_action_promotion.py), [직접 API 테스트](../../../services/core-control-plane/tests/core/executor/test_direct_api_executor.py) |
...
| 온톨로지 안전성 platform | 카탈로그에서 로드한 Interface 및 FunctionType 선언을 포함하는 exact 의미 release, release-aware 조회 profile 및 함수 등록, principal 범위로 한정된 매니페스트, 검증된 Resource와 ResourceType 분류, 범용/temporal 조회 algebra, canonical하게 중복 제거된 digest를 갖는 source-receipt-complete bitemporal 토폴로지/차이, 범위가 제한된 blast-radius 차이, exact-release로 게이트된 proposal-ready 상태, authoritative inventory rebuild pointer 및 서로 다른 검토자와 회귀 증적에 결속된 catalog PR 제안을 포함하는 immutable direction-generation shadow comparison, 검토된 메트릭 개념, topology-aware causal 결합, Resource별 최신성 메타데이터가 완전한 그래프 읽기, 다이제스트로 차단된 지속형 운영 모델 replay 복구, 요청(release, catalog revision, cutoff, target)에 결속되고 byte 상한(principal, authority 및 bundle identity 필드 포함)이 있으며 프로덕션 쓰기 권한이 없는 근거 기반 읽기 및 copy-on-write scenario branch, 별도의 planner-function 및 operational-plan lineage와 안전하게 닫히도록 문서화된 인자, 근거, 대상 및 효과 검증 계약이 있는 변경 계획, compact typed effect-reconciliation event, 인증된 독립 observer binding 및 lease-fenced 영속 terminal outbox 전달 | [ontology_platform](../../../services/core-control-plane/src/fdai/core/ontology_platform/) | [온톨로지 platform 테스트](../../../services/core-control-plane/tests/core/ontology_platform/) |
...
| Kubernetes 워크로드 근거 | 허용 목록에 있는 Deployment 및 Pod 관측, 정확한 대상 선택, rollout 및 Pod 소유자 방향에서 모두 발급된 2단계 소유권 근거, 검토된 불변 Pod 재시작 이력 메트릭, 근본 원인 또는 실행 권한을 주장하지 않는 최신성 및 충돌 인식 rollout, 같은 UID 복구 및 서로 다른 UID 교체 순수 축약기. 이제 영속 재개 가능 수명 주기 collector가 cluster/namespace/UID/reason 타입 근거를 atomic cursor-plus-append-only store 뒤에서 보존하고, retained 행을 기존 Resource-event query dependency와 공통 local/deployed analyzer schedule로 전달하며 정확한 old UID에 해당하는 종료 행만 lifecycle 기반 교체 축약기가 선택합니다. 누락된 과거 관측은 추론하지 않습니다. 확인된 서로 다른 UID 교체는 이제 현재 Pod 자체의 재시작 상태를 승격하거나 지우지 않고 전용 `replacement_recovery_verified` 레인으로만 노출되며, lifecycle 기반 predecessor 매칭은 정확한 대상 Pod id에 바인딩되어 같은 controller 아래의 sibling Pod가 predecessor로 잘못 귀속되지 않고, 과거 predecessor의 evidence_refs는 현재 Pod 자체의 근거가 아니라 오직 과거 신원/수명 주기 근거만 포함하며, `desired_replicas_before`는 현재(교체 이후) Deployment 스냅샷에서 조작되지 않고, 선언된 artifact digest도 교체 축약기 모듈을 해시합니다. 같은 UID 재시작은 더 이상 조작된 과거 신원을 요구하지 않고, server-owned 복구 계획은 manifest가 exact-Pod 진단 함수를 노출할 때마다 이를 dependency로 연결해 그 완전성을 복합 결과에 반영합니다. | [Kubernetes 인벤토리 source](../../../services/core-control-plane/src/fdai/delivery/kubernetes_api_inventory.py), [rollout 조회](../../../services/core-control-plane/src/fdai/core/ontology_platform/kubernetes_rollout_queries.py), [Pod 복구 조회](../../../services/core-control-plane/src/fdai/core/ontology_platform/kubernetes_pod_recovery_queries.py), [Pod 교체 축약기](../../../services/core-control-plane/src/fdai/core/ontology_platform/kubernetes_pod_replacement_evidence.py), [Pod 복구 플래너](../../../services/core-control-plane/src/fdai/core/conversation/semantic_kubernetes_pod_recovery_planning.py), [의미 플래너](../../../services/core-control-plane/src/fdai/core/conversation/), [durable Resource-event adapter](../../../services/core-control-plane/src/fdai/delivery/durable_kubernetes_resource_event_history.py), [수명 주기 observation 모델](../../../services/core-control-plane/src/fdai/core/ontology_platform/kubernetes_lifecycle_observation.py), [수명 주기 source](../../../services/core-control-plane/src/fdai/delivery/kubernetes_lifecycle_source.py), [수명 주기 collector](../../../services/core-control-plane/src/fdai/delivery/kubernetes_lifecycle_collector.py), [수명 주기 Postgres store](../../../services/core-control-plane/src/fdai/delivery/persistence/postgres_kubernetes_lifecycle.py), [수명 주기 Core migration](../../../service-migrations/branches/core-control-plane/versions/20260828_core_kubernetes_lifecycle.py), [수명 주기 collector CLI](../../../services/core-control-plane/src/fdai/delivery/kubernetes_lifecycle_collector_cli.py) | [인벤토리 source 테스트](../../../services/core-control-plane/tests/delivery/test_kubernetes_api_inventory.py), [워크로드 축약기 테스트](../../../services/core-control-plane/tests/core/ontology_platform/), [플래너 테스트](../../../services/core-control-plane/tests/conversation/), [조립 테스트](../../../services/core-control-plane/tests/composition/), [durable adapter 테스트](../../../services/core-control-plane/tests/delivery/test_durable_kubernetes_resource_event_history.py), [수명 주기 observation 테스트](../../../services/core-control-plane/tests/core/ontology_platform/test_kubernetes_lifecycle_observation.py), [수명 주기 source 테스트](../../../services/core-control-plane/tests/delivery/test_kubernetes_lifecycle_source.py), [수명 주기 collector 테스트](../../../services/core-control-plane/tests/delivery/test_kubernetes_lifecycle_collector.py), [수명 주기 Postgres store 테스트](../../../services/core-control-plane/tests/persistence/test_postgres_kubernetes_lifecycle.py), [수명 주기 CLI 테스트](../../../services/core-control-plane/tests/delivery/test_kubernetes_lifecycle_collector_cli.py) |
...
각 근거 경로의 provenance ref는 같은 secured object 자신의 속성에서 독립적으로 다시 계산한
값과 반드시 일치해야 하며, 그렇지 않으면 결속되지 않았거나 위조된 provenance ref로 거부됩니다.
...
보존합니다. 문서 worker는 native PDF를 리소스 상한이 있는 별도 프로세스에서 구문 분석합니다.
하나의 wall-clock deadline이 입력 전송과 전체 IPC frame 수신을 포함하며, 부모 프로세스가 page,
문자 및 출력 byte 상한을 독립적으로 강제하므로 신뢰할 수 없는 압축 해제나 자식 프로세스 출력이
장기 실행 서비스를 멈추거나 고갈시키지 못합니다.
