...
translation_source_sha: e60f43b0f586f0401ce5c88767b4f66d9117299b
...
| 영속 Rule 세대 종결 | 구현됨 | `core/rule_semantic_generation/ledger.py`, `rule_catalog/schema/rule_semantic_generation_events.py`, 집중 계약 및 ledger 검사(`15 passed`) | Core는 첫 최종 활성화 결과를 lease로 차단된 하나의 발행 대기 레코드에 원자적으로 연결합니다. Delivery 상태는 의미 인덱스, 정책, 승인, 변경 또는 실행 권한을 부여하지 않습니다. |
...
| Rule 의미 세대 종결 | 타입 기반 활성화 명령 및 최종 결과, 원자적 StateStore 결과/outbox 영속성, 중복 replay, revision compare-and-swap, lease 차단, 재시도 예약, 손상 거부 및 의미 인덱스 또는 실행 권한이 없는 broker 확인 기반 발행 상태 | [rule_semantic_generation](../../../services/core-control-plane/src/fdai/core/rule_semantic_generation/) | [Rule 의미 세대 테스트](../../../services/core-control-plane/tests/core/rule_semantic_generation/) |
