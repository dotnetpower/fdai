...
translation_source_sha: 2bf95a7f573979e2b08fdcb320d913ab28fb067d
...
| risk_gate | 통합 auto vs HIL vs deny 권위. 선언된 모든 ActionType precondition에 indexed deterministic evidence를 요구하며, evidence가 없거나 실패하면 사람 승인으로 라우팅합니다. Runtime은 event evidence, Thor의 durable active-run index, effective `ChangeWindow` record를 위한 bounded ontology read를 결합합니다. Indexed run이 없으면 conflict로 처리하고, 잘리거나 malformed인 state는 authority를 높일 수 없습니다. Ceiling override는 Rego를 렌더하므로 삽입되는 모든 field는 pattern으로 제한되고 escape됩니다. 요청이 정책을 쓸 수 있어서는 안 됩니다. | [src/fdai/core/risk_gate/](../../../src/fdai/core/risk_gate/) | [tests/core/risk_gate/](../../../tests/core/risk_gate/) | [decisioning/](../decisioning/) |
| impact_analysis | 정규화된 `Change` 하나에 대해 bounded affected set과 replay-stable `ChangeAssessment`를 계산합니다. Huginn은 causal Event와 owner topic에 동일한 evidence를 포함하고 Forseti는 결과를 Verdict와 DecisionCase evidence에 project합니다. Stale, truncated, unmapped, over-cap, conflicting, failed 또는 불완전한 planned-change evidence는 사람 검토를 요구하며 실행 authority를 부여하지 않습니다. | [src/fdai/core/impact_analysis/](../../../src/fdai/core/impact_analysis/) | [tests/core/impact_analysis/](../../../tests/core/impact_analysis/) | [operating-ontology-ko.md](operating-ontology-ko.md) |
...
| read_investigation | 검증된 typed current/activity resource query, observed-facet deterministic compilation, exact-resource follow-up, 30일 Activity Log history, bounded parallel evidence, RG-scoped subscription health, durable latency profile, owner-scoped direct/stream replay, honest cost usage, SSE heartbeat 및 stream-close cancellation ([설계](../interfaces/azure-read-investigations-ko.md)) | [src/fdai/core/read_investigation/](../../../src/fdai/core/read_investigation/), [inventory application capability](../../../src/fdai/delivery/operator_api/application/conversation/capabilities/inventory/), [inventory conversation projection](../../../src/fdai/delivery/operator_api/projections/conversation/inventory/), [resource_context.py](../../../src/fdai/delivery/operator_api/projections/conversation/resource_context.py) 및 [src/fdai/delivery/azure/read_investigation/](../../../src/fdai/delivery/azure/read_investigation/) | [tests/core/read_investigation/](../../../tests/core/read_investigation/), [tests/delivery/azure/read_investigation/](../../../tests/delivery/azure/read_investigation/), [test_chat_inventory_query.py](../../../tests/delivery/operator_api/test_chat_inventory_query.py), [test_chat_inventory_compiler.py](../../../tests/delivery/operator_api/test_chat_inventory_compiler.py) 및 focused Operator API/persistence test |
...
| conversation_progress | Deterministic `none` / `compact` / `timeline` / `detached` presentation 선택, Web/Slack/Teams용 ordered redacted activity snapshot, request 또는 identity payload를 저장하지 않는 bounded aggregate progress counter 및 latency | [conversation_channel.py](../../../src/fdai/shared/providers/conversation_channel.py), [channel_gateway.py](../../../src/fdai/core/conversation/channel_gateway.py), [publishers.py](../../../src/fdai/delivery/channels/publishers.py), [conversation_progress.py](../../../src/fdai/shared/telemetry/conversation_progress.py), [stream_metrics.py](../../../src/fdai/delivery/operator_api/projections/conversation/stream_metrics.py) | [test_channel_gateway.py](../../../tests/conversation/test_channel_gateway.py), [test_publishers_and_routes.py](../../../tests/delivery/channels/test_publishers_and_routes.py), [test_rich_contract.py](../../../tests/delivery/channels/test_rich_contract.py), [test_conversation_progress_metrics.py](../../../tests/shared/test_conversation_progress_metrics.py), [test_chat_stream_metrics.py](../../../tests/delivery/operator_api/test_chat_stream_metrics.py), focused Web recovery test |
...
| change_lineage | Canonical `ChangeRecord`와 `ChangeAssessment`부터 선택한 `DecisionCase` option, `Action`, `ResponseOutcome`까지 변경 불가능하고 replay-stable하게 연결합니다. Nested decision 및 resilience trace는 rationale, impact, rollback, verification, recovery를 보존합니다. Canonical identity serialization은 기존 public identity API를 보존하면서 aggregate validation과 분리됩니다. Deterministic learning projection은 candidate-only이며 별도로 sealed된 operational case를 요구하고 단독으로 reuse할 수 없습니다. 기존 GitHub 및 Azure DevOps `ChangeFeed` adapter는 동일한 lineage boundary를 통과합니다. Identity, target, digest, causal order 및 ambiguous score 불일치를 차단하며 execution 또는 promotion authority를 부여하지 않습니다. | [src/fdai/core/change_lineage/](../../../src/fdai/core/change_lineage/) | [tests/core/change_lineage/](../../../tests/core/change_lineage/) |
...
| operator | Operator-console coordinator ([surface](../interfaces/operator-console-ko.md), [module map](../interfaces/operator-console-module-map-ko.md), [progressive conversations](../interfaces/operator-console-progressive-conversations-ko.md), [narrator routing](../interfaces/narrator-routing-and-latency-ko.md)) | [src/fdai/core/operator/](../../../src/fdai/core/operator/) | (delivery/operator_api 통합) |
...
[`followup.py`](../../../src/fdai/delivery/operator_api/application/conversation/capabilities/inventory/followup.py)에
...
측정된 LLM usage와 server-owned analytical continuation은 [`chat_llm_usage.py`](../../../src/fdai/delivery/operator_api/application/conversation/capabilities/llm_usage.py)에 있습니다. Resolver는 `MeteringReader`를 읽고 terminal verification 이후에만 bounded `analysis_context`를 발급하며 browser history를 신뢰하지 않고 기간, grouping 또는 presentation refinement를 다시 실행합니다.
...
JSON 및 streamed turn lifecycle coordination은
[`turn_execution/`](../../../src/fdai/delivery/operator_api/application/conversation/turn_execution/)에 있습니다.
Typed service는 request preparation, planning, evidence, generation과 stream collection,
verification, persistence, metering 및 completion을 소유합니다. `chat.py`는 JSON authentication,
bounded parsing, status mapping, response delivery 및 factory binding을 유지합니다. `chat_stream.py`는
SSE authentication, bounded request transport, status mapping, framing, heartbeat, sequence,
revision, connection-close cancellation 및 response construction을 유지합니다.
...
| isolated_executor | Versioned shadow/effect receipt, stable-offset EventBus command/receipt transport, logical-target lock, durable idempotency, pre-effect/terminal audit, default-off authority cutover, independent effect probe, supervised health 및 internal Container App을 담당합니다. Live exact-plan, effect, rollback evidence는 남아 있습니다. | [src/fdai/runtime/isolated_executor.py](../../../src/fdai/runtime/isolated_executor.py), [src/fdai/runtime/isolated_executor_client.py](../../../src/fdai/runtime/isolated_executor_client.py), [src/fdai/runtime/isolated_executor_runtime.py](../../../src/fdai/runtime/isolated_executor_runtime.py), [src/fdai/runtime/isolated_executor_cli.py](../../../src/fdai/runtime/isolated_executor_cli.py), [src/fdai/runtime/executor_authority_probe_cli.py](../../../src/fdai/runtime/executor_authority_probe_cli.py) 및 [infra/modules/isolated-executor/container-app/](../../../infra/modules/isolated-executor/container-app/) | [tests/runtime/test_isolated_executor_shadow.py](../../../tests/runtime/test_isolated_executor_shadow.py), [tests/runtime/test_isolated_executor_lock.py](../../../tests/runtime/test_isolated_executor_lock.py), [tests/runtime/test_isolated_executor_runtime.py](../../../tests/runtime/test_isolated_executor_runtime.py), [tests/runtime/test_isolated_executor_cli.py](../../../tests/runtime/test_isolated_executor_cli.py), [tests/runtime/test_isolated_executor_effect.py](../../../tests/runtime/test_isolated_executor_effect.py), [tests/runtime/test_executor_authority_probe_cli.py](../../../tests/runtime/test_executor_authority_probe_cli.py), [tests/infra/test_isolated_executor_boundary.py](../../../tests/infra/test_isolated_executor_boundary.py) 및 [tests/contracts/test_executor_transport.py](../../../tests/contracts/test_executor_transport.py) |
| inventory_ontology | 승격된 inventory generation 하나에서 파생하는 single-writer provider 관측 resource subgraph입니다. 소유 revision을 pin하고 자기 subgraph를 원자적으로 교체하며, stale 삭제를 위해 소유 identity를 보존하고, 다른 projection이 소유한 object를 거부하며, 빈 결과와 미관측을 구별할 수 있도록 coverage를 보고합니다. Projection 실패가 승격된 snapshot을 무효화하지 않습니다. | [src/fdai/runtime/inventory_ontology.py](../../../src/fdai/runtime/inventory_ontology.py) 및 [inventory_projection.py](../../../src/fdai/core/ontology_platform/inventory_projection.py) | [tests/runtime/test_inventory_ontology.py](../../../tests/runtime/test_inventory_ontology.py) 및 [test_inventory_projection.py](../../../tests/core/ontology_platform/test_inventory_projection.py) |
...
| assurance_twin | Scalar 및 graph active/challenger effect model, 필수 active-trajectory invariant, durable StateStore trajectory episode, independent off-path closure, challenger-only update 및 shadow audit을 포함한 read-only ontology twin (실행 또는 promotion 금지) | [src/fdai/core/assurance_twin/](../../../src/fdai/core/assurance_twin/) | [tests/assurance_twin/](../../../tests/assurance_twin/) |
...
| workflow | Version-pinned WorkflowDefinition을 컴파일 및 실행합니다. Durable approval decision은 revision CAS로 정확한 role, normalized principal, attempt를 bind하고 projection 중단을 견디며 sibling slot closure를 복구하고 expired slot을 숨긴 뒤 Process journal에 반영됩니다. Approval expiry는 late quorum보다 우선하고 timely quorum은 delayed resume에서도 유효하며 timeout은 최신 revision CAS에서 이긴 경우에만 terminal이 되고 terminal approval state는 단조롭게 유지됩니다. `workflow_resume.py`는 audit-safe input을 재구성합니다. `workflow_cancellation.py`는 safe boundary에서만 중단하고 applied work를 compensation으로 넘깁니다. `workflow_retry.py`는 allowlist에 포함된 effect-free `failed` attempt를 수락하고 `timed_out`은 terminal approval timeout인 경우에만 server cap 아래 수락합니다. Action proposal 및 Process step identity는 positive attempt를 포함하므로 서로 다른 attempt가 deduplicate되지 않습니다. Contextual guard는 정확한 target과 time에 reviewed ChangeWindow evidence를 resolve합니다. Enforce action step은 independently verified outcome receipt를 기다리고 partial failure는 reverse typed compensation 전에 intent와 original params를 기록합니다. Recovery-incomplete 종료는 durable target hold를 기록합니다. RiskGate는 ordinary action을 deny하고 정확한 Process-bound compensation만 사람 승인을 통해 허용합니다. 모든 compensation receipt 검증과 CAS hold release가 끝나야 `compensated`가 됩니다. | [src/fdai/core/workflow/](../../../src/fdai/core/workflow/) | [tests/core/workflow/](../../../tests/core/workflow/) |
| scheduler | Create/pause/resume/edit/run-now/cancel lifecycle, cron dispatch, run history, inert configuration-review handoff를 포함한 blueprint, idempotent lifecycle audit 및 CAS winner만 expiry audit을 기록하는 범위 제한 continuation ([설계](../interfaces/scheduled-result-continuations-ko.md)) | [src/fdai/core/scheduler/](../../../src/fdai/core/scheduler/) | [tests/core/scheduler/](../../../tests/core/scheduler/) |
...
| operator_api | 콘솔 read-only HTTP 서피스, provider-neutral request-local backend routing, concrete Azure 및 OpenAI-compatible narrator adapter, production optional-service builder, application-owned chat request preparation, route-owned bounded HTTP parsing, cursor-paged conversation summary, principal 범위 knowledge-context 조립, projection-owned terminal payload와 bounded timing, trajectory-detail replay, application-owned terminal verification, background, busy-input, skill, read-investigation helper | [src/fdai/delivery/operator_api/](../../../src/fdai/delivery/operator_api/), [application/conversation/backend/](../../../src/fdai/delivery/operator_api/application/conversation/backend/), [adapters/conversation/](../../../src/fdai/delivery/operator_api/adapters/conversation/), [application/conversation/request_preparation/](../../../src/fdai/delivery/operator_api/application/conversation/request_preparation/), [application/conversation/verification/](../../../src/fdai/delivery/operator_api/application/conversation/verification/), [user_context_conversations.py](../../../src/fdai/delivery/operator_api/routes/user_context_conversations.py), [production/knowledge_context.py](../../../src/fdai/delivery/operator_api/production/knowledge_context.py), [production/python_tasks.py](../../../src/fdai/delivery/operator_api/production/python_tasks.py), [chat_knowledge_context.py](../../../src/fdai/delivery/operator_api/application/conversation/capabilities/knowledge_context.py), [chat_stream_request.py](../../../src/fdai/delivery/operator_api/routes/chat_stream_request.py), [projections/conversation/terminal/](../../../src/fdai/delivery/operator_api/projections/conversation/terminal/), [trajectory.py](../../../src/fdai/delivery/operator_api/projections/conversation/trajectory.py), [chat_vision_prompt.py](../../../src/fdai/delivery/operator_api/application/conversation/vision_prompt.py), [read_investigation_payload.py](../../../src/fdai/delivery/operator_api/routes/read_investigation_payload.py), [read_investigation_execution.py](../../../src/fdai/delivery/operator_api/routes/read_investigation_execution.py) |
...
| 패키지 | 목적 | 소스 |
|--------|------|------|
| contracts | Versioned isolated-Executor command와 applied effect를 주장할 수 없는 terminal shadow receipt를 포함한 크로스 패키지 Pydantic 계약 | [src/fdai/shared/contracts/](../../../src/fdai/shared/contracts/) |
