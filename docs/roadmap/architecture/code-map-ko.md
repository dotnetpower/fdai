...
| change_lineage | Canonical `ChangeRecord`와 `ChangeAssessment`부터 선택한 `DecisionCase` option, `Action`, `ResponseOutcome`까지 변경 불가능하고 replay-stable하게 연결합니다. Nested decision trace는 objective score, constraint, effect, approval requirement, reasoning receipt를 보존하고 resilience trace는 execution mode, impact scope, rollback contract, verification timing, recovery result를 보존합니다. Record 간 identity, target, digest, causal order 및 ambiguous score 불일치를 차단하며 execution 또는 promotion authority를 부여하지 않습니다. | [src/fdai/core/change_lineage/](../../../src/fdai/core/change_lineage/) | [tests/core/change_lineage/](../../../tests/core/change_lineage/) |
