...
translation_source_sha: 81d66b029d78d1e92c9070f6e8b632b59f3d498a
...
| workflow | Version-pinned WorkflowDefinition을 컴파일 및 실행합니다. Durable approval decision은 revision CAS로 정확한 role, normalized principal, attempt를 bind하고 projection 중단을 견디며 rejection 때 sibling slot을 닫고 Process journal에 반영됩니다. `workflow_resume.py`는 audit-safe input을 재구성합니다. `workflow_cancellation.py`는 safe boundary에서만 중단하고 applied work를 compensation으로 넘깁니다. `workflow_retry.py`는 server cap 아래 effect-free failure와 terminal approval reject 또는 timeout을 수락합니다. Action proposal 및 Process step identity는 positive attempt를 포함하므로 서로 다른 attempt가 deduplicate되지 않습니다. Contextual guard는 정확한 target과 time에 reviewed ChangeWindow evidence를 resolve합니다. Enforce action step은 independently verified outcome receipt를 기다리고 partial failure는 reverse typed compensation 전에 intent와 original params를 기록합니다. Recovery-incomplete 종료는 durable target hold를 기록합니다. RiskGate는 ordinary action을 deny하고 정확한 Process-bound compensation만 사람 승인을 통해 허용합니다. 모든 compensation receipt 검증과 CAS hold release가 끝나야 `compensated`가 됩니다. | [src/fdai/core/workflow/](../../../src/fdai/core/workflow/) | [tests/core/workflow/](../../../tests/core/workflow/) |
