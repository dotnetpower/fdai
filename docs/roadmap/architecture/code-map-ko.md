...
translation_source_sha: 181a419d2ab2d83c2098a07eb5ef7561d2ac6bad
...
| MSCP 응답 결과 변환과 고정 전체 루프 종결 | 리소스 참조 대신 대상 다이제스트를 저장하고, 계약이 표현할 수 없는 관측을 발송 도중 오류로 만들지 않고 `unscorable`로 낮추며, 실행이나 promote 권한을 갖지 않는 strict shadow `ResponseOutcome` 변환. 제품에 포함된 shadow executor 위에서 실제 제어 루프를 실행하고 독립적이고 권위 있는 효과 관측에서만 종결하는 고정 SRE 전체 루프 재생이 이를 검증합니다 | [응답 결과 변환](../../../services/core-control-plane/src/fdai/core/mscp_profile/response_outcome.py)과 [효과 검증](../../../services/core-control-plane/src/fdai/core/mscp_profile/effect_verification.py) | [변환 테스트](../../../services/core-control-plane/tests/core/mscp_profile/test_response_outcome.py)와 [고정 전체 루프 재생](../../../services/core-control-plane/tests/scenarios/test_v2026_07_replay.py) |
