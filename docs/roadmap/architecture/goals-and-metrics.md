...
|------|-------|--------|----------|-----------|
| 2026-08-31 | in-progress | Registered the cohort claim as a decision-critical positive boundary and closed the shared preflight gaps found during final review. The complete-inventory guard now rejects nonconstant short-circuit decoys, workflow gates accept only the literal boolean `true`, and effect observations timestamped before dispatch completion are held instead of closing an action. | `current change`; decision-boundary inventory and checker, cohort claim evaluator, workflow gate resolver, MSCP effect admission, and focused boundary, workflow, and effect tests. | Retain the governed non-synthetic dual cohort and its independent admissions in deployed runtime. |
