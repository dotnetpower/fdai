...
| Independent decision-evidence verification | implemented | `decision_evidence_verification.py`; `core/readiness/decision_evidence.py`; `shared/providers/decision_evidence_verifier.py`; `delivery/azure/decision_evidence.py`; focused contract, readiness, and Azure adapter tests | Five content-addressed proofs cover authentication, evidence, completeness, conflict, and freshness policy. Core selects a current non-revoked verifier binding, blocks producer self-verification and proof mismatch, and accepts a bundle only inside its validity window. The Azure adapter uses a short-lived Managed Identity token for authoritative readback without retaining the credential. Runtime composition and migration of existing decision boundaries remain open. |
...
- [ ] Bind the independent verifier registry in runtime composition, adopt `DecisionCriticalEvidenceReceipt` and `DecisionEvidenceVerificationBundle` at every readiness, query, state, promotion, and qualification decision boundary, then prove synthetic or incomplete evidence cannot satisfy a live claim.
