...
translation_source_sha: 68a2da7ffa642917b63c192b4bd8a5eda22456bd
...
| Versioned 관계 후보 materialization | implemented | `provider_schema_relationship_generation.py`, `provider_schema_relationship_ledger.py`, `test_provider_schema_relationship_generation.py`, `direction_shadow` exact-release comparator | Exact provider-schema 및 REST evidence digest, mapping revision, projection manifest, 명시적 direction/cardinality/link metadata, 변경 타입 무효화, bounded candidate, rollback 및 replay를 content-address합니다. Active pointer는 generation digest를 검증하며 graph 또는 migration 권한으로 변조될 수 없습니다. Partial, stale, duplicate 또는 mixed-release 입력은 비활성 상태로 유지되며 promotion은 graph 및 migration 권한을 false로 고정한 proposal-only입니다. |
