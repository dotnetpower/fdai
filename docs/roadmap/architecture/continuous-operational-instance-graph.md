...
| Ontology projection | implemented | `InventoryOntologyProjector.apply` is the single writer for the inventory-owned Resource and Link subgraph. Journal and projection watermarks plus pending tombstones independently lower source completeness. |
...
| Durable normalized observation history | implemented | `inventory_observation.py`, `operational_history_lifecycle.py`, `postgres_inventory_observation*.py`, `postgres_observation_lifecycle.py`, Core migrations through `20260906_core_operational_history_lifecycle.py`, typed replay and source-coverage checks | OI-13 and OI-14 retain exact object and relationship observations, incarnation identity, late correction partitions, deterministic correction closure, and case or legal-hold pins. Pending corrections lower source completeness, and a corrected base partition requires a newer checkpoint before purge. |
| Operational archive and bounded-history purge | in-progress | `operational_history_archive.py`, Azure Blob artifact adapter, PostgreSQL lifecycle store and database purge gate, deployment policy loader, lifecycle planner and fixed schedule, OI-16 certification types and CLI, focused checks, protected apply receipt, and successful shadow Job execution | OI-15 is deployed in shadow mode with private versioned storage under the non-executor inventory identity. OI-16 remains open because bounded growth, archive restore, safe purge, schema replay, database recovery/restart, holds, and false-complete failure scenarios do not yet have one complete pinned operational receipt. |
...
- [ ] `OI-16` retains one pinned-revision operational receipt proving bounded steady-state storage,
  ...
  provider-failure, database-restart, and archive-outage scenarios. The typed local certification
  and private receipt writer are implemented, and they cannot report `operationally_validated=true`
  without a protected deployment receipt. That protected receipt remains open.
