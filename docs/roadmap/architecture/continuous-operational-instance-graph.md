...
boundaries remain later delivery work.
...
| Durable normalized observation history | implemented | `inventory_observation.py`, `postgres_inventory_observation*.py`, Core migration `20260905_core_inventory_observation_journal.py`, typed replay and source-coverage checks | OI-13 dual-writes versioned full, partial, change-hint, and tombstone object or relationship observations in shadow mode. Content-bound replay is monotonic and preserves unobserved properties. OI-14 incarnation and correction semantics and OI-15 retention partitions remain open. |
...
- [ ] `OI-14` completes resource incarnation identity, relationship correction coverage, late
