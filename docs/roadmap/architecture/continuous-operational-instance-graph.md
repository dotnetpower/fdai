...
Publication also commits a content-addressed receipt with actual object revisions. Storage-level pages pin its digest across later publication and fetch only selected graph chunks; receipt and manifest metadata remain bounded, not constant-size. Missing content after a database rebuild fails closed without live-row fallback. These owner-subgraph reads exclude foreign endpoint contents and retain caller authorization requirements; gateway integration, ownership epochs, retention, and short pointer publication remain open. Existing writable tables and N-1 UPSERT contracts are unchanged; partial candidates never replace the complete ownership set.
