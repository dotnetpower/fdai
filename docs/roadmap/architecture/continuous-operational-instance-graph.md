...
| Operational archive and bounded-history purge | in-progress | `operational_history_archive.py`, Azure Blob artifact adapter, PostgreSQL lifecycle store and database purge gate, deployment policy loader, lifecycle planner and fixed schedule, OI-16 certification types and CLI, and focused checks | Writer, verified principal-scoped reader, exact source purger, policy, checkpoint, pressure, schema replay, recovery, and certification contracts exist. The dedicated scheduled runtime composition and protected pinned-revision deployment receipt remain open, so OI-15 and OI-16 are not complete. |
