...
Incident-creation regressions are claimed exactly once by the Core or Operator service suite. The generated question-bank and semantic-intent coverage artifacts are deterministic derivatives, so their reviewed source changes carry design impact instead of regenerated provenance digests requiring duplicate owner updates.
...
| Log ingestion | `LogQueryProvider` | **CSP-neutrality contract** - [logs](csp-neutrality.md#7-log-query-contract--structured-log-records) | `NoopLogQueryProvider`; the configured Azure adapter resolves an event-time inventory identity, discovers at most three Diagnostic Settings or workspace-based Application Insights destinations, appends one explicit static fallback, and runs exact-scope KQL. RCA passes only bounded fact tokens and opaque citations from this telemetry leg to the model. Its exact source path is registered in the semantic-routing audit as typed `LogRecord` evidence reduction, never operator-language intent or authority. | Loki, Elasticsearch, CloudWatch Logs, or another structured log adapter |
