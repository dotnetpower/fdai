...
| **Action precondition evidence** | `PreconditionEvaluator` in `core/risk_gate/preconditions.py`; indexed `PreconditionEvaluation` records consumed by RiskGate | - | `GovernedPreconditionEvaluator` combines canonical event evidence with bounded `ChangeWindow` ontology queries. `OntologyOpenActionEvidenceProvider` is available for an authoritative `ActionRun` projection but stays unbound until that projection exists. Missing providers leave conditions unresolved, truncated action results conflict, and truncated or malformed windows stay inactive. | replace the read-only ontology projection while preserving every condition index and the rule that evidence can only preserve or lower authority |
