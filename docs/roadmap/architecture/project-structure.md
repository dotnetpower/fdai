...
- **operational ownership draft delivery stays review-only**: Runtime composition and protected Core deployment preserve the GitOps, merge-effect, identity-health, and knowledge-lifecycle boundaries defined by [Agent operational ownership lifecycle](../interfaces/agent-stewardship-operations.md). The exact-effect IAM reader and replacement planner remain provider-neutral; dedicated delivery adapters verify target-bound membership and inverse rollback receipts. Catalog timing and current-role HIL readers bind at runtime without enabling escalation or IAM enforcement.
  Operator goal commands revalidate ownership and bind the full retry payload; Core reconciliation rotates bounded pages, retains a failed page for retry, and uses durable case/revision audit claims without invoking a provider. Assignment notices resolve an insert-only Operator receipt through a Core read-only adapter; database guards reserve `human_assignment:` writes for Core. Huginn, Forseti, Var, Saga, and Muninn exchange typed owner events before the existing review-only artifact delivery opens a PR. Neither transport nor the Operator's Core-effect projection can grant IAM authority.
