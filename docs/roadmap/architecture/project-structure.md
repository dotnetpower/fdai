...
| Log ingestion | `LogQueryProvider` | **CSP-neutrality contract** - [logs](csp-neutrality.md#7-log-query-contract--structured-log-records) | `NoopLogQueryProvider`; the configured Azure adapter resolves an event-time inventory identity, discovers at most three Diagnostic Settings or workspace-based Application Insights destinations, appends one explicit static fallback, and runs exact-scope KQL. RCA passes only bounded fact tokens and opaque citations from this telemetry leg to the model. | Loki, Elasticsearch, CloudWatch Logs, or another structured log adapter |
