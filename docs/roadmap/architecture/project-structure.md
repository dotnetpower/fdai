...
[Alert noise governance](../operations/alert-noise-governance.md) owns source, period, signature, Process and effect contracts. Its codecs share no service implementations; grants stay Core-only journal `USAGE`, and generated/UI/offline evidence grants no authority or certification. A no-op migration joins alert and human-access heads without rewriting either applied history. Installed-wheel checks distinguish the three alert codecs from the seven historically transition-certified edges. Runtime support lists all four actions as conditional, not operationally ready.
