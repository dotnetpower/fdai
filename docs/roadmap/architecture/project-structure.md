...
│   ├── runtime/               # headless lifecycle and composition, including versioned isolated Executor shadow/effect handling, stable-offset remote client, EventBus/DLQ/health supervision, production entry point, reversible authority probe, operating-model and diagnostic-catalog startup projection/status, durable T2 recovery observation/backfill, lease-based reconciliation outbox proposal routing, StateStore-backed proposer route selection with Thor/Vidar execution and rollback, transport/identity bindings, startup readiness, worker gating, and post-turn review wiring into Norns
...
| **Network query receipt verification** | `NetworkQueryReceiptVerifier` in `services/core-control-plane/src/fdai/core/ontology_platform/network_path.py` plus one opaque composition-owned verification context | - | `build_network_path_runtime` shares a bounded process-local HMAC authority between `SecuredObjectSetQueryGateway` and the exact-release function registry; restart invalidates seals | inject another issuer-backed verifier only when it preserves receipt role, singleton purpose, exact ontology release, projected-result digest, `FunctionInvocationContext`, no executor identity, and fail-closed restart behavior |
