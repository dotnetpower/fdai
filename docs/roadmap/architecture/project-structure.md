...
│   ├── runtime/               # headless lifecycle and composition, including the no-effect `isolated_executor.py` durable handler, `isolated_executor_lock.py` ResourceLock wrapper, and `isolated_executor_runtime.py` EventBus/DLQ/health supervisor, plus operating-model and diagnostic-catalog startup projection/status, durable T2 recovery observation/backfill, StateStore-backed proposer route selection with Thor/Vidar execution and rollback, transport/identity bindings, startup readiness, worker gating, and post-turn review wiring into Norns
