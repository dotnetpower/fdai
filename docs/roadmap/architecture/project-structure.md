...
| Rule / policy source | rule-catalog + `policies/` loader, `RuleIndex`, and `CatalogIndexLifecycle` | - | bundled generic rules with locked atomic current/N-1 dispatch indexes; conflicting retained versions are rejected | customer rule set / thresholds |
