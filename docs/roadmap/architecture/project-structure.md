...
`object.issue` idempotency key before fingerprint learning, so broker replay cannot inflate evidence. HIL resume resolves catalog rules from the current catalog and accepts a parked server-validated operator-request rule
