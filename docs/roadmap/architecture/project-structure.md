...
Cloud update comparison validates raw and processing identities; Core rechecks provenance and the existing excerpt budget. Document-query schema upgrades also require the exact catalog-owned prompt layer, never capability presence alone. Post-merge System Knowledge metadata binds the now-reachable protected base without changing record payloads or cloud source-check times.
...
| Bounded worker planning | `TaskWorkerPlanningProvider` in `core/task_worker/planning_executor.py` | Forward token and cost ceilings before dispatch; return measured usage even for abstention. Unmetered providers fail executor construction. | Unbound; #805 still requires a production provider and runtime composition | Inject a read-only metered provider with proven pre-dispatch limits and failure accounting |
