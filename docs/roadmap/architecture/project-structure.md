...
- **semantic target resolution is deterministic**: Core removes resource-identity clarification only
  ...
  bounded clarification for subtype-only exact-target operations. A sole typed
  `resource_identity` gap renders as a locale-specific request for the exact resource name or ID;
