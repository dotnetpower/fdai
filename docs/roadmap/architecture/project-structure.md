...
| **T1 incident members** | `IncidentMemberSource` plus `Container.resource_dependency_graph` | Read-only RCA context; neither input grants action authority | unbound, so T1 causal-chain analysis remains unavailable | inject `DeploymentHistoryMemberSource` and a reviewed graph whose opaque resource ids match incident evidence |
