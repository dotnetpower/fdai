...
| Semantic frame construction boundary | implemented | `core/conversation/semantic_planning_frame.py`; focused planner, tier-routing, investigation, and Azure adapter checks (`111 passed`) | A focused Core module binds candidate frames to exact input and investigation identities and resolves only server-owned clarification context. It performs no provider I/O and grants no decision, approval, mutation, or execution authority. |
