...
judgment, recovery, publication, or execution authority. Saga checkpoints a successful issue mutation before audit and publication, so
retry resumes incomplete evidence stages without duplicating the external comment. The `verticals.resilience` package exposes deterministic recovery-plan compilation without adding execution authority. DR objective evidence reports a nearest-rank p90, so a small cohort keeps its slowest measured run instead of reporting an
...
through either `remediates` or `alternatives`. A T2 proposal also cannot bypass grounding authority when a provider fails. HIL approval ids
and executor idempotency keys are claimed atomically, and Var preserves the source ActionRun idempotency key in its final approval. Per-resource locking serializes competing applies before any delivery adapter
