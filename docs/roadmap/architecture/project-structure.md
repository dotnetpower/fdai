...
| Deterministic subscription conversation boundary | implemented | `fdai_service_contracts/semantic_turn.py`; `fdai_core_service/semantic_turn_consumer.py`; `fdai_operator_service/families/conversation/subscription_scope.py`; focused contract, Core, and Operator checks | The Operator owns the catalog-matched current-subscription terminal route and ARM evidence adapter. Core retains semantic wire processing with session-keyed partition ordering, and production composition grants neither route model fallback nor execution authority. |
