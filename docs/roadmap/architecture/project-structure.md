...
an operation-aware issue adapter, so retry or restart resumes validated checkpoints without duplicating the external comment. The
...
| **Saga issue handoff** | `IssueTrackerAdapter` plus additive `IdempotentIssueTrackerAdapter` from the `fdai.agents` facade; runtime `StateStore` journal | - | `InMemoryGithubIssueAdapter` supports direct and operation-bound mutation; runtime composition injects the same durable store used by Saga audit | implement `create_or_comment_once` with an atomic provider-side operation id/content binding. Legacy adapters remain usable for direct escalation, while typed handoffs fail closed without the additive seam |
