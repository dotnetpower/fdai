...
Restart resumes incomplete application or rebuilds undelivered work, so broker replay cannot inflate or erase evidence. HIL resume resolves catalog rules from the current catalog and accepts a parked server-validated operator-request rule
