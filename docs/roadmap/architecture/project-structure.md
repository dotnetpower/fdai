...
Upstream defines generic interfaces and working defaults. Forks customize through dependency injection at their own composition root, without editing or copying `core/`; see [the fork model](../../../.github/instructions/generic-scope.instructions.md). [Alert noise governance](../operations/alert-noise-governance.md) uses these seams for bounded readers, independently admitted evidence, existing Workflow/Process orchestration, GitOps delivery, source fencing, and effect relays. Its opt-in factories supply mechanics, not independent authority or recipient/effect receipts; operational adoption remains gated.
