...
| **Recovery attempt and dispatch** | `recovery_attempt.py` in `core/workflow/`; `recovery_effect_claim.py` in `core/workflow/`; `recovery_terminalization.py` in `core/workflow/`; `hold_dispatch_fence.py` in `core/executor/` | - | durable recovery attempt identity binding, exclusive pre-dispatch claim, separate approval/safeguard evidence, authoritative effect completion claims with identity separation and supersession, crash-safe terminal Process/Saga outbox, and hold-aware dispatch fencing inside the logical-target lock | new modules consume existing automation-hold and recovery-admission contracts; no new execution authority or provider bindings are introduced. See [#652, #656, #658, #640](../decisioning/process-automation.md). |
