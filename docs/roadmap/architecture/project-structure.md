...
rehydrates the current projection before consumers. Runtime composition injects the same store into Norns; Norns claims each
...
Before consumers start, bounded runtime recovery restores pending operations, counts, and candidates; typed, startup, and public batch
flushes share one durable delivery-completion boundary. Broker replay therefore cannot inflate, erase, or strand evidence. HIL resume resolves catalog rules from the current catalog and accepts a parked server-validated operator-request rule
