...
Incident-creation regressions are claimed exactly once by the Core or Operator service suite. The generated question-bank JSON and review catalog are deterministic derivatives, so their reviewed source changes carry design impact instead of regenerated provenance digests requiring duplicate owner updates.
