...
| **Recovery attempt and dispatch** | `recovery_attempt.py` in `core/workflow/`; `recovery_effect_claim.py` in `core/workflow/`; `recovery_terminalization.py` in `core/workflow/`; `recovery_coordinator.py` in `core/workflow/`; `hold_dispatch_fence.py` in `core/executor/`; `AutomationHoldStateReader` and `HoldReleaseAuthorizationReader` in `shared/providers/automation_hold_state.py`; `workflow_recovery.py` in `delivery/persistence/` | - | durable recovery attempt identity binding, one exclusive compare-and-set pre-dispatch claim, separate approval/safeguard evidence, authoritative effect completion claims with identity separation and supersession, crash-safe terminal Process/Saga outbox, and authorization-bound dispatch fencing inside the logical-target lock. The runtime binds the recovery approval journal, the finalized safeguard bundle retention wrapped around the workflow outcome recorder, and the independent effect observation journal | new modules consume existing automation-hold, approval-journal, and recovery-admission contracts; no new execution authority or provider bindings are introduced. A fork may replace each reader or writer seam but cannot let persistence grant approval or effect-verification authority. See [#652, #656, #658, #640](../decisioning/process-automation.md). |
