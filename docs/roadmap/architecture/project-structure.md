...
| **Browser evidence** | `BrowserEvidenceProvider`, origin policy, capture request, artifact store, and custody sink in `shared/providers/browser_evidence.py`; policy and services in `core/browser_evidence/`; v1 compatibility and v2 workspace DTOs in `fdai-service-contracts` | - | unbound by default; optional isolated Playwright delivery adapter, PostgreSQL artifacts, append-only custody, evidence workflow step, compatible GET-only metadata, and a drift-aware payload-free investigation workspace | bind exact server-owned policies and a restricted-egress runtime without executor identity; content stays untrusted and shadow-only, admitted identity stays separate from non-identifying withheld aggregates, and Console filtering never becomes capture or action authority ([design](../interfaces/browser-evidence.md)) |
