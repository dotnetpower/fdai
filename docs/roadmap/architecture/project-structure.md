...
| **Runtime-call evidence projection** | `RuntimeCallObservation` and pure `project_runtime_call(...)` in `services/core-control-plane/src/fdai/core/ontology_platform/runtime_call_projection.py` | - | unbound; exact, fresh, principal-readable endpoints can produce only an authority-free candidate link, never a persisted graph write | bind an authenticated typed telemetry producer and the inventory single writer while preserving exact-release, active-generation, scope, freshness, independent-verifier, and no-authority checks |
