...
- **allowed imports**: `shared/` imports nothing from `core/`; `core/` may import only
  ...
  The transport-only Core Executor client injects the shared-provider `ExecutorReceiptJournal`; runtime owns durable attempt/closure joins. Dispatch receipts grant no authorization or independent effect verification.
