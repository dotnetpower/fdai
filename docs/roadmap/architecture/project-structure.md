...
[Alert noise governance](../operations/alert-noise-governance.md) owns its typed evidence, Process and conditional manual-PR boundaries. A dedicated Operator request composition binds projections, preferences, history and one supervised bridge; the shared root owns lifecycle only. Codecs share no service implementation, and generated/offline evidence grants no authority or operational certification. Catalog tests use current execution-path contracts, not the rejected historical R7 proposal.
