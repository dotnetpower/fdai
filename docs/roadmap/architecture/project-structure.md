...
| Evaluation and semantic-regression boundary | in-progress | `evaluation-sdk/`; `benchmarks/`; `eval/golden-dataset/`; `tests/integration/evaluation/test_golden_dataset.py` (`4 passed`) | The external host integration is dormant and absent from Core. The retained packages remain independent workspace members. The new 20-pair English/Korean corpus validates schemas, catalog identities, relationship direction, depth, evidence, and read-only authority; campaign execution against answer results remains open. |
