...
[Alert noise governance](../operations/alert-noise-governance.md) owns typed evidence, Process and conditional manual PRs. Dedicated Operator composition binds request dependencies and one supervised bridge; the shared root owns lifecycle only. Role-local framework mixins own alert callbacks, preserving public agent APIs and instance isolation. Shared admission inventory pins decision guards. Catalog tests follow current contracts, not rejected R7; generated/offline evidence grants no authority or certification.
