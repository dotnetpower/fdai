...
│   │   ├── conversation/       # operator-console intent/tool coordination, bounded read plans, role-scoped clarification, dynamic grounded narration with exact-ref fallback, safe attachments, per-turn isolation, durable delivery, adapter health, and busy-input arbitration
