...
Targetless recent Resource changes use a separate server-scoped FunctionType backed by the
append-only observation journal. Composition injects the PostgreSQL reader and exact event-id
ingestion fence; Core keeps provider changes distinct from operational-state transitions.
