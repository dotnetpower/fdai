...
| Resource-discovery contract boundary | implemented | `fdai_service_contracts/discovery.py`; `fdai_service_contracts/discovery_evidence.py`; `core/discovery/router.py`; focused discovery checks (`40 passed`) | The shared SDK owns immutable no-authority wire records, Core owns provider-neutral exact-equivalent routing and merge, and Azure delivery owns version-pinned profiles, rendering, execution receipts, and coverage reconciliation. |
