...
│   │   ├── ontology_platform/   # exact releases, semantic interfaces, bounded object sets, secured purpose/ACL query receipts, shared exact-number property semantics, immutable diagnostic ledger/result projection, mutation plans, typed functions, authenticated reconciliation with proposal-only terminal outbox, and proposal-only SDK generation
