...
- **Development tag-remediation binding**: `remediate.tag-add@1.1.0` resolves a bounded logical
  Resource id through the shared executor-target contract. Core and the isolated Executor route it
  to the development operations gateway only when that gateway, the change identity, and the
  ordinary authorization and promotion boundaries are present. The gateway owns tag-only mutation,
  snapshot rollback, and reader-identity readback. Runtime wiring supplies the same readback as the
  MSCP independent effect observer without changing the promotion registry or Console authority.
