...
- **Independent effect observation**: the durable kinetic artifact store is the exact-plan source.
  `StateStoreExecutedActionObservationStore` accepts verifier-approved Heimdall observations; the
  unbound `StateStoreShadowReversionWriter` requires current approval and reverts one ActionType only.
