...
- **human approval stays split by service authority**: Operator owns Teams/Slack authentication,
  ...
  but never imports Core implementation or receives executor identity. **Document OCR stays split by contract and provider ownership**: the shared service-contract SDK owns the revisioned provider policy without deployment authority, the document worker owns the bounded local Tesseract adapter and Azure adapter selection, and infrastructure supplies only the selected endpoint, identity, and provider value, so neither ingestion service imports another service implementation. Service migration CI runs legacy downgrade checks before adoption, serializes schema-mutating lifecycle tests, and runs remaining integration tests against the adopted schema. Local composite actions are repository code and are exempt from remote action SHA syntax; every remote action remains pinned.
