...
승인과 운영 배포 또는 enforce 승인 범위를 분리하고, 모든 주장을 저장소 산출물
...
>
> **Customer 경계:** Upstream은 재사용 가능한 아키텍처와 근거 계약을 정의합니다.
> 포크는 환경 값, 책임자, privacy 결정, 서비스 목표, 운영 근거를 제공합니다.

## 한눈에 보는 설계

FDAI는 non-privileged 콘솔과 GitOps/ChatOps 전달을 사용하는 agent-driven headless 컨트롤
plane입니다. 고정된 15개 에이전트가 타입이 지정된 pub/sub를 통해 sensing, judgment, arbitration,
...
- Headless 컨트롤 plane의 Azure 구현과 프로바이더 경계.
...
- Upstream에서 소유자 및 근거 연결이 비어 있으므로 운영 승인.
...
| 상태 | 설명 | 근거 상태 |
|------|------|---------------|
| 현재 upstream | 재사용 코드, Terraform 모듈, 테스트, 범용 구성, 설계 문서이며 customer 운영 값은 포함하지 않습니다. | 이 저장소에서 검증할 수 있습니다. |
...
| 운영 대상 | Signed 이미지, 비공개 또는 allow-listed 데이터 흐름, 한계 소유자, 승인 objective, 차단 release 컨트롤, operational-readiness 보고 | 매니페스트 운영 게이트가 통과할 때까지 차단된입니다. |
...
| 직무 분리 | 요청자, 승인자, judge, 실행기를 별도 principal로 유지 | RBAC 구성과 HIL 테스트 |
| 재시도 안전성 | 고정된 멱등성 키와 리소스별 직렬화 | 멱등성과 재생 테스트 |
| 복구 가능성 | ActionType별 롤백 계약과 stop 조건 | Rollback 예행 연습 근거 |
| Customer 격리 | Fork-supplied 값과 의존성 injection | Generic-scope 게이트와 구성 검증 |
...
배포에 따라 달라지는 대상은 upstream universal constant가 아닙니다. 운영 배포는 승인
값, 측정 방법, 결과, 시각, 승인자를 근거 연결에 기록합니다.

| 영역 | 필수 운영 근거 | 통과 조건 |
|------|--------------------------|-----------|
| 가용성 | Control-plane SLO와 오류 예산 | 승인 objective와 측정된 staging 결과 |
| 지연 시간 | Tier별 p50/p95/p99와 종단 간 canary | 포크 승인 예산 이내 |
| 용량 | 지속/burst 이벤트 비율, 파티션 lag, DB 포화, 할당량 headroom | 손실 없음, 범위가 제한된 lag, 포화 지점 기록 |
| Reliability | 서비스별 RPO/RTO와 business-impact analysis | Numeric objective 승인 |
| 복구 | Isolated 복원과 regional 장애 조치/failback drill | 무결성과 smoke 통과, 기본 fencing, 이벤트 복구 및 failback 검증, objective 충족 |
...
legal 보류, deletion, 모델 프로바이더, privacy-assessment 계약을 정의합니다. Upstream 설계는
...
| `reliability-owner` | SLO, RPO/RTO, 복구 설계, drill 수락 |
| `release-owner` | 산출물 출처 이력, 배포, 롤백, 승격 게이트 |
| `cost-owner` | 예산, 할당량, price 확인, 용량 graduation |

에이전트 담당 체계는 별도 accountability overlay입니다. 권한 확인을 부여하거나 운영
소유자 자리를 대체하지 않습니다.
...
Upstream required-evidence 키는 Azure Well-Architected Reliability `RE:01-10` 및 Operational
Excellence `OE:01-11` Best Practice 요구사항 전체를 커버합니다. Checklist evaluator는
...
| Key Vault 참조 | 환경 시크릿 injection | 필수 시크릿 미해결 시 시작 실패 | 교대와 unavailable-vault 테스트 |
...
Accepted risk는 resolved blocker가 아닙니다. 운영 게이트는 critical/high 항목의 상태와
...
`mode=enforce`를 요청할 수 있습니다. ARB는 control-only이므로 enforce는 실제 승인 및
결정 전이를 저장하지만 리소스를 배포하거나 ActionType을 승격하지 않습니다.

## 운영 종료 절차

1. Customer 포크에서 모든 필수 소유자 자리를 연결합니다.
2. 필수 근거 산출물을 첨부하고 upstream 저장소에는 시크릿/customer 데이터가 없도록
  확인합니다.
3. 적절한 거버넌스 경로에서 각 blocker를 해결하거나 공식적으로 수용합니다.
