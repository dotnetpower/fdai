...
사용하지 않습니다.
정확한 리소스용 실시간 새로 고침 어댑터는 더 넓은 보호 ObjectSet을 처리하지 않을 수 있습니다.
결과로 생성되는 타입 지정 보류는 최초 최신성, 완전성, 충돌 및 합성 근거 사유를 보존하며, 거부된
새로 고침을 공급자 근거 성공으로 표시하지 않습니다.
조회가 한 운영 상태 축의 상태 사실 메타데이터 키를 선언하면 그래프 최신성은 해당 키에서만
완전하고 최신이며 합성이 아니고 충돌이 없는 최적 후보를 선택합니다. 다른 축의 메타데이터가
없거나 충돌해도 선택한 조회를 무효화하지 않으며, 기존의 키 없는 상태 사실은 조회가 일반
`state` 키를 요청할 때만 참여합니다.
...
질의 경로 밖의 스캔은 ACL 투영 전에 각 원본 레코드의 정확한 선언 참조, 필수 필드, 속성 유형 및 키도 다시 검증합니다. 참조 누락, 오래된 선언 버전 또는 인스턴스 키 불일치는 완전한 인덱스 스냅샷에 들어갈 수 없습니다. 오류에는 원본 값을 포함하지 않습니다.
독립 준비 원본 검증은 게이트웨이를 통해 완전한 원본을 다시 읽고 전체 후보 집합을 재구성합니다. 준비 문서 해시가 서로 일치하더라도 객체가 실제로 존재하거나 속성이 현재 그래프와 같다는 증거가 될 수 없습니다. 이 증적은 검증 전용이며 벡터, 활성화 또는 실행을 인증하지 않습니다.
...
조용히 다르게 해석할 수 없습니다. 별도로 고정된 탐지 및 라우팅 범위 계약은 활성 구성의 제어 임계값에도 같은 정확한 형식, 단위, 적용 범위 및 허용 구간 규칙을 적용하지만 객체 그래프에 들어가거나 조회, 승격 또는 실행 권한을 부여하지 않습니다.
...
활성 인벤토리 영향 범위 변환 결과는 모든 탐색 에지에 하나의 검증 값을 일괄 할당하지
않고 같은 관계 근거 묶음을 재사용합니다. 각 에지는 근거 가용성(`available`, `stale`,
`unavailable`)과 검증 등급을 별도로 유지합니다.

- `configuration_observed`는 검토된 공급자 대응, 하나의 완전한 인벤토리 세대에서 확인한
  양쪽 끝점, 결정론적 교차 확인 근거 및 현재 근거 기준 시점이 필요합니다. 이 값은 기록된
  구성 관계를 확인하며 런타임 효과 검증을 의미하지 않습니다.
- `independently_verified`는 완전하고 충돌이 없으며 합성되지 않은 관측 근거가 필요합니다.
  검증자 신원은 원본 신원과 달라야 하며 검증 증적은 활성 세대에 연결되어야 합니다.
  런타임 호출 관측은 구현에 종속되지 않는 서비스 계약의 대응 ID와 개정 번호, 원본 스키마
  버전과 다이제스트 및 검증 방법 묶음과도 정확히 일치해야 합니다.
- `stale` 또는 `unavailable` 근거는 보존된 검증 등급이 과거의 근거 방식을 나타내더라도
  현재 해결되지 않은 상태로 표시합니다.

쿼리 완전성과 관계 근거 완전성은 서로 독립적입니다. 요청한 깊이와 에지 상한 안의 모든
에지에 도달했더라도 누락되거나 오래된 근거가 완전해지지 않습니다. 후보 산정의
`complete`도 모든 관계가 구체화되었다는 뜻이 아닙니다. `reviewed_unavailable` 또는
`unclassified` 후보가 하나라도 있으면 영향 원본 범위는 일부로 유지됩니다. Console은 두
검증 등급을 구분해 표시하고, 보조 세부 정보에 원본, 대응, 방법, 기준 시점 및 복구 사유를
제공하며, `verified` 또는 `unverified`만 운반한 기존 영향 범위 응답을 승격하지 않습니다.
버전 1.0 영향 범위 응답은 기존 근거로 계속 읽을 수 있으며 새 생산자는 근거를 추가한
버전을 내보냅니다. 관계 원본이 불완전하거나 관계 범위 산정이 없는 세대에서는 구성
에지를 검증되지 않은 상태로 유지하면서 범위가 제한된 원본, 대응, 방법 및 기준 시점의
출처 이력을 보존합니다. 영향 범위와 온톨로지 인스턴스는 이러한 상태에 서로 다른 복구
사유를 사용하고 기록된 경로 대응을 지우지 않습니다. 영향 쿼리와 같은 스냅샷을 사용하는
지도 보기는 모두
`runtime_calls`를 허용하며 호출자에서 대상으로 향하는 저장 방향과 독립 관측 등급을
보존합니다. Console은 대상, 깊이 및 순서가 있는 LinkType 집합이 제출한 쿼리와 일치하는
시뮬레이션만 허용합니다. 지도 보기는 해당 결과의 모든 도달 Resource와 탐색 에지도
요구합니다. 형식이 잘못되거나 범위에서 제외되거나 깊이가 제한되는 등 불완전한 그래프는
명시적으로 사용할 수 없는 상태로 유지합니다.
...
| P1-B | 활성 인벤토리 영향 범위입니다. | 탐색은 범위가 제한되고 스냅샷에 고정되며 저장 방향을 따르고 각 에지의 현재 구성 확인, 독립 검증, 오래됨, 사용 불가 또는 기존 근거 상태를 운반합니다. 쿼리 완전성은 근거 완전성을 대신하지 않으며 변환 결과는 실행 또는 변경 권한을 부여하지 않습니다. |
