...
translation_source_sha: e9d51645e3ebfada1a40f3499bb502dc391c7aea
...
| Kubernetes 워크로드 근거 조회 | implemented | Rollout, Pod 복구, Resource 이벤트 이력 FunctionType, durable lifecycle reader, 의미 플래너 및 집중 조립 검사 | Rollout 및 Pod 복구는 exact-release 소유권과 재시작 변화량 근거를 인증하고 `source_incomplete`를 보존하며 근본 원인과 실행 권한을 false로 유지합니다. `resource_event.kubernetes`는 판단의 duration과 frame lookback이 같아야 하며 durable cursor가 있으면 보존된 UID lifecycle 행을 사용합니다. cursor 없음, 빈 history, unavailable logs 및 incomplete source는 명시적인 gap으로 남고 성공이나 과거 부재로 바뀌지 않습니다. 인증된 exact-target 계획과 operator 표현은 읽기 전용이며 live replacement/recovery 증적은 외부 검증으로 남아 있습니다. |
