...
translation_source_sha: 7c6226e2c33b9d9e64e58779129006f242ebee71
...
| 대화 티어 및 전송 격리 | implemented | `semantic_query_model_targets.py`, `semantic_turn_consumer.py`, 집중 티어 라우팅 및 파티션 작업자 검사 | 운영 대화 조립은 T1만 연결합니다. 일반 Control Loop와 RCA의 T2 대상 해석은 이 조립 밖에서 계속 사용할 수 있습니다. 같은 그룹의 소비자 두 개가 브로커 파티션을 사용하고 Operator 요청은 `session_id`를 파티션 키로 사용하므로 세션 간 작업은 겹쳐 실행되며 같은 세션의 순서는 인과 순서를 유지합니다. |
...
| 2026-08-17 | implemented | 운영 대화 조립에서 자동 T2 승격을 비활성화하고 범위가 제한된 파티션 수준 의미 소비자를 추가했습니다. 일반 T2 대상 해석은 관리되는 비대화 소유자가 계속 사용할 수 있습니다. | `current change`, 집중 티어 라우팅, 의미 처리기, 세션 키 게시 및 로컬 토픽 파티션 검사 | 정확한 소스의 로컬 스택에서 제한 응답 세션 격리 증적을 보존합니다. |
