...
translation_source_sha: e9fd3dc76e8f2804bb5695722485c7bb1e0a155a
...
| Kubernetes 워크로드 근거 조회 | implemented | Rollout 및 Pod 복구 축약기와 FunctionType, 검토된 메트릭 의미 규칙, 의미 플래너, 집중 증적 권한, 메트릭 범위, 조립, 불완전 원본 검사, 인증된 대상 없는 Console 증적 | Rollout FunctionType은 Deployment, ReplicaSet, Pod 소유권 단계별로 발급된 exact-release Reader `operations-review` 증적만 받습니다. Pod 복구 FunctionType은 정확한 Pod, 나가는 ReplicaSet 소유자, 나가는 Deployment 소유자 증적을 인증하고, 클러스터 ID 및 불변 Pod UID로 선택한 검토된 30분 재시작 횟수 변화량을 결합하며, 소유자 replica 전체의 복구를 요구합니다. 행이 0개인 후보 표는 `source_incomplete`를 보존하고, 범위가 불완전하거나 모호하면 메트릭 읽기 전에 중단하며, 부분 이력은 복구를 검증할 수 없습니다. 이전부터 누적된 현재 횟수는 범위가 제한된 양수 변화량을 대신하지 못합니다. 두 축약기 모두 근본 원인과 실행 권한을 false로 유지합니다. 정확한 대상 Console 근거는 열린 상태입니다. |
