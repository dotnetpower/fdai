...
translation_source_sha: 7dcac144edf36bd6659f2b600ca820a9a14040a3
...
범위가 지정된 운영 Pattern을 읽으려면 상태에 접근하기 전에 인증된 principal과 정확한 인자에 결합된 독립 `case-history-read` 허용 근거가 필요합니다. 경계 인벤토리는 허용 여부를 결정하는 함수와 집중 테스트를 고정합니다. 다시 컴파일한 요약은 비활성 상태를 유지하며 현재 원본 근거가 없다고 접근 권한이나 실행 권한이 생기지 않습니다.
