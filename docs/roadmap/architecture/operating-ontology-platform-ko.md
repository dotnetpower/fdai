...
translation_source_sha: 6978e43654cacf5f969f7b63624ab05bcbc92c47
...
| Kubernetes 워크로드 근거 조회 | implemented | Rollout, Pod 복구, Resource 이벤트 이력 및 Pod lifecycle cohort FunctionType, 검토된 metric semantics, semantic planner, Kubernetes 및 durable lifecycle reader, 집중 증적, metric, event, 조립, persistence 및 incomplete-source 검사 | 일반 `resource_event.kubernetes`는 정확한 대상 표시를 유지하며 형제 UID 행을 다시 표시할 수 없습니다. Pod 교체는 Event `related.uid`가 아니라 완전한 inventory snapshot에서 얻은 identity를 사용하는 별도 컨트롤러 기반 cohort를 사용합니다. 복구 축약기는 과거 UID 근거 경로 하나와 정확한 현재 Pod 및 Deployment dependency를 받고, 누락되거나 모호한 계보는 incomplete로 유지합니다. Rollout과 복구는 근본 원인 및 실행 권한을 false로 유지합니다. 인증된 실제 교체 및 복구 증적은 외부 검증으로 남아 있습니다. |
...
| 2026-08-28 | implemented | 별도의 typed Pod lifecycle cohort를 추가했습니다. Inventory snapshot은 변경할 수 없는 Pod 컨트롤러 계보를 저장하고 cohort 조회는 해당 정확한 UID로 lifecycle 행을 결합합니다. 일반 Resource 이벤트 표시는 요청한 UID로 다시 필터링하며 교체 축약기는 과거 근거와 현재 근거를 분리해 보존합니다. | `current change`, 집중 identity, cohort, 축약기, semantic planning, 조립, 표시 격리, migration, Ruff 및 strict mypy 검사, worktree 전용 로컬 Docker database에서 PostgreSQL lifecycle 검사 7개 통과 | 배포 PostgreSQL 및 인증된 정확한 대상 교체 근거는 별도로 보존합니다. |
...
| 2026-08-28 | implemented | 같은 응답에 대한 후속 리뷰 결함 두 건을 닫았습니다. `max_bytes` byte-budget 계산이 번들 자체의 `bundle_id`와 `digest`를 여전히 빠뜨려 보장된 상한이 그 바이트만큼 조용히 초과될 수 있었습니다. 이제 두 값 모두 (번들이 존재하기 전에는 고정 길이 SHA-256 placeholder로 크기를 계산해) 예약하고, 번들의 실제 값으로 계산합니다. Context snapshot 결속은 ontology release는 검증했지만 catalog revision은 검증하지 않았습니다. 이제 `catalog_versions['catalog']` 항목이 요청의 `catalog_revision`과 일치하지 않는 snapshot도 거부합니다(secured receipt 계약에는 catalog-revision 필드가 없으므로 receipt 측 검사는 적용되지 않습니다). | `current change`; `evidence_read.py`; 집중 evidence-read 검사(`15개 통과`, 기존 11개에서 증가) 및 더 넓은 operational-context 검사(`84개 통과`) | 인증된 런타임 응답 하나를 별도로 보존합니다. Live 또는 배포 근거는 아직 없습니다. |
