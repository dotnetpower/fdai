...
translation_source_sha: d138d54229b4902cbebe3fabbb2784b79c1e7b39
...
| Versioned provider 관계 materialization | implemented | `delivery/provider_schema_relationship_generation.py`, `delivery/provider_schema_relationship_ledger.py`, `core/ontology_platform/direction_shadow`, 집중 generation 및 direction-shadow 검사 | Provider-schema 및 REST evidence, mapping revision, projection manifest, 명시적 direction/cardinality/link metadata, 변경 subset 무효화, rollback, replay 및 exact-release 비교는 bounded content-addressed 상태입니다. Candidate promotion은 proposal-only이며 graph와 migration 권한은 false입니다. |
...
| Kubernetes 워크로드 근거 조회 | implemented | Rollout, Pod 복구, Resource 이벤트 이력 FunctionType, 검토된 메트릭 의미 규칙, 의미 플래너, Kubernetes 및 복합 읽기 경로, 집중 증적, 메트릭, 이벤트, 조립 및 불완전 원본 검사, 인증된 대상 없는 Console 증적과 정확한 대상 Console 증적 | Rollout 및 Pod 복구는 exact-release 소유권과 재시작 변화량 근거를 인증하고 `source_incomplete`를 보존하며 근본 원인과 실행 권한을 false로 유지합니다. `resource_event.kubernetes`는 판단의 duration과 frame lookback이 같아야 합니다. 출처에 근거한 대상 하나는 검토된 유형 범위에 정확한 Resource 이름 조건식을 추가합니다. 이 identity 조건식은 Resource 하나로 정확히 해소되어야 하며 일치 항목이 없거나 여러 개면 provider I/O 전에 `target_resolution_not_exact`로 중단합니다. 정확한 child 하나는 secured ObjectSet이 일치하는 불변 UID와 cluster를 노출할 때만 identity-aware reader를 사용할 수 있습니다. Function은 분리된 불변 two-field snapshot을 전달하고 adapter는 `involvedObject.uid`를 사용하기 전에 canonical id를 다시 계산합니다. Legacy reader는 기존 protocol을 유지하고 identity가 없거나 일치하지 않으면 provider I/O 전에 중단합니다. 명시적인 cluster 읽기는 삭제된 객체와 cluster 이력을 보존합니다. 어댑터는 혼합 범위의 적용 가능한 이벤트를 불완전 상태로 보존하고, 인코딩 응답을 거부하고, raw 입력을 256 KiB로 제한하고, 최대 256개 항목만 정규화하며, 원시 프로바이더 페이로드를 반환하지 않고 Event 보존이 권위 있게 확인될 때까지 `source_retention_unverified`를 보고합니다. 이중 언어 답변은 출처 완전성, 안정적인 제한 사항, 표시 개수, 전체 개수 및 `display_truncated`를 보여주고 가장 최근 Event 행 8개를 시간순으로 보존합니다. 인증된 정확한 대상 계획과 표현 근거는 보존했으며 성공적인 Event 출처 접근, 영속 보존, 원인 및 복구 결합은 열린 상태입니다. |
