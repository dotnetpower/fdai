...
translation_source_sha: 4d43d7bc46f9c467b5b410979b91465bd05ac490
...
| 2026-08-16 | implemented | 검토 후 연속 수집 루프를 강화했습니다. 실패한 시도는 매 틱 재시도하는 대신 정상 간격까지 지수적으로 백오프하고, 시도 하나는 무진행 마감과 포기 창보다 먼저 끝나는 절대 상한으로 제한되며 두 상한은 따로 보고됩니다. 소스 스트림은 모든 종료 경로에서 닫히고, 변경은 프로바이더가 말하는 발생 시각이 아니라 이 컨트롤 플레인 시계로 활성 스냅샷 시작 이후에 기록되었을 때 미조정으로 셉니다. | `current change`; 집중 인벤토리·영속성·프로젝션·인프라 검사는 1940개 통과와 환경 의존 129개 건너뜀을 보고하고, strict mypy는 소스 9개에서 문제를 보고하지 않았으며, 게이트 SQL은 마이그레이션한 임시 PostgreSQL 데이터베이스에서 시드된 15개 상태에 대해 설계대로 판정했습니다. | ARM과 declarative 소스는 배치를 하나만 내보내므로 실효 상한이 진짜 무진행 신호가 아니라 진행 마감입니다. 변경 기반 조정에 대한 인증된 live 근거는 아직 없습니다. |
