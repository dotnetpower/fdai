...
| 2026-08-10 | IS-09 | Startup probe consumer readiness | Round 50 | Live Core logs showed each unique Event Hubs consumer group needed about three seconds to join, while the independent service omitted the configured settle budget and published after the runtime's 0.5-second default. The consumer then started at the latest offset after the synthetic record and the round trip timed out. The service now injects a 12-second settle budget, a 30-second probe deadline, and a 60-second phase deadline with ordering validation; the probe still fails closed when no exact round trip is observed. |
