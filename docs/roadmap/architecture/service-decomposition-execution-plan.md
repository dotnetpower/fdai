...
|-------------|---------------|----------------|-------------------|
| SD-01 conversation persistence | Persistent worker at `/home/moonchoi/dev/fdai-worktrees/sd01-conversation-persistence` | `chat_history.py`, `chat_image_history.py`, `chat_document_evidence.py`, destination persistence/projection packages, matching tests, and module-map updates | Persistence handoff preserves idempotent transcript writes, image compensation/finalization, and document evidence identity with zero persistence/projection-to-route imports |
