...
|-------------|---------------|----------------|-------------------|
| SD-01 final route closure | Persistent worker at `/home/moonchoi/dev/fdai-worktrees/sd01-route-closure` | `routes/chat_response_tail.py`, `routes/chat_route_common.py`, destination conversation application/projection modules, matching tests, the Operator module inventory/map pair, and SD-01 status evidence | Non-transport coordination moves behind explicit owners; the remaining chat routes classify only as transport, registration, request transport, protocol, or the reviewed compatibility facade; JSON, SSE, authentication, cancellation, history, and route contracts remain unchanged with zero application/projection/persistence-to-route imports |
...
| SD-06 read-only Operator projection | Integration owner on `main`; completed projection and core workers retained read-only | Projection paths released after `e76874409`; SD-01 conversation persistence retains route and module-map ownership | Pure bounded projection preserves the candidate seal gate and zero authority with no Starlette, route, app, persistence, composition, shared-contract, or core imports; SD-06 remains open until the active SD-01 handoff can add canonical module-map ownership without collision |
| SD-06 critique hardening | Persistent worker at `/home/moonchoi/dev/fdai-worktrees/sd06-hardening` | `src/fdai/core/change_lineage/**`, `src/fdai/delivery/operator_api/projections/change_lineage/**`, `src/fdai/delivery/github/change_feed.py`, `tests/core/change_lineage/**`, and `tests/delivery/operator_api/test_change_lineage_projection.py` only | At least 10 independent critique rounds complete, every reproducible Medium-or-higher finding is fixed with focused evidence, and final independent review reports only Low-or-lower residuals; the GitHub adapter addition is limited to the reproduced naive-window parity defect, while SD-01 routes and module maps remain read-only |
