...
| 2026-08-10 | IS-09 | Bounded rollback revision suffix | Round 74 | The same Worker failure exposed that the verbose automatic rollback suffix exceeded Azure's combined 54-character Container App revision-name limit for the longest service name, so recovery could not start. The rollback suffix is now the lowercase `r` prefix plus the unique workflow run id. It remains deterministic and collision-resistant while fitting every canonical service name; rollback still copies only the exact captured revision and verifies the restored image and sidecar contracts. |
| 2026-08-10 | IS-09 | Corrected N-1 artifact rebuild | Round 75 | Live rollback proved the original 0.1.2 document images cannot select their attached user-assigned identity and therefore cannot become ready in the protected topology. A dedicated artifact-only source now rebuilds all five 0.1.2 distributions from the current identity-, probe-, and recovery-hardened service code. The source is temporary and immediately followed by restoration of the 0.1.3 development line; final evidence pins the exact 0.1.2 source, supply-chain run, image digests, and attestations instead of relabeling the broken images. |
| 2026-08-10 | IS-09 | N-1 override retirement | Round 76 | Supply-chain run `31367288968` succeeded from exact source `352c8d1e661a6a53f0958767550fd57c2b975706` while the tracked override was active, so its immutable artifacts remain 0.1.2. The later run `31367329056` belongs to the automatic release commit and failed after the override source. The override is now inactive on `main`; every later image build uses the committed 0.1.3 service and lockfile versions. This creates distinct, attributable source revisions for patched N-1 and final N and prevents accidental future publication of 0.1.2 artifacts. |
| 2026-08-10 | IS-09 | Corrected remote release binding | Round 77 | The release contract now binds remote N-1 to corrected source `352c8d1e661a6a53f0958767550fd57c2b975706` while retaining historical local-focused N-1 source `9f1234f93d356dedbddcb3b88aa7bc4da38b2dc2` in a separate field. All five corrected 0.1.2 SBOMs report the intended service package versions, and provenance, SBOM, and Core resolved-model attestations verify against the exact source and protected supply-chain signer. |
| 2026-08-10 | IS-09 | Per-run deployment controls | Round 78 | Final-proof critique found that the initial N runs and corrected rollback runs use different protected-main revisions, while one aggregate controls SHA either rejects valid evidence or misstates history. Remote evidence now records the plan-sealed controls on each plan/apply pair, requires apply controls to match its plan, and has the GitHub binder prove every API-bound workflow head and artifact-bound controls revision deployment-input-equivalent to the aggregate controls. The artifact-only image-build override helper is explicitly outside service-deploy inputs; every other deployment helper remains strict. |
| 2026-08-10 | IS-09 | Controls-verifier coverage | Round 79 | Follow-up critique added an executable check that every unique transition workflow head and plan-sealed controls revision reaches the equivalence verifier, including deduplication and distinct controls across stages. One-time adoption remains ancestry-bound because later rollout hardening intentionally changed deployment inputs after adoption; exact runs, successful steps, artifacts, and rollback controls remain independently bound. High- and Medium-severity residuals are zero. |
