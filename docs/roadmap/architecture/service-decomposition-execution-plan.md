...
| SD-06 read-only Operator projection | Persistent worker at `/home/moonchoi/dev/fdai-worktrees/sd06-operator-projection`; completed core worker retained read-only | `src/fdai/delivery/operator_api/projections/change_lineage/**` and `tests/delivery/operator_api/test_change_lineage_projection.py` only | Pure bounded projection preserves the candidate seal gate and zero authority with no Starlette, route, app, persistence, composition, shared-contract, or core edits; HTTP registration remains deferred to the SD-01 route owner |
