...
|-------------|---------------|----------------|-------------------|
| SD-01 terminal projections | Persistent worker at `/home/moonchoi/dev/fdai-worktrees/sd01-terminal-projections` | `chat_trajectory_detail.py`, `chat_screen_data.py`, `chat_model_trace.py`, the response half of `chat_resource_context.py`, destination conversation projections, matching tests, and module-map updates | Pure projection handoff passes focused JSON/SSE and boundary checks with zero application/projection-to-route imports |
