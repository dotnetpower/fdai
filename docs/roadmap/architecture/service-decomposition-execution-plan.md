...
| 2026-08-10 | IS-09 | N-1 override retirement | Round 76 | Supply-chain run `31367329056` started from the exact validated source while the tracked override was active, so its immutable artifacts remain 0.1.2. The override is now inactive on `main`; every later image build uses the committed 0.1.3 service and lockfile versions. This creates distinct, attributable source revisions for patched N-1 and final N and prevents accidental future publication of 0.1.2 artifacts. |
