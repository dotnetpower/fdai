...
| SD-06 canonical Change lineage | Integration owner on `main`; completed core worker retained read-only at `/home/moonchoi/dev/fdai-worktrees/sd06-change-lineage` | Core reservation released after `52dbb2ba3`; remaining read-only Operator projection waits for the SD-01 route handoff | Canonical lineage, decision/resilience traces, candidate-only learning, and GitHub/Azure DevOps provider compatibility pass focused tests with zero execution or promotion authority; no worker may reopen shared contracts, agents, composition, Operator API, ingestion, infra, or this plan |
