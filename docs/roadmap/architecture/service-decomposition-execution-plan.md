...
| 2026-08-09 | IS-09 | Provisioned startup probe topic | Round 49 | Catalog-corrected Core run `31316016509` reached the health server but remained not ready because the default `runtime.startup.probe` Event Hub was absent and the Standard namespace was already at its ten-entity limit. Core now binds the startup round-trip probe to its existing provisioned event topic, retaining the synthetic scope and dedicated probe consumer without requiring an eleventh entity. |
