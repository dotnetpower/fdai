...
translation_source_sha: 7bdc3f16234383f272a8c70c3115617113f9382d
...
| 카탈로그 자산 | `rule-catalog/catalog/`의 비용 범주 규칙과 `cost-aware-remediation.yaml` | 마이그레이션은 안정적인 id를 보존하고 각 일반 자산을 기본 카탈로그에 둘지 패키지로 이동할지 결정해야 합니다. CI는 스키마를 변경하는 수명 주기 검사를 직렬화하고 forward service 마이그레이션이 Cost 마이그레이션 인벤토리 검증 전에 root 소유 T2 lookup index를 복원합니다. |
