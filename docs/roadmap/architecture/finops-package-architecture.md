...
Preserve the reviewed declaration set while rebinding every semantic-profile declaration to its exact active ref. Refresh the exact release and canonical profile digest, packaged profile-byte
SHA-256, asset manifest reference, and W6 source inventory's `package_manifest_sha256` together with all 16 F1-F8 fixture references. Re-evaluate the unchanged expected outcomes for eight positive and eight
