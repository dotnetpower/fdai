...
The package exposes one bounded analytics command to the local coordinator and deployed scheduled job. It reuses the validated Cost Management Query path instead of the slow Usage Details list API. Its durable run receipt and read projection report freshness and source readiness; they do not enable the package, create a case, or grant approval, execution, or promotion authority.
