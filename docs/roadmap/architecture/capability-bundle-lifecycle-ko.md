...
translation_source_sha: e9ab68b07951b8dd964cbdd7998df509b804318f
...
운영 근거는 하나의 범위가 제한된 실행 또는 여러 개의 연결된 실행에서 얻을 수 있습니다.
별도 증적은 동일한 산출물 digest, release 리비전, 환경 ID, 감사 상관관계 ID에 바인딩될
때만 하나의 집합을 구성합니다. 완전한 집합은 설치, 활성화, 비활성화, 철회, reload, 재시작
후 확인을 순서대로 계속 다룹니다. 누락되거나 충돌하는 연결은 수명 주기를 미완료 상태로
유지합니다.

이 완화는 근거 패키징만 변경하며 런타임 권한은 변경하지 않습니다. 각 전환은 자체 행위자,
멱등성, 감사, 효과 근거를 보존하고 어떤 증적도 `ActionType` 또는 `Workflow`를 promotion할
수 없습니다.
