...
| System Knowledge Service candidate | in-progress | `services/system-knowledge-service/`; `packages/service-contracts/src/fdai_service_contracts/system_knowledge.py`; focused service checks in the current change | The independent read-only package and image support mutually exclusive Bot Framework and team-scoped Outgoing Webhook transports without changing the validated five-service baseline. Production graduation evidence remains open. |
...
| Service-owned test coverage | implemented | `tests/integration/service-suites.json`; service-suite ownership gate; focused Operator measurement tests | Every service test file has exactly one owning service suite. Dashboard aggregation and source projection tests remain in the Operator unit group and do not imply a new service boundary. |
### Implementation history

| Date | State | Change | Evidence | Remaining |
|------|-------|--------|----------|-----------|
| 2026-09-15 | implemented | Registered ten alert runtime composition/relay modules in the exact Core wheel inventory without changing forbidden Operator or isolated-Executor imports. | `current change`; [Core packaging test](../../../tests/integration/services/test_core_service_package.py). | The wheel is source packaging, not runtime promotion or deployment evidence. |
| 2026-09-15 | implemented | Added Operator-owned alert PostgreSQL integration tests and parameterized the outbox namespace for the actual driver. | `current change`; [PostgreSQL alert checks](../../../services/operator-service/tests/test_alert_quality_postgres.py): 4 passed with zero skips in disposable local databases under `fdai_operator`. | Minimal state primitives were exercised, not full service migrations or deployed identities. |
...
| 2026-09-15 | implemented | Recompiled System Knowledge after integrating protected main `91ff893cb`, preserving the reviewed handover UI changes and every upstream task-worker budget correction. | `current change`; main CI `34928980843` succeeded; canonical 14-record catalog and exact source regression. | No Console input, service ownership, identity, transport or runtime authority changed in this integration. |
...
| 2026-09-11 | implemented | Bound the Core recovery observation intake to an explicit deployment-configured trusted observer identity rather than trusting payload identity. | `current change`; Core service Terraform, observer ingress, and production writer tests. | Retain governed runtime evidence for the deployed identity. |
| 2026-09-11 | implemented | Added read-only isolated-Executor access to exact Core safeguard bundles with an explicit Core migration dependency and matching JSONB index. The default effect service rejects legacy unbound commands; an authority-cutover-only transition flag supports the bounded upgrade or rollback window and defaults off. | `current change`; isolated bundle resolver, migration, ownership manifest, service Terraform, runtime config, and focused rollout tests. | Retain governed cross-service rollout evidence with the transition disabled on the final revision. |
| 2026-09-11 | implemented | Registered the isolated-Executor deadline helper tests as unit coverage and safeguard-bundle revalidation tests as contract coverage in the service-owned suite manifest. | `current change`; `tests/integration/service-suites.json`; focused service-suite ownership tests passed 29 cases. | Keep every new service test assigned to exactly one service-owned group. |
| 2026-09-11 | implemented | Regenerated the repository-owned Python and Console TypeScript service-contract projections after the safeguard-bound Executor command and observation schemas changed. The five-service topology and authority assignments are unchanged. | `current change`; `generate_service_contracts.py`; focused generation drift tests. | Retain governed cross-service evidence before enabling the new command in production. |
| 2026-09-11 | implemented | Unified isolated-Executor deadline checks at one timezone-aware boundary and made the exact deadline instant expire before any new provider invocation. | `current change`; `effect_safety.py`; isolated-Executor effect safety tests. | Retain governed deadline and provider-status reconciliation evidence on the promoted service revision. |
| 2026-09-11 | implemented | Added an isolated-Executor-owned partial index for pending receipt publication so broker backpressure does not turn each bounded outbox claim into a full-table scan. The migration follows the service branch head and has an explicit rollback. | `current change`; `executor_outbox_pending_index_20260911`; focused service-migration inventory checks. | Retain governed runtime queue-depth and claim-latency evidence with the exact deployed migration revision. |
...
| 2026-09-13 | implemented | Restored exact service-test ownership for the isolated Executor automation-hold fence suite. The new fence test had no owning group, so the suite runner could not resolve exactly one owner and every coverage, ordering, and argument check failed closed. It now belongs to the isolated-executor `unit` group next to the sibling effect-safety fence test; the five-service topology, source roots, and group semantics are unchanged. | `current change`; `tests/integration/service-suites.json`; 30 service-suite ownership and models-facade checks passed. | Retain the governed Executor runtime evidence under its owning capability design. |
### Remaining work
...
|----------|--------------|----------|-----------|---------------|---------------|------------------------------------|
| Assignment request and disposition `1.0.0` | `fdai-service-contracts` typed assignment records | Operator immutable proposal outbox; Core receipt and Saga-sealed case observations | Huginn/Forseti/Var/Saga/Muninn; review-only artifact delivery; Operator reads canonical effects | immutable Operator case id | Exact first version; unknown versions fail closed | Lease-fenced at-least-once notices; five-minute source validity; exact payload receipt in case CAS; redacted poison DLQ; normal/DLQ retention 1/7 days; retained immutable audit is not current approval |
