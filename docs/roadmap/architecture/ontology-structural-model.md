...
The selected-instance view revalidates its bounded response every 15 seconds while the document is
visible and immediately after focus, online, or visible-state recovery. It coalesces overlapping
requests, preserves the last verified response when a refresh fails, and displays that failure
instead of silently treating stale data as current. This is near-real-time presentation polling, not
