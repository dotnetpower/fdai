...
translation_source_sha: d8316c2e6f2239f83524b3828fcb722f733c3463
...
- **안전한 보강:** 실시간 조회는 현재 답변을 지원할 수 있으며 같은 ingress를 통해 타입이
  ...
  정확한 리소스용 실시간 공급자가 더 넓은 보호 집합을 처리하지 않으면 조회 보류가 일반 새로
  고침 실패만 보고하지 않고 최초 최신성, 완전성, 충돌 및 합성 근거 사유를 유지합니다.
...
단일 세대는 Resource 50,000개, 관계 또는 제외 기록 200,000개, 정규화된 수집 레코드 16 MiB까지만 허용합니다. 한도 초과는 최종 종료 경계 전에 중단하며 이전 세대를 유지합니다. 더 큰 범위는 별도로 검토한 분할 소유권이 필요합니다. 이 레코드 한도만으로 프로세스 메모리를 제한하지는 않으며, 전용 Linux 진입점은 아래의 별도 주소 공간 상한을 적용합니다. 용량 오류로 활성 범위를 임의로 줄이거나 새로 승인된 소유 범위 밖의 삭제를 허용하지 않습니다. 구현 원장은 최종 10000/50000개 객체의 게시와 재시작, 해당 상한 아래의 수집부터 게시까지 측정값을 보존합니다. 활성화된 버전 저장소는 측정된 작성자 잠금 점유를 줄이지만 전체 준비 비용은 그래프 크기에 비례합니다. 합성 RSS 측정은 운영 지연 보장이나 DB 서버 메모리 한도가 아닙니다.
...
ARG는 이전 원본 행을 보존하지 않고 다음 페이지 요청 전에 정규화하며, 정규화된 세대만 한도 안에서 보존합니다. 샤드 실패나 세대 한도 소진 뒤에는 대기 중인 샤드의 공급자 조회를 시작하지 않으며, 진행 중인 요청은 제한된 취소 절차로 정리하고 최종 종료 경계를 내보내지 않습니다. 운영 활동 래퍼도 영속 청크 쓰기를 전달합니다. 미완료 복구는 기존 실행 잠금 아래에서 기한 내의 정확한 문맥을 가진 청크 체인 하나를 후보 행과 대조하고, 원래 관측 시각을 보존하며 새 Resource만 추가 저장합니다. 토큰을 넘기지 않고 공급자 원본 전체를 다시 읽으며, 보존된 식별자의 변경이나 누락, 관측 시각 후퇴, 체크포인트에 없는 보강, 모호한 시도 또는 최종 종료 표식 누락은 승격을 막습니다. 새 관계와 커버리지는 봉인과 실제 승격 전에 다시 검증합니다. 이 방식은 반복 저장을 줄이지만 공급자 호출은 줄이지 않습니다.
...
경계를 만들지 않고 전체 체인을 청크별로 검증합니다. 호출은 30초, 수집 시도는 30분으로 제한합니다. 공급자 I/O 전에 직렬화된 수락 검사가 보존 스냅샷 128개에서 새 시도를 거부하며, 공간을 만들기 위해 전달 대기 근거를 삭제하지 않습니다.
분할된 배치의 마지막 청크에서만 커서를 전진시킵니다. 증적은 승격, 삭제 또는 범위 변경을 허용하지 않습니다. 재시작은 설정된 수집기의 기존 읽기 신원과 기한 안에서만 수행하며, 원본 누적과 보강 결과는 각각 정규 레코드 16 MiB 상한을 적용합니다. 전용 Linux 인벤토리 진입점은 OS 주소 공간의 두 한도를 `FDAI_INVENTORY_MEMORY_LIMIT_MIB` 값(기본 2048, 허용 범위 256-4096 MiB)으로 낮추고, 더 엄격한 기존 한도는 높이지 않으며 수집기 호출 전에 64 MiB의 여유 공간을 요구합니다. 같은 강제 상한이 네이티브 버퍼를 포함한 수집, 보강 및 게시 전체에 적용되며, 할당 실패로 부분 승격을 허용하지 않습니다. 이는 RSS 추정치나 DB 서버 메모리 한도가 아니며, 임베드된 `run()` 호출자의 프로세스 한도를 바꾸지도 않습니다.
재개 지점 `1.1.0`은 누적 개수를 포함한 모든 필드를 다이제스트에
...
각 청크는 최대 1 MiB 및 레코드 1,000개이며 그래프 내용과 매니페스트는 각각 32 MiB로 제한합니다. 매니페스트는 청크 다이제스트, release, 세대, 소유권, 수정 번호, 상태 및 워터마크를 고정합니다. 완전한 분할 집합은 객체와 링크 개수 및 겹치지 않는 정렬된 식별자 범위를 고정하며, 저장소 분할이 공급자 커버리지나 독립 소유자가 되지는 않습니다. 활성화된 준비 경로는 완전한 관계형 후보와 정확한 증적을 봉인해 커밋하되 게시하지 않습니다. 새 프로세스는 원래 입력, 그래프 내용, 의존성, 기준 버전과 epoch를 검증한 뒤 30분 안에서만 후보를 재사용합니다. 손상은 차단하고 만료되면 새로 준비하며, 기준 변경을 묵시적으로 재적용하지 않습니다. 대기 메타데이터와 보존 버전에는 각각 16개 상한을 적용합니다. 대기 후보는 기준 버전, 매니페스트와 청크를 고정하고 성공한 게시는 같은 트랜잭션에서 대기 고정을 해제합니다. 프로세스가 사라져도 부분 그래프가 활성화되지 않습니다.
준비 시 삭제 대상과 외부 끝점의 수정 번호, 타입, release 및 전체 속성을 하나의 읽기 스냅샷으로 고정하고 작성자 잠금 전에 타입이 지정된 입력을 복원합니다. 인벤토리 매니페스트는 이전과 새 객체 및 링크 소유 집합을 독립적으로 고정합니다. 새 소유권으로 기존 외부 객체를 가져올 수 없으며, 소유권이나 의존 내용이 바뀌면 게시를 거부합니다. 저장된 JSON과 세대, 수정 번호 및 다중성 검증도 유지합니다. 이전에 없던 행은 삭제 대상에서 제외합니다. 활성화된 저장소는 그래프, 증적, 상태, 전달, 관측 체크포인트와 외래 참조 식별자를 전역 게시 잠금 전에 준비합니다. 최종 경계는 기준 버전과 epoch를 비교하고 활성 인벤토리 세대를 잠가 재검증하며, 한 트랜잭션이 모든 준비 변경을 함께 게시하거나 되돌립니다. 상태 갱신과 커서 복구도 상태 잠금 후 전역 잠금 순서를 유지합니다. 기존 저장소의 교체 잠금은 유지합니다.
게시 시 실제 객체 수정 번호를 포함한 내용 기반 증적도 커밋합니다. 저장소 페이지는 이후 게시에도 같은 다이제스트를 유지하고 선택된 청크만 읽으며, 증적과 매니페스트 크기는 제한되지만 고정 크기는 아닙니다. 페이지는 정확한 분할 범위를 검증하고 분할 완전성을 표시합니다. 링크 페이지는 같은 매니페스트의 정확한 외부 끝점과 명시적 의존 완전성을 포함하며, 구형 속성 누락은 미완료로 유지하고 현재 끝점으로 대체하지 않습니다. 내부 페이지는 접근 권한을 주지 않으며 소비자는 principal 및 속성 권한 검사를 유지합니다. 현재 스캔은 같은 연결과 다이제스트 안에서만 메타데이터를 재사용하고, DB 재구성으로 내용이 없으면 차단합니다. 조건 없는 Resource 전용 현재 조회는 하나의 읽기 트랜잭션에서 매니페스트와 상태, 커버리지, 실제 식별자, 수정 번호 및 release가 일치해야 커밋된 페이지를 선택하며 게이트웨이는 ACL을 유지합니다. 조건 검색, 다른 ObjectType 및 구형 현재 조회는 기존 경로를 유지하지만 명시적 버전 고정 조회는 대체하지 않습니다. 지원하는 분할 집합은 인벤토리 소유자 하나와 전역 epoch를 유지하며 병렬 소유권 확장을 주장하지 않습니다. 활성화 전에는 기존 테이블과 N-1 UPSERT를 유지하며 부분 후보는 완전한 소유 집합을 교체하지 않습니다.

승인된 전환에서는 N-1 작성자를 차단할 수 있습니다. 추가 마이그레이션은 비활성 상태를 유지합니다. `activate_versioned_graph`는 작성자 최소 버전을 올리기 전에 그래프 테이블을 잠그고 정확한 내용을 봉인하며, 테이블 수준 읽기 권한과 원래 테이블을 보존하고 기존 조회 이름으로 활성 버전을 제공합니다. 알 수 없는 트리거, 종속 view, RLS, 열별 권한 및 지원하지 않는 유입 외래 키가 있으면 활성화를 거부합니다. `materialize_legacy_graph`는 옛 백업이 아니라 최신 활성 버전을 독립적으로 검증해 복원하며 최종 커밋은 호출자가 맡습니다. 현재 작성자는 완전한 비공개 복사본을 검증하고 전역 잠금 전에 봉인한 뒤 정확한 기준 버전과 epoch를 비교합니다. 동일한 재생은 포인터와 epoch를 유지합니다. 기존 식별자 동기화는 유입 Resource 참조를 보존하며 참조 중인 객체 삭제는 보존 근거를 지우지 않고 거부합니다. 전체 그래프는 500000행 및 64 MiB로 제한합니다. 커밋 후 정리는 활성 버전과 이전 버전 7개를 보존하고 대기 중인 준비를 보호합니다. 전체 준비 비용은 그래프 크기에 비례하며 최종 경계가 짧더라도 고정 커밋 지연을 보장하지는 않습니다. 프로토콜과 정리 표식은 호환성 메타데이터이지 권한이 아니며, 설치나 로컬 테스트로 실제 활성화를 승인하지 않습니다. 게시 시 기준 세대와
...
S01-S07은 로컬 구현을 완료했으며 구현 원장에 최종 측정과 12경계 종결 검토를 기록합니다. 실제 스키마 검증과 운영 적용은 별도 작업으로 남아 있습니다.
...
재구축 가능한 hot 변환 결과에는 제한된 보존 구간을 적용합니다. Inventory는 활성 세대와 수집 중인 세대를 유지하고, 대체된 스냅샷과 실패한 스냅샷은 각각 최신 3개를 유지합니다. 더 오래된 대체 세대도 정확한 전달 완료 표식이 그래프 게시 근거와 일치하기 전에는 보호하며, 대기, 누락, 손상 또는 불일치 표식으로 삭제를 허용하지 않습니다. 전달 표식이 없는 실패 후보는 정리할 수 있고, 청크 정리는 정확히 해시한 시도 접두사를 사용합니다. 따라서 전달 확인이 장기간 지연되면 근거 삭제가 아니라 명시적인 저장소 압력 복구가 필요합니다. Ontology snapshot 조회는 최신 committed pin 8개와 각 pin의 정확한 prepared 매니페스트 및 chunk를 보존합니다. PostgreSQL inventory progress는 기한 내 실행 중 시도와 최신 종결 시도 16개의 완전한 chain을 보존합니다. 기한이 지난 실행 중 chain과 더 오래된 종결 chain은 전체 시도 단위로 hot projection에서 제거하며, 더 오래된 변경 불가 부트스트랩 근거는 비공개 Blob 저장소에 남깁니다. PostgreSQL LLM metering과 컨트롤 루프 분류 변환 결과는 각각 최신 레코드 50,000개를 보존합니다. 이 상한은 관측 원장, 감사 체인, 승인, 실행, rollback 또는 archive 근거를 삭제하지 않습니다. 이러한 레코드는 아래의 통제된 partition 및 hold 수명 주기를 따릅니다.
### 범위가 제한된 관측 이력
...
전체 구현 감사와 상태 전이 원장은 [지속형 운영 인스턴스 그래프 근거](continuous-operational-instance-graph-evidence-ko.md)를 참조하세요.
