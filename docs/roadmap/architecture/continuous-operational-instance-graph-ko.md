...
translation_source_sha: 2ffe32c2069a733cb44fc10a3c9647657d3918f9
...
| 영속 Kubernetes 수명 주기 수집 | implemented | lifecycle observation/source/collector, PostgreSQL store, durable Resource-event adapter, shared analyzer schedule 및 집중 테스트 | [이슈 #292](https://github.com/dotnetpower/fdai/issues/292)를 충족합니다. bounded resumable Event source와 append-only cursor store가 UID 기반 lifecycle 관측을 보존합니다. adapter는 cursor가 존재한 뒤에만 보존 행을 사용하며 cursor 없음 또는 빈 history를 명시적인 incomplete 근거로 반환합니다. 동일한 analyzer tick이 server-owned binding이 있을 때 local과 Container Apps Job에서 선택적인 lifecycle pass를 실행하고, incomplete 근거는 bounded retry를 위해 tick을 실패시킵니다. Live cluster 근거는 수집하지 않았습니다. |
