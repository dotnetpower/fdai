...
translation_source_sha: e92fea8d32c6722d689de4c7b9a9e5105c792efd
...
ARG는 이전 원본 행을 보존하지 않고 다음 페이지 요청 전에 정규화합니다. 정규화된 세대는 한도
안에서 보존하며, 공급자 조회의 재개에는 식별자, 관계 및 최종 커버리지의 재생 연결이 필요합니다.
...
추가 ARG 계약 다이제스트는 생성 쿼리, 검토된 관계와 전송 한도를 고정합니다. 자동 재시작 전에는 ARM 보강 수집과 생성기 버전도 고정해야 합니다. 순서 누락이나 청크 간 Resource 충돌은 차단합니다. 다시 읽기는 최종 종료
...
시점 일관성을 보장하지 않습니다. 토큰만으로 완전성, 부재 또는 공급자 내용의 불변성을 증명할 수
없습니다. 재개 시 관측 구간을 유지하고 전체 식별자 커버리지를 검증하며, 만료되거나 검증할 수 없는
연속 조회는 한도가 정해진 새 시도로 시작합니다. 정확한 원본 종료와 커버리지만 부재를 증명하며, 빈 페이지나 저장 버킷은 근거가 될 수 없습니다. 부분 결과의 승격은 계속 금지합니다.

준비 단계는 `collecting -> sealed -> verified -> prepared`이며 기존 변환 소유자만
`committed`로 게시합니다. 불변 청크 매니페스트는 범위, 소유권 epoch, 카탈로그 release,
관측 구간, 커버리지 및 의존 버전을 고정합니다. 조회와 페이지 이동은 검증된 파티션 버전
집합 하나를 고정하며 각 파티션의 최신값을 임의로 섞지 않습니다. 부분 후보는 완전한 활성
소유 집합을 교체하지 않습니다.
