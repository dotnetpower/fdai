...
translation_source_sha: 45161c33596450702e478978c311cc4b54cdbd36
...
| 불변 Pod 교체 상관 분석 | in-progress | `kubernetes_pod_replacement_evidence.py`, 집중 교체 축약기 테스트(`13 passed`) | 결정론적 축약기는 클러스터, 네임스페이스 및 루트 컨트롤러 UID를 통해 정확히 한 후보만 허용합니다. 같은 UID의 컨테이너 재시작, 같은 소유자의 Pod 교체 및 소유자가 바뀐 rollout을 구분하고 순서가 확인된 종료, 비정상 근거, 안정적인 desired replica, 최신 현재 상태 및 엄격한 Deployment 복구를 요구합니다. 이름으로 상관 분석하거나 원인 또는 실행 권한을 부여하지 않습니다. 아래 행의 영속 수명 주기 수집은 이제 구현되었습니다. 이 축약기 및 조회 구성으로의 연결은 아직 남아 있습니다. |
| 영속 Kubernetes 수명 주기 수집 | implemented | `core/ontology_platform/kubernetes_lifecycle_observation.py`, `delivery/kubernetes_lifecycle_source.py`, `delivery/kubernetes_lifecycle_collector.py`, `delivery/persistence/postgres_kubernetes_lifecycle.py`, `20260828_core_kubernetes_lifecycle.py` Core migration, `runtime/resource_event_providers.py`, `delivery/kubernetes_lifecycle_collector_cli.py`, 집중 observation·source·collector·CLI·provider 테스트(`68 passed`), migration-inventory 테스트(`58 passed`), hardening 집중 단위 테스트(live database 없이 `45 passed`, `5 skipped`) 및 live Postgres advisory-lock 동시성 테스트(ephemeral하고 보존하지 않는 로컬 인스턴스 대상 `5 passed`) | [이슈 #292](https://github.com/dotnetpower/fdai/issues/292)를 충족합니다: bounded resumable Event source가 클러스터별 durable resourceVersion cursor로 list 후 watch를 수행하고, cluster, namespace, object UID, 가능한 경우 owner UID, reason, event type, event time, recorded time, source revision을 보존하며, `reason`만(메시지 텍스트는 절대 사용하지 않음) killing/failed/backoff/unhealthy/successful_create/scheduled/started/deletion/other의 고정 vocabulary로 정규화하고, DB trigger가 insert-only로 강제하는 content-addressed observation을 추가하므로 collector는 ontology instance를 변경하지 않습니다. 중복 및 순서가 바뀐 delivery는 content-addressed `evidence_ref`로 idempotent합니다. store의 트랜잭션 cursor 검증은 cursor row가 이미 존재하는 이후로는 동시 writer가 상태를 되돌리는 것을 막으며, 클러스터별 `pg_advisory_xact_lock`이 durable cursor가 아직 없을 때(최초 수집 또는 410 이후 초기화) 두 writer가 모두 "cursor 없음"을 관측하고 둘 다 유일한 writer로 진행할 수 있었던 race도 이제 차단합니다. LIST pagination은 최대 8 page까지 완전히 소진하며, snapshot resourceVersion으로 cursor를 앞당기거나 소비되지 않은 `continue` page를 버리지 않고, 예산이 소진된 경우에만 명시적 `result_limit` gap을 보고합니다. WATCH는 byte/line 길이/UTF-8 경계를 위반한 첫 지점에서 멈추고 폐기된 envelope을 지나 조용히 계속하지 않습니다. Cursor expiry, provider outage, authorization failure(LIST와 WATCH 모두에서 Kubernetes API `401`/`403` 포함), truncation은 각각 명시적 gap을 반환합니다. cursor expiry만 durable cursor를 초기화하고, outage와 authorization gap은 cursor를 보존하여 일시적 장애가 재개 진행을 조용히 폐기하지 않도록 합니다. Deletion 탐지는 Event `reason` 값(`SuccessfulDelete`/`FailedDelete`/`Deleted`)으로 제한되며, owner가 그런 Event를 내지 않고 삭제된 object는 이 collector만으로는 관측되지 않습니다. `build_kubernetes_lifecycle_source`는 기존 on-demand Event reader와 동일한 `FDAI_KUBERNETES_*` credential binding을 재사용합니다. `kubernetes_lifecycle_collector_cli.py`는 주입 가능한 one-shot 실행 seam이며, 사용할 수 없었던 cluster-ref override를 받아들이는 대신 positional argument를 만나면 fail-fast합니다. 아직 Container Apps Job이나 Terraform schedule이 이를 호출하지 않고 이번 변경은 관측을 교체 축약기나 조회 구성에 연결하지 않습니다. Live cluster 근거는 수집하지 않았습니다. advisory lock은 ephemeral하고 보존하지 않는 로컬 Postgres 인스턴스(exercise 이후 제거됨)로만 검증했으며, durable하게 보존된 live Postgres 근거는 없습니다. |
...
- [ ] 구현된 [타입이 지정된 재개 가능 Kubernetes 수명 주기 수집](https://github.com/dotnetpower/fdai/issues/292)을 교체 축약기 및 조회 구성에 연결하고, 주입 가능한 CLI seam을 위한 Container Apps Job/Terraform schedule을 추가한 뒤 인증된 정확한 대상 교체 및 새 기준 시점 복구 증적을 보존합니다.
