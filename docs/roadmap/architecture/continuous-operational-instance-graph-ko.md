...
translation_source_sha: 4a3289e36edca6639e5333f02b70051f5ced0df0
...
게시 시 실제 객체 수정 번호를 포함한 내용 기반 증적도 함께 커밋합니다. 저장소 수준의 페이지 조회는 이후 게시에도 같은 다이제스트를 유지하고 선택된 그래프 청크만 읽습니다. 증적과 매니페스트 메타데이터에는 크기 한도가 있지만 고정 크기는 아닙니다. DB 재구성으로 내용이 사라지면 현재 행으로 대체하지 않고 차단합니다. 이 소유 하위 그래프 조회는 다른 소유자의 끝점 내용을 포함하지 않으며 호출자의 기존 권한 검사를 요구합니다. 게이트웨이 연결, 소유권 epoch, 보존 정책 및 짧은 포인터 게시는 아직 남아 있습니다. 기존 쓰기 테이블과 N-1 UPSERT 계약은 유지하며 부분 후보는 완전한 소유 집합을 교체하지 않습니다.
