...
translation_source_sha: 2113e16a711eb0073da3320146506a0550446a3c
...
> **Kubernetes lifecycle 조회 계약:** 보존된 Resource 이벤트 이력은 최신 durable collector cursor와 정확한 UID 범위가 있을 때만 사용할 수 있습니다. 형제 소유자로 범위를 넓히거나 다른 Pod의 이벤트를 다시 표시하지 않습니다. 교체 상관 분석은 아래의 별도 컨트롤러 기반 cohort를 사용합니다.
> collection completeness와 gap reason은 cursor와 함께 원자적으로 보존됩니다. incomplete LIST
> snapshot은 이전 cursor를 유지하고, 성공적인 no-op poll은 durable heartbeat를 갱신합니다.
| Kubernetes 워크로드 관측 | implemented | `kubernetes_api_inventory.py`, Kubernetes 이벤트 읽기 경로, rollout 및 Pod 복구 FunctionType, 집중 인벤토리, 이벤트, 메트릭, 축약기, 플래너, 증적, 조립 및 런타임 검사, 인증된 대상 없는 Console 증적과 정확한 대상 Console 증적 | UID에 근거한 세대는 허용 목록에 있는 rollout 상태를 보존합니다. `query.resource_event_history`는 검토된 유형 범위와 함께 출처에 근거한 정확한 Resource 이름 하나를 보존하고, secured ObjectSet 증적에 보존된 불변 `uid`와 `cluster_ref`로 정확한 child 하나를 좁힐 수 있습니다. Core는 이 두 field만 분리된 상태로 전달하고 adapter는 provider I/O 전에 canonical Resource id를 다시 계산합니다. 정확한 identity 조건식은 provider I/O 전에 Resource 하나로 해소되어야 하며 일치 항목이 없거나 여러 개면 `target_resolution_not_exact`로 불완전하게 유지합니다. 누락, redaction, malformed 또는 불일치 identity는 인증 전에 중단합니다. 삭제된 객체와 범위가 제한된 클러스터 이력에는 명시적인 정확한 클러스터 읽기를 계속 사용합니다. 혼합 범위는 적용할 수 있는 클러스터 이벤트를 `source_scope_incomplete`와 함께 보존합니다. 인코딩하지 않은 raw 응답은 JSON 구체화 전에 256 KiB로 제한하고 provider 항목은 최대 256개만 정규화합니다. 권위 있는 보존 출처가 생길 때까지 성공한 Kubernetes Event API 결과는 `source_retention_unverified`를 보존하고 일부만 읽었거나 미래 시각이거나 잘못됐거나 상한에 도달했거나 사용할 수 없는 읽기도 불완전하게 유지합니다. 이중 언어 답변은 정확한 제한 사항을 보여주며 행 0개를 과거 부재로 바꾸지 않습니다. 행이 답변 상한을 넘으면 표시 개수, 전체 개수 및 `display_truncated`를 보고하고 가장 최근 8개를 시간순으로 표시합니다. 범위가 제한된 현재 및 이전 컨테이너 종료 기록은 컨테이너 이름, 사유, 종료 코드, 신호 및 표준 시간대가 있는 완료 시각을 보존합니다. 원시 이미지 이름, 메시지, 런타임 id 및 프로바이더 페이로드는 제외합니다. Rollout 및 복구 축약기는 정확한 소유권과 재시작 변화량 근거를 요구하고 원인 및 실행 권한을 false로 유지하며 source-incomplete 행 0개를 부재로 취급하지 않습니다. 인증된 정확한 대상 계획과 제한 사항 표현 근거는 보존했으며 성공적인 Event 출처 접근과 영속 보존은 열린 상태입니다. |
| 불변 Pod 교체 상관 분석 | implemented | `kubernetes_pod_replacement_evidence.py`, `kubernetes_pod_lifecycle_cohort_queries.py`, 영속 Pod identity 및 cohort adapter, 집중 축약기, 조회, 조립, 표시 격리 및 migration 검사 | 완전한 Kubernetes inventory snapshot은 Event `related.uid`와 별도로 변경할 수 없는 Pod→컨트롤러→루트 컨트롤러 identity를 저장합니다. 정확한 대상 계획은 이 identity를 통해 범위가 제한된 typed cohort를 읽고 일반 Resource 이벤트 표시는 정확한 UID만 유지합니다. 축약기는 과거 UID 하나와 정확한 현재 후보를 허용하고 두 근거 경로를 보존하며 누락, 모호성, 오래됨 또는 불완전한 계보를 안전하게 차단합니다. 실제 정확한 대상 교체 근거는 아직 남아 있습니다. |
| 영속 Kubernetes 수명 주기 수집 | implemented | lifecycle observation/source/collector, PostgreSQL 이벤트 및 Pod identity store, 정확한 Resource-event adapter, typed cohort reader, shared analyzer schedule 및 집중 테스트 | 범위가 제한된 재개 가능 Event source와 append-only cursor store가 UID 기반 lifecycle 관측을 보존합니다. 완전한 inventory snapshot은 컨트롤러 기반 Pod identity를 별도로 추가하며 cohort 읽기는 보존된 UID로만 이벤트를 결합합니다. 각 LIST page는 같은 256개 항목 상한을 요청하고 모두 소비하며 atomic store는 8개 page 전체 poll 상한을 수용합니다. Cursor, identity 또는 history가 없으면 명시적인 incomplete 근거로 남습니다. Live cluster 및 live PostgreSQL 근거는 주장하지 않습니다. |
...
| 날짜 | 상태 | 변경 | 근거 | 남은 작업 |
|------|------|------|------|-----------|
| 2026-08-28 | implemented | 현재 Pod 재시작 결론과 교체 불확실성을 분리했습니다. 수명 주기 및 교체 미비점은 전용 교체 근거 경로에 남고, 운영 Pod ID는 UID에서 도출되므로 컨트롤러 전체 코호트가 이전 Pod를 자동으로 식별할 수 없습니다. 정확한 Pod 진단은 현재 UID의 범위가 제한된 영속 `reason` 값을 사용하고 사유 또는 참조 상한을 넘으면 명시적인 불완전 근거를 반환합니다. | `current change`; 집중 Pod 복구, 코호트, 진단, 계획, 런타임 및 조립 검사(`394 passed`); Ruff 및 strict mypy | 자동 교체 검증을 활성화하기 전에 독립적으로 근거가 확인된 이전 Pod 결속을 추가해야 합니다. 실제 공급자는 조회하지 않았습니다. |
...
| 2026-08-28 | implemented | 정확한 대상 Resource 이벤트 표시와 Pod 교체 cohort를 분리했습니다. 완전한 inventory snapshot은 변경할 수 없는 Pod 컨트롤러 계보를 추가하고 전용 FunctionType은 해당 계보를 통해서만 과거 UID 이벤트를 읽습니다. 일반 이벤트 reader는 반환된 모든 행을 요청한 UID로 다시 필터링합니다. | `current change`, 집중 identity, cohort, 축약기, semantic plan, 조립, 표시 격리, migration inventory, Ruff 및 strict mypy 검사, worktree 전용 로컬 Docker database에서 PostgreSQL lifecycle 검사 7개 모두 통과 | Live cluster 및 인증된 정확한 대상 교체 증적은 별도로 보존합니다. 로컬 Docker 근거는 배포 근거가 아닙니다. |
...
| 2026-08-27 | implemented | lifecycle 기반 교체 축약기 진입점을 추가했습니다. 정확한 이전 Pod UID에 해당하는 종료 근거만 선택하고 보존된 모든 근거 참조를 유지한 뒤 기존의 엄격한 소유권 및 복구 검사를 호출합니다. | `current change`, `kubernetes_pod_replacement_evidence.py`, 집중 교체 테스트(`16 passed`), Ruff 및 strict mypy 통과 | 인증된 exact-target 교체 및 복구 증적을 보존해야 합니다. live 근거는 만들지 않았습니다. |
...
| 2026-08-28 | implemented | `graph_at`의 bitemporal topology replay completeness와 digest 무결성을 강화했습니다. Completeness는 이제 선택된 모든 revision batch가 null이 아닌 source receipt digest를 가지고 있을 것도 요구하므로, 출처 receipt가 없는 batch는 더 이상 complete graph로 보고될 수 없습니다. Replay digest는 이제 응답이 반환하는 것과 동일한 canonical하고 중복 제거된 source receipt digest tuple을 해시하며, 반환 필드와 어긋날 수 있던 별도의 raw list를 더 이상 사용하지 않습니다. | `current change`; `core/ontology_platform/topology_history.py`; 집중 topology-history 검사(`8개 통과`), 이전 digest/필드 불일치를 재현한 fix-되돌리기 회귀 실행 포함 | 배포된 replay 근거는 별도로 보존합니다. 현재 모든 실제 운영 writer는 이미 각 batch에 source receipt digest를 설정합니다. |
...
- [x] 영속 [타입이 지정된 재개 가능 Kubernetes 수명 주기 수집](https://github.com/dotnetpower/fdai/issues/292)을 정확한 Resource 이벤트 표시, 컨트롤러 기반 교체 cohort 구성 및 공통 local/deployed analyzer schedule에 연결했습니다. 수집, identity capture 또는 lifecycle 근거를 사용할 수 없으면 보존 행은 incomplete로 남습니다. 인증된 exact-target 교체 및 새 기준 시점 복구 증적은 아직 남아 있습니다.
