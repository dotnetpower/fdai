...
translation_source_sha: 9f37aa9460f4e8e5478d287f5cb7b5bdba139f78
...
이전 절의 한도를 확장하는 다음 계약은 수집 분할과 그래프 소유권을 분리합니다. 공급자 페이지는
정규화와 민감정보 제거 후 영속 준비 영역에 들어가며, 내용 다이제스트와 재개 지점을 함께
...
보존하지 않지만 정규화된 세대는 여전히 한도 안에서 보존하며 공급자 연속 조회의 재개 연결은
별도 구현 작업으로 남아 있습니다.
조정기는 기존 PostgreSQL 후보 작성기를 통해 Resource 전용 청크를 준비합니다. 청크는 최대
1 MiB 및 Resource 1,000개이며 불변 본문, 다이제스트 및 연속 조회 재개 지점을 후보 행과 함께
커밋합니다. 원본, 범위, 유형 및 수집 메타데이터는 맥락 다이제스트에 결속됩니다. 동일 청크의
재전송은 아무것도 바꾸지 않으며 순서 누락, 내용 변경 및 청크 간 Resource 충돌은 차단합니다.
다시 읽기는 순서가 있는 전체 다이제스트 체인을 청크별로 검증하며 최종 원본 종료 경계를
생성하지 않습니다. 호출은 30초로 제한하고 기존 30분 수집 수명을 벗어난 시도는 재개하지
않습니다. 분할된 배치의 마지막 청크에서만 커서를 전진시킵니다. 이 증적은 승격, 삭제, 범위
변경 또는 자동 공급자 재시작을 허용하지 않으며 해당 경로를 연결하려면 관계와 최종 커버리지의
재생도 필요합니다. 정규 청크는 비공개로 저장하고 환경변수의 민감정보를 미리 제거해야 합니다.
