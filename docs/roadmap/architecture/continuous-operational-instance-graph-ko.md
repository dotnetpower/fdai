...
translation_source_sha: 242a2a09b4578f6a0d44439d81bb6e72d4038b1d
...
- **배포 근거:** 보호된 플랫폼 계획 메타데이터는 전용 저장소 모듈에서 생성합니다. 워크플로
  ...
  이후 계획은 롤백된 이미지와 원본 수정본을 두 원본 수정본이 모두 유효한 커밋이고 새로 고친
  원본 바인딩이 계획의 `before` 상태와 정확히 일치할 때만 복구 메타데이터로 처리합니다. 고정
  Heimdall 복구 관찰자는 명시적 Core 근거 전환을 통해 최초로 채택할 때만 추가할 수 있으며,
...
- **안전한 보강:** 실시간 조회는 현재 답변을 지원할 수 있으며 같은 ingress를 통해 타입이
  ...
  이름과 값을 제거하며, 일반 컨테이너와 초기화 컨테이너 모두에 적용합니다.
- **시간과 출처:** 모든 사실은 유효 시간, 가능한 경우 프로바이더 이벤트 시각, 관측 시각,
  ...
  이벤트 시각을 덮어쓰지 않습니다. ARG 스냅샷은 파싱 시각 대신 보수적인 읽기 시작 시각을 사용합니다. 상태 관측 시각이 없거나 세대 기록 시각보다 미래인 근거는 불완전하게 유지합니다. 토폴로지 사실의 유효 시각은 기준 세대의 가시화 시각과 구분하고, 재생은 각 사실의 원래 최신성 예산을 보존합니다.
- **잘못된 부재 방지:** 누락 이벤트, 잘린 조회, cursor 지연, 열린 실시간 overlay, archive
  사용 불가는 명시적인 알 수 없음 또는 불완전한 근거로 유지합니다. 변환 한도를 넘은 후보는 승격하거나 삭제를 확정할 수 없으며, 원장은 DB 접근 전에 불완전한 스냅샷 입력을 거부합니다.
  공급자 유형 집계 결과가 비어 있으면 빈 구독과 공급자 객체를 볼 수 없는 신원을 구분할 수
  없으므로 전체 공급자 범위를 입증할 수 없습니다.
...
유지합니다.
변경 스트림 행은 ID에서 파생한 유형과 정규화한 공급자 제공 유형이 모두 검토된 ResourceType
...
ARM replica 검증과 짝 완전성을 평가하기 전에 정확한 엔드포인트 쌍별 최신 관측을 선택합니다. 더 새로운
완전한 쌍은 이전의 짝이 없는 관측을 대체하지만, 더 새로운 짝이 없는 관측은 60초 후행 유예
구간 동안 대기 상태로 남은 뒤 원본을 불완전하게 만듭니다. 원본은 최신성 구간보다 유예 구간
...
순수 delivery helper에 유지하므로 모듈 분리는 cursor 진행, 완전성 또는 쓰기 권한을 바꾸지 않습니다. 관측 레코드 보조 모듈 `postgres_inventory_observation_records.py`는 `confirmed_tombstone`, `mapping`, `observation_from_row`, `snapshot_records`만 공개합니다.
...
단일 세대는 Resource 50,000개, 관계 또는 제외 기록 200,000개, 정규화된 수집 레코드 16 MiB까지만 허용합니다. 한도 초과는 최종 종료 경계 전에 중단하며 이전 세대를 유지합니다. 더 큰 범위는 별도로 검토한 분할 소유권이 필요합니다. 이 수치는 프로세스 전체 메모리 한도가 아니며 전송 중인 버퍼에는 별도 한도가 적용됩니다. 용량 오류로 활성 범위를 임의로 줄이거나 새로 승인된 소유 범위 밖의 삭제를 허용하지 않습니다. 구현 원장에는 새 프로세스의 합성 RSS, 초기 게시와 재생의 잠금 점유 및 재시작 측정값을 보존합니다. 현재 영속 준비는 비용을 늘리며 아직 게시 지연을 개선한 상태가 아닙니다.
매니페스트는 점진적 해시 계산 중 32 MiB 한도를 확인합니다. PostgreSQL은 교체 쓰기를 배치 처리하고 같은 내용의 수정 번호를 유지하며 끝점 집합으로 관계 다중성을 검증합니다. 교체 기한 60초로 전역 단일 작성자 잠금 점유를 제한하되 작성자 간 다중성을 위해 잠금 자체는 유지합니다. 외부 소유 관계가 있으면 객체 삭제를 거부합니다. 객체 조회는 관계도 16,000개로 제한하고 잘림을 표시합니다. 무효화 커밋은 별도의 단조 증가 커서 하한을 저장해 손상된 표식을 복구하며, 하한이 없는 이전 손상은 명시적인 복구가 필요합니다.
엄격한 전달 완료 표식의 형식과 해시 검증으로 손상된 완료 기록이 전달을 억제하지 못하게 합니다. 잘린 속성은 완전한 스테이징, 저널 및 Resource Event에 들어갈 수 없으며, 스냅샷에는 최종 검증된 관계 집합만 기록합니다. 관계 근거가 부족하면 토폴로지는 불완전하게 유지하되 검증된 객체의 전달까지 억제하지 않습니다.

### 준비와 게시를 분리하는 후속 계약

수집 분할은 그래프 소유권과 분리합니다. 비공개 준비 영역은 정규화와 민감정보 제거를 요구하며, 같은 내용의 재생은 아무것도 바꾸지 않고 같은 식별자의 다른 내용은 차단합니다. 원본 종료 정리는 전체 제한 시간이나 취소 뒤에도 별도 제한 시간을 적용합니다. 한도는 최대 5초이며 설정된 진행 제한 시간이 더 짧으면 그 값을 사용합니다. 종료 실패가 취소를 가리거나 최종 fence 뒤의 승격을 허용해서는 안 됩니다.
ARG는 이전 원본 행을 보존하지 않고 다음 페이지 요청 전에 정규화하며, 정규화된 세대만 한도 안에서 보존합니다. shard 실패나 세대 한도 소진 뒤에는 대기 중인 shard의 공급자 조회를 시작하지 않으며, 진행 중인 요청은 제한된 취소 절차로 정리하고 최종 종료 경계를 내보내지 않습니다. 미완료 공급자 조회의 재개에는 식별자, 관계 및 최종 커버리지의 재생 연결이 필요합니다.

조정기는 기존 PostgreSQL 작성기를 통해 최대 1 MiB 및 Resource 1,000개의 전용 청크를 준비합니다.
후보 행, 불변 청크, 다이제스트 및 재개 지점을 함께 커밋합니다. 맥락은 원본, 범위, 유형, 메타데이터 및 유효 구성 다이제스트에 결속합니다. 이 다이제스트는 정책, 관리 대상과 인증 대상, 요청 속도, vocabulary 매핑 및 서명된 대체 입력을 고정합니다.
생산자 다이제스트는 ARM 보강 수집을 포함한 설치된 FDAI Python 소스도 고정하며, 소스 변경 뒤에는 프로세스를 다시 시작해야 합니다. 추가 ARG 계약 다이제스트는 생성 쿼리, 검토된 관계와 전송 한도를 고정합니다. 순서 누락이나 청크 간 Resource 충돌은 차단합니다. 다시 읽기는 최종 종료
경계를 만들지 않고 전체 체인을 청크별로 검증합니다. 호출은 30초, 수집 시도는 30분으로 제한합니다.
분할된 배치의 마지막 청크에서만 커서를 전진시킵니다. 증적은 승격, 삭제, 범위 변경 또는 자동
공급자 재시작을 허용하지 않습니다. 재개 지점 `1.1.0`은 누적 개수를 포함한 모든 필드를 다이제스트에
결속합니다. 후속 쓰기는 재개 지점과 마지막 청크를 검증합니다. 이전 `1.0.0`은 전체 체인을 검증한
경우에만 이어 쓰며, 쓰기가 성공할 때 새 형식으로 전환합니다.

원본 수집과 보강을 마친 후보는 정확한 준비 행, 원래 관측 시각, 커버리지, 원본 상태, 전체 매핑 근거 및 예상 활성 기준 세대를 봉인합니다. 그래프 내용과 봉인 레코드는 각각 32 MiB로 제한합니다.
새 프로세스는 동일한 실제 수집 맥락에서 만료되지 않은 단일 봉인 후보를 공급자 재조회나 재보강 없이 재개할 수 있습니다. 봉인이 없거나 충돌하거나 검증되지 않으면 부분 승격을 허용하지 않습니다.
봉인 뒤에는 추가 준비 쓰기를 차단합니다. 승격 시 DB의 원래 시작 시각, 맥락, 봉인, 그래프 내용 및 기존 활성 기준 세대를 다시 검사하며, 거부된 후보는 활성 포인터를 바꾸지 않습니다.

[ARG 페이지 조회](https://learn.microsoft.com/en-us/azure/governance/resource-graph/concepts/paging-results)는
시점 일관성을 보장하지 않습니다. 토큰만으로 완전성, 부재 또는 공급자 내용의 불변성을 증명할 수 없습니다. 재개 시 관측 구간을 유지하고 전체 식별자 커버리지를 검증하며, 만료되거나 검증할 수 없는 연속 조회는 한도가 정해진 새 시도로 시작합니다. 정확한 원본 종료와 커버리지만 부재를 증명하며, 빈 페이지나 저장 버킷은 근거가 될 수 없습니다. 부분 결과의 승격은 계속 금지합니다.

준비 단계는 `collecting -> sealed -> verified -> prepared`이며 기존 변환 소유자만 `committed`로 게시합니다. 인벤토리 세대에 결속된 게시 경로는 그래프 작성자 잠금 전에 정규화된 불변 교체 입력을 영속 저장합니다.
각 청크는 최대 1 MiB 및 레코드 1,000개이며, 그래프 내용과 매니페스트는 각각 32 MiB로 제한합니다. 매니페스트는 청크 다이제스트, release, 세대, 이전 소유 식별자, 수정 번호, 상태 갱신 및 변환 워터마크를 고정합니다.
준비 시 삭제 대상과 외부 끝점의 수정 번호를 하나의 읽기 스냅샷으로 고정하고 작성자 잠금 전에 타입이 지정된 입력을 복원합니다. 게시 시 서버에서 저장된 JSON 텍스트의 정확한 일치를 확인하고 해당 행을 잠그며, 의존 행과 세대, 수정 번호 및 다중성을 다시 검증하고 준비 참조를 그래프 및 상태 변경과 함께 커밋합니다. 준비 당시 없던 행은 삭제 대상에서 제외해 이후 다른 작성자의 생성을 보호합니다. 여전히 기존 잠금 안에서 행을 교체하므로 짧은 포인터 전환은 아닙니다.
게시 시 실제 객체 수정 번호를 포함한 내용 기반 증적도 함께 커밋합니다. 저장소 수준의 페이지 조회는 이후 게시에도 같은 다이제스트를 유지하고 선택된 그래프 청크만 읽습니다. 증적과 매니페스트 메타데이터에는 크기 한도가 있지만 고정 크기는 아닙니다. 현재 스캔은 해당 읽기 안에서만 검증된 메타데이터를 재사용하며 다른 연결이나 다이제스트에서는 거부합니다. DB 재구성으로 내용이 사라지면 현재 행으로 대체하지 않고 차단합니다. 조건 없는 Resource 전용 현재 조회는 하나의 읽기 트랜잭션에서 현재 매니페스트와 상태, 커버리지 및 실제 식별자, 수정 번호, release의 일치를 확인한 뒤 커밋된 페이지를 선택합니다. 기존 게이트웨이의 principal 및 속성 권한 검사는 유지합니다. 조건 검색, 다른 ObjectType 및 커밋 포인터가 없는 구형 현재 조회는 기존 경로를 유지하지만 명시적 버전 고정 조회는 대체하지 않습니다. 소유권 epoch, 보존 정책, 완전한 의존 스냅샷 및 짧은 포인터 게시는 아직 남아 있습니다. 기존 테이블과 N-1 UPSERT 계약은 유지하며 부분 후보는 완전한 소유 집합을 교체하지 않습니다.

준비는 병렬화할 수 있지만 초기 게시 권한은 단일 작성자에게 유지합니다. 게시 시 기준 세대와
fencing token을 다시 검증하고 활성 그래프 참조, 관측 재개 지점, 무효화 및 전달 의도를 함께
확정합니다. Resource 이벤트 전달은 전역 표식 하나를 덮어쓰지 않고 세대별 식별자를 사용합니다.
broker 결과가 불확실하면 동일 이벤트 식별자로 재시도하며 규칙 평가 완료로 주장하지 않습니다.
...
새 계약은 내부 읽기 모델 메타데이터이며 관리 리소스의 실행 권한을 부여하지 않습니다.
구현 원장은 계획된 계약과 실제 연결되어 검증된 경로를 구분하며 배포 완료를 주장하지 않습니다.
...
아무것도 진술하지 않으므로 자신의 객체 커버리지를 그대로 유지합니다. 분류되지 않거나 잘못된 관계 후보는 제외하고 관계 완전성을 낮추되 독립적으로 검증된 객체는 갱신합니다.
부분 원본 세대, 객체 충돌 또는 저장된 관계의 다중성 위반은 계속 차단되며 이전
...
검증기 버전 `inventory-generation-verifier.v3`는 검토된 카탈로그도 다시 검증합니다. 중복 연결, 근거 없는
역방향, 자기 참조 연결은 계속 제외하고 관계 완전성을 낮춥니다.
...
릴리스 독립적 content digest가 전환 중에도 같은 generation의 변조 감지를 유지합니다. 정확한 source replay 명령은 snapshot loader가 활성 pointer는 전진했지만 이전 manifest는 전진하지 않았음을 입증한 경우에만 pending 활성 generation 하나를 사용할 수 있습니다. 이 명시적 경로는 원자적인 활성 generation 및 content 검사를 재사용합니다. 일반 replay는 계속 generation 변경을 거부하며 프로바이더 읽기나 관리 리소스 변경은 발생하지 않습니다.
보존된 관계 분류가 새 release에서 해당 replay를 충족할 수 없으면 자동 복구는 계속 차단됩니다. 운영자가 명시적으로 요청한 전체 조정은 pending 근거를 보존하고 현재 release에서 새로운 프로바이더 수집으로 진행할 수 있지만 반복 실행 루프는 이 예외를 활성화할 수 없습니다.
