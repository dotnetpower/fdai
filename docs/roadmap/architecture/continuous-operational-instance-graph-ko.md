...
translation_source_sha: e01883e5c0620314a17db6983bac3249bd277b46
...
게시 시 실제 객체 수정 번호를 포함한 내용 기반 증적도 함께 커밋합니다. 저장소 수준의 페이지 조회는 이후 게시에도 같은 다이제스트를 유지하고 선택된 그래프 청크만 읽습니다. 증적과 매니페스트 메타데이터에는 크기 한도가 있지만 고정 크기는 아닙니다. DB 재구성으로 내용이 사라지면 현재 행으로 대체하지 않고 차단합니다. 조건 없는 Resource 전용 현재 조회는 하나의 읽기 트랜잭션에서 현재 매니페스트와 상태, 커버리지 및 실제 식별자, 수정 번호, release의 일치를 확인한 뒤 커밋된 페이지를 선택합니다. 기존 게이트웨이의 principal 및 속성 권한 검사는 유지합니다. 조건 검색, 다른 ObjectType 및 커밋 포인터가 없는 구형 현재 조회는 기존 경로를 유지하지만 명시적 버전 고정 조회는 대체하지 않습니다. 소유권 epoch, 보존 정책, 완전한 의존 스냅샷 및 짧은 포인터 게시는 아직 남아 있습니다. 기존 테이블과 N-1 UPSERT 계약은 유지하며 부분 후보는 완전한 소유 집합을 교체하지 않습니다.
