...
translation_source_sha: 1e57a1237cde7ef882e46aa5fb6c2e937cd552cd
...
| Runtime-call 근거 기반 | implemented | `runtime_calls.yaml`, `runtime_call_projection.py`, 집중 catalog 및 endpoint matrix 검사 | 순수 projector는 fresh, exact, active-generation, principal-readable endpoint에 대해서만 검증된 caller-to-target candidate를 반환합니다. Producer, single-writer binding, 인증된 runtime 근거는 주장하지 않습니다. |
...
- [ ] Role assignment 225개, scope attachment 185개, Managed Identity attachment 106개로
  검증된 Azure 범위를 넘어 authorization을 완료합니다. [Issue #260](https://github.com/dotnetpower/fdai/issues/260)은
  검토된 unavailable child scope와 별도의 타입 지정 및 principal-safe PostgreSQL role 근거를
  요구하며, RBAC grant와 database credential은 일반 Resource `depends_on` edge가 되지 않습니다.
...
- [ ] [타입 지정 runtime-call telemetry producer와 single writer를 binding하고](https://github.com/dotnetpower/fdai/issues/260)
  인증된 `8010`/`5273` 근거를 보존합니다. 이름, group, 환경 변수, credential, RBAC 기반 endpoint
  추론은 계속 거부합니다.
