...
translation_source_sha: 0a589f2df6562d5147df41608d92291a5fc10ed7
...
> **Kubernetes lifecycle 조회 계약:** 보존된 history는 fresh durable collector cursor와 exact UID scope가 있을 때만 사용할 수 있습니다. adapter는 256행 출력 한도보다 하나 더 sentinel 행을 조회하고 stale, incomplete, empty 또는 capped history를 낮춥니다. 이벤트 부재를 복구로 바꾸지 않습니다.
...
| 불변 Pod 교체 상관 분석 | in-progress | `kubernetes_pod_replacement_evidence.py`, 집중 교체 축약기 테스트(`13 passed`), durable lifecycle Resource-event adapter 및 semantic plan 검사 | 결정론적 축약기는 클러스터, 네임스페이스 및 루트 컨트롤러 UID를 통해 정확히 한 후보만 허용합니다. 같은 UID의 컨테이너 재시작, 같은 소유자의 Pod 교체 및 소유자가 바뀐 rollout을 구분하고 순서가 확인된 종료, 비정상 근거, 안정적인 desired replica, 최신 현재 상태 및 엄격한 Deployment 복구를 요구합니다. 보존된 lifecycle 관측은 기존 Resource event query dependency로 전달되며 원인 또는 실행 권한을 부여하지 않습니다. live exact-target 교체 근거는 아직 남아 있습니다. |
| 영속 Kubernetes 수명 주기 수집 | implemented | lifecycle observation/source/collector, PostgreSQL store, durable Resource-event adapter, shared analyzer schedule 및 집중 테스트 | [이슈 #292](https://github.com/dotnetpower/fdai/issues/292)를 충족합니다. bounded resumable Event source와 append-only cursor store가 UID 기반 lifecycle 관측을 보존합니다. 각 LIST page는 같은 256개 항목 상한을 요청하고 모두 소비하며, atomic store는 8개 page 전체 poll 상한을 수용합니다. adapter는 cursor가 존재한 뒤에만 보존 행을 사용하며 cursor 없음 또는 빈 history를 명시적인 incomplete 근거로 반환합니다. 동일한 analyzer tick이 server-owned binding이 있을 때 local과 Container Apps Job에서 선택적인 lifecycle pass를 실행하고, incomplete 근거는 bounded retry를 위해 tick을 실패시킵니다. Live cluster 근거는 수집하지 않았습니다. |
...
- [x] 구현된 [타입이 지정된 재개 가능 Kubernetes 수명 주기 수집](https://github.com/dotnetpower/fdai/issues/292)을 Resource-event 조회 구성과 공통 local/deployed analyzer schedule에 연결했습니다. 수집이 시작되지 않았거나 lifecycle 행이 없으면 보존 행은 incomplete로 남습니다. 인증된 exact-target 교체 및 새 기준 시점 복구 증적은 아직 남아 있습니다.
