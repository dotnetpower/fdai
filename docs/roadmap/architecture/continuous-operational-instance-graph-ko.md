...
translation_source_sha: 09a929c52850320289eab6deaa251d4e2acf7fda
...
| Graph-first query | in-progress | 순수 refresh reducer가 권한 없이 문서화된 5개 결과를 모두 선택하지만, 일반 semantic query composition은 실행 전에 이를 호출하지 않습니다. |
| 범위가 제한된 live read | in-progress | Resource-state shadow read와 정식 live-evidence writer가 존재하지만, 일반 semantic query는 refresh 결정을 통해 이를 연결하지 않습니다. |
| Live evidence write-through | in-progress | `InventoryLiveEvidenceWriter`가 정식 observation ingress를 통해 안전한 부분 overlay를 내보냅니다. Runtime query composition은 아직 남아 있습니다. |
...
| 그래프 우선 조건부 실시간 보강 | in-progress | `graph_evidence_refresh.py`, `inventory_live_evidence.py`, 부분 overlay 영속성, 집중 테스트 | 순수 결정과 안전한 write-through 경로를 구현했습니다. 일반 semantic query composition은 아직 이를 종단으로 연결하지 않습니다. |
| 운영 및 semantic 인증 | in-progress | `operational_instance_competency.py`, direct golden campaign runner, 집중 및 실제 운영 근거 | 대표 typed competency가 답변 text 비교 없이 통과합니다. 정식 live campaign은 action-draft frame 분류에서 차단됐습니다. |
...
| 애플리케이션 중심 공급자 관계 | in-progress | `azure-arg-v1.yaml`, ARG와 범위가 제한된 ARM child 수집, 완전 세대 alias 변환, endpoint closure, snapshot 분류 metadata, 집중 Core/Operator/Console 검사, Terraform service-root 검사, 인증된 `5273` 근거 | 검토된 mapping 75개가 정확한 containment, identity, authorization, registry, observability, network, ingress, 구성된 data-service reference, Container Apps Key Vault secret reference, Private DNS closure 및 AKS node Resource Group ownership을 처리합니다. 다섯 runtime service Terraform root는 secret DSN binding과 함께 secret이 아닌 `POSTGRES_HOST`를 노출합니다. 최신 권위 있는 로컬 refresh는 Resource 832개와 link 1,519개를 promotion했고 `properties.nodeResourceGroup`은 완전 세대 전체에서 Resource Group 이름 하나와 고유하게 일치할 때만 해석했습니다. Container App Job 3개는 관측된 `properties.hostName`이 구성된 endpoint URI와 고유하게 일치한 뒤 Communication Service 하나로 향하는 정확한 `depends_on` edge를 변환합니다. Registry 구성이 있는 Container App 9개와 Container App Job 11개 모두 정확한 `workload -> depends_on -> registry` edge를 하나 이상 변환합니다. Registry 5개가 서로 다른 workload dependency 21개를 받고 fan-in은 1개부터 16개입니다. Consumer에서 dependency로 향하는 방향은 의도된 것이며 근거 없는 image-supply edge로 뒤집지 않습니다. Snapshot metadata는 mapping id, source property path, provider-native endpoint type, candidate multiplicity, mapping별 unavailable reason을 보존합니다. 남은 candidate 66개는 모두 `target_outside_active_generation`, `target_provider_type_unmodeled`, `reference_not_observed`로 분류되므로 inventory ontology 변환 결과는 이전 완전 하위 그래프를 교체하지 않고 unavailable 상태를 유지합니다. Runtime call과 배포된 database host 근거는 열려 있습니다. |
...
- [ ] `OI-11`은 keyword routing 없이 action-draft frame 분류를 수정한 다음 영어와 한국어
  direct case 70개를 pressure signal 없이 완료하고 typed no-authority 증적을 보존합니다.
...
- [ ] Role assignment 225개, scope attachment 185개, Managed Identity attachment 106개로
  검증된 Azure 범위를 넘어 authorization을 완료합니다. 관측되지 않은 Azure child scope에
  검토된 unavailable 상태를 부여하고 별도의 PostgreSQL database-role 근거를 보존해야 합니다.
  RBAC grant와 database credential은 일반 Resource `depends_on` edge가 되지 않습니다.
- [ ] 하나의 telemetry observation이 caller와 target Resource ID를 모두 입증할 때만 runtime
  call edge를 변환합니다. Positive, missing-caller, missing-target, ambiguous-target, stale,
  wrong-scope 테스트와 인증된 `8010` 및 `5273` source evidence가 필요합니다. Resource Group과
  해석되지 않은 이름을 사용한 추론은 계속 거부합니다.
