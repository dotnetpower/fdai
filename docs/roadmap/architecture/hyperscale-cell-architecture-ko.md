...
되고, 런타임은 수평 샤딩된 셀이 되며, 감사 경로는 락-프리 쓰기 평면과 비동기
...
구성). 둘 다 셀 모델, 두 로그 평면, CQRS 감사 원장, 런타임 계약, 모든 안전
...
| LLM (T2) | 모든 프로바이더([llm-strategy-ko.md](llm-strategy-ko.md) 예산 내) | **리전 내만**: Azure OpenAI KC 또는 AKS GPU 위 self-host 오픈모델 |
| 키 | Microsoft-관리 또는 CMK | **CMK**; 국방 / 금융은 Managed HSM(FIPS 140-2 L3) |
| 네트워크 | 공개 + Private Endpoint 선택 | **모든 곳 Private Endpoint**; 공개 접근 차단; 폐쇄 VNet |
...
|------|------|-----------|------|--------------|
| **감사 원장 (L0)** | 최종 결정마다 1개(`execute` / `hil` / `deny` / `abstain` / `dedupe`) | *결정* 수 (상관관계 후 작음) | 해시체인 원장 (Kafka + Postgres/ADLS) | **절대 안 됨.** 안전 불변식: 모든 자율 동작이 감사 1줄 |
...
런타임 환경, 즉 modular 코어 앱 하나, 분리된 Operator API와 인제스트 게이트웨이 앱,
...
- **블라스트-반경 격리:** 실패하거나 포화된 셀은 자기 랜딩존 그룹만 저하시킨다.
 ...
 가 다른 셀을 건드리지 않고 그 셀만 그림자 로 캡한다.
...
단일 Event Hubs Standard 네임스페이스는 약 **40k 이벤트/s**(40 TU x 1k
...
현재 `PostgresStateStore.append_audit_entry` 는 덧붙이기 마다 커넥션을 열고
**글로벌 참고용 락** 하나를 잡아 단일 선형 해시 체인을 확장한다 - 직렬 단일-
쓰기 담당 천장이다. 초대규모에서 이것이 가장 먼저 터진다. B안은 쓰기와 인덱스를
분리한다(CQRS).
...
| 리소스 | 상태 | 최소비용 -> B안 | 계약 |
|--------|------|-----------------|------|
| Azure 데이터 Explorer (ADX) 클러스터 | NEW | 없음 -> 텔레메트리 + 감사 인덱스 / 조회 엔진 | 6-8 (텔레메트리 인제스트) |
| Event Hubs (셀별 샤딩) | 변경 | Standard 1 TU 단일 -> 셀별 네임스페이스, 필요시 Dedicated CU | 1 (이벤트 버스) |
| Container Apps 환경 (셀별) | 변경 | 단일 env -> 셀당 env 1개(KEDA scale-to-zero) | 2 (런타임) |
| ADLS Gen2 | NEW | 없음 -> 감사 오프로드 + Merkle 체크포인트 + warm 티어 | 객체 저장소 |
| Azure Policy initiatives (DeployIfNotExists) | NEW | 없음 -> 300 구독 fan-in 자동화 | 거버넌스 (policy-as-code) |
| 관리 그룹 계층 + 할당 | NEW | 없음 -> 자동 온보딩용 정책 스코프 | - |
| Azure Lighthouse (선택) | NEW | 없음 -> cross-tenant 위임 온보딩 | 4 (워크로드 신원) |
| Log Analytics (Dedicated 클러스터) | 변경 | PAYG workspace -> 리전 dedicated 클러스터 (commitment 계층) | 6-8 |
| PostgreSQL Flexible (승급 + 파티셔닝) | 변경 | B1ms 단일 -> general-purpose, pg_partman + BRIN | 5 / 상태 저장소 |
| User-assigned Managed Identity (셀별) | 변경 | 단일 -> 셀별 스코프 실행 아이덴티티 | 4 (워크로드 신원) |
| Key Vault Managed HSM (`sovereign`) | NEW | 없음 -> 국방 / 금융용 FIPS 140-2 L3 CMK 보관 | 3 (시크릿) |
| AKS 클러스터 (`sovereign` / heavy 셀) | NEW | Container Apps -> self-host 나 노드 제어가 필요한 곳에서 AKS | 2 (런타임) |
| Private Endpoints + 폐쇄 VNet (`sovereign`) | NEW | 공개 -> 모든 서비스에 Private Endpoint, 공개 접근 차단 | - |
...
| 리전 내 LLM (`sovereign`) | NEW | 모든 프로바이더 -> Azure OpenAI KC 또는 AKS GPU 위 self-host 모델 | - |
...
| OTel 내보내기 도구 분기 | 변경 | 텔레메트리-평면 tail-sampling + 롤업 |

### 프로바이더 경계 (DI 추가 - `core/` 불변)

| 경계 | 상태 | 계약 |
|------|------|------|
| `AuditLedger` (덧붙이기 / 기준점 / verify) | NEW | 쓰기 평면; 조회와 CQRS 분리 |
| `AuditQueryStore` (읽기 전용) | NEW | ADX / Postgres 조회 어댑터 |
| `CellRouter` | NEW | 신호 -> 셀 배정 |
| `StateStore` | 변경 | 파티션 + BRIN 구현; 프로토콜 불변 |
| `Inventory` | 변경 | 300 구독 전반의 Azure Resource Graph |
...
- **안전 쪽으로 실패:** 포화된 셀은 `DegradationController` 를 통해 그림자 로
...
초대규모 프로파일 모두(KEDA scale-to-zero, 최저 운영부담). Modular 코어 앱을
셀별로 샤딩하고 간선/읽기 앱은 독립적으로 규모하면 **약 10-16k 이벤트/s**를 목표로 한다(Event
Hubs 파티션 32개 x 복제본 당 수백 결정/s); ~40k 이벤트/s Standard 천장까지의
...
렌더이지 `core/` 리라이트가 아니다. AKS 는 완전 0 이 아니라 노드 하한(시스템 +
...
> **Illustrative - 견적 아님.** USD 목록 price, PAYG, Korea Central 상당의
> 단일 리전; 리전·계약 차이 20-55% 는 정상(EA / RI / 절감 계획). 모든
> 수치는 커밋 전
> [Azure Pricing Calculator](https://azure.microsoft.com/pricing/calculator/)
> 로 재확인해야 한다. T2 LLM 추론은 여기서 **제외**되며
> [cost-model-ko.md § T2 LLM 비용](../interfaces/cost-model-ko.md#t2-llm-cost) 에 별도 보고된다.
...
| 리소스 | 상태 | 월 (USD) | 비용 동인 |
|--------|------|----------|-----------|
| Azure 데이터 Explorer (ADX) | NEW | **$1,000 - $2,500** | 2-3 노드 클러스터; dev/테스트 단일 노드(No SLA)는 $150-300 부터 |
| Log Analytics (텔레메트리) | 변경 | **$1,400 - $3,500** (PAYG) | 인제스트 GB 가 지배; 300 구독에선 100 GB/일 Dedicated 클러스터 commitment(~$6,000+) 보다 측정 볼륨이 정당화될 때까지 PAYG 선호 |
| Event Hubs (셀 3개) | 변경 | **$130 - $250** | 셀당 ~2-3 TU x 3 + 유입 이벤트 |
| Container Apps (셀 3개) | 변경 | **$100 - $250** | KEDA scale-to-zero 가 하한을 낮춤; 상시 부하는 올림 |
| ADLS Gen2 | NEW | **$50 - $200** | 감사 오프로드 + warm 티어, 볼륨-의존 |
| PostgreSQL Flexible (GP 승급) | 변경 | **$150 - $300** | B1ms -> general-purpose 2 vCore + 파티션 저장소 |
...
| 프로파일 | 가정 | 월 (USD) |
|----------|------|----------|
| **Lean** | 필수 진단 settings 로 텔레메트리 절제, ADX dev SKU, 셀 scale-to-zero | **≈ $1,900 - $3,000** |
...
| 소버린 추가 | 월 (USD) | 비고 |
|-------------|----------|------|
| Key Vault **Managed HSM** | **$1,500 - $4,000** | 2 인스턴스, FIPS 140-2 L3 - 프로파일의 하한 비용 |
...
| 이것이 확장 하는 최소비용 토폴로지 | [deploy-and-onboard-ko.md](../deployment/deploy-and-onboard-ko.md) |
