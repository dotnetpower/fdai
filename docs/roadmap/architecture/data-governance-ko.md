...
Azure case-history의 안전한 기본값은 조회 가능한 산출물, 삭제 예정일, 이전 Blob 버전 및 변경
피드 레코드에 30일을 사용합니다. 운영 이력과 의사 결정 근거 레코드는 별도의 메타데이터 및
감사 일정을 따릅니다. 배포 재정의 값은 승인된 인벤토리, 데이터 등급, 소유자, legal hold 동작
및 삭제 검증이 해당 값을 뒷받침할 때만 유효합니다.
