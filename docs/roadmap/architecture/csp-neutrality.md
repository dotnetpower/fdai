...
- **Coverage counts are exact bounded integers.** Boolean values are not counts. Zero provider
  objects require zero observed provider types and vice versa, and each observed type owns at least
  one object, so `provider_type_count` cannot exceed `provider_object_count`. Mapped and unmapped
  object counts must still reconcile exactly with the provider total.
- **Azure coverage counts provider-native rows, not projected graph objects.** The coverage query
  groups all ARG `Resources` and resource-group rows from `ResourceContainers` by normalized ARM
  type. It excludes the subscription anchor and derived nested resources such as materialized
  subnets, then compares those groups with the complete reviewed ARM vocabulary. A provider type
