...
| WARA bounded assessment reads | implemented | `shared/providers/wara_assessment.py`; `delivery/azure/wara_observation.py`; exact evaluator overlay and focused adapter tests | Azure Resource Graph reads are pinned to an exact reviewed query, evaluator, ARM resource scope, complete target visibility, provider type, page/row/byte bounds, approved management origin, and deterministic no-authority receipt. |
...
|------|-------|--------|----------|-----------|
| 2026-09-04 | implemented | Required exact target-visibility coverage before the Azure WARA adapter can interpret zero violation rows as satisfied. Partial provider visibility now remains unknown. | `current change`; focused WARA scope-coverage regression. | Retain a separately authorized live-Azure shadow receipt. |
| 2026-09-04 | implemented | Added the Azure adapter for exact WARA assessment reads behind the provider-neutral observation contract. It binds approved token targets, exact resource scope, query and evaluator digests, and bounded deterministic receipts without adding remediation authority. | `current change`; focused WARA overlay, runtime, Azure adapter, Ruff, and strict mypy checks. | Retain a separately authorized live-Azure shadow receipt before claiming operational validation. |
