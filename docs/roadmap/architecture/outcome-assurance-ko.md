...
|-------------|------------------|---------------|
| AI 준비 상태 | 통제된 자율 클라우드 운영을 위한 준비도 | onboarding, telemetry, detection, 롤백, 감사, 소유권, 승격 게이트 |
...
콘솔은 이 라벨을 설명에만 사용합니다. FDAI는 클라우드 telemetry에서 직원 생산성,
...
기록이 실행 및 효과 근거를 제공합니다. 배포별 서비스, 워크로드, objective, 소유권
대응은 downstream 구성에 남습니다.
...
| `alignment` | objective 참조, 작업 흐름과 ActionType 귀속, 커버리지 | DecisionCase, 감사, 온톨로지 링크 |
| `outcomes` | 현재, 기준선, 대상, 단위, 샘플 크기, 확신도 간격 | 측정 파이프라인 |
| `guards` | 임계값, 관찰된 값, pass 상태, 근거 참조 | 가드 및 승격 evaluation |
...
| Objective 귀속 | `unattributed`, `partial`, `attributed` | finalized-event 귀속 커버리지를 기준으로 합니다 |
...
전체 응답은 네 축을 별도로 보고합니다. Blocker를 숨길 수 있는 평균 점수를 만들지
...
| 근거 | telemetry와 인벤토리 출처 연결 및 최신성 충족 | stale objective, 사용 불가 telemetry, 불완전한 인벤토리 |
| Detection | 필수 detection dimension 준비된 | SLO 또는 detector 근거 누락, stale 스냅샷 |
| 액션 안전성 | stop, 롤백, 영향 범위, dry 실행, lock, 멱등성, 감사 수명 주기 | safeguard 또는 필수 의존성 누락 |
...
| Njord | 비용 관측과 비용 objective 상태를 공급합니다 |
| Forseti | 결정 맥락에 protected objective와 예상 효과를 기록합니다 |
| Odin | 복원력, 변경 안전성, 비용 거버넌스 objective 사이 충돌을 중재합니다 |
...
- **개요:** Operational 준비 상태, Objective alignment, 컨트롤 assurance의 세 linked 요약을 제공합니다.
- **Operating outcomes:** Objective별 기준선, treatment, 귀속 커버리지, 근거 기록을 제공합니다.
...
준비 상태, objective, 감사, 인시던트, 액션, 승격 경로로 연결합니다. 누락된 값도
...
Odin은 arbitration을 위해 objective 점수 입력을 비교할 수 있지만 읽기 변환 결과는
business 값을 순위하거나 결정을 변경하지 않습니다. Objective priority와 대상 값은
구성이 설정합니다.
...
- 사용 불가 상태를 포함한 objective 및 근거 drill-down을 추가합니다.
...
- Objective 근거가 불완전하면 cross-vertical arbitration을 human 승인에 둡니다.
...
- Finalized pilot 이벤트의 95% 이상이 objective로 해석되고 나머지도 표시됩니다.
...
| 기존 objective와 결과 모델 | [operating-ontology-ko.md](operating-ontology-ko.md) |
