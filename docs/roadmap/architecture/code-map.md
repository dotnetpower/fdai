...
| Declaration-driven finite question universe | implemented | `core/conversation/question_universe.py`; `core/conversation/epistemic_coverage.py`; `tests/conversation/test_question_universe.py`; focused question-universe and epistemic-coverage tests (`10 passed`) | Complete principal-scoped manifests expand through a canonical bounded grammar into stable case identities. Unavailable declarations remain typed exclusions, overflow fails before expansion, and generated records grant no execution authority. |
