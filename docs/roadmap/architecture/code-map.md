...
| Governed model deployment action | Owner-requested, shadow-first creation of one version-pinned Global Standard deployment through the existing Forseti, Var, Thor, Saga, and Vidar lifecycle; exact target normalization, quota and create-only preflight, independent ARM readback, inventory typing, and deployment-name progress grant no new role or bypass | [ActionType](../../../rule-catalog/action-types/ops.deploy-model.yaml), [target contract](../../../packages/service-contracts/src/fdai_service_contracts/executor_targets.py), [Core adapter](../../../services/core-control-plane/src/fdai/delivery/azure/gateway_direct_api.py), and [development gateway](../../../delivery/dev_operations_gateway/gateway.py) | [catalog tests](../../../services/core-control-plane/tests/rule_catalog/test_action_type_catalog.py), [gateway tests](../../../services/core-control-plane/tests/delivery/dev_operations_gateway/test_gateway.py), and [executor adapter tests](../../../services/isolated-executor/tests/test_executor_http_adapters.py) |
