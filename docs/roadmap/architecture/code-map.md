...
> the same reviewed Kubernetes bindings as the inventory Job, selects venue-specific broker
> security, and enforces the shared 300-second tick ceiling for local/deployed parity.
> LIST pagination persists private continuation after each bounded 2048-observation pass and
> establishes the watch cursor only after the snapshot drains. Pod inventory retains creation
> timestamps plus explicitly controller-marked owner identity.
...
| Pod restart and replacement separation | Current-Pod restart status remains independent from controller-wide replacement uncertainty; replacement gaps use a separate lane, and positive replacement verification requires an independently evidenced exact predecessor rather than a sibling heuristic | [recovery query](../../../services/core-control-plane/src/fdai/core/ontology_platform/kubernetes_pod_recovery_queries.py) and [recovery evidence](../../../services/core-control-plane/src/fdai/core/ontology_platform/kubernetes_pod_recovery_evidence.py) | [recovery tests](../../../services/core-control-plane/tests/core/ontology_platform/test_kubernetes_pod_recovery_queries.py) |
...
| Planned-change graph freshness | Exact-target impact analysis with a bounded three-hop default that reaches the normal Resource-to-Workload-to-BusinessService-to-Objective chain, bound to stable persisted inventory manifests, one matching projected operating-model revision, the active generation, and the ontology release through a content-addressed no-authority receipt. Post-traversal revalidation compares stable graph state while retaining, but not mistaking, each audit read time for a graph mutation. | [impact assessment](../../../services/core-control-plane/src/fdai/core/impact_analysis/change_assessment.py), [inventory receipt source](../../../services/core-control-plane/src/fdai/delivery/persistence/postgres_graph_freshness.py), and [Pantheon binding](../../../services/core-control-plane/src/fdai/runtime/bootstrap_pantheon.py) | [impact tests](../../../services/core-control-plane/tests/core/impact_analysis/), [receipt source tests](../../../services/core-control-plane/tests/persistence/test_postgres_graph_freshness.py), and [change-chain tests](../../../services/core-control-plane/tests/agents/test_change_management_chain.py) |
...
| ActionType promotion delivery | Exact governance-attestation reservation with lease fencing, retryable failure semantics that avoid failed-result deduplication, consumed-state receipt replay after restart, verified durable authority refresh, and revision-CAS persistence across replicas | [promotion delivery](../../../services/core-control-plane/src/fdai/delivery/promotion.py), [promotion persistence](../../../services/core-control-plane/src/fdai/delivery/persistence/state_store_action_promotion.py), and [direct-API execution](../../../services/core-control-plane/src/fdai/core/executor/direct_api.py) | [governance dispatch tests](../../../services/core-control-plane/tests/delivery/test_governance_dispatch.py), [promotion executor tests](../../../services/core-control-plane/tests/delivery/test_promotion_executor.py), [persistence tests](../../../services/core-control-plane/tests/persistence/test_state_store_action_promotion.py), and [direct-API tests](../../../services/core-control-plane/tests/core/executor/test_direct_api_executor.py) |
...
| Ontology safety platform | Exact semantic releases with catalog-loaded Interface and FunctionType declarations, release-aware query profiles and function registration, principal-scoped manifests, verified Resource-to-ResourceType classification, generic and temporal query algebra, source-receipt-complete bitemporal topology and diffs with a canonical deduplicated digest, immutable direction-generation shadow comparisons with bounded blast-radius deltas, exact-release-gated proposal readiness, authoritative-inventory rebuild pointers, and distinct-reviewer regression-bound catalog PR proposals, reviewed metric concepts, topology-aware causal joins, per-Resource freshness-complete graph reads, digest-fenced continuous operating-model replay recovery, request-bound (release, catalog revision, cutoff, target) and byte-bounded (principal, authority, and bundle identity fields included) evidence reads and copy-on-write scenario branches without production write authority, mutation plans with separate planner-function and operational-plan lineage plus documented fail-closed argument, evidence, target, and effect validation contracts, compact typed effect-reconciliation events, authenticated independent-observer binding, and lease-fenced durable terminal outbox delivery | [ontology_platform](../../../services/core-control-plane/src/fdai/core/ontology_platform/) | [ontology platform tests](../../../services/core-control-plane/tests/core/ontology_platform/) |
