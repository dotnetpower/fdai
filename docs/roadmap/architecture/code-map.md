...
Knowledge-only adaptive plans can carry a candidate draft, saving one call while retaining independent review; typed context-independent review omits irrelevant history, caches immutable schemas, and exposes elapsed-time budgets. Prompt types use public agent/model facades; `families/conversation/conversation_history.py` restores principal-bound durable replies without model replay. The question bank binds both Console catalogs by digest and is regenerated
