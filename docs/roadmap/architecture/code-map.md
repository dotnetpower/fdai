...
| change_lineage | Immutable replay-stable join from canonical `ChangeRecord` and `ChangeAssessment` through the selected `DecisionCase` option, `Action`, and `ResponseOutcome`. Nested decision traces preserve objective scores, constraints, effects, approval requirements, and reasoning receipts. Resilience traces preserve execution mode, impact scope, rollback contract, verification timing, and recovery result. Cross-record identity, target, digest, causal-order, and ambiguous score mismatches are rejected; the lineage grants no execution or promotion authority. | [src/fdai/core/change_lineage/](../../../src/fdai/core/change_lineage/) | [tests/core/change_lineage/](../../../tests/core/change_lineage/) |
