...
Revisioned conversation settings are written by the Operator Service to the shared `runtime-settings:policy` record and consumed once by Core at startup. IAM composition binds dialogue relationship adapters without expanding root imports. Core records bounded T1 measurements in a no-authority state projection; the Operator conversation adapter reads only that state and removes endpoint details before Console health presentation. Adaptive answers use independent configured T1 author/reviewer models, with T2 only for optional refinement. Prompt ablation records every optional exclusion for replay, and version-negotiated adaptive `1.6.0` wires preserve semantic evidence.
