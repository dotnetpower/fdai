...
| FDAI Console Dashboard v2 and recorded Resource state | Additive resource-first `/dashboard-v2` route with a bounded honeycomb, one active hover preview, type autocomplete, and shared operational, provisioning, and availability facts from the existing ontology instance reader. Bounded server pages retain generation and query context; the Dashboard and Instances screens share one decoder and fact view. The existing Dashboard and Cost Governance routes remain unchanged. Authenticated runtime validation remains separate. | [route](../../../console/src/routes/dashboard-v2.tsx), [shared decoder](../../../console/src/recorded-resource-state.ts), [state API](../../../services/operator-service/src/fdai_operator_service/families/operations/instance_states.py), [recorded-state design](../interfaces/recorded-resource-state.md), and [adoption ledger](../../roadmap-implementation/interfaces/console-operations.md) |
