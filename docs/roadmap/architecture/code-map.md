...
| Durable Rule generation closure | implemented | `core/rule_semantic_generation/activation.py`; `core/rule_semantic_generation/ledger.py`; `rule_catalog/schema/rule_semantic_generation_events.py`; focused activation, contract, ledger, and live PostgreSQL checks | Core verifies the exact validation receipt and expected prior active identity before activation, suppresses completed-command provider replay, and atomically binds the first terminal result to one lease-fenced pending publication record. Delivery state grants no policy, approval, mutation, or execution authority. |
