...
| FDAI Console Dashboard v2 | Additive resource-first `/dashboard-v2` route, bounded honeycomb, one active hover preview, type autocomplete, and inventory projection decoding. The existing Dashboard and Cost Governance routes remain unchanged. Native presentation is implemented; inventory-wide contracts and authenticated runtime validation remain separate. | [route](../../../console/src/routes/dashboard-v2.tsx), [decoder](../../../console/src/routes/dashboard-v2.model.ts), [browser checks](../../../console/tests/e2e/dashboard-v2.spec.ts), and [adoption ledger](../../roadmap-implementation/interfaces/console-operations.md) |
