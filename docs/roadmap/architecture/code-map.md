...
| workflow | Compile and run version-pinned WorkflowDefinition records. Approval quorums count normalised principals. Enforce action steps wait for independently verified outcome receipts; partial failure records intent before reverse typed compensation and closes only after verified recovery. Recovery-incomplete closure first writes a durable target hold, and the RiskGate denies ordinary actions when that hold is active or unreadable. Approved recovery bypass and hold release remain planned. | [src/fdai/core/workflow/](../../../src/fdai/core/workflow/) | [tests/core/workflow/](../../../tests/core/workflow/) |
