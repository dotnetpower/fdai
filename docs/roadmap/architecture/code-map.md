...
| Trusted current-state ObjectSet cutoff | implemented | `core/conversation/semantic_planning.py`; `composition/wire_semantic_query.py`; `tests/conversation/test_semantic_planning.py`; focused planner, tier-routing, and relationship composition tests | Model-proposed ObjectSet timestamps remain candidate-only. Core rebuilds every semantic ObjectSet `as_of` from the same injected trusted clock used by the secured query gateway, while direct historical or future ObjectSet requests remain rejected. |
...
| Receipt-bound current instance relationship query | implemented | `core/ontology_platform/instance_relationships.py`; `composition/wire_semantic_query.py`; `fdai_core_service/semantic_instance_relationship_projection.py`; focused evaluator, composition, and processor tests | `query.instance_relationships` filters exact active-release LinkTypes from one issued principal-scoped ObjectSet receipt, preserves stored direction and source completeness, and never converts a partial empty result into an absence claim. Authenticated Browser evidence remains open. |
...
| T1-first semantic planning | implemented | `core/conversation/semantic_planning.py`; `core/conversation/semantic_planning_cascade.py`; `core/conversation/semantic_planning_models.py`; `composition/semantic_query_model_targets.py`; `composition/wire_semantic_query.py`; focused tier-routing and composition tests | Semantic planning always uses the resolved T1 mini-model first. Only an unavailable or deterministically invalid T1 frame or plan proposal can retry that same stage once with the optional T2 primary reasoner. Typed clarification requirements distinguish legitimate missing user context, which terminates at T1, from a request for server-bound principal scope or purpose, which invalidates the T1 frame before one frame-only retry. The planner never borrows the secondary quality-cross-check role. Scope denial and evidence holds do not invoke T2. |
...
| 2026-08-14 | implemented | Added receipt-bound current instance relationship selection and localized projection as a separate authority surface from schema relationship meaning. | `current change`; focused instance evaluator, semantic composition DAG, prompt, and Core processor checks pass. | Retain an authenticated current relationship answer before reporting runtime validation. |
| 2026-08-15 | implemented | Hardened current instance relationship binding against LinkTypes absent from the exact release and rejected secured graphs with duplicate object identities or missing link endpoints before projection. | `current change`; 52 focused relationship and Core processor tests pass; Ruff, strict mypy, and independent review report no reproducible Medium-or-higher defect. | Retain an authenticated current relationship answer before reporting runtime validation. |
| 2026-08-15 | implemented | Rebound semantic ObjectSet `as_of` values to the trusted server clock shared with execution after an authenticated relationship probe exposed planning-latency drift beyond the secured gateway skew. | `current change`; focused planner and relationship composition tests passed 18 cases, tier-routing tests passed 9 cases, and Ruff plus strict mypy passed. | Restart the exact-source Core and retain an evidence-complete relationship answer before reporting runtime validation. |
