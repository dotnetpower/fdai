...
Knowledge-only adaptive plans can carry a candidate draft; independent review omits unrelated history and optional refinement requires time and calls for verification. Stages cache immutable schemas and expose elapsed budgets. Prompt types use public agent/model facades; `families/conversation/conversation_history.py` restores principal-bound durable replies without model replay. The question bank binds both Console catalogs by digest and is regenerated
...
| FDAI Console Dashboard v2 and recorded Resource state | Additive resource-first `/dashboard-v2` route with a bounded honeycomb, one active hover preview, type autocomplete, and shared operational, provisioning, and availability facts from the existing ontology instance reader. Bounded server pages fence the inventory generation to the committed ontology manifest, qualify reviewed provider state paths from immutable snapshot times, and preserve distinct unknown causes. The Dashboard and Instances screens share one decoder and fact view. Compact ontology graph nodes keep operation primary, but select an applicable availability fact or evidence gap and then provisioning when operation is explicitly not applicable; the axis label prevents either fact from becoming an inferred operational or health verdict. The existing Dashboard and Cost Governance routes remain unchanged. Authenticated runtime validation remains separate. | [route](../../../console/src/routes/dashboard-v2.tsx), [shared decoder](../../../console/src/recorded-resource-state.ts), [state API](../../../services/operator-service/src/fdai_operator_service/families/operations/instance_states.py), [recorded-state design](../interfaces/recorded-resource-state.md), and [adoption ledger](../../roadmap-implementation/interfaces/console-operations.md) |
