...
Knowledge-only adaptive plans can carry a candidate draft; review omits unrelated history and refinement reserves verification budgets. Missing required evidence keeps a supported explanation limited instead of spending T2 to invent facts. Stages cache schemas and expose elapsed budgets. Prompt types use public agent/model facades; `families/conversation/conversation_history.py` restores principal-bound durable replies without model replay. The question bank binds both Console catalogs by digest and is regenerated
