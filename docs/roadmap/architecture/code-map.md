...
| change_lineage | Immutable replay-stable join from canonical `ChangeRecord` and `ChangeAssessment` through the selected `DecisionCase` option, `Action`, and `ResponseOutcome`. Nested decision and resilience traces preserve rationale, impact, rollback, verification, and recovery. A deterministic learning projection remains candidate-only, requires a separately sealed operational case, and is never reusable by itself. The existing GitHub and Azure DevOps `ChangeFeed` adapters pass the same lineage boundary. Identity, target, digest, causal-order, and ambiguous score mismatches are rejected; the lineage grants no execution or promotion authority. | [src/fdai/core/change_lineage/](../../../src/fdai/core/change_lineage/) | [tests/core/change_lineage/](../../../tests/core/change_lineage/) |
