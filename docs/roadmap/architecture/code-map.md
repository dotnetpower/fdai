...
| [src/fdai/rule_catalog/pipeline/distill/](../../../src/fdai/rule_catalog/pipeline/distill/) | Build-time manual compilation plus the `DocumentEnvelope` provenance bridge, normalized cross-format graph comparison, review-only ontology proposals, partition release gates, provider conformance, and lifecycle/evaluation plans; local parser resource ceilings live in `shared/providers/local/document_limits.py`. |
