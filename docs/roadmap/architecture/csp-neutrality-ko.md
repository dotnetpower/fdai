...
translation_source_sha: 1ce5108c52f4ec9e626e1165c17aeafa1d349087
...
- **주기적 조정은 계속 필요합니다.** 가져올 수 있는 인벤토리 sync CLI는 기본 6시간 주기로
  ...
  newer 시도가 실패 또는 포기되었거나 관측된 변경이 하한을 넘어 아직 조정되지 않은 경우에만
  검사합니다. 변경은 활성 스냅샷이 시작된 뒤에 기록되었을 때 미조정으로 셉니다. 스캔은
  시작한 시점 이후의 상태만 관측하기 때문입니다. 시도 하나는 무진행 마감과 절대 상한으로
  제한되므로 멈춘 프로바이더는 자기 시도를 실패시키고 느린 소스는 끝까지 가며, 실패한 시도는
  정상 간격까지 지수적으로 백오프합니다. ARG 요청은 모든 샤드가 공유하는 지속 예산에 맞춰
  속도를 조절합니다. 로컬 새로 고침과 배포 작업은 영속 시도 전이, 활성 포인터
