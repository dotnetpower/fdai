...
translation_source_sha: 5110a7393a052927d2fd3d2482fc7d6fce46344d
...
재사용 가능한 계산 기반은 구현했지만 클러스터 수집과 서명된 용량·저장소 사실 생성은 남아
있습니다. `kubernetes_quantity.py`는 단위 변환기를 따로 만들지 않고 Apache-2.0 라이선스의
...
`ValueError`를 발생시킵니다. 후속 수집기는 이를 알 수 없음으로 유지해야 합니다. 실제 중지
상태의 관측 매니페스트가 CPU 100밀리코어, 메모리 128 MiB, PVC 1 GiB로 계산됨을 확인했습니다.
이는 산술 검증이지 배치 가능성의 증명은 아닙니다.

계획된 리소스 점검은 정확한 클러스터 UID에 연결된 GET 요청만 사용합니다. 모든 페이지의
...
계획된 저장소 점검은 명시적으로 제공한 기존 PVC UID, 정확한 네임스페이스·이름과
