...
| Kubernetes remediation effect identity | implemented | `services/isolated-executor/src/fdai_executor_service/adapters/kubernetes_direct_api.py`; `infra/runtimes/aks/workloads/main.tf`; focused Executor, deployment renderer, and RBAC tests | Only the isolated Executor ServiceAccount receives namespaced Pod and Deployment mutation verbs for the four registered Kubernetes ActionTypes. Inventory and Core identities receive no Kubernetes write permission. A reviewed apply and independent effect readback remain open. |
| Operational activity identity separation | implemented | `packages/service-contracts/src/fdai_service_contracts/control_loop_measurement.py`; Core measurement, RCA, observation-campaign, startup-probe, and Saga audit adapters; Operator activity projection and focused tests | Audit rows preserve the mechanical `actor`; `owner_agent` names the accountable Pantheon role; `producer_principal` is reserved for an authenticated event-bus publisher. The projection validates declared ownership or a bounded legacy mapping and omits unknown custom sources from Agent Activity without deleting their audit evidence. None of these fields grants authority. |
| Read-only evidence-source isolation | validated | `core/rca/evidence.py`; `delivery/azure/log_query.py`; focused RCA and Azure query tests; bounded local source probe | Independent log and trace reads run concurrently with separate deadlines. A delayed or unavailable source cannot erase completed evidence from another source, and missing evidence remains a hold rather than a healthy observation or authority grant. |
| Non-authoritative Incident guidance | validated | `fdai_service_contracts.incident_intervention`; Core intervention consumer and Pantheon binding; Saga and Forseti guidance handling; `docs/baselines/incident-intervention-assurance-2026-09-15.json` | Applied guidance re-enters only through Huginn with no Incident correlation or execution authority. Saga audits the normalized event, Forseti does not judge it, and retained local evidence contains no matching ActionRun. |
...
| 2026-09-14 | implemented | Separated operational activity ownership from machine execution and authenticated publication identity across the control-loop measurement, RCA, observation, startup, audit-mirror, and Operator projection boundaries. | `current change`; focused service-contract, Core, pipeline, and Operator projection tests passed. | Capture deployed evidence for the exact merged revision; attribution remains descriptive and grants no runtime authority. |
| 2026-09-15 | validated | Added a no-authority Incident guidance boundary: Core publishes only after durable application, Huginn normalizes, Saga audits, and Forseti explicitly defers judgment. | `current change`; `docs/baselines/incident-intervention-assurance-2026-09-15.json`; focused contract, Core, Pantheon, and Console tests. | No remaining local implementation work for the Incident guidance authority boundary. |
| 2026-09-13 | in-progress | Added the inert A3-E shadow-reversion command contract, writer Protocol, and fail-closed orchestration seam, plus a strictly local-only cohort CLI and closed corpus decoder. The command binds the exact revision, reverted ActionTypes, failed or unknown observation evidence, reason, stop/rollback/kill-switch bindings, expected standing-authorization fence and state, distinct human proposer and reviewer, a stable idempotency key, and two-phase intent and terminal audit. The CLI refuses non-local venue or evidence class, bounds document size, case, review, and elapsed inputs, enforces a monotonic deadline, and rejects symlinked corpus or output paths. Neither grants execution or promotion authority, mutates the registry, nor reaches network, Azure, provider, or database paths. | `current change`; `core/standing_authority/{shadow_reversion_command,shadow_cohort_cli,shadow_cohort_corpus}.py`; `tests/core/standing_authority/{test_shadow_reversion_command,test_shadow_cohort_cli}.py`; 323 focused standing-authority tests passed (99 new); Ruff, format, and strict mypy passed; a whole-tree scanner that resolves absolute, relative, package re-export, aliased, and dynamic import forms proves no shipped module outside the package reaches either module, and no shipped module names the writer Protocol or the orchestrator; an independent critique found and fixed an incorrect exit code on nested record errors and an incomplete hardcoded guard root set. | No reversion writer adapter exists, so a failed effect verification still has no executable path back to shadow mode. The durable provider boundary, governed runtime cohort, independent review, explicit current human approval, and retained independent effect observations under #632 remain open. |
