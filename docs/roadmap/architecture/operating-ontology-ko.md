...
활성 컨트롤 plane은 에이전트이며, 온톨로지는 대상 신원, 의존성, objective, 근거,
허용 액션, 예상 효과의 해석이 서로 달라지지 않도록 제한합니다. Upstream은 안정적인
cloud-operations 개념을 소유하고 배포는 관찰된 instance와 의도를 제공합니다.
...
> 실행 표면이 아닙니다. Event, 승인된 구성, telemetry 출처, 추가 전용 감사
...
> wiring, decision-case 선택, 응답 종결, Muninn/Norns learning intake를 구현합니다.
> `OperatingModelProvider`는 범위가 제한된 배포 instance를 project하고 맥락 스냅샷은 타입이 지정된
...
> `OperationalEvidenceBundle` foundation을 제공합니다. Admission에는 온톨로지 release, 카탈로그 및
...
> 근거를 출력하고 bundle의 자율성 상한을 유지하거나 낮출 수만 있습니다. 아직 런타임
> 또는 조립 경로가 이 bundle을 소비하지 않으므로 운영 자율성 경로의 일부가 아니며
> 액션 권한이 없습니다.
...
> 추가하고 읽기 전용 결정론적 `query.network_path_segments` 함수 foundation을 제공합니다.
...
> purpose-bound 조회 결과를 사용합니다. 운영 발급자가 없으므로 foundation은 unwired 상태로
...
`rule-catalog/vocabulary/property-semantics.yaml`은 선택된 Property instance에 검토된 의미 규칙을
...
M5는 Kubernetes Pod, 서비스, Endpoints instance에 `Resource`를 재사용하고 범위가 제한된 메트릭 샘플에
...
읽기 전용 evaluator는 용도 범위가 지정된 secured ObjectSet 결과 하나와 각 관계 및
...
이 구획은 platform foundation일 뿐입니다. 런타임 FunctionType을 등록하거나 Kubernetes 또는
...
FDAI는 domain-agnostic하지 않습니다. 안정적인 domain 모델을 가진 cloud operations 컨트롤
...
| Customer organization | 범용 타입과 링크만 포함하고 customer instance나 값을 포함하지 않습니다. |
| Business 의미 규칙 | 안정적인 개념은 upstream에 두고 배포별 대응과 값은 downstream에 둡니다. |
...
| `BusinessService` | 소유권, criticality, objective, 영향에 사용하는 안정적인 서비스 신원입니다. |
...
Objective는 free-form 메트릭 라벨이 아닙니다. 종류, 단위, 대상 또는 범위, 측정 출처,
...
| `Experiment` | 관측 episode에 intervention을 줄 수 있는 범위 제한 chaos 또는 검증 활동입니다. |
...
| ObjectType | 목적 |
|------------|------|
| `DecisionCase` | Objective, 제약, 근거, no-action 기준선이 있는 변경할 수 없는 결정 맥락입니다. |
...
| `governed_by` | 서비스/워크로드 -> Objective/제약 | 대상에 적용하는 의도입니다. |
| `owned_by` | 서비스/워크로드/Objective -> 소유권 | 책임 운영 소유자입니다. |
| `observes` | 관측/신호 -> 서비스/워크로드/Resource | 측정 근거의 대상입니다. |
| `observation_targets_resource` | 관측 -> Resource | 범위가 제한된 telemetry 검증에 사용하는 물리 measured-evidence 대상입니다. |
| `affects` | 변경/인시던트/Experiment -> 서비스/워크로드/Resource | Episode가 영향을 주는 범위입니다. |
| `predicts_breach_of` | 예측 -> Objective | 선언된 horizon 안에서 위험한 objective입니다. |
| `considers` | DecisionCase -> ActionOption | 함께 평가한 범위가 제한된 alternative입니다. |
| `protects` | DecisionCase/ActionOption -> Objective | 결정이 보존하려는 objective입니다. |
...
| `change_instantiates_process` | 변경 -> 프로세스 | Multi-step 변경을 기록하는 영속 작업 흐름 journal입니다. |
...
| `change_conflicts_with_change` | 변경 -> 변경 | 대상, objective 또는 effective 시간이 겹치는 충돌입니다. |
...
- **Effective 시간:** Objective, 소유권, 예산, 제약은 `effective_from`과 선택적인
...
- **Current-state instance 저장소:** Instance 그래프는 subgraph별 단일 쓰기 담당 아래에서 현재 관찰된
 상태를 보관합니다. Bitemporal 저장소가 아닙니다. 갱신은 이전 속성 값을 대체하고, 사라진
 객체는 소유 변환 결과가 삭제합니다. 과거 instance 값은 instance 그래프가 아니라 그것을 만든
 권위 있는 출처 세대에 남습니다.
- **최신성:** 모든 결정 맥락은 출처별 최신성을 기록합니다. 하나의 fresh 출처가
 오래된 objective, 토폴로지 간선, 비용 관측을 숨길 수 없습니다.
...
재생은 instance 그래프의 임의 과거 상태가 아니라 pin된 카탈로그 release와 보존된 결정 맥락을
...
| Objective, 예산, 제약, 소유권 | Approved system과 포크 구성 | Effective-time 의도 변환 결과입니다. |
| Telemetry 및 비용 관측 | 구성된 근거 프로바이더 | 출처 참조가 있는 event-time 관측입니다. |
| 결정, 승인, 액션, 롤백 | 추가 전용 감사와 프로세스 journal | 변경할 수 없는 의미 링크입니다. |
...
| Njord | 비용 관측, 비용 예측, 비용 objective 상태입니다. |
| Freyr | Demand, 용량 예측, sizing 옵션입니다. |
| Loki | Experiment와 복원력 근거입니다. |
| Forseti | 결정 사례와 통제된 결정입니다. |
| Odin | Cross-objective arbitration 결정과 점수 breakdown입니다. |
| Var | 독립적인 승인 기록입니다. |
| Thor | 액션 실행과 시도입니다. |
...
- 활성 서비스, 복구, 비용, 아키텍처 objective;
- 소유권 및 에스컬레이션 참조;
- 활성 변경, experiment, 인시던트, maintenance 구간;
- 현재 관측과 범위가 제한된 예측;
...
`OperationalEvidenceBundle` foundation은 권한을 하나로 flatten하지 않고 그래프 및 문서
...
`StateFactMetadata`와 정확히 일치해야 하며, bundle은 보류를 도출할 때 이 메타데이터 필드를 직접
평가합니다.
...
contradiction 그룹을 분리하지 않으며 supersession을 암시하지 않습니다. Foundation에는 암시적인
latest-wins 룰이 없습니다. 향후 supersession 정책에는 명시적으로 검토된된 점유 관계가
필요합니다. Detector는 산문에서 의미 disagreement를 추론하지 않습니다. 후보 및
진단 개수와 필드 length에는 한계가 있고 중첩된 순서는 변경할 수 없는 tuple로 copy됩니다.
...
배치합니다. 이러한 테스트는 safe foundation을 입증하지만 운영 wiring은 입증하지 않습니다.
Bundle은 읽기 전용 근거이며 승인 또는 액션 권한을 부여하지 않습니다.

Forseti는 스냅샷에서 `DecisionCase`를 만듭니다. 각 사례는 no-action 기준선, 범위가 제한된 옵션,
예상 효과, protected objective, violated 제약, uncertainty, 근거 참조를 포함합니다.
Odin은 조건을 충족한 옵션이 objective 사이에서 충돌할 때만 중재합니다. 사람 승인이 필요하면 Var가 같은
...
상태, 출처 개정 번호, 집계 개수만 노출하며 배포 instance 속성은 반환하지 않습니다.
...
time-consistent 스냅샷을 materialize하고 공유 사례를 만들어 arbitration 요청에 포함합니다.
...
이 루프는 개별 리소스 사용률이 아니라 서비스 objective와 error-budget risk를 우선합니다.
...
비용 optimization은 선택한 옵션이 서비스 및 복구 objective를 보존할 때만 유효합니다. Estimated
...
case-history materializer를 요구하고 strict 입력을 seal한 뒤 실패 fingerprint별 변경할 수 없는 사례를
최대 100개 영속하게 보존하여 `operational_case_fingerprint_cohort` 맥락을 publish합니다.
Norns는 하나의 실패 fingerprint와 ActionType, 최소 하나의 검증된 reusable 성공, 최소 하나의
실패, 거절, no-op, 롤백 또는 recurrence 컨트롤을 요구한 뒤 기존 consensus 및 비율 한도
경로로 inert 후보를 발행합니다. 모든 후보는 사례 id, 개정 번호, 매니페스트 다이제스트, 리소스
타입, fingerprint, 결과별 개수, 다이제스트 근거를 인용합니다. Raw
`measurement.action_outcome.v1`은 방식 근거가 부족한 telemetry로 유지되며 promotable
집단에 들어갈 수 없습니다.
...
1. **Operating kernel:** 모든 배포가 공유하는 upstream ObjectType 및 LinkType입니다.
2. **버티컬 pack:** Upstream reliability, 아키텍처 검토, cost-governance 프로파일입니다.
3. **포크 확장:** Kernel을 따르는 검토된 industry 또는 organization-specific 타입, 링크,
  objective, 어댑터입니다.
4. **배포 instance:** Upstream 출처 컨트롤 외부에 유지하는 customer 서비스 대응,
  objective, 예산, 소유자, 리소스, 근거입니다.
...
1. 이 리소스 변경이 어떤 business 서비스와 objective에 영향을 줄 수 있습니까?
2. 설정된 horizon 안에 어떤 서비스가 objective를 위반할 수 있으며 그 이유는 무엇입니까?
3. 어떤 활성 변경 또는 experiment가 현재 서비스 성능 저하를 설명할 수 있습니까?
4. 비용 묶음 안에서 reliability 및 복구 objective를 보존하는 응답 옵션은 무엇입니까?
5. FDAI가 아무 액션도 취하지 않으면 어떻게 됩니까?
6. Odin이 한 objective를 선호한 이유와 alternative와의 차이는 얼마입니까?
7. 선택한 액션이 가드 메트릭 회귀 없이 예상 효과를 냈습니까?
8. 현재 토폴로지, objective, 정책 버전에서 이전 사례를 계속 재사용할 수 있습니까?
...
| O3 - Reliability 루프 | Core 구현됨: objective-aware 결정 사례, 옵션 선택, `ResponseOutcome` 종결입니다. | 고정된 테스트가 서비스 -> objective -> 옵션 -> 액션 -> 효과를 하나의 상관관계로 통과합니다. |
| O4 - ARB 및 비용 루프 | Core 구현됨: architecture-constraint exclusion, 타입이 지정된 변경 수명 주기 선언, protected-objective 비용 tradeoff입니다. | 변경 및 비용 옵션은 protected reliability objective를 희생하거나 그래프에서 권한을 얻을 수 없습니다. |
| O5 - 통제된 learning | Operational-learning O2까지 구현됨: strict Huginn 사례 이벤트, Muninn fingerprint 집단, balanced inert Norns 후보입니다. Mimir 카탈로그 행동은 변경하지 않았습니다. | Success-only 및 raw-response 집단을 보류하고 후보가 변경할 수 없는 개정 번호를 인용하며 결과는 실제 운영 카탈로그 선언을 직접 수정하지 않습니다. |
...
| 항목 | 필요한 증명 |
|------|-------------|
| 의미 | Decision-critical 필드가 타입이 지정된되거나 타입이 지정된 objective를 참조하며 열림 bag이 권한이 아닙니다. |
| 출처 이력 | 모든 instance가 출처, 개정 번호, effective/이벤트 시간, 기록된 시간, 최신성을 명시합니다. |
| 알 수 없음 안전성 | 누락 대응, stale 토폴로지, conflicting objective가 자율성을 낮추고 계속 표시됩니다. |
...
| Customer 격리 | Upstream 고정본은 synthetic 값을 사용하고 배포 instance를 포함하지 않습니다. |
| 네트워크 근거 | 모든 구간이 stored direction과 근거 상태를 보존하며 unilateral 피어링, 누락된 엔드포인트, cycle, 탐색 한도가 도달 가능성 점유로 바뀌지 않습니다. |
| Pod telemetry | 완전한, missing-selector, stale, synthetic, wrong-cluster, bounded-cycle 및 missing-observation 고정본이 구간 상태를 보존하고 상태를 주장하지 않습니다. |
...
| 현재 리소스, 룰, 신호, 발견 사항 foundation | [LLM strategy](llm-strategy-ko.md#ontology-foundation) |
| 런타임 온톨로지 저장소 | [Rule 조회 온톨로지 저장소](rule-lookup-ontology-storage-ko.md) |
| 액션 안전성 계약 | [액션 온톨로지](../decisioning/action-ontology-ko.md) |
| 에이전트 역할 및 arbitration | [에이전트 pantheon](../agents/agent-pantheon-ko.md) |
