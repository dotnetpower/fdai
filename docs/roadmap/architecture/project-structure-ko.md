...
`bootstrap_bindings.py`에 위임하여 provider construction을 bounded하게 유지합니다. Thin wrapper는 test와
fork가 사용하는 injectable identity-builder boundary를 보존합니다.
...
- **console에는 privileged identity가 없음**: 상태, 감사, shadow 결과, HIL 큐를 시각화합니다.
  ...
  검증된 canonical JSON document만 저장합니다. Realtime projector와 immutable snapshot staging은 모두 사전 검증된 canonical JSON document만 저장하며 snapshot coverage metadata도 begin 또는
  promotion 전에 같은 규칙을 적용합니다. Inventory link는 source authority, revision, cutoff, freshness, completeness, verification evidence를 고정하는 immutable observation metadata도 운반할 수 있습니다.
  Incomplete inventory observation은 relationship claim을 project하지 않으며 stale 또는 conflicting link evidence는 operational-context autonomy를 낮출 수만 있습니다.
  ...
  cursor를 유지합니다. Final batch가 cursor를 생략하면 forwarder는 pull 시작 시점의 cursor로
  되돌리지 않고 마지막 non-null page cursor를 commit합니다.
...
- **Azure operational evidence**: `bind_azure_operational_evidence`는 strict promoted-inventory
  ...
  value를 거부합니다. Partial binding은 container construction에서 실패합니다.
...
| Inventory | `Inventory` 및 `InventorySnapshotStore` (CSP-중립 배치, immutable candidate staging, atomic active pointer) | **CSP-중립성 계약** - [인벤토리](csp-neutrality-ko.md#5-인벤토리-계약--리소스-그래프) | 전용 read-only MI의 scheduled Azure collector: ARG full-scan, direct ARM-list fallback, 서명된 declarative recovery, PostgreSQL last-known-good projection; Core-owned migration은 observed `peered_with` link와 여러 valid `attached_to` anchor를 허용 | Fork가 coverage, authority, relationship cardinality 및 atomic-promotion semantics를 유지하면서 다른 ordered source를 주입 |
...
| **Ontology ObjectType / LinkType / InterfaceType** | `services/core-control-plane/src/fdai/rule_catalog/schema/`의 fail-closed ObjectType, LinkType, InterfaceType 및 explicit Interface implementation loader | - | `rule-catalog/vocabulary/{object-types,link-types,interface-types,interface-implementations}/` 아래의 shipped declaration을 대응하는 immutable `Container.ontology_*` tuple로 load합니다. Interface binding은 compile되어 exact runtime release에 pin됩니다. | Fork는 fork-local vocabulary directory에 추가 YAML을 제공하고 composition root에서 두 root를 load하며 combined Interface binding을 compile한 뒤 concatenated tuple을 `dataclasses.replace`로 전달합니다. Duplicate name과 dangling binding은 fail-close합니다. 자세한 절차는 [downstream-fork-seam-recipes-ko.md § 5.8a](../fork-and-sequencing/downstream-fork-seam-recipes-ko.md#58a-ontology-object-type--link-type-additions). |
| **Network query receipt verification** | `services/core-control-plane/src/fdai/core/ontology_platform/network_path.py`의 `NetworkQueryReceiptVerifier`와 composition이 소유한 opaque verification context 하나 | - | Unbound 상태이며 receipt issuer와 verifier 없이는 `query.network_path_segments`를 authenticated production function으로 등록할 수 없습니다. | Secured receipt role, singleton purpose, exact ontology release, projected-result digest 및 `FunctionInvocationContext`를 인증하는 issuer-backed verifier를 inject합니다. Opaque context는 function argument에 포함되지 않으며 verification은 execution authority를 부여하지 않습니다. |
...
| **LLM 계량(metering)** | `MeteringSink` / `MeteringReader` (`core/metering/sink.py`); `MeteringEmitter`가 명시적인 `control_plane` 또는 `operator_chat` scope와 함께 provider가 측정한 `usage`를 기록 | - | 단일 프로세스 dev harness는 하나의 `InMemoryMeteringSink`를 공유합니다. T1, T2, narrator adapter가 측정된 token을 emit합니다. Independent Operator Service는 `GET /kpi/llm-cost`를 유지하고 SELECT-only role로 durable `llm_invocation` row를 읽으며 detail을 제한하되 token-only aggregate는 정확하게 유지합니다. Interactive local은 준비된 authoritative input에서 sanitized inventory와 Settings projection을 별도로 materialize합니다. | 설정된 가격은 내부 budget control에 남고 provider 지출로 projection되지 않으며, 누락된 provider는 synthetic 대신 unavailable 상태를 유지합니다. |
