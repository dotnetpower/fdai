...
translation_source_sha: 744194f6293a95bffe8ccca783c148e6dc3c8d10
...
| **복구 시도 및 디스패치** | `core/workflow/`의 `recovery_attempt.py`, `recovery_effect_claim.py`, `recovery_terminalization.py`; `core/executor/`의 `hold_dispatch_fence.py` | - | 영속적 복구 시도 신원 결속, 독점 사전 디스패치 청구, 별도 승인/안전장치 증적, 신원 분리가 적용된 권위 있는 효과 완료 주장, 비정상 종료에 안전한 최종 Process/Saga outbox, 논리 대상 잠금 내부의 보류 인식 디스패치 fence | 새 모듈은 기존 자동화 보류 및 복구 승인 계약을 사용하며 새로운 실행 권한이나 프로바이더 연결을 도입하지 않습니다. [#652, #656, #658, #640](../decisioning/process-automation-ko.md) 참조. |
