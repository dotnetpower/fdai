...
translation_source_sha: c586f33a0a5cddecb81345a2eb7aa75e22fb8638
...
- **사람 승인 권한은 서비스별로 분리**: Operator는 Teams/Slack 인증, `cryptography`를 통한 로컬 JWT/JWK 암호학적 검증, 콜백 감사, 영속 결정 발신함을 소유합니다. Core 구현을 가져오거나 실행기 신원을 받지 않습니다.
  ...
  별도로 구성된 Slack A1 아웃바운드 어댑터는 Teams 신원 없이 조립 계층이 소유한 HTTP 클라이언트를 사용합니다. 로컬 전송 증적과 보류 상태의 조회는 아직 없는 영속 발송 조정이나 인증된 브라우저 행위자 결합을 대신할 수 없습니다. [채널과 알림](../interfaces/channels-and-notifications-ko.md)을 참조하세요.
