...
translation_source_sha: eca737fc7b46d0d2fa8f6b0395f606ba6b287dee
...
세대 캐시 검증을 소유합니다. 기존 스키마 복구 정책은 복구 전에 완전한 정본 단일 종류 매니페스트 목록을 인식하며, 카탈로그 소유 프롬프트 프로필이 발화 라우팅이나 새 권한 없이 이 형태를 제안합니다.
...
Core는 명시적 객체 읽기 집합과 인증된 호출 문맥을 사용하는 제한된 `query.ontology_instance_candidates` 계약을 정의합니다. 전달 계층은 감사된 현재 인덱스 조회기를 제공하고 현재 그래프에서 후보 사실의 접근 권한을 다시 검증합니다. 전용 `semantic_query_instance_candidates.py` 조립 모듈은 기본 release와 의미 카탈로그 구성에서 하나의 소스 기반 선언을 공유합니다. 초기화는 관리되는 모델의 동일 작업자를 Pantheon에 주입하고 제한된 별도 조정 작업을 감독합니다. 임베딩 공간이나 모델 버전 식별자가 없으면 인덱스는 사용 불가 상태를 유지합니다. 자격을 검증하지 않은 의미 순위 검색은 닫아 두며 정확한 ID 조회도 현재 그래프 검증을 거칩니다. 후보 결과는 전체 집합을 뜻하지 않으며 부재나 실행 권한을 입증할 수 없습니다.

물리적인 서비스 workspace는 [다중 서비스 저장소 레이아웃](multi-service-repository-layout-ko.md)이 소유합니다. Core 소유의 `kubernetes` 의존성은 전달 계층의 재사용 가능한 수량 계산에 사용하며 루트 개발 도구에도 동일하게 선언합니다. 공유 계약과 독립 패키지 서비스에는 추가하지 않습니다. 이 문서는 의존성 방향, 구조 게이트, 확장 seam, 컨트롤 루프 배선, 구성 및 저장소
...
않고 사용할 수 없음으로 유지합니다.
기록된 Resource 상태 정규화, 유형별 적용성 및 허용 목록 기반 정식 사용 불가 사유는 공유 계약, Core 및 Azure delivery에 유지하고 Operator는 읽기 전용 변환을 소유하며 Console은 그 결과 이유만 지역화합니다. 구성 표류 전달도 `delivery/azure/`와 보호된 Core 서비스 구성에 유지합니다. 검토된 스냅샷은 콘텐츠 주소 기반 private Blob을 통해서만 전달하고, 런타임은 Managed Identity로 읽으며, 적용 후에는 정확한 서버 소유 바인딩을 독립적으로 검증합니다. 독립 서비스 계획 가드는 명령과 환경 변경을 롤백 경계의 일부로 취급하고, 플랫폼 소유 비밀 참조는 보관된 서비스 입력을 신뢰하지 않고 구체화 전에 현재 플랫폼 상태에서 갱신합니다. 격리 실행기는 이전에 없던 기본 비활성 legacy-unbound 전환 연결을 정확히 한 번만 도입할 수 있습니다. 이를 활성화하거나 반복 적용하거나 관련 없는 런타임 표류와 결합하는 작업은 허용하지 않습니다.

ResourceType 카탈로그는 검토된 공급자-중립 신원을 소유하고, 공유 기록 상태 계약은 운영 및 가용성
적용성을 독립적으로 소유합니다. `compute.image`와 `network.firewall-policy` 추가는 공급자 어댑터나
실행 권한을 바꾸지 않습니다. 세대에서 미분류 표식을 제거하려면 각 매핑이 두 레지스트리를 모두
충족해야 합니다.

검증된 대화 산출물 컴파일은 v2 파사드에 경로 선택과 조립을 유지하고, 도메인별 운영 레이아웃과 일반 표, 차트, 타임라인, 비교 및 제한 사항 렌더링을 각각 별도 모듈에 둡니다. 렌더러는 검증되고 범위가 제한된 값만 복사하고 권한을 부여하지 않으며, 읽을 수 있는 스칼라 값이 유한하지 않으면 구조화된 산출물을 만들지 않습니다. 이 경우 정규 텍스트가 안전한 대체 결과로 유지됩니다.

검색 계약은 권한을 부여하지 않는 공유 기록으로 유지합니다. 계획 스키마 `1.2.0`은 결과 종류를
보존하며 Core는 결과 병합 전에 프로필 제한과 예상한 전체 계획 집합을 검증합니다. Azure 전달
계층은 실행 증적을 계획에 연결하고 기대 범위, 플랫폼 및 결과 개수에 맞춰 커버리지를 대조합니다.
과거 집계 카나리는 운영 서비스 연결이나 수집기의 승격을 검증하지 않습니다.
자세한 내용은 [검색 검증 경계](../interfaces/azure-resource-discovery-commands-ko.md#검증-경계)를 참조하세요.

인벤토리 전달 계층은 구성된 각 구독을 독립적으로 검증하고 공급자 고유 타입별 개수를 대조합니다.
보충 수집한 ARM 자식으로 누락된 원본 식별자를 대체할 수 없습니다. 추가된 타입별 대조 정보는
수집 내부에서만 사용하며 기존 영속 커버리지 형식은 유지합니다. 예약 수집의 상태가 완전하려면
활성 세대와 온톨로지 세대가 일치하고 객체, 관계 및 변환 근거가 완전해야 합니다.
구성된 수집 간격은 원본 정책의 제한 안에서 적용합니다. Core 관측 판정 모듈은 부작용 없는 세대 시각 검사를 소유하고 인벤토리 변환은 독립적인 속성 충돌과 정규 상태 출처를 보존합니다. 공급자 의존성이나 쓰기 담당은 추가하지 않습니다.

스냅샷 정책은 Resource 50,000개 또는 관계 200,000개를 넘는 용량 설정을 거부합니다. 원본 조립은
선언된 페이지, 레코드 및 동시성 상한을 적용하며 ARM 페이지 수집은 선언된 바이트 상한도
사용합니다. 범위와 세대에 기반한 지터는 재생 시 동일하고 최대 수집 간격을 넘지 않습니다.
이 제한만으로 원본 간 공유 이동 시간창 바이트 예산이나 디스크 기반 세대 스트리밍이 구현되는
것은 아닙니다.

인벤토리는 전송 버퍼와 별도로 보존하는 정규 세대 레코드의 바이트도 제한합니다. PostgreSQL 공개 모듈은 `postgres_ontology_replacement.py`에 일괄 교체를 위임하며, 버전 활성화, 복사 후 게시 및 보존은 같은 Core 작성자 아래의 집중된 영속 계층 모듈이 담당합니다. `ontology_*_version`과 `ontology_graph_control` 테이블 네 개는 Core 소유이며, 비활성 마이그레이션은 Operator, 문서 또는 대화 작성자를 바꾸지 않습니다. 명시적 활성화는 읽기 권한과 참조 식별자를 보존하고 실체화는 최신 버전을 복원합니다. 도우미는 독립적인 권한을 얻지 않습니다. 부작용 없는 저널 인자 변환과 관측 상태 검증은 기존 레코드 모듈에 유지하며 기존 가져오기 경로와 근거 의미를 보존합니다. 저널 쓰기, 활성 스냅샷 읽기 및 교정 종결도 집중된 영속 계층 인접 모듈에 둘 수 있습니다. 기존 모듈은 호환 표면을 다시 내보내고 트랜잭션, 잠금 및 쓰기 소유권을 유지합니다. 의미 기능 투영은 바이트 예산 안에서 완전한 우선순위 descriptor만 내보내며, 너무 큰 속성 축은 일부만 표시하지 않고 생략합니다.
...
- **허용된 가져오기**: `shared/`는 `core/`를 가져오기하지 않습니다. `core/`는 `shared/`의
  ...
  단일 책임을 가진 인접 모듈은 정본 신원 변환과 해시 계산을 소유할 수 있으며 기존 소유 모듈은 해당 공개 기능을 다시 내보냅니다. 분석기 발견 증적, 분석기 작업 구성과 조립, 인벤토리 스냅샷 맥락 보조 로직도 같은 분리를 따릅니다. 기존 모듈은 공개 계약을 계속 다시 내보내며 쓰기 담당, 프로바이더 또는 권한은 이동하지 않습니다. 멱등성 예약의 안정된 작업 비교도 이 분리를 따르며 직렬화 바이트, 전이 검증, 재생 의미는 바뀌지 않습니다. 버전이 있는 최종 측정 계약도 같은 서비스 경계를 따릅니다. Core는 정규화 이벤트의 분류를 감사 기록과 원자적으로 보존하고 재구축 가능한 상태 변환 결과는 최신 50,000개로 제한하며, Operator는 Core를 가져오거나 분류를 실행 및 효과 권한으로 해석하지 않고 읽습니다. 변환 결과 보존은 추가 전용 감사 항목을 제거하지 않습니다. 브로커가 이벤트를 다시 전달하면 Core는 경로를 선택하기 전에 안정적인 정규화 이벤트 신원을 보존된 최종 측정과 비교하고, 정확히 일치할 때만 중복 결과를 반환합니다. 충돌은 Rule, 유사도 또는 모델 평가 전에 안전하게 실패하며, 대기 중인 최종 측정 하나의 충돌이 이후 재시도 처리를 막지 않습니다. 버전이 있는 운영 활동도 같은 경계를 따릅니다. Core는 스키마로 검증되고 개인정보 범위가 제한된 기록을 내보내며, Operator는 읽기 모델만 저장하고 중계하고 Console은 표현만 지역화합니다. 충돌 근거는 원본 토큰을 바꾸지 않고 기계 판독에 안전한 활동 사유 코드로 매핑합니다. 중복 확인 응답은 일치하는 보존 기록을 요구하며, 충돌 때문에 원래 분류를 조용히 대체하거나 버리지 않습니다. 측정 시각에는 시간대가 명시된 datetime 또는 ISO 8601 문자열을 사용하고 숫자를 암묵적으로 epoch 시각으로 바꾸지 않습니다. 인벤토리 진행률도 같은 소유권 분리를 따릅니다. 공유 패키지는 개수 전용 계약을 소유하고 Core만 추가 전용 원장을 기록하며, Operator는 `SELECT` 권한으로 허용 목록을 투영하고 Console은 Blob, 수집, 승격, 재시도, 승인 또는 실행 권한을 얻지 않습니다.
- **사람 승인 권한은 서비스별로 분리**: Operator는 Teams/Slack 인증, `cryptography`를 통한 로컬 JWT/JWK 암호학적 검증, 콜백 감사, 영속 결정 발신함을 소유합니다. Core 구현을 가져오거나 실행기 신원을 받지 않습니다.
  Core는 타입이 지정된 결정만 소비하며 워크플로 슬롯은 레지스트리로, 승인 대기 작업은 HIL 조정기 또는 정확한 사용자 접근 경로로 전달합니다. 활성 HIL 큐 조회는 `StateStore`에서 대기 중인 수명 주기 상태만 필터링합니다. 해결된 park와 추가 전용 감사 근거는 계속 보존되며 승인 경로에 다시 들어갈 수 없습니다. Core가 소유한 PostgreSQL 부분 인덱스는 승인 또는 보존 권한을 옮기지 않고 이 범위 제한 조회를 지원합니다. Core 조립 루트는 별도로 연결된 Bot Managed Identity를 Teams A1 전달에 사용하며 실행기 신원을 재사용하지 않습니다.
  [Slack A1 연결](../../../services/core-control-plane/src/fdai/delivery/chatops/slack_binding.py)은 Teams 신원 없이 조립 계층이 소유한 HTTP 클라이언트를 사용합니다. 로컬 전송 증적과 보류 상태의 조회는 아직 없는 영속 발송 조정이나 인증된 브라우저 행위자 결합을 대신할 수 없습니다. [채널과 알림](../interfaces/channels-and-notifications-ko.md)을 참조하세요.
- **예약 결과 복사본은 하나의 삭제 소유자 아래 유지**: `core/scheduler/continuation_retention.py`가 보존 순서, legal hold, 지속적인 삭제 fence, 다른 reader가 재사용하는 fence 확인을 소유합니다. `delivery/persistence/postgres_scheduled_continuation_retention.py`는 projected 대화 턴, 출처 briefing 실행, anchor 행에 대한 범위가 제한된 삭제 및 독립 readback 구문을 소유하며 보존 정책을 추가하지 않습니다. `core/scheduler/continuation_delivery.py`는 이미 저장된 anchor 하나를 영속 outbound ledger로 재생하며 예약, 재생성, 실행 권한을 갖지 않습니다. Anchor 생성과 외부 전달이 같은 fence를 확인하므로 재생이나 재전달이 삭제된 본문을 복원할 수 없습니다. Delivery persistence 패키지는 `PostgresScheduledContinuationDeleter`와 `RetentionReadbackError`를 다시 내보내므로 조립이 다른 모든 어댑터와 같은 facade로 삭제기를 연결하며, 조정기는 식별자를 다시 쓰지 않고 채널 렌더링을 제한합니다. `ScheduledContinuationService`는 fence를 필수 협력자로 받으므로 어떤 조립도 tombstone을 확인하지 않고 anchor를 생성할 수 없습니다. 자기 readback을 통과하지 못한 fence 쓰기는 모든 삭제를 막으며, fence 키 접두사는 보존 정리 대상이 아니라 영구적입니다. `shared/providers/conversation_delivery.py`는 메모리 내 원장의 출처 참조, 프로세스 범위 tombstone, 저장소 내 부재 확인을 갖춘 출처 범위 삭제를 소유하며, 시도 및 확인 응답 계보를 유지하고 예약이나 보존 정책 권한은 갖지 않습니다. Operator 소유 PostgreSQL 원장에는 별도의 영속 구현이 필요합니다. `core/scheduler/continuation_delivery.py`는 기록된 출처 fence가 있을 때만 이 삭제를 호출하므로 ledger는 자체 쓰기 담당을 유지하고 출처는 삭제 의도를 유지합니다.
...
- **콘솔에는 privileged 신원이 없음**: 상태, 감사, shadow 결과, HIL 큐를 시각화합니다. 접근 권한은
  ...
  snapshot을 원자적으로 promote합니다. 승격된 각 Resource는 계속 정확한 결정론적 Rule 평가에
  들어갑니다. 완전한 기준선 관측에 일치하는 Rule이 없으면 Core는 T0 발견 사항 없음을 기록하고
  T1 유사도 또는 T2 RCA를 호출하지 않습니다. Heimdall은 최신성, lag, 범위를 관찰하며 복구를 시작하지
...
독립 서비스 계획 가드는 facade에 전환 조정을 유지하고 계획 형태, 컨테이너, 신원 및 런타임 비교는 구조 디코더에 위임하며 URL, 모델 엔드포인트 및 데이터베이스 호스트 검증은 바인딩 정책 모듈에 위임합니다. 정책 평가 전에 계획 바이트, 리소스 항목, 표류 항목 및 재귀 차이 깊이에 상한을 적용합니다. 이 분리는 서비스 범위, 전환 허용 목록, 롤백 경계, 이미지 요건 또는 실행 권한을 바꾸지 않습니다.
