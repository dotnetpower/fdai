...
translation_source_sha: 1850a8d49622b4c4bec8f74876bc61859070824c
...
- **모델 카탈로그 신원은 가능한 경우 발행기로 한정**: Core는 계열 전용 adapter 계약을
  ...
  충족하지 못하게 합니다. 결정론적 의미 사전 프레임 선택은 함수 요약, 온톨로지 추적, 서비스
  담당 프레임을 서로 다른 타입 후보로 유지한 뒤 읽기 전용 프레임 하나를 선택합니다. 호환성
  facade는 안정적인 import 이름을 유지하고 집중 모듈이 helper 구현을 소유합니다.
- **자격 검증 축약에는 권한이 없음**:
