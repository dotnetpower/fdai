...
translation_source_sha: 8852905ef90719f02a30098845e95a94dc3e9054
...
## 모듈 경계(모듈 Boundaries)
[알림 과다 수신 관리](../operations/alert-noise-governance-ko.md)가 타입이 지정된 근거, Process 및 조건부 수동 PR 경계를 소유합니다. 전용 Operator 요청 조립 모듈이 변환 결과, 선호 설정, 이력과 감독되는 브리지 하나를 연결하며 공통 루트는 수명 주기만 소유합니다. 코덱은 서비스 구현을 공유하지 않으며 생성/오프라인 근거는 권한이나 실제 운영 인증을 부여하지 않습니다. 카탈로그 테스트는 채택되지 않은 과거 R7 제안이 아니라 현재 실행 경로 계약을 따릅니다.
