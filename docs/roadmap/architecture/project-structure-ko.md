...
translation_source_sha: f23077b2eb05c8ec815c117755bcb258cd5fa62b
...
- **모델 카탈로그 신원은 가능한 경우 발행기로 한정**: Core는 계열 전용 adapter 계약을
  ...
  필요한 경우에만 partner private endpoint/DNS를 생성합니다. Runtime은 exact account 참조의
  범위 제한 map을 해석하고 provider/hostname 불일치를 차단합니다.
- **자격 검증 축약에는 권한이 없음**:
