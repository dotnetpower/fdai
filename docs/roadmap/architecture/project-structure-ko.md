...
translation_source_sha: 3c62120f7d596498eeaf17d105df4fc7a7d7d824
...
- **논리 서비스 조회 소유권**: `semantic_logical_service_planning.py`만 승인된 BusinessService 또는 Workload의 정확한 id, 이름, alias를 typed 워크로드 및 Resource 경로로 컴파일합니다. 모델 계획, 프로바이더 이름, 태그 또는 레이블로 이 서버 소유 읽기를 대체할 수 없으며 표현 계층은 실행 권한이 없는 검증된 행만 사용합니다. 생성된 System Knowledge 카탈로그는 이 설계 문구를 색인할 수 있지만 런타임 인스턴스 또는 조회 권한을 부여하지 않습니다.
