...
translation_source_sha: f8680785216fc73f225c28610974089943f44904
...
`bootstrap_bindings.py`에 위임하여 프로바이더 construction을 범위가 제한된하게 유지합니다. Thin 래퍼는 테스트와
포크가 사용하는 injectable identity-builder 경계를 보존합니다.

> 디렉토리 이름은 정본 어휘(정본 vocabulary)입니다. 모듈 이름은
...
> `remediation-pr`, `shadow-mode`, `HIL`) 와 정렬해서 유지하세요. Core 테스트는
> `services/core-control-plane/tests/` 아래에서 출처 배치를 미러링합니다. 독립 서비스와
> shared SDK는 각 패키지 옆의 테스트를 소유합니다. 서비스 간 및 repository-wide 검사만
...
- **허용된 가져오기**: `shared/`는 `core/`를 가져오기하지 않습니다. `core/`는 `shared/`의
 계약, 프로바이더, telemetry, 구성만 가져옵니다. `delivery/`는 어댑터 경계 뒤에서
...
- **콘솔에는 privileged 신원이 없음**: 상태, 감사, 그림자 결과, HIL 큐를 시각화합니다.
 역할이 할당된 principal의 접근은 검증된 App 역할에서 직접 도출되며 선택적 access-request
 변환 결과에 의존하지 않습니다. Command 표면은 인증된 기록 또는 타입이 지정된 제안을 제출할 수 있고
 dev-only 서비스 어댑터는 Azure CLI auth로 서술할 수 있지만 어느 경로도 리소스 실행기를 직접
 호출하지 않으며 risk, 승인, 감사, 실행기 경계는 서버 측에 유지됩니다
 ([security-and-identity-ko.md](security-and-identity-ko.md) 참조).
 독립 Operator 서비스는 의미 전송 계층이 활성화되면 하나의 semantic-aware 어댑터를 통해
 변환 결과, 제안 및 스트림 포트를 함께 연결합니다. PostgreSQL 발신함은 점유 충족 여부와 임차 기간
 기한에 데이터베이스 `NOW()` 값을 사용합니다. Held 변환 결과 신원은 요청 신원과 정본
 terminal-result 다이제스트를 결합하고 영속 결과 키는 요청과 변환 결과 신원을 결합합니다.
 기존 결과를 재사용할 때는 같은 트랜잭션에서 요청, principal 및 다이제스트를 검증한 후 발신함 행을
 완전한합니다.
 ...
 토큰 엔드포인트는 userinfo, 경로, 조회, fragment가 없는 HTTPS 출처 URL이어야 합니다.
 ...
 정렬된 lock 순서로 획득합니다. Resource lock은 음수 키 범위의 seeded 63-bit 참고용 키를
 ...
 realtime 리소스 overlay가 하나라도 pending 상태이면 base 스냅샷이 최신성 예산 안에 있어도
 그래프 최신성은 `unknown`이고 읽기 변환 결과는 degraded 상태입니다. 완전한 조정
 승격이 포함된 overlay를 정리하면 스냅샷 기반 최신성이 복원됩니다.
 ...
 Event 타입을 명시한 페이로드는 `inventory.resource_changed`인 경우에만 변환 결과됩니다. 다른 domain
 ...
 정상 6시간 검사 간격을 유지하며 newer 실패한/abandoned 시도는 다음 tick에 재시도합니다. 로컬 실행 장치는
 Azure 발견을 실행하지 않습니다.
...
| 게이트 | 규칙 | 현재 모드 |
|--------|------|-----------|
| [check-core-imports.sh](../../../scripts/quality/architecture/check-core-imports.sh) | `core/` 는 클라우드 SDK, HTTP 클라이언트, `fdai.delivery.*` 를 가져오기 금지 | enforce |
...
| [check-subsystem-fanout.sh](../../../scripts/quality/architecture/check-subsystem-fanout.sh) | 한 파일이 `core.*` 형제 subsystem 을 8개 이상 가져오기 하면 warn, 15개 이상이면 enforce 모드에서 fail | warn-only |
...
4. `services/core-control-plane/tests/test_check_structural_gates.py` 에 warn / enforce / 임계값 재정의 /
...
- **Azure operational 근거**: `bind_azure_operational_evidence`는 strict promoted-inventory
 스냅샷 읽기 담당, 현재 안전성 evaluator, 구성된 Azure 메트릭, 범위가 제한된 가지 estimator,
...
같은 bundle이 제공하는 도구 또는 기존 `ActionType`, `Workflow`를 가리킵니다. 별도 실행
...
프로바이더와 bundle이 일치하지 않거나, 패키지 도구 또는 프로바이더가 참조되지 않거나, 패키지
...
읽기 전용 프로바이더와 bundle은
[Core 패키지 루트](../../../services/core-control-plane/src/fdai/)를
참조하세요.

배포에서 해당 bundle에 install, 활성화, 비활성화, uninstall 수명 주기가 필요하면
...
설치됩니다. 활성화는 변경할 수 없는 base와 활성화된 bundle 전체에서 후보 `CapabilityRuntime`을
...
`fdai.skill-bundle-signature.v1` domain을 사용합니다. 시작은 skill을 bundle보다 먼저 재구성해
...
phase 는 `core/` 를 편집하지 않고 조립 루트 에서 새 구현을 등록합니다.
...
| **통제된 명령, 셸 작업 및 코드 workspace** | `shared/providers/`의 `CommandRunner`, `CommandPlan`, `ShellTaskChecker`, `ShellTaskSpec`, `CodeWorkspaceProvider`, `CodePatchSet`; `core/tools/` 및 `core/python_task/`의 `CommandCatalog`, 기본값 명령 spec, 셸 structural 검증, workspace patch 검증 | - | `RecordingCommandRunner`, `BashSyntaxChecker`, 명시적 선택 `BubblewrapCommandRunner`, copy-on-write `GitCodeWorkspaceProvider`, 타입이 지정된 `azure.resource.list`, `azure.group.list`, `azure.vm.list`, `azure.vm.status` 읽기용 명시적 선택 `AzureCliCommandRunner`; 로컬 VM 인벤토리는 `az vm list --show-details`를 사용합니다. 생성된 Python은 `process`를 거부하고 셸 산출물은 validate하지만 실행하지 않으며 upstream 앱은 기본적으로 실제 운영 실행기를 연결하지 않습니다. | 포크는 credential-free 로컬 실행기와 비공개 workspace 프로바이더 또는 credentialed Azure 읽기 브로커를 연결할 수 있습니다. 서버가 소유한 범위 및 신원, 결정론적 argv 렌더링, raw 명령 문자열 금지, stale-file 해시 검사, 멱등성, 출력 한계, `tool_call` / `direct_api` / `run_runbook` 경로 분리를 유지해야 합니다. [(4[56]) § 4.6](../decisioning/workflow-control-loop-integration-ko.md#46-governed-command-및-shell-artifact) 참조. |
...
점유되고, 리소스별 lock은 전달 어댑터가 상태를 변경하기 전에 경합하는 적용을 직렬화합니다.
