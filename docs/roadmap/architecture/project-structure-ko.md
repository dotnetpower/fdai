...
translation_source_sha: 752ce7eac4728b209fb70385635737e9f570f433
...
- **자격 검증 축약에는 권한이 없음**:
  ...
  밖의 서로 다른 모델 계열 검토자는 상관관계가 연결된 30점 진단을 추가합니다. 하위 캠페인 종료는
  시도한 사례 ID의 순서와 판단 보류 사례 ID만 보존하므로, 이후 명시적 캠페인은 라이브 요청을
  재시도하지 않고 이전 시도를 제외할 수 있습니다. 로컬 캠페인 CLI는 별도의 소유자 전용
  transcript에 범위가 제한된 질문과 답변 내용, 답변 및 평가 모델 귀속을 보존할 수 있습니다.
  민감한 본문은 생략하며 이 transcript는 추적되는 qualification, 정책, 감사 또는 권한 근거가
  아닙니다. 일반 `operations-review` 턴은 등록된 함수 권한의 변경할 수 없는 스냅샷을 준비 상태
  소비자에게 노출하는 기존 의미 런타임을 계속 사용합니다. 스키마로 검증된 판단은 활성 매니페스트와
  정본 주체 및 항목이 일치하는 일반 타입 프레임만 복구할 수 있습니다. Golden 인증은 정확한 예상
