...
| **Runtime-call 근거 변환** | `services/core-control-plane/src/fdai/core/ontology_platform/runtime_call_projection.py`의 `RuntimeCallObservation`과 순수 `project_runtime_call(...)` | - | Unbound 상태이며 exact, fresh, principal-readable endpoint는 권한 없는 candidate link만 만들 수 있고 graph에 영속하지 않습니다. | Exact-release, active-generation, scope, freshness, independent-verifier, no-authority 검사를 보존하면서 인증된 타입 지정 telemetry producer와 inventory single writer를 binding합니다. |
