...
| Schema relationship conversation query | implemented | [`relationship_queries.py`](../../../services/core-control-plane/src/fdai/core/ontology_platform/relationship_queries.py), [`wire_semantic_query.py`](../../../services/core-control-plane/src/fdai/composition/wire_semantic_query.py), [`semantic_turn_processor.py`](../../../services/core-control-plane/src/fdai_core_service/semantic_turn_processor.py), and focused English/Korean composition, prompt, processor, and stale-release checks (`6 passed`) | `query.ontology_relationships` reads exact ObjectType and LinkType declarations, preserves direction, cardinality, and description, renders English or Korean, and fixes `execution_authority=false`. Authenticated Browser evidence remains open. |
...
- [ ] Retain an authenticated Browser receipt showing that an ObjectType relationship question returns the exact LinkType direction, cardinality, description, release-bound evidence, and `execution_authority=false`.
