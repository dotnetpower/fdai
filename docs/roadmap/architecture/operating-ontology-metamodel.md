...
| Relationship direction and classification hardening | in-progress | The direction contract and `resource_classified_as` design in this document; [`kubernetes_relationships.py`](../../../services/core-control-plane/src/fdai/delivery/kubernetes_relationships.py), [`direction_shadow`](../../../services/core-control-plane/src/fdai/core/ontology_platform/direction_shadow), and their focused tests. | Reviewed Kubernetes mappings cover the D3 adversarial inputs. The D4 comparator, replay check, directional and blast-radius deltas, and rebuild pointer exist. The full D1 producer audit and a retained comparison receipt from production generations remain open. |
...
- [ ] Complete the D1 audit across every shipped relationship producer, then run the existing D4 comparator over retained legacy and aligned production generations and preserve its reviewed receipt and rebuild pointer before migration.
