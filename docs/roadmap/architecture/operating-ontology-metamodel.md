...
| Relationship direction and classification hardening | in-progress | The direction contract and `resource_classified_as` design in this document; [`kubernetes_relationships.py`](../../../services/core-control-plane/src/fdai/delivery/kubernetes_relationships.py), [`direction_shadow`](../../../services/core-control-plane/src/fdai/core/ontology_platform/direction_shadow), [`inventory_ontology.py`](../../../services/core-control-plane/src/fdai/runtime/inventory_ontology.py), and focused tests. | D1 and D3 are covered, and new inventory projection manifests and statuses now pin the exact ontology release. Historical generations remain honestly unbound, so a complete retrospective D4 receipt cannot be claimed. |
| Network and Pod telemetry competency | in-progress | [`operational_functions.py`](../../../services/core-control-plane/src/fdai/core/ontology_platform/operational_functions.py), [`test_wire_semantic_query.py`](../../../services/core-control-plane/tests/composition/test_wire_semantic_query.py), and [`test_wire_pod_telemetry.py`](../../../services/core-control-plane/tests/composition/test_wire_pod_telemetry.py). | Production composition now proves issued network and Pod function dependencies, including verified and synthetic-unverified Pod segments. Live inventory binding and retained assurance receipts remain open. |
...
- [ ] Refresh inventory to create release-bound generations, then run the existing D4 comparator and preserve its reviewed receipt and rebuild pointer. Treat historical unbound generations as `review_required`, never as complete migration evidence.
- [ ] Bind the reviewed Kubernetes relationship projector through production inventory composition, then run the passing VM connectivity and Pod telemetry competency checks against retained inventory evidence on the exact release.
