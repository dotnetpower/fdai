...
| Relationship direction and classification hardening | in-progress | The direction contract and `resource_classified_as` design in this document; [`kubernetes_relationships.py`](../../../services/core-control-plane/src/fdai/delivery/kubernetes_relationships.py), [`direction_shadow`](../../../services/core-control-plane/src/fdai/core/ontology_platform/direction_shadow), and their focused tests. | An exact audit pins every shipped mapping to its LinkType, endpoint orientation, and reference format. D3 adversarial inputs and the D4 comparator are covered. Runtime producer and ownership-rule audits plus a retained production comparison receipt remain open. |
...
- [ ] Complete the D1 audit across the remaining runtime relationship producers and ownership rules, then run the existing D4 comparator over retained legacy and aligned production generations and preserve its reviewed receipt and rebuild pointer before migration.
