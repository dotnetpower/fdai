...
| Relationship direction and classification hardening | in-progress | The direction contract and `resource_classified_as` design in this document; [`kubernetes_relationships.py`](../../../services/core-control-plane/src/fdai/delivery/kubernetes_relationships.py), [`direction_shadow`](../../../services/core-control-plane/src/fdai/core/ontology_platform/direction_shadow), and their focused tests. | D1 now pins the five Resource LinkType declarations, all 17 provider mappings, Azure and Kubernetes producer direction, complete-generation verification, and delta ownership. D3 adversarial inputs and the D4 comparator are covered. A retained production comparison receipt remains open. |
...
- [ ] Run the existing D4 comparator over retained legacy and aligned production generations and preserve its reviewed receipt and rebuild pointer before migration.
