...
| Global provider schema accounting | implemented | [`provider-schema-catalog`](../../../provider-schema-catalog/index.json); `provider_schema.py`; `provider_relationship_schema.py`; `provider_schema_state_ledger.py`; focused parser, ledger, watcher, agent, catalog, and infrastructure checks | The pinned Azure Bicep corpus retains 3,405 unique types and explicit dispositions for all of them. A pinned Azure REST corpus separately retains 6,896 ARM ID references, including 4,707 exact and 2,189 unresolved references. The 62 modeled types remain a reviewed semantic subset. The daily Job hydrates and persists the raw ledger through PostgreSQL and sends strict material packages through Heimdall's existing shadow `object.drift` ownership. No record grants ontology or execution authority. |
