...
| Versioned relationship candidate materialization | implemented | `provider_schema_relationship_generation.py`; `provider_schema_relationship_ledger.py`; `test_provider_schema_relationship_generation.py`; `direction_shadow` exact-release comparator | Exact provider-schema and REST evidence digests, mapping revision, projection manifest, explicit direction/cardinality/link metadata, changed-type invalidation, bounded candidates, rollback, and replay are content addressed. Partial, stale, duplicate, or mixed-release inputs remain inert and promotion is proposal-only with graph and migration authority fixed false. |
