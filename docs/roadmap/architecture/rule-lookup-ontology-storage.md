...
- `t2_cache` is partitioned by `catalog_version`, and `expires_at` is the TTL read guard.
  ...
  audit history.
- UUID primary keys are generated by PostgreSQL. `action_signature` is unique within
  `catalog_version`, while `input_hash` and catalog digests provide stable correlation keys used
  for idempotent writes and replay.
- The bounded digest tombstone set refuses a new version when capacity is exhausted instead of
  forgetting an accepted identity.
- A service migration downgrade refuses to remove version-aware uniqueness while duplicate
  `action_signature` values span catalog versions. Resolve the collision first so rollback never
  silently drops a valid learned action.
