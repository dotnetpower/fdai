...
translation_source_sha: 1ba1fcae63232b7569caa7aab7b6e27239216221
...
| SD-06 read-only Operator projection | `/home/moonchoi/dev/fdai-worktrees/sd06-operator-projection`의 persistent worker. 완료된 core worker는 read-only로 유지합니다. | `src/fdai/delivery/operator_api/projections/change_lineage/**`와 `tests/delivery/operator_api/test_change_lineage_projection.py`만 소유합니다. | Pure bounded projection이 candidate seal gate와 authority 0을 보존하고 Starlette, route, app, persistence, composition, shared-contract, core를 변경하지 않음을 증명합니다. HTTP registration은 SD-01 route owner에게 계속 defer합니다. |
