...
translation_source_sha: 5bbf2706bf5cd54db2924ab8c5d283481180ca97
...
|------|------------|-----------|-----------|
| SD-01 conversation persistence | `/home/moonchoi/dev/fdai-worktrees/sd01-conversation-persistence`의 persistent worker | `chat_history.py`, `chat_image_history.py`, `chat_document_evidence.py`, destination persistence/projection package, 해당 test와 module-map 업데이트 | Persistence handoff가 idempotent transcript write, image compensation/finalization, document evidence identity를 보존하고 persistence/projection-to-route import가 0임을 증명합니다. |
...
| SD-06 canonical Change lineage | `main`의 integration owner. 완료된 core worker는 `/home/moonchoi/dev/fdai-worktrees/sd06-change-lineage`에 read-only로 유지합니다. | `52dbb2ba3` 후 core 예약을 해제했습니다. 남은 read-only Operator projection은 SD-01 route handoff를 기다립니다. | Canonical lineage, decision/resilience trace, candidate-only learning, GitHub/Azure DevOps provider compatibility가 focused test를 통과하고 execution 또는 promotion authority 0을 증명합니다. 어떤 worker도 shared contract, agent, composition, Operator API, ingestion, infra 또는 이 plan을 다시 열 수 없습니다. |
