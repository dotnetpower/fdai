...
translation_source_sha: dba47a347078a320898ba4fb9d290fd1486a3e0e
...
|------|------------|-----------|-----------|
| SD-01 final route closure | `/home/moonchoi/dev/fdai-worktrees/sd01-route-closure`의 persistent worker | `routes/chat.py`, `routes/chat_stream.py`, `application/conversation` 아래의 새롭거나 기존인 explicit typed application coordinator destination(구현 후 `turn_execution` 또는 `turn_runtime` naming 선호), 해당 JSON/SSE test, structural boundary test, Operator module inventory/map 쌍 및 SD-01 evidence | 두 route body에 transport, authentication, request parsing, status, frame 및 cancellation 책임만 남고 planning, evidence, persistence 또는 metering lifecycle orchestration이 없어졌을 때만 해제합니다. JSON, SSE, authentication, cancellation, history 및 route contract를 변경하지 않고 application/projection/persistence-to-route import 0을 유지합니다. |
