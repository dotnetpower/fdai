...
translation_source_sha: 0fb3a70f7986eea6be85a51d57cecdbae25c97f8
...
- **전역 kill-switch** 는 모든 auto-execution을 즉시 중단하고 모든 경로를 shadow/HIL로 드롭;
  ...
  `kill_switch` ceiling axis 로 실현함 ([execution-model.md](../decisioning/execution-model.md)
  2.6b); upstream 은 in-memory 기본값을 제공하고 fork 가 그 상태를 state store 에 backing 해
  switch 를 durable + cluster-wide 로 만듦.
