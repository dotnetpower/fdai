...
translation_source_sha: 4b7426d64e6da6095ce651301594785f8122735d
...
| 자동화 보류 복구 승인 강화 | implemented | `core/workflow/{recovery_admission,automation_hold,recovery_coordinator*,recovery_effect_ingress,compensation}.py`; `delivery/persistence/{workflow_recovery,workflow_approval}.py`; `delivery/workflow_recovery_observation_handler.py`; `core/executor/safeguard_lifecycle*.py`; 보호 조건을 적용한 메모리 내 및 PostgreSQL 상태 어댑터; [프로세스 자동화 구현 상태](../../roadmap-implementation/decisioning/process-automation.md#implementation-status); 이슈 `#622`, `#630`, `#640`, `#652`, `#656`, `#658` | 이제 운영 보상은 보호된 해제로만 보류를 닫으며 기존의 검증된 복구 호출은 제거했습니다. 완전한 허용 판정이 모든 공급자 디스패치를 막고, 배타적인 compare-and-set 청구 하나가 그 디스패치를 소유하며, 실행기 fence는 해당 Process 단계에 결속된 디스패치 권한 부여에서 기대 계보를 읽습니다. RiskGate의 보류 예외도 단계 이름이 아니라 같은 증거를 읽으므로, 최신 보류 범위 권한 부여를 가진 표준 `recover_<시도>` 단계만 보류를 넘을 수 있습니다. 독립 효과 관측은 관측자 주체를 인증하고 실행기 작성, 공급자 작성, 합성, 비최종, 비봉쇄, 오래된 근거를 거부하며 중복과 순서 뒤바뀜에 안전한 단일 버전 관측자 경로 수집 지점으로만 들어옵니다. 이 수집 지점은 Heimdall이 소유한 `object.recovery-effect-observation` 토픽만 읽고 `Heimdall`만 허가하므로, 외부 관측은 Huginn 정규화와 Heimdall 중계를 거쳐야 수집 지점에 도달할 수 있고 유일한 특권 실행기는 그 경로에서 어떤 토픽도 소유하지 않습니다. 복구 승인 요청, 확정된 안전조건 묶음 보존, 독립 효과 관측에는 모두 운영 기록기가 있습니다. FDAI-CONST-009는 `implemented`를 유지합니다. |
...
| 사전 사람 권한 부여(A3-E) | in-progress | `config/constitution-traceability.json`의 `FDAI-CONST-008` 요구 사항; [에스컬레이션과 사전 권한](../decisioning/escalation-and-standing-authority-ko.md); `core/standing_authority/{lease,provider_eligibility,promotion_candidate*,shadow_cohort_runner,shadow_cohort_cli,shadow_cohort_corpus,effect_shadow_reversion,shadow_reversion_command,provider_acceptance*}.py`; `delivery/azure/{vm_start_shadow_fence,vm_power_state,vm_power_state_observation}.py`; 완료된 이슈 `#331`, `#621`, `#629`, `#631`; 이슈 `#632` | 기존의 엄격한 lease 계약은 바뀌지 않습니다. 새 process-local acceptance 모델은 I/O 전에 기록하고 모든 중복 또는 불명확한 시도를 차단하며 `production_eligible=false`를 유지합니다. 이 모델은 분산 원자성이나 효과 성공이 아닙니다. 모든 A3-E 계약은 연결하지 않은 상태이고 선택한 ActionType은 계속 부적격입니다. 프로바이더 커밋 fence 자격은 이제 후보 작성자가 선언하지 않고 비어 있는 어댑터 레지스트리에서 도출되므로, 배포된 모든 ActionType이 `INELIGIBLE_CAPABILITY`로 분류됩니다. 나중에 승인된 어댑터가 실패한 효과 검증에 대응할 수 있도록 콘텐츠 주소 shadow 복귀 명령, 작성기 Protocol, 안전한 방향으로 실패하는 조율 이음새를 추가했지만, 이 Protocol을 구현한 어댑터는 없고 변경 이음새는 패키지 이름 공간에 노출되지 않습니다. 새 집단 CLI는 로컬 합성 전용이며 로컬이 아닌 venue나 근거 등급을 거부합니다. 이 CLI의 자료 집합 스키마는 프로바이더 자격을 선언할 수 없으므로, 등록된 어댑터가 하나도 없는 상태에서 실제 케이스는 모두 부적격으로 유지됩니다. 적격한 프로바이더는 하나도 없으며 #632의 통제된 런타임 집단, 독립 검토, 현재 사람 승인은 계속 열려 있습니다. |
### 구현 이력
...
| 2026-09-13 | in-progress | 호출자가 A3-E 프로바이더 자격을 선언하던 표면을 제거했습니다. `provider_eligibility.py`는 비어 있는 명시적 어댑터 레지스트리에서 커밋 fence 수행 능력을 도출하고, 승격 후보 빌더는 더 이상 부적격 ActionType 집합을 받지 않으며, 레코드 생성자는 손으로 선언한 구분을 거부합니다. 따라서 배포된 모든 ActionType이 `INELIGIBLE_CAPABILITY`로 도출되고 실제 후보는 생성 시점에 거부됩니다. 이 모듈은 순수하고 연결되지 않았으며 실행 또는 승격 권한을 부여하지 않습니다. | `current change`; `core/standing_authority/provider_eligibility.py`; `tests/core/standing_authority/test_provider_eligibility.py`; 집중 상시 권한 테스트 242개 통과(신규 18개). 배포된 ActionType 49개 전체 카탈로그 검사와, 어떤 권한 경로도 하위 모듈 경로나 패키지 재노출로 이 모듈을 가져오지 않고 어떤 배포 모듈도 레지스트리를 변경하지 않는다는 정적 증명을 포함합니다. 독립 비평이 거짓 음성 import 가드와 기존의 무의미한 검사 경로를 찾아 수정했습니다. Ruff, 포맷, strict mypy 통과. | 적격한 프로바이더는 하나도 없습니다. 효과 커밋 시점에 lease와 fencing 세대를 원자적으로 검증하는 영속 프로바이더 경계, #632의 통제된 런타임 집단, 독립 검토, 명시적 현재 사람 승인, 보존된 효과 관측은 모두 열려 있습니다. |
| 2026-09-12 | in-progress | 콘텐츠 주소 신원, 대상별 1회 준비, 정확한 재읽기, 보수적인 불확실성, 취소 처리, 재제출 금지를 갖춘 process-local provider acceptance 순서 모델을 추가했습니다. 이 모델은 명시적으로 권한을 부여하지 않으며 운영 provider fence가 아닙니다. | `current change`; `provider_acceptance*.py`; 집중 결정론적 동시성 테스트. | 영구 구현과 provider 상관관계를 독립적으로 검토할 때까지 엄격한 lease 요구사항과 부적격 결과를 유지합니다. |
| 2026-09-12 | in-progress | `ops.start-vm@1.0.0`을 위한 shadow-only 단일 VM provider fence, 독립 전원 상태 관측, 안전한 방향으로 실패하는 shadow 복귀 계약을 추가했습니다. 모든 출력은 콘텐츠 주소 및 무권한 상태를 유지하며 provider 변경, 레지스트리 쓰기, 런타임 연결 또는 복구 디스패치를 추가하지 않았습니다. | `current change`; 새 상시 권한 및 Azure 전달 모듈; 집중 테스트 통과. | A3-E 자격을 주장하기 전에 provider 원자성을 해결하고 검토된 의미 ActionType 선언을 추가하며 통제된 런타임 근거를 보존합니다. |
| 2026-09-11 | implemented | Heimdall 소유 토픽과 실행기 및 공급자 신원 분리에 더해 복구 효과 근거를 composition이 제공한 신뢰된 observer 신원 집합에 결속했습니다. payload는 보류를 해제할 수 있는 신뢰되지 않은 observer를 지정할 수 없습니다. | `current change`; 복구 관측 ingress 및 운영 writer 테스트. | 정확히 배포된 observer 신원의 통제된 런타임 근거를 보존합니다. |
| 2026-09-11 | implemented | 독립적인 복구 사후 효과 관측을 책임 있는 에이전트 경계 위로 옮겼습니다. 기존 수집 지점은 전용 소비자 그룹으로 공유 `object.event` 수신 토픽을 직접 구독하고 `Huginn`을 생산 주체로 허가했기 때문에, 그 토픽에 완성된 페이로드를 발행하면 경로에 관측자 에이전트가 하나도 없는 상태로 영구 수집 지점에 도달했습니다. 이미 최종 효과 관측자인 Heimdall이 이제 `RecoveryEffectObservation` 객체 타입과 여기서 파생되는 `object.recovery-effect-observation` 토픽을 소유합니다. 외부 관측은 `Huginn.ingest`로만 들어오고, Heimdall은 Huginn이 정규화했음을 입증한 뒤 허용 목록으로 범위를 제한한 레코드를 자신이 소유한 토픽으로 다시 발행합니다. 수집 지점은 그 토픽만 읽고 `Heimdall`만 허가합니다. 이 중계는 출처와 형태를 입증할 뿐 효과를 검증하지 않고 어떤 권한도 부여하지 않으며, 심판자, 승인자, 실행기 권한은 전혀 바뀌지 않았습니다. | `현재 변경`; `pantheon.py`; `topics.py`; `heimdall.py`; `heimdall_huginn_projection.py`; `runtime_subscriptions.py`; `recovery_effect_ingress.py`; `workflow_recovery_observation_handler.py`; 라우팅과 운영 복구 기록기 테스트가 이제 실제 Huginn 수집, Heimdall 중계, 팬테온 버스, 전용 관측자 구독 연쇄를 구동하며 수집 지점이 공유 수신 토픽을 결코 읽지 않음을 증명하는 음성 대조를 포함합니다. 에이전트, Workflow, 전달, 런타임 집중 검사 5,298건 통과, 데이터베이스 전제 3건 건너뜀; 소스 623개에 대한 Ruff와 strict mypy 통과. | 실시간 공급자 관측 근거는 이슈 `#633`의 배포 작업에서 확보합니다. |
| 2026-09-11 | implemented | 보류 복구 경로의 마지막 도달성 결함을 해소했습니다. RiskGate의 보류 예외는 이제 `compensate_` 단계 이름 접두사가 아니라 정확한 대상, Process, 표준 `recover_<시도>` 단계, 활성 보류 개정에 대한 최신 보류 범위 디스패치 권한 부여를 요구하므로 일반적인 `recover_` 문자열, 다른 Process의 계보, 대체된 개정, 읽을 수 없는 권한 부여는 모두 거부됩니다. 운영 보상 재개 호출자는 자신의 보류 확인 이전에 해제되었지만 최종 처리되지 않은 복구를 수리하므로, 비정상 종료한 복제본의 Process와 Saga 전달이 임의의 지연 이후에도 복구됩니다. 독립적인 사후 효과 관측은 관측자 주체를 인증하고 권위 등급, 시도 결속, 공급자 증적, 효과 및 봉투 다이제스트, 근거 구간, 최종성, 봉쇄를 검증한 뒤 `record_independent_observation`으로 영구 저장하는 버전이 지정된 타입 안전 관측자 경로 수집 지점으로만 들어오며, 관측자나 수집 결속이 없으면 조용한 미검증 효과가 아니라 이름이 부여된 `observer_unavailable` 준비 상태 결과가 됩니다. | `현재 변경`; `automation_hold.py`; `recovery_effect_ingress.py`; `compensation.py`; `recovery_coordinator.py`; `workflow_recovery.py`; `workflow_recovery_observation_handler.py`; `runtime_subscriptions.py`; Workflow, 실행기, 제어 루프, 에이전트, 전달, 런타임 집중 검사 5,934건 통과, 데이터베이스 전제 3건 건너뜀; 소스 2,428개에 대한 Ruff와 strict mypy 통과. | 실시간 공급자 관측 근거는 이슈 `#633`의 배포 작업에서 확보합니다. |
| 2026-09-11 | implemented | 독립 검토 결과에 따라 운영 보류 복구 경로를 강화했습니다. 현재 승인, 정족수, 거부 없음, 만료, 자기 승인 금지, 실행기 분리, 현재 결정 근거 허용 판정을 모두 증명하기 전에는 공급자 디스패치가 일어나지 않습니다. 디스패치 직전 청구는 임대 기간이 있는 배타적 compare-and-set 진행 중 청구가 되어, 동시 복구에서도 공급자를 정확히 한 번만 호출하고 경쟁에서 밀린 호출자는 보류를 다시 발행하지 않고 미확정 상태로 남습니다. 실행기 fence는 현재 보류 기록이 아니라 해당 Process 단계에 결속된 영구 디스패치 권한 부여에서 기대 계보를 읽으므로, 그 권한 부여 이후 다시 발행되거나 해제되거나 다시 해제된 보류는 디스패치를 거부하며, 확정된 안전조건 묶음 보존은 자신의 공급자 호출을 명시하고 수명주기에 결속된 증적을 반환한 실행에만 적용됩니다. 충돌 복구는 영구 저장된 소비 청구와 해제의 결합을 검증하고 소비 시점을 기준으로 최종 처리하므로 뒤늦은 복구도 종결되며, 일치하지 않거나 철회된 계보는 계속 실패 시 차단됩니다. 복구 승인 요청, 확정된 안전조건 묶음 보존, 독립 효과 관측에 운영 기록기를 추가해 사전 주입 상태 없이도 경로가 완료되고, 기록기가 없으면 눈에 보이는 실패 시 차단 상태로 남습니다. | `현재 변경`; `recovery_coordinator.py`; `automation_hold.py`; `workflow_recovery.py`; `safeguard_lifecycle_coordinator.py`; `control_loop_support.py`; Workflow, 실행기, 런타임, 영속성, 계약 집중 검사 1,494건 통과; 소스 2,426개에 대한 Ruff와 strict mypy 통과. | 이슈 `#633`에서 경로 전반의 통제된 안전조건 근거를 완성합니다. |
...
- [x] 이슈 `#681`과 `#627`에서 공유 근거 수명 주기 조정기와 Core 생성기를 구현하고 연결합니다.
- [x] 이슈 `#628`에서 안전조건 결속 명령을 Isolated 실행기가 다시 검증합니다.
