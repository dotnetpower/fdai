...
독립 효과 검증은 같은 주체의 다음 단계가 아니라 별도의 신원 경계입니다. 관측은 관측자, 해당 효과를
디스패치한 실행기, 읽히는 권위 있는 소스가 서로 다른 세 신원일 때만 보존되며, 한 주체가 셋인 척할 수
없도록 대소문자를 구분하지 않고 비교합니다.

모든 관측은 정확한 안전장치 번들, Action, 대상, 소스 리비전, 영속 증적 레코드, 실행기 영수증, 실행
출처(경로, 오케스트레이션 출처, 실행 장소)에 결속된 추가 전용 영수증 하나입니다. 저장소는 `SELECT`와
`INSERT` 권한만 부여하므로, 모순되는 나중 관측은 기존 행을 수정하는 대신 새 행으로 남습니다.

영수증은 실행, sink 커밋, 잠금 해제, 승격 권한을 거짓으로 고정합니다. 따라서 침해되거나 혼란에 빠진
관측자는 증적 가용성을 떨어뜨릴 수는 있어도 효과를 일으키거나 완료하거나 해제하거나 승격할 수 없습니다.

신뢰 가능성은 결과를 받아들이기 전에 판정합니다. 증적이 합성이거나, 다른 소스와 충돌하거나, 선언된 신선도
한계를 넘었거나, 잠정적이거나, 불완전하거나, 선언된 논리 대상 안에 있음을 증명하지 못하면 관측을
하향 조정합니다.

| 관측 결과 | 처분 | 무엇을 승인하는가 |
|-----------|------|-------------------|
| `verified` | 효과 검증됨 | 추가 조치 없음. 효과는 종결되고 최종입니다. |
| `failed` | 복구 필요 | 비활성 복구 요청만 가능합니다. 복구는 일곱 가지 안전장치를 모두 갖춘 별도 승인 Action입니다. |
| `missing`, `stale`, `conflicting`, `censored`, `unavailable` | 미상 보류 | 없음. 재시도, 새 효과, 잠금 해제, 승격 모두 불가합니다. |
