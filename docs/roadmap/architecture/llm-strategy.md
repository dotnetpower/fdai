...
| Publisher-qualified Foundry catalog, infrastructure, and endpoint contract | implemented | Resolver, partner module, endpoint resolver, sealer, Terraform endpoint output, semantic and post-turn consumers, and focused checks | Root Terraform splits publishers and private networking, exports exact account-qualified origins, and supplies them to independent Core through protected materialization. ChatOps validation requires the exact sealed Foundry secondary before planning. Live deployment remains open. |
