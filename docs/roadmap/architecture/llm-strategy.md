...
| Publisher-qualified Foundry catalog, infrastructure, and endpoint contract | implemented | Resolver, partner module, endpoint resolver and sealer, focused checks | Root Terraform splits publishers and private networking. Partner endpoint bindings are sealed before plan metadata. Runtime endpoint-map delivery and registry activation remain open. |
