...
| Publisher-qualified Foundry catalog and partner module | implemented | `delivery/azure/llm/resolver_queries.py`; `infra/modules/llm/foundry-partner/`; focused resolver and Terraform checks | Discovery distinguishes OpenAI, Anthropic, and MistralAI. The isolated private module pins AIServices deployment format, version, capacity, and project-user roles. Root activation and endpoint routing remain open. |
...
compatible. The private `foundry-partner` module owns account, project, pinned deployments, and user roles; root activation remains open.
