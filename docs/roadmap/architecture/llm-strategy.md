...
| Publisher-qualified Foundry catalog, infrastructure, and endpoint contract | implemented | Resolver, `infra/modules/llm/foundry-partner/`, runtime endpoint resolver, focused checks | Root Terraform splits publishers and private networking. Runtime accepts strict OpenAI/Foundry endpoint references and API styles. Sealed binding generation and registry activation remain open. |
...
compatible. Root Terraform splits publishers and composes private partner resources conditionally; runtime resolves strict OpenAI/Foundry account references, while sealed binding generation and registry activation remain open.
