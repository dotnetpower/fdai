...
| Publisher-qualified Foundry catalog and partner module | implemented | `delivery/azure/llm/resolver_queries.py`; `infra/modules/llm/foundry-partner/`; focused resolver and Terraform checks | Discovery distinguishes OpenAI, Anthropic, and MistralAI. Root Terraform splits OpenAI and partner capabilities, composes private AIServices and DNS, and keeps endpoint routing and registry activation open. |
...
compatible. Root Terraform splits publishers and composes the private partner account, project, deployments, roles, endpoint, and DNS only when partner capabilities resolve; runtime endpoint binding remains open.
