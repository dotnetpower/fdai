...
translation_source_sha: f2bfa8bbe3cbc60152e299930444c695fb6d345e
...
| 통제된 기준선 및 처리 코호트 주장 | implemented | `packages/service-contracts/src/fdai_service_contracts/baseline_cohort.py`; `services/core-control-plane/src/fdai/core/measurement/baseline_cohort_claim.py`; `tools/cohort_receipt.py`; `tools/baseline_run.py`; 집중 계약, 주장 및 실행기 테스트 | 증적은 고정된 개정 하나, 고정 시나리오 집합과 그 다이제스트, 두 갈래의 보고서 및 출처 다이제스트, 갈래별 표본 수, 절대값을 포함하는 신뢰 구간, 완전성과 출처 참조, 임계값 0 가드 전체를 연결합니다. 적격 여부는 평가기의 출력으로만 존재하며 최신 독립 의사 결정 근거 승인과 통제된 외부 산출물 출처를 요구하므로, 이 저장소에 커밋한 산출물은 적격이 될 수 없습니다. 비합성 코호트 보존 자체는 아직 남아 있습니다. |
...
| 고정된 시나리오 집합 계산 | in-progress | `tests/scenarios/manifests/v2026.07.json`; `tests/scenarios/cross-objective/v2026.07-sre.json`; `tests/scenarios/enrichment/v2026.07/sre-cluster-diagnostics-missing-001.json`; `test_frozen.py`; `test_v2026_07_replay.py` | 이제 SRE 묶음은 실행 가능한 차원 6개를 모두 갖추었고 manifest에서 `complete` 상태입니다. 전체 루프 종결 근거는 shadow로 재생한 프로세스 내 고정 관측 소스에서 나오므로 배포 런타임의 권위 있는 관측은 여전히 열려 있고, 나머지 묶음 4개도 불완전합니다. |
...
> SRE는 이제 실행 가능한 차원 6개를 모두 갖춰 `complete` 상태이지만,
> 나머지 4개 묶음은 하나 이상의 필수 사례를 여전히 누락합니다. 다섯 묶음이 모두
