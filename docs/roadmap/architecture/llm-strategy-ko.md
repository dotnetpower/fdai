...
translation_source_sha: f93a91094d1930e5256944228086233617dde420
...
| 발행기 한정 Foundry 카탈로그 및 partner 모듈 | implemented | `delivery/azure/llm/resolver_queries.py`; `infra/modules/llm/foundry-partner/`; 집중 resolver 및 Terraform 검사 | 검색은 OpenAI, Anthropic 및 MistralAI를 구분합니다. Root Terraform은 OpenAI와 partner 기능을 분리하고 private AIServices 및 DNS를 조립하며 endpoint routing과 registry 활성화는 보류합니다. |
...
호환됩니다. Root Terraform은 발행기를 분리하고 partner 기능이 해석된 경우에만 private account, project, 배포, 역할, endpoint 및 DNS를 조립합니다. Runtime endpoint binding은 미완료 상태입니다.
