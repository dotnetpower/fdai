...
translation_source_sha: cb0df6d8bca5a0fd27f4fbf631604d2d321a5508
...
| 발행기 한정 Foundry 카탈로그 및 partner 모듈 | implemented | `delivery/azure/llm/resolver_queries.py`; `infra/modules/llm/foundry-partner/`; 집중 resolver 및 Terraform 검사 | 검색은 OpenAI, Anthropic 및 MistralAI를 구분합니다. 격리된 private 모듈은 AIServices 배포 format, 버전, 용량 및 project-user 역할을 고정합니다. Root 활성화 및 endpoint routing은 미완료 상태입니다. |
...
호환됩니다. Private `foundry-partner` 모듈은 account, project, 고정 배포 및 user 역할을 소유하며 root 활성화는 미완료 상태입니다.
