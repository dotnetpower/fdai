...
예약된 주간 작업은 더 선호되는 새 계열, 사용 중단, 측정된 용량 또는 품질 표류를 감시합니다.
정제된 제안 스키마 v3 근거를 만들고 제안 다이제스트를 멱등성에 사용하며 초안 PR만 엽니다.
실제 매핑은 바꾸지 않습니다. 공급자 실패는 초안 대신 권한이 없는 처리 보류를 만듭니다.

스키마 v3는 모델 계열, 발행자, 상태뿐 아니라 SKU와 유효 용량 단위 및 값을 비교하므로 제자리
