...
- **T2 쌍 원자성:** 다이제스트에 결속된 환경 정책이 reasoner 중 하나를 명시적으로
  `hil-only`로 보류하지 않는 한 primary와 secondary는 서로 다른 발행기여야 합니다. 명시적
  보류는 모든 T2 결정을 사람 승인으로 보냅니다. 결정론적 불일치 대체 구현은 시작 모델
  후보가 아니며 모델 호출이나 계측 레코드를 만들지 않습니다.
...
해석기는 프로비저닝 불가능한 기능을 `hil-only`로 강등시키고 계속합니다. 하나의 누락된
계열 때문에 전체 부트스트랩을 막지 않으므로 **부분 배포가 별도 신호 없이 남을 수
있습니다**. `resolved-models.json`이 T1 쌍과 `t2.reasoner.primary`만 담고 있는데
레지스트리는 보조 reasoner, 비평자, RCA reasoner, 에스컬레이션 상한까지 선언할 수
있습니다. 다이제스트에 결속된 정책이 T2 reasoner 중 하나를 명시적으로 보류하면 조립
루트는 강제 불일치 교차 검증을 사용하고 모든 T2 케이스를 사람 승인으로 보냅니다.
`auto` 또는 `pinned` 상태에서 reasoner가 예기치 않게 누락되면 구성된 모드를 재해석하지
않고 런타임 조립을 차단합니다. 두 경우 모두 배포 평가는 reasoning 계층이 정족수를
구성할 수 없음을 드러내야 합니다.
...
- 명시적으로 보류한 reasoner는 결정론적 불일치를 사용해 요청을 사람 승인으로 보냅니다.
  예기치 않게 배포가 누락되면 장애로 취급하고 런타임 시작을 차단합니다. 두 경우 모두
  운영 근거를 남기며 다른 기능으로 조용히 전환하지 않습니다.
