...
| Versioned provider relationship materialization | implemented | `delivery/provider_schema_relationship_generation.py`; `delivery/provider_schema_relationship_ledger.py`; `core/ontology_platform/direction_shadow`; focused generation and direction-shadow tests | Provider-schema and REST evidence, mapping revision, projection manifest, explicit direction/cardinality/link metadata, changed-subset invalidation, rollback, replay, and exact-release comparison are bounded and content addressed. Candidate promotion remains proposal-only; graph and migration authority are false. |
