...
| Kubernetes workload evidence queries | implemented | Rollout, Pod recovery, Resource event-history, and Pod lifecycle-cohort FunctionTypes; reviewed metric semantics; semantic planners; Kubernetes and durable lifecycle readers; focused receipt, metric, event, composition, persistence, and incomplete-source checks | Generic `resource_event.kubernetes` remains exact-target presentation and cannot relabel sibling UID rows. Pod replacement uses a separate controller-grounded cohort whose identities come from complete inventory snapshots, not Event `related.uid`. The recovery reducer receives one historical UID lane plus exact current Pod and Deployment dependencies, while missing or ambiguous lineage remains incomplete. Rollout and recovery keep root cause and execution authority false. Authenticated live replacement and recovery receipts remain external validation. |
...
|------|-------|--------|----------|-----------|
| 2026-08-28 | implemented | Required an exact-release comparison before a direction-shadow generation review can become proposal-ready. `assess_direction_mapping_promotion` now adds an `exact_release_mode_required` reason and refuses `proposal_ready` whenever the receipt was produced without `require_exact_releases=True`, closing a gap where a loose or fuzzy comparison with a complete disposition could still reach an approve-proposal decision. | `current change`; `core/ontology_platform/direction_shadow/promotion.py`; focused direction-shadow promotion checks (`9 passed`); Ruff, formatter, and strict mypy. | Bind every upstream comparator call site that feeds promotion review to request `require_exact_releases=True`; no direction-shadow authority was granted. |
| 2026-08-28 | implemented | Changed planned-change revalidation to compare stable graph state instead of receipt audit time, while preserving each complete receipt digest. Corrected the catalog lifecycle integration test to downgrade and upgrade the exact catalog revision rather than the current branch head's unrelated predecessor. | `current change`; focused assessment and receipt checks (`28 passed`); local Docker PostgreSQL catalog migration lifecycle (`1 passed`); Ruff and strict mypy. | Retain deployed migration rollback and planned-change receipts separately; no remote database was queried. |
| 2026-08-28 | implemented | Persisted bounded Kubernetes LIST continuation separately from the watch resource version and added a continuous-coverage boundary for lifecycle lookbacks. Large snapshots now converge across collector restarts, expired continuations reset explicitly, and durable readers withhold completeness until the full requested interval is covered. | `current change`; focused source, collector, durable-reader, migration, and local Docker PostgreSQL checks (`58 passed`); migration inventory (`58 passed`); Ruff and strict mypy. | Retain deployed Kubernetes and PostgreSQL receipts separately; no live cluster or remote database was queried. |
...
| 2026-08-28 | implemented | Added a separate typed Pod lifecycle cohort. Inventory snapshots persist immutable Pod controller lineage, cohort queries join lifecycle rows by those exact UIDs, generic Resource event presentation filters back to the requested UID, and the replacement reducer preserves historical and current evidence separately. | `current change`; focused identity, cohort, reducer, semantic planning, composition, presentation-isolation, migration, Ruff, and strict mypy checks; seven PostgreSQL lifecycle tests passed against the worktree-owned local Docker database. | Retain deployed PostgreSQL and authenticated exact-target replacement evidence separately. |
