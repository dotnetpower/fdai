...
| OI-12 aggregate certification receipt | implemented | `operational_instance_certification.py`; focused certification checks (`8 passed`) | The pure receipt requires freshness, API pressure, lag, storage growth, rollup coverage, archive restore, and provider-failure recovery exactly once. Unmeasured axes remain unavailable with bounded reasons, `complete` reports measurement coverage rather than a pass verdict, and all authority fields stay false. Deployed measurements and certification remain open. |
