...
| Kubernetes workload evidence queries | implemented | Rollout, Pod recovery, and Resource event-history FunctionTypes; reviewed metric semantics; semantic planners; Kubernetes and composite readers; focused receipt, metric, event, composition, and incomplete-source checks; authenticated targetless Console receipts | Rollout and Pod recovery authenticate exact-release ownership and restart-delta evidence, preserve `source_incomplete`, and keep root cause and execution authority false. `resource_event.kubernetes` requires the judgment duration to equal the frame lookback and an explicitly secured exact cluster before cluster-wide provider I/O. Child-only hashed identities stop as incomplete; selected child UIDs can be attributed only within an explicit cluster read. The adapter preserves applicable events from mixed scope as incomplete, rejects encoded responses, caps raw input at 256 KiB, normalizes at most 256 items, returns no raw provider payload, and reports `source_retention_unverified` until Event retention is authoritative. Durable Event retention, cause/recovery joins, and exact-target Console evidence remain open. |
