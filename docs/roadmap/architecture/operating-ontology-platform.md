...
| Kubernetes workload evidence queries | implemented | Rollout, Pod recovery, and Resource event-history FunctionTypes; reviewed metric semantics; semantic planners; Kubernetes and durable lifecycle readers; focused receipt, metric, event, composition, and incomplete-source checks | Rollout and Pod recovery authenticate exact-release ownership and restart-delta evidence, preserve `source_incomplete`, and keep root cause and execution authority false. `resource_event.kubernetes` requires the judgment duration to equal the frame lookback and now consumes retained UID-scoped lifecycle rows when the durable cursor exists. Missing cursor, empty durable history, unavailable logs, and incomplete sources remain explicit gaps rather than absence claims. One source-grounded target adds an exact Resource name predicate to the reviewed type scope. That identity predicate must resolve to exactly one Resource; zero or multiple matches stop before provider I/O with `target_resolution_not_exact`. Authenticated exact-target planning and operator presentation remain read-only; live replacement and recovery receipts are still external validation. |
