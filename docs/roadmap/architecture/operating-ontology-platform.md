...
| Kubernetes workload evidence queries | implemented | Rollout and Pod recovery reducers and FunctionTypes; reviewed metric semantics; semantic planners; focused receipt-authority, metric-scope, composition, and incomplete-source checks; authenticated targetless Console receipts | The rollout FunctionType accepts only exact-release Reader `operations-review` receipts for Deployment, ReplicaSet, and Pod ownership hops. The Pod recovery FunctionType authenticates the exact Pod, outgoing ReplicaSet owner, and outgoing Deployment owner receipts, joins a reviewed 30-minute restart-count delta selected by cluster identity and immutable Pod UID, and requires full owner replica recovery. Zero-row candidate tables preserve `source_incomplete`; incomplete or ambiguous scope stops before the metric read, partial history cannot verify recovery, and an older cumulative current count does not replace a positive bounded delta. Both reducers keep root-cause and execution authority false. Exact-target Console evidence remains open. |
