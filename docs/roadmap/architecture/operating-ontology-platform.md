...
| Conversation tier and transport isolation | implemented | `semantic_query_model_targets.py`; `semantic_turn_consumer.py`; focused tier-routing and partition-worker checks | Production conversation composition binds T1 only. Generic Control Loop and RCA T2 target resolution remains available outside this composition. Two same-group consumers use broker partitions, and Operator requests use `session_id` as the partition key so cross-session work can overlap while same-session order remains causal. |
...
| 2026-08-17 | implemented | Disabled automatic T2 escalation in production conversation composition and added bounded partition-level semantic consumers. Generic T2 target resolution remains available to its governed non-conversation owners. | `current change`; focused tier-routing, semantic processor, session-key publication, and local topic partition checks. | Retain a throttled-session isolation receipt against the exact-source local stack. |
