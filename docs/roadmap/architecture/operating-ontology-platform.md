...
| Kubernetes workload evidence queries | implemented | Rollout, Pod recovery, and Resource event-history FunctionTypes; reviewed metric semantics; semantic planners; Kubernetes and composite readers; focused receipt, metric, event, composition, and incomplete-source checks; authenticated targetless and exact-target Console receipts | Rollout and Pod recovery authenticate exact-release ownership and restart-delta evidence, preserve `source_incomplete`, and keep root cause and execution authority false. `resource_event.kubernetes` requires the judgment duration to equal the frame lookback. One source-grounded target adds an exact Resource name predicate to the reviewed type scope. That identity predicate must resolve to exactly one Resource; zero or multiple matches stop before provider I/O with `target_resolution_not_exact`. One exact child may use an identity-aware reader only when its secured ObjectSet exposes a matching immutable UID and cluster; the Function passes a detached immutable two-field snapshot, and the adapter recomputes the canonical id before using `involvedObject.uid`. Legacy readers keep the original protocol, and missing or mismatched identity stops before provider I/O. Explicit cluster reads preserve deleted-object and cluster history. The adapter preserves applicable events from mixed scope as incomplete, rejects encoded responses, caps raw input at 256 KiB, normalizes at most 256 items, returns no raw provider payload, and reports `source_retention_unverified` until Event retention is authoritative. The bilingual answer exposes source completeness, stable limitations, displayed and total counts, and `display_truncated`; it preserves the most recent eight Event rows in chronological order. Authenticated exact-target planning and presentation are retained; successful Event source access, durable retention, and cause/recovery joins remain open. |
