...
| 2026-08-16 | implemented | Made the observed resource subgraph track provider change instead of a six-hour batch. Azure Resource Graph requests are paced at a sustained shared budget, one quota deferral is capped, one source attempt is bounded by a wall-clock deadline, and the reconciliation gate now reads the change marker that previously had no consumer so an observed change reconciles above a floor. The promoted snapshot remains the single writer, and the declared observed-state freshness ceiling now equals the cadence the job guarantees. | `current change`; focused inventory, gate, projection, and infra checks report 285 passed with 21 environment-dependent skips; strict mypy reports no issues in 7 source files; the new gate SQL was probed against a migrated throwaway PostgreSQL database and returned the designed decision for seven seeded states. | Restart the local stack and retain authenticated evidence that a real provider change reconciles within one tick; no deployed run has exercised the minute cadence yet. |
| 2026-08-16 | implemented | Hardened the continuous collection loop after review. A failed attempt now backs off exponentially to the routine interval instead of retrying every tick; one attempt is bounded by a no-progress deadline plus an absolute ceiling that resolves before the abandonment window, and the two report separately; the source stream is closed on every exit path; and a change counts as unreconciled when it was recorded on this control plane's clock after the active snapshot started, not when the provider says it happened. | `current change`; focused inventory, persistence, projection, and infra checks report 1940 passed with 129 environment-dependent skips; strict mypy reports no issues in 9 source files; the gate SQL was re-probed against a migrated throwaway PostgreSQL database and returned the designed decision for fifteen seeded states. | ARM and declarative sources emit one batch, so their effective bound is the progress deadline rather than a true no-progress signal. Authenticated live evidence for change-driven reconciliation is still missing. |
