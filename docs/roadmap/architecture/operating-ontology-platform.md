...
| 2026-08-18 | implemented | Declared `Resource.status` and `Resource.location` at version 1.1.0 and lifted both out of the adapter property bag in inventory projection, so a planner can filter and read provider state instead of reaching into an undeclared open bag. The changed declaration moves the ontology release digest, so the promoted semantic surface's derived manifest digest was recomputed with the runtime builder; its validation receipt binds surface, generation, and catalog digests and is unaffected. | `current change`; `Resource.yaml`, `inventory_projection.py`, `kubernetes-node-pool.multi-zone.ko.yaml`; the catalog loads at `Resource@1.1.0` and task-scoped Ruff and format passed. Suite execution is delegated to the batched validation run. | Rows written before this release keep the old top-level shape until inventory reconciliation re-projects them; the historical release stays registered so they remain readable. |
