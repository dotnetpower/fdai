...
translation_source_sha: 91b668b77e7e497efb6987495bde97eb2e0751f3
...
아니라 중첩으로 모델링합니다. 다른 상자 안에 그린 상자는 방향을 잘못 가리킬 수 없기 때문입니다.
모델은 상자 크기를 아래에서 위로 계산하고, 소유자마다 자식을 정사각형에 가까운 격자로 채우며,
한도가 제외한 자식 수를 소유자별로 밝힙니다. 따라서 자식을 감춘 상자가 빈 소유자로 읽히지
않습니다. 중첩은 흡수한 포함 관계의 선을 모두 없애며, 실측한 클러스터에서는 385개 관계 중
190개가 그렇게 사라집니다. 상자 안의 순서는 계층형 레이아웃과 같은 선언 워크로드 우선 순위를
따르므로, 한도는 선언된 Resource보다 파생된 Resource를 먼저 제외합니다. 중첩은 읽기를 위한
배치일 뿐입니다. evidence가 보고하지 않은 포함 관계를 주장하지 않으며, 소유 `contains` 관계가
없는 Resource를 그림을 정돈하려고 상자 안에 넣지 않습니다.
