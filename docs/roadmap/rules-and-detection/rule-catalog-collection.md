...
>
> Dedicated config-baseline and measurement-baseline schemas, loaders, and repository stores are also implemented; the configuration store now ships one reviewed baseline resolved by the T0 `evaluate_configuration_baseline_control_set` consumer (see [Config Baseline](#config-baseline-hardened-reference-set)), and the measurement store still ships empty by design. The versioned MCSB catalog imports all 86 v1 controls and all 81 v2 preview controls, pins every source document, and validates each version's independent implementation crosswalk.
>
> The watcher's durable snapshot mirror and review-only PR delivery are also implemented and wired opt-in into the deployment (see [Remaining work](#remaining-work)); live activation, every external connector/parser, and every compliance/threat crosswalk listed below remain open.
