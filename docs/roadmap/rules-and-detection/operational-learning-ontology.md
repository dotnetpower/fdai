...
| O7 - Promotion measurement | Implemented: immutable FDAI revision and scenario-set batches join frozen benchmark and live-shadow cohorts. The audited runner measures Wilson 95% accuracy bounds, live-only observation days, rollback, recurrence, policy escapes, causal grade, and Dynamic review rate without mutating promotion state. | A separate promotion review is eligible only when every per-action gate passes. The current live drills prove the bindings but do not supply the required action-specific observation days and confidence sample size, so promotion remains held. |
