...
| O7 promotion measurement | implemented | `services/core-control-plane/src/fdai/core/measurement/operational_promotion.py`; `operational_promotion_runner.py`; `services/core-control-plane/src/fdai/delivery/measurement/{operational_promotion_evidence.py,operational_promotion_batch.py}`; `measurement_runner_cli.py`; `infra/modules/measurement-runners/`; focused O7 tests and Terraform validation | The exact-digest consumer, manifest verifiers, durable receipt sink, opt-in job, and governed live-shadow batch producer are implemented. Action-specific observation days, confidence samples, and authenticated runtime receipts remain incomplete. |
