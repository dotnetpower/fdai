...
두 어댑터는 같은 이중 언어 어휘 점수 계산, 설정 가능한 유한 가중치와 최소 점수, 안정적인 식별자
동점 순서를 사용합니다. 인가된 정확한 식별자 조회에는 임베딩 호출이 필요하지 않습니다.
PostgreSQL은 후보를 반환하기 전에 활성 세대 식별자를 계속 검증합니다. 크기가 제한된 문서 캐시는
매번 순서가 있는 MVCC 행 버전과 트랜잭션 epoch를 확인합니다. 행이 바뀌면 이전 검증을 재사용하지
않고, 현재 트랜잭션에서 변경된 행은 캐시하지 않습니다. 로컬 어댑터 평가 36개 사례는 영어와 한국어
양성 질의 24개, 일치 없음 질의 8개, 임베딩을 사용할 수 없는 정확한 식별자 질의 4개를 포함합니다.
