...
The current 8,537-record corpus should not become one operating-graph release.
...
| Performance | Objective expansion, binding fan-out, result count, graph objects, chunks, and request time use configured hard bounds. Baseline and treatment latency, CPU, and storage are measured on the same current 8,537-record revision before promotion; this design claims no unmeasured SLA. |
...
| Full-corpus generation identity | implemented | [`catalog_search.py`](../../../services/core-control-plane/src/fdai/shared/providers/catalog_search.py), [`rule_semantic_generation.py`](../../../services/core-control-plane/src/fdai/rule_catalog/schema/rule_semantic_generation.py), [`test_discovery_catalog_search.py`](../../../services/core-control-plane/tests/rule_catalog/test_discovery_catalog_search.py), [`test_postgres_rule_corpora_integration.py`](../../../services/core-control-plane/tests/delivery/catalog_search/test_postgres_rule_corpora_integration.py) | Provider-neutral metadata carries count, hierarchical digest root, and bounded chunks. The current 50 active and 8,487 discovery Rule projections have isolated in-memory and local PostgreSQL lifecycles. Governed deployed PostgreSQL evidence remains open. |
...
|------|-------|--------|----------|-----------|
| 2026-09-12 | implemented | Reconciled current full-corpus identity at 50 active and 8,487 discovery Rules and repeated lifecycle isolation through local PostgreSQL. | `current change`; in-memory corpus checks (`12 passed`); local PostgreSQL full-corpus lifecycle (`1 passed`). | Retain governed deployed PostgreSQL evidence without changing the dated historical 62-Rego inventory; current backfill scope is 50 Rules. |
...
- [ ] P1 exits when all 50 current authored Rego Rules have reviewed bindings and migration reports account
  for every active Rule without changing Rule ids or verdict behavior.
- [x] P2 exited when the durable PostgreSQL adapter persisted and validated the provider-neutral
  count/root/chunk metadata and repeated the passing 50-active/8,487-discovery activation,
  replacement, and rollback isolation proof (`1 passed`).
...
| P2 - scalable generations | Extend generation identity to count/root/chunk manifests and add objective/binding search documents without merging corpora. | Full current 8,537-record load, atomic activation, stale rejection, and rollback tests pass. |
...
- Backfill starts with the 50 current authored Rego Rules. Collected expression records remain discovery
...
- full-corpus tests for 8,537 current records, active/discovery isolation, chunk completeness, activation,
