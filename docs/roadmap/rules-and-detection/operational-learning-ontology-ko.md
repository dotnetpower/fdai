...
간결하게 정리합니다. 검토된 카탈로그 PR과 reload만 activation 경로이며 Saga는 Mimir-owned `object.rule` 이벤트의
검토 결과를 봉인합니다.
