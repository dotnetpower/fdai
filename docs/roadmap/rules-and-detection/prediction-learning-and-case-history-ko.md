...
translation_source_sha: c47ba084f240a9efbb9f1cf851b5d90d915aba54
...
| 날짜 | 상태 | 변경 | 근거 | 남은 작업 |
|------|------|------|------|-----------|
| 2026-09-14 | implemented | 실제 평가기의 예상 시각과 구간 신뢰 수준을 기존 예측 게시 대기열에 보존해 정확한 원본 기반 승인 시간에 연결했습니다. PostgreSQL 시간 정규화는 에피소드를 다시 쓰지 않고 원본 식별자와 같은 시각을 유지합니다. | `current change`; `test_forecast_evaluation.py`, `test_forecast_urgency.py`, 실제 예측 에이전트 연결 및 SQL 검사; 관련 테스트 104개와 후속 원본/시간대 테스트 31개 통과 | 운영 보정과 긴급도 코호트는 별도 요건이며 예측 결과나 승격을 추론하지 않습니다. |
