...
publishes the Drift but does not execute repair. The Inventory job wakes every minute, skips
while another candidate is collecting or the last success is under six hours old with no
unreconciled change, and retries on the next tick when a newer attempt failed or was abandoned.
An observed change reconciles early above its floor, so change latency is bounded by the tick
rather than by the interval. Full ARG/ARM scan cadence remains six hours when nothing changed.
