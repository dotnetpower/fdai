...
translation_source_sha: 584c2feeeded343dfc1919e27c2fffe73cda7d3b
...
> 공유 `Provenance` 값 객체
> ([`provenance.py`](../../../src/fdai/rule_catalog/schema/provenance.py))가 이제 ship 되어 모든
> 아티팩트가 optional `provenance` 블록을 실음. 아직 수렴 중인 나머지 간극: 로더는
> `schema_version`를 키로 쓰고(assignment 에 아직 아티팩트 `kind` / `version` 없음), rule-set 을
> `rule_set` / `target_rule_ids`(not `target`)로 바인딩하며, scope 를 `level` + `id` + `excludes` +
> 타입된 `selector`(not `scope://` include/exclude URI) + flat `parameters`(not rule별
> `parameter_overrides`)로 모델링함.
