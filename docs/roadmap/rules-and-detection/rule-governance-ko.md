...
translation_source_sha: 06525f76f20e84c96a1717ee3e2a4dada9a038a5
...
>
> 참고: ship 된 catalog-as-code 필드 형태는 실용적 부분집합이며 아래 "YAML Shapes" 섹션의 예시와
> 다름 - 로더는 `schema_version`(not `kind` + `version`)를 키로 쓰고, rule-set 을 `rule_set` /
> `target_rule_ids`(not `target`)로 바인딩하며, scope 를 `level` + `id` + `excludes` + 타입된
> `selector`(not `scope://` include/exclude URI)로 모델링하고, flat `parameters`(not rule별
> `parameter_overrides`)를 실음; `provenance` 는 아직 모델링 안 됨. 둘을 일치시키는 것 - 스키마를
> YAML-Shapes 목표로 풍부하게 하거나 목표를 ship 된 부분집합으로 내리거나 - 은 보류 중인
> 거버넌스-스키마 결정.
