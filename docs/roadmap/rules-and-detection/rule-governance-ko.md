...
> **구현 상태**: Effect/scope/assignment/rule-set domain model, strict YAML loaders, directory
> catalog loader 및 effect/enforcement transition CI가 구현되어 있습니다. Resolved assignment를
> T0 runtime이 소비하는 composition은 아직 남아 있습니다. Exemption은 별도 Azure-shaped
...
| exemption (waiver / mitigated) | **exemption** | bounded Azure scope에서 규칙을 time-boxed, justified하게 억제; assignment/category metadata는 향후 schema 작업 |
...
> **enforcement** `do-not-enforce` -> `enforce` 활성화(enforce-tier effect 를 shadow 에서 꺼내는
...
> base ref 와 working tree 의 catalog 를 materialize 해 거부된 전이가 있으면 빌드를 실패시킴.
> 게이트는 **effect + enforcement** 전이를 관장하며 scope / blast-radius **확대**는 플래그하지 않음(낮은
...
- 커스텀 규칙은 같은 스키마 사용, `source: custom` + shipped 전체 `provenance`
  (`source_url`, immutable ref/hash, license/redistribution, retrieval time, optional mapper).
...
- 현재 필수 필드: `rule_id`, resource-group 또는 resource로 좁혀진 Azure-shaped `scope`,
  **justification**, 서로 다른 `requested_by` / `approved_by` UUID, `state`, `created_at`,
  `expires_at`. Loader는 no self-exemption과 `expires_at > created_at`을 강제합니다.
- 현재 schema는 assignment reference나 waiver/mitigated category를 저장하지 않고, 설정된 최대
  기간도 강제하지 않습니다. 이 metadata와 maximum-duration policy는 후속 contract입니다.
- Auto-renew는 지원되지 않습니다. 만료 시 재적용과 사전 ChatOps 알림은 아직 배선되지 않은 운영
  workflow이며, 현재 artifact state는 CLI/검토 프로세스가 갱신해야 합니다.
- 모든 exemption과 만료는 감사; exemption은 기저 finding의 감사 기록을 절대 억제하지 않음 -
  발생 안 함이 아니라 *왜 수용됐는지* 기록.

## Override

> **현재 상태**: 아래 override 계약은 아직 구현되지 않았습니다. Repository에는 override 전용
> schema, catalog loader 또는 runtime resolver가 없으므로 현재 지원되는 완화는 time-boxed
> exemption과 catalog/rule retirement 경로입니다.
...
- 규칙, 룰셋, 할당은 versioned catalog-as-code입니다. Exemption은 stable id, state,
  creation/expiry timestamp를 가지지만 현재 schema에는 artifact `version`이 없습니다. Tracked file
  변경은 PR history로 revert할 수 있습니다.
...
`requested_by`와 `approved_by`는 deployment가 제공하는 서로 다른 UUID여야 합니다. 여기서는 실제
tenant identifier를 repository에 넣지 않기 위해 named placeholder를 사용합니다.
...
> `rule-set`과 `assignment`는 governance catalog loader가 읽는 strict schema를 갖고,
> `exemption`은 별도 strict schema/CLI가 읽습니다. `override` shape는 아직 목표 예시이며 loader가
> 없습니다. 각 rule-set 멤버는
> 규칙 `version` 을 고정; 각 `parameter_overrides` 값은 대상 규칙이 그 파라미터에 선언한
> 타입에 대해 검증하는 것은 아직 후속 작업이며 현재 assignment schema는 string 값을 받습니다.
...
- [ ] Implemented `scope://...` syntax를 Azure resource hierarchy로 resolve하는 adapter
  (비-Azure resolution은 TBD; [Always-On Rules](../../../.github/copilot-instructions.md#always-on-rules-must) 참조).
