...
translation_source_sha: 242fa2115ce9897deba518bd5d7100d6622ca9f9
...
> `selector` 폼이 될 수 있으며, 리솔버가 소비하는 `ScopeMatcher` 프로토콜 뒤로 통일됨. rule별
> `parameter_overrides`(assignment-wide `parameters` 위로 병합, rule별이 이김)도 ship 됨. 예시와의
> 유일한 차이는 명명뿐: 현 스키마는 rule-set 을 `rule_set` / `target_rule_ids`(sketch 의 `target`
> 보다 명확)로 바인딩하고 더 풍부한 `selector`(not `selectors`)를 쓰며, 아래 "YAML Shapes"
> 예시는 현 스키마 필드명으로 정리 중임.
