...
맞는 조합을 고를 수 있게 합니다. Upstream은 가장 안전한 pull 기준선을 그림자 모드에서
...
| AKS Managed Prometheus (`RoutedMetricProvider` 경로 #1) | **~15~60s** | `FDAI_PROMETHEUS_ENDPOINT` | pull (tick) |
...
| Azure Monitor Metrics REST API (`RoutedMetricProvider` 경로 #2) | **~1~3분** | `FDAI_MONITOR_WORKSPACE_ID` 세트되면 자동 | pull (tick) |
| Azure Monitor Logs KQL (`RoutedMetricProvider` 경로 #3) | **~2~5분** | `FDAI_MONITOR_WORKSPACE_ID` 세트되면 자동 | pull (tick) |
...
- [정규화기](../../../services/core-control-plane/src/fdai/delivery/azure/) -
 진단 AllMetrics 배치 -> tuple of `Event`. Pure 함수,
...
- **Kafka 소비자 배선**이 Event 허브의 Kafka 엔드포인트를 읽고
 ...
 루트가 두 번째 소비자 instance를 진단 허브에 붙이고 각 배치를
 정규화기로 흘려주면 됨.
...
참조).
[analyzer tick 작업](../../../infra/modules/compute/container-apps/analyzer_tick_job.tf)이
...
변환 결과를 사용하므로 새로 발견된 지원 리소스가 배포 변경 없이 다음 tick에 포함됩니다.
Cron을 명시적으로 빈 값으로 설정하면 작업이 비활성화됩니다. 명시적 대상과 영속 인벤토리
모두에 지원 리소스가 없으면 CLI가 조용히 종료됩니다.

### 에이전트 소유 AKS 감지 준비도

같은 tick은 각 AKS 대상에 대해 발견, 수집기 구성, 최근 telemetry, detector 연결,
...
스냅샷이 없으므로 부분이며, 이후 pass에서 파이프라인 연속성을 증명할 수 있습니다.

한 대상 tick의 6개 관측은 결정론적 `pass_id`와 대상 리소스 파티션 키를 공유합니다.
따라서 런타임 복제본이 여러 개여도 Event Hubs 정렬과 Heimdall 소비자 그룹이 각 대상을
한 소비자에 전달합니다. Heimdall은 차원을 어떤 순서로든 받고 겹친 pass ID를 독립적으로
추적하며, 한 pass의 6개가 모두 도착하기 전에는 표류를 발행하지 않습니다. 불완전한 pass는
...
실패 또는 포기되면 다음 tick에 재시도합니다. 정상 상태의 full ARG/ARM 검사 cadence는 6시간입니다.

## 조합 규칙

- **모든 push 정규화기는 별개의 `event_type`을 발행**해서 trust
 라우터 (와 downstream 대시보드)가 분명하게 필터 가능:
...
어느 조합도 upstream 코어 변경을 요구하지 않지만 포크의 Terraform/조립 연결이
...
- **경로 #2의 Kafka-consumer glue** (위 "포크 작업" 노트 참조). 소비자
 ...
 정규화기로 흘리는 composition-root wiring만 upstream에 안 씀.
