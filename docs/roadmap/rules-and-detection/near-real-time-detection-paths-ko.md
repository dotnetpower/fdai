...
FDAI route는 `Authorization: Bearer <FDAI_AZURE_MONITOR_WEBHOOK_TOKEN>`을 요구합니다.
Shipped Action Group webhook receiver는 이 header를 추가하지 않으므로 fork는 token을 주입하는
trusted proxy 또는 Entra-authenticated secure-webhook adapter를 Action Group과
...
- **Correlation id는 series당 / rule당 fold**. 한 alert rule의 모든
  ...
  grouping key로 전달하고 incident lifecycle consumer가 상태 전이를 별도로 결정합니다.
...
어느 조합도 upstream core 변경을 요구하지 않지만 fork의 Terraform/composition binding이
필요하고 Path #1은 인증 bridge도 필요합니다.
...
세 항목 모두 fork가 형태를 정한 뒤 추가할 수 있는 준비 상태입니다.
