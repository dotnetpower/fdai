...
| Catalog-backed bilingual promotion hold | implemented | `tests/rule_catalog/test_discovery_catalog_search.py`; commit `d1787f4d8`; focused shipped-catalog check | The real 62-Rule active generation passes exact English lookup, reports zero Korean positive recall without promoted Korean surfaces, preserves Korean no-match precision, and returns `HOLD`. This is negative gate evidence, not Korean retrieval readiness. |
...
- [ ] Expand the shipped-catalog probe to independently authored English and Korean positive,
  ambiguity, adversarial-input, and corpus-isolation cohorts, and exercise stale state through the
  real semantic index. Add governed Korean surfaces, then exit when configuration-backed thresholds
  produce reviewable passing receipts and a governed promotion gate consumes them without granting
  execution authority.
