...
| Corpus-scale generation identity | in-progress | `rule_catalog/schema/rule_semantic_generation.py`; `rule_catalog/schema/discovery_rule.py`; `tests/rule_catalog/test_rule_semantic_retrieval.py`; `tests/rule_catalog/test_discovery_catalog_search.py` | Count, hierarchical root, and bounded chunk identities represent 8,549 rows. The actual 8,487 discovery Rules produce 34 ordered chunks. Delivery metadata integration remains open. |
...
- [ ] Bind exact document count, hierarchical root, and ordered chunk identities into delivery
  generation metadata; exit when stage, activation, search, and rollback reject any identity drift.
