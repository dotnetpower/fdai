...
| Production generation build worker | in-progress | `delivery/catalog_search/rule_generation.py`; `delivery/catalog_search/rule_generation_worker.py`; `rule_catalog/schema/rule_semantic_generation_events.py`; focused worker and lifecycle checks | Durable builder and independent validator primitives stage exact generations, replay one stable bounded result across restart and concurrency, and fail closed on provider or snapshot errors. Pantheon subscribers and the production trigger remain open. |
