...
> schema/loader는 목표 shape로 남아 있습니다. 버전별 MCSB 카탈로그는 v1 컨트롤 86개와 v2 미리
> 보기 컨트롤 81개를 모두 가져오고 모든 소스 문서를 고정하며, 버전별 독립 구현 crosswalk를
> 검증합니다. Source 목록은 지원 목표와 shipped implementation을 함께 설명합니다.
...
| AWS / GCP (TBD) | AWS Well-Architected / Config managed rules, GCP Recommender / Policy Controller | JSON | REST API, git - **연기**, 비-Azure 대상은 TBD ([Always-On Rules](../../../.github/copilot-instructions.md#always-on-rules-must) 참조) |
...
- `source_documents`는 각 v2 도메인 페이지의 URL, 소스 commit, content hash를 고정합니다.
...
제한된 Learn allowlist에서 현재 v2 미리 보기 정의를 가져옵니다.
...
YAML) 되고 catalog-as-code로 저장. JSON은 와이어 포맷(event/message 스키마, API body)과
보호된 runtime artifact(`resolved-models.json` path/CI materialization) 에 유지; `rule-catalog/` 에서 사람이
저작하는 모든 것은 YAML.
...
- **`remediates` vs `remediation` - 두 필드, 하나의 개념:** `remediates` 는 이 규칙이 제안하는
  *mutation 카테고리* 를 선언하는 **ActionType id** (M:1); `remediation` 은 구체적 *how*인
  ...
  `remediates`를 registered ActionType으로 resolve하고 template path를 disk에서 검증할 수 있습니다.
...
`fetch.revision`은 fetch가 고정하는 immutable commit/digest이며 all-zero placeholder로
표시했습니다. Credential은 이 manifest가 아니라 deployment configuration으로 fetch adapter에
제공합니다.
...
전체 후보 생성 루프, 검증 요건, override 피드백, 안전 경계, CandidateGuard 동작은
[자율 규칙 발견](rule-catalog-autonomous-discovery-ko.md)에서 설명합니다.

## Open Decisions

- [ ] 어떤 소스가 reference-only vs embeddable, 각 라이선스에 대해 확인.
- [ ] 남은 소스를 위한 parser plugin(docs, Checkov/tfsec/KICS/Trivy 및 추가 vendor format).
