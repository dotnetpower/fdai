...
translation_source_sha: 5a26f7cd6a841d5e1f2eea46536d84beef1996ae
...
점수화 가능한 분모에는 넣지 않으며, snapshot은 실행 또는 승격 권한을 부여하지 않습니다. Console의 게시 부채는 게시되지 않았고 배달 실패로 확정되지 않았으며 처리 가능 시각이 도래한 행만 셉니다. 예정된 재시도는 현재 부채가 아니며, 가장 오래된 대기 시각은 재시도 시각이 아니라 처리할 수 있는 행의 원래 생성 시각을 유지합니다.
