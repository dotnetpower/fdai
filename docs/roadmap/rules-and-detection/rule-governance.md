...
| Rule retirement approver | `aw-approvers` (quorum-2, Owner-tier) | approve moving a rule out of the enforce set (global, catalog-wide) | approve a retirement without an Owner-tier reviewer among the quorum, or approve their own request |
...
| 2026-08-28 | implemented | Closed three review-authority and idempotency gaps against action governance. Rule-retirement changes now declare their own `GovernanceChangeClass.RULE_RETIREMENT` (quorum-2, phishing-resistant, Owner-tier review), so a `rule-catalog/retirements/*.yaml`-only PR no longer bypasses the review-authority CI gate. `governance.retire-rule`'s `blast_radius.static_bucket` moved from `resource` to `subscription`, because retirement disables a rule catalog-wide, not for one resource, and its `provenance.content_hash` was recomputed to match. `GovernedGovernancePrPublisher.publish` now snapshots the caller's document once before computing the digest and takes every later read (path validation, distinct-approval check, digest, and rendered patch) from that same snapshot, closing a window where an external mutation between digest and render could desynchronize the audited hash from the shipped patch; the idempotency key and the persisted `GovernancePrLifecycleReceipt` (schema bumped to `1.1.0`) now bind `correlation_id` alongside the document digest, so distinct source events no longer collapse into one receipt or PR branch. | `current change`; `services/core-control-plane/src/fdai/rule_catalog/schema/governance_review_authority.py`; `scripts/governance/check-governance-review-authority.py`; `rule-catalog/action-types/governance.retire-rule.yaml`; `services/core-control-plane/src/fdai/delivery/gitops_pr/governance.py`; `PYTHONPATH="$PWD/services/core-control-plane/src:$PWD/packages/service-contracts/src" .venv/bin/python -m pytest -q --no-cov services/core-control-plane/tests/rule_catalog/schema/test_governance_review_authority.py tests/integration/scripts/test_check_governance_review_authority.py services/core-control-plane/tests/rule_catalog/test_action_type_catalog.py services/core-control-plane/tests/delivery/test_governance_dispatch.py services/core-control-plane/tests/delivery/test_governance_writers.py services/core-control-plane/tests/delivery/test_governance_review_metadata.py` passed 113 cases. | Deploy the trusted Entra verifier GitHub App and retain one evidence record showing a self-approval or sub-quorum change blocked and the corrected change cleared; override delivery remains separate. |
| 2026-08-28 | implemented | Corrected the prior idempotency and CI-boundary claims. Governance PR identity now uses a stable `source_event_id`, keeps correlation only as audit grouping, rejects content drift for the same event, and probes both legacy digest-only and correlation-plus-digest keys before publication so an existing open PR cannot become undiscoverable. The CI prefilter now includes `rule-catalog/retirements/`, ensuring retirement-only changes reach authenticated review classification. | `current change`; focused governance publisher and review-authority boundary checks (`21 passed`); Ruff and strict mypy. | Deploy the trusted verifier and retain an authenticated retirement-only PR check receipt; no remote PR was created. |
| 2026-08-28 | implemented | Failed closed when the PR adapter reports an existing governance PR but no matching lifecycle receipt exists. This recovery case now requires explicit reconciliation instead of attaching the retry's digest and source-event identity to an unverified pre-existing patch. | `current change`; focused governance publisher checks (`15 passed`); Ruff and strict mypy. | Add a content-verifying remote reconciliation adapter before automating this recovery; no remote PR was queried. |
| 2026-08-28 | implemented | Disabled rename detection when the CI governance prefilter enumerates changed paths. A rename from `rule-catalog/retirements/` to an ungoverned path now appears as a governed deletion plus an addition, so the authenticated review check cannot be bypassed by moving the file. | `current change`; focused governance review-authority checks (`7 passed`). | Retain an authenticated CI receipt for a governed-path rename; no remote workflow was run. |
...
- [x] Classify rule-retirement changes with their own review-authority class (quorum-2, phishing-resistant, Owner-tier), so a retirement-only PR cannot bypass the CI gate; proven by `test_check_governance_review_authority.py` and `test_governance_review_authority.py`.
