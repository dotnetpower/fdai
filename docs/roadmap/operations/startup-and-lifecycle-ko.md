...
- [ ] 새로 시작한 고정 Core 개정 번호에 보호된 `t2-startup-proof-evidence.yml` 워크플로를
   실행합니다. 후보별로 성공한 T2 시작 샘플 세트가 한 번만 실행되고 해당 프로세스에서 최소
   두 번의 후속 준비 상태 새로 고침 동안 T2 호출이 추가되지 않음을 보여 주는 스키마 유효
   증적을 보존한 뒤 최종 검증 근거를 이슈 #90에 기록합니다.
