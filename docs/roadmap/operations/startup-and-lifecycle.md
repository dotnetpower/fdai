...
window, the first and latest reuse observation and expiry times, and a cumulative reuse count. The protected
...
fields with bounded, read-only PostgreSQL invocation, token, and cost queries. Before retaining the
receipt, the workflow verifies that the same revision, source, image, and replica remain current. The receipt hashes the runtime
