...
| Exact WARA assessment reads | implemented | `delivery/azure/wara_observation.py`; exact evaluator overlay; focused adapter tests | The adapter is a separate shadow assessment source, not a general campaign query. It accepts only an exact reviewed query, ARM resource scope, complete target-visibility receipt, Azure management token target, one total observation deadline, and a bounded deterministic receipt. It does not register itself as a campaign source or grant remediation authority. |
