...
스키마 `1.3.0`은 안정적인 `activity_instance_id`를 전이 멱등성에서 분리하고 등록된 요약 키,
안전한 범위 분류, 선택적 대상 수, 타입이 지정된 결과 수와 단위, 결과 상태, 소스 기준 시각,
선택적 시작 및 완료 시각만 추가합니다. `result_state`는 측정된 0을 기록되지 않았거나 얻을 수
...
`/agents/stream`에 유지됩니다. 스냅샷과 delta는 `activity_instance_id`를 공유하므로 수명 주기
전이는 이력 GET 없이 현재 카드 하나를 갱신합니다. Agent Activity는 운영자가 해당 경로를
명시적으로 열 때만 범위가 제한된 보존 이력을 요청합니다. 모든 출처를 건너뛰는 수동 scheduler
기동은 새 작업 행으로 표시하지 않고 정상 상태를
만들어내는 대신 마지막 커버리지 결과를 보존합니다.

공유 `agent.operational-activity` 계약은 이 캠페인 밖에서 쓰이는 종류도 하나
