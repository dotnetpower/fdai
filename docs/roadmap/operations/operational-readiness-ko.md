...
- **승인과 실행은 구별 유지**: 핸드오프는 submitter 가 요청하고 구별된
  ...
  받지 않으므로 Var approval은 아직 연결되지 않았습니다.
...
- **감사됨**: 현재 service는 ORR verdict, `blocks_handoff`, submitter, 대상 scope,
  environment 및 delivery/assessment failure를 append-only state-store audit entry로 기록합니다.
  Approver identity와 Saga agent attribution은 향후 approval workflow binding에서 추가해야 합니다.
