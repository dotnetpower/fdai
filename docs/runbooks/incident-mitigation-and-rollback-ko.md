...
translation_source_sha: 4815e550c2213790b8151ee3669943aa66c58ca1
...
- **Action contract**: Registered `ActionType`, mode, precondition, stop condition,
  blast radius, rollback contract입니다.
...
| Judge | Required verification 후 typed verdict를 발행합니다. |
| Approver | Human-in-the-loop action을 검토하며 executor와 구분됩니다. |
...
3. Policy, what-if, security, blast-radius check를 실행합니다.
4. Action이 예상된 shadow 또는 enforce mode인지 확인합니다.
...
2. **Verdict를 받습니다.** Registered judge가 verified proposal을 수락할 때만 계속합니다.
...
3. Typed pipeline을 통해 필요한 rollback verdict와 approval을 받습니다.
...
- **Audit**: Proposal, verdict, approval, execution, verification, rollback이 연결됩니다.
