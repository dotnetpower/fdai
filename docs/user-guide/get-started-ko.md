...
translation_source_sha: c4b4d7669309405115c58d79b806dca1b3e47fd3
...
임계값을 넘는 것은 반드시 사람 승인 대기로 넘어갑니다.
...
롤백 경로, 영향 범위 제한, 감사 기록을 지닌 타입 있는 **온톨로지** 엔트리의
인스턴스입니다.
...
규칙과 액션을 로드하지만, 동일한 컨트롤 루프·관측성·감사 로그·안전성 검토를
공유합니다.

### Change Safety

제안된 모든 변경에 대해 규칙 카탈로그 기반 정책 게이트를 적용합니다. 각 후보는
policy-as-code에 대해 dry-run되고 영향 범위가 한정되며, 자동 병합되거나 사람 승인으로
라우팅됩니다.

예시: IaC PR이 public-egress NSG 규칙을 도입 -> 안전성 검토가 고위험으로 판정 ->
Teams에 사람 승인 카드 -> 승인자가 approve 클릭 -> executor가 수정 PR을 병합하고
감사 엔트리를 기록.

### Resilience

예약된 DR 훈련, DB DR 훈련, 영향 범위가 한정된 카오스 실험. 주기·범위·증거는
분리됩니다. 스케줄러가 주기, 안전성 검토가 범위, 감사 로그가 증거를 담당합니다.
...
shadow로 정확도 증명 -> 적용 모드로 승격 -> 롤백 경로를 갖춘 사이즈 최적화 수정
PR이 나갑니다.
...
- **전달 채널**: 액션은 수정 사항을 적용하는 remediation pull request(PR)로 전달되고,
  사람 승인은 Teams 또는 Slack Adaptive Card로 도착합니다. Git은 변경 기록과 롤백
  참조를 제공합니다.
...
안전성 검토는 결과 액션이 자동 실행되는지 승인 대기로 넘어가는지 판단합니다.

1. **T0 (결정론, 목표 커버리지 ~70-80%)**: policy-as-code 결정. 모델 호출 없음, 애매함
   없음.
2. **T1 (경량, ~15-20%)**: 감사 로그 이력 위의 패턴 매칭·임베딩 유사도·소형 모델
   분류기. 저렴하고 빠르며 감사 가능.
3. **T2 (심층 추론, ~5-10%)**: mixed-model 교차 검증·결정론 verifier·근거 확인 검사를
...
4. **shadow 모드에서 관찰합니다.** FDAI가 리소스를 변경하지 않고 판단과 감사만
  수행하도록 합니다. false positive, 사람 승인 결정, verifier 실패, 실행 예정 액션을
  검토합니다.
5. **하나의 액션만 독립적으로 승격합니다.** 고정된 시나리오, 롤백 리허설, 정책 검사가
  promotion gate를 충족한 액션만 적용 모드로 전환합니다. 나머지 액션은 모두 shadow로
  유지합니다.
...
- **Month 1**: 승격된 액션은 롤백 경로와 함께 자율 실행됩니다. discovery loop가 여러분의
  운영 신호(사람 승인, shadow 드리프트, 오버라이드)에서 카탈로그 갱신을 제안하기
  시작합니다.
