...
FDAI는 하나의 영한 YAML 계약을 접근 가능한 SVG, PNG, manifest 및 인터랙티브 viewer
asset으로 컴파일합니다. 새 아키텍처 또는 운영 보기를 작성하기 전에 이 갤러리에서 적합한
다이어그램 유형을 선택하세요.

## 한눈에 보는 설계

컴파일러는 검증된 FDAI kind를 통해 Mermaid 11 다이어그램 유형을 지원합니다. 구조형
다이어그램은 결정론적 graph layout을 사용합니다. 타임라인, 좌표, 방사형, 그리드 및 가중
흐름 다이어그램은 장식용 alias가 아니라 전용 데이터 필드와 layout 전략을 사용합니다.
...
Sankey 방식의 가중 연결선은 상대적 기여도를 보여 주면서 기반 event, read, approval,
mutation 및 audit 의미를 유지합니다.
...
| 다이어그램 컴파일러 작성 계약 | [Architecture Diagram Compiler](../../tools/architecture-diagrams/README.md) |
