...
| Action delivery | Approved action을 remediation pull request 또는 registered provider call로 render합니다. | 모든 action은 typed safety contract와 rollback reference를 유지합니다. |
...
3. **Risk 분류 전 검증**: T2 proposal은 mixed-model agreement, grounding, schema,
   policy, security, what-if check를 통과합니다.
4. **Autonomy ceiling 적용**: Risk gate는 action risk, scope, system health, policy를
...
|-----------------------|-------|------------------------|
| Sense 및 observe | Huginn, Heimdall | Huginn은 normalized event와 real-time resource discovery ingress를 소유합니다. Heimdall은 anomaly, drift, forecast finding을 소유합니다. |
| Judge 및 arbitrate | Forseti, Odin | Forseti가 verdict를 발행합니다. Odin은 Forseti가 verdict를 확정하기 전에 cross-vertical conflict를 해결합니다. |
...
authoritative object type에는 writer가 하나뿐입니다. 예를 들어 여러 agent가 verdict를
...
> 아닙니다. Durable safety binding이 모두 준비될 때까지 enforce mode는 차단됩니다.
...
- **Blast-radius limit**: Action이 영향을 줄 수 있는 maximum scope, batch, concurrency
  또는 rate입니다.
- **Audit record**: Event, verdict, authority, execution, outcome을 재구성하는 데 필요한
  evidence입니다.
...
3. T0가 versioned network policy를 match하고 typed remediation을 propose합니다.
4. What-if가 intended diff를 확인하고 risk gate는 scope에 approval이 필요함을 감지합니다.
5. ChatOps가 rule, evidence, scope, stop condition, rollback reference가 포함된 approval
   card를 보냅니다.
6. Approval 후 executor는 console에서 resource를 mutate하는 대신 remediation pull
   request를 생성합니다.
...
| Forseti를 사용할 수 없음 | New agent verdict가 발행되지 않으며 work는 review를 위해 hold됩니다. |
