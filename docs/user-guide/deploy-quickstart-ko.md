...
translation_source_sha: efd88898233eb56b8e9f8133c14f1759ca0d3f37
...
2. **런타임 상태와 자격 증명 검증.** 내부 코어 프로브가 정상인지, 15개 에이전트가 모두 상태
   ...
   - **Analyzer tick**: `FDAI_INVENTORY_DSN`이 설정되면 Job이 명시적 대상과 영속 인벤토리
     ...
     Application Insights 근거를 조회합니다. 완전한 추적은 발견 사항을 만들지 않고, 누락되거나
     분리된 hop은 shadow 발견 사항을 만듭니다. 빈 값은 연속성 검사만 비활성화합니다.
