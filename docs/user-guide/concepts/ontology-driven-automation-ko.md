...
이 페이지는 온톨로지, 엔트리가 실행 작업이 되는 과정, 그리고 신호에서 감사까지
이어지는 파이프라인을 설명합니다.
...
3. **Gate** - 타입의 티어 상한을 읽어 auto·사람 승인·deny를 판정합니다.
4. **Execute** - preconditions 통과와 per-resource 락 확보 후에만 변경을 적용하며,
   stop-conditions와 영향 범위를 준수합니다.
5. **Deliver** - 수정 PR 또는 직접 API 호출로 변경을 배포합니다.
