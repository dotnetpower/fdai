...
translation_source_sha: 92948f125a43ace5ba9589f934164c1399d79208
...
안전 불변식(stop-condition, 롤백 경로, 영향 범위 제한, 감사 엔트리)이 타입 위에
...
스키마의 나머지는 트리거 종류에 종속되지 않습니다. 안전성 검토와 감사 계약은 두 경로에
...
- 최신 상태가 필요한 인벤토리 그래프가 오래되면 안전성 검토가 deny합니다.
- 안전 불변식이 누락되면 액션은 불완전하므로 실행할 수 없습니다.
- 지원되지 않거나 모호한 결과는 변경 없이 사람 승인으로 보류합니다.
...
계약이 타입 위에 있으므로, 기능을 shadow에서 적용 모드로 승격하는 것은 타입의
