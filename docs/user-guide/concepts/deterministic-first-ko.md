---
title: 결정적 처리 우선(Deterministic first)
description: FDAI가 반복 가능한 다수는 규칙으로 해소하고 애매한 소수만 LLM 추론에 넘기는 이유.
translation_of: deterministic-first.md
translation_source_sha: 9cdf94658a569e7868f836ad50c4832ae09ab13c
translation_revised: 2026-08-20
sidebar:
  order: 2
---

# 결정적 처리 우선(Deterministic first)

**결정적 처리 우선(Deterministic first)**은 FDAI의 중심 설계 원칙입니다. 정책, 규칙,
...
들어오는 모든 이벤트는 케이스를 결정할 수 있는 가장 낮은 티어를 사용하는 **티어
사다리**를 따릅니다. 티어 선택과 실행 권한은 서로 분리되어 있습니다.

![FDAI는 타입이 지정된 컨텍스트를 검증한 다음 T0 결정적 규칙, T1 검증된 재사용 및 T2 근거 기반 추론을 순서대로 시도합니다. 티어가 검증된 결정을 만들 수 없을 때만 다음 티어로 이동합니다. T2 제안은 품질 게이트를 통과하며, 조건을 충족한 모든 티어 결과는 자동 실행, 사람 승인 또는 변경 없음으로 가기 전에 별도의 위험 및 권한 게이트를 통과합니다. 모든 경로는 감사에서 종료됩니다.](../../diagrams/generated/fdai-deterministic-first-01.ko.svg)

> 이 그림은 개념적인 티어 계약을 보여 줍니다. 현재 구현에서 `TrustRouter`는 `T0`,
> `T1` 또는 `abstain`을 반환하며 컨트롤 루프의 대체 경로가 T1을 평가하고 조건에 따라
> T2를 호출합니다. 이러한 구현 분리는 안전 규칙을 바꾸지 않습니다. 명시적으로 판단을
> 보류한 경우에만 다음 티어로 이동하며 어떤 티어도 실행 권한을 부여하지 않습니다.
...
|------|---------------------|-------------------------------------------|
| T0 | 유효한 규칙이나 정책이 모호하지 않은 판정을 생성 | 일치하는 규칙이 없음. 타입이 지정된 컨텍스트가 잘못되었거나 충돌을 해결할 수 없으면 추측하지 않고 검토를 위해 보류 |
...
T0 또는 T1에서 일치 항목이 없어 판단을 보류하면 필요한 입력을 갖춘 다음 티어로 이동할
수 있습니다. 타입이 지정된 컨텍스트가 잘못되었거나 정책 충돌을 해결할 수 없거나 필수
의존성이 없으면 모델 입력으로 바꾸지 않고 검토를 위해 보류합니다. T2에서 판단을 보류하면
자율 변경 없이 사람 승인으로 전달합니다. 예상하지 못한 오류도 같은 안전한 경로를 따르며
감사 추적에 기록됩니다.
