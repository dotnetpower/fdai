...
translation_source_sha: f6066201b6b3ab748f65d52fb13a07698d8ec184
...
| 질문 | 축 | 값 |
|------|----|-----|
| 이 사람은 FDAI에서 무엇을 할 수 있나요? | 접근 권한(RBAC) | 읽기 담당, 기여자, Approver, Owner, Break-Glass |
...
![인수인계 문서는 안전 검사를 거친 수집, 출처를 인용하는 결정적 추출, 선택적 근거 기반 해석, 정확한 디렉터리 신원 확인, 신뢰도 및 담당 범위 경고, 고정된 에이전트 집합에 대한 계약 검증, 멱등한 검토 전용 초안 pull request, 독립된 사람 검토, 병합 후 로더 검증 및 감사를 거칩니다. 미해결, 판단 보류, 거부 또는 잘못된 제안은 활성 담당 체계를 바꾸지 않으며 담당 체계는 접근 권한이나 실행 권한을 부여하지 않습니다.](../../diagrams/generated/fdai-ownership-and-handover-01.ko.svg)

1. **업로드.** 콘솔의 지식 인수인계 양식이나 수집 경로로 문서를 제출합니다. 다른 업로드와 같은
   안전 검사를 거칩니다.
2. **추출.** 결정적 추출기가 줄 단위로 에이전트 영역 용어, 책임 표현, 사람을 찾습니다. 배포
  환경은 선택적 근거 기반 해석기를 연결할 수 있으며 상위 저장소의 기본 구현은 판단을
  보류합니다. 수락된 모든 주장은 원본을 인용하며 근거가 없는 제안은 폐기합니다.
3. **연결.** 문서에 나온 사람을 여러분의 디렉터리에서 조회합니다. FDAI는 신원을 추측하지
   않습니다. 연결되지 않은 이름은 미해결로 표시해 사람이 정리하도록 남깁니다.
4. **초안.** 신뢰도 기준보다 낮은 후보는 사람 검토 대상으로 따로 둡니다. 결과는 미해결 사람,
  보류된 후보 및 신뢰할 수 있는 최종 책임자가 없는 에이전트에 대한 경고를 포함한 비활성 담당
  체계 초안입니다.
5. **검증 및 제안.** 초안은 고정된 15개 에이전트만 사용하고 접근 역할 필드를 포함하지 않으며
  담당 체계 스키마를 충족해야 합니다. 거버넌스 경로가 켜져 있으면 비어 있지 않은 초안을 내용
  기반 식별자를 사용하는 하나의 검토 전용 초안 pull request로 게시합니다. 판단을 보류했거나
  비어 있는 초안은 pull request를 만들지 않으며 같은 문서를 다시 처리해도 중복 요청이 열리지
  않습니다.
6. **병합 및 활성화.** 사람이 pull request를 검토하고 병합할 수 있습니다. 병합 후 로더가 전체
  담당 체계를 검증한 뒤 활성화합니다. 거부되거나 구성이 잘못되면 이전 담당 체계가 그대로
  유지되며 수정 또는 되돌리기를 위한 감사 이력을 보존합니다.
...
콘솔은 읽고 제안하는 표면입니다. 담당 체계 변환 결과를 보여 주고 인수인계 제안을 제출할 수
있지만, 담당 체계 맵을 직접 쓰지는 못합니다. 실제 변경은 모두 검토된 pull 요청을 거칩니다.
...
> 구현되어 있습니다. 격리 멤버십과 승인된 역방향 복구 경로도 소스에 연결되어 있지만 관찰
> 모드에서는 멤버십을 바꾸지 않습니다. 실행에는 별도의 승격, 현재 독립 승인과 안전장치,
> 독립 효과 관찰이 필요합니다. Console은 이 검사를 우회할 수 없습니다.
> [운영 근거는 아직 필요합니다](../guides/validate-ownership-handover-ko.md). 소스 완료는 실제
> 접근 권한 부여나 수동 우회 허가가 아닙니다.
...
- 사람마다 활성 세션은 하나이며 고유한 대화 차례 3개와 연장되지 않는 5분으로 제한합니다.
...
> 영어/한국어 초대와 별도로 검토하는 회수 수명 주기는 구현되어 있습니다. 회수에는 여전히
> 현재 대체 담당 범위와 새 검토가 필요하며, 독립 IAM 제거 후 이전 임무 변경을 검토합니다.
> 지식을 재사용해도 수락이나 승인을 복사하지 않습니다. 실제 회수, 복구, 도입 근거는 이
> 소스 기능과 별도로 필요합니다.
...
| 학습 대상 | 문서 |
|-----------|------|
| 실제 음성과 남은 운영 근거 수집 방법 | [담당자 인수인계 검증](../guides/validate-ownership-handover-ko.md) |
