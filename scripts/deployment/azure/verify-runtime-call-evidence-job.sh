...
[[ "$TF_VAR_core_image" =~ $image_pattern ]] || {
  echo "runtime-call evidence expected image is not one digest-pinned ACR reference" >&2
  exit 2
}
...
image="$(
  az containerapp job show --name "$job" \
    --resource-group "rg-fdai-${TF_VAR_env}-${TF_VAR_region_short}" \
    --query "properties.template.containers[?name=='inventory'].image | [0]" \
    --output tsv --only-show-errors
)"
[[ "$image" == "$TF_VAR_core_image" ]] || {
  echo "runtime-call evidence Inventory Job image does not match the protected digest" >&2
  exit 1
}
