...
request_coordinates="${request_id#plan-model-}"
...
[[ "$proposal_token" =~ ^[0-9a-f]{32}$ \
  ...
  echo "model-binding plan request coordinates are invalid" >&2
  exit 1
}
...
az storage container create \
  --account-name "$STATE_STORAGE_ACCOUNT" \
  --name deployment-plans --auth-mode login --public-access off --only-show-errors \
  --output none
...
[[ -n "$migration_dsn" ]] || {
  echo "model binding proposal database is unavailable" >&2
  exit 1
}
...
PYTHONPATH="$repo_root/packages/service-contracts/src" \
  uv run --directory "$repo_root" --frozen --package fdai-core-control-plane python \
    scripts/deployment/azure/model_binding_proposal.py \
    --from-database \
...
[[ "$policy_digest" == "$expected_policy_digest" ]] || {
  echo "model-binding plan request does not match the proposal policy digest" >&2
  exit 1
}
