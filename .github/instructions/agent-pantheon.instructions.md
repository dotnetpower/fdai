...
| **Saga** | Auditor - append-only chain + handoff-to-GitHub-issue | governance | AuditEntry, Issue | `object.audit-entry`, `object.issue` | all terminal states, including `object.forecast-outcome`, `object.state-snapshot`, and Mimir's catalog-review `object.rule`; `object.handoff-escalation` | no | **yes** |
...
| **Muninn** | Memory - state snapshots + case-history/context index (RAG) | governance | StateSnapshot, ContextIndex | `object.state-snapshot`, `object.context-index` | `object.turn`, `object.audit-entry`, `object.drift` (detection readiness), `object.forecast-outcome`, `object.event` (retention tick), `object.change` | no | no |
