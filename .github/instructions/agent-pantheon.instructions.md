...
| **Bragi** | Narrator - conversational-port translator ONLY | pipeline | Conversation, Turn, UserPreference, HandoffEscalation | `object.conversation`, `object.turn`, `object.user-preference`, `object.handoff-escalation` | (operator console) | yes (translator only) | no |
| **Saga** | Auditor - append-only chain + handoff-to-GitHub-issue | governance | AuditEntry, Issue | `object.audit-entry`, `object.issue` | all terminal states, including `object.forecast-outcome`; `object.handoff-escalation` | no | **yes** |
