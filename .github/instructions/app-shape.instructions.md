...
- An inventory-backed analyzer MUST retain the ontology `Resource.id` in findings, Events, receipts,
  ...
  identity MUST fail the analyzer tick instead of querying with a sanitized logical id. When Cost Governance is available and enabled with an explicit authoritative source binding, the managed full stack MUST also keep its bounded analytics collector alive and include its latest run receipt in readiness; process presence or package enablement is insufficient. A partial or failed refresh keeps the affected evidence facet explicit, but a retained snapshot inside the authoritative freshness threshold MAY keep the process degraded-ready; a missing or stale snapshot remains unready.
