...
- An inventory-backed analyzer MUST retain the ontology `Resource.id` in findings, Events, receipts,
  ...
  identity MUST fail the analyzer tick instead of querying with a sanitized logical id. When Cost Governance is available and enabled with an explicit authoritative source binding, the managed full stack MUST also keep its bounded analytics collector alive and include its latest run receipt in readiness; process presence, package enablement, or retained observations from an older run are insufficient, and a partial, failed, or stale run keeps the affected cost evidence facet unavailable while preserving the last retained snapshot as historical evidence.
