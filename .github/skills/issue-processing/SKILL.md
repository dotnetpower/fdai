...
Every mutating batch claims its frozen issue set with a runtime-derived
`inprogress:<hostname>` label. The helper lowercases and normalizes the hostname, creates the label
when necessary, and fails closed if any selected issue already carries another worker's
`inprogress:*` label. It never removes another worker's claim.

Claim the selected issue numbers immediately after `next`:

```bash
python3 .github/skills/issue-processing/scripts/issue_worker_label.py claim 123 124
```

Release each claim after its lifecycle action and before the final fetch and ledger record:

```bash
python3 .github/skills/issue-processing/scripts/issue_worker_label.py release 123
```

The label is coordination state, not review evidence. Treat the issue state fetched after claim as
the stale-review baseline. Always attempt to release this worker's labels when a batch fails or is
interrupted; leave issues unrecorded when release or final verification fails. Status requests are
read-only and do not create worker labels.

## Batch procedure

1. Run `next --limit 10 --json` once. Freeze those issue numbers for the batch; do not replace a
  difficult issue with a newer one. Claim the complete frozen set with `issue_worker_label.py`;
  abort and release this worker's partial claims if any claim cannot be verified.
...
6. Re-fetch the issue immediately before mutation. Compare it with the post-claim baseline. If its
  title, body, labels other than this worker's claim, linked work, or update time changed after that
  baseline, do not apply a stale verdict; release the claim and leave it unrecorded for a fresh
  review. Otherwise apply the lifecycle action allowed for that verdict.
7. Release this worker's claim, then re-fetch and verify the final body, labels, and state. Record
  the review only after the claim is absent and the final state is confirmed. If GitHub mutation,
  release, or verification fails, leave the issue unrecorded so a later invocation retries it.
...
- GitHub or authentication unavailable: stop the batch, report the exact boundary, and do not
  create terminal review records for unverified issues. Attempt to release this worker's claims
  from every frozen issue before stopping.
- Foreign `inprogress:*` label: do not inspect deeply or mutate that issue. Stop the frozen batch,
  release only this worker's claims, and report the conflicting worker label.
...
.venv/bin/ruff check \
  ...
  .github/skills/issue-processing/scripts/issue_worker_label.py \
