...
- **Validate code before Azure mutation.** Before creating an account, role assignment, or model
  deployment, finish focused checks, commit and push the local code slice, and confirm the required
  CI checks passed for that exact SHA. A local validation-queue receipt is optional diagnostic
  evidence. A standalone environment repair with no code change still targets the current tested
  `HEAD`. Read-only account and inventory checks may run earlier.
