...
def test_every_legacy_table_has_one_migrator_and_one_write_contract() -> None:
    ...
    assert future_tables == {
        ...
        "kubernetes_pod_lifecycle_identity",
    }
...
def test_core_runtime_role_and_forward_grants_cover_only_core_owned_tables() -> None:
    ...
    kubernetes_lifecycle_path = (
        MIGRATION_ROOT
        / "branches/core-control-plane/versions/20260828_core_kubernetes_lifecycle.py"
    )
    kubernetes_lifecycle_migration = inventory_module.load_revision_metadata(
        kubernetes_lifecycle_path
    )
    kubernetes_pod_lifecycle_path = (
        MIGRATION_ROOT / "branches/core-control-plane/versions/"
        "20260830_core_kubernetes_pod_lifecycle_identity.py"
    )
    kubernetes_pod_lifecycle_migration = inventory_module.load_revision_metadata(
        kubernetes_pod_lifecycle_path
    )
    ...
    granted_tables = (
        ...
        | set(kubernetes_pod_lifecycle_migration.owned_tables)
    )
