...
def test_console_publisher_binds_and_verifies_same_origin_manuals() -> None:
    ...
    assert "browser_gateway_operator_url operator_api_fqdn operator-service" in publisher
    assert (
        "browser_gateway_ingestion_url ingestion_gateway_fqdn document-ingestion-api" in publisher
    )
