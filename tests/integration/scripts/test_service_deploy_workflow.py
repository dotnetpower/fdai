...
def test_core_service_tolerates_unapplied_optional_observation_output() -> None:
    ...
    assert "source_identity_client_id" in materialize
    assert "source_identity_resource_id" in materialize
    assert "microsoft.managedidentity/userassignedidentities" in materialize
    assert (
...
def test_console_publish_binds_auth_and_verifies_exact_static_artifact() -> None:
    ...
    assert "browser_gateway_operator_url operator_api_fqdn operator-service" in _CONSOLE_PUBLISHER
    assert (
        "browser_gateway_ingestion_url ingestion_gateway_fqdn document-ingestion-api"
        in _CONSOLE_PUBLISHER
    )
