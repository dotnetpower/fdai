...
def test_validation_reset_clean_rejects_symlinked_state_root(
    git_repo: Path, tmp_path: Path
) -> None:
    paths = queue_paths(git_repo)
    target = tmp_path / "other-state"
    target.mkdir()
    scratch = target / "worktree"
    scratch.mkdir()
    sentinel = scratch / "uncommitted.txt"
    sentinel.write_text("preserve\n", encoding="utf-8")
    paths.state_root.symlink_to(target, target_is_directory=True)
    head = _run(git_repo, "git", "rev-parse", "HEAD").stdout.strip()
