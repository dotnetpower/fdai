...
_REQUEST = (_ROOT / ".github/workflows/request-protected-operation.yml").read_text(encoding="utf-8")
...
def test_catalog_refresh_is_exact_approved_and_postgresql_verified() -> None:
    ...
    assert "astral-sh/setup-uv@11f9893b081a58869d3b5fccaea48c9e9e46f990" in _REFRESH
    assert _REFRESH.index("Install pinned GitHub CLI") < _REFRESH.index("Verify required CI")
    assert "Verify required CI" in _REFRESH
    assert "Verify protected environment approval policy" not in _REFRESH
    assert "environment: ${{ inputs.environment }}" not in _REFRESH
...
def test_catalog_refresh_request_is_bot_owned() -> None:
    ...
    assert "catalog-refresh)" in _REQUEST
    assert "target_workflow=refresh-catalogs.yml" in _REQUEST
    assert '"repos/$GITHUB_REPOSITORY/actions/workflows/$target_workflow/dispatches"' in _REQUEST
