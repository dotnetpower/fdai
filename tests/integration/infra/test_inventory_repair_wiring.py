...
def test_runtime_call_source_survives_operator_service_state_migration() -> None:
    ...
    assert "plan-runtime-call-" in workflow
    assert "apply-runtime-call-" in workflow
    assert "RUNTIME_CALL_EVIDENCE_TRANSITION:" in workflow
    target_binding = next(line for line in workflow.splitlines() if "TF_CLI_ARGS_plan:" in line)
    assert "plan-runtime-call-" in target_binding
    assert "-target=module.compute.azurerm_container_app_job.inventory[0]" in target_binding
