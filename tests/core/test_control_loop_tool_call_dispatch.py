...
- When an ActionType declares ``tool_call`` but its executor is not wired,
    ...
    return Action.model_validate(
        {
            ...
            "stop_conditions": [{"kind": "time_box_exceeded_seconds", "seconds": 120}],
...
async def test_declared_tool_call_without_executor_fails_closed() -> None:
    pr_exec = MagicMock()
    pr_exec.execute = AsyncMock()

    action_type = _action_type(execution_path=ExecutionPath.TOOL_CALL)
    loop = _make_loop(
        pr_executor=pr_exec,
        tool_executor=None,
        action_types_by_name={"tool.generate-pdf": action_type},
    )
    got = await loop._dispatch_action(action=_action(), rule=_rule())

    assert got.outcome is ExecutorOutcome.REJECTED_INVARIANT
    assert got.reason == "execution_path 'tool_call' has no wired executor"
...
async def test_tool_call_action_without_executor_logs_fail_closed_reason(caplog) -> None:
    """A missing tool executor is observable without changing dispatch paths."""
    import logging
    ...
    assert got.outcome is ExecutorOutcome.REJECTED_INVARIANT
    pr_exec.execute.assert_not_called()
    assert any(r.message == "action_dispatch_executor_unavailable" for r in caplog.records)
