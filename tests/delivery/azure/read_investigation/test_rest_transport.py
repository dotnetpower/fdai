...
async def test_arg_retry_refreshes_reader_token() -> None:
    authorizations: list[str] = []

    class _RotatingIdentity:
        def __init__(self) -> None:
            self.calls = 0
    ...
    assert result == []
    assert authorizations == ["Bearer attempt-1", "Bearer attempt-2"]
