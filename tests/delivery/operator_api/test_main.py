"""Tests for :mod:`fdai.delivery.operator_api.main`.
...
def _build_stack(
    *,
    ...
    if verifier_override is not None:
        ...
        from fdai.delivery.operator_api.auth import UnsafeClaimsExtractor

        verifier = UnsafeClaimsExtractor()
    authenticator = build_authenticator(verifier=verifier, resolver=resolver)
    read_model = InMemoryConsoleReadModel()
    config = OperatorApiConfig(dev_mode=dev_mode, extra_panels=extra_panels)
...
class TestAuthenticationGate:
    ...
    def test_authentication_failure_is_logged_without_credentials(
        ...
        del no_dev_env
        app, _ = _build_stack(dev_mode=False)
        caplog.set_level(logging.WARNING, logger="fdai.delivery.operator_api.app.factory")
        ...
        assert response.status_code == 401
        record = next(
            item for item in caplog.records if item.message == "operator_api_authentication_failed"
        )
    ...
    def test_authorization_failure_is_logged_without_identity(
        ...
        assert response.status_code == 403
        record = next(
            item for item in caplog.records if item.message == "operator_api_authorization_failed"
        )
        assert record.path == "/audit"  # type: ignore[attr-defined]
    ...
    def test_verifier_error_wrapped_to_401(self, no_dev_env: None) -> None:
        ...
        app = build_app(
            authenticator=authenticator,
            read_model=InMemoryConsoleReadModel(),
            config=OperatorApiConfig(dev_mode=False),
        )
        client = TestClient(app)
...
def _build_with_panels(
    *panels: Any, dev_mode: bool = True
    ...
    authenticator = build_authenticator(verifier=verifier, resolver=resolver)
    read_model = InMemoryConsoleReadModel()
    config = OperatorApiConfig(dev_mode=dev_mode, extra_panels=tuple(panels))
    app = build_app(authenticator=authenticator, read_model=read_model, config=config)
    return app, read_model
...
class TestExtensionPanels:
    ...
    def test_panel_requires_reader_role(self, no_dev_env: None) -> None:
        ...
        app = build_app(
            authenticator=authenticator,
            read_model=InMemoryConsoleReadModel(),
            config=OperatorApiConfig(dev_mode=False, extra_panels=(_FakePanel("/finops"),)),
        )
        client = TestClient(app)
