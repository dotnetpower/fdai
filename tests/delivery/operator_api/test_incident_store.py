...
from fdai.delivery.operator_api.dev.fixtures.seed_data import _seed
...
async def test_local_sample_audit_does_not_create_incident_roster_entries() -> None:
    ...
    assert audit.items
    assert all(item.entry.get("fixture_source") == "operator-api-dev-seed" for item in audit.items)
