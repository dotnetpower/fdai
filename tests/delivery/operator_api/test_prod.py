"""Unit tests for :mod:`fdai.delivery.operator_api.prod`.
...
def test_plain_dsn_rejects_asyncpg_driver_suffix() -> None:
    with pytest.raises(ProdOperatorApiConfigError, match=r"\+asyncpg"):
        _plain_dsn("postgresql+asyncpg://u:p@h/db")


def test_plain_dsn_rejects_psycopg2_driver_suffix() -> None:
    with pytest.raises(ProdOperatorApiConfigError, match=r"\+psycopg2"):
        _plain_dsn("postgresql+psycopg2://u:p@h/db")


def test_plain_dsn_rejects_non_postgres_scheme() -> None:
    with pytest.raises(ProdOperatorApiConfigError, match="different scheme"):
...
def test_parse_cors_origins_rejects_wildcard_element() -> None:
    with pytest.raises(ProdOperatorApiConfigError, match="'\\*'"):
        _parse_cors_origins("*")
...
def test_parse_positive_int_rejects_non_int() -> None:
    with pytest.raises(ProdOperatorApiConfigError, match="K"):
        _parse_positive_int({"K": "seven"}, "K", 42)


def test_parse_positive_int_rejects_non_positive() -> None:
    with pytest.raises(ProdOperatorApiConfigError, match=">= 1"):
...
def test_build_prod_read_model_requires_database_url() -> None:
    ...
    with pytest.raises(ProdOperatorApiConfigError, match="FDAI_DATABASE_URL"):
...
def test_build_prod_read_model_honors_timeout_overrides() -> None:
    env = dict(_GOOD_ENV)
    env["FDAI_OPERATOR_API_STATEMENT_TIMEOUT_MS"] = "5000"
    env["FDAI_OPERATOR_API_CONNECT_TIMEOUT_S"] = "3"
...
def test_build_prod_app_rejects_unimplemented_identity_provider() -> None:
    ...
    with pytest.raises(ProdOperatorApiConfigError, match="not implemented"):
        build_prod_app(env)
...
def test_build_prod_app_rejects_partial_onboarding_probe_config() -> None:
    ...
    with pytest.raises(ProdOperatorApiConfigError, match="onboarding probe configuration"):
        build_prod_app(env)
...
def test_build_prod_app_rejects_malformed_incident_sla_policy(
    ...
    with pytest.raises(ProdOperatorApiConfigError, match="FDAI_INCIDENT_SLA_POLICY_JSON"):
        build_prod_app(env)
...
def test_build_prod_app_opts_into_hil_callback_without_executor_identity(
    ...
    # The production Operator API publishes a decision event; it never receives
...
def test_build_prod_app_requires_every_rbac_slot(missing: str) -> None:
    env = dict(_GOOD_ENV)
    del env[missing]
    with pytest.raises(ProdOperatorApiConfigError, match=missing):
        build_prod_app(env)


def test_build_prod_app_wildcard_cors_refused_in_prod(monkeypatch: pytest.MonkeyPatch) -> None:
    env = dict(_GOOD_ENV)
    env["FDAI_OPERATOR_API_CORS_ALLOW_ORIGINS"] = "*"
...
def test_build_prod_app_never_boots_in_dev_mode(monkeypatch: pytest.MonkeyPatch) -> None:
    # The prod factory never sets FDAI_OPERATOR_API_DEV_MODE; even if the env
    # carries it, build_app refuses because OperatorApiConfig.dev_mode is False
...
def test_build_prod_app_reports_every_missing_env_in_one_error() -> None:
    ...
    with pytest.raises(ProdOperatorApiConfigError) as excinfo:
        build_prod_app({})
