...
from fdai.delivery.operator_api.routes.demo_inventory_graph import demo_inventory_graph_provider
...
def _client(*, wired: bool) -> TestClient:
    ...
    app = build_app(
        ...
        config=OperatorApiConfig(
            dev_mode=True,
            inventory_graph_provider=_provider if wired else None,
        ),
    )
...
def _client_with_provider(provider: Any) -> TestClient:
    ...
    app = build_app(
        authenticator=auth,
        read_model=InMemoryConsoleReadModel(),
        config=OperatorApiConfig(dev_mode=True, inventory_graph_provider=provider),
    )
