"""Tests for the dev-mode entrypoint at :mod:`fdai.delivery.operator_api.dev.local`."""
...
from fdai.delivery.operator_api.dev import factory as _local_factory
from fdai.delivery.operator_api.dev import local as _local
from fdai.delivery.operator_api.dev.azure_cli_identity import LocalAzureCliIdentity
from fdai.delivery.operator_api.dev.config import (
    entra_application_id_from_env,
    local_entra_verifier_environment,
)
from fdai.delivery.operator_api.dev.runtime_wiring import build_interactive_pantheon_wiring
...
_AUTHORITATIVE_OPERATOR_API_ENV = "FDAI_AUTHORITATIVE_OPERATOR_API_BASE_URL"
...
def test_full_stack_launch_uses_entra_rbac_without_fixture_or_cli_principal() -> None:
    ...
    api_env = configs["Console Web: Operator API"]["env"]
    ...
    assert api_env["FDAI_OPERATOR_API_LOCAL_ENTRA"] == "1"
    assert api_env["FDAI_OPERATOR_API_DEV_MODE"] == "0"
    assert api_env["FDAI_OPERATOR_API_LOCAL_AZURE_CLI"] == "0"
    ...
    core_task = tasks.split(
        '"label": "console: core runtime (Azure transport, local PostgreSQL)"',
        1,
    )[1].split('"label": "console: Operator API (Local Entra)"', 1)[0]
    operator_api_task = tasks.split('"label": "console: Operator API (Local Entra)"', 1)[1].split(
    ...
    assert '"dependsOn": "console: prepare full stack"' in operator_api_task
    assert '"runOn": "folderOpen"' in operator_api_task
    assert "preLaunchTask" not in configs["Console Web: Core Runtime"]
    assert "preLaunchTask" not in configs["Console Web: Operator API"]
    assert configs["Console Web: Operator API"]["envFile"].endswith("/.fdai/local-runtime.env")
...
class TestLocalEntrypoint:
    def test_maps_local_vite_msal_settings_to_operator_api_contract(self) -> None:
    ...
    def test_refuses_without_interactive_auth_mode(self, monkeypatch: pytest.MonkeyPatch) -> None:
        ...
        with pytest.raises(RuntimeError, match="interactive local Operator API"):
            _local.app()
    ...
    def test_custom_console_origin_is_allowed(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv(_DEV_ENV, "1")
        monkeypatch.setenv("FDAI_OPERATOR_API_CORS_ALLOW_ORIGINS", "http://127.0.0.1:5178")
        client = TestClient(_local.app(test_fixtures=True))
    ...
    def test_vite_preview_origin_is_allowed_by_default(
        ...
        monkeypatch.setenv(_DEV_ENV, "1")
        monkeypatch.delenv("FDAI_OPERATOR_API_CORS_ALLOW_ORIGINS", raising=False)
        client = TestClient(_local.app(test_fixtures=True))
    ...
    def test_custom_console_origin_rejects_wildcard(self, monkeypatch: pytest.MonkeyPatch) -> None:
        monkeypatch.setenv(_DEV_ENV, "1")
        monkeypatch.setenv("FDAI_OPERATOR_API_CORS_ALLOW_ORIGINS", "*")
...
class TestLocalEntraLoginHarness:
    ...
    def test_full_stack_core_owns_pantheon_while_operator_api_relays_streams(
        self, monkeypatch: pytest.MonkeyPatch
    ) -> None:
        self._enable(monkeypatch)
        monkeypatch.delenv(_START_PANTHEON_ENV, raising=False)
...
class TestLocalAzureCliHarness:
    ...
    def test_authoritative_proxy_is_declared_without_synthetic_fallback(
        ...
        monkeypatch.setenv(
            _AUTHORITATIVE_OPERATOR_API_ENV,
        ...
        assert sources["operational-state"]["source"] == "remote-operator-api"
    ...
    def test_rejects_local_postgresql_with_authoritative_proxy(
        ...
        monkeypatch.setenv(_AUTHORITATIVE_OPERATOR_API_ENV, "https://read.example.test")
