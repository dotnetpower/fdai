"""Operator API auth wire - Bearer extraction + verifier + RBAC glue."""
...
class TestAuthenticateHappyPath:
    ...
    def test_empty_roles_still_returns_principal_deny_is_403_later(self) -> None:
        ...
        # Operator API layer distinguishes 401 (auth) from 403 (rbac).
        auth = Authenticator(
...
class TestAuthenticateFailurePaths:
    ...
    def test_missing_oid_wrapped_as_authentication_error(self) -> None:
        ...
        # Operator API layer treats it as 401 (bad token) not 500.
        auth = Authenticator(
