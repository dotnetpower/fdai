"""Structural drift guards for the G-5 operator_api split.

The delivery operator_api (9,696 LOC) was grouped into routes/, streaming/,
...
def test_top_level_operator_api_is_only_allowed_files() -> None:
    ...
    assert not extras, (
        f"Top-level operator_api/*.py must be limited to "
...
_DEV_IMPORT = re.compile(r"(?:from|import)\s+fdai\.delivery\.operator_api\.dev(?:\.|$|\s)")
...
def test_routes_and_streaming_are_populated() -> None:
    routes = list((_OPERATOR_API_DIR / "routes").glob("*.py"))
    ...
    streams = list((_OPERATOR_API_DIR / "streaming").glob("*.py"))
...
_ROUTE_IMPORT = re.compile(
    r"(?:from|import)\s+fdai\.delivery\.operator_api\.routes\.[a-zA-Z_][a-zA-Z0-9_]*"
)


def test_no_external_caller_reaches_into_routes() -> None:
    # External = outside src/fdai/delivery/operator_api/ and outside tests/.
...
def test_main_stays_slim_composition_root() -> None:
    ...
    assert loc < 1200, (
        f"operator_api/main.py has grown to {loc} LOC (> 1200). Extract "
